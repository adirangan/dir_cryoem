...
if ~exist('a_R_k_p_gpu_Rqk__','var'); a_R_k_p_gpu_Rqk__=[]; end;
if ~exist('ba_from_single_shell_gpu_Rbaba___','var'); ba_from_single_shell_gpu_Rbaba___=[]; end;
if ~exist('wT_from_R_single_shell_gpu_Rsba___','var'); wT_from_R_single_shell_gpu_Rsba___=[]; end;
if ~exist('dwTda_from_R_single_shell_gpu_Rsba___','var'); dwTda_from_R_single_shell_gpu_Rsba___=[]; end;
if ~exist('dwTdb_from_R_single_shell_gpu_Rsba___','var'); dwTdb_from_R_single_shell_gpu_Rsba___=[]; end;
if ~exist('ddwTdaa_from_R_single_shell_gpu_Rsba___','var'); ddwTdaa_from_R_single_shell_gpu_Rsba___=[]; end;
if ~exist('ddwTdab_from_R_single_shell_gpu_Rsba___','var'); ddwTdab_from_R_single_shell_gpu_Rsba___=[]; end;
if ~exist('ddwTdbb_from_R_single_shell_gpu_Rsba___','var'); ddwTdbb_from_R_single_shell_gpu_Rsba___=[]; end;
%}
function ...
[ ...
 parameter ...
...
,nR_from_nT_ ...
,a_R_k_p_gpu_Rqk__ ...
,ba_from_single_shell_gpu_Rbaba___ ...
,wT_from_R_single_shell_gpu_Rsba___ ...
...
,dwTda_from_R_single_shell_gpu_Rsba___ ...
,dwTdb_from_R_single_shell_gpu_Rsba___ ...
...
,ddwTdaa_from_R_single_shell_gpu_Rsba___ ...
,ddwTdab_from_R_single_shell_gpu_Rsba___ ...
,ddwTdbb_from_R_single_shell_gpu_Rsba___ ...
] = ...
interpolate_template_gpu_8( ...
 parameter ...
,n_order ...
,n_qk ...
,n_qk_csum_ ...
...
str_thisfunction = 'interpolate_template_gpu_8';

if nargin<1;
rng(0);
flag_verbose = 2; nf=1;
if (flag_verbose); disp(sprintf(' %% testing %s',str_thisfunction)); end;
test_interpolate_template_gpu_8;
...
if (nargin<1+na); a_R_k_p_gpu_Rqk__=[]; end; na=na+1;
if (nargin<1+na); ba_from_single_shell_gpu_Rbaba___=[]; end; na=na+1;
if (nargin<1+na); wT_from_R_single_shell_gpu_Rsba___=[]; end; na=na+1;
if (nargin<1+na); dwTda_from_R_single_shell_gpu_Rsba___=[]; end; na=na+1;
if (nargin<1+na); dwTdb_from_R_single_shell_gpu_Rsba___=[]; end; na=na+1;
if (nargin<1+na); ddwTdaa_from_R_single_shell_gpu_Rsba___=[]; end; na=na+1;
if (nargin<1+na); ddwTdab_from_R_single_shell_gpu_Rsba___=[]; end; na=na+1;
if (nargin<1+na); ddwTdbb_from_R_single_shell_gpu_Rsba___=[]; end; na=na+1;
...
if ~strcmp(class(a_k_p_gpu_qk_),'gpuArray'); tmp_t = tic(); a_k_p_gpu_qk_ = gpuArray( (a_k_p_gpu_qk_)); tmp_t = toc(tmp_t); if (flag_verbose>0); disp(sprintf(' %% a_k_p_gpu_qk_ %0.6fs',tmp_t)); end; end;
...
%%%%%%%%;
if flag_dd;
%ddtemplatedba_ori_gpu_wkS__ = ddtemplatedab_ori_gpu_wkS__; %<-- not explicitly referenced. ;
%ddtemplatedca_ori_gpu_wkS__ = ddtemplatedac_ori_gpu_wkS__; %<-- not explicitly referenced. ;
%ddtemplatedcb_ori_gpu_wkS__ = ddtemplatedbc_ori_gpu_wkS__; %<-- not explicitly referenced. ;
end;%if flag_dd;
%%%%%%%%;
...
if isempty(a_R_k_p_gpu_Rqk__); a_R_k_p_gpu_Rqk__ = cell(n_R,1); end;
if isempty(ba_from_single_shell_gpu_Rbaba___); ba_from_single_shell_gpu_Rbaba___ = cell(n_R,1); end;
if isempty(wT_from_R_single_shell_gpu_Rsba___); wT_from_R_single_shell_gpu_Rsba___ = cell(n_R,1); end;
if isempty(dwTda_from_R_single_shell_gpu_Rsba___); dwTda_from_R_single_shell_gpu_Rsba___ = cell(n_R,1); end;
if isempty(dwTdb_from_R_single_shell_gpu_Rsba___); dwTdb_from_R_single_shell_gpu_Rsba___ = cell(n_R,1); end;
if isempty(ddwTdaa_from_R_single_shell_gpu_Rsba___); ddwTdaa_from_R_single_shell_gpu_Rsba___ = cell(n_R,1); end;
if isempty(ddwTdab_from_R_single_shell_gpu_Rsba___); ddwTdab_from_R_single_shell_gpu_Rsba___ = cell(n_R,1); end;
if isempty(ddwTdbb_from_R_single_shell_gpu_Rsba___); ddwTdbb_from_R_single_shell_gpu_Rsba___ = cell(n_R,1); end;
viewing_R_polar_a_SR__ = zeros(n_S,n_R);
viewing_R_azimu_b_SR__ = zeros(n_S,n_R);
viewing_R_gamma_z_SR__ = zeros(n_S,n_R);
template_rec_gpu_RwkT___ = cell(n_R,1);
if flag_d;
dtemplateda_rec_gpu_RwkT___ = cell(n_R,1);
dtemplatedb_rec_gpu_RwkT___ = cell(n_R,1);
dtemplatedc_rec_gpu_RwkT___ = cell(n_R,1);
end;%if flag_d;
if flag_dd;
ddtemplatedaa_rec_gpu_RwkT___ = cell(n_R,1);
ddtemplatedab_rec_gpu_RwkT___ = cell(n_R,1);
ddtemplatedac_rec_gpu_RwkT___ = cell(n_R,1);
ddtemplatedbb_rec_gpu_RwkT___ = cell(n_R,1);
ddtemplatedbc_rec_gpu_RwkT___ = cell(n_R,1);
ddtemplatedcc_rec_gpu_RwkT___ = cell(n_R,1);
end;%if flag_dd;
%%%%%%%%%%%%%%%%;
for nR=0:n_R-1;
%%%%%%%%%%%%%%%%;
if (flag_verbose>0); disp(sprintf(' %% nR %d/%d calling interpolate_template_gpu_R_1.',nR,n_R)); end;
R_use__ = R_use_R___{1+nR};
a_R_k_p_gpu_qk_ = a_R_k_p_gpu_Rqk__{1+nR};
ba_from_single_shell_gpu_baba__ = ba_from_single_shell_gpu_Rbaba___{1+nR};
wT_from_R_single_shell_gpu_sba__ = wT_from_R_single_shell_gpu_Rsba___{1+nR};
if flag_d;
dwTda_from_R_single_shell_gpu_sba__ = dwTda_from_R_single_shell_gpu_Rsba___{1+nR};
dwTdb_from_R_single_shell_gpu_sba__ = dwTdb_from_R_single_shell_gpu_Rsba___{1+nR};
end;%if flag_d;
if flag_dd;
ddwTdaa_from_R_single_shell_gpu_sba__ = ddwTdaa_from_R_single_shell_gpu_Rsba___{1+nR};
ddwTdab_from_R_single_shell_gpu_sba__ = ddwTdab_from_R_single_shell_gpu_Rsba___{1+nR};
ddwTdbb_from_R_single_shell_gpu_sba__ = ddwTdbb_from_R_single_shell_gpu_Rsba___{1+nR};
end;%if flag_dd;
%%%%%%%%;
if  flag_1 & ~flag_d & ~flag_dd;
tmp_t = tic();
...
tmp_t = toc(tmp_t); if (flag_verbose>0); disp(sprintf(' %% interpolate_template_gpu_R_1: time %0.6fs',tmp_t)); end;
end;%if  flag_1 & ~flag_d & ~flag_dd;
%%%%%%%%;
if  flag_1 &  flag_d & ~flag_dd;
tmp_t = tic();
...
a_R_k_p_gpu_Rqk__{1+nR} = a_R_k_p_gpu_qk_;
ba_from_single_shell_gpu_Rbaba___{1+nR} = ba_from_single_shell_gpu_baba__;
...
wT_from_R_single_shell_gpu_Rsba___{1+nR} = wT_from_R_single_shell_gpu_sba__;
if flag_d;
dwTda_from_R_single_shell_gpu_Rsba___{1+nR} = dwTda_from_R_single_shell_gpu_sba__;
dwTdb_from_R_single_shell_gpu_Rsba___{1+nR} = dwTdb_from_R_single_shell_gpu_sba__;
end;%if flag_d;
if flag_dd;
ddwTdaa_from_R_single_shell_gpu_Rsba___{1+nR} = ddwTdaa_from_R_single_shell_gpu_sba__;
ddwTdab_from_R_single_shell_gpu_Rsba___{1+nR} = ddwTdab_from_R_single_shell_gpu_sba__;
ddwTdbb_from_R_single_shell_gpu_Rsba___{1+nR} = ddwTdbb_from_R_single_shell_gpu_sba__;
end;%if flag_dd;
template_rec_gpu_RwkT___{1+nR} = template_rec_gpu_wkT__;
if flag_d;
dtemplateda_rec_gpu_RwkT___{1+nR} = dtemplateda_rec_gpu_wkT__;
dtemplatedb_rec_gpu_RwkT___{1+nR} = dtemplatedb_rec_gpu_wkT__;
dtemplatedc_rec_gpu_RwkT___{1+nR} = dtemplatedc_rec_gpu_wkT__;
end;%if flag_d;
if flag_dd;
ddtemplatedaa_rec_gpu_RwkT___{1+nR} = ddtemplatedaa_rec_gpu_wkT__;
ddtemplatedab_rec_gpu_RwkT___{1+nR} = ddtemplatedab_rec_gpu_wkT__;
ddtemplatedac_rec_gpu_RwkT___{1+nR} = ddtemplatedac_rec_gpu_wkT__;
ddtemplatedbb_rec_gpu_RwkT___{1+nR} = ddtemplatedbb_rec_gpu_wkT__;
ddtemplatedbc_rec_gpu_RwkT___{1+nR} = ddtemplatedbc_rec_gpu_wkT__;
ddtemplatedcc_rec_gpu_RwkT___{1+nR} = ddtemplatedcc_rec_gpu_wkT__;
end;%if flag_dd;
%%%%%%%%%%%%%%%%;
end;%for nR=0:n_R-1;
%%%%%%%%%%%%%%%%;
...
%%%%%%%%;
if flag_1;
if (flag_verbose>0); disp(sprintf(' %% Overwriting template_ori_gpu_wkS__.')); end;
for nT=0:n_T-1;
nS = index_nS_from_nT_(1+nT);
nR = nR_from_nT_(1+nT);
template_ori_gpu_wkS__(:,1+nS) = template_rec_gpu_RwkT___{1+nR}(:,1+nT);
end;%for nT=0:n_T-1;
end;%if flag_1;
%%%%%%%%;
if flag_d;
if (flag_verbose>0); disp(sprintf(' %% Overwriting dtemplateda_ori_gpu_wkS__.')); end;
for nT=0:n_T-1;
nS = index_nS_from_nT_(1+nT);
nR = nR_from_nT_(1+nT);
dtemplateda_ori_gpu_wkS__(:,1+nS) = dtemplateda_rec_gpu_RwkT___{1+nR}(:,1+nT);
dtemplatedb_ori_gpu_wkS__(:,1+nS) = dtemplatedb_rec_gpu_RwkT___{1+nR}(:,1+nT);
dtemplatedc_ori_gpu_wkS__(:,1+nS) = dtemplatedc_rec_gpu_RwkT___{1+nR}(:,1+nT);
end;%for nT=0:n_T-1;
end;%if flag_d;
%%%%%%%%;
if flag_dd;
if (flag_verbose>0); disp(sprintf(' %% Overwriting ddtemplatedaa_ori_gpu_wkS__.')); end;
for nT=0:n_T-1;
nS = index_nS_from_nT_(1+nT);
nR = nR_from_nT_(1+nT);
ddtemplatedaa_ori_gpu_wkS__(:,1+nS) = ddtemplatedaa_rec_gpu_RwkT___{1+nR}(:,1+nT);
ddtemplatedab_ori_gpu_wkS__(:,1+nS) = ddtemplatedab_rec_gpu_RwkT___{1+nR}(:,1+nT);
ddtemplatedac_ori_gpu_wkS__(:,1+nS) = ddtemplatedac_rec_gpu_RwkT___{1+nR}(:,1+nT);
ddtemplatedbb_ori_gpu_wkS__(:,1+nS) = ddtemplatedbb_rec_gpu_RwkT___{1+nR}(:,1+nT);
ddtemplatedbc_ori_gpu_wkS__(:,1+nS) = ddtemplatedbc_rec_gpu_RwkT___{1+nR}(:,1+nT);
ddtemplatedcc_ori_gpu_wkS__(:,1+nS) = ddtemplatedcc_rec_gpu_RwkT___{1+nR}(:,1+nT);
end;%for nT=0:n_T-1;
end;%if flag_dd;
%%%%%%%%;
