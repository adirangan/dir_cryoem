...
if flag_check;
%%%%%%%%;
% test fourier-transform on chebychev grid. ;
%%%%%%%%;
...
% Q_{mu_r,nu}(mu_i) = 2.^{mu}.a.^{1+mu} .frac{\Gamma(0.5(1+nu+mu))}{\Gamma(0.5(1+nu-mu))} ;
% with -Re(nu)-1 < mu_r < 1/2 ;
% J_{nu}(z) \approx (z/2)^{nu} * (1/Gamma(1+nu) - z^{2}/Gamma(2+nu)) * exp(-\beta z^{4}) ;
...
n_gamma_z = 1024+1; gamma_z_max = 50*1;
%for n_gamma_z = 1024*[1,2,4,8]; for gamma_z_max = 50*[1,2,3];
...
% Compare with fht_1. ;
%%%%%%%%;
n_gamma_z = 1*1024/4; gamma_z_max = n_x_p_r/2 ;
n_gamma_z = 1*1024; gamma_z_max = 50+1;
flag_sign = -1;
[ ...
 ~ ...
,L_k_q_k_ ...
...
fnorm_disp(flag_verbose,'tmp_fA_form_z_',tmp_fA_form_z_,'tmp_fA_form_z_',tmp_fA_form_z_,'%<-- should be zero');
fnorm_disp(flag_verbose,'O_k_q_k_',O_k_q_k_,'L_k_q_k_',L_k_q_k_,'%<-- should be zero.');
fnorm_disp(flag_verbose,'O_k_q_k_',O_k_q_k_,'M_k_q_k_',M_k_q_k_,'%<-- should be small.');
fnorm_disp(flag_verbose,'L_k_q_k_',L_k_q_k_,'M_k_q_k_',M_k_q_k_,'%<-- should be small.');
%%%%%%%%;
flag_sign = +1;
n_theta_z = 1*1024; theta_z_max = n_k_p_r/2 ;
[ ...
 ~ ...
,L_x_q_x_ ...
,n_theta_z ...
...
k_eq_d = k_eq_d_double/(2*pi)/ 2 * 1.00; %<-- to check dimensions. ;
%%%%%%%%;
[ ...
 n_x_p_r ...
,x_p_r_ ...
...
tmp_t = toc(tmp_t); if (flag_verbose); disp(sprintf(' %% fht_1: time %0.6fs',tmp_t)); end;
tmp_t = tic();
[ ...
 ~ ...
,M_k_q_k_sht1_ ...
...
tmp_t = toc(tmp_t); if (flag_verbose); disp(sprintf(' %% sht_1: time %0.6fs',tmp_t)); end;
fnorm_disp(flag_verbose,'M_k_q_k_sht1_',M_k_q_k_sht1_,'M_k_q_k_fht1_',M_k_q_k_fht1_,'%<-- should be small.');
fnorm_disp(flag_verbose,'M_x_q_x_orig_',M_x_q_x_orig_,'M_x_q_x_sht1_',M_x_q_x_sht1_,'%<-- should be small.');
fnorm_disp(flag_verbose,'M_x_q_x_orig_',M_x_q_x_orig_,'M_x_q_x_fht1_',M_x_q_x_fht1_,'%<-- should be small.');
%%%%;
if flag_disp>0;
...
subplot(1,2,1);
plot(x_p_r_,real(M_x_q_x_orig_),'kx',x_p_r_,real(M_x_q_x_sht1_),'go',x_p_r_,real(M_x_q_x_fht1_),'ro');
xlim([0,x_p_r_max]); ylim(M_x_lim_);
subplot(1,2,2);
plot(k_p_r_,real(M_k_q_k_form_),'kx',k_p_r_,real(M_k_q_k_sht1_),'go',k_p_r_,real(M_k_q_k_fht1_),'ro');
