...
disp(sprintf(' %% testing tfpmh_Z_wSM___13: flag_speed_vs_error %d',flag_speed_vs_error));
n_1 = 1; n_2 = 2;
...
%%%%;
n_CTF = n_M;
CTF_phi_M_ = reshape(linspace(1*pi/3,2*pi/3,n_M),[n_M,1]);
CTF_k_p_wkM__ = zeros(n_w_sum,n_M);
for nM=0:n_M-1;
CTF_phi = CTF_phi_M_(1+nM);
CTF_k_p_wkM__(:,1+nM) = 2*k_p_r_wk_.*cos(k_p_w_wk_ - CTF_phi);
end;%for nM=0:n_M-1;
%%%%;
[ ...
 X_kk__ ...
...
if flag_speed_vs_error==0; pm_n_UX_rank = n_k_p_r-1; end;
...
tmp_t = toc(tmp_t); if (flag_verbose>0); disp(sprintf(' %% tfpmh_Z_wSM___13: time %0.6fs',tmp_t)); end;
for ij=2:size(parameter.timing,1); disp(sprintf(' %% %s: %0.6fs',parameter.timing{ij,1},parameter.timing{ij,2})); end;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
...
parameter=struct('type','parameter');
parameter.flag_verbose = 1;
if ntype==0; parameter.flag_optimize_over_gamma_z = 0; parameter.flag_CTF_anisotropic = 1; end;
if ntype==1; parameter.flag_optimize_over_gamma_z = 0; parameter.flag_CTF_anisotropic = 0; end;
if ntype==2; parameter.flag_optimize_over_gamma_z = 1; parameter.flag_CTF_anisotropic = 1; end;
if ntype==3; parameter.flag_optimize_over_gamma_z = 1; parameter.flag_CTF_anisotropic = 0; end;
...
CTF_k_p_wk_ = CTF_k_p_wkM__(:,1+nM);
gamma_z = (2*pi*nw)/max(1,n_w_max);
RS_k_p_wk_ = rotate_p_to_p_fftw(n_k_p_r,n_w_,n_w_sum,S_k_p_wk_,+gamma_z);
UX_RS_k_p_wn_ = reshape(reshape(RS_k_p_wk_,[n_w_max,n_k_p_r])*pm_wUX_kn__,[pm_n_w_sum,1]);
...
UX_CTF_TM_k_p_wn_ = reshape(reshape(CTF_k_p_wk_.*TM_k_p_wk_,[n_w_max,n_k_p_r])*pm_wUX_kn__,[pm_n_w_sum,1]);
...
UX_RS_k_p_UX_CTF_TM_k_p = sum(conj(UX_RS_k_p_wn_).*UX_CTF_TM_k_p_wn_) / max(1,pm_n_w_max) ;
%Z_quad = UX_RS_k_p_UX_CTF_TM_k_p; %<-- note: this is what is actually being calculated. ;
Z_quad = UX_CTF_RS_k_p_UX_TM_k_p; %<-- however, it approximates this when the radial-compression is not lossy. ;
if parameter.flag_CTF_anisotropic==0; X_quad = UX_CTF_RS_k_p_UX_TM_k_p / max(1e-12,sqrt(UX_CTF_QS_l2)) / max(1e-12,sqrt(UX_TM_l2)); end;
if parameter.flag_CTF_anisotropic==1; X_quad = UX_CTF_RS_k_p_UX_TM_k_p / max(1e-12,sqrt(UX_CTF_RS_l2)) / max(1e-12,sqrt(UX_TM_l2)); end;
%%%%;
...
if (flag_verbose>0); disp(sprintf(' %% parameter.flag_optimize_over_gamma_z = %d; parameter.flag_CTF_anisotropic = %d: tmp_Z_errrel: %0.16f',parameter.flag_optimize_over_gamma_z,parameter.flag_CTF_anisotropic,tmp_Z_errrel)); end;
if (flag_verbose>0); disp(sprintf(' %% parameter.flag_optimize_over_gamma_z = %d; parameter.flag_CTF_anisotropic = %d: tmp_X_errrel: %0.16f',parameter.flag_optimize_over_gamma_z,parameter.flag_CTF_anisotropic,tmp_X_errrel)); end;
%%%%%%%%%%%%%%%%;
