...
%%%%%%%%;
pole_k_c_0_ = k_p_r_wk_.*cos(k_p_w_wk_);
pole_k_c_1_ = k_p_r_wk_.*sin(k_p_w_wk_);
...
delta_r_p = 0.05;
delta_r_s = delta_r_max/max(1e-12,sqrt(2*log(1/max(1e-12,delta_r_p))));
...
for nCTF=0:n_CTF-1;
CTF_k_p_r_kC__(:,1+nCTF) = 1.5 + sin(2*pi*(1+nCTF/max(1,n_CTF))*k_p_r_/max(1e-12,k_p_r_max)); %<-- invertible. ;
...
%%%%%%%%;
UX_CTF_S_l2_SM_quad__ = zeros(n_S,n_M);
...
%%%%;
gamma_z = gamma_z_SM_tfpm__(1+nS,1+nM);%gamma_z = 2*pi*nw/max(1,n_w_max);
cc = cos(+gamma_z); sc = sin(+gamma_z);
R2 = [+cc , -sc ; +sc , +cc];
delta_x = delta_x_SM_tfpm__(1+nS,1+nM);
delta_y = delta_y_SM_tfpm__(1+nS,1+nM);
delta_temp_0b_ = R2*delta_temp_0_(1+[0:1]); %<-- rotate delta_temp_0_ by +gamma_z = rotate k by -gamma_z = rotate S_0 by +gamma_z. ;
...
%%%%;
S_l2_quad = real(sqrt(sum(reshape(conj(S_k_p_temp_).*S_k_p_temp_,[n_w_max,n_k_p_r])*weight_2d_k_p_r_)/max(1,n_w_max)));
M_l2_quad = real(sqrt(sum(reshape(conj(M_k_p_temp_).*M_k_p_temp_,[n_w_max,n_k_p_r])*weight_2d_k_p_r_)/max(1,n_w_max)));
%%%%;
X_form = h2d_(2*pi*k_p_r_max*fnorm(delta_temp_0b_ - delta_temp_1b_))/(2*pi)^2 * (pi*k_p_r_max^2); %<-- note sign of translation. ;
X_SM_form__(1+nS,1+nM) = real(X_form) / max(1e-12,S_l2_quad*M_l2_quad) ;
%%%%;
X_quad = sum(reshape(conj(S_k_p_temp_).*M_k_p_temp_,[n_w_max,n_k_p_r])*weight_2d_k_p_r_)/max(1,n_w_max);
X_SM_quad__(1+nS,1+nM) = real(X_quad) / max(1e-12,S_l2_quad*M_l2_quad) ;
UX_CTF_S_l2_SM_quad__(1+nS,1+nM) = S_l2_quad.^2;
UX_T_M_l2_SM_quad__(1+nS,1+nM) = M_l2_quad.^2;
%%%%;
end;%for nM=0:n_M-1;
end;%for nS=0:n_S-1;
%%%%%%%%;
...
%%%%;
n_source_C = 9; source_sC__ = randn(n_source_C,n_CTF);
for nCTF=0:n_CTF-1;
CTF_k_p_wkC__(:,1+nCTF) = zeros(n_w_sum,1);
for nsource_C=0:n_source_C-1;
tmp_wk_ = source_sC__(1+nsource_C,1+nCTF) * cos(2*pi*nsource_C*k_p_r_wk_/max(1e-12,k_p_r_max));
...
pm_n_UX_rank_max = min(n_k_p_r-1,5); %<-- to check accuracy with lossy projection. ;
pm_n_UX_rank_c_ = max(1,min(n_k_p_r-1,pm_n_UX_rank_max - transpose([0:n_cluster-1])));
pm_UX_knc___ = zeros(n_k_p_r,pm_n_UX_rank_max,n_cluster);
pm_X_weight_rc__ = zeros(n_k_p_r,n_cluster);
for ncluster=0:n_cluster-1;
...
UX_CTF_S_l2_SM_quad__(1+nS,1+nM) = UX_CTF_RS_l2;
