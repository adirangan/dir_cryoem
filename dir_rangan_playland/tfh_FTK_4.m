...
%%%%%%%%;
% if flag_p_vs_c==0: cartesian displacement-grid, using get_delta_2.m ;
% if flag_p_vs_c==1; polar displacement-grid, prepends a zero-translation to the list produced by get_delta_3.m. ;
%%%%%%%%;
...
if ~isfield(parameter,'delta_x_requested_'); parameter.delta_x_requested_ = []; end;
delta_x_requested_ = parameter.delta_x_requested_;
if ~isfield(parameter,'delta_y_requested_'); parameter.delta_y_requested_ = []; end;
delta_y_requested_ = parameter.delta_y_requested_;
if ~isfield(parameter,'flag_p_vs_c'); parameter.flag_p_vs_c = 0; end;
...
if ~isfield(parameter,'flag_tf_vs_bf'); parameter.flag_tf_vs_bf = 1; end;
...
%%%%;
if delta_r_max> 0;
if ~isempty(delta_x_requested_) & ~isempty(delta_y_requested_);
assert(numel(delta_x_requested_)==n_delta_v_requested);
assert(numel(delta_y_requested_)==n_delta_v_requested);
FTK.n_delta_v = n_delta_v_requested;
FTK.delta_x_ = delta_x_requested_(:);
FTK.delta_y_ = delta_y_requested_(:);
end;%if ~isempty(delta_x_requested_) & ~isempty(delta_y_requested_);
if  isempty(delta_x_requested_) |  isempty(delta_y_requested_);
