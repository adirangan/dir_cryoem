c$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
     ...
     $     ,svd_calculation_type_in,n_r,grid_p_,n_w_,half_diameter_x_c
     $     ,n_S,ld_S ,S_p_,tesselation_distance_req,S_alpha_polar_a_
     $     ,S_alpha_azimu_b_,L_ ,nl_max,nm_sum ,ll_sum,T_nl_,T_vm_,T_tr_
     $     ,T_ll_ ,T_lf_,T_c0_ ,T_c1_,T_c2_ ,T_c3_,T_ls_,T_LT_
     $     ,n_T_root_base ,T_root_base_ ,n_M,ld_M,M_p_ ,n_CTF,ld_CTF
     $     ,CTF_p_,polar_a_est_ ,azimu_b_est_,gamma_z_est_ ,delta_x_est_
     $     ,delta_y_est_ ,l2_norm_est_,ctf_ind_est_ ,eps_target
     $     ,n_pixels_in ,displacement_max,n_delta_x ,n_delta_y
     $     ,n_gamma_z ,delta_x_optimal_,delta_y_optimal_
     $     ,gamma_z_optimal_ ,C_M_ ,C_S_optimal_ ,C_Z_optimal_
...
c$$$      parameters for timing
     ...
     $        ,'; n_gamma_z: ',n_gamma_z
      ...
      do na=0,n_S*n_M-1
         delta_x_optimal_(na) = 0.0d0
         delta_y_optimal_(na) = 0.0d0
         gamma_z_optimal_(na) = 0.0d0
         C_S_optimal_(na) = (1.0d0,0.0d0)
         C_Z_optimal_(na) = (0.0d0,0.0d0)
      enddo
...
c$$$         call rotate_p2p_fz(n_r,n_w_,n_A,T_p_,+gamma_z,T_p_)
...
c$$$     $        ,T_p_)
     ...
     $              ' and template ' , ns_use
            ...
            call test_innerproduct_batch_stage_2(verbose ,nm,ns_use
     $           ,n_r,grid_p_,n_w_,n_S,ld_S,n_A,nctf,n_w_max ,S_p_,S_p
     $           ,S_q,M_q_,Z_q_ ,C_M,C_S_,C_S_optimal,C_S_optimal_
     $           ,C_Z_optimal,C_Z_optimal_ ,svd_calculation_type
     $           ,n_svd_r,svd_r_,n_svd_l,svd_l_,svd_s_,svd_V_r_ ,Z_svdd_
     $           ,Z_svdr_,Z_tmpC_ ,displacement_max ,delta_x,delta_y
     $           ,gamma_z ,n_delta_x,n_delta_y,n_gamma_z ,delta_x_
     $           ,delta_y_,gamma_z_ ,delta_x_est_,delta_y_est_
     $           ,gamma_z_est_ ,ndx_optimal,ndy_optimal,ngz_optimal
     $           ,delta_x_optimal_,delta_y_optimal_,gamma_z_optimal_
     $           ,fftw_plan_frwd_ ,fftw_plan_back_ ,fftw_in1_ ,fftw_out_
     $           ,timing_tic_1,timing_toc_1,timing_tot_1 ,timing_tic_2
     $           ,timing_toc_2,timing_tot_2 ,timing_tic_3,timing_toc_3
     $           ,timing_tot_3 ,timing_tic_4,timing_toc_4,timing_tot_4)
         enddo !do nLT=0,n_LT-1
         ...
         if (0+verbose.gt.1) then
            ...
            write (6,'(A)') ' local-search complete, and look through  '
            write (6,'(A)') ' our list for the templates that yield '
            write (6,'(A)') ' the largest innerproducts. '
            write (6,'(A)') ' '
         end if !if (0+verbose.gt.1) then
         ...
         do while (continue_flag)
            ...
            do ns=0,n_S-1
               if (S_use_(ns).eqv..true.) then
                  C_Z_optimal = C_Z_optimal_(ns + nm*n_S)
                  C_S_optimal = C_S_optimal_(ns + nm*n_S)
     ...
     $                    ' and template ' , ns_use
                  ...
                  call test_innerproduct_batch_stage_2(verbose ,nm
     $                 ,ns_use ,n_r,grid_p_,n_w_,n_S,ld_S,n_A,nctf
     $                 ,n_w_max ,S_p_,S_p,S_q,M_q_,Z_q_ ,C_M,C_S_
     $                 ,C_S_optimal,C_S_optimal_,C_Z_optimal
     $                 ,C_Z_optimal_ ,svd_calculation_type ,n_svd_r
     $                 ,svd_r_,n_svd_l,svd_l_,svd_s_,svd_V_r_ ,Z_svdd_
     $                 ,Z_svdr_,Z_tmpC_ ,displacement_max ,delta_x
     $                 ,delta_y,gamma_z ,n_delta_x,n_delta_y,n_gamma_z
     $                 ,delta_x_,delta_y_,gamma_z_ ,delta_x_est_
     $                 ,delta_y_est_,gamma_z_est_ ,ndx_optimal
     $                 ,ndy_optimal,ngz_optimal ,delta_x_optimal_
     $                 ,delta_y_optimal_,gamma_z_optimal_
     $                 ,fftw_plan_frwd_ ,fftw_plan_back_ ,fftw_in1_
     $                 ,fftw_out_ ,timing_tic_1,timing_toc_1
     $                 ,timing_tot_1 ,timing_tic_2,timing_toc_2
     $                 ,timing_tot_2 ,timing_tic_3,timing_toc_3
     $                 ,timing_tot_3 ,timing_tic_4,timing_toc_4
     $                 ,timing_tot_4)
                  S_use_(ns_use) = .true.
                  n_S_use = n_S_use+1
                  n_LT_use = n_LT_use + 1
         ...
         do ns=0,n_S-1
            if (S_use_(ns).eqv..false.) then
               C_Z_optimal_(ns + nm*n_S) = C_Z_min
               C_S_optimal_(ns + nm*n_S) = C_S_min
               delta_x_optimal_(ns + nm*n_S) = 0.0d0
               delta_y_optimal_(ns + nm*n_S) = 0.0d0
               gamma_z_optimal_(ns + nm*n_S) = 0.0d0
