      subroutine test_innerproduct_5_ZZ_scan_3(flag_RTRT_vs_RTTR
     ...
     $     ,ngz_optimal
      ...
      if (verbose.gt.0) then
         write(6,'(A)') ' [entering test_innerproduct_5_ZZ_scan_3] '
      ...
      do na=0,n_gamma_z*n_delta_x*n_delta_y-1
         if (real(ZZ_(na)).gt.real(C_Z_optimal)) then
            na_tmp = na
            ndx = mod(na_tmp,n_delta_x)
            na_tmp = na_tmp/n_delta_x
            ndy = mod(na_tmp,n_delta_y)
     ...
     $              (delta_y_(ndy) + delta_y_est)**2)
            ...
            if (verbose.gt.1) then
               write(6,'(A,I0,A,I0,A,I0)') ' ngz ' , ngz , ' ndy ' , ndy
     $              , ' ndx ' , ndx
     ...
     $        ,'in test_innerproduct_5_ZZ_scan_3.f'
      end if !if (flag_found.eqv..false.) then
      if (verbose.gt.0) then
         write(6,'(A)') ' [finished test_innerproduct_5_ZZ_scan_3] '
      end if !if (verbose.gt.0) then
      end
