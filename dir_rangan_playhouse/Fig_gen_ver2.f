      subroutine Fig_gen_ver2(ncur,nlats_,grid_k_p_,n_S,ld_S,S_k_p_,n_M
     $     ,ld_M,Y_slice_,M_k_p_,ld_CTF,CTF_k_p_,n_alpha,alpha_est_
...
c$$$      Reads in a few templates and images and plots a comparison
      ...
      complex *16 S_k_p_(0:0),Y_slice_(0:0),M_k_p_(0:0),CTF_k_p_(0:0)
...
c$$$         Display image without ctf
         call stdlim_c16(n_A,.true.,Y_slice_(nm*ld_M),F_avg,F_std)
     ...
     $        ,Y_slice_(nm*ld_M),F_min,F_max,x_offset,y_offset,x_side
     $        ,y_side)
         write(fig_title,'(A,I0,A,I4,A)') 'R(Y_slice(',nm,')) '
     $        ,nint(dlog(F_std)),'    '
         text_length = 18
         color_index = 0
         font_type = 0
     ...
     $        ,y_side)
         call stdlim_c16(n_A,.false.,Y_slice_(nm*ld_M),F_avg,F_std)
         F_max = F_avg + 2.5*F_std
         F_min = F_avg - 2.5*F_std
         x_offset = +5.0d0 + 10.0d0*nM_sample
     ...
     $        ,Y_slice_(nm*ld_M),F_min,F_max,x_offset,y_offset,x_side
     $        ,y_side)
         write(fig_title,'(A,I0,A,I4,A)') 'I(Y_slice(',nm,')) '
     $        ,nint(dlog(F_std)),'    '
         text_length = 18
         color_index = 0
         font_type = 0
...
c$$$         Display template with perturbation but no ctf
     ...
     $        +delta_y,T_k_p_)
         call rotate_p2p_fz(n_r,n_w_,n_A,T_k_p_,+gamma_z
     $        ,T_k_p_)
         call stdlim_c16(n_A,.true.,T_k_p_,F_avg,F_std)
         F_max = F_avg + 2.5*F_std
         F_min = F_avg - 2.5*F_std
...
c$$$      Printing figure file for image
     ...
     $     ,trim(fname_fig),trim(fname_jpg)
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[finished Fig_gen_ver2]'
      end if
