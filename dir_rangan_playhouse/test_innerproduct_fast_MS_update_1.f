      subroutine test_innerproduct_fast_MS_update_1(verbose,rseed,n_S
     $     ,n_M,C_M_,delta_x_sort_MS_ ,delta_y_sort_MS_,gamma_z_sort_MS_
...
c$$$      parameters for timing
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[entering test_innerproduct_fast_MS_update_1]'
      end if
      if (verbose.gt.0) then
...
c$$$               %%%%%%%%%%%%%%%%
     ...
     $        ,ns_tmp,'; ns ',ns,'; finished'
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[finished test_innerproduct_fast_MS_update_1]'
      end if
      end
