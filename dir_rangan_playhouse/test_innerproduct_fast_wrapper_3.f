      subroutine test_innerproduct_fast_wrapper_3(verbose,rseed
     ...
     $     ,M_p__,n_CTF ,ld_CTF ,CTF_p__ ,n_alpha,alpha_est_
...
c$$$  ld_CTF is the stride between adjacent ctfs in CTF_p__
     ...
     $     -1),ld_S,n_M,I_M_sample_(0:n_M-1),ld_M,n_CTF,ld_CTF,n_alpha
...
c$$$      format_string for printing output
      ...
      if (verbose.gt.0) then
          write(6,'(A)')
     $        '[entering test_innerproduct_fast_wrapper_3]: '
       end if
...
c$$$c$$$      We do not actually use the forward plan
     ...
     $        ,' total_time ',timing_toc-timing_tic
      ...
      if (tesselation_distance_req.ge.2.0d0) then
         if (verbose.gt.1) then
            write(6,'(A)') ''
            write(6,*) 'Now we call test_innerproduct_fast_omp_stage_3'
     $           ,' to calculate all innerproducts:'
         end if                 !if (verbose.gt.1) then
         timing_tic = omp_get_wtime()
         call test_innerproduct_fast_stage_3(verbose,rseed,n_omp_sub
     ...
     $        ,N_pixels_in ,displacement_max ,n_delta_x ,delta_x_
     ...
     $           'test_innerproduct_fast_omp_stage_3:',' total_time: '
     ...
     $           *n_delta_y)
         ...
         if (verbose.gt.0) then
            write(6,'(A)')
     $           '[finished test_innerproduct_fast_wrapper_3]'
         end if
      end if                    !if (tesselation_distance_req.ge.2.0d0) then

c$$$      if (tesselation_distance_req.ge.2.0d0) then
c$$$         if (verbose.gt.1) then
c$$$            write(6,'(A)') ''
c$$$            write(6,*) 'Now we call test_innerproduct_fast_omp_stage_2'
c$$$     $           ,' to calculate all innerproducts:'
c$$$         end if                 !if (verbose.gt.1) then
c$$$         timing_tic = omp_get_wtime()
c$$$         call test_innerproduct_fast_stage_2(verbose,rseed,n_omp_sub
c$$$     $        ,svd_calculation_type,eps_svd,n_r,grid_p_,n_w_
c$$$     $        ,half_diameter_x_c ,n_S,I_S_sample_,ld_S ,S_p__
c$$$     $        ,tesselation_distance_req ,S_alpha_polar_a_
c$$$     $        ,S_alpha_azimu_b_ ,L_ ,nl_max,nm_sum ,ll_sum ,T_nl_,T_vm_
c$$$     $        ,T_tr_ ,T_ll_ ,T_lf_ ,T_c0_ ,T_c1_,T_c2_ ,T_c3_ ,T_ls_
c$$$     $        ,T_LT_ ,n_T_root_base ,T_root_base_ ,n_M,I_M_sample_,ld_M
c$$$     $        ,M_p__ ,n_CTF,ld_CTF ,CTF_p__ ,polar_a_est_
c$$$     $        ,azimu_b_est_ ,gamma_z_est_ ,delta_x_est_ ,delta_y_est_
c$$$     $        ,l2_norm_est_ ,ctf_ind_est_ ,S_index_est_ ,polar_a_upd_
c$$$     $        ,azimu_b_upd_ ,gamma_z_upd_ ,delta_x_upd_ ,delta_y_upd_
c$$$     $        ,l2_norm_upd_ ,ctf_ind_upd_ ,S_index_upd_ ,alpha_update_f
c$$$     $        ,flag_MS_vs_SM ,n_pixels_in ,displacement_max ,n_delta_x
c$$$     $        ,n_delta_y ,n_gamma_z ,fpm_howmany_max,C_M_ ,n_SM_use)
c$$$         timing_toc = omp_get_wtime()
c$$$         if (verbose.gt.0) then
c$$$            write(6,'(A,A,F8.3,A,F8.6,A,F8.7)')
c$$$     $           'test_innerproduct_fast_omp_stage_2:',' total_time: '
c$$$     $           ,timing_toc-timing_tic,'; time_per S,M: ',(timing_toc
c$$$     $           -timing_tic)/max(1,n_SM_use),'; time_per S,M,dx,dy: '
c$$$     $           ,(timing_toc -timing_tic)/(n_SM_use*n_delta_x
c$$$     $           *n_delta_y)
c$$$         end if
c$$$      end if                    !if (tesselation_distance_req.ge.2.0d0) then
