      subroutine innerproduct_q__k_svdd_4(flag_S_vs_M,svd_d_max,n_svd_d
...
c$$$      real *8, allocatable :: U_d_(:)
     ...
     $        ' % [entering innerproduct_q__k_svdd_4] ',n_svd_d,n_svd_l
     $        ,n_delta_v
      end if
...
c$$$            theta = nl_pre*(pi/2 - omega).
     ...
     $              ,'>l_max in innerproduct_q__k_svdd_4'
...
c$$$      deallocate(U_d_)
      deallocate(C_q_pre_)
      if (verbose.gt.0) then
         write (6,'(A)') ' % [finished innerproduct_q__k_svdd_4]'
      end if
      end
