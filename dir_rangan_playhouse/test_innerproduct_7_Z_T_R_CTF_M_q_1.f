      subroutine test_innerproduct_7_Z_T_R_CTF_M_q_1(verbose,svd_r_max
...
c$$$      real *8, allocatable :: V_r_(:)
      if (verbose.gt.0) then
         write(6,'(A)')
     $        '[entering test_innerproduct_7_Z_T_R_CTF_M_q_1]'
...
c$$$      deallocate(V_r_)
      ...
      if (verbose.gt.0) then
         write(6,'(A)')
     $        '[finished test_innerproduct_7_Z_T_R_CTF_M_q_1]'
      end if !if (verbose.gt.0) then
      end
