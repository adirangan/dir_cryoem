C***********************************************************************
      subroutine rebuild_model_0(n_M,ld_M,M_k_p__,n_CTF,ld_CTF
     ...
     $     ,lsq_interpolation_order,lsq_eps ,Y_)
C***********************************************************************
...
C***********************************************************************
      ...
      complex *16 M_k_p__(0:ld_M,0:n_M-1);
      complex *16 CTF_k_p_(0:ld_CTF*n_CTF-1)
      complex *16 Y_(0:0)
      integer nk,n_A,n_w,nw
      ...
      if (verbose.gt.0) then
         write(6,*) '[entering rebuild_model_0]'
      end if
...
c
      ...
      allocate(M_k_c_(0:n_w_M_max-1))
      allocate(polar_a_(0:n_w_M_max-1))
      allocate(azimu_b_(0:n_w_M_max-1))
      allocate(weight_CTF_k_c_(0:n_w_M_max-1))
c
c
c
      n_A = 0
     ...
     $        ,' .neq. n_A: ',n_A,' in rebuild_model_0'
      end if
c
      do nk = n_k_low-1,n_k_cur-1
         ...
         call get_lsqdata_0(n_M,ld_M,M_k_p__,n_CTF,ld_CTF
     $        ,CTF_k_p_,grid_k_p_(nk),n_w_sum_(nk),n_w_(nk)
     $        ,n_alpha,alpha2d__,M_k_c_,polar_a_,azimu_b_
     $        ,weight_CTF_k_c_,n_w_M)
