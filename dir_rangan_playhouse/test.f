      program test
c$$$      gfortran -o test.out test.f -fopenmp -lopenblas ; ./test.out
      implicit none
      include 'omp_lib.h'
...
c$$$      write(6,'(A,F8.4)') 'total Gflops: ' , timing_tmp
      ...
      timing_tic = omp_get_wtime()
      call zgemm('T','N',n_L,n_N,n_M,1.0d0,A_,n_M,B_,n_M,0.0d0,C1_,n_L)
      timing_toc = omp_get_wtime()
      timing_tot = timing_toc-timing_tic
      write(6,'(A,F8.4)') 'total time: ' , timing_tot
