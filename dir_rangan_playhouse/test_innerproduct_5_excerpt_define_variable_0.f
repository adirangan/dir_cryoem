      ...
      include 'omp_lib.h'
      include 'nalpha_define.f' !defines length n_alpha and indexing of image-parameters. For example, the polar angle associated with image I_M_sample_(nm) would be stored within alpha_est__(nalpha_polar_a + n_alpha*nm). ;
      ...
      complex *16 M_k_p__(0:0) !complex *16 array (length at least ld_M*max_i4_f_(I_M_sample_)): stack of images. sometimes called Y associated with reconstructed molecule. 
      ...
      integer *4 n_delta_x !integer *4: if displacements are considered, this value determines the number of displacements considered in the x-direction (in the plane). ;
      integer *4 n_delta_y !integer *4: if displacements are considered, this value determines the number of displacements considered in the y-direction (in the plane). ;
...
c$$$      svd_calculation_type == 2 --> account for displacements via brute-force. ;
      ...
      integer *4 n_S_0_sub__in !integer *4: number of requested sub-blocks at level-0 for n_S (used for S_q__, T_S_q__, Z_S_q__). ;
      ...
      integer *4 n_M_0_sub__in !integer *4: number of requested sub-blocks at level-0 for n_M (used for T_R_CTF_M_q__, T_T_R_CTF_M_q__, Z_T_R_CTF_M_q__). ;
...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      real *8, allocatable :: L_(:) ! list of n_point points used to define tesselation tree. ;
...
c$$$      declaration of svd-expansion and associated variables
      ...
      complex *16, allocatable :: S_q__(:) !array to hold collection of S in bessel-coefficients. ;
      ...
      complex *16, allocatable :: S_T_T_R_CTF_M_q__(:) !array to hold bessel-coefficients for the product of T_R_CTF_M_q__ and T_S_q__ (or T_T_R_CTF_M_q__ and S_q__) integrated over k (i.e., nr). ;
      complex *16, allocatable :: S_Z_T_R_CTF_M_q__(:) !array to hold bessel-coefficients for the product of Z_R_CTF_M_q__ and T_S_q__ (or Z_T_R_CTF_M_q__ and S_q__) integrated over k (i.e., nr). ;
c$$$      indices
      ...
      integer *4, allocatable :: n_w_sum_(:) !array (length at least n_k_cur) to store cumulative sum of n_w_. ;
...
c$$$      fftw plan many (fpm) for omp
      integer *4 n_transf !number of entries in transformed vector. this could be either n_delta_x*n_delta_y (if svd_calculation_type.eq.2) or n_svd_l (if svd_calculation_type.eq.1). ;
...
c$$$      array of displacements and rotations to measure
      integer *4 ndx,ndy,ngz,ndx_optimal,ndy_optimal,ngz_optimal
      real *8 delta_x,delta_y,gamma_z
      real *8, allocatable :: delta_x_(:) !temporary: array of length n_delta_x holds values for delta_x_update. ;
      real *8, allocatable :: delta_y_(:) !temporary: array of length n_delta_y holds values for delta_y_update. ;
...
c$$$      parameters for blocking S and M
      ...
      integer *4 n_r_tmp,n_A_tmp,n_X_tmp,n_F_tmp
...
c$$$      parameters for timing and testing
      ...
      real *8 timing_total_S_q,gnump_total_S_q
      real *8 timing_total_T_S_q,gnump_total_T_S_q
      real *8 timing_total_Z_S_q,gnump_total_Z_S_q
      real *8 timing_total_T_R_CTF_M_q,gnump_total_T_R_CTF_M_q
