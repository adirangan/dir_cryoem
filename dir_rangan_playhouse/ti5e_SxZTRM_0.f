...
c$OMP DO 
      do nw=0,n_w_max-1
         nx1 = nr*(nS_1_sum + nS_0_per*nw)
         nx2 = nr*n_transf*(nM_1_sum + nM_0_per*nw)
...
c$OMP DO 
      do nx0=0,nM_1_per*n_w_max-1
         ...
         call trn0_c16(nS_1_per,n_transf,S_Z_T_R_CTF_M_q__(nx1)
...
c$$$      %%%%%%%%%%%%%%%%
      ...
      if (flag_test) then
         if (verbose.gt.1) then
            write(6,'(A)')
     $           ' calling test_innerproduct_5_SxZTRM_2 to test. '
            write(6,'(A)') ' '
         end if !if (verbose.gt.1) then
         call  test_innerproduct_5_SxZTRM_2(verbose-1,n_svd_d,svd_d_
     $        ,n_svd_r,svd_r_,n_svd_l,svd_l_,svd_U_d_,svd_s_,svd_V_r_
     $        ,Z_M_svdd_,n_delta_x ,delta_x_,n_delta_y ,delta_y_
     $        ,n_gamma_z,gamma_z_,delta_x_est_(nM_1_sum)
     $        ,delta_y_est_(nM_1_sum) ,gamma_z_est_(nM_1_sum)
     $        ,ctf_ind_est_(nM_1_sum),fftw_plan_frwd_ ,fftw_in1_
     $        ,fftw_out_,fftw_plan_back_last,fftw_in1_last_
     $        ,fftw_out_last_ ,n_r,grid_k_p_,n_w_,n_A,S_p_,S_q_,nS_1_per
     $        ,I_S_sample_(nS_0_sum + nS_1_sum),ld_S ,S_p__,M_p_,M_q_
     $        ,nM_1_per,I_M_sample_(nM_0_sum + nM_1_sum),ld_M ,M_p__
     $        ,CTF_p_,n_CTF ,ld_CTF ,CTF_p__ ,C_M_(nM_0_sum + nM_1_sum)
