      subroutine test_innerproduct_fast_wrapper_1 (verbose,n_omp_sub
     ...
     $     ,n_alpha,alpha_est_,alpha_update_f,flag_MS_vs_SM,n_pixels_in
...
c$$$      parameters for timing
      ...
      if (verbose.gt.0) then
          write(6,'(A)')
     $        '[entering test_innerproduct_fast_wrapper_1]: '
       end if
...
c$$$      calculating all innerproducts
      ...
      call test_innerproduct_fast_stage_1(verbose,n_omp_sub
     ...
     $        ,(timing_toc -timing_tic)/(n_SM_use*n_delta_x *n_delta_y)
      ...
      if (verbose.gt.0) then
         write(6,'(A)')
     $        '[finished test_innerproduct_fast_wrapper_1]'
      end if
