...
for nF=1:n_F;
tmp_t = 1.5 + (nF-1)/(n_F-1);
tmp_x_c = 0*max_x_c/2 -0.1 + 0.0625*(tmp_t-1).^2.5 + 0.125*tmp_t*cos(+13.0*2*pi*tmp_t); tmp_y_c = 0*max_x_c/2 + 0.125*tmp_t*sin(+14.0*2*pi*tmp_t); tmp_s = 1/32; tmp_s = (1/64) * (1.50 + (2-tmp_t)*sin(+5.0*2*pi*tmp_t));
...
% Now translate F and G by their centers. ;
%tmp_delta_ = -[ mean(F_x_c_(:).*X_x_c_(:)) , mean(F_x_c_(:).*Y_x_c_(:)) ];
%F_x_c_ = real(ifft2(fft2(F_x_c_).*exp(-i*(X_k_c_*tmp_delta_(1) + Y_k_c_*tmp_delta_(2)))));
%tmp_delta_ = -[ mean(G_x_c_(:).*X_x_c_(:)) , mean(G_x_c_(:).*Y_x_c_(:)) ];
