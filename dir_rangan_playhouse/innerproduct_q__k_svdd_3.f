      subroutine innerproduct_q__k_svdd_3(flag_S_vs_M,svd_d_max,n_svd_d
...
c$$$      l_max must equal or exceed order of bessel-function expansion
     ...
     $        ' % [entering innerproduct_q__k_svdd_3] ',n_svd_d,n_svd_l
     $        ,n_delta_v
      end if
...
c$$$            theta = nl_pre*(pi/2 - omega).
     ...
     $              ,'>l_max in innerproduct_q__k_svdd_3'
...
c$$$               nC = nl + ndv*n_svd_l
      ...
      deallocate(C_q_pre_)
      if (verbose.gt.0) then
         write (6,'(A)') ' % [finished innerproduct_q__k_svdd_3]'
      end if
      end
