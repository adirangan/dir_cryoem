function C_q_ = innerproduct_q_k_svds_1(flag_S_vs_M,n_svd_r,svd_r_,n_svd_l,svd_l_,svd_s_,svd_V_r_,n_r,grid_p_,n_w_,n_A,T_q_,M_q_);
...
verbose = 0;
if (verbose>0);
disp(sprintf(' %% [entering innerproduct_q_k_svds_1] n_r %d',n_r));
end;%if;
svd_r_max = svd_r_(1+n_svd_r-1);
R_q = 2*pi*grid_p_(1+n_r-1);
if (R_q>svd_r_max & warning_flag);
disp(sprintf(' %% Warning, 2*pi*r %f > svd_r_max %f in innerproduct_q_k_svds_1',R_q,svd_r_max));
end;%if;
...
if (verbose>0);
disp(sprintf(' %% [finished innerproduct_q_k_svds_1] n_r %d',n_r));
end;%if;
