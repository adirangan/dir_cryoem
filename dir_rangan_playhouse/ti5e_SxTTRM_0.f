...
c$OMP END PARALLEL
     ...
     $        ' transposing dimensions (1,2) of S_T_T_R_CTF_M_q__. '
      ...
      timing_tic = omp_get_wtime()
      if (n_omp_sub__in.gt.1) then
c$OMP PARALLEL PRIVATE(nx1)
c$OMP DO 
      do nx0=0,nM_1_per*n_w_max-1
         nx1 = nx0*nS_1_per*n_transf
         call trn0_c16(nS_1_per,n_transf,S_T_T_R_CTF_M_q__(nx1)
...
c$$$      %%%%%%%%%%%%%%%%
      ...
      if (flag_test) then
         if (verbose.gt.1) then
            write(6,'(A)')
     $           ' calling test_innerproduct_5_SxTTRM_2 to test. '
            write(6,'(A)') ' '
         end if !if (verbose.gt.1) then
         call test_innerproduct_5_SxTTRM_2(verbose-1,n_delta_x
