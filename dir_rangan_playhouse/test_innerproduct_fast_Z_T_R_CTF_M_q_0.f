      subroutine test_innerproduct_fast_Z_T_R_CTF_M_q_0(verbose,n_svd_r
     ...
     $     ,I_M_sample_,IM_per,ld_M ,M_p__,CTF_p_ ,n_CTF,ld_CTF,CTF_p__
...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%
     ...
     $     ,n_M_tot,ld_M,n_CTF,ld_CTF
      ...
      if (verbose.gt.0) then
         write(6,'(A)')
     $        '[entering test_innerproduct_fast_Z_T_R_CTF_M_q_0]'
...
c$$$                     no conjugate, since we are acting on M and not S
     ...
     $                    *dsqrt_dAn
      ...
      if (verbose.gt.0) then
         write(6,'(A)')
     $        '[finished test_innerproduct_fast_Z_T_R_CTF_M_q_0]'
      end if !if (verbose.gt.0) then
      end
