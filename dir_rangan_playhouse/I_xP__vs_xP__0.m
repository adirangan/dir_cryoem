...
%%%%%%%%;
str_thisfunction = 'I_xP__vs_xP__0';
...
dtau_dtau_ssnll_dif = (+ ssnll_pos - 2*ssnll_mid + ssnll_neg)/max(1e-12,fnorm((delta_S_pos_2S__ - delta_S_neg_2S__)/2).^2);
dtau_dtau_ssnll_mid = ...
...
;
dtau_dtau_ssnll_mid = dtau_dtau_ssnll_mid/max(1e-12,fnorm([da*da_delta_S_2S__ + db*db_delta_S_2S__ + dc*dc_delta_S_2S__]).^2);
...
db = 1*1.2*tolerance_dtau;
dc = 1*2.4*tolerance_dtau;
da_delta_S_ = rand(2,n_source_S);
db_delta_S_ = rand(2,n_source_S);
dc_delta_S_ = rand(2,n_source_S);
dada_delta_S_ = rand(2,n_source_S);
dadb_delta_S_ = rand(2,n_source_S);
dadc_delta_S_ = rand(2,n_source_S);
dbdb_delta_S_ = rand(2,n_source_S);
dbdc_delta_S_ = rand(2,n_source_S);
dcdc_delta_S_ = rand(2,n_source_S);
[ ...
 ssnll_mid ...
,da_ssnll_mid ...
,db_ssnll_mid ...
...
%%%%%%%%%%%%%%%%;
n_source_X = tab_XS*n_source_S + tab_XM*n_source_M;
if tab_XS==1;
phi_X = phi_S;
...
%%%%%%%%;
n_source_Y = tab_YS*n_source_S + tab_YM*n_source_M;
if tab_YS==1;
phi_Y = phi_S;
...
dcdc_output = 0.0;
%%%%;
ssnll = ssnll + tab_sgn * v_source_X * v_source_Y * output;
%%%%;
if flag_d;
da_output = da_output ...
...
;
da_ssnll = da_ssnll + tab_sgn * v_source_X * v_source_Y * da_output;
db_ssnll = db_ssnll + tab_sgn * v_source_X * v_source_Y * db_output;
dc_ssnll = dc_ssnll + tab_sgn * v_source_X * v_source_Y * dc_output;
end;%if flag_d;
if flag_dd;
%%%%;
...
;
%%%%;
dada_ssnll = dada_ssnll + tab_sgn * v_source_X * v_source_Y * dada_output;
dadb_ssnll = dadb_ssnll + tab_sgn * v_source_X * v_source_Y * dadb_output;
dadc_ssnll = dadc_ssnll + tab_sgn * v_source_X * v_source_Y * dadc_output;
dbdb_ssnll = dbdb_ssnll + tab_sgn * v_source_X * v_source_Y * dbdb_output;
dbdc_ssnll = dbdc_ssnll + tab_sgn * v_source_X * v_source_Y * dbdc_output;
dcdc_ssnll = dcdc_ssnll + tab_sgn * v_source_X * v_source_Y * dcdc_output;
%%%%;
end;%if flag_dd;
%%%%%%%%;
%%%%%%%%%%%%%%%%;
