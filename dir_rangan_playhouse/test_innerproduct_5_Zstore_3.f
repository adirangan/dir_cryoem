      subroutine test_innerproduct_5_Zstore_3(
     $     verbose
     $     ,rseed
     $     ,flag_RTRT_vs_RTTR
     $     ,n_delta_v
...
c$$$      MS storage
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[entering test_innerproduct_5_Zstore_3]'
...
c$$$            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     ...
     $           +gamma_z_est_(nm) ,ngz_optimal,CTF_R_S_optimal)
            if (verbose.gt.2) then
               write(6,'(2(A,I0),A,I2,1X,I2,3(A,2F8.3))')
...
c$$$            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[finished test_innerproduct_5_Zstore_3]'
      end if !if (verbose.gt.0) then
