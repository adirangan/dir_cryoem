      subroutine innerproduct_q__k_svdr_1(flag_S_vs_M,n_svd_r,svd_r_
     $     ,n_svd_l,svd_l_,svd_s_,svd_V_r_,n_r,grid_p_,n_w_,n_A,T_q_
     $     ,M_q_,C_q_)
...
c$$$      flag_S_vs_M .eqv. .false. --> transformation applied to M, use -.
      ...
      if (verbose.gt.0) then
         write(6,'(A,I0)') ' % [entering innerproduct_q__k_svdr_1] n_r '
     $        ,n_r
      end if
      pi = 4.0*atan(1.0)
     ...
     $        ,svd_r_max,' in innerproduct_q__k_svdr_1'
      end if
      n_w_max = n_w_(n_r-1)
      if (verbose.gt.0) then
...
c$$$         We assume that the fourier basis is orthonormal (not merely orthogonal)
     ...
     $                 *T_q_(ic))*M_q_(ict)
      ...
      enddo
      if (verbose.gt.0) then
         write(6,'(A)') ' % [finished innerproduct_q__k_svdr_1]'
      end if
      end
