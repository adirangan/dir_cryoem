      subroutine test_innerproduct_fast_ZZ_0(verbose,n_delta_x,delta_x_
     ...
     $     ,displacement_max
     $     ,C_M_,CTF_R_S_ ,S_T_T_R_CTF_M_q__,ZZ_)
      implicit none
     ...
     $     ,l2_norm_upd_(0:n_M-1)
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[entering test_innerproduct_fast_ZZ_0]'
...
c$$$            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[finished test_innerproduct_fast_ZZ_0]'
      end if !if (verbose.gt.0) then
