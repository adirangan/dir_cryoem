function test_fft_sampling_8(k_r,n_polar_a,model_type);
%{

  model_type = 'random'; n_polar_a = 32; for k_r = 4:4:48; test_fft_sampling_8(k_r,n_polar_a,model_type); end;
  model_type = 'lissajous'; n_polar_a = 32; for k_r = 4:4:48; test_fft_sampling_8(k_r,n_polar_a,model_type); end;
  model_type = 'helix'; n_polar_a = 32; for k_r = 4:4:48; test_fft_sampling_8(k_r,n_polar_a,model_type); end;
  model_type = 'trpv1'; n_polar_a = 32; for k_r = 2:2:48; test_fft_sampling_8(k_r,n_polar_a,model_type); end;
...
flag_plot=0; 
if flag_plot;
figure(1); isosurface_f_x_c_0(f_x_c___,99.5); 
axis vis3d; view(10,25);
set(gcf,'Position',1+[0,0,512,512]);
fname_tmp = sprintf('%s/dir_jpg/test_fft_sampling_8_%s',pwd,model_type);
...
if (strcmp(model_type,'random') | strcmp(model_type,'lissajous') | strcmp(model_type,'helix'));
for nsource=1:n_source;
...
end;%if (strcmp(model_type,'random') | strcmp(model_type,'lissajous') | strcmp(model_type,'helix'));
na = na+1;
...
%%%%%%%%;
if (strcmp(model_type,'trpv1'));
k_max = n_x_c/2;
tic;
F_ = nufft3d2(n_all,k__(:,1)*pi/k_max,k__(:,2)*pi/k_max,k__(:,3)*pi/k_max,-1,1e-12,n_x_c,n_x_c,n_x_c,decenter3(f_x_c___));
disp(sprintf(' %% nufft3d2: nj %d, t: %0.2f',n_all,toc));
tic;
equator_F__(:) = nufft3d2(n_all*(n_equator+1),equator_x__(:)*pi/k_max,equator_y__(:)*pi/k_max,equator_z__(:)*pi/k_max,-1,1e-12,n_x_c,n_x_c,n_x_c,decenter3(f_x_c___));
disp(sprintf(' %% nufft3d2: nj %d, t: %0.2f',n_all*(n_equator+1),toc));
end;%if (strcmp(model_type,'trpv1'));
%%%%%%%%;
...
Y__(na1,na2) = max(0,AA_(na1) - 2*tmp_x + AA_(na2));
Y__(na2,na1) = Y__(na1,na2);
X__(na1,na2) = max(-1,min(+1,tmp_x / sqrt((AA_(na1)*AA_(na2)))));
...
figbig;
fname_tmp = sprintf('%s/dir_jpg/test_fft_sampling_8_%s_X_k%da%d',pwd,model_type,round(k_r),n_polar_a);
disp(sprintf(' %% writing %s.jpg',fname_tmp));
print('-djpeg',sprintf('%s.jpg',fname_tmp));
end;%if flag_plot;
...
cos_angle_max = -1; 
if strcmp(model_type,'random'); cos_angle_max = -1; end;
if strcmp(model_type,'lissajous'); cos_angle_max = 0; end;
if strcmp(model_type,'helix'); cos_angle_max = -1; end;
if strcmp(model_type,'trpv1'); cos_angle_max = -1; end;
snr_sigma_ = linspace(0.01,0.24,24);
if strcmp(model_type,'trpv1'); snr_sigma_ = linspace(0.01,0.24,24)*median(Y__(:)); end;
for ns=1:length(snr_sigma_);
snr_sigma = snr_sigma_(ns);
if (cos_angle_max<=-1);
tmp_x__ = exp(-Y__.^2/(2*snr_sigma^2));
end;%if (cos_angle_max<=-1);
if (cos_angle_max>-1);
tmp_x__ = zeros(n_all,n_all);
for nall=1:n_all;
tmp_k_ = k__(nall,:);
tmp_cos_angle_ = sum(repmat(tmp_k_,n_all,1).*k__,2)/k_r^2;
tmp_ij_ = find(tmp_cos_angle_>cos_angle_max);
tmp_x__(nall,tmp_ij_) = exp(-Y__(nall,tmp_ij_).^2/(2*snr_sigma^2));
end;%for nall=1:n_all;
end;%if (cos_angle_max>-1);
tmp_x__ = tmp_x__./repmat(sum(tmp_x__,2),1,n_all);
disp(sprintf(' %% ns %d/%d: cos_angle_max %0.2f, tmp_x__ filled %d/%d',ns,length(snr_sigma_),cos_angle_max,length(find(tmp_x__>0)),numel(tmp_x__)));
tmp_k__ = tmp_x__*k__;
subplot(4,6,ns);
%%%%%%%%;
hold on;
...
%%%%%%%%;
axis vis3d; view(10,25);
title(sprintf('\\sigma %0.2f',snr_sigma));
end;%for ns=1:length(snr_sigma_);
figbig;
fname_tmp = sprintf('%s/dir_jpg/test_fft_sampling_8_%s_mesh_k%da%d_cam%0.2f',pwd,model_type,round(k_r),n_polar_a,cos_angle_max);
disp(sprintf(' %% writing %s.jpg',fname_tmp));
print('-djpeg',sprintf('%s.jpg',fname_tmp));
end;%if flag_plot;
