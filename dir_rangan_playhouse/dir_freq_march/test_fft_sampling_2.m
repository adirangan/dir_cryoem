...
%%%%%%%%;
sigma = 0.1;
k_r_ = linspace(1,16,24);
for nk_r=1:length(k_r_);
k_r = k_r_(nk_r);
...
nc = max(1,min(n_c,floor(n_c*periodize(abs(pi/2-polar_a_k_cur),0,pi/2))));
...
p_c_(:,1+na,:) = repmat(c_(nc,:),4,1);
hold on; plot(k_real_cur_cur,k_imag_cur_cur,'.','Color',c_(nc,:)); hold off;
na=na+1;
end;%for npolar_a=1:n_polar_a;
end;%for nazimu_b=1:n_azimu_b;
...
%%%%%%%%;
end;%for nk_r=1:length(k_r_);
set(gcf,'Position',1+[0,0,1024*2,1024]);
dir_trunk = sprintf('/data/rangan/dir_cryoem/dir_rangan_playhouse');
tmp_fname = sprintf('%s/dir_freq_march/freq_march_2_figB_nX%d',dir_trunk,length(k_r_));
...
set(gcf,'Position',1+[0,0,1024*2,1024]);
dir_trunk = sprintf('/data/rangan/dir_cryoem/dir_rangan_playhouse');
tmp_fname = sprintf('%s/dir_freq_march/freq_march_2_figC_k%d',dir_trunk,k_r);
disp(sprintf(' %% writing %s',sprintf('%s.jpg',tmp_fname)));
print('-djpeg',sprintf('%s.jpg',tmp_fname));
