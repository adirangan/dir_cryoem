...
c$$$      temporary arrays
      ...
      if (verbose.gt.2) then
         ...
         call write_sub_i4(n_S,n_MS_,8,' n_MS_: ')
         write(6,'(A,I0)') ' n_S: ' , n_S
         write(6,'(A,I0)') ' n_M: ' , n_M
         call write_sub_r8(n_alpha*n_MS_max*n_S,alpha_MS__,13
     $        ,' alpha_MS__: ')
         write(6,'(A,L1)') ' flag_RTRT_vs_RTTR: ' , flag_RTRT_vs_RTTR
         do nM=0,n_M-1
            call write_all_r8(n_alpha,alpha_est__(n_alpha*nM),14
     $           ,' alpha_est__: ')
            call write_all_r8(n_alpha,alpha_upd__(n_alpha*nM),14
     $           ,' alpha_upd__: ')
         enddo ! do nM=0,n_M-1
     ...
     $        *n_MS_max*ns))
      ...
      if (verbose.gt.1) then
         call write_all_i4(n_S,I_S_,7,' I_S_: ')
         call write_all_i4(n_S,n_MS_,8,' n_MS_: ')
      ...
      do while (nm_sum.le.n_M-1 .and. nm_sum.le.n_MS_sum-1)
         ns = I_S_(ns_tmp)
         if (verbose.gt.2) then
            call write_all_i4(n_S,n_MS_tmp_,12,' n_MS_tmp_: ')
