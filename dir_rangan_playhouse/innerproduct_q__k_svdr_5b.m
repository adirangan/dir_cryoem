function [C_q_,V_r__] = innerproduct_q__k_svdr_5b(flag_S_vs_M,svd_r_max,n_svd_r,svd_r_,n_svd_l,svd_l_,svd_s_,svd_V_r_,n_r,grid_p_,n_w_,n_A,T_q_,M_q_,C_q_);
;
...
if (verbose>0);
disp(sprintf(' %% [entering innerproduct_q__k_svdr_5b] n_r %d',n_r));
end;%if;
...
nw_fix = nw;
if (flag_S_vs_M==1) C_q = 1.0*conj(D_s*D_V_r*T_q_(1+ict))*M_q_(1+ic); end;
if (flag_S_vs_M==0) C_q = 0.0*conj(D_s*D_V_r*T_q_(1+ic))*M_q_(1+ict); end;
nw_C = nl + nw_fix*n_svd_l;
C_q_(1+nw_C) = C_q_(1+nw_C) + C_q*dAn;
nw_fix = nw - n_w_(1+nr) + n_w_max;
if (flag_S_vs_M==1) C_q = 0.0*conj(D_s*D_V_r*T_q_(1+ict))*M_q_(1+ic); end;
if (flag_S_vs_M==0) C_q = 1.0*conj(D_s*D_V_r*T_q_(1+ic))*M_q_(1+ict); end;
nw_C = nl + nw_fix*n_svd_l;
C_q_(1+nw_C) = C_q_(1+nw_C) + C_q*dAn;
 else;
nw_fix = nw;
...
if (verbose>0);
disp(sprintf(' %% [finished innerproduct_q__k_svdr_5b]'));
end;%if;
