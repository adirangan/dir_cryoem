      subroutine test_innerproduct_fast_memory_estimate_0(verbose
     $     ,n_omp_sub,n_cur,n_polar_a_in_,grid_p_,half_diameter_x_c,n_S
     $     ,n_M ,n_CTF,n_alpha ,n_pixels_in ,displacement_max,n_delta_x
     $     ,n_delta_y,n_gamma_z ,svd_calculation_type ,eps_svd
     $     ,fpm_howmany_max,n_SM)
      implicit none
      integer verbose
      integer n_omp_sub,n_cur,n_polar_a_in_(0:0),n_S,n_M,n_CTF,n_alpha
     $     ,n_delta_x,n_delta_y,n_gamma_z,svd_calculation_type,n_SM
      integer n_r,nr,n_A,n_w_max
      real *8 grid_p_(0:0),half_diameter_x_c
      integer fpm_howmany_max,fpm_howmany,nM_per_max
      real *8 eps_svd,eps_target,n_pixels_in,displacement_max
      integer, allocatable :: n_w_(:)
      integer, allocatable :: n_w_sum_(:)
c$$$      declaration of svd-expansion and associated variables
      ...
      integer *4 n_k_p_max,nk,nl,n_Y_lm_sum_max,ld_S,ld_M
      ...
      if (verbose.gt.1) then
         write(6,'(A)')
     $        '[entering test_innerproduct_fast_memory_estimate_0]'
      ...
      if (verbose.gt.1) then
         ...
         call write_sub_i4(n_r,n_polar_a_in_,13,' n_polar_a_in_: ')
...
c$$$      Calculating template size using 'get_template_size'
      ...
      allocate(n_w_sum_(0:n_r-1))
      call get_template_size(n_polar_a_in_,n_r,n_A,n_w_,n_w_sum_)
      if (verbose.gt.1) then
         write(6,'(A,I0)') ' n_A = ',n_A
         call write_sub_i4(n_r,n_w_,7,' n_w_: ')
         call write_sub_i4(n_r,n_w_sum_,11,' n_w_sum_: ')
      end if !if (verbose.gt.1) then
     ...
     $        ,' n_delta_x*n_delta_y: ' , n_delta_x*n_delta_y
      ...
      do nr = 0,n_r-1
         ...
         ld_S = 0
         do nk=0,n_k_p_max-1
            n_polar_a = 2*ceiling(max(6,nint(pi*grid_p_(nk)))
     $           /2.0d0)
            n_polar_a_(nk) = n_polar_a
     ...
     $              *sin_polar_a_(npolar_a)))
            ...
            n_polar_a_azimu_b_sum_(nk) = n_polar_a_azimu_b_sum
            if (n_w_(nk).ne.n_polar_a_(nk)*2) write(6,'(A)')
     $           'Warning, n_w_(nk) .ne. n_polar_a_(nk)'
            n_Y_lm_sum_(nk) = n_Y_lm_sum_max
            n_Y_l_(nk) = nint(grid_p_(nk)+2)
     ...
     $     ' GB '
      ...
      if (verbose.gt.1) then
         write(6,'(A)')
     $        '[finished test_innerproduct_fast_memory_estimate_0]'
      end if !if (verbose.gt.1) then
