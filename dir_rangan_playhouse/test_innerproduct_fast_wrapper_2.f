      subroutine test_innerproduct_fast_wrapper_2 (verbose,rseed
     $     ,n_omp_sub,n_k_cur,n_polar_a_,grid_k_p_,half_diameter_x_c,n_S
     $     ,I_S_sample_,ld_S ,S_k_p_,tesselation_distance_req
     $     ,S_alpha_polar_a_ ,S_alpha_azimu_b_,n_M,I_M_sample_,ld_M
     $     ,M_k_p_,n_CTF ,ld_CTF ,CTF_k_p_ ,n_alpha,alpha_est_
...
c$$$      The output (i.e., which innerproducts are highest) is returned.
      ...
      integer *4 n_k_cur,n_polar_a_(0:n_k_cur-1),n_S,I_S_sample_(0:n_S
...
c$$$      parameters for memory map
      ...
      if (verbose.gt.0) then
          write(6,'(A)')
     $        '[entering test_innerproduct_fast_wrapper_2]: '
       end if
...
c$$$      preparing updated translations and rotations
     ...
     $           ,S_index_upd_(nm)
      ...
      if (verbose.gt.1) then
         write(6,'(A)') ''
         write(6,*) 'Now we call test_innerproduct_fast_omp_stage_2'
...
c$$$      calculating all innerproducts
      ...
      call test_innerproduct_fast_stage_2(verbose,rseed,n_omp_sub
     $     ,svd_calculation_type,eps_svd,n_r,grid_k_p_,n_w_
     $     ,half_diameter_x_c ,n_S,I_S_sample_,ld_S ,S_k_p_
     $     ,tesselation_distance_req ,S_alpha_polar_a_ ,S_alpha_azimu_b_
     $     ,L_ ,nl_max,nm_sum ,ll_sum ,T_nl_,T_vm_,T_tr_ ,T_ll_ ,T_lf_
     $     ,T_c0_ ,T_c1_,T_c2_ ,T_c3_ ,T_ls_,T_LT_ ,n_T_root_base
     $     ,T_root_base_ ,n_M,I_M_sample_,ld_M ,M_k_p_ ,n_CTF,ld_CTF
     ...
     $     ,S_index_upd_ ,alpha_update_f ,flag_MS_vs_SM ,n_pixels_in
     ...
     $        'test_innerproduct_fast_omp_stage_2:',' total_time: '
     ...
     $        ,(timing_toc -timing_tic)/(n_SM_use*n_delta_x *n_delta_y)
      ...
      if (verbose.gt.0) then
         write(6,'(A)')
     $        '[finished test_innerproduct_fast_wrapper_2]'
      end if
