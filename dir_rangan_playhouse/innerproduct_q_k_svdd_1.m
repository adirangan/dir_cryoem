function C_q_ = innerproduct_q_k_svdd_1(flag_S_vs_M,n_svd_d,svd_d_,n_svd_l,svd_l_,svd_U_d_,n_delta_x,delta_x_,n_delta_y,delta_y_,l_max);
...
warning_flag = 1;
verbose=1;
if (verbose>1); disp(sprintf(' [entering innerproduct_q_k_svdd_1] l_max %d',l_max)); end;
...
disp(sprintf(' %% Warning, delta %f > svd_d_max %f; ratio %f in innerproduct_q_k_svdd_1',delta,svd_d_max,delta/svd_d_max));
end;%if;
...
I_l = svd_l_(1+nl);
if (abs(I_l)>l_max);
disp(sprintf(' %% Warning, I_l %d > l_max in innerproduct_q_k_svdd_1',I_l));
end;%if;
if (verbose>2);
...
if (verbose>1); disp(sprintf(' [finished innerproduct_q_k_svdd_1]')); end;
