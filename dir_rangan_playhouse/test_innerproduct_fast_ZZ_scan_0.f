      subroutine test_innerproduct_fast_ZZ_scan_0(delta_x_est
     $     ,delta_y_est,gamma_z_est,displacement_max,n_delta_x,delta_x_
     $     ,n_delta_y,delta_y_,n_gamma_z,gamma_z_,ZZ_,ndx_optimal
     ...
     $     ,ngz_optimal
      ...
      if (verbose.gt.0) then
         write(6,'(A)') ' [entering test_innerproduct_fast_ZZ_scan_0] '
     ...
     $        ,'in test_innerproduct_fast_ZZ_scan_0.f'
      end if !if (flag_found.eqv..false.) then
      if (verbose.gt.0) then
         write(6,'(A)') ' [finished test_innerproduct_fast_ZZ_scan_0] '
      end if !if (verbose.gt.0) then
      end
