function C_q_ = innerproduct_q_k_stretch_0(n_r,grid_p_,n_w_,n_A,T_q_,M_q_) ;
...
if (verbose>0); disp(sprintf(' %% [entering innerproduct_q_k_stretch_0] n_r %d',n_r)); end%if;
...
if (verbose>1); disp(sprintf(' %% nr %d n_w_(1+nr) %d R_pre %0.2f R_pos %0.2f dr %0.2f',nr,n_w_(1+nr),R_pre,R_pos,dr)); end%if;
...
if (verbose>0); disp(sprintf(' %% [finished innerproduct_q_k_stretch_0]')); end;%if;
