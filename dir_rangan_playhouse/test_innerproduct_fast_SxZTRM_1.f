      subroutine test_innerproduct_fast_SxZTRM_1(verbose,n_svd_d,svd_d_
     $     ,n_svd_r,svd_r_,n_svd_l,svd_l_,svd_U_d_,svd_s_,svd_V_r_
     $     ,Z_S_svdd_ ,Z_M_svdd_,n_delta_x,delta_x_ ,n_delta_y,delta_y_
     $     ,n_gamma_z ,gamma_z_,delta_x_est_,delta_y_est_ ,gamma_z_est_
     $     ,ctf_ind_est_,fftw_plan_frwd_,fftw_in1_ ,fftw_out_
     $     ,fftw_plan_back_last,fftw_in1_last_,fftw_out_last_ ,n_r
     $     ,grid_p_,n_w_,n_A,S_p_,S_q_,n_S,I_S_sample_,ld_S ,S_p__,M_p_
     $     ,M_q_,n_M ,I_M_sample_,ld_M,M_p__,CTF_p_ ,n_CTF ,ld_CTF
...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[entering test_innerproduct_fast_SxZTRM_1]'
      ...
      allocate(C_S_q_(0:n_w_max*n_delta_x*n_delta_y-1))
      call cl1_c16(n_w_max*n_delta_x*n_delta_y,C_S_q_)
      allocate(C_M_q_(0:n_w_max*n_delta_x*n_delta_y-1))
      call cl1_c16(n_w_max*n_delta_x*n_delta_y,C_M_q_)
...
c$$$         Combine S_q and M_q into Z_S_svdr_
         call innerproduct_q_k_svdr_1(.true.,n_svd_r,svd_r_,n_svd_l
     $        ,svd_l_,svd_s_,svd_V_r_,n_r,grid_p_,n_w_,n_A,S_q_,M_q_
     $        ,Z_S_svdr_)
...
c$$$         Combine S_q and M_q into Z_M_svdr_
         call innerproduct_q_k_svdr_1(.false.,n_svd_r,svd_r_,n_svd_l
     $        ,svd_l_,svd_s_,svd_V_r_,n_r,grid_p_,n_w_,n_A,S_q_,M_q_
     $        ,Z_M_svdr_)
...
c$$$         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
         ...
         do ndx=0,n_delta_x-1
            do ndy=0,n_delta_y-1
               write(6,'(A,I0,A,I0)') ' ndx:' , ndx , '; ndy:' , ndy
               call write_sub_c16(n_w_max,C_S_q_(n_w_max*(ndx +
     $              n_delta_x*ndy)),9,' C_S_q_: ')
               call write_sub_c16(n_w_max,C_M_q_(n_w_max*(ndx +
     $              n_delta_x*ndy)),9,' C_M_q_: ')
            enddo !do ndy=0,n_delta_y-1
...
c$$$         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     ...
     $                 ,n_svd_l,svd_l_,svd_U_d_,svd_s_,svd_V_r_,n_r
     $                 ,grid_p_,n_w_,n_A,T_q_,delta_x,delta_y,T_q_)
                  call rotate_q_to_q(n_r,n_w_,n_A,T_q_,gamma_z,T_q_)
...
c$$$                  C_M_tmp = C_M_tmp + zabs(C_M_q_(nC4))**2
     ...
     $                 ,' C_S_q_ vs S_R_T_x_T_R_CTF_M_q__: ' ,' nx1: ' ,
     ...
     $                 ,real(zabs(Z_S_tmp) /zabs(C_S_tmp))
               ...
               if (verbose.gt.-2) then
                  write(6
     $                 ,'(A,A,I0,A,I0,A,I0,A,I0,A,I0,A,F16.8,A ,F16.8)')
     $                 ,' C_M_q_ vs S_T_R_x_T_R_CTF_M_q__: ' ,' nx1: ' ,
     $                 nx1 ,' ndx: ' , ndx , ' ndy: ' , ndy ,' ns: ' ,
     $                 ns ,' nm: ' , nm , ' absolute error: '
...
c$$$         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
         do ndx=0,n_delta_x-1
            do ndy=0,n_delta_y-1
c$$$               %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
               call cp1_c16(n_w_max,C_M_q_(0+n_w_max*(ndx+n_delta_x
     $              *ndy)),Z_tmp_)
...
c$$$               %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%               
      ...
      if (verbose.gt.0) then
         write(6,'(A)')
     $        '[finished test_innerproduct_fast_SxZTRM_1]'
      end if !if (verbose.gt.0) then
      end
