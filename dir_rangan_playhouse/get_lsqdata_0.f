      subroutine get_lsqdata_0(n_M,ld_M,M_k_p__,n_CTF
     $     ,ld_CTF,CTF_k_p_,k_p,n_w_sum,n_w,n_alpha,alpha2d__
     $     ,M_k_c_ ,polar_a_,azimu_b_,weight_CTF_k_c_,n_w_M)
      implicit none
      integer verbose
      data verbose / 0 /
      integer n_M,ld_M,n_CTF,ld_CTF,n_w_sum,n_w,n_w_M
      integer *4 n_alpha
...
c
         ...
         do nw = 0,n_w-1
            M_k_p_1_(nw) = M_k_p__(n_w_sum-1+nw,nM)/l2_norm
            CTF_k_p_1_(nw) = CTF_k_p_(n_w_sum-1+nw + nctf*ld_CTF)
         enddo !do nw = 0,n_w-1
c
