      subroutine test_alpha_update_SM_2(verbose,rseed,n_SM_max,n_SM
     ...
     $     ,alpha_upd_)
      ...
      if (verbose.gt.1) then
         write(6,'(A)') ' [entering test_alpha_update_SM_2].'
      ...
      if (verbose.gt.2) then
         ...
         call write_sub_r8(n_alpha*n_SM_max,alpha_SM_,12,' alpha_SM_: ')
         write(6,'(A,L1)') ' flag_RTRT_vs_RTTR: ' , flag_RTRT_vs_RTTR
         call write_all_r8(n_alpha,alpha_est_,13,' alpha_est_: ')
         call write_all_r8(n_alpha,alpha_upd_,13,' alpha_upd_: ')
...
c$$$      choose an entry from 0,n_SM-1
     ...
     $        ,delta_y_upd)
      ...
      if (verbose.gt.1) then
         write(6,'(A)') ' [finished test_alpha_update_SM_2].'
      end if !if (verbose.gt.1) then
      end
