...
c$$$      svd_calculation_type == 2 --> brute force displacements (see test_innerproduct_batch_stage_2.f). ;
      parameter (svd_calculation_type=2)
      ...
      logical flag_fig !flag determining whether or not to generate figures (in dir_jpg11)
...
c$$$      real *8, allocatable :: azimu_b_step_tmp_(:) !temporary: similar to azimu_b_step_. ;
      ...
      complex *16, allocatable :: Y_slice__(:,:) ! mixed stack of images associated with molecules-A,B,C. These are taken directly from the molecular function itself (with no noise or l2-norm or ctf). sometimes called cslices. ;
...
c$$$      integer *4 n_SM_use !temporary: total number of image-template pairs considered when calculating innerproducts. Might be smaller than the total number when calculating innerproducts adaptively. ;
      ...
      integer n_M_A_sample,n_M_B_sample,n_M_C_sample !temporary: values used to track number of sampled images from molecule-A,B,C. ;
...
c-----------------------------------------------------------------------
     ...
     $              *sin(source_w)
      ...
      write(tmp_fname,'(A)')
     $     './dir_mda11/source_x1_c_x2_c_x3_c_A___.mda'
      ...
      if (flag_heterogeneity) then
         ...
         tmp_d_(1) = n_source_use
         write(tmp_fname,'(A)')
     $        './dir_mda11/source_x1_c_x2_c_x3_c_B___.mda'
     ...
     $        ,trim(tmp_fname)
         ...
         tmp_d_(1) = n_source_use
         write(tmp_fname,'(A)')
     $        './dir_mda11/source_x1_c_x2_c_x3_c_C___.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : '
     $        ,trim(tmp_fname)
...
c     create physical space grid and sample function_A on it.
     ...
     $     ,f_x_c_l2_norm
      ...
      tmp_d_(1) = n_x_c_max
      tmp_d_(2) = n_x_c_max
      write(tmp_fname,'(A)') './dir_mda11/grid_x1_c___.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(tmp_fname)
      call MDA_write_r8(3,tmp_d_,grid_x1_c___,tmp_fname)
      write(tmp_fname,'(A)') './dir_mda11/grid_x2_c___.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(tmp_fname)
      call MDA_write_r8(3,tmp_d_,grid_x2_c___,tmp_fname)
      write(tmp_fname,'(A)') './dir_mda11/grid_x3_c___.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(tmp_fname)
      call MDA_write_r8(3,tmp_d_,grid_x3_c___,tmp_fname)
      write(tmp_fname,'(A)') './dir_mda11/f_x_c_A___.mda'
...
c     create physical space grid and sample function_B on it.
     ...
     $        ,f_x_c_l2_norm
         ...
         tmp_d_(1) = n_x_c_max
         tmp_d_(2) = n_x_c_max
         write(tmp_fname,'(A)') './dir_mda11/f_x_c_B___.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : '
     $        ,trim(tmp_fname)
     ...
     $        ,f_x_c_l2_norm
         ...
         tmp_d_(1) = n_x_c_max
         tmp_d_(2) = n_x_c_max
         write(tmp_fname,'(A)') './dir_mda11/f_x_c_C___.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : '
     $        ,trim(tmp_fname)
...
c$$$      save both Y_A_ and Y_B_ Y_C_ to disk.
     ...
     $     ,trim(tmp_fname)
      ...
      if (flag_heterogeneity) then
         tmp_d_(0) = n_Y_lm_sum_max
         tmp_d_(1) = 1
         write(tmp_fname,'(A)') './dir_mda11/Y_B_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : '
     $        ,trim(tmp_fname)
         call MDA_write_c16(2,tmp_d_,Y_B_,tmp_fname)
         tmp_d_(0) = n_Y_lm_sum_max
         tmp_d_(1) = 1
         write(tmp_fname,'(A)') './dir_mda11/Y_C_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : '
     $        ,trim(tmp_fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      ...
      do nm=0,n_M-1
         if (flag_heterogeneity) then
            if (mod(nm,7).le.2) I_model_(nm)=0
            if (mod(nm,7).gt.2 .and. mod(nm,7).le.4) I_model_(nm)=1
            if (mod(nm,7).gt.4) I_model_(nm)=2
         else
            I_model_(nm) = 0
         end if !if (flag_heterogeneity) then
      enddo !do nm=0,n_M-1
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      if (flag_fig) then
      write(tmp_fname,'(A)') './dir_jpg11/Fig_ctf_c'
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $           ,Y_slice_sample_(nM_sample*ld_M))
         ...
         if (verbose.gt.1) then
            write(6,'(A,I0,A,I0,A,I0,A,I0)') 'n_M_sample: ' , n_M_sample
     $           ,'; n_M_A_sample: ' , n_M_A_sample , '; n_M_B_sample: '
     $           , n_M_B_sample , '; n_M_C_sample: ' , n_M_C_sample
         end if !if (verbose.gt.0) then
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         tmp_d_(0) = n_alpha
         tmp_d_(1) = n_M_sample
         write(tmp_fname,'(A,I0,A)') './dir_mda11/alpha1d_tru_',n_k_cur
     $        ,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : '
     $        ,trim(tmp_fname)
         ...
         tmp_d_(0) = n_alpha
         tmp_d_(1) = n_M_sample
         write(tmp_fname,'(A,I0,A)') './dir_mda11/alpha1d_est_',n_k_cur
     $        ,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : '
     $        ,trim(tmp_fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         if (flag_fig) then
         write(tmp_fname,'(A,I0)') './dir_jpg11/Fig_MNC_c_',n_k_cur
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         ...
         timing_tic = second()
         call rebuild_model_0(n_M_sample,ld_M,M_sample_,n_ctf ,ld_CTF
     $        ,CTF_k_p_,n_alpha,alpha1d_tru_ ,n_w_sum_ ,n_polar_a_
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        ,grid_x3_c___,f_x_c_tru___ ,ier)
         ...
         tmp_d_(1) = n_x_c_max
         tmp_d_(2) = n_x_c_max
         write(tmp_fname,'(A,I0,A)') './dir_mda11/f_x_c_tru___',n_k_cur
     $        ,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : '
     $        ,trim(tmp_fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */         
         if (flag_residual) then
            ...
            call test_residual_1(n_M_sample,ld_M,M_sample_,n_ctf ,ld_CTF
     $           ,CTF_k_p_,n_alpha,alpha1d_tru_ ,n_w_sum_ ,n_polar_a_
     $           ,quadrature_type_azimu_b,grid_k_p_,n_w_ ,n_k_1,n_k_cur
     $           ,n_Y_lm_sum_cur ,n_Y_lm_sum_ ,n_Y_l_ ,lsq_oversample
     $           ,lsq_interpolation_order,Y_tru_ ,M_residual_transform_
     $           ,Y_residual_slice_ ,M_residual_residual_
     $           ,n_residual_loading,n_residual_iteration
     $           ,M_residual_loading_)
            tmp_d_(0) = n_M_sample
            tmp_d_(1) = 1
            write(tmp_fname,'(A,I0,A)')
     $           './dir_mda11/I_model_sample_tru_',n_k_cur,'_.mda'
     ...
     $           ,trim(tmp_fname)
            ...
            tmp_d_(1) = n_M_sample
            write(tmp_fname,'(A,I0,A)')
     $           './dir_mda11/M_residual_loading_tru_',n_k_cur,'_.mda'
            if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : '
     $           ,trim(tmp_fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         if (flag_residual .and. flag_fig) then
            write(tmp_fname,'(A,I0)') './dir_jpg11/Fig_residual_tru_'
     $           ,n_k_cur
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         ...
         call test_alpha_update_2(verbose,n_omp_sub,fpm_howmany_max
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        ,grid_x3_c___,f_x_c_est___ ,ier)
         ...
         tmp_d_(1) = n_x_c_max
         tmp_d_(2) = n_x_c_max
         write(tmp_fname,'(A,I0,A)') './dir_mda11/f_x_c_est___',n_k_cur
     $        ,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : '
     $        ,trim(tmp_fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */         
         if (flag_residual) then
            ...
            call test_residual_1(n_M_sample,ld_M,M_sample_,n_ctf ,ld_CTF
     $           ,CTF_k_p_,n_alpha,alpha1d_bkp_ ,n_w_sum_ ,n_polar_a_
     $           ,quadrature_type_azimu_b,grid_k_p_,n_w_ ,n_k_1,n_k_cur
     $           ,n_Y_lm_sum_cur ,n_Y_lm_sum_ ,n_Y_l_ ,lsq_oversample
     $           ,lsq_interpolation_order,Y_est_ ,M_residual_transform_
     $           ,Y_residual_slice_ ,M_residual_residual_
     $           ,n_residual_loading,n_residual_iteration
     $           ,M_residual_loading_)
            tmp_d_(0) = n_M_sample
            tmp_d_(1) = 1
            write(tmp_fname,'(A,I0,A)')
     $           './dir_mda11/I_model_sample_est_',n_k_cur,'_.mda'
            if (verbose.gt.1) write(6,'(A,A)') 'Writing i4 : '
     $           ,trim(tmp_fname)
            call MDA_write_i4(2,tmp_d_,I_model_sample_,tmp_fname)
            tmp_d_(0) = n_residual_loading
            tmp_d_(1) = n_M_sample
            write(tmp_fname,'(A,I0,A)')
     $           './dir_mda11/M_residual_loading_est_',n_k_cur,'_.mda'
            if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : '
     $           ,trim(tmp_fname)
            call MDA_write_c16(2,tmp_d_,M_residual_loading_,tmp_fname)
         end if !if (flag_residual) then
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         if (flag_residual .and. flag_fig) then
            write(tmp_fname,'(A,I0)') './dir_jpg11/Fig_residual_est_'
     $           ,n_k_cur
            call Fig_R_ver0(n_k_cur,n_polar_a_,grid_k_p_,n_M_sample
     $           ,ld_M,M_sample_,M_residual_transform_,Y_residual_slice_
     $           ,M_residual_residual_,n_alpha ,alpha1d_bkp_,min(16
     $           ,n_M_sample) ,tmp_fname)
         end if !if (flag_residual .and. flag_fig) then
