...
n_w_ = max(6,ceil(2*pi*grid_k_p_)); %<-- not necessarily even-valued. ;
n_w_ = n_w_ + mod(n_w_,2); %<-- force each n_w_(1+nk_p_r) to be even. ;
n_A = sum(n_w_); n_w_max = max(n_w_);
...
% In the figures below the variables: ;
% E_S_zxy___, D_S_zxy___ and C_S_zxy___ each correspond to the innerproduct between S and M, 
% where S has been translated and *then* rotated. ;
% The variables: ;
% E_M_zxy___, D_M_zxy___ and C_M_zxy___ each correspond to the innerproduct between S and M, 
% where S has been rotated and *then* translated. ;
% The different arrays refer to different strategies for calculating these innerproducts.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
...
%%%%%%%%%%%%%%%% ;
E_S_zxy___ = zeros(n_gamma_z,n_delta_x,n_delta_y);
...
E_S_zxy___(1+ngz,1+ndx,1+ndy) = innerproduct_p(n_r,grid_k_p_,n_w_,n_A,T_k_q_,M_k_q_);
...
%%%%%%%%%%%%%%%% ;
D_S_zxy___ = zeros(n_gamma_z,n_delta_x,n_delta_y);
for ndx=0:n_delta_x-1; delta_x = delta_x_(1+ndx);
for ndy=0:n_delta_y-1; delta_y = delta_y_(1+ndy);
for ngz=0:n_gamma_z-1; gamma_z = gamma_z_(1+ngz);
T_k_p_ = S_k_p_;
...
D_S_zxy___(1+ngz,1+ndx,1+ndy) = innerproduct_p(n_r,grid_k_p_,n_w_,n_A,T_k_p_,M_k_p_);
end;%for ngz=0:n_gamma_z-1; gamma_z = gamma_z_(1+ngz);
end;%for ndy=0:n_delta_y-1; delta_y = delta_y_(1+ndy);
end;%for ndx=0:n_delta_x-1; delta_x = delta_x_(1+ndx);
%%%%%%%%%%%%%%%% ;
C_q_xyz__ = transpose(Z_S_svdd_)*Z_S_svds_;
C_S_zxy___ = zeros(n_gamma_z,n_delta_x,n_delta_y);
for ndx=0:n_delta_x-1; delta_x = delta_x_(1+ndx);
for ndy=0:n_delta_y-1; delta_y = delta_y_(1+ndy);
T_q_z_ = C_q_xyz__(1 + ndx + ndy*n_delta_x,:);
for ngz=0:n_gamma_z-1; gamma_z = gamma_z_(1+ngz);
C_S_zxy___(1+ngz,1+ndx,1+ndy) = interp1_0(n_w_max,0,2*pi,T_q_z_,gamma_z);
end;%for ngz=0:n_gamma_z-1; gamma_z = gamma_z_(1+ngz);
end;%for ndy=0:n_delta_y-1; delta_y = delta_y_(1+ndy);
end;%for ndx=0:n_delta_x-1; delta_x = delta_x_(1+ndx);
%%%%%%%%%%%%%%%% ;
E_M_zxy___ = zeros(n_gamma_z,n_delta_x,n_delta_y);
for ndx=0:n_delta_x-1; delta_x = delta_x_(1+ndx);
for ndy=0:n_delta_y-1; delta_y = delta_y_(1+ndy);
for ngz=0:n_gamma_z-1; gamma_z = gamma_z_(1+ngz);
T_k_q_ = S_k_q_;
T_k_q_ = rotate_q_to_q(n_r,n_w_,n_A,T_k_q_,gamma_z);
T_k_q_ = transf_svd_q_to_q(n_svd_r,svd_r_,n_svd_d,svd_d_,n_svd_l,svd_l_,svd_U_d_,svd_s_,svd_V_r_,n_r,grid_k_p_,n_w_,n_A,T_k_q_,delta_x,delta_y);
E_M_zxy___(1+ngz,1+ndx,1+ndy) = innerproduct_p(n_r,grid_k_p_,n_w_,n_A,T_k_q_,M_k_q_);
end;%for ngz=0:n_gamma_z-1; gamma_z = gamma_z_(1+ngz);
end;%for ndy=0:n_delta_y-1; delta_y = delta_y_(1+ndy);
end;%for ndx=0:n_delta_x-1; delta_x = delta_x_(1+ndx);
%%%%%%%%%%%%%%%% ;
D_M_zxy___ = zeros(n_gamma_z,n_delta_x,n_delta_y);
for ndx=0:n_delta_x-1; delta_x = delta_x_(1+ndx);
for ndy=0:n_delta_y-1; delta_y = delta_y_(1+ndy);
for ngz=0:n_gamma_z-1; gamma_z = gamma_z_(1+ngz);
T_k_p_ = S_k_p_;
T_k_p_ = rotate_p_to_p_lagrange(n_r,n_w_,n_A,T_k_p_,gamma_z);
T_k_p_ = transf_p_to_p(n_r,grid_k_p_,n_w_,n_A,T_k_p_,delta_x,delta_y);
D_M_zxy___(1+ngz,1+ndx,1+ndy) = innerproduct_p(n_r,grid_k_p_,n_w_,n_A,T_k_p_,M_k_p_);
end;%for ngz=0:n_gamma_z-1; gamma_z = gamma_z_(1+ngz);
end;%for ndy=0:n_delta_y-1; delta_y = delta_y_(1+ndy);
end;%for ndx=0:n_delta_x-1; delta_x = delta_x_(1+ndx);
%%%%%%%%%%%%%%%% ;
C_q_xyz__ = transpose(Z_M_svdd_)*Z_M_svds_;
C_M_zxy___ = zeros(n_gamma_z,n_delta_x,n_delta_y);
for ndx=0:n_delta_x-1; delta_x = delta_x_(1+ndx);
for ndy=0:n_delta_y-1; delta_y = delta_y_(1+ndy);
T_q_z_ = C_q_xyz__(1 + ndx + ndy*n_delta_x,:);
for ngz=0:n_gamma_z-1; gamma_z = gamma_z_(1+ngz);
C_M_zxy___(1+ngz,1+ndx,1+ndy) = interp1_0(n_w_max,0,2*pi,T_q_z_,gamma_z);
end;%for ngz=0:n_gamma_z-1; gamma_z = gamma_z_(1+ngz);
end;%for ndy=0:n_delta_y-1; delta_y = delta_y_(1+ndy);
end;%for ndx=0:n_delta_x-1; delta_x = delta_x_(1+ndx);
%%%%%%%%%%%%%%%%;
...
figure(5);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
subplot(2,1,1); set(gcf,'Position',1+[0,0,1024,768]);
hold on;
plot((E_S_zxy___(:)-C_S_zxy___(:))./abs(E_S_zxy___(:)),'gx');
plot((D_S_zxy___(:)-C_S_zxy___(:))./abs(D_S_zxy___(:)),'ms');
xlabel('real'); ylabel('imag'); legend({'E vs C','D vs C'});
title('relative error for C_S_zxy___','Interpreter','none');
subplot(2,1,2);
hold on;
plot((E_M_zxy___(:)-C_M_zxy___(:))./abs(E_M_zxy___(:)),'gx');
plot((D_M_zxy___(:)-C_M_zxy___(:))./abs(D_M_zxy___(:)),'ms');
xlabel('real'); ylabel('imag'); legend({'E vs C','D vs C'});
title('relative error for C_M_zxy___','Interpreter','none');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
disp('returning'); return;
end;%if flag_disp;
