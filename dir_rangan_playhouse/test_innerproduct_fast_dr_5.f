c$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
      program test_innerproduct_fast_dr_5
c$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
c$$$      This program is a driver which tests the subroutine
c$$$      test_innerproduct_fast_wrapper_4.f ,
...
c$$$         various images.
c$$$      D. Then we call test_innerproduct_fast_wrapper_4 to calculate
...
c$$$      D. The routine test_innerproduct_fast_wrapper_4 is used
...
c$$$      SM storage
      ...
      real *8, allocatable :: alpha_SM__(:) ! array of size n_alpha*n_SM_max*n_M storing the image-parameters for each stored tempc$$$      parameters for figure generation
...
c$$$      List of parameters: ;
      ...
      if (verbose.gt.0) then
          write(6,'(A)')
     $        '[entering test_innerproduct_fast_dr_5]: '
       end if
       if (verbose.gt.0) then
...
c$$$      defining displacement_max to be 3.0d0 wavelengths (can change later)
      ...
      if (verbose.gt.1) then
         write(6,'(A)') ' Calling memory_estimate'
      end if !if (verbose.gt.1) then
      n_M = 100000
      n_CTF = max(1,n_M/500)
      call test_innerproduct_memory_estimate_0(verbose+2,n_omp_sub
     $     ,ncur,n_M,n_CTF,n_alpha,n_pixels_in*max_x_c/2
     $     ,displacement_max,n_delta_x ,n_delta_y,n_gamma_z
     $     ,svd_calculation_type ,eps_svd ,fpm_howmany_max,n_SM_max)
...
c$$$      write(6,'(A)') ' test_innerproduct_fast_dr_5: '
...
c$$$      enddo
     ...
     $        ,' away from the true translations and rotations.'
      ...
      do nm=0,n_M-1
         ...
         write(6,'(4(A,I0))') ' setting alpha_est: nm ' , nm , ' ndx ' ,
     $        ndx , ' ndy ' ,ndy , ' ngz ' , ngz 
...
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      ...
      call cl1_r8(n_alpha*n_M,alpha_tru_)
      allocate(alpha_est_(0:n_alpha*n_M-1))
      call cl1_r8(n_alpha*n_M,alpha_est_)
      do nm=0,n_M-1
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      ...
      do ns=0,n_S-1
         do nm=0,n_M-1
            nctf = I_ctf_(nm)
            call test_innerproduct_bruteforce_0(verbose+2 ,n_r
     $           ,grid_k_p_,n_w_ ,max_x_c,S_k_p_(n_A*ns),M_k_p_(n_A*nm)
     $           ,CTF_k_p_(n_A*nctf) , delta_x_est_(nm),delta_y_est_(nm)
     $           ,gamma_z_est_(nm) ,n_delta_x,delta_x_ ,n_delta_y
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $     *n_S,') innerproducts'
      timing_tic = omp_get_wtime()
      call test_innerproduct_fast_wrapper_4(verbose,rseed,n_omp_sub,n_r
     $     ,n_polar_a_,grid_k_p_,max_x_c,n_S,I_S_sample_,n_A ,S_k_p_
     $     ,tesselation_distance_req ,alpha_polar_a_,alpha_azimu_b_ ,n_M
     $     ,I_M_sample_,n_A,M_k_p_ ,n_CTF,n_A ,CTF_k_p_ ,alpha_est_
     ...
     $     ,C_M_)
      timing_toc = omp_get_wtime()
      if (verbose.gt.0) then
         write(6,'(A,A,F8.3)') 'test_innerproduct_fast_wrapper_4:'
     $        ,' total_time ',timing_toc-timing_tic
      end if
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
 ...
 10    continue
      ...
      if (verbose.gt.1) then
         write(6,'(A)') '[finished test_innerproduct_fast_dr_5]'
      end if
