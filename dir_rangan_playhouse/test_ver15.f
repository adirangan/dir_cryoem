...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
c$$$      Generally speaking, this driver first generates three molecules ;
c$$$      (i.e., molecules-A,B,C) which are rather similar. ;
c$$$      Using these three molecules, we produce a set of 'slices', some from ;
c$$$      molecule-A, some from B, some from C. ;
...
c$$$      We also distort these images by adding gaussian noise (iid per mode). ;
...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
c$$$      try: ;
c$$$      make -f test_ver15.make ; ./test_ver15.out 1 0 0 0 100.0d0 1 ; ./test_ver15.out 1 0 0 0 10.0d0 1 ; ./test_ver15.out 1 0 0 0 1.0d0 1 ; ./test_ver15.out 1 0 0 0 0.1d0 1 ;
c$$$      command line arguments: A,B,C,D,snr,rseed <-- relative abundance of molecules A,B,C and D, as well as snr (signal-to-noise) and random seed. ;
c$$$      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
      implicit none
      include 'omp_lib.h'
c$$$      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
      ...
      parameter (n_k_p_max=16) ! maximum number of k-values (in k-space polar coordinates). sometimes named ngridr. ;
...
c$$$      parameter (snr=10.0d0)
      ...
      integer n_omp_sub__in !integer *4: number of omp sub-blocks (e.g., number of available processors). ;
      parameter (n_omp_sub__in=8)
      integer n_S_0_sub__in !integer *4: number of requested sub-blocks at level-0 for n_S (used for O_S_q__, T_S_q__, Z_S_q__). ;
c$$$      parameter (n_S_0_sub__in=2)
      integer n_S_1_sub__in !integer *4: number of requested sub-blocks at level-1 for n_S (used for S_T_T_R_CTF_M_q__, S_Z_T_R_CTF_M_q__). ;
c$$$      parameter (n_S_1_sub__in=3)
      integer n_M_0_sub__in !integer *4: number of requested sub-blocks at level-0 for n_M (used for O_T_R_CTF_M_q__, T_T_R_CTF_M_q__, Z_T_R_CTF_M_q__). ;
c$$$      parameter (n_M_0_sub__in=3)
      integer n_M_1_sub__in !integer *4: number of requested sub-blocks at level-1 for n_M (used for S_T_T_R_CTF_M_q__, S_Z_T_R_CTF_M_q__). ;
c$$$      parameter (n_M_1_sub__in=4)
      real *8 d_memory_limit !real *8: upper limit on the memory allowed by test_innerproduct_7. ;
...
c-----------------------------------------------------------------------
      ...
      allocate(n_Y_lm_sum_(0:1+n_k_p_max-1))
      call cs1_i4(n_k_p_max,n_Y_lm_sum_)
      ...
      allocate(n_w_sum_(0:1+n_k_p_max-1))
      call cs1_i4(n_k_p_max,n_w_sum_)
...
c
      ...
      allocate(Y_A_(0:1+n_Y_lm_sum_max-1))
      n = n_Y_lm_sum_max
      call cs1_c16(n,Y_A_)
      if (flag_heterogeneity) then
         allocate(Y_B_(0:1+n_Y_lm_sum_max-1))
         n = n_Y_lm_sum_max
         call cs1_c16(n,Y_B_)
         allocate(Y_C_(0:1+n_Y_lm_sum_max-1))
         n = n_Y_lm_sum_max
         call cs1_c16(n,Y_C_)
      end if !if (flag_heterogeneity) then
      allocate(Y_tru_(0:1+n_Y_lm_sum_max-1))
      n = n_Y_lm_sum_max
      call cs1_c16(n,Y_tru_)
      allocate(Y_est_(0:1+n_Y_lm_sum_max-1))
      n = n_Y_lm_sum_max
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        ,n_ctf ,ld_CTF,CTF_k_p__,alpha_tru__ ,n_w_sum_
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */         
         if (flag_residual) then
            ...
            call test_residual_4(rseed,n_M_sample,I_M_sample_,ld_M
...
c$$$      svd_calculation_type == 2 --> account for displacements via brute-force. ;
     ...
     $      ,d_memory_limit !real *8: upper limit on the memory allowed by test_innerproduct_7. ;
     $      ,n_w_sum_ ! cumulative sum of n_w_. also called icstart(nk). ;
     ...
     $      ,n_Y_lm_sum_max ! total number of points used in spherical harmonic expansion (in k-space polar coordinates). sometimes called nsphstore. ;
     $      ,n_Y_lm_sum_ ! cumulative sum of n_Y_lm_. also called isph_start(nk). ;
...
c$$$            call test_residual_3(rseed,n_M_sample,I_M_sample_,ld_M
c$$$     $           ,Y_slice__,n_ctf,ld_CTF,CTF_k_p__,alpha_bkp__
c$$$     $           ,n_w_sum_,n_polar_a_,quadrature_type_azimu_b,grid_k_p_
c$$$     $           ,n_w_,n_k_1,n_k_cur,n_Y_lm_sum_cur ,n_Y_lm_sum_ ,n_Y_l_
c$$$     $           ,lsq_oversample,lsq_interpolation_order,Y_est_
c$$$     $           ,M_residual_sample__,n_residual_loading
c$$$     $           ,n_residual_iteration,M_residual_loading_)
            tmp_d_(0) = n_M_sample
            tmp_d_(1) = 1
            write(tmp_fname,'(A,A,I0,A)') trim(dir_base) ,
