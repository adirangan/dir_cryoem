C***********************************************************************
      subroutine test_residual_0(n_M,ld_M,M_k_p__,n_CTF,ld_CTF
     $     ,CTF_k_p_,n_alpha,alpha2d__,n_w_sum_,n_polar_a_
     $     ,quadrature_type_azimu_b ,grid_k_p_ ,n_w_,n_k_low,n_k_cur
     $     ,n_Y_lm_sum_cur,n_Y_lm_sum_ ,n_Y_l_ ,lsq_oversample
     $     ,lsq_interpolation_order ,Y_ ,M_transform__
     $     ,Y_slice__ ,M_residual__ ,n_M_loading ,M_loading_)
C***********************************************************************
...
C***********************************************************************
      ...
      integer n_M,ld_M,n_CTF,ld_CTF
      complex *16 M_k_p__(0:ld_M-1,0:n_M-1)
...
c$$$      The vector H is a temporary variable storing Et_{nM}*R_{nM}. ;
      ...
      complex *16, allocatable :: H_(:) !temporary: stores single Et*R (in terms of spherical harmonic coefficients). ;
      complex *16 HH,HG,RR
      integer n_iteration,niteration
      external multaha
      character(len=1024) format_string
      if (verbose.gt.0) then
         write(6,*) '[entering test_residual_0]'
      end if
...
c
     ...
     $              ,n_w_sum_(nk)
            ...
            call get_lsqdata_0(1,ld_M,M_k_p__(0,nM),1 ,ld_CTF
     $           ,CTF_k_p_(nctf*ld_CTF),grid_k_p_(nk),n_w_sum_(nk)
     $           ,n_w_(nk),n_alpha,alpha2d__(0,nM)
     $           ,M_transform__(n_w_sum_(nk)-1,nM)
     $           ,polar_a__(n_w_sum_(nk)-1,nM)
     $           ,azimu_b__(n_w_sum_(nk)-1,nM)
     $           ,weight_CTF_k_c__(n_w_sum_(nk)-1,nM) ,n_w_M_(nk))
         enddo                  ! do nk = n_k_low-1,n_k_cur-1
         if (verbose.gt.2) then
            write(format_string,'(A,I0,A)') '(A,' , ld_M ,
     $           '(2F8.4,1X))'
            write(6,format_string) 'M_k_p__: ' , (M_k_p__(ntmp,nM)
     $           ,ntmp=0,ld_M-1)
     ...
     $                 n_M_loading*(nM)),cmplx(0.0d0,0.0d0),H_,G_(0
     $                 +nM_loading*n_Y_lm_sum_cur))
         ...
         do nM=0,n_M-1
            do nk = n_k_low-1,n_k_cur-1
               ...
               n_polar_a_oversample = nint(lsq_oversample
     $              *n_polar_a_(nk))
               call test_residual_multah(polar_a__(n_w_sum_(nk)-1,nM)
     $              ,azimu_b__(n_w_sum_(nk)-1,nM),n_w_M_(nk)
     $              ,M_residual__(n_w_sum_(nk)-1,nM),n_Y_l
     $              ,n_polar_a_oversample ,lsq_interpolation_order
     $              ,weight_CTF_k_c__(n_w_sum_(nk)-1,nM)
     $              ,H_(n_Y_lm_sum_(nk)))
            enddo               ! do nk = n_k_low-1,n_k_cur-1
            call dot_c16(n_Y_lm_sum_cur,H_,H_,HH)
            do nM_loading=0,n_M_loading-1
               call dot_c16(n_Y_lm_sum_cur,H_,G_(0+nM_loading
     $              *n_Y_lm_sum_cur),HG)
