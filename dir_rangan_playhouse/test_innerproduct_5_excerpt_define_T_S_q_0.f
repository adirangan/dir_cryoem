     ...
     $           ,'n_delta_x*(ndy + n_delta_y*(ns + nS_0_per*nw)))) '
...
c$OMP DO 
     ...
     $              ,' with thread: ',omp_get_thread_num()
            ...
            call test_innerproduct_5_T_S_q_3(verbose-1,n_delta_x
...
c$OMP END PARALLEL
     ...
     $           *n_delta_x*n_delta_y*nS_9_sum))
         ...
         gnump_tot = (n_A*1.0d0)*(n_delta_x*1.0d0)*(n_delta_y*1.0d0)
     $        *(nS_0_per*1.0d0)
