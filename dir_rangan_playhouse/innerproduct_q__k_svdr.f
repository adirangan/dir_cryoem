      subroutine innerproduct_q__k_svdr(n_svd_r,svd_r_,n_svd_l,svd_l_
...
c$$$      The mode-k for term-l is stored in C_q_(l + k*n_svd_l).
      ...
      if (verbose.gt.0) then
         write(6,'(A,I0)') ' % [entering innerproduct_q__k_svdr] n_r '
     $        ,n_r
      end if
      pi = 4.0*atan(1.0)
     ...
     $        ,svd_r_max,' in innerproduct_q__k_svdr'
      end if
      n_w_max = n_w_(n_r-1)
      if (verbose.gt.0) then
...
c$$$         We assume that the fourier basis is orthonormal (not merely orthogonal)
     ...
     $                    ,'; nw_fix ',nw_fix,'; (full orig)'
      ...
      enddo
      if (verbose.gt.0) then
         write(6,'(A)') ' % [finished innerproduct_q__k_svdr]'
      end if
      end
