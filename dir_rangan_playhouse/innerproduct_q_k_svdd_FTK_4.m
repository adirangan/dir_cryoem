function [C_q_] = innerproduct_q_k_svdd_FTK_4(flag_S_vs_M,FTK);
...
if (verbose>0);
disp(sprintf(' %% [entering innerproduct_q_k_svdd_FTK_4] %d %d %d',n_svd_d,n_svd_l,n_delta_v));
end;%if (verbose>0);
l_max = max(svd_l_);
...
disp(sprintf(' %% Warning, delta %0.6f > svd_d_max %0.6f in innerproduct_q_k_svdd_FTK_4',delta,svd_d_max));
...
U_d_ = svd_polyval_U_d_(1+ndv*n_svd_l+(0:n_svd_l-1));
for nl=0:n_svd_l-1;
I_l = svd_l_(1+nl);
if (abs(I_l)>l_max);
disp(sprintf(' %% Warning, I_l %d > l_max in innerproduct_q_k_svdd_FTK_4',I_l));
...
if (verbose>0);
disp(sprintf(' %% [finished innerproduct_q_k_svdd_FTK_4]'));
end;%if (verbose>0);
