      subroutine test_innerproduct_fast_T_R_CTF_M_q_1(verbose
     $     ,delta_x_est_,delta_y_est_,gamma_z_est_,ctf_ind_est_
     $     ,fftw_plan_frwd_ ,fftw_in1_,fftw_out_,n_r,grid_p_,n_w_,n_A
     $     ,M_p_ ,M_q_,n_M_sub,ld_M ,M_p__,CTF_p_,n_CTF,ld_CTF,CTF_p__
     $     ,C_M_ ,n_M_tot,T_R_CTF_M_q__)
c$$$      %%%%%%%%%%%%%%%%%%%%%%%
...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%
      ...
      integer verbose
      integer n_r,n_w_(0:n_r-1),n_A,n_M_sub,n_M_tot,ld_M,n_CTF,ld_CTF
      ...
      if (verbose.gt.0) then
         write(6,'(A)')
     $        '[entering test_innerproduct_fast_T_R_CTF_M_q_1]'
     ...
     $           ,Z_tmp)
      ...
      if (verbose.gt.0) then
         write(6,'(A)')
     $        '[finished test_innerproduct_fast_T_R_CTF_M_q_1]'
      end if !if (verbose.gt.0) then
      end
