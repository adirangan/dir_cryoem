      subroutine test_innerproduct_5_T_T_R_CTF_M_q_1(verbose
     $     ,n_delta_x,delta_x_,n_delta_y,delta_y_,delta_x_est_
     $     ,delta_y_est_,gamma_z_est_,ctf_ind_est_,fftw_plan_frwd_
     $     ,fftw_in1_,fftw_out_,n_r,grid_p_,n_w_,n_A,Z_p_,M_p_ ,M_q_
     $     ,n_M_sub ,I_M_sample_,ld_M ,M_p__,CTF_p_,n_CTF,ld_CTF
...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%
     ...
     $     ,n_M_tot,ld_M,n_CTF,ld_CTF
      ...
      if (verbose.gt.0) then
         write(6,'(A)')
     $        '[entering test_innerproduct_5_T_T_R_CTF_M_q_1]'
     ...
     $           ,Z_tmp)
         ...
         call rotate_p2p_fz(n_r,n_w_,n_A,Z_p_,-gamma_z_est,Z_p_)
         if (verbose.gt.2) then
            write(6,'(A,I0,A,I0,A,I0)') ' nm: ' , nm 
     ...
     $                 ,Z_tmp)
      ...
      if (verbose.gt.0) then
         write(6,'(A)')
     $        '[finished test_innerproduct_5_T_T_R_CTF_M_q_1]'
      end if !if (verbose.gt.0) then
      end
