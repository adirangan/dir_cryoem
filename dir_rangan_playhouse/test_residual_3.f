C***********************************************************************
      subroutine test_residual_3(rseed,n_M,I_M_sample_,ld_M,M_k_p__
     $     ,n_CTF,ld_CTF,CTF_k_p_,alpha2d__,n_w_sum_,n_polar_a_
...
c$$$      complex *16, allocatable :: H_(:) !temporary: stores single Et*R (in terms of spherical harmonic coefficients). ;
      ...
      if (verbose.gt.0) then
         write(6,*) '[entering test_residual_3]'
      end if
...
c
     ...
     $              ,n_w_sum_(nk)
            ...
            call get_lsqdata_0(1,ld_M,M_k_p__(I_M_sample_(nM)*ld_M),1
     $           ,ld_CTF,CTF_k_p_(nctf*ld_CTF),grid_k_p_(nk)
     $           ,n_w_sum_(nk),n_w_(nk),n_alpha,alpha2d__(0,nM)
     $           ,M_transform_(n_w_sum_(nk)-1),polar_a__(n_w_sum_(nk)-1
     $           ,nM),azimu_b__(n_w_sum_(nk)-1,nM)
     ...
     $           ,ntmp=0,ld_M-1)
         ...
         do nk=max(2,n_k_low-1),n_k_cur-1
            do nw=0,n_w_(nk)-1
               M_residual__(n_w_sum_(nk)-1+nw + nM*ld_M) =
     $              M_transform_(n_w_sum_(nk)-1+nw)
     $              -Y_slice_(n_w_sum_(nk)-1+nw)
...
c$$$      allocate(H_(0:n_Y_lm_sum_cur-1))
     ...
     $           ,M_residual__(n_w_sum_(nk)-1+nM*ld_M),n_Y_l
     ...
     $           ,H__(n_Y_lm_sum_(nk) + nM*n_Y_lm_sum_cur))
      ...
      allocate(G_(0:n_residual_loading*n_Y_lm_sum_cur-1))
     ...
     $              *n_Y_lm_sum_cur))
            enddo               !do nM_loading=0,n_residual_loading*n_Y_lm_sum_cur-1
         end if                 !if (niteration.eq.0) then
         if (niteration.gt.0) then
            call cl1_c16(n_Y_lm_sum_cur*n_residual_loading,G_)
            do nM=0,n_M-1
               do nM_loading=0,n_residual_loading-1
                  call ac1_c16(n_Y_lm_sum_cur,M_loading_(nM_loading +
     $                 n_residual_loading*(nM)),cmplx(0.0d0,0.0d0),H__(0
     $                 + nM *n_Y_lm_sum_cur),G_(0 +nM_loading
     $                 *n_Y_lm_sum_cur))
               enddo            !do nM_loading=0,n_residual_loading-1
            enddo               ! do nM=0,n_M-1
            call gramschmidt_c16(n_Y_lm_sum_cur,n_residual_loading,G_)
