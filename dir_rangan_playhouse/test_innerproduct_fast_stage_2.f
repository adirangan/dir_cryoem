c$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
      subroutine test_innerproduct_fast_stage_2(verbose,rseed,n_omp_sub
     $     ,svd_calculation_type,eps_svd,n_r,grid_p_,n_w_
     $     ,half_diameter_x_c,n_S,I_S_sample_,ld_S,S_p__
     ...
     $     ,T_root_base_ ,n_M,I_M_sample_,ld_M,M_p__ ,n_CTF ,ld_CTF
     $     ,CTF_p__,polar_a_est_ ,azimu_b_est_ ,gamma_z_est_
...
c$$$      parameters for memory map
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[entering test_innerproduct_fast_stage_2]'
      end if      
...
c$OMP DO 
     ...
     $        ,fftw_plan_frwd__(n_r_tmp),fftw_in1__(n_A_tmp)
     $        ,fftw_out__(n_A_tmp) ,n_r ,grid_p_,n_w_,n_A
     $        ,M_p_omp__(n_A_tmp),M_q_omp__(n_A_tmp),nM_per,I_M_sample_
     $        ,IM_per,ld_M ,M_p__,CTF_p_omp__(n_A_tmp)
...
c$OMP DO 
      do nM_sub=0,n_M_sub_use-1
         ...
         call test_innerproduct_fast_ZZ_2(verbose-1,rseed,n_delta_x
     $        ,delta_x_,n_delta_y,delta_y_,n_gamma_z,gamma_z_,n_r
     $        ,grid_p_,n_w_,n_A ,n_S,S_alpha_polar_a_,S_alpha_azimu_b_
     $        ,nM_per,polar_a_est_(IM_per) ,azimu_b_est_(IM_per)
...
c$$$         call dfftw_destroy_plan_(fpm_frwd_(nomp_sub))
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[finished test_innerproduct_fast_stage_2]'
      end if
