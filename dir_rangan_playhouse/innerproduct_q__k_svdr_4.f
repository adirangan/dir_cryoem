      subroutine innerproduct_q__k_svdr_4(flag_S_vs_M,svd_r_max,n_svd_r
...
c$$$      flag_S_vs_M .eqv. .false. --> transformation applied to M, use -.
      ...
      if (verbose.gt.0) then
         write(6,'(A,I0)') ' % [entering innerproduct_q__k_svdr_4] n_r '
     $        ,n_r
      end if
      pi = 4.0*atan(1.0)
...
c$$$         We assume that the fourier basis is orthonormal (not merely orthogonal)
     ...
     $                 *T_q_(ic))*M_q_(ict)
      ...
      enddo
      if (verbose.gt.0) then
         write(6,'(A)') ' % [finished innerproduct_q__k_svdr_4]'
      end if
      end
