      subroutine test_innerproduct_5_Zstore_1b(
     $     verbose
     $     ,rseed
     $     ,flag_RTRT_vs_RTTR
     $     ,n_delta_x
...
c$$$      MS storage
      ...
      integer nx2,nx3,nx2_stride,nx3_stride
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[entering test_innerproduct_5_Zstore_1b]'
...
c$$$     $           n_gamma_z*(nctf + n_CTF*ns)),CTF_R_S_use_)
            ...
            do ngz=0,n_gamma_z-1
               CTF_R_S_use_(ngz) = cmplx( 1.0d0 , 0.0d0 )
...
c$$$c$$$            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c$$$            call test_innerproduct_5_ZZ_scan_2(flag_RTRT_vs_RTTR
c$$$     $           ,delta_x_est_(nm),delta_y_est_(nm),gamma_z_est_(nm)
c$$$     $           ,displacement_max,n_delta_x ,delta_x_ ,n_delta_y
c$$$     $           ,delta_y_,n_gamma_z,gamma_z_,ZZ_ ,ndx_optimal
c$$$     $           ,ndy_optimal ,ngz_optimal,C_Z_optimal)
c$$$            delta_x_optimal = delta_x_(ndx_optimal)
c$$$            delta_y_optimal = delta_y_(ndy_optimal)
...
c$$$            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[finished test_innerproduct_5_Zstore_1b]'
      end if !if (verbose.gt.0) then
