...
c$$$      complex *16, allocatable :: C_M_(:) !temporary: estimated l2-norm associated each sampled image. ;
c$$$      real *8, allocatable :: S_alpha_polar_a_all_(:) !temporary: array of polar_a associated with templates (used for updating image parameters). ;
c$$$      real *8, allocatable :: S_alpha_azimu_b_all_(:) !temporary: array of azimu_b associated with templates (used for updating image parameters). ;
c$$$      real *8, allocatable :: S_alpha_polar_a_sample_(:) !temporary: array of polar_a associated with sampled templates (used for updating image parameters). ;
c$$$      real *8, allocatable :: S_alpha_azimu_b_sample_(:) !temporary: array of azimu_b associated with sampled templates (used for updating image parameters). ;
c$$$      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
c$$$      real *8, allocatable :: delta_x_optimal_(:) !temporary: array of size n_S_sample * n_M_sample; stores the optimal value of image-parameter delta_x (horiontal translation). ;
c$$$      real *8, allocatable :: delta_y_optimal_(:) !temporary: array of size n_S_sample * n_M_sample; stores the optimal value of image-parameter delta_y (vertical translation). ;
c$$$      real *8, allocatable :: gamma_z_optimal_(:) !temporary: array of size n_S_sample * n_M_sample; stores the optimal value of image-parameter gamma_z (in-plane rotation, or third-euler-angle). ;
c$$$      complex *16, allocatable :: C_S_optimal_(:) !temporary: array of size n_S_sample * n_M_sample; stores the optimal value of template-l2-norm || CTF .* R(S) || associated with each of the sampled templates and each of the sampled images. Note that, in this expression, the CTF is associated with the image, and the rotation R is associated with the estimated image-parameter gamma_z. ;
c$$$      complex *16, allocatable :: C_Z_optimal_(:) !temporary: array of size n_S_sample * n_M_sample; stores the optimal value of innerproduct associated with each of the sampled templates and each of the sampled images. ;
c$$$      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
c$$$      real *8, allocatable :: delta_x_sort_SM_(:)
c$$$      real *8, allocatable :: delta_y_sort_SM_(:)
c$$$      real *8, allocatable :: gamma_z_sort_SM_(:)
c$$$      complex *16, allocatable :: C_S_sort_SM_(:)
c$$$      complex *16, allocatable :: C_Z_sort_SM_(:)
c$$$      integer *4, allocatable :: I_permute_SM_(:)
c$$$      integer *4, allocatable :: I_inverse_SM_(:)
c$$$      real *8, allocatable :: delta_x_sort_MS_(:)
c$$$      real *8, allocatable :: delta_y_sort_MS_(:)
c$$$      real *8, allocatable :: gamma_z_sort_MS_(:)
c$$$      complex *16, allocatable :: C_S_sort_MS_(:)
c$$$      complex *16, allocatable :: C_Z_sort_MS_(:)
c$$$      integer *4, allocatable :: I_permute_MS_(:)
c$$$      integer *4, allocatable :: I_inverse_MS_(:)
...
c     create physical space grid and sample function_A on it.
     ...
     $     ,f_x_c_A___ ,f_x_c_l2_norm)
      ...
      tmp_d_(1) = n_x_c_max
      tmp_d_(2) = n_x_c_max
      write(tmp_fname,'(A)') './dir_mda8/grid_x1_c___.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(tmp_fname)
      call MDA_write_r8(3,tmp_d_,grid_x1_c___,tmp_fname)
      write(tmp_fname,'(A)') './dir_mda8/grid_x2_c___.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(tmp_fname)
      call MDA_write_r8(3,tmp_d_,grid_x2_c___,tmp_fname)
      write(tmp_fname,'(A)') './dir_mda8/grid_x3_c___.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(tmp_fname)
      call MDA_write_r8(3,tmp_d_,grid_x3_c___,tmp_fname)
      write(tmp_fname,'(A)') './dir_mda8/f_x_c_A___.mda'
...
c     create physical space grid and sample function_B on it.
     ...
     $     ,f_x_c_B___ ,f_x_c_l2_norm)
      ...
      tmp_d_(1) = n_x_c_max
      tmp_d_(2) = n_x_c_max
      write(tmp_fname,'(A)') './dir_mda8/f_x_c_B___.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : '
     $     ,trim(tmp_fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      write(tmp_fname,'(A)') './dir_jpg8/Fig_ctf_c'
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         tmp_d_(0) = n_alpha
         tmp_d_(1) = n_M_sample
         write(tmp_fname,'(A,I0,A)') './dir_mda8/alpha1d_tru_',n_k_cur
     $        ,'_.mda'
     ...
     $        ,trim(tmp_fname)
         ...
         tmp_d_(0) = n_alpha
         tmp_d_(1) = n_M_sample
         write(tmp_fname,'(A,I0,A)') './dir_mda8/alpha1d_est_',n_k_cur
     $        ,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : '
     $        ,trim(tmp_fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         write(tmp_fname,'(A,I0)') './dir_jpg8/Fig_MNC_c_',n_k_cur
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         ...
         call test_alpha_update_0(verbose,half_diameter_x_c_tmp
     ...
     $        ,flag_tesselation,tesselation_distance_req, .true.
     $        ,tmp_dname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        ,grid_x3_c___,f_x_c_est___ ,ier)
         ...
         tmp_d_(1) = n_x_c_max
         tmp_d_(2) = n_x_c_max
         write(tmp_fname,'(A,I0,A)') './dir_mda8/f_x_c_est___',n_k_cur
     $        ,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : '
     $        ,trim(tmp_fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         if (flag_residual) then
            write(tmp_fname,'(A,I0)') './dir_jpg8/Fig_residual_est_'
     $           ,n_k_cur
