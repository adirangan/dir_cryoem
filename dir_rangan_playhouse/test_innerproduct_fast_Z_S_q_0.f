...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%
      ...
      integer n_r,n_w_(0:n_r-1),n_A,n_S,ld_S
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[entering test_innerproduct_fast_Z_S_q_0]'
...
c$$$  We assume that the fourier basis is orthonormal (not merely orthogonal)
     ...
     $                 ' % % l I_l D_V_r D_s: ',nl,I_l,D_V_r,D_s
               ...
               do nw=0,n_w_(nr)-1
                  ...
                  if (verbose.gt.3 .and. nr.lt.5) then
                     write(6,'(A,I0,1X,I0,1X,I0)') ' % % % nw I_l nwt: '
     $                    ,nw,I_l,nwt
     ...
     $                    ld_X*nw_C) +C_q*dsqrt_dAn
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[finished test_innerproduct_fast_Z_S_q_0]'
      end if !if (verbose.gt.0) then
      end
