      subroutine innerproduct_q_k_svdd_1(flag_S_vs_M,n_svd_d,svd_d_
...
c$$$      l_max must equal or exceed order of bessel-function expansion
     ...
     $        ' % [entering innerproduct_q_k_svdd_1] ',n_svd_d,n_svd_l
     $        ,n_delta_x,n_delta_y
      end if
      pi = 4.0*atan(1.0)
      nC = 0
...
c$$$            theta = nl_pre*(pi/2 - omega).
     ...
     $                 ,'>l_max in innerproduct_q_k_svdd_1'
               end if
...
c$$$               nC = l + ndx*n_svd_l + ndy*n_delta_x*n_svd_l
      ...
      enddo
      if (verbose.gt.0) then
         write (6,'(A)') ' % [finished innerproduct_q_k_svdd_1]'
      end if
      end
