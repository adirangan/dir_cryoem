      subroutine test_innerproduct_fast_Z_S_q_2(verbose,n_svd_r,svd_r_
     $     ,n_svd_l,svd_l_,svd_s_,svd_V_r_,fftw_plan_frwd_,fftw_in1_
     $     ,fftw_out_ ,n_r,grid_p_,n_w_,n_A,S_p_,S_q_,n_S_sub
     $     ,I_S_sample_,IS_per,ld_S,S_p__ ,n_S_tot,Z_S_q__)
c$$$      %%%%%%%%%%%%%%%%%%%%%%%
...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%
     ...
     $     ,n_S_tot,ld_S
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[entering test_innerproduct_fast_Z_S_q_2]'
...
c$$$  We assume that the fourier basis is orthonormal (not merely orthogonal)
     ...
     $                    ld_X*nw_C) +C_q*dsqrt_dAn
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[finished test_innerproduct_fast_Z_S_q_2]'
      end if !if (verbose.gt.0) then
      end
