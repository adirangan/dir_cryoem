      subroutine Fig_R_ver0(ncur,n_polar_a_,grid_k_p_,n_M ,ld_M,N_k_p_
     $     ,P_k_p_,Q_k_p_,R_k_p_,n_alpha,alpha_est_ ,n_M_sample,prefix)
c$$$      Reads in a few images and ctransf and plots a comparison
      ...
      integer *4 n_M,ld_M,n_alpha,n_M_sample
      real *8 grid_k_p_(0:0)
      real *8 alpha_est_(0:0)
      include 'nalpha_define.f'
      complex *16 P_k_p_(0:0),Q_k_p_(0:0),R_k_p_(0:0),N_k_p_(0:0)
...
c$$$         Display original image with ctf, transformed
         ...
         call cp1_c16(n_A,N_k_p_(nm*ld_M),T_k_p_)
...
c$$$         Display corresponding ctransf (calculated via get_lsqdata)
         call stdlim_c16(n_A,.true.,P_k_p_(nm*ld_M),F_avg,F_std)
         F_max = F_avg + 2.5*F_std
         F_min = F_avg - 2.5*F_std
         x_offset = +5.0d0 + 10.0d0*nM_sample
         y_offset = +17.0d0 - 0.0d0 + 22.5d0
         ...
         y_side = 10.0d0/(max_k_c)
         call Fig_c16_polar(unitnumber,n_r,grid_k_p_,n_w_,.true.
     $        ,P_k_p_(nm*ld_M),F_min,F_max,x_offset,y_offset,x_side
     $        ,y_side)
         write(fig_title,'(A,I0,A,I4,A)') 'R(P_k_p(',nm,')) '
     $        ,nint(dlog(F_std)),'    '
         text_length = 18
         color_index = 0
         font_type = 0
     ...
     $        ,font_type,font_size,x_loc,y_loc,x_offset,y_offset,x_side
     $        ,y_side)
         call stdlim_c16(n_A,.false.,P_k_p_(nm*ld_M),F_avg,F_std)
         F_max = F_avg + 2.5*F_std
         F_min = F_avg - 2.5*F_std
         x_offset = +5.0d0 + 10.0d0*nM_sample
         y_offset = +17.0d0 - 10.0d0 + 22.5d0
     ...
     $        ,P_k_p_(nm*ld_M),F_min,F_max,x_offset,y_offset,x_side
     $        ,y_side)
         write(fig_title,'(A,I0,A,I4,A)') 'I(P_k_p(',nm,')) '
     $        ,nint(dlog(F_std)),'    '
         text_length = 18
         color_index = 0
         font_type = 0
...
c$$$         Display corresponding mslices (calculated via test_resid_multa)
         call stdlim_c16(n_A,.true.,Q_k_p_(nm*ld_M),F_avg,F_std)
         F_max = F_avg + 2.5*F_std
         F_min = F_avg - 2.5*F_std
         x_offset = +5.0d0 + 10.0d0*nM_sample
         ...
         y_side = 10.0d0/(max_k_c)
         call Fig_c16_polar(unitnumber,n_r,grid_k_p_,n_w_,.true.
     $        ,Q_k_p_(nm*ld_M),F_min,F_max,x_offset,y_offset,x_side
     $        ,y_side)
         write(fig_title,'(A,I0,A,I4,A)') 'R(Q_k_p(',nm,')) '
     $        ,nint(dlog(F_std)),'    '
         text_length = 18
         color_index = 0
         font_type = 0
     ...
     $        ,font_type,font_size,x_loc,y_loc,x_offset,y_offset,x_side
     $        ,y_side)
         call stdlim_c16(n_A,.false.,Q_k_p_(nm*ld_M),F_avg,F_std)
         F_max = F_avg + 2.5*F_std
         F_min = F_avg - 2.5*F_std
         x_offset = +5.0d0 + 10.0d0*nM_sample
     ...
     $        ,Q_k_p_(nm*ld_M),F_min,F_max,x_offset,y_offset,x_side
     $        ,y_side)
         write(fig_title,'(A,I0,A,I4,A)') 'I(Q_k_p(',nm,')) '
     $        ,nint(dlog(F_std)),'    '
         text_length = 18
         color_index = 0
         font_type = 0
