...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
      ...
      parameter (n_k_p_max=20) ! maximum number of k-values (in k-space polar coordinates). sometimes named ngridr. ;
...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
      ...
      real *8 eps !temporary: tolerance epsilon, typically 1e-6. used in many places (e.g., least-squares-solver). ;
...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
      ...
      integer, allocatable :: n_azimu_b_tmp_(:) !temporary: similar to n_azimu_b_. ;
      ...
      real *8, allocatable :: grid_k_p_tmp_(:) ! similar to grid_k_p_. ;
      real *8, allocatable :: grid_cos_polar_a_tmp_(:) ! similar to grid_cos_polar_a_. 
      real *8, allocatable :: grid_sin_polar_a_tmp_(:) ! similar to grid_sin_polar_a_. ;
      real *8, allocatable :: weight_cos_polar_a_tmp_(:) ! similar to weight_cos_polar_a_. ;
      real *8, allocatable :: azimu_b_step_tmp_(:) !temporary: similar to azimu_b_step_. ;
...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
c$$$      The following '*_optimal_' arrays are all temporaray, ;
...
c     create physical space grid and sample function_A on it.
     ...
     $     ,f_x_c_A___ ,f_x_c_l2_norm)
      if (verbose.gt.1) write(6,*) '  f_x_c_l2_norm is ',f_x_c_l2_norm
      tmp_d_(0) = n_x_c_max
      tmp_d_(1) = n_x_c_max
      tmp_d_(2) = n_x_c_max
      write(tmp_fname,'(A)') './dir_mda7/grid_x1_c___.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(tmp_fname)
      call MDA_write_r8(3,tmp_d_,grid_x1_c___,tmp_fname)
      write(tmp_fname,'(A)') './dir_mda7/grid_x2_c___.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(tmp_fname)
      call MDA_write_r8(3,tmp_d_,grid_x2_c___,tmp_fname)
      write(tmp_fname,'(A)') './dir_mda7/grid_x3_c___.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(tmp_fname)
      call MDA_write_r8(3,tmp_d_,grid_x3_c___,tmp_fname)
      write(tmp_fname,'(A)') './dir_mda7/f_x_c_A___.mda'
...
c     create physical space grid and sample function_B on it.
     ...
     $     ,f_x_c_B___ ,f_x_c_l2_norm)
      ...
      tmp_d_(1) = n_x_c_max
      tmp_d_(2) = n_x_c_max
      write(tmp_fname,'(A)') './dir_mda7/f_x_c_B___.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : '
     $     ,trim(tmp_fname)
...
c
     ...
     $     ,n_azimu_b_polar_a_k_p_sum)
      if (verbose.gt.1) write(6,*) '  n_azimu_b_polar_a_k_p_sum is '
     $     ,n_azimu_b_polar_a_k_p_sum
...
c
     ...
     $     ,f_x_c_A___,x_c_step,n_x_c_max,eps,k_p_max,n_k_p_max
     ...
     $     ,ier)
      ...
      if (verbose.gt.1) then
         write(6,*) 'f_k_c_l2_norm: ',f_k_c_l2_norm
         write(6,*) 'numonsphere: ',(numonsphere(nk),nk=1,n_k_p_max)
         write(6,*) 'n_azimu_b_polar_a_sum_: '
     $        ,(n_azimu_b_polar_a_sum_(nk),nk=0,n_k_p_max-1)
      end if
c     create kspace grid data from physical space model B
c
      eps = 1.0d-6
      call fun_to_kspacegrid(grid_x1_c___,grid_x2_c___,grid_x3_c___
     $     ,f_x_c_B___,x_c_step,n_x_c_max,eps,k_p_max,n_k_p_max
     $     ,quadrature_type_radial,n_polar_a_,quadrature_type_azimu_b
     $     ,n_azimu_b_polar_a_k_p_sum,numonsphere , grid_k1_c_
...
c-----------------------------------------------------------------------
     ...
     $     ,n_azimu_b_ ,azimu_b_step_ ,n_azimu_b_polar_a_sum_tmp)
      if (verbose.gt.0) write(6,*) '  n_image is ',n_image
      if (verbose.gt.0) write(6,*) '  n_M is ',n_M
      if (verbose.gt.0) then
         write(6,*) ' n_azimu_b_polar_a_sum_: ' ,
     $        (n_azimu_b_polar_a_sum_(nk),nk=0,n_k_p_max-1)
      end if! if (verbose.gt.0) then
      if (verbose.gt.0) write(6,*) '  n_azimu_b_polar_a_k_p_sum is '
     $     ,n_azimu_b_polar_a_k_p_sum
      if (verbose.gt.0) write(6,*) '  n_azimu_b_polar_a_sum_tmp is '
...
c$$$            estimated parameters used to initialize least-squares-solve
     ...
     $              *(mod(1+nA+1,3)-1)
      ...
      if (verbose.gt.1) write(6,*) '  n_azimu_b_polar_a_sum_tmp is '
     $     ,n_azimu_b_polar_a_sum_tmp
...
c
      ...
      allocate(S__(0:ld_S-1,0:n_M-1))
      allocate(S_alpha_polar_a_all_(0:n_S-1))
      allocate(S_alpha_azimu_b_all_(0:n_S-1))
      call get_template_size(n_polar_a_,n_k_p_max,ld_S,ngridc_tmp,
     $     icstart)
      call cp1_i4(n_k_p_max,icstart,n_w_sum_)
      if (verbose.gt.1) write(6,*) '  ld_S is ',ld_S
...
c
     ...
     $     ,eps,n_k_p_max,quadrature_type_radial,n_w_,k_p_max,n_alpha
...
c
     ...
     $     ,eps,n_k_p_max,quadrature_type_radial,n_w_,k_p_max,n_alpha
     $     ,alpha2d_tru__,n_M ,ld_M,Y_slice_B__)
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
...
c$$$  with batches of roughly 30-300 images each having the same ctf.
      n_ctf = 3
      allocate(CTF_k_p_(0:n_ctf*ld_M-1))
...
c$$$  of anisotropy to test our code.
      do nctf=0,n_ctf-1
         if (flag_ctf) then
            ...
            ctf_p_2 = 2
            call get_ctf_star_k_p_(n_k_p_max,grid_k_p_,n_w_,ld_M
     $           ,CTF_k_p_(nctf*ld_M),ctf_p_0,ctf_p_1,ctf_p_2)
         else
            call get_ctf_ones_k_p_(n_k_p_max,grid_k_p_,n_w_,ld_M
     $           ,CTF_k_p_(nctf*ld_M))
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      write(tmp_fname,'(A)') './dir_jpg7/Fig_ctf_c'
      call Fig_gen_ctf_ver0(n_k_p_max,n_polar_a_,grid_k_p_,n_ctf,ld_M
     $     ,CTF_k_p_,tmp_fname)
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
...
c$$$  n_S_sample_max = maximum number of templates to use at each step.
      ...
      allocate(delta_x_optimal_(0:n_S_sample_max*n_M_sample_max-1))
      allocate(delta_y_optimal_(0:n_S_sample_max*n_M_sample_max-1))
      allocate(gamma_z_optimal_(0:n_S_sample_max*n_M_sample_max-1))
      allocate(C_S_optimal_(0:n_S_sample_max*n_M_sample_max-1))
      allocate(C_Z_optimal_(0:n_S_sample_max*n_M_sample_max-1))
      ...
      do n_k_cur=n_k_bot,n_k_p_max
         if (verbose.gt.0) write(6,*) 'n_k_cur: ',n_k_cur
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         tmp_d_(0) = n_alpha
         tmp_d_(1) = n_M_sample
         write(tmp_fname,'(A,I0,A)') './dir_mda7/alpha1d_tru_',n_k_cur
     $        ,'_.mda'
     ...
     $        ,trim(tmp_fname)
         ...
         tmp_d_(0) = n_alpha
         tmp_d_(1) = n_M_sample
         write(tmp_fname,'(A,I0,A)') './dir_mda7/alpha1d_est_',n_k_cur
     $        ,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : '
     $        ,trim(tmp_fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         write(tmp_fname,'(A,I0)') './dir_jpg7/Fig_MNC_c_',n_k_cur
         call Fig_gen_ctf_ver1(n_k_cur,n_polar_a_,grid_k_p_,n_M_sample
     $        ,ld_M,Y_slice_sample_,M_sample_,ld_M,CTF_k_p_,n_alpha
     $        ,alpha1d_est_,min(16,n_M_sample),tmp_fname)

c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
c$$$  rebuild model based on current alpha.
c$$$  We rebuild two models.
c$$$  The first, Y_tru_ is the model obtained by using the 
c$$$  true alpha-parameters for each image.
c$$$  The second, Y_est_ is the model obtained by using the
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         ...
         if (verbose.gt.1) then
            ...
            write(6,*) 'icstart: ',(icstart(nk),nk=1,n_k_cur)
     ...
     $        ,ld_M,CTF_k_p_,n_alpha,alpha1d_tru_ ,n_w_sum_ ,n_polar_a_
     $        ,quadrature_type_azimu_b,grid_k_p_,n_w_,n_k_low,n_k_cur
     $        ,n_Y_lm_sum_,n_Y_l_ ,lsq_oversample
     $        ,lsq_interpolation_order,eps ,Y_tru_)
         timing_toc = second()
         if (verbose.gt.0) then
            write(6,'(A,A,F8.3)') 'rebuild_model_0 (tru):'
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         if (verbose.gt.0) write(6,*) 'Y_tru_ to kspacegrid'
     ...
     $        ,grid_k2_c_,grid_k3_c_,f_k_c_tru_,fft_iflag,eps
     $        ,n_x1_c_x2_c_x3_c_sum,grid_x1_c___,grid_x2_c___
     $        ,grid_x3_c___ ,f_x_c_tru___,ier)
         tmp_d_(0) = n_x_c_max
         tmp_d_(1) = n_x_c_max
         tmp_d_(2) = n_x_c_max
         write(tmp_fname,'(A,I0,A)') './dir_mda7/f_x_c_tru___',n_k_cur
     $        ,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : '
     $        ,trim(tmp_fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $           ,ld_M,CTF_k_p_,n_alpha,alpha1d_tru_ ,n_w_sum_
...
c$$$  write model to disk for postmortem analysis
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         if (verbose.gt.0) write(6,*) 'Y_est_ to kspacegrid'
     ...
     $        ,n_Y_lm_sum_max)
         ...
         if (verbose.gt.0) write(6,*) 'finufft3d3_f'
         call  finufft3d3_f(n_azimu_b_polar_a_k_p_sum,grid_k1_c_
     $        ,grid_k2_c_,grid_k3_c_ ,f_k_c_est_,fft_iflag,eps
     $        ,n_x1_c_x2_c_x3_c_sum,grid_x1_c___,grid_x2_c___
     $        ,grid_x3_c___,f_x_c_est___ ,ier)
         tmp_d_(0) = n_x_c_max
         tmp_d_(1) = n_x_c_max
         tmp_d_(2) = n_x_c_max
         write(tmp_fname,'(A,I0,A)') './dir_mda7/f_x_c_est___',n_k_cur
     $        ,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : '
     $        ,trim(tmp_fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */         
         if (flag_residual) then
            ...
            call test_residual_0(n_M_sample,ld_M,M_sample_,n_ctf
     $           ,ld_M,CTF_k_p_,n_alpha,alpha1d_est_ ,n_w_sum_
     $           ,n_polar_a_ ,quadrature_type_azimu_b,grid_k_p_,n_w_
     $           ,n_k_1,n_k_cur ,n_Y_lm_sum_cur ,n_Y_lm_sum_ ,n_Y_l_
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         if (flag_residual) then
            write(tmp_fname,'(A,I0)') './dir_jpg7/Fig_residual_est_'
     $           ,n_k_cur
            call Fig_R_ver0(n_k_cur,n_polar_a_,grid_k_p_,n_M_sample
     $           ,ld_M,M_sample_,M_residual_transform_,Y_residual_slice_
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        ,tesselation_distance_req ,S_alpha_polar_a_sample_
     $        ,S_alpha_azimu_b_sample_ ,n_M_sample ,ld_M,M_sample_
     $        ,n_ctf,ld_M ,CTF_k_p_,n_alpha ,alpha1d_est_,0.1d0
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         tmp_d_(0) = n_S_sample
         tmp_d_(1) = n_M_sample
         write(tmp_fname,'(A,I0,A)') './dir_mda7/C_Z_optimal_',n_k_cur
     $        ,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : '
     $        ,trim(tmp_fname)
         call MDA_write_c16(2,tmp_d_,C_Z_optimal_,tmp_fname)
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
c$$$  sort results
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         write(tmp_fname,'(A,I0)') './dir_jpg7/Fig_MTS_c_',n_k_cur
         call Fig_gen_ver2(n_k_cur,n_polar_a_,grid_k_p_,n_S_sample,ld_S
     $        ,S_sample_,n_M_sample,ld_M,Y_slice_sample_,M_sample_
     $        ,ld_M,CTF_k_p_,n_alpha,alpha1d_est_ ,delta_x_sort_SM_
