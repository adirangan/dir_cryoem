...
c     $OMP DO 
     ...
     $           ,' test_innerproduct_5_S_q_1'
     $           ,' with thread: ',omp_get_thread_num()
         end if
         call test_innerproduct_5_S_q_1(verbose-1
...
c     $OMP END PARALLEL
     ...
     $        ,S_q__(n_r*nS_9_sum))
      ...
      timing_total_S_q = timing_total_S_q + timing_tot
      gnump_total_S_q = gnump_total_S_q + gnump_tot
     ...
     $        ' total time ' , timing_tot
         ...
         if (flag_MDA.eqv..true.) then
            ...
            MDA_d_(1) = nS_0_per
            MDA_d_(2) = n_w_max
            write(MDA_string,'(A,1(A,I0),A)') './dir_mda/S_q__' ,
     $           'nS0_' , nS_0_sub,'.mda'
            call MDA_write_c16(MDA_n_d,MDA_d_,S_q__,MDA_string)
