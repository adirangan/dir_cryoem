      subroutine test_innerproduct_bruteforce_0(verbose ,n_r ,grid_p_
     $     ,n_w_ ,half_diameter_x_c ,S_p__in_,M_p__in_ ,CTF_p__in_
     $     ,delta_x_est ,delta_y_est ,gamma_z_est ,n_delta_x ,delta_x_
...
c$$$      indices
      ...
      integer ndx,ndy,ngz,ndx_optimal,ndy_optimal,ngz_optimal,n_X
...
c$$$      parameters for timing
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[entering test_innerproduct_bruteforce_0]'
      end if      
      ...
      if (verbose.gt.1) then
         ...
         call write_all_i4(n_r,n_w_,7,' n_w_: ')
      end if
      ld_S = n_A
      ld_M = n_A
      ld_CTF = n_A
      ...
      if (verbose.gt.1) then
         ...
         call write_sub_c16(n_A,S_p__in_,11,' S_p__in_: ')
         call write_sub_c16(n_A,M_p__in_,11,' M_p__in_: ')
         call write_sub_c16(n_A,CTF_p__in_,13,' CTF_p__in_: ')
     ...
     $     ,S_p__in_,CTF_p_ ,CTF_q_ ,n_CTF,ld_CTF ,CTF_p__in_
     $     ,CTF_R_S_xx_)
      timing_toc = omp_get_wtime()
     ...
     $        /1e9
      ...
      do ngz=0,n_gamma_z
         ...
         call rotate_p2p_fz(n_r,n_w_,n_A,S_p__in_,+gamma_z
     $        ,S_p_)
     ...
     $        /1e9
      ...
      if (verbose.gt.1) then
         call write_all_c16(n_gamma_z,CTF_R_S_xx_,14,' CTF_R_S_xx_: ')
         call write_all_c16(n_gamma_z,CTF_R_S_bf_,14,' CTF_R_S_bf_: ')
     ...
     $        ,CTF_R_S_xx_,CTF_R_S_bf_)
      ...
      allocate(S_T_R_T_R_M_xx_(0:n_delta_x*n_delta_y*n_gamma_z))
      call cl1_c16(n_delta_x*n_delta_y*n_gamma_z,S_T_R_T_R_M_xx_)
      if (verbose.gt.1) then
         write(6,'(A)') ' Calculate S_T_R_T_R_M_xx_.'
         write(6,'(A)') ' More specifically, the value: '
         write(6,'(A)')
     $        ' S_T_R_T_R_M_xx_(ngz + n_gamma_z*(ndx + ndy*n_delta_x)) '
      ...
      call rotate_p2p_fz(n_r,n_w_,n_A,M_p_,-gamma_z_est
     $     ,M_p_)
...
c$$$     $     -delta_y,M_p_)
     ...
     $        timing_tmp
      ...
      allocate(S_T_R_T_R_M_bf_(0:n_delta_x*n_delta_y*n_gamma_z))
      call cl1_c16(n_delta_x*n_delta_y*n_gamma_z,S_T_R_T_R_M_bf_)
      timing_tic = omp_get_wtime()
      call xc1_c16(n_A,M_p__in_,CTF_p__in_,M_p_)
     ...
     $              ,delta_y,S_p_)
               call rotate_p2p_fz(n_r,n_w_,n_A,S_p_,gamma_z
     $              ,S_p_)
      ...
      if (verbose.gt.1) then
         do ndx=0,n_delta_x-1
            ...
            do ndy=0,n_delta_y-1
               delta_y = delta_y_(ndy)
               write(6,'(2(A,I0))') ' ndx ' , ndx , ' ndy ' , ndy
               n_X = n_gamma_z*(ndx + ndy*n_delta_x)
               call write_all_c16(n_gamma_z ,S_T_R_T_R_M_xx_(n_X),18
     $              ,' S_T_R_T_R_M_xx_: ')
               call write_all_c16(n_gamma_z ,S_T_R_T_R_M_bf_(n_X),18
     $              ,' S_T_R_T_R_M_bf_: ')
     ...
     $              ,S_T_R_T_R_M_bf_(n_X))
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[finished test_innerproduct_bruteforce_0]'
      end if      
