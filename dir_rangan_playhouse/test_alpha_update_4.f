      subroutine test_alpha_update_4(verbose,rseed,n_omp_sub
     $     ,fpm_howmany_max,half_diameter_x_c_tmp,n_k_p_max,n_M_sample
     $     ,I_M_sample_,ld_M,M_k_p__ ,n_w_sum_,n_polar_a_
     $     ,quadrature_type_azimu_b,grid_k_p_,n_w_ ,n_k_low,n_k_cur
     $     ,n_Y_lm_sum_max,n_Y_lm_sum_,n_Y_l_ ,lsq_oversample
     $     ,lsq_interpolation_order,eps_default,n_ctf ,ld_CTF ,CTF_k_p_
     $     ,n_alpha,alpha1d_est_,alpha_update_f,flag_MS_vs_SM,Y_est_
     $     ,n_azimu_b_polar_a_sum_,ld_S,n_S_sample_max ,S_k_p__
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      ...
      integer I_M_sample_(0:n_M_sample-1) ! array indicating relevant n_M_sample indices within M_k_p__ ;
      ...
      complex *16 M_k_p__(0:0) ! full array of images. these will be subsampled by I_M_sample_. These have already been modified from their original Y_slice_ by multiplying by the CTF and/or modifying the l2-norm (depending on which flags are set above). ;
      ...
      real *8 alpha1d_est_(0:n_alpha*n_M_sample-1) ! estimated image parameters associated with sampled image set (e.g., used to be stored in Y_slice_sample_ or M_sample_) stored in 1-dimensional array. ;
      ...
      complex *16 S_k_p__(0:0) ! full array of templates. These will be later subsampled by I_S_sample_. ;
...
c$$$  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
     ...
     $        ,half_diameter_x_c_tmp
         ...
         call write_sub_c16(ld_M,M_k_p__(I_M_sample_(0)*ld_M),20
     $        ,'M_k_p__(I_M_sample_()): ')
         call write_sub_c16(ld_M,M_k_p__(I_M_sample_(n_M_sample-1)*ld_M)
     $        ,32,'M_k_p__(I_M_sample_(n_M_sample-1)): ')
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      ...
      timing_tic = second()
      call rebuild_model_1(n_M_sample,I_M_sample_,ld_M,M_k_p__,n_ctf
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $     -1) + n_w_(n_k_cur-1)
      ...
      call test_innerproduct_fast_wrapper_3(0,rseed,n_omp_sub,n_k_cur
     $     ,n_polar_a_,grid_k_p_,half_diameter_x_c_tmp,n_S_sample
     $     ,I_S_sample_,ld_S ,S_k_p__,tesselation_distance_req_use
     $     ,S_alpha_polar_a_sample_,S_alpha_azimu_b_sample_ ,n_M_sample
     $     ,I_M_sample_,ld_M,M_k_p__ ,n_ctf,ld_CTF ,CTF_k_p_,n_alpha
     $     ,alpha1d_est_ ,alpha_update_f ,flag_MS_vs_SM,n_pixels_in
     $     ,displacement_max ,n_delta_x ,n_delta_y,n_gamma_z,
     $     svd_calculation_type ,eps_svd ,fpm_howmany_max,C_M_
     $     ,n_SM_use)
      timing_toc = omp_get_wtime()
      if (verbose.gt.0) then
         write(6,'(A,A,A,F8.4)') 'test_alpha_update_4: ',
     $        'test_innerproduct_fast_wrapper_1:',
     $        ' tesselation_distance_req_use ' ,
     $        tesselation_distance_req_use
         write(6,'(A,A,A,I0,A,I0,A,I0,A)') 'test_alpha_update_4: ',
     $        'test_innerproduct_fast_wrapper_1:',
     $        ' calculated '
     $        , n_SM_use , ' / ' , n_M_sample *n_S_sample , ' = ' ,
     $        floor((n_SM_use*100.0d0) /(1.0d0*n_M_sample
     $        *n_S_sample)) , '% '
         write(6,'(A,A,A,F16.9)') 'test_alpha_update_4: ',
     $        'test_innerproduct_fast_wrapper_1:',
     $        ' total_time                    ' ,timing_toc-timing_tic  
         write(6,'(A,A,A,F16.9)') 'test_alpha_update_4: ',
     $        'test_innerproduct_fast_wrapper_1:',
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        ,M_k_p__ ,ld_CTF,CTF_k_p_,n_alpha,alpha1d_est_ ,min(16
     $        ,n_M_sample) ,tmp_fname)
      end if !if (flag_fig) then

      if (verbose.gt.0) write(6,'(A)')
     $     '[finished test_alpha_update_4]'
