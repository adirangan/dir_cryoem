      subroutine innerproduct_q__k_svdd_2(flag_S_vs_M,n_svd_d,svd_d_
...
c$$$      l_max must equal or exceed order of bessel-function expansion
     ...
     $        ' % [entering innerproduct_q__k_svdd_2] ',n_svd_d,n_svd_l
     $        ,n_delta_v
      end if
      pi = 4.0*atan(1.0)
      nC = 0
...
c$$$            theta = nl_pre*(pi/2 - omega).
     ...
     $              ,'>l_max in innerproduct_q__k_svdd_2'
...
c$$$               nC = l + ndv*n_svd_l
      ...
      if (verbose.gt.0) then
         write (6,'(A)') ' % [finished innerproduct_q__k_svdd_2]'
      end if
      end
