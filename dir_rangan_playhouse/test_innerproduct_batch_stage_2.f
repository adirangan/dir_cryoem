      subroutine test_innerproduct_batch_stage_2(
     $      verbose
     $     ,nm,ns
     $     ,n_r,grid_p_,n_w_,n_S,ld_S,n_A,nctf,n_w_max
     $     ,S_p_,S_p,S_q,M_q_,Z_q_
     $     ,C_M,C_S_,C_S_optimal,C_S_optimal_,C_Z_optimal,C_Z_optimal_
     ...
     $     )
      ...
      real *8 delta_x_optimal_(0:0)
      real *8 delta_y_optimal_(0:0)
      real *8 gamma_z_optimal_(0:0)
      complex *16 C_M_(0:0)
      complex *16 C_S_optimal_(0:0)
      complex *16 C_Z_optimal_(0:0)
...
c$$$      parameters for timing
     ...
     $              ,Z_svdd_,Z_svdr_,Z_tmpC_,fftw_plan_back_last
     ...
     $              ,M_q_ ,Z_svdr_)
               do svd_l=0,n_svd_l-1
                  call cps_c16(n_w_max,Z_svdr_(svd_l),n_svd_l
     $                 ,fftw_out_last_,1)
     ...
     $              timing_tic_2)
               timing_tic_3 = second()
               call test_innerproduct_batch_excerpt_2(n_delta_x
     $              ,n_delta_y,n_gamma_z,gamma_z_,n_w_max,n_svd_l
     $              ,Z_svdd_,Z_svdr_,Z_tmpC_,n_r,C_M,C_S_(0 + ns
     $              *n_gamma_z + nctf*n_gamma_z*n_S),+gamma_z,Z_q_)
               timing_toc_3 = second()
               timing_tot_3 = timing_tot_3 + (timing_toc_3 -
     $              timing_tic_3)
               ...
               timing_tic_3 = second()
               call test_innerproduct_batch_excerpt_3(n_delta_x
     ...
     $           ,ndx_optimal,ndy_optimal ,ngz_optimal ,C_Z_optimal)
            delta_x_optimal_(ns + nm*n_S) = delta_x_(ndx_optimal)
            delta_y_optimal_(ns + nm*n_S) = delta_y_(ndy_optimal)
            gamma_z_optimal_(ns + nm*n_S) = gamma_z_(ngz_optimal)
            call test_innerproduct_batch_excerpt_5(n_gamma_z,gamma_z_
     $           ,C_S_(0 +ns*n_gamma_z + nctf*n_gamma_z*n_S),+gamma_z
     $           ,ngz_optimal,C_S_optimal)
            C_S_optimal_(ns + nm*n_S) = C_S_optimal
            C_Z_optimal_(ns + nm*n_S) = C_Z_optimal
            timing_toc_4 = second()
     ...
     $              ,ndx_optimal,ndy_optimal ,ngz_optimal
     $              ,'; C_S_optimal: ',C_S_optimal,'; C_Z_optimal: '
     $              ,C_Z_optimal
            end if !verbose
            end
