...
c$$$      svd_calculation_type == 2 --> brute force displacements (see test_innerproduct_batch_stage_2.f). ;
      ...
      logical flag_heterogeneity !flag determining whether or not to include multiple (i.e., 2) molecules in image pool. ;
      ...
      logical flag_fig !flag determining whether or not to generate figures (in dir_jpg10)
...
c-----------------------------------------------------------------------
     ...
     $           *sin(source_w)
      ...
      write(tmp_fname,'(A)')
     $     './dir_mda10/source_x1_c_x2_c_x3_c_A___.mda'
      ...
      tmp_d_(1) = n_source_use
      write(tmp_fname,'(A)')
     $     './dir_mda10/source_x1_c_x2_c_x3_c_B___.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(tmp_fname)
...
c     create physical space grid and sample function_A on it.
     ...
     $     ,f_x_c_l2_norm
      ...
      tmp_d_(1) = n_x_c_max
      tmp_d_(2) = n_x_c_max
      write(tmp_fname,'(A)') './dir_mda10/grid_x1_c___.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(tmp_fname)
      call MDA_write_r8(3,tmp_d_,grid_x1_c___,tmp_fname)
      write(tmp_fname,'(A)') './dir_mda10/grid_x2_c___.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(tmp_fname)
      call MDA_write_r8(3,tmp_d_,grid_x2_c___,tmp_fname)
      write(tmp_fname,'(A)') './dir_mda10/grid_x3_c___.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(tmp_fname)
      call MDA_write_r8(3,tmp_d_,grid_x3_c___,tmp_fname)
      write(tmp_fname,'(A)') './dir_mda10/f_x_c_A___.mda'
...
c     create physical space grid and sample function_B on it.
     ...
     $     ,f_x_c_l2_norm
      ...
      tmp_d_(1) = n_x_c_max
      tmp_d_(2) = n_x_c_max
      write(tmp_fname,'(A)') './dir_mda10/f_x_c_B___.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : '
     $     ,trim(tmp_fname)
...
c$$$      save both Y_A_ and Y_B_ to disk.
     ...
     $     ,trim(tmp_fname)
      ...
      tmp_d_(0) = n_Y_lm_sum_max
      tmp_d_(1) = 1
      write(tmp_fname,'(A)') './dir_mda10/Y_B_.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : '
     $     ,trim(tmp_fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      if (flag_fig) then
      write(tmp_fname,'(A)') './dir_jpg10/Fig_ctf_c'
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         tmp_d_(0) = n_alpha
         tmp_d_(1) = n_M_sample
         write(tmp_fname,'(A,I0,A)') './dir_mda10/alpha1d_tru_',n_k_cur
     $        ,'_.mda'
     ...
     $        ,trim(tmp_fname)
         ...
         tmp_d_(0) = n_alpha
         tmp_d_(1) = n_M_sample
         write(tmp_fname,'(A,I0,A)') './dir_mda10/alpha1d_est_',n_k_cur
     $        ,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : '
     $        ,trim(tmp_fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         if (flag_fig) then
         write(tmp_fname,'(A,I0)') './dir_jpg10/Fig_MNC_c_',n_k_cur
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        ,grid_x3_c___,f_x_c_tru___ ,ier)
         ...
         tmp_d_(1) = n_x_c_max
         tmp_d_(2) = n_x_c_max
         write(tmp_fname,'(A,I0,A)') './dir_mda10/f_x_c_tru___',n_k_cur
     $        ,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : '
     $        ,trim(tmp_fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */         
         if (flag_residual .and. n_source_cut.lt.n_source_use) then
     ...
     $           './dir_mda10/I_model_sample_tru_',n_k_cur,'_.mda'
     ...
     $           ,trim(tmp_fname)
            ...
            tmp_d_(1) = n_M_sample
            write(tmp_fname,'(A,I0,A)')
     $           './dir_mda10/M_residual_loading_tru_',n_k_cur,'_.mda'
            if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : '
     $           ,trim(tmp_fname)
     ...
     $              dreal(M_residual_loading_(0 + n_residual_loading
     $              *nM_sample)).ge.0.0d0) n_M_A_pos_sample =
     $              n_M_A_pos_sample +1
               if (I_model_sample_(nM_sample).eq.1 .and.
     $              dreal(M_residual_loading_(0 + n_residual_loading
     $              *nM_sample)).ge.0.0d0) n_M_B_pos_sample =
     $              n_M_B_pos_sample +1
               if (I_model_sample_(nM_sample).eq.0 .and.
     $              dreal(M_residual_loading_(0 + n_residual_loading
     $              *nM_sample)).lt.0.0d0) n_M_A_neg_sample =
     $              n_M_A_neg_sample +1
               if (I_model_sample_(nM_sample).eq.1 .and.
     $              dreal(M_residual_loading_(0 + n_residual_loading
     $              *nM_sample)).lt.0.0d0) n_M_B_neg_sample =
     $              n_M_B_neg_sample +1
            enddo               !do nM_sample=0,n_M_sample-1
     ...
     $           n_M_B_pos_sample ,n_M_B_neg_sample
         end if !if (flag_residual .and. n_source_cut.lt.n_source_use) then

c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
c$$$  print out a subset of image-slice pairs for Y_tru_
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         if (flag_residual .and. flag_fig) then
            write(tmp_fname,'(A,I0)') './dir_jpg10/Fig_residual_tru_'
     $           ,n_k_cur
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        ,grid_x3_c___,f_x_c_est___ ,ier)
         ...
         tmp_d_(1) = n_x_c_max
         tmp_d_(2) = n_x_c_max
         write(tmp_fname,'(A,I0,A)') './dir_mda10/f_x_c_est___',n_k_cur
     $        ,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : '
     $        ,trim(tmp_fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */         
     ...
     $           ,M_residual_loading_)
            ...
            tmp_d_(1) = 1
            write(tmp_fname,'(A,I0,A)')
     $           './dir_mda10/I_model_sample_est_',n_k_cur,'_.mda'
            if (verbose.gt.1) write(6,'(A,A)') 'Writing i4 : '
     $           ,trim(tmp_fname)
            call MDA_write_i4(2,tmp_d_,I_model_sample_,tmp_fname)
            tmp_d_(0) = n_residual_loading
            tmp_d_(1) = n_M_sample
            write(tmp_fname,'(A,I0,A)')
     $           './dir_mda10/M_residual_loading_est_',n_k_cur,'_.mda'
            if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : '
     $           ,trim(tmp_fname)
            call MDA_write_c16(2,tmp_d_,M_residual_loading_,tmp_fname)
            n_M_A_pos_sample=0
     ...
     $              n_M_B_neg_sample +1
            enddo               !do nM_sample=0,n_M_sample-1
            write(6,'(A,I0,1X,I0)') ' alpha_bkp: n_M_A_x_sample: ',
     $           n_M_A_pos_sample ,n_M_A_neg_sample
            write(6,'(A,I0,1X,I0)') ' alpha_bkp: n_M_B_x_sample: ',
     $           n_M_B_pos_sample ,n_M_B_neg_sample
         end if !if (flag_residual .and. n_source_cut.lt.n_source_use) then
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         if (flag_residual .and. flag_fig) then
            write(tmp_fname,'(A,I0)') './dir_jpg10/Fig_residual_est_'
     $           ,n_k_cur
            call Fig_R_ver0(n_k_cur,n_polar_a_,grid_k_p_,n_M_sample
     $           ,ld_M,M_sample_,M_residual_transform_,Y_residual_slice_
