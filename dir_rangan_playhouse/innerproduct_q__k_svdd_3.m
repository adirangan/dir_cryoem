function [C_q_,U_d__] = innerproduct_q__k_svdd_3(flag_S_vs_M,svd_d_max,n_svd_d,svd_d_,n_svd_l,svd_l_,svd_U_d_,n_delta_v,delta_x_,delta_y_);
...
if (verbose>0);
disp(sprintf(' %% [entering innerproduct_q__k_svdd_3] %d %d %d',n_svd_d,n_svd_l,n_delta_v));
end;%if (verbose>0);
l_max = max(svd_l_);
...
disp(sprintf(' %% Warning, delta %0.6f > svd_d_max %0.6f in innerproduct_q__k_svdd_3',delta,svd_d_max));
...
I_l = svd_l_(1+nl);
if (abs(I_l)>l_max);
disp(sprintf(' %% Warning, I_l %d > l_max in innerproduct_q__k_svdd_3',I_l));
...
if (verbose>0);
disp(sprintf(' %% [finished innerproduct_q__k_svdd_3]'));
end;%if (verbose>0);
