...
c$OMP END PARALLEL
     ...
     $        nM_0_per , ' nM_1_per ' , nM_1_per
         call write_sub_c16(n_r*n_transf*nS_0_per*n_w_max,T_S_q__
     $        ,10,' T_S_q__: ')
         call write_sub_c16(n_r*nM_0_per*n_w_max,O_T_R_CTF_M_q__,16
     $        ,' O_T_R_CTF_M_q__: ')
         call write_sub_c16(n_transf*nS_1_per*nM_1_per
     $        *n_w_max,S_T_T_R_CTF_M_q__,19,' S_T_T_R_CTF_M_q__:')
         do nw=0,n_w_max-1
            call write_sub_c16(n_r*n_transf*nS_0_per,T_S_q__(n_r
     $           *n_transf*nS_0_per*nw),10,' T_S_q__: ')
         enddo                  !do nw=0,n_w_max-1
         do nw=0,n_w_max-1
            call write_sub_c16(n_r*nM_0_per,O_T_R_CTF_M_q__(n_r*nM_0_per
     $           *nw),16,' O_T_R_CTF_M_q__: ')
         enddo                  !do nw=0,n_w_max-1
         do nw=0,n_w_max-1
            call write_sub_c16(n_transf*nS_1_per*nM_1_per
     $           ,S_T_T_R_CTF_M_q__(n_transf*nS_1_per*nM_1_per*nw),19,
     $           ' S_T_T_R_CTF_M_q__:')
         enddo                  !do nw=0,n_w_max-1
...
c$$$      %%%%%%%%%%%%%%%%
      ...
      if (flag_test) then
         if (verbose.gt.1) then
            write(6,'(A)')
     $           ' calling ti7_check_STxTRM_3 to test. '
            write(6,'(A)') ' '
         end if !if (verbose.gt.1) then
         call ti7_check_STxTRM_3(verbose-1,n_delta_v ,delta_x_
