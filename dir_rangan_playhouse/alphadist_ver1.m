function alphadist_ver1(n_,dirname)
figure;
...
ncur = n_(nF);
if (ncur==0);
alpha_ = MDA_read_r8(sprintf('%s/alpha1d_tru_3_.mda',dirname,ncur)); 
elseif (ncur>0);
alpha_ = MDA_read_r8(sprintf('%s/alpha1d_est_%d_.mda',dirname,ncur)); 
end;%if;
