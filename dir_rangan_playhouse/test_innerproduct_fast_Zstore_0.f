      subroutine test_innerproduct_fast_Zstore_0(verbose,rseed
     ...
     $     ,l2_norm_est_,ctf_ind_est_ ,S_index_est_,alpha_update_f
     $     ,displacement_max,n_SM_max,n_SM_,alpha_SM__,C_M_,CTF_R_S_
     $     ,n_M_tot,S_T_T_R_CTF_M_q__ ,ZZ_)
      implicit none
...
c$$$      SM storage
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[entering test_innerproduct_fast_Zstore_0]'
      end if !if (verbose.gt.0) then
      pi = 4.0*atan(1.0)
      n_w_max = n_w_(n_r-1)

      allocate(alpha__in(0:n_alpha-1))
      call cl1_r8(n_alpha,alpha__in)
...
c$$$            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     ...
     $              /zabs(CTF_R_S_optimal)
            ...
            call cl1_r8(n_alpha,alpha__in)
            alpha__in(nalpha_polar_a) = S_alpha_polar_a_(ns)
            alpha__in(nalpha_azimu_b) = S_alpha_azimu_b_(ns)
            alpha__in(nalpha_gamma_z) = gamma_z_optimal
            alpha__in(nalpha_delta_x) = delta_x_optimal
            alpha__in(nalpha_delta_y) = delta_y_optimal
            alpha__in(nalpha_l2_norm) = l2_norm_optimal
            alpha__in(nalpha_ctf_ind) = nctf
            alpha__in(nalpha_S_index) = ns
            alpha__in(nalpha_CTF_R_S) = CTF_R_S_optimal
            alpha__in(nalpha_C_Z_opt) = C_Z_optimal
            if (verbose.gt.2) then
               write(6,'(A)') ' calling alpha_SM_update_0 with '
               write(6,'(5(A,I0))') ' n_alpha ' , n_alpha , ' n_SM_max '
     $              , n_SM_max , ' nm ' , nm ,' nm*n_alpha*n_SM_max ' ,
     $              nm*n_alpha*n_SM_max , ' n_SM_(nm) ' , n_SM_(nm) 
               write(6,'(A,I0)') ' calling alpha_SM_update_0 with '
               call write_all_r8(n_alpha,alpha__in,13
     $              ,' alpha__in_: ')
            end if !if (verbose.gt.2) then
            call alpha_SM_update_0(n_SM_max,n_SM_(nm)
     $           ,alpha_SM__(nm*n_alpha*n_SM_max),alpha__in)
         enddo !do ns=0,n_S-1
      enddo !do nm=0,n_M_sub-1
c$$$            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

      deallocate(alpha__in)
      deallocate(ZZ_sub_)
      deallocate(CTF_R_S_use_)

      if (verbose.gt.0) then
         write(6,'(A)') '[finished test_innerproduct_fast_Zstore_0]'
      end if !if (verbose.gt.0) then
