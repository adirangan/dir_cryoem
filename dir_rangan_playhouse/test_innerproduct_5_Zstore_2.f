      subroutine test_innerproduct_5_Zstore_2(
     $     verbose
     $     ,rseed
     $     ,flag_RTRT_vs_RTTR
     $     ,n_delta_x
...
c$$$      MS storage
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[entering test_innerproduct_5_Zstore_2]'
...
c$$$            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     ...
     $                 **4)
            ...
            nx2 = n_delta_x*n_delta_y*(ns + n_S_sub*nm)
            nx3_stride = n_delta_x*n_delta_y*n_S_sub*n_M_tot
            nC_stride = n_delta_x*n_delta_y
            do ndy=0,n_delta_y-1
               do ndx=0,n_delta_x-1
                  nx3 = 0
                  nC = ndx + ndy*n_delta_x
                  do nw=0,n_w_max-1
                     ngz = nw
                     CTF_R_S_use = CTF_R_S_use_(ngz)
c$$$                     if (dreal(S_T_T_R_CTF_M_q__(nx2 + nx3)
c$$$     $                    *CTF_R_S_use).gt.1.0d0) then
c$$$                        write(format_string,'(A,A)')
c$$$     $                       ' (5(A,I0) , (A,F16.2,1X,F16.2) ' ,
c$$$     $                       ' (A,F16.14,1X,F16.14) , A,F8.4) '
c$$$                        write(6,format_string)
c$$$     $                       ' nm ' ,nm ,' ns ' , ns , ' ndx ' ,ndx ,
c$$$     $                       ' ndy ' ,ndy, ' nw ' , nw , ' X1 '
c$$$     $                       ,S_T_T_R_CTF_M_q__(nx2 + nx3) , ' X2 '
c$$$     $                       ,CTF_R_S_use , ' X1X2 '
c$$$     $                       ,dreal(S_T_T_R_CTF_M_q__(nx2 + nx3)
c$$$     $                       *CTF_R_S_use)
c$$$                     end if !too large
...
c$$$            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            call test_innerproduct_5_ZZ_scan_3(flag_RTRT_vs_RTTR
...
c$$$            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     ...
     $              /zabs(CTF_R_S_optimal)
            ...
            alpha__in_(nalpha_S_index) = ns + n_S_sum
...
c$$$            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[finished test_innerproduct_5_Zstore_2]'
      end if !if (verbose.gt.0) then
