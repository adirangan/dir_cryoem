      subroutine test_innerproduct_5_ZZ_scan_4x(flag_RTRT_vs_RTTR
     ...
     $     ,ngz_optimal
      ...
      if (verbose.gt.0) then
         write(6,'(A)') ' [entering test_innerproduct_5_ZZ_scan_4x] '
...
c$$$            end if !if (delta_pos.le.displacement_max) then 
     ...
     $        ,'in test_innerproduct_5_ZZ_scan_4x.f'
      end if !if (flag_found.eqv..false.) then
      if (verbose.gt.0) then
         write(6,'(A)') ' [finished test_innerproduct_5_ZZ_scan_4x] '
      end if !if (verbose.gt.0) then
      end
