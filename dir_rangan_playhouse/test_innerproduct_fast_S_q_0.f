      subroutine test_innerproduct_fast_S_q_0(verbose,fftw_plan_frwd_
     $     ,fftw_in1_,fftw_out_ ,n_r,grid_p_,n_w_,n_A,S_p_,S_q_,n_S_sub
     $     ,I_S_sample_,IS_per,ld_S,S_p__ ,n_S_tot ,S_q__)
c$$$      %%%%%%%%%%%%%%%%%%%%%%%
c$$$      Calculate bessel coefficients of S.
c$$$      S = template in k-space polar coord.
c$$$      S_q__(nr + n_r*(ns + n_S_tot*nw))
...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%
      ...
      integer verbose
      integer n_r,n_w_(0:n_r-1),n_A,n_S_sub,I_S_sample_(0:0),IS_per
     $     ,n_S_tot,ld_S
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[entering test_innerproduct_fast_S_q_0]'
     ...
     $           ,S_p__(I_S_sample_(IS_per+ns)*ld_S)
     $           ,S_p__(I_S_sample_(IS_per+ns)*ld_S),Z_tmp)
            Z_tmp = zsqrt(Z_tmp)/(n_r*n_r)
            write(6,'(A,I0,A,F8.4,1X,F8.4)') '|S_p_(',ns,')|: ' , Z_tmp
            call cp1_c16(n_A,S_p__(I_S_sample_(IS_per+ns)*ld_S),S_p_)
     ...
     $           ,Z_tmp)
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[finished test_innerproduct_fast_S_q_0]'
      end if !if (verbose.gt.0) then
      end
