...
c$$$      svd_calculation_type == 2 --> brute force displacements (see test_innerproduct_batch_stage_2.f). ;
      ...
      real *8 r_heterogeneity_A !fraction of heterogeneous components: molecules A,B,C and D
      real *8 r_heterogeneity_B !fraction of heterogeneous components: molecules A,B,C and D
      real *8 r_heterogeneity_C !fraction of heterogeneous components: molecules A,B,C and D
      real *8 r_heterogeneity_D !fraction of heterogeneous components: molecules A,B,C and D
...
c$$$      integer *4 n_SM_use !temporary: total number of image-template pairs considered when calculating innerproducts. Might be smaller than the total number when calculating innerproducts adaptively. ;
      ...
      if (verbose.gt.1) then
         write(6,'(A)') '[entering test_ver12]'
      end if

      rseed = 1

      r_heterogeneity_A = 0.6d0
      r_heterogeneity_B = 0.4d0
      r_heterogeneity_C = 0.0d0
      r_heterogeneity_D = 0.0d0
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         if (flag_fig) then
            write(tmp_fname,'(A,A,I0)') trim(dir_base) , '/Fig_MNC_c_'
     $           ,n_k_cur
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */         
         if (flag_residual) then
            ...
            call test_residual_2(rseed,n_M_sample,ld_M,M_sample_,n_ctf
     $           ,ld_CTF,CTF_k_p_,n_alpha,alpha1d_tru_ ,n_w_sum_
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */         
         if (flag_residual) then
            ...
            call test_residual_2(rseed,n_M_sample,ld_M,M_sample_,n_ctf
     $           ,ld_CTF,CTF_k_p_,n_alpha,alpha1d_bkp_ ,n_w_sum_
     $           ,n_polar_a_,quadrature_type_azimu_b,grid_k_p_,n_w_
     $           ,n_k_1,n_k_cur,n_Y_lm_sum_cur ,n_Y_lm_sum_ ,n_Y_l_
