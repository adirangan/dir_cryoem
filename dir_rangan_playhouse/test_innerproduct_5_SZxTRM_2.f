      subroutine test_innerproduct_5_SZxTRM_2(verbose,n_svd_r,svd_r_
     $     ,n_svd_l,svd_l_,svd_s_,svd_V_r_,Z_S_svdd_,n_delta_x ,delta_x_
...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[entering test_innerproduct_5_SZxTRM_2]'
      ...
      allocate(C_q0_(0:n_w_max*n_delta_x*n_delta_y-1))
      call cl1_c16(n_w_max*n_delta_x*n_delta_y,C_q0_)
      allocate(C_q1_(0:n_w_max*n_delta_x*n_delta_y-1))
      call cl1_c16(n_w_max*n_delta_x*n_delta_y,C_q1_)
      allocate(C_q2_(0:n_w_max*n_delta_x*n_delta_y-1))
      call cl1_c16(n_w_max*n_delta_x*n_delta_y,C_q2_)
...
c$$$         Define C_q0_
     ...
     $              ,-delta_y_est,M_p_)
               do nw=0,n_w_max-1
                  ...
                  call rotate_p2p_fz(n_r,n_w_,n_A,S_p_,+gamma_z,S_q_)
                  call innerproduct_p(n_r,grid_p_,n_w_,n_A,S_q_,M_p_
     $                 ,C_q0_(nw + n_w_max*(ndx + n_delta_x*ndy)))
...
c$$$         Define C_q1_
     ...
     $              ,fftw_in1_,fftw_out_,M_p_,M_q_)
               call innerproduct_q_k_only(n_r,grid_p_,n_w_,n_A,S_q_
     $              ,M_q_,C_q1_(n_w_max*(ndx + n_delta_x*ndy)))
               call cp1_c16(n_w_max,C_q1_(n_w_max*(ndx + n_delta_x*ndy))
     $              ,fftw_out_last_)
               call dfftw_execute_(fftw_plan_back_last)
               call cp1_c16(n_w_max,fftw_in1_last_,C_q1_(n_w_max*(ndx +
     $              n_delta_x*ndy)))
            enddo !do ndy=0,n_delta_y-1
         enddo !do ndx=0,n_delta_x-1
c$$$         Define S_q
...
c$$$              nC1 = nw + ndx*n_w_max + ndy*n_w_max*n_delta_x
                  C_q2_(nC1) = cmplx( 0.0 , 0.0 )
                  nC2 = ndx*n_svd_l + ndy*n_svd_l*n_delta_x
                  nC3 = nw*n_svd_l
                  do nsvd_l=0,n_svd_l-1
                     C_q2_(nC1) = C_q2_(nC1) + Z_S_svdd_(nC2)
     $                    *Z_svdr_(nC3)
                     nC2 = nC2 + 1
                     nC3 = nC3 + 1
                  enddo !do nsvd_l=0,n_svd_l-1
...
c$$$         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
         do ndx=0,n_delta_x-1
            do ndy=0,n_delta_y-1
               ...
               do nw=0,n_w_max-1
                  nC4 = nw + n_w_max*(ndx + n_delta_x*ndy);
                  C_tmp = C_tmp + zabs(C_q2_(nC4))**2
                  Z_tmp = Z_tmp + zabs(C_q2_(nC4) - C_q0_(nC4))**2
               enddo !do nw=0,n_w_max-1
               C_tmp = zsqrt(C_tmp)
               Z_tmp = zsqrt(Z_tmp)
               if (verbose.gt.-2) then
...
c$$$         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     ...
     $                 ,' C_q2_ vs S_T_T_R_CTF_M_Q__: ' ,' nx1: ' , nx1
     $                 ,' ndx: ' , ndx , ' ndy: ' , ndy ,' ns: ' , ns
     $                 ,' nm: ' , nm , ' absolute error: '
     $                 ,real(zabs(Z_tmp)) , ' relative error: '
     $                 ,real(zabs(Z_tmp) /zabs(C_tmp))
...
c$$$         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
         do ndx=0,n_delta_x-1
            do ndy=0,n_delta_y-1
c$$$               %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
               call cp1_c16(n_w_max,C_q2_(0+n_w_max*(ndx+n_delta_x
     $              *ndy)),Z_tmp_)
...
c$$$               %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%               
      ...
      if (verbose.gt.0) then
         write(6,'(A)')
     $        '[finished test_innerproduct_5_SZxTRM_2]'
      end if !if (verbose.gt.0) then
      end
