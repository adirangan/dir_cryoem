      subroutine test_alpha_update_3(verbose,rseed,n_omp_sub
     $     ,fpm_howmany_max,half_diameter_x_c_tmp,n_k_p_max,n_M_sample
     $     ,I_M_sample_,ld_M,M__ ,n_w_sum_,n_polar_a_
     ...
     $     ,n_azimu_b_polar_a_sum_,ld_S,n_S_sample_max ,n_delta_x
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      ...
      integer I_M_sample_(0:n_M_sample-1) ! array indicating relevant n_M_sample indices within M__ ;
      ...
      complex *16 M__(0:0) !temporary: full array of images. these will be subsampled by I_M_sample_. These have already been modified from their original Y_slice_ by multiplying by the CTF and/or modifying the l2-norm (depending on which flags are set above). ;
...
c$$$  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
     ...
     $        ,half_diameter_x_c_tmp
         ...
         call write_sub_c16(ld_M,M__(I_M_sample_(0)*ld_M),20
     $        ,'M__(I_M_sample_()): ')
         call write_sub_c16(ld_M,M__(I_M_sample_(n_M_sample-1)*ld_M),32
     $        ,'M__(I_M_sample_(n_M_sample-1)): ')
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      ...
      timing_tic = second()
      call rebuild_model_1(n_M_sample,I_M_sample_,ld_M,M__,n_ctf
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $     -1) + n_w_(n_k_cur-1)
      ...
      call test_innerproduct_fast_wrapper_2(0,rseed,n_omp_sub,n_k_cur
     $     ,n_polar_a_,grid_k_p_,half_diameter_x_c_tmp,n_S_sample
     $     ,I_S_sample_,ld_S ,S__,tesselation_distance_req_use
     $     ,S_alpha_polar_a_sample_,S_alpha_azimu_b_sample_ ,n_M_sample
     $     ,I_M_sample_,ld_M,M__ ,n_ctf,ld_CTF ,CTF_k_p_,n_alpha
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        ,I_S_sample_,ld_S,S__,n_M_sample,I_M_sample_,ld_M ,M__
     $        ,ld_CTF,CTF_k_p_,n_alpha,alpha1d_est_ ,min(16 ,n_M_sample)
     $        ,tmp_fname)
      end if !if (flag_fig) then

      if (verbose.gt.0) write(6,'(A)')
     $     '[finished test_alpha_update_3]'
