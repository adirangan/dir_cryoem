      subroutine test_innerproduct_batch_SM_wrapper_0a (verbose,n_M_sub
     ...
     $     ,svd_calculation_type ,eps_svd,C_M_ ,n_SM_use)
...
c$$$      parameters for timing
      ...
      if (verbose.gt.0) then
          write(6,'(A)')
     $        '[entering test_innerproduct_batch_SM_wrapper_0a]: '
       end if
...
c$$$      preparing updated translations and rotations
     ...
     $           ,S_index_upd_(nm)
      ...
      if (verbose.gt.1) then
         write(6,'(A)') ''
         write(6,*) 'Now we call test_innerproduct_batch_SM_stage_0a'
...
c$$$      calculating all innerproducts
      ...
      call test_innerproduct_batch_SM_stage_0a(verbose,n_M_sub
     ...
     $        'test_innerproduct_batch_SM_stage_0a:',' total_time: '
     ...
     $        ,(timing_toc -timing_tic)/(n_SM_use*n_delta_x *n_delta_y)
      ...
      if (verbose.gt.0) then
         write(6,'(A)')
     $        '[finished test_innerproduct_batch_SM_wrapper_0a]'
      end if
