c$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
      program test_innerproduct_slow_dr
c$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
c$$$      This program is a driver which tests the subroutine
c$$$      test_innerproduct_batch_SM_wrapper_0a.f ,
...
c$$$         various images.
c$$$      D. Then we call test_innerproduct_batch_SM_wrapper_0a to calculate
...
c$$$      D. The routine test_innerproduct_batch_SM_wrapper_0a is used
...
c$$$
c$$$      integer n_M_sub       Number of sub-blocks for omp parallelization.
...
c$$$      parameters 
c$$$      list: verbose ncur eps_svd N_pixels_in n_delta_x n_delta_y n_gamma_z svd_calculation_type n_M_sub
      ...
      if (verbose.gt.0) then
          write(6,'(A)')
     $        '[entering test_innerproduct_slow_dr]: '
       end if
       if (verbose.gt.0) then
...
c$$$      generating grids for templates and images 
     ...
     $        ' ncur or so.'
      ...
      enddo
      if (verbose.gt.0) then
         call write_all_r8(n_delta_x,delta_x_,10,'delta_x_: ')
         call write_all_r8(n_delta_y,delta_y_,10,'delta_y_: ')
         call write_all_r8(n_gamma_z,gamma_z_,10,'gamma_z_: ')
...
c$$$      Calculating x-space image on regular cartesian-grid
      ...
      do nm=0,n_M-1
         ...
         ngz = max(0,min(n_gamma_z-1,mod(nm+2,7)))
...
c$$$         these transformations will produce the M_k_p_ used below.
...
c$$$         the M_k_p_ in k-space.
         call cp1_c16(n_A,S_k_p_(ns*n_A),M_k_p_(nm*n_A))
         call transf_p_to_p(n_r,grid_k_p_,n_w_,n_A,M_k_p_(nm*n_A),
     $        +delta_x_tru_(nm),+delta_y_tru_(nm),M_k_p_(nm*n_A))
         call rotate_p2p_fz(n_r,n_w_,n_A,M_k_p_(nm*n_A),
     $        +gamma_z_tru_(nm),M_k_p_(nm*n_A))
         call interp_p_to_q_fftw(n_r,fftw_plan_frwd_,n_w_,n_A,fftw_in1_
     $        ,fftw_out_,M_k_p_(nm*n_A),M_k_q_(nm*n_A))         
      enddo !do nm=0,n_M-1
...
c$$$     $     ,S_k_p_(ns*n_A),M_k_p_(nm*n_A),Z_p_)
...
c$$$      write(6,'(A)') ' test_innerproduct_slow_dr: '
...
c$$$      enddo
     ...
     $        ,' away from the true translations and rotations.'
      ...
      do nm=0,n_M-1
         ...
         call get_interchange_delta(delta_x,delta_y
     $     ,gamma_z_est_(nm),delta_x_tmp,delta_y_tmp)
         azimu_b_est_(nm) = 0.0d0
         delta_x_est_(nm) = delta_x_tru_(nm) - delta_x_tmp
         delta_y_est_(nm) = delta_y_tru_(nm) - delta_y_tmp
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      if (flag_fig) then
         ...
         call Fig_gen_ctf_ver1(n_r,n_polar_a_,grid_k_p_,n_M,n_A,M_k_p_
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $     *n_S,') innerproducts'
      ...
      call test_innerproduct_batch_SM_wrapper_0a(verbose,n_M_sub,n_r
     $     ,n_polar_a_,grid_k_p_,max_x_c,n_S,n_A ,S_k_p_
     $     ,tesselation_distance_req ,alpha_polar_a_,alpha_azimu_b_ ,n_M
     $     ,n_A,N_k_p_ ,n_CTF,n_A ,CTF_k_p_,n_alpha ,alpha_est_
     $     ,alpha_update_f,N_pixels_in ,displacement_max ,n_delta_x
     $     ,n_delta_y,n_gamma_z, svd_calculation_type ,eps_svd,C_M_
     $     ,n_SM_use)
      timing_toc = omp_get_wtime()
      if (verbose.gt.0) then
         write(6,'(A,A,F8.3)') 'test_innerproduct_batch_SM_wrapper_0a:'
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        ,n_A,N_k_p_ ,n_A,CTF_k_p_ ,n_alpha,alpha_est_ ,min(16
     $        ,n_M),fname)
      end if
 ...
 10    continue
      ...
      if (verbose.gt.1) then
         write(6,'(A)') '[finished test_innerproduct_slow_dr]'
      end if
