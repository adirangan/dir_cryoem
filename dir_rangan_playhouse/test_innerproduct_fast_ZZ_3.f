      subroutine test_innerproduct_fast_ZZ_3(verbose,rseed
     $     ,flag_RTRT_vs_RTTR,n_delta_x,delta_x_,n_delta_y,delta_y_
     $     ,n_gamma_z,gamma_z_,n_r ,grid_p_,n_w_,n_A,n_S
     $     ,S_alpha_polar_a_,S_alpha_azimu_b_,n_M_sub,polar_a_est_
     ...
     $     ,displacement_max,C_M_,CTF_R_S_ ,n_M_tot,S_T_T_R_CTF_M_q__
     $     ,ZZ_)
      implicit none
     ...
     $     ,l2_norm_upd_(0:n_M_sub-1)
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[entering test_innerproduct_fast_ZZ_3]'
      ...
      if (verbose.gt.1) then
         ...
         call write_sub_r8(n_S,S_alpha_polar_a_,19 ,
     $        ' S_alpha_polar_a_: ')
         call write_sub_r8(n_S,S_alpha_azimu_b_,19 ,
     $        ' S_alpha_azimu_b_: ')
         ...
         call write_sub_r8(n_M_sub,polar_a_upd_,15 , ' polar_a_upd_: ')
         call write_sub_r8(n_M_sub,azimu_b_upd_,15 , ' azimu_b_upd_: ')
         call write_sub_r8(n_M_sub,gamma_z_upd_,15 , ' gamma_z_upd_: ')
         call write_sub_r8(n_M_sub,delta_x_upd_,15 , ' delta_x_upd_: ')
         call write_sub_r8(n_M_sub,delta_y_upd_,15 , ' delta_y_upd_: ')
         call write_sub_r8(n_M_sub,l2_norm_upd_,15 , ' l2_norm_upd_: ')
         call write_sub_r8(n_M_sub,ctf_ind_upd_,15 , ' ctf_ind_upd_: ')
         call write_sub_r8(n_M_sub,S_index_upd_,15 , ' S_index_upd_: ')
         ...
         call write_sub_c16(n_gamma_z*n_S*1,CTF_R_S_,11,' CTF_R_S_: ')
            ...
            do ndy=0,n_delta_y-1
               ...
               do ndx=0,n_delta_x-1
                  ...
                  do ngz=0,n_gamma_z-1
                     gamma_z = gamma_z_(ngz)
                     nC = ndx + ndy*n_delta_x + ngz*n_delta_x*n_delta_y
                     write(6,'(3(A,I4),A,F8.4)') ' ndx ' , ndx , ' ndy '
     $                    , ndy , ' ngz ' , ngz , ' ZZ ' , real(ZZ_(nC))
                  enddo !do ngz
               enddo !do ndx
            enddo ! do ndy               
            end if !if (verbose.gt.2) then
c$$$            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            call test_innerproduct_fast_ZZ_scan_1(flag_RTRT_vs_RTTR
...
c$$$            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     ...
     $           ,delta_y_est_(nm),gamma_z_SM_(ns_use),delta_x_tmp
     $           ,delta_y_tmp)
         ...
         if (flag_RTRT_vs_RTTR.eqv..false.) then
            delta_x_tmp = delta_x_est_(nm) + delta_x_SM_(ns_use)
            delta_y_tmp = delta_y_est_(nm) + delta_y_SM_(ns_use)
            call get_interchange_delta(delta_x_tmp
     $           ,delta_y_tmp,gamma_z_SM_(ns_use),delta_x_upd_(nm)
     $           ,delta_y_upd_(nm))
            gamma_z_upd_(nm) = gamma_z_SM_(ns_use) + gamma_z_est_(nm)
...
c$$$            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[finished test_innerproduct_fast_ZZ_3]'
      end if !if (verbose.gt.0) then
