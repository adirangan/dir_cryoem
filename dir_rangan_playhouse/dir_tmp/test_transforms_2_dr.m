...
imagesc_c(2*n_r,k_c_,2*n_r,k_c_,real(decenter2(S_k_c_)),[],colormap('jet'));
xlim(max_k_c/2*[-1,1]);ylim(max_k_c/2*[-1,1]);
title('real(S_k_c_)','Interpreter','none');
set(gca,'XTick',[],'YTick',[],'Ydir','normal'); axis square;
subplot(2,pcols,2+1*pcols); 
imagesc_c(2*n_r,k_c_,2*n_r,k_c_,imag(decenter2(S_k_c_)),[],colormap('jet'));
xlim(max_k_c/2*[-1,1]);ylim(max_k_c/2*[-1,1]);
...
M_k_p_ = interp_c_to_p(2*n_r,max_k_c,2*n_r,max_k_c,M_k_c_,n_r,grid_k_p_,n_w_,n_A);
N_k_p_ = rotate_p_to_p_lagrange(n_r,n_w_,n_A,S_k_p_,gamma_z_tru);
disp(sprintf(' %% create M and N time: %0.4f',toc));
flag_disp=0;
if flag_disp;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
figure(2); pcols=3;
...
%%%%%%%%%%%%%%%% ;
disp(sprintf(' %% calculate C time: %0.4f',toc));
disp(sprintf(' %% norm(D_-C_)/norm(D_) = %0.16f/%0.16f = %0.16f',norm(D_-C_,'fro'),norm(D_,'fro'),norm(D_-C_,'fro')/norm(D_,'fro')));
%%%%%%%%;
flag_disp=0;
if flag_disp;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
...
M_k_c_ = fft2(M_x_c_); M_k_c_ = recenter2(M_k_c_); M_k_c_ = M_k_c_(1+tmp_kx_ij,1+tmp_ky_ij); M_k_c_ = decenter2(M_k_c_);
N_k_c_ = transf_c_to_c(2*n_r,max_k_c,2*n_r,max_k_c,S_k_c_,delta_x_tru,delta_y_tru);
...
disp(sprintf(' %% calculate M N C time: %0.4f',toc));
flag_disp=0;
if flag_disp;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
figure(4); pcols=4;
...
%%%%%%%%%%%%%%%%;
disp(sprintf(' %% calculate Z time: %0.4f',toc));
disp_flag=1;
if flag_disp;
figure(5);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
