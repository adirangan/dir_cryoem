      subroutine test_alpha_update_1(verbose,half_diameter_x_c_tmp
     ...
     $     ,n_S_sample_max,n_SM,n_MS,n_delta_x,n_delta_y,n_gamma_z
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      ...
      integer n_MS ! maximum number of templates to store per image when calculating innerproducts. Either this or n_SM should be nonzero. ;
      integer n_SM ! maximum number of templates to store per image when calculating innerproducts. Either this or n_MS should be nonzero. ;
...
c$$$  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
      ...
      integer *4 n_MS_use !temporary: total number of image-template pairs considered when calculating innerproducts. Might be smaller than the total number when calculating innerproducts adaptively. ;
      ...
      integer n_XX,n_SM_use,n_MS_use,nSM,nMS !temporary: values used to track larger of n_SM,n_MS. ;
...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
      ...
      if (verbose.gt.0) write(6,'(A)') '[entering test_alpha_update_1]'
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      ...
      n_SM_use = 0
      n_MS_use = 0
      if (n_SM.gt.n_MS) then
         n_SM_use = min(n_S_sample,n_SM)
         n_XX = n_SM_use*n_M_sample
      end if !if (n_SM.gt.n_MS) then
      if (n_SM.le.n_MS) then
         n_MS_use = min(n_M_sample,n_MS)
         n_XX = n_S_sample*n_MS_use
      end if !if (n_SM.le.n_MS) then
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        , n_MS_use , ' / ' , n_M_sample *n_S_sample , ' = ' ,
     $        floor((n_MS_use*100.0d0) /(1.0d0*n_M_sample
     $        *n_S_sample)) , '% , ; time for each: ' , (timing_toc
     $        -timing_tic) / max(1,n_MS_use)
      end if
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      ...
      if (verbose.gt.0) write(6,'(A)') '[finished test_alpha_update_1]'
