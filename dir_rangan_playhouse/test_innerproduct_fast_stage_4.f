c$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
      subroutine test_innerproduct_fast_stage_4(verbose,rseed,n_omp_sub
...
c$$$      parameters for omp
      integer *4 nomp_sub,n_r_tmp,n_A_tmp,n_X_tmp,n_Y_tmp,n_F_tmp
...
c$$$      parameters for memory map
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[entering test_innerproduct_fast_stage_4]'
      end if      
...
c$OMP END PARALLEL
     ...
     $     timing_tmp
      ...
      if (flag_test) then
         if (verbose.gt.1) then
            write(6,'(A)')
     $           ' calling test_innerproduct_fast_SxTTRM_1 to test. '
            write(6,'(A)') ' '
         end if !if (verbose.gt.1) then
...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c$$$      Now update n_SM_ and alpha_SM
c$$$      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
c$OMP DO 
      do nM_sub=0,n_M_sub_use-1
         ...
         n_Y_tmp = IM_per*n_alpha*n_SM_max
c$$$         call test_innerproduct_fast_Zstore_0(verbose-1,rseed
c$$$     $        ,flag_RTRT_vs_RTTR,n_delta_x,delta_x_,n_delta_y,delta_y_
c$$$     $        ,n_gamma_z,gamma_z_,n_r ,grid_p_,n_w_,n_A,n_S
c$$$     $        ,S_alpha_polar_a_,S_alpha_azimu_b_,nM_per
c$$$     $        ,polar_a_est_(IM_per) ,azimu_b_est_(IM_per)
c$$$     $        ,gamma_z_est_(IM_per) ,delta_x_est_(IM_per)
c$$$     $        ,delta_y_est_(IM_per) ,l2_norm_est_(IM_per)
c$$$     $        ,ctf_ind_est_(IM_per) ,S_index_est_(IM_per)
c$$$     $        ,alpha_update_f ,displacement_max,n_SM_max,n_SM_(IM_per)
c$$$     $        ,alpha_SM__(IM_per*n_alpha*n_SM_max) ,C_M_(IM_per)
c$$$     $        ,CTF_R_S_ ,n_M ,S_T_T_R_CTF_M_q__(n_X_tmp)
c$$$     $        ,ZZ_omp__(n_A_tmp))
...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[finished test_innerproduct_fast_stage_4]'
      end if
