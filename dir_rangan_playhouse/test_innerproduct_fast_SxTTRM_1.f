      subroutine test_innerproduct_fast_SxTTRM_1(verbose,n_delta_x
...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[entering test_innerproduct_fast_SxTTRM_1]'
      ...
      call test_innerproduct_bruteforce_0(2,n_r ,grid_p_ ,n_w_
     $     ,1.0d0 ,S_p__(ld_S*ns),M_p__(ld_M*nm)
     $     ,CTF_p__(ld_CTF*nctf) ,delta_x_est_(nm) ,delta_y_est_(nm)
     $     ,gamma_z_est_(nm) ,n_delta_x ,delta_x_ ,n_delta_y ,delta_y_
     $     ,n_gamma_z ,gamma_z_)
...
c$$$         write(6,'(2(A,I0))') ' ndx ' , ndx , ' ndy ' , ndy
...
c$$$         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     ...
     $           nx3))**2
         ...
         Z_tmp = zsqrt(Z_tmp)
         if (verbose.gt.-2) then
            write(6,'(A,A,I0,A,I0,A,I0,A,I0,A,I0,A,F16.8,A,F16.8)') ,
...
c$$$         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
         ...
         Z_tmp = zsqrt(Z_tmp)
         if (verbose.gt.-2) then
            write(6, '(A,A,A,I0,A,I0,A,I0,A,I0,A,I0,A,F16.8,A,F16.8)')
...
c$$$         call write_all_c16(n_gamma_z ,ZZ2_,7 ,' ZZ2_: ')         
      ...
      if (verbose.gt.0) then
         write(6,'(A)')
     $        '[finished test_innerproduct_fast_SxTTRM_1]'
      end if !if (verbose.gt.0) then
      end
