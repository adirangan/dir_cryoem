...
def convert_k_p_to_spharm_4(
    ...
    a_k_Y_ = torch.zeros(int(torch.sum(n_lm_).item())).to(dtype=torch.complex64);
    ix=0;
    for nk_p_r in range(n_k_p_r):
        ...
        a_k_all_sub_ = torch.reshape(a_k_all_[index_sub_], mtr((n_sub,1)));
        a_k_Y_sub_ = torch.zeros(mtr((n_lm,1))).to(dtype=torch.complex64);
        str_einsum = msr('ya') + ',' + msr('a') + '->' + msr('y') ;
        a_k_Y_sub_ = torch.einsum( str_einsum , torch.conj(Ylm_lma__).to(dtype=torch.complex64) , (a_k_all_sub_*weight_shell_k_sub_).ravel().to(dtype=torch.complex64) ) ;
        tmp_index_lhs_ = torch.arange(ix,ix+n_lm);
