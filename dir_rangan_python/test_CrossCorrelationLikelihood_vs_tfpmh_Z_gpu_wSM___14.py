...
from EMPM.likelihoods import template_first_comparator, compute_cross_correlation_complete ;
from cross_correlation_fixtures_20251127 import (
    make_polar_grid,
    make_viewing_angles,
...
flag_speed_vs_error = 1;

disp(sprintf(' %% testing CrossCorrelationLikelihood vs tfpmh_Z_gpu_wSM___14: flag_speed_vs_error %d',flag_speed_vs_error));
n_1 = 1; n_2 = 2;
...
n_gamma_z = n_w_max; gamma_z_ = torch.linspace(0,2*pi,1+n_gamma_z); gamma_z_ = gamma_z_[:n_gamma_z];
#%%%%;
radius_max = k_p_r_max;
dist_radii = k_eq_d * pi/2;
n_inplanes = n_w_max;
...
polar_grid.radius_shells = k_p_r_;
polar_grid.n_shells = n_k_p_r;
polar_grid.theta_shell = gamma_z_;
polar_grid.n_inplanes = n_w_max;
polar_grid.radius_points = k_p_r_wk_;
polar_grid.theta_points = k_p_w_wk_;
polar_grid.weight_points = weight_2d_wk_;
#%%%%;
...
CTF_k_p_wk_ = torch.reshape(torch.tile(torch.reshape(CTF_k_p_r_k_,mtr((1,n_k_p_r))),mtr((n_w_max,1))),mtr((n_w_sum,1)));
#%%%%;

#%%%%%%%%;
M_k_p_wkM___ = torch.reshape(M_k_p_wkM__,mtr((n_w_max,n_k_p_r,n_M)));
M_object = Images( fourier_data = FourierImages( images_fourier = M_k_p_wkM___ , polar_grid = polar_grid ) ) ;
S_k_p_wkS___ = torch.reshape(S_k_p_wkS__,mtr((n_w_max,n_k_p_r,n_S)));
S_object = Templates( fourier_data = FourierImages( images_fourier = S_k_p_wkS___ , polar_grid = polar_grid ) ) ;
CTF_k_p_wk1___ = torch.reshape(CTF_k_p_wk_,mtr((n_w_max,n_k_p_r,1))).astype(np.float64)
CTF_object = CTF(
    polar_grid = polar_grid,
    box_size = 2.0, # is this really in Angstroms ? ;
    anisotropy = True,
...
cc = CrossCorrelationLikelihood(
    ...
    n_displacements_x = int(np.ceil(np.sqrt(n_delta_v))),
    n_displacements_y = int(np.ceil(np.sqrt(n_delta_v))),
    precision = precision,
    device = device,
    verbose = False
) ;
...
res = cc._compute_cross_correlation_likelihood(
    ...
    images_fourier = M_object.images_fourier,
...
tmp_t=toc(tmp_t);
if (flag_verbose>0): disp(sprintf(' %% CrossCorrelationLikelihood: %0.6f',tmp_t)); #end;
#%%%%%%%%;
...
tmp_index_rhs_ = matlab_index_2d_0(n_k_p_r,':',size(tmp_UX_kn__,1),torch.arange(pm_n_UX_rank));
pm_UX_kn__ = torch.reshape(tmp_UX_kn__.ravel()[tmp_index_rhs_],mtr((n_k_p_r,pm_n_UX_rank)));
