...
def test_weight_3d_k_p_qk(
    ...
    for nfrequency in range(n_frequency):
        ...
        T_k_p_qk_ = torch.exp(torch.tensor(2*np.pi*1j* (k_p_0_qk_*delta_T_0 + k_p_1_qk_*delta_T_1 + k_p_2_qk_*delta_T_2) ))
        ...
        I_T_quad = torch.sum(T_k_p_qk_ * torch.tensor(weight_3d_k_p_qk_)) ; 
