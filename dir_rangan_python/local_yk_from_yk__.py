...
def local_yk_from_yk__(
    ...
    tmp_yk_ = torch.zeros(n_lm_sum).to(dtype=torch.complex64);
    for nk_p_r in range(n_k_p_r):
        n_lm = int(n_lm_[nk_p_r].item());
        tmp_index_lhs_ = int(n_lm_csum_[nk_p_r].item()) + torch.arange(n_lm);
        tmp_index_rhs_ = matlab_index_2d_0(n_lm_max,torch.arange(n_lm),n_k_p_r,nk_p_r);
        tmp_yk_[tmp_index_lhs_] = tmp_yk__.ravel()[tmp_index_rhs_].to(dtype=torch.complex64);
