...
def local_yk_from_yk__(
    ...
    tmp_yk_ = torch.zeros(n_y_sum).to(dtype=torch.complex64);
    for nk_p_r in range(n_k_p_r):
        n_y = int(n_y_[nk_p_r].item());
        tmp_i8_index_lhs_ = int(n_y_csum_[nk_p_r].item()) + torch.arange(n_y);
        tmp_i8_index_rhs_ = matlab_index_2d_0(n_y_max,torch.arange(n_y),n_k_p_r,nk_p_r);
        tmp_yk_[tmp_i8_index_lhs_] = tmp_yk__.ravel()[tmp_i8_index_rhs_].to(dtype=torch.complex64);
