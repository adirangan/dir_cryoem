...
double R01GET(unsigned long int *rseed_p)
{
 /* basically:
  ...
  int verbose=0;
  RGET(rseed_p); //*rseed_p = (*rseed_p*POW2RPOWPLUSRADD%POW22RPOWMINUSONE);
...
void RSEED_adv8(unsigned long int *rseed_p)
{
  ...
  if (verbose){ printf(" %% [entering RSEED_adv8], rseed %lld\n",*rseed_p);}
  for (niteration=0;niteration<n_iteration;niteration++){ RGET(rseed_p); if (verbose){ printf(" %% rseed %lld\n",*rseed_p);}}
  if (verbose){ printf(" %% [finished RSEED_adv8], rseed %lld\n",*rseed_p);}
}
...
void R01GET_test()
{
  ...
  GLOBAL_tic(0);  
  rseed=3; RSEED_adv8(&rseed); RSEED_adv8(&rseed); 
  printf(" %% rseed %lld\n",rseed);
  for (niteration=0;niteration<n_iteration;niteration++){ d_out_[niteration] = R01GET(&rseed);}
  GLOBAL_toc(0,1," R01GET: ");
  array_sub_printf(d_out_,"double",1,1,n_iteration,4," % d_out_: ");
  printf(" %% rseed %lld\n",rseed);
  free1(&d_out_);
}
