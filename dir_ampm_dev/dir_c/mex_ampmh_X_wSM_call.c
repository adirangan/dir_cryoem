 ...
 )
{
  ...
  int flag_local_t = !flag_omp;
  int ntick=0,n_tick = 11;
  ...
  int ntick_permute=0;
  int ntick_svd_SVUXM_SMwl=1;
  int ntick_svd_SVUXM_lwSM=2;
  int ntick_svd_SVUXM_lwSM_prm=3;
  int ntick_svd_SVUXM_lwSM_fft=4;
  int ntick_svd_SVUXM_lwsM=5;
  int ntick_svd_USESVUXM_dwSM=6;
  int ntick_f2_dSM=7;
  int ntick_X_sub_dwSM=8;
  int ntick_I_value_sub_dwSM=9;
  int ntick_arrange_max=10;
  double elct_sum_[n_tick],elrt_sum_[n_tick];
  memset(elct_sum_,0,n_tick*sizeof(double));
  memset(elrt_sum_,0,n_tick*sizeof(double));
  /* %%%% */
  if (verbose){ printf(" %% [entering mex_ampmh_X_wSM___15_omp]\n");}
  /* %%%%%%%%%%%%%%%% */
  if (flag_local_t){ local_tic(0,t_start_,d_start_);}
  ...
  if (flag_local_t){ local_toc(0,t_start_,t_final_,d_start_,d_final_,l_msec_,l_ssec_,l_usec_,elct_,elrt_,tab,verbose-1,"mex_ampmh_X_wSM___15_omp (initialization): ");}
  /* %%%%%%%%%%%%%%%% */
  n_Mbatch = ceil((double)n_M/(double)n_M_per_Mbatch);
  if (verbose>2){ printf(" %% n_Mbatch %d\n",n_Mbatch);}
  index_M_in_Mbatch_ = (int *) malloc(n_M_per_Mbatch*sizeof(int));
  n_Sbatch = ceil((double)n_S/(double)n_S_per_Sbatch);
  if (verbose>2){ printf(" %% n_Sbatch %d\n",n_Sbatch);}
  ...
  if (n_M_sub>0){
    ...
    /* %%%% */
    if (flag_local_t){ local_tic(ntick_permute,t_start_,d_start_);}
    if (flag_slow){
      tab =
  	(unsigned long long int)FTK_n_svd_l
  	*(unsigned long long int)n_w_max
    ...
    if (flag_fast){
  	...
  	*(unsigned long long int)(n_M_per_Mbatch*nMbatch);
      ...
      /* if (flag_fast){ } */}
    if (flag_local_t){ local_toc(ntick_permute,t_start_,t_final_,d_start_,d_final_,l_msec_,l_ssec_,l_usec_,elct_,elrt_,0,0,""); elct_sum_[ntick_permute] += elct_[ntick_permute]; elrt_sum_[ntick_permute] += elrt_[ntick_permute]; }
    /* %%%% */
    /*
    ...
    */
    /* %%%% */
    if (flag_local_t){ local_tic(ntick_svd_SVUXM_SMwl,t_start_,d_start_);}
    if (flag_slow){
    ...
    if (flag_fast){
  	...
  	/* for (nl=0;nl<FTK_n_svd_l;nl++){ } */}
      /* if (flag_fast){ } */}
    if (flag_local_t){ local_toc(ntick_svd_SVUXM_SMwl,t_start_,t_final_,d_start_,d_final_,l_msec_,l_ssec_,l_usec_,elct_,elrt_,0,0,""); elct_sum_[ntick_svd_SVUXM_SMwl] += elct_[ntick_svd_SVUXM_SMwl]; elrt_sum_[ntick_svd_SVUXM_SMwl] += elrt_[ntick_svd_SVUXM_SMwl]; }
    /* %%%% */
    /* svd_SVUXM_lwSM____ = permute(ifft(permute(svd_SVUXM_SMwl____,[3,4,1,2]),[],1)*n_w_max,[2,1,3,4]); */
    /* %%%% */
    if (flag_local_t){ local_tic(ntick_svd_SVUXM_lwSM,t_start_,d_start_);}
    if (flag_slow){
      if (flag_local_t){ local_tic(ntick_svd_SVUXM_lwSM_prm,t_start_,d_start_);}
  	...
  	for (nw=0;nw<n_w_max;nw++){
  	  ...
  	  /* for (nw=0;nw<n_w_max;nw++){ } */}
  	/* for (nl=0;nl<FTK_n_svd_l;nl++){ } */}
      if (flag_local_t){ local_toc(ntick_svd_SVUXM_lwSM_prm,t_start_,t_final_,d_start_,d_final_,l_msec_,l_ssec_,l_usec_,elct_,elrt_,0,0,""); elct_sum_[ntick_svd_SVUXM_lwSM_prm] += elct_[ntick_svd_SVUXM_lwSM_prm]; elrt_sum_[ntick_svd_SVUXM_lwSM_prm] += elrt_[ntick_svd_SVUXM_lwSM_prm]; }
      if (flag_local_t){ local_tic(ntick_svd_SVUXM_lwSM_fft,t_start_,d_start_);}
      tab =
  	(unsigned long long int)n_w_max
  	*(unsigned long long int)n_S
  	*(unsigned long long int)n_M_per_Mbatch
  	...
  	*(unsigned long long int)FTK_n_svd_l;
  	...
  	/* for (tabA=0;tabA<tabB;tabA++){ } */}
      if (flag_local_t){ local_toc(ntick_svd_SVUXM_lwSM_fft,t_start_,t_final_,d_start_,d_final_,l_msec_,l_ssec_,l_usec_,elct_,elrt_,0,0,""); elct_sum_[ntick_svd_SVUXM_lwSM_fft] += elct_[ntick_svd_SVUXM_lwSM_fft]; elrt_sum_[ntick_svd_SVUXM_lwSM_fft] += elrt_[ntick_svd_SVUXM_lwSM_fft]; }
      if (flag_local_t){ local_tic(ntick_svd_SVUXM_lwSM_prm,t_start_,d_start_);}
      tab = (unsigned long long int)n_S*(unsigned long long int)n_M_sub*(unsigned long long int)n_w_max*(unsigned long long int)FTK_n_svd_l;
    ...
    if (flag_fast){
  	...
  	*(unsigned long long int)n_M_sub_rup;
      ...
      if (flag_local_t){ local_toc(ntick_svd_SVUXM_lwSM_prm,t_start_,t_final_,d_start_,d_final_,l_msec_,l_ssec_,l_usec_,elct_,elrt_,0,0,""); elct_sum_[ntick_svd_SVUXM_lwSM_prm] += elct_[ntick_svd_SVUXM_lwSM_prm]; elrt_sum_[ntick_svd_SVUXM_lwSM_prm] += elrt_[ntick_svd_SVUXM_lwSM_prm]; }
      /* if (flag_fast){ } */}
    if (flag_local_t){ local_toc(ntick_svd_SVUXM_lwSM,t_start_,t_final_,d_start_,d_final_,l_msec_,l_ssec_,l_usec_,elct_,elrt_,0,0,""); elct_sum_[ntick_svd_SVUXM_lwSM] += elct_[ntick_svd_SVUXM_lwSM]; elrt_sum_[ntick_svd_SVUXM_lwSM] += elrt_[ntick_svd_SVUXM_lwSM]; }
    /* %%%% */
	...
	if (flag_local_t){ local_toc(ntick_svd_SVUXM_lwsM,t_start_,t_final_,d_start_,d_final_,l_msec_,l_ssec_,l_usec_,elct_,elrt_,0,0,""); elct_sum_[ntick_svd_SVUXM_lwsM] += elct_[ntick_svd_SVUXM_lwsM]; elrt_sum_[ntick_svd_SVUXM_lwsM] += elrt_[ntick_svd_SVUXM_lwsM]; }
  	...
  	/* %%%% */
	if (flag_local_t){ local_tic(ntick_svd_USESVUXM_dwSM,t_start_,d_start_);}
  	if (flag_slow){
  	...
  	if (flag_fast){
  	  ...
  	  /* if (flag_fast){ } */}
	if (flag_local_t){ local_toc(ntick_svd_USESVUXM_dwSM,t_start_,t_final_,d_start_,d_final_,l_msec_,l_ssec_,l_usec_,elct_,elrt_,0,0,""); elct_sum_[ntick_svd_USESVUXM_dwSM] += elct_[ntick_svd_USESVUXM_dwSM]; elrt_sum_[ntick_svd_USESVUXM_dwSM] += elrt_[ntick_svd_USESVUXM_dwSM]; }
  	/* %%%% */
  	/*
  	...
  	*/
  	/* %%%% */
	if (flag_local_t){ local_tic(ntick_f2_dSM,t_start_,d_start_);}
  	tab =
  	  (unsigned long long int)n_S_sub
  	  *(unsigned long long int)n_M_sub
  	  *(unsigned long long int)FTK_n_delta_v;
  	...
  	for (nS_sub=0;nS_sub<n_S_sub;nS_sub++){
  	  ...
  	  for (nM_sub=0;nM_sub<n_M_sub;nM_sub++){
  		...
  		*(unsigned long long int)FTK_n_delta_v;
  	    ...
  	    /* for (nM_sub=0;nM_sub<n_M_sub;nM_sub++){ } */}
  	  /* for (nS_sub=0;nS_sub<n_S_sub;nS_sub++){ } */}
	if (flag_local_t){ local_toc(ntick_f2_dSM,t_start_,t_final_,d_start_,d_final_,l_msec_,l_ssec_,l_usec_,elct_,elrt_,0,0,""); elct_sum_[ntick_f2_dSM] += elct_[ntick_f2_dSM]; elrt_sum_[ntick_f2_dSM] += elrt_[ntick_f2_dSM]; }
  	/* %%%% */
  	/* X_sub_dwSM____ = repmat(reshape(n2_dSM___,[FTK.n_delta_v,1,n_S_sub,n_M_sub]),[1,n_w_max,1,1]) .* svd_USESVUXM_dwSM____; %<-- correlation. ; */
  	/* %%%% */
	if (flag_local_t){ local_tic(ntick_X_sub_dwSM,t_start_,d_start_);}
  	if (flag_slow){
  	  tab =
  	    (unsigned long long int)n_S_sub
  	    *(unsigned long long int)n_M_sub
  	...
  	if (flag_fast){
  	  ...
  	  for (nM_sub=0;nM_sub<n_M_sub;nM_sub++){
  	    ...
  	    /* for (nM_sub=0;nM_sub<n_M_sub;nM_sub++){ } */}
  	  /* if (flag_fast){ } */}
	if (flag_local_t){ local_toc(ntick_X_sub_dwSM,t_start_,t_final_,d_start_,d_final_,l_msec_,l_ssec_,l_usec_,elct_,elrt_,0,0,""); elct_sum_[ntick_X_sub_dwSM] += elct_[ntick_X_sub_dwSM]; elrt_sum_[ntick_X_sub_dwSM] += elrt_[ntick_X_sub_dwSM]; }
  	/* %%%% */
  	...
  	*/
  	/* %%%% */
 	if (flag_local_t){ local_tic(ntick_I_value_sub_dwSM,t_start_,d_start_);}
 	if (flag_compute_I_value){
  	  ...
  	  /* %%%% */
  	  /* if (flag_compute_I_value){ } */}
	if (flag_local_t){ local_toc(ntick_I_value_sub_dwSM,t_start_,t_final_,d_start_,d_final_,l_msec_,l_ssec_,l_usec_,elct_,elrt_,0,0,""); elct_sum_[ntick_I_value_sub_dwSM] += elct_[ntick_I_value_sub_dwSM]; elrt_sum_[ntick_I_value_sub_dwSM] += elrt_[ntick_I_value_sub_dwSM]; }
  	/* %%%% */
 	if (flag_local_t){ local_tic(ntick_arrange_max,t_start_,d_start_);}
  	/* %%%% */
  	/*
  	...
  	*/
  	...
  	if (flag_optimize_over_gamma_z==1){
  	  ...
  	  for (nM_sub=0;nM_sub<n_M_sub;nM_sub++){
  	    ...
  	    /* for (nM_sub=0;nM_sub<n_M_sub;nM_sub++){ } */}
  	  /* if (flag_optimize_over_gamma_z==1){ } */}
	if (flag_local_t){ local_toc(ntick_arrange_max,t_start_,t_final_,d_start_,d_final_,l_msec_,l_ssec_,l_usec_,elct_,elrt_,0,0,""); elct_sum_[ntick_arrange_max] += elct_[ntick_arrange_max]; elrt_sum_[ntick_arrange_max] += elrt_[ntick_arrange_max]; }
  	/* if (n_S_sub>0){ } */}
      /* for (nSbatch=0;nSbatch<n_Sbatch;nSbatch++){ } */}
    /* if (n_M_sub>0){ } */}
  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
  if (flag_local_t && verbose>1){
    printf(" %% permute            %0.6f/%0.6f\n",elct_sum_[ntick_permute],elrt_sum_[ntick_permute]);
    printf(" %% svd_SVUXM_SMwl     %0.6f/%0.6f\n",elct_sum_[ntick_svd_SVUXM_SMwl],elrt_sum_[ntick_svd_SVUXM_SMwl]);
    printf(" %% svd_SVUXM_lwSM     %0.6f/%0.6f\n",elct_sum_[ntick_svd_SVUXM_lwSM],elrt_sum_[ntick_svd_SVUXM_lwSM]);
    printf(" %% svd_SVUXM_lwSM_prm %0.6f/%0.6f\n",elct_sum_[ntick_svd_SVUXM_lwSM_prm],elrt_sum_[ntick_svd_SVUXM_lwSM_prm]);
    printf(" %% svd_SVUXM_lwSM_fft %0.6f/%0.6f\n",elct_sum_[ntick_svd_SVUXM_lwSM_fft],elrt_sum_[ntick_svd_SVUXM_lwSM_fft]);
    printf(" %% svd_SVUXM_lwsM     %0.6f/%0.6f\n",elct_sum_[ntick_svd_SVUXM_lwsM],elrt_sum_[ntick_svd_SVUXM_lwsM]);
    printf(" %% svd_USESVUXM_dwSM  %0.6f/%0.6f\n",elct_sum_[ntick_svd_USESVUXM_dwSM],elrt_sum_[ntick_svd_USESVUXM_dwSM]);
    printf(" %% f2_dSM             %0.6f/%0.6f\n",elct_sum_[ntick_f2_dSM],elrt_sum_[ntick_f2_dSM]);
    printf(" %% X_sub_dwSM         %0.6f/%0.6f\n",elct_sum_[ntick_X_sub_dwSM],elrt_sum_[ntick_X_sub_dwSM]);
    printf(" %% I_value_sub_dwSM   %0.6f/%0.6f\n",elct_sum_[ntick_I_value_sub_dwSM],elrt_sum_[ntick_I_value_sub_dwSM]);
    printf(" %% arrange_max        %0.6f/%0.6f\n",elct_sum_[ntick_arrange_max],elrt_sum_[ntick_arrange_max]);
    /* if (flag_local_t && verbose>1){ } */}
  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
 ...
 )
{
  ...
  double elct_[n_tick],elrt_[n_tick];
  /* %%%% */
  if (verbose){ printf(" %% [entering mex_ampmh_X_wSM___15] verbose %d\n",verbose);}
  /* %%%%%%%%%%%%%%%% */
...
int mex_ampmh_X_wSM_test()
{
  ...
  unsigned long long int na=0,n_a=0;
  /* %%%% */
  if (verbose){ printf(" %% [entering mex_ampmh_X_wSM_test]\n");}
  /* %%%%; */
  ...
  if (verbose){ printf(" %% [finished mex_ampmh_X_wSM_test]\n");}
  return 0;
}
