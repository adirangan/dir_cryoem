...
n_CTF_0in = 50; %<-- number of distinct CTFs. ;
...
a_k_p_quad_ = xxnufft3d3(n_xxx_u,x_u_0___(:)*eta,x_u_1___(:)*eta,x_u_2___(:)*eta,a_x_u_base_(:).*xxx_u_weight_(:),-1,1e-12,n_qk,2*pi*k_c_0_qk_/eta,2*pi*k_c_1_qk_/eta,2*pi*k_c_2_qk_/eta)/sqrt(2*pi)/sqrt(2*pi)/sqrt(2*pi);
...
%%%%%%%%;
fname_fig = sprintf('%s_jpg/S_k_q_wkS__',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1+nf);nf=nf+1;clf;figbig;
Slim_ = std(abs(S_k_p_wkS__(:)))*2.5*[-1,+1];
...
%%%%%%%%;
fname_mat = sprintf('%s_mat/M_k_p_wkM__.mat',dir_pm);
if (flag_recalc | ~exist(fname_mat,'file'));
disp(sprintf(' %% %s not found, creating',fname_mat));
%%%%%%%%;
...
tmp_t = toc(tmp_t); if (flag_verbose>0); disp(sprintf(' %% M_k_q_wkM__ time %0.2fs',tmp_t)); end;
%%%%%%%%;
fname_fig = sprintf('%s_jpg/M_k_q_wkM__',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1+nf);nf=nf+1;clf;figbig;
...
%%%%%%%%;
fname_fig = sprintf('%s_jpg/N_k_q_wkM__',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1+nf);nf=nf+1;clf;figbig;
...
%%%%%%%%;
fname_mat = sprintf('%s_mat/X_2d_Memp_d1_kkc___.mat',dir_pm);
if (flag_recalc | ~exist(fname_mat,'file'));
disp(sprintf(' %% %s not found, creating',fname_mat));
...
%%%%%%%%;
fname_mat = sprintf('%s_mat/X_2d_xavg_d0_kkc___.mat',dir_pm);
if (flag_recalc | ~exist(fname_mat,'file'));
disp(sprintf(' %% %s not found, creating',fname_mat));
...
fname_mat = sprintf('%s_mat/tfpm_a_k_Y_reco_from_%s__.mat',dir_pm,tmp_str);
if (flag_recalc | ~exist(fname_mat,'file'));
disp(sprintf(' %% %s not found, creating',fname_mat));
%%%%%%%%;
parameter = struct('type','parameter');
parameter.flag_verbose = 1;
parameter.flag_CTF_anisotropic = 0;
parameter.tolerance_master = tolerance_master;
parameter.n_iteration = 8;
parameter.delta_r_max = 0.03;
parameter.n_delta_v_requested = 24;
parameter.delta_r_upb = 0.25;
parameter.fname_align_a_k_Y_pre = sprintf('%s_mat/tfpm_a_k_Y_reco_from_%s__',dir_pm,tmp_str);
[ ...
 parameter ...
,a_k_Y_reco_yki__ ...
,corr_a_k_Y_i_ ...
...
%%%%%%%%;
fname_fig = sprintf('%s_jpg/euler_angle_true_M_',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
dscale = 2.5;
