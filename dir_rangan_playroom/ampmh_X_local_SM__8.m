...
%%%%%%%%;
% Uses ampmh_X_SM__8 to conduct a local search. ;
%%%%%%%%;

verbose = 0;
if (verbose>0); disp(sprintf(' %% [entering ampmh_X_local_SM__8]')); end;
...
if (~isfield(parameter,'n_neighborhood_retain')); parameter.n_neighborhood_retain = 1; end; %<-- parameter_bookmark. ;
%%%%%%%%;
...
if ( isempty(template_tree)); %<-- default to global search. ;
...
assert(n_S_max==n_S); %<-- ensure that, at the very least, the number of tree templates matches the number passed in. ;
...
if (verbose>0); disp(sprintf(' %% [finished ampmh_X_local_SM__8]')); end;
