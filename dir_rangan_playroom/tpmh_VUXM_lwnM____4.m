...
UX__ = orth(randn(n_k_p_r,n_UX_rank));
X_weight_r_ = rand(n_k_p_r,1);
%%%%;
flag_check=0;
if flag_check;
tmp_t = tic();
VUXM_2_lwnM____ = tpmh_VUXM_lwnM____2(FTK,n_k_p_r,n_w_,n_M,M_k_q__,n_UX_rank,UX__,X_weight_r_);
...
fnorm_disp(flag_verbose,'VUXM_3_lwnM____',VUXM_3_lwnM____,'VUXM_4_lwnM____',VUXM_4_lwnM____);
...
%{
tmp_t = tic();
VUXM_centered_padded_lwMn____ = zeros(FTK.n_svd_l,n_w_max+2*l_max,n_M,n_UX_rank);
...
for nUX_rank=0:n_UX_rank-1;
for nM=0:n_M-1;
VUXM_centered_padded_lwMn____(1+nl,1+index_nw_centered_padded_out_,1+nM,1+nUX_rank) = V_UX_lrn___(1+nl,:,1+nUX_rank)*M_k_q_centered_padded_rwM___(:,1+index_nw_centered_padded_0in_,1+nM) / n_w_max ;
...
tmp_t = toc(tmp_t); if (flag_verbose); disp(sprintf(' %% VUXM_centered_padded_lwMn____: %0.6fs',tmp_t)); end;
tmp_t = tic();
VUXM_lwnM____ = permute(VUXM_centered_padded_lwMn____(:,1+l_max+index_nw_zerobased_from_centered_,:,:),[1,2,4,3]);
%VUXM_lwnM____ = permute(VUXM_centered_lwMn____(:,1+index_nw_zerobased_from_centered_,:,:),[1,2,4,3]);
tmp_t = toc(tmp_t); if (flag_verbose); disp(sprintf(' %% VUXM_lwMn____: %0.6fs',tmp_t)); end;
%}
...
index_nw_centered_padded_out_ = [index_nw_centered_padded_out_start:index_nw_centered_padded_out_final];
index_nw_centered_padded_0in_ = [index_nw_centered_padded_0in_start:index_nw_centered_padded_0in_final];
VUXM_centered_nwMl____(:,2:end,:,1+nl) = pagemtimes(reshape(V_UX_nrl___(:,:,1+nl),[n_UX_rank,n_k_p_r,1])/n_w_max,reshape(M_k_q_centered_padded_rwM___(:,1+index_nw_centered_padded_0in_,:),[n_k_p_r,n_w_max-1,n_M]))  ;
end;%for nl=0:FTK.n_svd_l-1;
tmp_t = toc(tmp_t); if (flag_verbose); disp(sprintf(' %% VUXM_centered_padded_lwMn____: %0.6fs',tmp_t)); end;
tmp_t = tic();
VUXM_lwnM____ = permute(VUXM_centered_nwMl____(:,1+index_nw_zerobased_from_centered_,:,:),[4,2,1,3]);
tmp_t = toc(tmp_t); if (flag_verbose); disp(sprintf(' %% VUXM_lwMn____: %0.6fs',tmp_t)); end;

%fnorm_disp(1,'A',VUXM_centered_lwMn____(:,1+[0:n_w_max-1],:,:),'B',VUXM_centered_padded_lwMn____(:,1+l_max+[0:n_w_max-1],:,:));
