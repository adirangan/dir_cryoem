...
%%%%%%%%;
% Similar to test_pm_clathrin_7, ;
% except that we use a circular mask of radius x_p_r_mask in all cases. ;
% To be consistent with this circular mask, ;
% we center the molecule (i.e., mean zero) ;
% within a sphere of radius x_p_r_mask, ;
% and set the volume to be identically zero outside this radius. ;
% Of course, the reconstructed volume will not exactly adhere to these consraints ;
% (due to discretization error in the k_p and k_Y bases). ;
%%%%%%%%;
...
% global_parameter=struct('type','parameter','flag_invert',1,'flag_center_volume',0);fname_prefix='clathrin_montage_Micrograph';fname_infix='clathrin';Pixel_Spacing=1.06;fname_nopath_volume='6sct_one_leg_bf60_center.mrc';fname_nopath_micrograph='simulated_clathrin_t1000A_d5000A_1.06Apix_p1_384.mrc';

verbose=1;
if (verbose); disp(sprintf(' %% [entering test_pm_clathrin_8]')); end;
...
%%%%%%%%;
save(fname_mat ...
     ,'half_diameter_x_c','diameter_x_c','x_p_r_max','x_p_r_mask','n_x_u_pack','n_pack','pack_row_ij_','pack_col_ij_','pack_val_ij_','x_u_pack_' ...
...
%%%%%%%%;
% Now subtract a constant from the volume so that the average in a sphere is zero. ;
%%%%%%%%;
x_u_r___ = sqrt(x_u_0___.^2 + x_u_1___.^2 + x_u_2___.^2);
a_x_u_mask_ = (x_u_r___<=x_p_r_mask);
x_p_r_mask = sqrt(0.5);
%%%%%%%%
% quickly check to see that the mask is not too small. ;
%%%%%%%%;
flag_disp=0;
if flag_disp;
figure(1+nf);nf=nf+1;clf;figmed;
subplot(1,2,1);isosurface_f_x_c_0(a_x_u_pack_,95.5); title('a pack');
subplot(1,2,2);isosurface_f_x_c_0(a_x_u_pack_.*a_x_u_mask_,95.5); title('a mask');
end;%if flag_disp;
%%%%%%%%;
sum_A1M1 = sum(a_x_u_base_.^1.*a_x_u_mask_.^1,'all');
sum_A0M1 = sum(a_x_u_base_.^0.*a_x_u_mask_.^1,'all');
a_avg0_x_u_base_ = a_x_u_mask_.*(a_x_u_base_ - sum_A1M1./max(1e-12,sum_A0M1));
tmp_sum_A1M1 = sum(a_avg0_x_u_base_.^1.*a_x_u_mask_.^1,'all');
disp(sprintf(' %% sum_A1M1: %0.16f --> %0.16f',sum_A1M1,tmp_sum_A1M1));

%%%%%%%%;
% simple visualization of a_avg0_x_u_base_. ;
%%%%%%%%
flag_plot=1;
if flag_plot;
fname_fig = sprintf('%s_jpg/a_avg0_x_u_base_FIGB_',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
...
isosurface_f_x_c_0(a_avg0_x_u_base_,tmp_p);
...
%%%%%%%%;
verbose=0;
fname_mat = sprintf('%s_mat/a_avg0_k_p_quad_.mat',dir_pm);
if (flag_recalc | ~exist(fname_mat,'file'));
disp(sprintf(' %% %s not found, creating',fname_mat));
tmp_t = tic;
...
a_avg0_k_p_quad_ = xxnufft3d3(n_xxx_u,x_u_0___(:)*eta,x_u_1___(:)*eta,x_u_2___(:)*eta,a_avg0_x_u_base_(:).*xxx_u_weight_(:),-1,1e-12,n_k_all,2*pi*k_c_0_all_/eta,2*pi*k_c_1_all_/eta,2*pi*k_c_2_all_/eta)/sqrt(2*pi)/sqrt(2*pi)/sqrt(2*pi);
tmp_t = toc(tmp_t); disp(sprintf(' %% xxnufft3d3: a_avg0_k_p_quad_ time %0.2fs',tmp_t));
%%%%%%%%;
eta = pi/k_p_r_max; tmp_t = tic;
a_avg0_x_u_reco_ = xxnufft3d3(n_k_all,2*pi*k_c_0_all_*eta,2*pi*k_c_1_all_*eta,2*pi*k_c_2_all_*eta,a_avg0_k_p_quad_.*(2*pi)^3.*weight_3d_k_all_,+1,1e-12,n_xxx_u,x_u_0___(:)/eta,x_u_1___(:)/eta,x_u_2___(:)/eta)/sqrt(2*pi)/sqrt(2*pi)/sqrt(2*pi);
tmp_t = toc(tmp_t); disp(sprintf(' %% xxnufft3d3: a_avg0_x_u_reco_ time %0.2fs',tmp_t));
disp(sprintf(' %% xxnufft3d3: a_avg0_x_u_reco error: %0.16f',fnorm(a_avg0_x_u_base_(:)-a_avg0_x_u_reco_)/fnorm(a_avg0_x_u_base_(:))));
disp(sprintf(' %% at this point one should ensure that a_avg0_k_p_quad_ on the outer shells (i.e., near k_p_r_max) has decayed to the desired precision.'));
save(fname_mat ...
     ,'k_p_r_max','k_eq_d' ...
...
%%%%%%%%;
fname_fig = sprintf('%s_jpg/a_avg0_k_p_quad_',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1+nf);nf=nf+1;
clf;
plot(k_p_r_all_,log10(abs(a_avg0_k_p_quad_)),'.'); xlabel('k'); ylabel('log10(|a(k)|)');
figbig;
disp(sprintf(' %% writing %s',fname_fig));
print('-djpeg',sprintf('%s.jpg',fname_fig));
print('-depsc',sprintf('%s.eps',fname_fig));
...
%%%%%%%%;
verbose=0;
fname_mat = sprintf('%s_mat/a_avg0_k_Y_quad_.mat',dir_pm);
if (flag_recalc | ~exist(fname_mat,'file'));
disp(sprintf(' %% %s not found, creating',fname_mat));
...
%%%%%%%%;
tmp_t = tic;
[a_avg0_k_Y_quad_] = convert_k_p_to_spharm_1(verbose,n_k_all,n_k_all_csum_,k_p_r_all_,k_p_azimu_b_all_,k_p_polar_a_all_,weight_3d_k_all_,weight_shell_k_,n_k_p_r,k_p_r_,weight_3d_k_p_r_,l_max_,a_avg0_k_p_quad_);
tmp_t = toc(tmp_t); disp(sprintf(' %% a_avg0_k_Y_quad_ time %0.2fs',tmp_t));
...
tmp_t = tic;
[a_avg0_k_p_reco_] = convert_spharm_to_k_p_1(verbose,n_k_all,n_k_all_csum_,k_p_r_all_,k_p_azimu_b_all_,k_p_polar_a_all_,weight_3d_k_all_,weight_shell_k_,n_k_p_r,k_p_r_,weight_3d_k_p_r_,l_max_,a_avg0_k_Y_quad_);
tmp_t = toc(tmp_t); disp(sprintf(' %% a_avg0_k_Y_quad_ --> a_avg0_k_p_reco_ time %0.2fs',tmp_t));
disp(sprintf(' %% xxnufft3d3: a_avg0_k_p_reco error: %0.16f',fnorm(a_avg0_k_p_quad_-a_avg0_k_p_reco_)/fnorm(a_avg0_k_p_quad_))); %<-- this should be 2-3 digits. ;
%%%%%%%%;
a_avg0_k_Y_quad__ = zeros(n_lm_max,n_k_p_r);
for nk_p_r=0:n_k_p_r-1;
tmp_index_ = n_lm_csum_(1+nk_p_r) + (0:n_lm_(1+nk_p_r)-1);
a_avg0_k_Y_quad__(1:n_lm_(1+nk_p_r),1+nk_p_r) = a_avg0_k_Y_quad_(1+tmp_index_);
end;%for nk_p_r=0:n_k_p_r-1;
save(fname_mat ...
     ,'l_max_' ...
...
%%%%%%%%;
fname_fig = sprintf('%s_jpg/a_avg0_k_Y_quad_A',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1+nf);nf=nf+1;
clf;
subplot(1,3,1); plot(Y_l_val_,log10(abs(a_avg0_k_Y_quad_)),'.'); xlabel('Y_l_val_','Interpreter','none'); ylabel('log10(abs(a))');
subplot(1,3,2); plot(Y_m_val_,log10(abs(a_avg0_k_Y_quad_)),'.'); xlabel('Y_m_val_','Interpreter','none'); ylabel('log10(abs(a))');
subplot(1,3,3); plot(Y_k_val_,log10(abs(a_avg0_k_Y_quad_)),'.'); xlabel('Y_k_val_','Interpreter','none'); ylabel('log10(abs(a))');
figbig;
disp(sprintf(' %% writing %s',fname_fig));
print('-djpeg',sprintf('%s.jpg',fname_fig));
print('-depsc',sprintf('%s.eps',fname_fig));
...
%%%%%%%%;
fname_fig = sprintf('%s_jpg/a_avg0_k_Y_quad_',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1+nf);nf=nf+1;
clf;
imagesc_Y(k_p_r_max,n_k_p_r,k_p_r_,l_max_,log10(abs(a_avg0_k_Y_quad_)),[-10,0],colormap_beach());
xlabel('l_val','Interpreter','none');
ylabel('m_val','Interpreter','none');
zlabel('k_p_r','Interpreter','none');
axis vis3d; view([-65,+20]);
title('a_avg0_k_Y_quad_','Interpreter','none');
%figbig;
disp(sprintf(' %% writing %s',fname_fig));
print('-djpeg',sprintf('%s.jpg',fname_fig));
print('-depsc',sprintf('%s.eps',fname_fig));
...
%%%%%%%%;
% generate templates S_avg0_k_p_ on k_p_ grid with uniform n_w_. ;
%%%%%%%%;
fname_mat = sprintf('%s_mat/S_avg0_k_p_wkS__.mat',dir_pm);
if (flag_recalc | ~exist(fname_mat,'file'));
disp(sprintf(' %% %s not found, creating',fname_mat));
...
%%%%;
tmp_t = tic();
S_avg0_x_u_xxS___ = zeros(n_x_u_pack,n_x_u_pack,n_S);
for nS=0:n_S-1;
S_avg0_k_p_wk_ = S_avg0_k_p_wkS__(:,1+nS);
S_avg0_x_u_xx_ = real( interp_k_p_to_x_c_xxnufft(n_x_u_pack,diameter_x_c,n_x_u_pack,diameter_x_c,n_k_p_r,k_p_r_,n_w_,S_avg0_k_p_wk_.*weight_2d_wk_*(2*pi)^2)*sqrt(n_x_u_pack*n_x_u_pack) * n_w_sum );
S_avg0_x_u_xx__ = reshape(S_avg0_x_u_xx_,[n_x_u_pack,n_x_u_pack]);
S_avg0_x_u_xxS___(:,:,1+nS) = S_avg0_x_u_xx__;
end;%for nS=0:n_S-1;
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% S_avg0_x_u_xxS___: %0.2fs',tmp_t)); end;
%%%%;
S_mask_threshold = tolerance_master;
M_x_u_xx__ = zeros(n_x_u_pack,n_x_u_pack); %<-- single circular template mask. ;
M_k_p_wk__ = zeros(n_w_sum,1);
[x_u_0__,x_u_1__] = ndgrid(x_u_0_,x_u_1_); n_xx_u = n_x_u_pack^2;
x_u_r__ = sqrt(x_u_0__.^2 + x_u_1__.^2);
M_x_u_xx__ = cast(x_u_r__<=x_p_r_mask,'double');
...
%%%%%%%%;
fname_fig = sprintf('%s_jpg/S_avg0_x_u_xxS_vs_M_x_u_xxS_',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
...
imagesc(S_avg0_x_u_xxS___(:,:,1+nS)); axis image; axisnotick; title(sprintf('S %.3d',nS));
subplot(p_row,p_col,1+np);np=np+1;
imagesc(M_x_u_xx__(:,:)); axis image; axisnotick; title(sprintf('M %.3d',nS));
end;%for nl=0:n_l-1;
...
%%%%%%%%;
fname_fig = sprintf('%s_jpg/S_avg0_k_p_wkS__',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1+nf);nf=nf+1;clf;figbig;
Slim_ = std(abs(S_avg0_k_p_wkS__(:)))*2.5*[-1,+1];
for nl=0:15-1;
subplot(3,5,1+nl); nS = max(0,min(n_S-1,floor(n_S*nl/(15-1))));
imagesc_p(n_k_p_r,k_p_r_,n_w_,n_w_sum,real(S_avg0_k_p_wkS__(:,1+nS)),Slim_,colormap_80s);
axis equal; axisnotick; title(sprintf('nS %d',nS));
end;%for nl=0:15-1;
sgtitle(sprintf('Sample S_avg0_k_p_wkS__'),'Interpreter','none');
disp(sprintf(' %% writing %s',fname_fig));
print('-djpeg',sprintf('%s.jpg',fname_fig));
print('-depsc',sprintf('%s.eps',fname_fig));
close(gcf);
...
%%%%%%%%;
% Now convert templates to S_avg0_k_q__. ;
%%%%%%%%;
S_avg0_k_q__ = zeros(n_w_sum,n_S);
for nS=0:n_S-1;
S_avg0_k_q__(:,1+nS) = interp_p_to_q(n_k_p_r,n_w_,n_w_sum,S_avg0_k_p_wkS__(:,1+nS));
end;%for nS=0:n_S-1;
%%%%%%%%;
fname_fig = sprintf('%s_jpg/S_avg0_k_q__',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1+nf);nf=nf+1;clf;figbig;
Slim_ = std(abs(S_avg0_k_p_wkS__(:)))*2.5*[-1,+1];
for nl=0:15-1;
subplot(3,5,1+nl); nS = max(0,min(n_S-1,floor(n_S*nl/(15-1))));
%imagesc_q(n_k_p_r,k_p_r_,n_w_,n_w_sum,abs(S_avg0_k_q__(:,1+nS)),Slim_,colormap_80s);
imagesc_q(n_k_p_r,k_p_r_,n_w_,n_w_sum,log10(abs(S_avg0_k_q__(:,1+nS))/max(abs(S_avg0_k_q__(:)))),[-4,0],colormap_80s);
title(sprintf('nS %d',nS));
end;%for nl=0:15-1;
sgtitle(sprintf('S_avg0_k_p_wkS__'),'Interpreter','none');
disp(sprintf(' %% writing %s',fname_fig));
print('-djpeg',sprintf('%s.jpg',fname_fig));
print('-depsc',sprintf('%s.eps',fname_fig));
close(gcf);
...
,l_max_max ...
,n_k_p_r ...
,reshape(a_avg0_k_Y_quad__,[n_lm_max,n_k_p_r]) ...
,viewing_k_eq_d/k_p_r_max ...
,template_k_eq_d ...
,n_w_max ...
);
tmp_t = toc(tmp_t); disp(sprintf(' %% pm_template_2: %0.2fs',tmp_t));
tmp_S_avg0_k_p_wkS__ = reshape(tmp_S_avg0_k_p_wkS__,[n_w_max*n_k_p_r,tmp_n_S]);
%%%%%%%%;
disp(sprintf(' %% S_avg0_k_p_wkS__ vs tmp_S_avg0_k_p_wkS__: %0.16f',fnorm(S_avg0_k_p_wkS__-tmp_S_avg0_k_p_wkS__)/fnorm(S_avg0_k_p_wkS__)));
end;%if flag_check;
...
tmp_delta_sigma = 0;
[X_2d_xavg_d0_kk__,X_2d_xavg_d0_weight_r_] = principled_marching_cost_matrix_6(n_k_p_r,k_p_r_,weight_2d_k_p_r_,l_max_,[],[],a_avg0_k_Y_quad_,tmp_CTF_k_p_r_xavg_kk__,tmp_delta_sigma);
...
S_avg0_k_p_wk_ = S_avg0_k_p_wkS__(:,1+nS);
S_avg0_x_u_xx__ = real( interp_k_p_to_x_c_xxnufft(n_x_u_pack,diameter_x_c,n_x_u_pack,diameter_x_c,n_k_p_r,k_p_r_,n_w_,S_avg0_k_p_wk_.*weight_2d_wk_*(2*pi)^2)*sqrt(n_x_u_pack*n_x_u_pack) * n_w_sum );
M1_sum = sum(M_x_u_xx__.^1,'all');
R0M1_S_(1+nS) = M1_sum;
S1_sum = sum(S_avg0_x_u_xx__.^1.*M_x_u_xx__,'all');
S2_sum = sum(S_avg0_x_u_xx__.^2.*M_x_u_xx__,'all');
R0S1_S_(1+nS) = S1_sum;
R0S2_S_(1+nS) = S2_sum;
%%%%;
...
clear S_avg0_k_p_wk_ S_avg0_x_u_xx__ R1S_x_u_xxz___ R1M_x_u_xxz___ R2M_x_u_xxz___ ;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
end;%for nS=0:n_S-1;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
...
S_avg0_x_u_xx__ = S_avg0_x_u_xxS___(:,:,1+nS);
n_S_x_0 = n_x_u_pack; n_S_x_1 = n_x_u_pack; dx_0 = dx_pack; dx_1 = dx_pack;
S_avg0_k_p_wk_ = interp_x_c_to_k_p_xxnufft(n_S_x_0,diameter_x_c,n_S_x_1,diameter_x_c,S_avg0_x_u_xx__,n_k_ref_p_r,k_ref_p_r_,n_w_ref_)*sqrt(n_S_x_0*n_S_x_1)*dx_0*dx_1;
S_avg0_k_p_wk_ = rotate_p_to_p_fftw(n_k_ref_p_r,n_w_ref_,n_w_ref_sum,S_avg0_k_p_wk_,+gamma_z);
S_avg0_x_u_xx__ = real( interp_k_p_to_x_c_xxnufft(n_x_u_pack,diameter_x_c,n_x_u_pack,diameter_x_c,n_k_ref_p_r,k_ref_p_r_,n_w_ref_,S_avg0_k_p_wk_.*weight_2d_ref_wk_*(2*pi)^2)*sqrt(n_x_u_pack*n_x_u_pack) * n_w_ref_sum );
%%%%;
figure(1+nf);nf=nf+1;clf;figbig; figmed; np=0;
subplot(1,2,1+np);np=np+1;
imagesc_c(n_x_u_pack,x_u_0_,n_x_u_pack,x_u_1_,S_avg0_x_u_xx__,[],colormap_beach);
axis image; axis square; axisnotick; title('S_avg0_x_u_xx__','Interpreter','none');
subplot(1,2,1+np);np=np+1;
...
R1S1_B_xx__ = conv2(R_sub_x_u_pack_.^1,S_avg0_x_u_xx__,'same');
%%;
...
%%;
nx0 = 7; nx1 = 3;
R1S1_D = sum(R_sub_x_u_pack_(1+nx0+[0:n_x_u_pack-1],1+nx1+[0:n_x_u_pack-1]).*fliplr(flipud(S_avg0_x_u_xx__)),'all');
...
clear S_avg0_x_u_xx__ S_avg0_k_p_wk_ ;
%%%%%%%%;
end;%if flag_check;
%%%%%%%%;
...
%%%%;
tmp_costh_ = reshape(max(X_RS_xxzS____,[],[3,4]),[n_R,1]);
tmp_theta_ = acos(tmp_costh_);
tmp_RuMl2_ = zeros(n_R,1);
nR=0;
for nx0=0:n_R_0-1; for nx1=0:n_R_1-1;
[tmp_X,tmp_ij] = max(X_RS_xxzS____(1+nx0,1+nx1,:,:),[],'all','linear'); tmp_index = tmp_ij-1;
tmp_nz = mod(tmp_index,n_gamma_z);
tmp_nS = floor(tmp_index/n_gamma_z);
tmp_RuMl2_(1+nR) = R2M1_xxzS____(1+nx0,1+nx1,1+tmp_nz,1+tmp_nS) - 2*backg_sum_xx__(1+nx0,1+nx1)*R1M1_xxzS____(1+nx0,1+nx1,1+tmp_nz,1+tmp_nS) + backg_sum_xx__(1+nx0,1+nx1).^2*R0M1_S_(1+tmp_nS);
nR=nR+1;
end;end;%for nx0=0:n_R_0-1; for nx1=0:n_R_1-1;
tmp_nlltr_ = reshape(nll_rel_sum_xx__,[n_R,1])./max(1e-12,tmp_RuMl2_);
gamma_z_ = linspace(0,2*pi,n_gamma_z+1); gamma_z_ = transpose(gamma_z_(1:n_gamma_z));
%%%%;
...
%%%%;
% Note also that, if backg and sigma are fixed, ;
% then the nll_rel_sum is given by: ;
% nll_rel_sum * 2*sigma^2 = |(Q-backg*M) - inten*S|^{2} - |Q-backg*M|^{2}. ;
% assuming that the residual ((Q-backg*M) - inten*S) is perpendicular to (Q-backg*M), ;
% we have that: ;
% nll_rel_sum * 2*sigma^2 = l*sin(theta)-l = l*(sin(theta)-1), ;
% where l = |(Q-backg*M)|, ;
% and cos(theta) = correlation(Q-backg*M,S). ;
% Note that, when theta=pi/2 both cos(theta) and l*(sin(theta)-1) are zero. ;
% However, when theta is slightly smaller than pi/2 ;
% we expect a quadratic relationship between the two. ;
%%%%%%%%;

flag_check=0;
if flag_check;
%%%%%%%%;
% quick snapshot of R0S2 vs R1S1. ;
%%%%%%%%;
%for nx0=0:12:n_R_0-1;for nx1=0:12:n_R_1-1; clf;figsml;imagesc(log2(1+hist2d_0(reshape(repmat(R0S2_S_,[1,1,n_gamma_z,1]),[n_gamma_z*n_S,1]),reshape(R1S1_xxzS____(1+nx0,1+nx1,:,:),[n_gamma_z*n_S,1]),128,128)));fig80s; drawnow(); end;end;
%%%%%%%%;
% check to see if R0S2 and R1S1 can be accumulated in a histogram. ;
%%%%%%%%;
R2M1_lim_ = [ prctile(R2M1_xxzS____, 5,'all') , prctile(R2M1_xxzS____,95,'all') ];
R1S1_lim_ = [ prctile(R1S1_xxzS____, 5,'all') , prctile(R1S1_xxzS____,95,'all') ];
R0S2_lim_ = [ prctile(R0S2_S_, 5,'all') , prctile(R0S2_S_,95,'all') ];
%%%%;
%%%%%%%%;
%%%%;
R2M1_lim_use_ = mean(R2M1_lim_) + 0.5*1.75*diff(R2M1_lim_)*[-1,+1];
R1S1_lim_use_ = mean(R1S1_lim_) + 0.5*1.75*diff(R1S1_lim_)*[-1,+1];
R0S2_lim_use_ = mean(R0S2_lim_) + 0.5*1.75*diff(R0S2_lim_)*[-1,+1];
tmp_dx = 4;
n_h = 128;
na=0;
tmp_h2__ = zeros(n_h,n_h);
for nx0=0:tmp_dx:n_R_0-1; for nx1=0:tmp_dx:n_R_1-1;
tmp_h2__ = tmp_h2__ + ...
  hist2d_0( ...
	    reshape(repmat(R0S2_S_,[1,1,n_gamma_z,1]),[n_gamma_z*n_S,1]) ...
	   ,reshape(R1S1_xxzS____(1+nx0,1+nx1,:,:),[n_gamma_z*n_S,1]) ...
	   ,n_h,n_h,R0S2_lim_use_,R1S1_lim_use_ ...
	    );
clf; figsml; fig80s;
%imagesc(log2(1+tmp_h2__(2:end-1,2:end-1))); %<-- ignore histogram overflow. ;
imagesc(   (1+tmp_h2__(2:end-1,2:end-1))); %<-- ignore histogram overflow. ;
axis image; title(sprintf('na %d',na));
na=na+1;
drawnow();
end;end;%for nx0=0:tmp_dx:n_R_0-1; for nx1=0:tmp_dx:n_R_1-1;
%%%%%%%%;
end;%if flag_check;
%%%%%%%%;
...
%%%%%%%%;
fname_mat = sprintf('%s_mat/test_pm_clathrin_avg0_R_ibp_nll_rel_sum_xx__.mat',dir_pm);
if (flag_recalc | ~exist(fname_mat,'file'));
disp(sprintf(' %% %s not found, creating',fname_mat));
%%%%%%%%;
...
fname_fig = sprintf('%s_jpg/test_pm_clathrin_avg0_R_ibp_rel_sum_xx_FIGA',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1+nf);nf=nf+1;clf;figbig;
...
fname_fig = sprintf('%s_jpg/test_pm_clathrin_avg0_R_ibp_rel_sum_xx_FIGB',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1+nf);nf=nf+1;clf;figbig;
p_row = 3;p_col = 4; np=0;
...
fname_mat = sprintf('%s_mat/test_pm_clathrin_avg0_CoC_xx__.mat',dir_pm);
if (flag_recalc | ~exist(fname_mat,'file'));
disp(sprintf(' %% %s not found, creating',fname_mat));
%%%%%%%%;
...
tmp_index_nS_max = floor(tmp_index_max/max(1,n_gamma_z));
%%%%;
tmp_M_x_u_xx__ = M_x_u_xx__;
tmp_S_avg0_x_u_xx__ = S_avg0_x_u_xxS___(:,:,1+tmp_index_nS_max);
tmp_S_avg0_k_p_wk_ = S_avg0_k_p_wkS__(:,1+tmp_index_nS_max);
tmp_CTF_S_avg0_k_p_wk_ = zeros(n_w_sum,1);
na=0;
for nk_p_r=0:n_k_p_r-1;
n_w = n_w_(1+nk_p_r);
CTF_k_p_r = CTF_k_p_r_k_(1+nk_p_r);
for nw=0:n_w-1;
tmp_CTF_S_avg0_k_p_wk_(1+na) = tmp_S_avg0_k_p_wk_(1+na)*CTF_k_p_r;
na=na+1;
end;%for nw=0:n_w-1;
end;%for nk_p_r=0:n_k_p_r-1;
tmp_gamma_z = (2*pi*tmp_index_nz_max)/n_gamma_z;
tmp_CTF_S_avg0_k_p_wk_ = rotate_p_to_p_fftw(n_k_p_r,n_w_,n_w_sum,tmp_CTF_S_avg0_k_p_wk_,+tmp_gamma_z);
tmp_CTF_S_avg0_x_c_xx_ = interp_k_p_to_x_c_xxnufft(n_x_u_pack,diameter_x_c,n_x_u_pack,diameter_x_c,n_k_p_r,k_p_r_,n_w_,tmp_CTF_S_avg0_k_p_wk_.*weight_2d_wk_*(2*pi)^2)*sqrt(n_x_u_pack*n_x_u_pack) * n_w_sum;
tmp_CTF_S_avg0_x_c_xx__ = real(reshape(tmp_CTF_S_avg0_x_c_xx_,[n_x_u_pack,n_x_u_pack]));
parameter_innerproduct = struct('type','parameter','nw_stride',2);
tmp_t = tic();
[ ...
 ~ ...
...
,n_x_u_pack ...
,n_x_u_pack ...
,fliplr(flipud(tmp_CTF_S_avg0_x_c_xx__)) ...
,n_x_u_pack ...
,n_x_u_pack ...
,tmp_CTF_S_avg0_x_c_xx__ ...
,n_k_ref_p_r ...
,k_ref_p_r_ ...
,k_ref_p_r_max ...
,n_w_ref_ ...
...
clear tmp_M_x_u_xx__ tmp_S_avg0_x_u_xx__ tmp_S_avg0_k_p_wk_ tmp_CTF_S_avg0_k_p_wk_ tmp_gamma_z tmp_CTF_S_avg0_k_p_wk_ tmp_CTF_S_avg0_x_c_xx_ ;
...
fname_mat = sprintf('%s_mat/test_pm_clathrin_avg0_Hessian_xx__.mat',dir_pm);
if (flag_recalc | ~exist(fname_mat,'file'));
disp(sprintf(' %% %s not found, creating',fname_mat));
%%%%%%%%;
...
fname_fig = sprintf('%s_jpg/test_pm_clathrin_avg0_CoC_xx_FIGC',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1+nf);nf=nf+1;clf;figbig;
p_row = 3;p_col = 4; np=0;
...
verbose=1;
if (verbose); disp(sprintf(' %% [finished test_pm_clathrin_8]')); end;
