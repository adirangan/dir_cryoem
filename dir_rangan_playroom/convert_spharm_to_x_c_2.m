function [a_x_all_] = convert_spharm_to_x_c_2(verbose,n_k_p_r,k_p_r_,weight_k_p_r_,l_val_,a_k_Y_,n_x_all,x_c_0_all_,x_c_1_all_,x_c_2_all_);
...
% n_x_all_ = integer number of points in x_c. ;
% x_c_0_all_ = real array of length n_x_all ; x_c_0 value to evaluate. ;
% x_c_1_all_ = real array of length n_x_all ; x_c_1 value to evaluate. ;
% x_c_2_all_ = real array of length n_x_all ; x_c_2 value to evaluate. ;
% ;
% outputs: ;
% ;
...
assert(n_x_all<=numel(x_c_0_all_));
assert(n_x_all<=numel(x_c_1_all_));
assert(n_x_all<=numel(x_c_2_all_));
a_x_all_ = zeros(n_x_all,1);
x_p_r_all_ = sqrt(x_c_0_all_.^2 + x_c_1_all_.^2 + x_c_2_all_.^2);
x_c_r_all_ = sqrt(x_c_0_all_.^2 + x_c_1_all_.^2);
x_p_azimu_b_ = atan2(x_c_1_all_,x_c_0_all_);
x_p_polar_a_ = atan2(x_c_r_all_,x_c_2_all_);
Ylm__ = get_Ylm__(l_val_max+1,0:l_val_max,n_x_all,x_p_azimu_b_(:),x_p_polar_a_(:));
ix=0;
...
assert(ix==sum(n_lm_));
a_x_all_ = (4*pi) * reshape(a_x_all_,size(x_c_0_all_));
