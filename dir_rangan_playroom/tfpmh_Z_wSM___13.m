...
[ ...
 parameter ...
,Z_wSM___ ... %<-- or Z_SM__. ;
,UX_CTF_R_S_l2_wSM___ ... %<-- or UX_CTF_R_S_l2_SM__. ;
,UX_T_M_l2_dM__ ...
,UX_M_l2_M_ ...
,X_wSM___ ... %<-- or X_SM__. ;
,delta_x_wSM___ ... %<-- or delta_x_SM__. ;
,delta_y_wSM___ ... %<-- or delta_y_SM__. ;
,gamma_z_wSM___ ... %<-- or gamma_z_SM__. ;
,index_sub_wSM___ ... %<-- or index_sub_SM__. ;
,Z_dwSM____ ...
,X_dwSM____ ...
] = ...
tfpmh_Z_wSM___13( ...
...
% Note that this takes in n_M (anisotropic) CTF_k_p_wkM__, ;
% assumed to individually correspond to the n_M images. ;
% Also does not return CTF_R_S_l2_wSM___. ;
...
str_thisfunction = 'tfpmh_Z_wSM___13';
...
%%%%%%%%;
if flag_precompute_UX_T_M_l2_dM__==0; UX_T_M_l2_dM__ = zeros(n_delta_v,n_M); end;
if flag_precompute_UX_M_l2_M_==0; UX_M_l2_M_ = zeros(n_M,1); end;
...
UX_CTF_R_S_l2_wSM___ = zeros(n_S,n_M);
...
if (flag_verbose>1); 
tmp_str = 'UX_CTF_R_S_l2_wSM___'; disp(sprintf(' %% memory: %s --> %0.6f GB',tmp_str,whos(tmp_str).bytes/1e9));
...
if isempty(UX_CC_k_q_wnM__); UX_CC_k_q_wnM__=zeros(pm_n_w_sum,n_M); end;
if isempty(svd_V_UX_CTF_M_lwnM____); svd_V_UX_CTF_M_lwnM____=zeros(n_svd_l,n_w_max,pm_n_UX_rank,n_M); end;
if isempty(svd_V_UX_M_lwnM____); svd_V_UX_M_lwnM____=zeros(n_svd_l,n_w_max,pm_n_UX_rank,n_M); end;
if isempty(UX_S_k_q_wnS__); UX_S_k_q_wnS__=zeros(pm_n_w_sum,n_S); end;
if isempty(UX_SS_k_q_wnS__); UX_SS_k_q_wnS__=zeros(pm_n_w_sum,n_S); end;
...
%%%%;
if flag_precompute_CTF_M_k_q_wkM__==0;
tmp_t = tic();
...
end;%if flag_precompute_CTF_M_k_q_wkM__==0;

%%%%;
% Prepare UX_CC_k_q_wnM__. ;
%%%%;
if flag_precompute_UX_CC_k_q_wnM__==0;
tmp_t = tic();
...
end;%if flag_precompute_UX_CC_k_q_wnM__==0;

%%%%;
% Prepare UX_S_k_q_wnS__. ;
%%%%;
if flag_precompute_UX_S_k_q_wnS__==0;
tmp_t = tic();
...
end;%if flag_precompute_UX_S_k_q_wnS__==0;

%%%%;
% Prepare UX_SS_k_q_wnS__. ;
%%%%;
if flag_precompute_UX_SS_k_q_wnS__==0;
tmp_t = tic();
...
end;%if flag_precompute_UX_SS_k_q_wnS__==0;
...
M_sub_k_q_wkM__ = M_k_q_wkM__(:,1+index_nM_in_Mbatch_);
tmp_t = toc(tmp_t); if (flag_verbose>1); disp(sprintf(' %% M_sub_k_q_wkM__: %0.6fs',tmp_t)); end;
...
%tmp_t = tic();
%CTF_sub_k_p_wkM__ = CTF_k_p_wkM__(:,1+index_nM_in_Mbatch_);
%CTF_sub_k_q_wkM__ = CTF_k_q_wkM__(:,1+index_nM_in_Mbatch_);
%tmp_t = toc(tmp_t); if (flag_verbose>1); disp(sprintf(' %% CTF_sub_k_q_wkM__: %0.6fs',tmp_t)); end;
%parameter = parameter_timing_update(parameter,sprintf('%s: CTF_sub_k_q_wkM__',str_thisfunction),tmp_t);
tmp_t = tic();
%CTF_M_sub_k_p_wkM__ = CTF_M_k_p_wkM__(:,1+index_nM_in_Mbatch_);
CTF_M_sub_k_q_wkM__ = CTF_M_k_q_wkM__(:,1+index_nM_in_Mbatch_);
tmp_t = toc(tmp_t); if (flag_verbose>1); disp(sprintf(' %% CTF_M_sub_k_q_wkM__: %0.6fs',tmp_t)); end;
...
%UX_CC_sub_k_p_wnM__ = UX_CC_k_p_wnM__(:,1+index_nM_in_Mbatch_);
UX_CC_sub_k_q_wnM__ = UX_CC_k_q_wnM__(:,1+index_nM_in_Mbatch_);
tmp_t = toc(tmp_t); if (flag_verbose>1); disp(sprintf(' %% UX_CC_sub_k_q_wnM__: %0.6fs',tmp_t)); end;
parameter = parameter_timing_update(parameter,sprintf('%s: UX_CC_sub_k_q_wnM__',str_thisfunction),tmp_t);
%%%%;
% Prepare quasi-images. ;
%%%%;
if flag_precompute_svd_V_UX_CTF_M_lwnM____==0;
tmp_t = tic();
...
tmp_t = toc(tmp_t); if (flag_verbose>1); disp(sprintf(' %% svd_V_UX_CTF_M_sub_lwnM____: %0.6fs',tmp_t)); end;
parameter = parameter_timing_update(parameter,sprintf('%s: svd_V_UX_CTF_M_sub_lwnM____',str_thisfunction),tmp_t);
end;%if flag_precompute_svd_V_UX_CTF_M_lwnM____==0;
...
tmp_t = toc(tmp_t); if (flag_verbose>1); disp(sprintf(' %% svd_V_UX_M_sub_lwnM____: %0.6fs',tmp_t)); end;
...
tmp_t = tic();
svd_V_UX_CTF_M_sub_lwnM____ = svd_V_UX_CTF_M_lwnM____(:,:,1:pm_n_UX_rank,1+index_nM_in_Mbatch_);
tmp_t = toc(tmp_t); if (flag_verbose>1); disp(sprintf(' %% loading svd_V_UX_CTF_M_sub_lwnM____: %0.6fs',tmp_t)); end;
parameter = parameter_timing_update(parameter,sprintf('%s: loading svd_V_UX_CTF_M_sub_lwnM____',str_thisfunction),tmp_t);
end;%if flag_precompute_svd_V_UX_CTF_M_lwnM____==1;
if flag_precompute_svd_V_UX_M_lwnM____==1;
tmp_t = tic();
svd_V_UX_M_sub_lwnM____ = svd_V_UX_M_lwnM____(:,:,1:pm_n_UX_rank,1+index_nM_in_Mbatch_);
tmp_t = toc(tmp_t); if (flag_verbose>1); disp(sprintf(' %% loading svd_V_UX_M_sub_lwnM____: %0.6fs',tmp_t)); end;
parameter = parameter_timing_update(parameter,sprintf('%s: loading svd_V_UX_M_sub_lwnM____',str_thisfunction),tmp_t);
end;%if flag_precompute_svd_V_UX_M_lwnM____==1;
tmp_t = tic();
svd_V_UX_CTF_M_sub_nMwl____ = permute(svd_V_UX_CTF_M_sub_lwnM____,1+[2,3,1,0]);
tmp_t = toc(tmp_t); if (flag_verbose>1); disp(sprintf(' %% svd_V_UX_CTF_M_sub_nMwl____: %0.6fs',tmp_t)); end;
...
tmp_t = toc(tmp_t); if (flag_verbose>1); disp(sprintf(' %% tfpmh_UX_T_M_sub_l2_dm__1: %0.6fs',tmp_t)); end;
parameter = parameter_timing_update(parameter,sprintf('%s: tfpmh_UX_T_M_sub_l2_dm__1',str_thisfunction),tmp_t);
UX_T_M_l2_dM__(:,1+index_nM_in_Mbatch_) = UX_T_M_sub_l2_dM__; %<-- store results. ;
end;%if flag_precompute_UX_T_M_l2_dM__==0;
if flag_precompute_UX_T_M_l2_dM__==1;
UX_T_M_sub_l2_dM__ = UX_T_M_l2_dM__(:,1+index_nM_in_Mbatch_);
...
UX_M_l2_M_(1+index_nM_in_Mbatch_) = UX_M_sub_l2_M_; %<-- store results. ;
end;%if flag_precompute_UX_M_l2_M_==0;
if flag_precompute_UX_M_l2_M_==1;
UX_M_sub_l2_M_ = UX_M_l2_M_(1+index_nM_in_Mbatch_);
...
tmp_t = tic();
%UX_SS_sub_k_p_wnS__ = UX_SS_k_p_wnS__(:,1+index_nS_in_Sbatch_);
UX_SS_sub_k_q_wnS__ = UX_SS_k_q_wnS__(:,1+index_nS_in_Sbatch_);
UX_CTF_sub_R_S_sub_l2_SM__ = squeeze(sum(bsxfun(@times,reshape(UX_CC_sub_k_q_wnM__,[pm_n_w_sum,1,n_M_sub]),reshape(conj(UX_SS_sub_k_q_wnS__),[pm_n_w_sum,n_S_sub,1])),1+0)) / max(1,pm_n_w_max) ;
UX_CTF_sub_R_S_sub_l2_SM__ = real(UX_CTF_sub_R_S_sub_l2_SM__);
tmp_t = toc(tmp_t); if (flag_verbose>1); disp(sprintf(' %% UX_CTF_sub_R_S_sub_l2_SM__: %0.6fs',tmp_t)); end;
parameter = parameter_timing_update(parameter,sprintf('%s: UX_CTF_sub_R_S_sub_l2_SM__',str_thisfunction),tmp_t);
end;%if flag_CTF_anisotropic==0;
if flag_CTF_anisotropic==1;
tmp_t = tic();
UX_CTF_sub_R_S_sub_l2_wSM___ = zeros(n_w_max,n_S_sub,n_M_sub);
UX_SS_sub_k_q_wnS__ = UX_SS_k_q_wnS__(:,1+index_nS_in_Sbatch_);
UX_CTF_sub_R_S_sub_l2_wSM___ = ifft(squeeze(sum(reshape(bsxfun(@times,reshape(UX_CC_sub_k_q_wnM__,[pm_n_w_sum,1,n_M_sub]),reshape(conj(UX_SS_sub_k_q_wnS__),[pm_n_w_sum,n_S_sub,1])),[pm_n_w_max,pm_n_k_p_r,n_S_sub,n_M_sub]),1+1)));
UX_CTF_sub_R_S_sub_l2_wSM___ = real(UX_CTF_sub_R_S_sub_l2_wSM___);
tmp_t = toc(tmp_t); if (flag_verbose>1); disp(sprintf(' %% UX_CTF_sub_R_S_sub_l2_wSM___: %0.6fs',tmp_t)); end;
parameter = parameter_timing_update(parameter,sprintf('%s: UX_CTF_sub_R_S_sub_l2_wSM___',str_thisfunction),tmp_t);
...
%%%%;
tmp_t = tic();
svd_S_sub_V_UX_CTF_M_sub_SMwl____ = pagemtimes(UX_S_sub_k_q_nSw___,'ctranspose',svd_V_UX_CTF_M_sub_nMwl____,'none');
...
tmp_t = tic();
svd_S_sub_V_UX_CTF_M_sub_lwSM____ = ifft(permute(svd_S_sub_V_UX_CTF_M_sub_SMwl____,1+[3,2,0,1]),[],1+1)*n_w_max; %<-- faster on cpu. ;
%svd_S_sub_V_UX_CTF_M_sub_lwSM____ = permute(ifft(permute(svd_S_sub_V_UX_CTF_M_sub_SMwl____,1+[2,3,0,1]),[],1+0)*n_w_max,1+[1,0,2,3]); %<-- faster on gpu. ;
...
tmp_t = tic();
svd_UES_S_sub_V_UX_CTF_M_sub_dwSM____ = reshape(pagemtimes(FTK.svd_U_d_expiw_s__,svd_S_sub_V_UX_CTF_M_sub_lwSM____),[n_delta_v,n_w_max,n_S_sub,n_M_sub]);
...
tmp_t = tic();
if flag_CTF_anisotropic==1;
%UX_CTF_sub_R_S_sub_l2_wSM___ = UX_CTF_R_S_l2_wSM___(:,1+index_nS_in_Sbatch_,1+index_nM_in_Mbatch_);
X_sub_dwSM____ = bsxfun(@rdivide,bsxfun(@rdivide,Z_sub_dwSM____,max(1e-12,reshape(sqrt(UX_CTF_sub_R_S_sub_l2_wSM___),[1,n_w_max,n_S_sub,n_M_sub]))),max(1e-12,reshape(sqrt(UX_T_M_sub_l2_dM__),[n_delta_v,1,1,n_M_sub])));
end;%if flag_CTF_anisotropic==1;
if flag_CTF_anisotropic==0;
%UX_CTF_sub_R_S_sub_l2_SM__ = UX_CTF_R_S_l2_SM__(1+index_nS_in_Sbatch_,1+index_nM_in_Mbatch_);
X_sub_dwSM____ = bsxfun(@rdivide,bsxfun(@rdivide,Z_sub_dwSM____,max(1e-12,reshape(sqrt(UX_CTF_sub_R_S_sub_l2_SM__),[1,1,n_S_sub,n_M_sub]))),max(1e-12,reshape(sqrt(UX_T_M_sub_l2_dM__),[n_delta_v,1,1,n_M_sub])));
end;%if flag_CTF_anisotropic==0;
...
if flag_CTF_anisotropic==0;
UX_CTF_R_S_l2_SM__(1+index_nS_in_Sbatch_,1+index_nM_in_Mbatch_) = reshape(UX_CTF_sub_R_S_sub_l2_SM__,[n_S_sub,n_M_sub]);
end;%if flag_CTF_anisotropic==0;
if flag_CTF_anisotropic==1;
UX_CTF_R_S_l2_wSM___(:,1+index_nS_in_Sbatch_,1+index_nM_in_Mbatch_) = reshape(UX_CTF_sub_R_S_sub_l2_wSM___,[n_w_max,n_S_sub,n_M_sub]);
end;%if flag_CTF_anisotropic==1;
...
if flag_CTF_anisotropic==1;
tmp_index_all_SM_ = tmp_index_gamma_SM__ + reshape([0:n_SM-1],size(tmp_index_gamma_SM__))*n_w_max;
tmp_UX_CTF_sub_R_S_sub_l2_SM__ = reshape(UX_CTF_sub_R_S_sub_l2_wSM___(1+tmp_index_all_SM_),[n_S_sub,n_M_sub]);
UX_CTF_R_S_l2_SM__(1+index_nS_in_Sbatch_,1+index_nM_in_Mbatch_) = reshape(tmp_UX_CTF_sub_R_S_sub_l2_SM__,[n_S_sub,n_M_sub]);
end;%if flag_CTF_anisotropic==1;
...
UX_CTF_R_S_l2_wSM___ = UX_CTF_R_S_l2_SM__;
end;%if flag_optimize_over_gamma_z==1;
