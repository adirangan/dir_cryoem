...
if ~isfield(parameter,'compass_n_side'); parameter.compass_n_side=64; end;
compass_n_side=parameter.compass_n_side;
...
if ~isfield(parameter,'compass_pointer_power'); parameter.compass_pointer_power=12.0; end;
compass_pointer_power=parameter.compass_pointer_power;
...
compass_base_3s__ = zeros(3,compass_n_side);
compass_pointer_3s__ = zeros(3,compass_n_side);
for compass_nside=0:compass_n_side-1+1;
compass_w = (2*pi*compass_nside)/max(1,compass_n_side);
compass_b_ = [ compass_r_base*cos(compass_w) ; compass_r_base*sin(compass_w) ; sqrt(1-compass_r_base.^2) * 1.0 ] ;
compass_base_3s__(:,1+compass_nside) = compass_b_;
compass_p_base = compass_r_base*( 0.5*(1+cos(compass_w - delta_xx_omega)) ).^compass_pointer_power;
compass_p_ = [ compass_p_base*cos(compass_w) ; compass_p_base*sin(compass_w) ; sqrt(1-compass_p_base.^2) * 1.0 ] ;
compass_pointer_3s__(:,1+compass_nside) = compass_p_;
end;%for compass_nside=0:compass_n_side-1+1;
%%%%;
...
if flag_2d_vs_3d==1;
%%%%;
compass_base_xs_ = reshape(compass_base_3s__(1+0,:),[1+compass_nside,1]);
compass_base_ys_ = reshape(compass_base_3s__(1+1,:),[1+compass_nside,1]);
compass_base_zs_ = reshape(compass_base_3s__(1+2,:),[1+compass_nside,1]);
compass_base_rs_ = reshape(sum(compass_base_3s__.^2,1),[1+compass_nside,1]);
compass_base_ps_ = reshape(sum(compass_base_3s__(1:2,:).^2,1),[1+compass_nside,1]);
...
%%%%;
compass_pointer_xs_ = reshape(compass_pointer_3s__(1+0,:),[1+compass_nside,1]);
compass_pointer_ys_ = reshape(compass_pointer_3s__(1+1,:),[1+compass_nside,1]);
compass_pointer_zs_ = reshape(compass_pointer_3s__(1+2,:),[1+compass_nside,1]);
compass_pointer_rs_ = reshape(sum(compass_pointer_3s__.^2,1),[1+compass_nside,1]);
compass_pointer_ps_ = reshape(sum(compass_pointer_3s__(1:2,:).^2,1),[1+compass_nside,1]);
