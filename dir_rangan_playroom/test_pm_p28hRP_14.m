%%%%%%%%;
% Warning, the different image-pools have different norms. ;
% Specifically, the magnitude of the T1 images is lower than the T2 and T3 images. ;
% auc_0(N_k_p_l2_(1+index_T1_),N_k_p_l2_(1+index_T23_)) = 0.9972. ;
%%%%%%%%;
clear;
...
dir_pm = sprintf('/%s/rangan/dir_cryoem/dir_p28hRP/dir_pm',string_root);
...
dir_relion = sprintf('/%s/rangan/dir_cryoem/dir_p28hRP/dir_relion',string_root);
...
Pixel_Spacing = 0.98; %<-- in angstroms, from https://www.ebi.ac.uk/pdbe/emdb/empiar/entry/10091/ ;
fname_nopath_volume = 'emd_8674.map'; %<-- not actually consensus volume. ;
%%%%%%%%;
...
{ ...
 ...
 'emd_8675.map'... %<-- class T2. ;
 'emd_8676.map'... %<-- class T3. ;
};
n_volume = numel(fname_nopath_volume_);
flag_het = 0; if (n_volume> 1); flag_het = 1; end;
% Note that the images associated with these classes could be mingled: ;
% emd_weight_ = [ 39520 ; 27629 ; 10891 ] ; 
% see: https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5580496/#SD1) ;
% 1024*emd_weight_/sum(emd_weight_) --> [ 519 , 362 , 143 ];
index_nvolume_from_nM_true_ = [ 0*ones(519,1) ; 1*ones(362,1) ; 2*ones(143,1) ] ;
fname_nopath_star = 'T123_1024.star'; %<-- synthetic mixture of T1.star, T2.star and T3.star. ;
...
%%%%%%%%;
% Note that volumes T2 and T3 are similar, while T1 is different. ;
%%%%%%%%;

index_nvolume_from_nM_reco_ = index_nvolume_from_nM_;
...
tmp_fname_pos = sprintf('class_T%d',1+tmp_nvolume);
%%%%%%%%;
parameter = struct('type','parameter');
[ ...
 parameter ...
...
subplot(1,2,1);
scatter3(SV_loading_Ml__(:,1),SV_loading_Ml__(:,2),SV_loading_Ml__(:,3),24,index_nvolume_from_nM_reco_,'filled');
axis vis3d;
title('index_nvolume_from_nM_reco_','Interpreter','none');
subplot(1,2,2);
scatter3(SV_loading_Ml__(:,1),SV_loading_Ml__(:,2),SV_loading_Ml__(:,3),24,index_nvolume_from_nM_true_,'filled');
axis vis3d;
title('index_nvolume_from_nM_true_','Interpreter','none');
disp(sprintf(' %% writing %s',fname_fig));
print('-djpeg',sprintf('%s.jpg',fname_fig));
print('-depsc',sprintf('%s.eps',fname_fig));
close(gcf);
...
%%%%%%%%;
tmp_index_ = 0:n_M-1;
if (nvolume<n_volume); tmp_index_ = efind(index_nvolume_from_nM_reco_==nvolume); end;
subplot(4,(1+n_volume),1+nvolume+0*(1+n_volume));
...
title(sprintf('reco %d',nvolume));
subplot(4,(1+n_volume),1+nvolume+1*(1+n_volume));
imagesc(log(1+hist2d_0(SV_loading_Ml__(1+tmp_index_,1+npc0),SV_loading_Ml__(1+tmp_index_,1+npc1),32,32,llim_0_,llim_1_)),[0,5]);
axis image; axisnotick;
title(sprintf('reco %d',nvolume));
%%%%%%%%;
tmp_index_ = 0:n_M-1;
if (nvolume<n_volume); tmp_index_ = efind(index_nvolume_from_nM_true_==nvolume); end;
subplot(4,(1+n_volume),1+nvolume+2*(1+n_volume));
plot(SV_loading_Ml__(1+tmp_index_,1+npc0),SV_loading_Ml__(1+tmp_index_,1+npc1),'ko','MarkerSize',markersize_use,'MarkerFaceColor',c_beach__(1+nc,:));
xlim(llim_0_); ylim(llim_1_); grid on ; axisnotick;
title(sprintf('true %d',nvolume));
subplot(4,(1+n_volume),1+nvolume+3*(1+n_volume));
imagesc(log(1+hist2d_0(SV_loading_Ml__(1+tmp_index_,1+npc0),SV_loading_Ml__(1+tmp_index_,1+npc1),32,32,llim_0_,llim_1_)),[0,5]);
axis image; axisnotick;
title(sprintf('true %d',nvolume));
%%%%%%%%;
...
%%%%%%%%;
index_T1_ = efind(index_nvolume_from_nM_true_==0);
h_T1_ = sum(h_het__(:,1+0),2);
index_T23_ = union(efind(index_nvolume_from_nM_true_==1),efind(index_nvolume_from_nM_true_==2));
h_T23_ = sum(h_het__(:,1+[1,2]),2);
auc_T1_vs_T23 = auc_0(SV_loading_Ml__(1+index_T1_,1),SV_loading_Ml__(1+index_T23_,1));
%%%%%%%%;
...
stairs(tmp_x_,h_T1_ ,'r-','LineWidth',3);
stairs(tmp_x_,h_T23_ ,'g-','LineWidth',3);
...
hold off;
title(sprintf('AUC=%0.3f',auc_T1_vs_T23),'Interpreter','none');
...
%%%%%%%%;
tmp_fname_pos = sprintf('X_2d_xcor_d0_a1t0039n18r0');
...
%%%%%%%%;
fname_fig = sprintf('%s_jpg/loading_het_%s_FIGF',dir_pm,tmp_fname_pos);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1);clf;figsml;figbeach();
