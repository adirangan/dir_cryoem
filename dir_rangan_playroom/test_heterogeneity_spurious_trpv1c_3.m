...
% we design tmp_squi to preserve 4-fold symmetry. ;
%%%%%%%%;
tmp_squi = @(azimu_b) periodize(atan2(4*sin(2*azimu_b),cos(2*azimu_b)),0,2*pi)/2 + pi*(azimu_b>pi);
viewing_polar_a_mode = mode(viewing_polar_a_all_);
tmp_nS_ = efind(abs(viewing_polar_a_all_-viewing_polar_a_mode)<1e-6); n_l = numel(tmp_nS_); tmp_n_S = numel(tmp_nS_);
flag_disp=1;
if flag_disp;
...
plot(tmp_azimu_b_,tmp_squi(tmp_azimu_b_),'go',tmp_azimu_b_,tmp_azimu_b_,'k-');
...
%%%%;
subplot(p_row,p_col,1+np);np=np+1;
isosurface_f_x_u_1(struct('percent_threshold_',99.0),a_x_u_norm_reco_xxx_);
title(sprintf('true: %.1f%%',99.0),'Interpreter','none');
set(gca,'FontSize',fontsize_use);
%%%%;
subplot(p_row,p_col,1+np);np=np+1;
isosurface_f_x_u_1(struct('percent_threshold_',99.0),a_x_u_norm_resq_xxx_);
title(sprintf('spurious: %.1f%%',99.0),'Interpreter','none');
set(gca,'FontSize',fontsize_use);
%%%%;
subplot(p_row,p_col,1+np);np=np+1;
isosurface_f_x_u_1(struct('percent_threshold_',99.5),a_x_u_norm_reco_xxx_);
title(sprintf('true: %.1f%%',99.5),'Interpreter','none');
set(gca,'FontSize',fontsize_use);
%%%%;
subplot(p_row,p_col,1+np);np=np+1;
isosurface_f_x_u_1(struct('percent_threshold_',99.5),a_x_u_norm_resq_xxx_);
title(sprintf('spurious: %.1f%%',99.5),'Interpreter','none');
set(gca,'FontSize',fontsize_use);
%%%%;
set(gcf,'Position',1+[0,0,1024,1024]);
fname_fig_pre = sprintf('%s_jpg/test_heterogeneity_spurious_trpv1c_true_vs_resq_FIGA',dir_pm);
fname_fig_jpg = sprintf('%s.jpg',fname_fig_pre);
...
tmp_n_S_use = numel(efind(cumsum(sort(tmp_nllr_S_))<-log(0.05))); %<-- 390 templates. ;
%%%%%%%%;
...
%%%%;
fname_fig_pre = sprintf('%s_jpg/test_heterogeneity_spurious_trpv1c_p%.4d_FIGA',dir_pm,round(100*tmp_prct));
sgtitle(fname_fig_pre,'Interpreter','none');
fname_fig_jpg = sprintf('%s.jpg',fname_fig_pre);
if (flag_replot | ~exist(fname_fig_jpg,'file'));
disp(sprintf(' %% %s not found, creating',fname_fig_jpg));
...
%%%%;
sgtitle('');
fname_fig_pre = sprintf('%s_jpg/test_heterogeneity_spurious_trpv1c_p%.4d_FIGA_stripped',dir_pm,round(100*tmp_prct));
fname_fig_jpg = sprintf('%s.jpg',fname_fig_pre);
if (flag_replot | ~exist(fname_fig_jpg,'file'));
disp(sprintf(' %% %s not found, creating',fname_fig_jpg));
print('-djpeg',fname_fig_jpg);
...
%%%%%%%%;
dir_mrc = sprintf('/%s/rangan/dir_cryoem/dir_trpv1c/dir_mrc',string_root);
...
set(gcf,'Position',1+[0,0,1024*2,512]); drawnow();
%%%%;
fname_fig_pre = sprintf('%s_jpg/test_heterogeneity_spurious_trpv1c_p%.4d_FIGD',dir_pm,round(100*tmp_prct));
sgtitle(fname_fig_pre,'Interpreter','none');
fname_fig_jpg = sprintf('%s.jpg',fname_fig_pre);
if (flag_replot | ~exist(fname_fig_jpg,'file'));
disp(sprintf(' %% %s not found, creating',fname_fig_jpg));
...
%%%%;
sgtitle('');
fname_fig_pre = sprintf('%s_jpg/test_heterogeneity_spurious_trpv1c_p%.4d_FIGD_stripped',dir_pm,round(100*tmp_prct));
fname_fig_jpg = sprintf('%s.jpg',fname_fig_pre);
if (flag_replot | ~exist(fname_fig_jpg,'file'));
disp(sprintf(' %% %s not found, creating',fname_fig_jpg));
print('-djpeg',fname_fig_jpg);
...
%%%%%%%%;
a_k_Y_norm_rem2_yk__ = zeros(n_lm_max,n_k_p_r);
for nk_p_r=0:n_k_p_r-1;
tmp_index_ = n_lm_csum_(1+nk_p_r) + (0:n_lm_(1+nk_p_r)-1);
a_k_Y_norm_rem2_yk__(1:n_lm_(1+nk_p_r),1+nk_p_r) = a_k_Y_norm_rem2_yk_(1+tmp_index_);
end;%for nk_p_r=0:n_k_p_r-1;
%%%%%%%%;
template_k_eq_d = -1;
viewing_k_eq_d = 1.0; %<-- subsample just a few templates for visualization. ;
...
,l_max_max ...
,n_k_p_r ...
,reshape(a_k_Y_norm_rem2_yk__,[n_lm_max,n_k_p_r]) ...
,viewing_k_eq_d/k_p_r_max ...
,template_k_eq_d ...
,n_w_max ...
);
tmp_t = toc(tmp_t); if (verbose); disp(sprintf(' %% pm_template_2: %0.6fs',tmp_t)); end;
S_rem2_k_p_wkS__ = reshape(S_rem2_k_p_wkS__,[n_w_max*n_k_p_r,n_S]);
S_rem2_k_q_wkS__ = zeros(n_w_sum,n_S);
for nS=0:n_S-1;
S_rem2_k_q_wkS__(:,1+nS) = interp_p_to_q(n_k_p_r,n_w_,n_w_sum,S_rem2_k_p_wkS__(:,1+nS));
end;%for nS=0:n_S-1;
%%%%%%%%;
...
%%%%%%%%;
% rsync -anvum rangan@access1.cims.nyu.edu:/data/rangan/dir_cryoem/dir_trpv1c/dir_mrc /mnt/home/rangan/ceph/dir_cryoem/dir_trpv1/ ;
...
%%%%%%%%;
% Now generate svd_VUXM_lwnM____. ;
%%%%%%%%;
tmp_t = tic();
svd_VUXM_lwnM____ = tpmh_VUXM_lwnM____3(FTK,n_k_p_r,n_w_,n_M,M_k_q__,n_UX_rank,UX_kn__,sqrt(weight_2d_k_p_r_));
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% svd_VUXM_lwnM____: %0.3fs',tmp_t)); end;
%%%%%%%%;
% Now calculate norms of the images. ;
%%%%%%%%;
tmp_t = tic();
UX_M_l2_dM__ = ampmh_UX_M_l2_dM__1(FTK,n_w_,n_M,n_UX_rank,svd_VUXM_lwnM____);
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% UX_M_l2_dM__: %0.3fs',tmp_t)); end;
if (verbose); disp(sprintf(' %% average l2-norm of images: %0.16f',mean(UX_M_l2_dM__(:))/(pi*k_p_r_max^2))); end;
...
hold off;
subplot(1,2,2); plot(UX_M_l2_M_/(pi*k_p_r_max^2),tmp_MM_M_/(pi*k_p_r_max^2),'g.'); xlabel('l2_A'); ylabel('l2_B');
end;%if flag_plot;
%%%%%%%%;
tmp_t = tic();
[UX_M_k_q_wnM___,UX_M_k_p_wnM___] = ampmh_UX_M_k_p_wnM___0(FTK,n_w_,n_UX_rank,n_M,svd_VUXM_lwnM____,zeros(n_M,1),zeros(n_M,1));
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% UX_M_k_q_wnM___: %0.6fs',tmp_t)); end;
UX_M_k_p_wnM__ = reshape(UX_M_k_p_wnM___(:,1:n_UX_rank,:),[n_w_max*n_UX_rank,n_M]);
UX_M_k_q_wnM__ = reshape(UX_M_k_q_wnM___(:,1:n_UX_rank,:),[n_w_max*n_UX_rank,n_M]);
%%%%%%%%;
% Visualize: ;
%%%%%%%%;
flag_plot=0;
if flag_plot;
figure(3);clf;figbig;fig80s;
subplot(2,2,1);imagesc(reshape(permute(log10(abs(svd_VUXM_lwnM____)),[1,3,4,2]),[FTK.n_svd_l*n_UX_rank*n_M,n_w_max]));axisnotick; colorbar;
subplot(2,2,2);plot(0:n_w_max-1,log10(sqrt(mean(reshape(permute(abs(svd_VUXM_lwnM____).^2,[1,3,4,2]),[FTK.n_svd_l*n_UX_rank*n_M,n_w_max]),1))));
...
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% X_TM__: %0.3fs',tmp_t)); end;
%%%%%%%%;
parameter = struct('type','parameter');
parameter.flag_optimize_over_gamma_z = 1;
parameter.flag_compute_I_value = 0;
...
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% X_half_TM__: %0.3fs',tmp_t)); end;
%%%%%%%%;
parameter = struct('type','parameter');
parameter.flag_optimize_over_gamma_z = 1;
parameter.flag_compute_I_value = 0;
...
%%%%%%%%;
%%%%%%%%%%%%%%%%;
% imagesc(hist2d_0(reshape(X_TM__,[n_S*n_M,1]),reshape(max(X_half_TM__,X_rem2_TM__),[n_S*n_M,1]),129,129));set(gca,'Ydir','normal'); %<-- histogram. ;
%%%%%%%%;
flag_disp=0;
if flag_disp;
%%%%;
% visualize distance. ;
%%%%;
D2_TM__ = bsxfun(@plus,reshape(UX_T_l2_S_,[n_S,1]),reshape(UX_M_l2_M_,[1,n_M])) ...
  - 2*X_TM__ .* (reshape(sqrt(UX_T_l2_S_),[n_S,1])*reshape(sqrt(UX_M_l2_M_),[1,n_M]));
D2_half_TM__ = bsxfun(@plus,reshape(UX_T_half_l2_S_,[n_S,1]),reshape(UX_M_l2_M_,[1,n_M])) ...
  - 2*X_half_TM__ .* (reshape(sqrt(UX_T_half_l2_S_),[n_S,1])*reshape(sqrt(UX_M_l2_M_),[1,n_M]));
D2_rem2_TM__ = bsxfun(@plus,reshape(UX_T_rem2_l2_S_,[n_S,1]),reshape(UX_M_l2_M_,[1,n_M])) ...
  - 2*X_rem2_TM__ .* (reshape(sqrt(UX_T_rem2_l2_S_),[n_S,1])*reshape(sqrt(UX_M_l2_M_),[1,n_M]));
hlim_ = [0.050,0.125];
tmp_h2__ = hist2d_0(reshape(D2_TM__,[n_S*n_M,1]),reshape(min(D2_half_TM__,D2_rem2_TM__),[n_S*n_M,1]),129,129,hlim_,hlim_);
...
%%%%%%%%;
% First note that the 'optimal' intensity for maximum-likelihood-alignment is 0.2857. ;
%%%%%%%%;
intensity_ = 0.28*exp(-1:0.01:+1); n_intensity = numel(intensity_);
ssnll_uni_Mi__ = zeros(n_M,n_intensity);
for nintensity=0:n_intensity-1;
[~,ssnll_uni_Mi__(:,1+nintensity)] = ssnll_from_X_0([],n_S,n_M,X_TM__,UX_T_l2_S_*intensity_(1+nintensity).^2,UX_M_l2_M_,viewing_weight_all_,1,0);
end;%for nintensity=0:n_intensity-1;
[~,tmp_ij] = min(sum(ssnll_uni_Mi__,1)); tmp_intensity_uni = intensity_(tmp_ij); %<-- tmp_intensity_uni = 0.2857;
%%%%%%%%;
intensity_ = 0.28*exp(-1:0.01:+1); n_intensity = numel(intensity_);
ssnll_emp_Mi__ = zeros(n_M,n_intensity);
for nintensity=0:n_intensity-1;
[~,ssnll_half_Mi__(:,1+nintensity)] = ssnll_from_X_0([],n_S,n_M,X_half_TM__,UX_T_half_l2_S_*intensity_(1+nintensity).^2,UX_M_l2_M_,viewing_weight_emp_,1,0);
[~,ssnll_rem2_Mi__(:,1+nintensity)] = ssnll_from_X_0([],n_S,n_M,X_rem2_TM__,UX_T_rem2_l2_S_*intensity_(1+nintensity).^2,UX_M_l2_M_,viewing_weight_emp_,1,0);
ssnll_emp_Mi__ = min(ssnll_half_Mi__,ssnll_rem2_Mi__);
end;%for nintensity=0:n_intensity-1;
[~,tmp_ij] = min(sum(ssnll_emp_Mi__,1)); tmp_intensity_emp = intensity_(tmp_ij); %<-- tmp_intensity_emp = 0.2944;
%%%%%%%%;
...
,n_M ...
,X_TM__ ...
,UX_T_l2_S_*tmp_intensity_uni^2 ...
,UX_M_l2_M_ ...
,viewing_weight_all_ ...
,n_sigma_bayesian ...
,sigma_bayesian_ ...
...
,n_M ...
,X_half_TM__ ...
,UX_T_half_l2_S_*tmp_intensity_emp^2 ...
,UX_M_l2_M_ ...
,viewing_weight_all_ ...
,n_sigma_bayesian ...
,sigma_bayesian_ ...
...
,n_M ...
,X_rem2_TM__ ...
,UX_T_rem2_l2_S_*tmp_intensity_emp^2 ...
,UX_M_l2_M_ ...
,viewing_weight_all_ ...
,n_sigma_bayesian ...
,sigma_bayesian_ ...
