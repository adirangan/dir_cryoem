...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
% Now perform stripped down innerproct. ;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
...
tmp_t1 = toc(tmp_t1); tmp_t1_kern_S = tmp_t1_kern_S + tmp_t1;
%%%%;
tmp_t2 = tic();
tmp_X_01__ = zeros(n_k_p_r,n_k_p_r);
...
tmp_t2 = toc(tmp_t2); tmp_t2_kern_S = tmp_t2_kern_S + tmp_t2;
%%%%;
tmp_t3 = tic();
tmp_X__ = diag(tmp_X_weight_r_) * (2*real(tmp_X_00__) - 2*real(tmp_X_01__)) * diag(tmp_X_weight_r_) ;
tmp_t3 = toc(tmp_t3); tmp_t3_kern_S = tmp_t3_kern_S + tmp_t3;
%%%%;
X_S_kk__ = tmp_X__;
...
tot_t_kern_S = tmp_t;
if (verbose>1); disp(sprintf(' %% X_S_kkc___: %0.3fs %0.3fs %0.3fs %0.3fs --> %0.3fs',tmp_t0_kern_S,tmp_t1_kern_S,tmp_t2_kern_S,tmp_t3_kern_S,tot_t_kern_S)); end;
%%%%%%%%;
...
%%%%%%%%;
% Now calculate alignment, aggregating clusters (i.e., using only the average CTF). ;
%%%%%%%%;
...
for nUX_rank=[0,5,n_UX_rank-1];%for nUX_rank=0:n_UX_rank-1;
pm_n_UX_rank = 1+nUX_rank;
...
for nM_batch=0:n_M_batch-1; for nS_batch=0:n_S_batch-1;
...
tmp_o = pm_n_UX_rank*n_k_p_r*(tmp_n_M + tmp_n_S);
