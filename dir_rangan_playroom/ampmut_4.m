...
if (verbose); disp(sprintf(' %% [testing ampmut_4.m]')); end;
%%%%%%%%;
...
index_nCTF_from_nM_ = sort(mod(transpose(0:n_M-1),n_ctf));
CTF_k_p_r__ = zeros(n_k_p_r,n_ctf);
rng(0);
for nctf=0:n_ctf-1;
CTF_k_p_r__(:,1+nctf) = sin(k_p_r_*(1+0.25*rand()));
end;%for nctf=0:n_ctf-1;
SCTF_ = svd(CTF_k_p_r__(:,1+index_nCTF_from_nM_)); %<-- weighted average. ;
n_CTF_rank = min(efind(cumsum(SCTF_,'reverse')/sum(SCTF_)<tolerance_master));
[UCTF_kc__,SCTF_c__,VCTF_Mc__] = svds(CTF_k_p_r__(:,1+index_nCTF_from_nM_),n_CTF_rank);
...
%%%%%%%%;
CTF_avg_k_p_r_ = mean(CTF_k_p_r__(:,1+index_nCTF_from_nM_),2);
CTF_k_p_r_xavg__ = CTF_avg_k_p_r_ * transpose(CTF_avg_k_p_r_);
CTF_k_p_r_xcor__ = CTF_k_p_r__(:,1+index_nCTF_from_nM_) * transpose(CTF_k_p_r__(:,1+index_nCTF_from_nM_)) / n_M;
%%%%%%%%;
...
verbose=2;
if (verbose); disp(sprintf(' %% [entering ampmut_4]')); end;
...
if (verbose); disp(sprintf(' %% [finished ampmut_4]')); end;
