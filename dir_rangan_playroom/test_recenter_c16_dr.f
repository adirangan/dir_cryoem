c     gfortran -w -o test_recenter_c16_dr.out test_recenter_c16_dr.f ; ./test_recenter_c16_dr.out
      program test_recenter_c16_dr
      implicit none
      integer verbose
      data verbose / 2 /
      ...
      call recenter_c16(n_x,n_y,a_,a_)
      
      if (verbose.gt.0) then
         write(format_string,'(A,I0,A)') '(' , n_x , '(2F8.4,4X))'
         write(6,'(A)') 'recentered a_: '
         write(6,format_string) (a_(na),na=0,n_x*n_y-1)
      end if !verbose
