...
%%%%%%%%;
eta = pi/max(1e-12,k_p_r_max); tmp_t = tic;
S_x_c_xx_ = xxnufft2d3(n_w_sum,2*pi*k_c_0_wk_*eta,2*pi*k_c_1_wk_*eta,S_k_p_wk_.*(2*pi)^4.*weight_2d_wk_,+1,1e-12,n_xx_c,x_c_0__(:)/eta,x_c_1__(:)/eta)/sqrt(2*pi)/sqrt(2*pi) / (sqrt(2*pi)^2) ;
%%%%%%%%;
S_x_c_xx_ = reshape(S_x_c_xx_,[n_x0,n_x1]);
