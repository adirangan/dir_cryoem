...
str_thisfunction = 'interpolate_template_7';

if nargin<1;
rng(0);
flag_verbose = 2; nf=1;
if (flag_verbose); disp(sprintf(' %% testing %s',str_thisfunction)); end;
test_interpolate_template_7;
...
if isempty(wS_from_R_single_shell_Rsba___); wS_from_R_single_shell_Rsba___ = cell(n_R,1); end;
if isempty(dwSda_from_R_single_shell_Rsba___); dwSda_from_R_single_shell_Rsba___ = cell(n_R,1); end;
if isempty(dwSdb_from_R_single_shell_Rsba___); dwSdb_from_R_single_shell_Rsba___ = cell(n_R,1); end;
if isempty(ddwSdaa_from_R_single_shell_Rsba___); ddwSdaa_from_R_single_shell_Rsba___ = cell(n_R,1); end;
if isempty(ddwSdab_from_R_single_shell_Rsba___); ddwSdab_from_R_single_shell_Rsba___ = cell(n_R,1); end;
if isempty(ddwSdbb_from_R_single_shell_Rsba___); ddwSdbb_from_R_single_shell_Rsba___ = cell(n_R,1); end;
viewing_R_polar_a_SR__ = zeros(n_S,n_R);
viewing_R_azimu_b_SR__ = zeros(n_S,n_R);
viewing_R_gamma_z_SR__ = zeros(n_S,n_R);
template_rec_RwkS___ = cell(n_R,1);
if flag_d;
dtemplateda_rec_RwkS___ = cell(n_R,1);
dtemplatedb_rec_RwkS___ = cell(n_R,1);
dtemplatedc_rec_RwkS___ = cell(n_R,1);
end;%if flag_d;
if flag_dd;
ddtemplatedaa_rec_RwkS___ = cell(n_R,1);
ddtemplatedab_rec_RwkS___ = cell(n_R,1);
ddtemplatedac_rec_RwkS___ = cell(n_R,1);
ddtemplatedbb_rec_RwkS___ = cell(n_R,1);
ddtemplatedbc_rec_RwkS___ = cell(n_R,1);
ddtemplatedcc_rec_RwkS___ = cell(n_R,1);
end;%if flag_dd;
%%%%%%%%%%%%%%%%;
for nR=0:n_R-1;
%%%%%%%%%%%%%%%%;
if (flag_verbose>0); disp(sprintf(' %% nR %d/%d calling interpolate_template_R_0.',nR,n_R)); end;
R_use__ = R_use_R___{1+nR};
a_R_k_p_qk_ = a_R_k_p_Rqk__{1+nR};
ba_from_single_shell_baba__ = ba_from_single_shell_Rbaba___{1+nR};
wS_from_R_single_shell_sba__ = wS_from_R_single_shell_Rsba___{1+nR};
if flag_d;
dwSda_from_R_single_shell_sba__ = dwSda_from_R_single_shell_Rsba___{1+nR};
dwSdb_from_R_single_shell_sba__ = dwSdb_from_R_single_shell_Rsba___{1+nR};
end;%if flag_d;
if flag_dd;
ddwSdaa_from_R_single_shell_sba__ = ddwSdaa_from_R_single_shell_Rsba___{1+nR};
ddwSdab_from_R_single_shell_sba__ = ddwSdab_from_R_single_shell_Rsba___{1+nR};
ddwSdbb_from_R_single_shell_sba__ = ddwSdbb_from_R_single_shell_Rsba___{1+nR};
end;%if flag_dd;
%%%%%%%%;
if  flag_1 & ~flag_d & ~flag_dd;
tmp_t = tic();
...
tmp_t = toc(tmp_t); if (flag_verbose>0); disp(sprintf(' %% interpolate_template_R_0: %0.6fs',tmp_t)); end;
end;%if  flag_1 & ~flag_d & ~flag_dd;
%%%%%%%%;
if  flag_1 &  flag_d & ~flag_dd;
tmp_t = tic();
...
viewing_R_polar_a_SR__(:,1+nR) = viewing_R_polar_a_S_;
viewing_R_azimu_b_SR__(:,1+nR) = viewing_R_azimu_b_S_;
viewing_R_gamma_z_SR__(:,1+nR) = viewing_R_gamma_z_S_;
wS_from_R_single_shell_Rsba___{1+nR} = wS_from_R_single_shell_sba__;
if flag_d;
dwSda_from_R_single_shell_Rsba___{1+nR} = dwSda_from_R_single_shell_sba__;
dwSdb_from_R_single_shell_Rsba___{1+nR} = dwSdb_from_R_single_shell_sba__;
end;%if flag_d;
if flag_dd;
ddwSdaa_from_R_single_shell_Rsba___{1+nR} = ddwSdaa_from_R_single_shell_sba__;
ddwSdab_from_R_single_shell_Rsba___{1+nR} = ddwSdab_from_R_single_shell_sba__;
ddwSdbb_from_R_single_shell_Rsba___{1+nR} = ddwSdbb_from_R_single_shell_sba__;
end;%if flag_dd;
template_rec_RwkS___{1+nR} = template_rec_wkS__;
if flag_d;
dtemplateda_rec_RwkS___{1+nR} = dtemplateda_rec_wkS__;
dtemplatedb_rec_RwkS___{1+nR} = dtemplatedb_rec_wkS__;
dtemplatedc_rec_RwkS___{1+nR} = dtemplatedc_rec_wkS__;
end;%if flag_d;
if flag_dd;
ddtemplatedaa_rec_RwkS___{1+nR} = ddtemplatedaa_rec_wkS__;
ddtemplatedab_rec_RwkS___{1+nR} = ddtemplatedab_rec_wkS__;
ddtemplatedac_rec_RwkS___{1+nR} = ddtemplatedac_rec_wkS__;
ddtemplatedbb_rec_RwkS___{1+nR} = ddtemplatedbb_rec_wkS__;
ddtemplatedbc_rec_RwkS___{1+nR} = ddtemplatedbc_rec_wkS__;
ddtemplatedcc_rec_RwkS___{1+nR} = ddtemplatedcc_rec_wkS__;
end;%if flag_dd;
%%%%%%%%%%%%%%%%;
end;%for nR=0:n_R-1;
%%%%%%%%%%%%%%%%;
...
viewing_R_polar_a_mod_S_subR__ = zeros(n_S_sub,n_R);
viewing_R_azimu_b_mod_S_subR__ = zeros(n_S_sub,n_R);
for nS_sub=0:n_S_sub-1;
nS = index_nS_from_nS_sub_(1+nS_sub);
for nR=0:n_R-1;
viewing_R_polar_a_mod_S_subR__(1+nS_sub,1+nR) = abs(periodize(viewing_R_polar_a_SR__(1+nS,1+nR),-pi/4,+pi/4));
viewing_R_azimu_b_mod_S_subR__(1+nS_sub,1+nR) = abs(periodize(viewing_R_azimu_b_SR__(1+nS,1+nR),-pi/4,+pi/4));
end;%for nR=0:n_R-1;
end;%for nS_sub=0:n_S_sub-1;
%%%%%%%%;
if (flag_verbose>0); disp(sprintf(' %% Select representative for each nR.')); end;
nR_from_nS_sub_ = zeros(n_S_sub,1);
for nS_sub=0:n_S_sub-1;
[~,ijR] = max(min(viewing_R_polar_a_mod_S_subR__(1+nS_sub,:),viewing_R_azimu_b_mod_S_subR__(1+nS_sub,:))); nR = ijR-1;
nR_from_nS_sub_(1+nS_sub) = nR;
end;%for nS_sub=0:n_S_sub-1;
%%%%%%%%;
...
for nS_sub=0:n_S_sub-1;
nS = index_nS_from_nS_sub_(1+nS_sub);
nR = nR_from_nS_sub_(1+nS_sub);
template_ori_wkS__(:,1+nS) = template_rec_RwkS___{1+nR}(:,1+nS);
end;%for nS_sub=0:n_S_sub-1;
end;%if flag_1;
%%%%%%%%;
if flag_d;
...
nS = index_nS_from_nS_sub_(1+nS_sub);
nR = nR_from_nS_sub_(1+nS_sub);
dtemplateda_ori_wkS__(:,1+nS) = dtemplateda_rec_RwkS___{1+nR}(:,1+nS);
dtemplatedb_ori_wkS__(:,1+nS) = dtemplatedb_rec_RwkS___{1+nR}(:,1+nS);
dtemplatedc_ori_wkS__(:,1+nS) = dtemplatedc_rec_RwkS___{1+nR}(:,1+nS);
end;%for nS_sub=0:n_S_sub-1;
end;%if flag_d;
%%%%%%%%;
if flag_dd;
...
nS = index_nS_from_nS_sub_(1+nS_sub);
nR = nR_from_nS_sub_(1+nS_sub);
ddtemplatedaa_ori_wkS__(:,1+nS) = ddtemplatedaa_rec_RwkS___{1+nR}(:,1+nS);
ddtemplatedab_ori_wkS__(:,1+nS) = ddtemplatedab_rec_RwkS___{1+nR}(:,1+nS);
ddtemplatedac_ori_wkS__(:,1+nS) = ddtemplatedac_rec_RwkS___{1+nR}(:,1+nS);
ddtemplatedbb_ori_wkS__(:,1+nS) = ddtemplatedbb_rec_RwkS___{1+nR}(:,1+nS);
ddtemplatedbc_ori_wkS__(:,1+nS) = ddtemplatedbc_rec_RwkS___{1+nR}(:,1+nS);
ddtemplatedcc_ori_wkS__(:,1+nS) = ddtemplatedcc_rec_RwkS___{1+nR}(:,1+nS);
end;%for nS_sub=0:n_S_sub-1;
end;%if flag_dd;
%%%%%%%%;
