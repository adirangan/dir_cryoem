...
%%%%%%%%;
% Compare X_best_ with X_relion across experiments/molecules. ;
%%%%%%%%;
fname_fig = sprintf('%s/test_pm_collect_plus_X_best_vs_X_relion_FIGH',dir_jpg);
...
hold on;
plot([-0.125,1.125],[-0.125,1.125],'-','Color',0.85*[1,1,1],'LineWidth',3);
%%%%%%%%;
for nu_experiment=0:n_u_experiment-1;
dir_nopath_data_star = u_dir_nopath_data_star_{1+nu_experiment};
index_nexperiment_from_nu_ = index_nexperiment_from_nu__{1+nu_experiment};
%%%%%%%%;
for nl0=0:numel(index_nexperiment_from_nu_)-1;
nexperiment = index_nexperiment_from_nu_(1+nl0);
collect_ = collect__{1+nexperiment};
collect_aX_ = collect_aX__{1+nexperiment};
%%%%%%%%;
n_a = size(collect_.X_best_a_,1);
tmp_rank_pm_use_ = 10:2:20; %<-- only high rank. ;
for na=0:n_a-1;
%%%%;
...
if (verbose>1); disp(sprintf(' %% %s: crop at index %d',collect_.dir_nopath_data_star,tmp_index_crop)); end;
tmp_X_ = tmp_X_(1+tmp_index_crop,:);
end;%if (~isempty(collect_.corr_crop_xa__));
%%%%;
tmp_delta_sigma_use = collect_.delta_sigma_use_a_(1+na);
tmp_rank_pm = collect_.rank_pm_a_(1+na);
if (0 | tmp_delta_sigma_use> 0);
for nl1=0:numel(tmp_rank_pm_use_)-1;
tmp_rank_pm_use = tmp_rank_pm_use_(1+nl1);
...
if (tmp_X_(1+na)~=0);
plot(collect_.X_relion,tmp_X_(1+na),str_symbol,'MarkerEdgeColor',0.85*[1,1,1],'MarkerFaceColor',c_80s__(1+nc_80s,:),'MarkerSize',markersize_use);
end;%if (tmp_X_(1+na)~=0);
end;%if (0 | tmp_rank_pm==tmp_rank_pm_use);
end;%for nl1=0:numel(tmp_rank_pm_use_)-1;
end;%if (tmp_delta_sigma_use> 0);
end;%for na=0:n_a-1;
end;%for nl0=0:numel(index_nexperiment_from_nu_)-1;
end;%for nu_experiment=0:n_u_experiment-1;
hold off;
xlabel('Relion correlation/max');
ylabel('AM correlation/max');
xlim([-0.125,1.125]);
ylim([-0.125,1.125]);
axis square;
grid on;
