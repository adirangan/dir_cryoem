...
if (verbose); disp(sprintf(' %% [entering ampmh_X_wSM___5_experiment_6]')); end;

n_level = 5;
template_tree = get_template_tree_0(viewing_k_eq_d,n_level);
...
% Compare to local search. ;
%%%%%%%%;
fname_mat = sprintf('ampmh_X_wSM___5_experiment_6.mat');
if (~exist(fname_mat,'file'));
disp(sprintf(' %% %s not found, creating',fname_mat));
n_neighbor_retain_ = 2.^[0:5]; n_n_neighbor_retain = numel(n_neighbor_retain_);
n_level_ = 1:11; n_n_level = numel(n_level_);
X_p_tree___ = zeros(tmp_n_M,n_n_neighbor_retain,n_n_level); %<-- correlation across landscape (using tree interpolation). ;
X_p_full___ = zeros(tmp_n_M,n_n_neighbor_retain,n_n_level); %<-- correlation only across evaluated representatives. ;
t_tree__ = zeros(n_n_neighbor_retain,n_n_level);
f_tree__ = zeros(n_n_neighbor_retain,n_n_level);
for nn_neighbor_retain=0:n_n_neighbor_retain-1;
n_neighbor_retain = n_neighbor_retain_(1+nn_neighbor_retain);
for nn_level=0:n_n_level-1;
n_level = n_level_(1+nn_level);
...
,UX_M_l2_dM__(:,1+tmp_index_M_) ...
,parameter ...
,get_template_tree_0(viewing_k_eq_d,n_level) ...
);
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% n_neighbor_retain %.2d n_level %.2d X_SM_tree__: %0.3fs',n_neighbor_retain,n_level,tmp_t)); end;
t_tree__(1+nn_neighbor_retain,1+nn_level) = tmp_t;
f_tree__(1+nn_neighbor_retain,1+nn_level) = numel(efind(t_level_SM_tree__(:)>=0));
for nM=0:tmp_n_M-1;
[~,tmp_ij] = max(X_SM_tree__(:,1+nM));
tmp_p = numel(efind(X_SM_full__(:,1+nM)<=X_SM_full__(tmp_ij,1+nM)))/n_S;
X_p_tree___(1+nM,1+nn_neighbor_retain,1+nn_level) = tmp_p;
...
X_p_full___(1+nM,1+nn_neighbor_retain,1+nn_level) = tmp_p;
end;%for nM=0:tmp_n_M-1;
end;%for nn_level=0:n_n_level-1;
end;%for nn_neighbor_retain=0:n_n_neighbor_retain-1;
%%%%%%%%;
save(fname_mat ...
     ,'n_neighbor_retain_','n_n_neighbor_retain' ...
     ,'n_level_','n_n_level' ...
     ,'f_tree__','t_tree__' ...
     ,'X_p_tree___','X_p_full___' ...
     );
%%%%%%%%;
end;%if (~exist(fname_mat,'file'));
load(fname_mat);

fname_fig = sprintf('ampmh_X_wSM___5_experiment_6_FIGA');
if (~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1);figbig;clf;colormap(colormap_80s());c_80s__ = colormap_80s(); n_c_80s = size(c_80s__,1);
p_row=2;p_col=3; np=0;
%%%%%%%%;
subplot(p_row,p_col,1+np);np=np+1;
imagesc(f_tree__./t_tree__,[0,f_full/t_full]);
ylabel('n_neighbor_retain','interpreter','none');
set(gca,'YTick',1:n_n_neighbor_retain,'YTickLabel',n_neighbor_retain_);
xlabel('n_level','interpreter','none');
set(gca,'XTick',1:n_n_level,'XTickLabel',n_level_);
title('f_tree__./t_tree__','interpreter','none');
%%%%%%%%;
subplot(p_row,p_col,1+np);np=np+1;
hold on;
for nn_level=0:n_n_level-1;
n_level = n_level_(1+nn_level);
nc_80s = max(0,min(n_c_80s-1,floor(n_c_80s*nn_level/n_n_level)));
plot(t_tree__(:,1+nn_level),mean(X_p_tree___(:,:,1+nn_level),1),'o-','Color',c_80s__(1+nc_80s,:));
end;%for nn_level=0:n_n_level-1;
plot(t_full+[0,0],[0,1],'k-');
hold off;
xlabel('time'); ylabel('p_tree','Interpreter','none');
title('line per n_level','interpreter','none');
...
n_level = n_level_(1+nn_level);
nc_80s = max(0,min(n_c_80s-1,floor(n_c_80s*nn_level/n_n_level)));
plot(t_tree__(:,1+nn_level),mean(X_p_full___(:,:,1+nn_level),1),'o-','Color',c_80s__(1+nc_80s,:));
end;%for nn_level=0:n_n_level-1;
plot(t_full+[0,0],[0,1],'k-');
hold off;
...
for nn_neighbor_retain=0:n_n_neighbor_retain-1;
n_neighbor_retain = n_neighbor_retain_(1+nn_neighbor_retain);
nc_80s = max(0,min(n_c_80s-1,floor(n_c_80s*nn_neighbor_retain/n_n_neighbor_retain)));
plot(t_tree__(1+nn_neighbor_retain,2:end),squeeze(mean(X_p_tree___(:,1+nn_neighbor_retain,2:end),1)),'o-','Color',c_80s__(1+nc_80s,:));
end;%for nn_neighbor_retain=0:n_n_neighbor_retain-1;
plot(t_full+[0,0],[0,1],'k-');
hold off;
xlabel('time'); ylabel('p_tree','Interpreter','none');
title('line per n_neighbor_retain','interpreter','none');
xlim([0,2*t_full]);ylim([0,1]);
%%%%%%%%;
subplot(p_row,p_col,1+np);np=np+1;
hold on;
...
n_neighbor_retain = n_neighbor_retain_(1+nn_neighbor_retain);
nc_80s = max(0,min(n_c_80s-1,floor(n_c_80s*nn_neighbor_retain/n_n_neighbor_retain)));
plot(t_tree__(1+nn_neighbor_retain,2:end),squeeze(mean(X_p_full___(:,1+nn_neighbor_retain,2:end),1)),'o-','Color',c_80s__(1+nc_80s,:));
end;%for nn_neighbor_retain=0:n_n_neighbor_retain-1;
plot(t_full+[0,0],[0,1],'k-');
hold off;
xlabel('time'); ylabel('p_full','Interpreter','none');
