...
figbig;
fname_pre = sprintf('/data/rangan/dir_cryoem/dir_rangan_playroom/dir_principled_marching_jpg/test_principled_marching_trpv1_3_FIGB');
...
a_UX_Y_quad__(1:tmp_n_lm,nrank) = a_UX_Y_quad__(1:tmp_n_lm,nrank) + tmp_UX_(nk_p_r,nrank)*a_k_Y_quad_(tmp_ij_)*CTF_avg_k_p_r_(nk_p_r);
end;%for nk_p_r=1:n_k_p_r;
...
subplot(1,2,1); imagesc(log10(abs(tmp_UX_)),[-3,0]); xlabel('rank'); ylabel('shell'); title('log10(abs(UX)) [-3,0]'); 
subplot(1,2,2); plot(log10(abs(diag(tmp_SX_))),'ko'); xlabel('rank'); ylabel('log10(\sigma)'); title('log10(SX)');
figbig;
fname_pre = sprintf('/data/rangan/dir_cryoem/dir_rangan_playroom/dir_principled_marching_jpg/test_principled_marching_trpv1_3_FIGC');
disp(sprintf(' %% writing %s',fname_pre));
print('-djpeg',sprintf('%s.jpg',fname_pre));
print('-depsc',sprintf('%s.eps',fname_pre));
end;%if flag_plot;
...
colormap(colormap_beach());
figbig;
fname_pre = sprintf('/data/rangan/dir_cryoem/dir_rangan_playroom/dir_principled_marching_jpg/test_principled_marching_trpv1_3_FIGD');
disp(sprintf(' %% writing %s',fname_pre));
print('-djpeg',sprintf('%s.jpg',fname_pre));
print('-depsc',sprintf('%s.eps',fname_pre));
end;%if flag_plot;
...
colormap(colormap_beach());
fname_pre = sprintf('/data/rangan/dir_cryoem/dir_rangan_playroom/dir_principled_marching_jpg/test_principled_marching_trpv1_3_FIGE');
disp(sprintf(' %% writing %s',fname_pre));
print('-djpeg',sprintf('%s.jpg',fname_pre));
print('-depsc',sprintf('%s.eps',fname_pre));
end;%if flag_plot;
...
figbig;
fname_pre = sprintf('/data/rangan/dir_cryoem/dir_rangan_playroom/dir_principled_marching_jpg/test_principled_marching_trpv1_3_FIGF');
disp(sprintf(' %% writing %s',fname_pre));
print('-djpeg',sprintf('%s.jpg',fname_pre));
print('-depsc',sprintf('%s.eps',fname_pre));
end;%if flag_plot;
...
figure(1); 
colormap(colormap_beach()); 
subplot(2,2,1); imagesc_c(n_x_u,grid_x_c_,n_x_u,grid_x_c_,M_x_c_,[],colormap_beach()); 
set(gca,'XTick',[],'YTick',[]); axis image; title('M(x)');
subplot(2,2,3); imagesc_p(n_k_p_r,k_p_r_,n_w_,sum(n_w_),real(CTF_k_p__(:,1)),[],colormap_beach());
set(gca,'XTick',[],'YTick',[]); axis image; title('CTF(k)');
subplot(2,2,2); imagesc_p(n_k_p_r,k_p_r_,n_w_,sum(n_w_),real(M_k_p_),[],colormap_beach());
set(gca,'XTick',[],'YTick',[]); axis image; title('real(M(k))');
subplot(2,2,4); imagesc_p(n_k_p_r,k_p_r_,n_w_,sum(n_w_),imag(M_k_p_),[],colormap_beach());
...
% Free up space ;
%%%%%%%%;
clear M_x_c___ N_x_c___ P_x_c___ ;
% Now O_x_c___ contains T(+delta)*M_x_c___; i.e., O contains the shifted images. ;
...
%%%%%%%%;
% Now generate templates for the meta-shells. ;
%%%%%%%%;
