...
fname_mat = sprintf('%s_mat/test_viewing_angle_surface_diffusion_2.mat',dir_trunk);
...
%%%%%%%%;
tmp_t=tic();
template_k_eq_d = 0.25*(2*pi*k_p_r_max) / (2*(l_val_use+1)) ; %<-- this only needs to be sufficiently large to sample each oscillation twice (i.e., nyquist). ;
...
%%%%%%%%;
flag_style = 0; %<-- 0 = pole displacement, 1 = corrected viewing-angle displacement. ;
...
tmp_k2 = cos(polar_a);
tmp_k012 = sqrt(tmp_k0^2 + tmp_k1^2 + tmp_k2^2);
tmp_k0 = tmp_k0/tmp_k012; tmp_k1 = tmp_k1/tmp_k012; tmp_k2 = tmp_k2/tmp_k012; 
...
if flag_style==1;
azimu_b = azimu_b + sigma*randn()/sin(polar_a);
polar_a_upd = polar_a + sigma*randn();
if (polar_a_upd<  0); polar_a_upd =  0 - polar_a_upd; azimu_b = azimu_b + pi; end;
if (polar_a_upd> pi); polar_a_upd = pi - polar_a_upd; azimu_b = azimu_b + pi; end;
polar_a = polar_a_upd;
end;%if flag_style==1;
...
var_ = sigma_.^2;
l_val_max = n_l_val_use-1;
n_m_val_use = 1+2*l_val_max;
lambda_D__ = zeros(n_l_val_use,n_m_val_use);
lambda_F__ = zeros(n_l_val_use,n_m_val_use);
lambda_L__ = zeros(n_l_val_use,n_m_val_use);
for nl_val_use=0:n_l_val_use-1;
l_val_use = l_val_use_(1+nl_val_use);
for nm_val_use=0:(1+2*l_val_use)-1;
m_val_use = -l_val_use + nm_val_use;
lambda = -l_val_use*(l_val_use+1) * (0.5 + (m_val_use/l_val_use)^2);
lambda_D__(1+nl_val_use,1+l_val_max+m_val_use) = lambda;
lambda = mean(log(abs(F_0lsq___(2:end,1+nl_val_use,1+nm_val_use)))./transpose(sigma_(2:end).^2));
lambda_F__(1+nl_val_use,1+l_val_max+m_val_use) = lambda;
lambda = mean(log(abs(L_0lsq___(2:end,1+nl_val_use,1+nm_val_use)))./transpose(sigma_(2:end).^2));
lambda_L__(1+nl_val_use,1+l_val_max+m_val_use) = lambda;
end;%for nm_val_use=0:(1+2*l_val_use)-1;
end;%for nl_val_use=0:n_l_val_use-1;
...
m_val_use = -l_val_use + nm_val_use;
subplot(2,3,1+nl_val_use); hold on;
tmp_diff_ = exp(-l_val_use*(l_val_use+1)*(0.5 + (m_val_use/l_val_use)^2)*sigma_.^2);
plot(var_,log(abs(tmp_diff_)),'b.-');
plot(var_,log(abs(F_0lsq___(:,1+nl_val_use,1+nm_val_use))),'ro-');
plot(var_,log(abs(L_0lsq___(:,1+nl_val_use,1+nm_val_use))),'gx-');
xlabel('variance'); ylabel('log(coefficient)');
