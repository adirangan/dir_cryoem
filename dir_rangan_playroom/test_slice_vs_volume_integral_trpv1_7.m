...
disp(sprintf(' %% %s found, not creating',fname_mat));
tmp_ = load(fname_mat);
euler_polar_a_true_tail_ = tmp_.euler_polar_a_Mi__(:,2:end);
euler_azimu_b_true_tail_ = tmp_.euler_azimu_b_Mi__(:,2:end);
euler_gamma_z_true_tail_ = tmp_.euler_gamma_z_Mi__(:,2:end);
euler_polar_a_true_ = tmp_.euler_polar_a_Mi__(:,end);
euler_polar_a_true_dif_ = periodize(bsxfun(@minus,tmp_.euler_polar_a_Mi__(:,2:end),euler_polar_a_true_),-pi/2,+pi/2); %<-- not symmetry. ;
euler_polar_a_tavg_ = periodize( euler_polar_a_true_ + mean(euler_polar_a_true_dif_(:,end-3:end-0),2) ,0,1*pi);
euler_azimu_b_true_ = tmp_.euler_azimu_b_Mi__(:,end);
euler_azimu_b_true_dif_ = periodize(bsxfun(@minus,tmp_.euler_azimu_b_Mi__(:,2:end),euler_azimu_b_true_),-pi/4,+pi/4); %<-- 4-fold symmetry. ;
euler_azimu_b_tavg_ = periodize( euler_azimu_b_true_ + mean(euler_azimu_b_true_dif_(:,end-3:end-0),2) ,0,2*pi);
euler_gamma_z_true_ = tmp_.euler_gamma_z_Mi__(:,end);
euler_gamma_z_true_dif_ = periodize(bsxfun(@minus,tmp_.euler_gamma_z_Mi__(:,2:end),euler_gamma_z_true_),-pi/2,+pi/2); %<-- not symmetry. ;
euler_gamma_z_tavg_ = periodize( euler_gamma_z_true_ + mean(euler_gamma_z_true_dif_(:,end-3:end-0),2) ,0,2*pi);
...
index_lambda = 1; %<-- 0 is the lowest, niteration-1 is the highest. ;
ij_use = ij_srt_(1+index_lambda); %<-- this is the highest;
...
title('v_min_dtau_','Interpreter','none');
%%%%;
sgtitle(sprintf('%s niteration %d index_lambda %d/%d: %s ',str_T,niteration,index_lambda,niteration-1,str_vv),'Interpreter','none');
%%%%%%%%%%%%%%%%;
end;%if ( exist(fname_mat,'file'));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
