...
sigma_opt = sqrt(2*ssnll_function/tmp_M); %<-- note that the ssnll_function is different than the real-case below. ;
if flag_disp;
...
sigma_opt = sqrt(2*ssnll_function/tmp_M); %<-- note that the ssnll_function is different than the complex-case above. ;
if flag_disp;
plot(sigma_value_q_,likelihood_sigma_function(sigma_value_q_),'ko-',sigma_opt,likelihood_sigma_function(sigma_opt),'rx');
xlim([0,8]); xlabel('sigma');
ylabel('likelihood_sigma','Interpreter','none');
