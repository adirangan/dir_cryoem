%%%%%%%%;
% Derived from test_viewing_angle_distribution_1.m ;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
for str_exte_type_ = {'ASP','DRG','RDN'};
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
...
dir_exte = sprintf('/%s/rangan/cryodrgn/dir_%s/dir_job_%d',string_root,dir_nopath_data_star,tmp_jindex); %<-- fix later;
if ~exist(dir_exte,'dir'); dir_exte = sprintf('/%s/rangan/cryodrgn/dir_%s/dir_job_0',string_root,dir_nopath_data_star); end;
if strcmp(dir_nopath_data_star,'precatalytic_spliceosome');
dir_exte = sprintf('/%s/rangan/cryodrgn/dir_%s/dir_job_%d',string_root,'ps1',tmp_jindex);
