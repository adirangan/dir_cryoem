...
% Starting with ISWINCP_x0. ;
% Verdict: only ran a1t runs, never t alone, ;
% so no example of low-entropy angular distribution. ;
%%%%%%%%;

global_parameter=[];
fname_prefix='ISWINCP_x0';
dir_nopath_data_star='ISWINCP';
Pixel_Spacing=1.07;
...
% test with: ;
% ndat_rseed=2; dat_rseed = dat_rseed_(1+ndat_rseed); ndelta_r_max_factor = 0; delta_r_max_factor = delta_r_max_factor_(1+ndelta_r_max_factor); delta_r_max_use = delta_r_max_factor * delta_sigma_use*sqrt(log(20^2)); delta_r_max_upb = 2.0 * delta_sigma_use*sqrt(log(20^2)); ntolerance_pm=0; tolerance_pm = tolerance_pm_(1+ntolerance_pm); flag_alternate_MS_vs_SM=1;
parameter = struct('type','parameter');
...
XA_fname_mat = sprintf('%s.mat',XA_fname_pre);
...
%%%%%%%%;
euler_polar_a_calc_Mi__ = tmp_XA_.euler_polar_a_Mi__;
euler_azimu_b_calc_Mi__ = tmp_XA_.euler_azimu_b_Mi__;
euler_gamma_z_calc_Mi__ = tmp_XA_.euler_gamma_z_Mi__;
image_delta_x_calc_Mi__ = tmp_XA_.image_delta_x_acc_Mi__ + tmp_XA_.image_delta_x_upd_Mi__;
image_delta_y_calc_Mi__ = tmp_XA_.image_delta_y_acc_Mi__ + tmp_XA_.image_delta_y_upd_Mi__;
%%%%%%%%;
verbose=2;
if (verbose); disp(sprintf(' %% [entering ampmut_align_to_a_k_Y_1]')); end;
%%%%%%%%;
if (~isfield(tmp_XA_.parameter,'tolerance_master')); tmp_XA_.parameter.tolerance_master = 1e-2; end; %<-- parameter_bookmark. ;
if (~isfield(tmp_XA_.parameter,'flag_qbp_vs_lsq')); tmp_XA_.parameter.flag_qbp_vs_lsq = 1; end; %<-- parameter_bookmark. ;
if (~isfield(tmp_XA_.parameter,'cg_lsq_n_order')); tmp_XA_.parameter.cg_lsq_n_order = 5; end; %<-- parameter_bookmark. ;
if (~isfield(tmp_XA_.parameter,'qbp_eps')); tmp_XA_.parameter.qbp_eps = tmp_XA_.parameter.tolerance_master; end; %<-- parameter_bookmark. ;
flag_qbp_vs_lsq = tmp_XA_.parameter.flag_qbp_vs_lsq;
cg_lsq_n_order = tmp_XA_.parameter.cg_lsq_n_order;
qbp_eps = tmp_XA_.parameter.qbp_eps;
%%%%%%%%;
n_w_max = max(n_w_);
n_w_sum = sum(n_w_);
n_w_csum_ = cumsum([0;n_w_]);
%%%%%%%%;
tmp_CTF_k_p_wkC__ = CTF_k_p_r_kC__;
tmp_a_k_Y_quad_ = a_k_Y_true_yk_;
tmp_n_iteration = [];
if ( isempty(tmp_n_iteration)); tmp_n_iteration=0; end;
if (tmp_n_iteration<=0); tmp_n_iteration = size(euler_polar_a_calc_Mi__,2); end;
%%%%%%%%;
...
%%%%%%%%;
flag_found=0;
if (~isempty(tmp_XA_.parameter.fname_align_a_k_Y_pre));
tmp_fname_mat = sprintf('%s.mat',tmp_XA_.parameter.fname_align_a_k_Y_pre);
...
end;%if (~isempty(tmp_XA_.parameter.fname_align_a_k_Y_pre));
%%%%%%%%;

tmp_fname_fig_jpg = sprintf('%s.jpg',tmp_XA_.parameter.fname_align_a_k_Y_pre);
...
markersize_delta = 25;
figure(3); clf; figbig; 
p_row = 4; p_col = 3*ceil((1+tmp_n_iteration)/p_row); np=0;
%%%%%%%%;
...
%%%%%%%%;
for niteration=0:tmp_n_iteration-1;
%%%%%%%%;
...
%%%%%%%%;
end;%for niteration=0:tmp_n_iteration-1;
%%%%%%%%;
