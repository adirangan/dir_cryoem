...
if ~isfield(parameter,'n_polar_a'); parameter.n_polar_a = round(7*k_max); end; %<-- parameter_bookmark. ;
n_polar_a = parameter.n_polar_a;
if ~isfield(parameter,'n_azimu_b'); parameter.n_azimu_b = round(12*k_max); end; %<-- parameter_bookmark. ;
n_azimu_b = parameter.n_azimu_b;
if ~isfield(parameter,'n_gamma_z'); parameter.n_gamma_z = round(128*k_max); end; %<-- parameter_bookmark. ;
...
tmp_x__(:,1+nazimu_b) = r_mid*[ cb_pre * sa_ ; cb_pos * flipud(sa_) ];
tmp_y__(:,1+nazimu_b) = r_mid*[ sb_pre * sa_ ; sb_pos * flipud(sa_) ];
tmp_z__(:,1+nazimu_b) = r_mid*[ ca_ ; flipud(ca_) ];
