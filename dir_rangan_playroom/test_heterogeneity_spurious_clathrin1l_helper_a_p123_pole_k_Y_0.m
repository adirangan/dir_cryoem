...
fname_mat = sprintf('%s_mat/test_heterogeneity_spurious_clathrin1l_cap_theta_%.3d_a_p123_pole_k_Y.mat',dir_pm,floor(100*cap_theta));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
if flag_recalc | ~exist(fname_mat,'file');
disp(sprintf(' %% %s not found, creating',fname_mat));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
% Now build a_p123_pole_x_u_xxx_ using a more restrictive polar cap ;
% associated with a distribution of viewing-angles concentrated around the equator and poles ;
%%%%%%%%;
S_p123_k_p_wkS__ = S_p123_k_p__;
...
S_amp_k_p_wkS__ = S_p123_k_p_wkS__;
...
S_cap_avg = real(mean(S_p123_k_p_wkS__(find(ind_out_wkS__)))); %<-- average of (unused) polar cap values. ;
...
tmp_t = toc(tmp_t); if (verbose); disp(sprintf(' %% a_p123_pole_k_Y_yk_ time %0.2fs',tmp_t)); end;
%%%%%%%%;
tmp_t = tic();
[ ... 
  a_p123_pole_x_u_xxx_ ...
] = ...
...
tmp_t = toc(tmp_t); if (verbose); disp(sprintf(' %% a_p123_pole_x_u_xxx_ time %0.2fs',tmp_t)); end;
%%%%%%%%;
flag_disp=1;
if flag_disp;
...
tmp_parameter = isosurface_f_x_u_1(tmp_parameter,a_p123_x_u_reco_    ); tmp_parameter = rmfield(tmp_parameter,'vval_');
title(sprintf('a_p123_x_u_reco_xxx_: %.1f%%',tmp_prct),'Interpreter','none');
%title(sprintf('A: %.1f%%',tmp_prct),'Interpreter','none');
set(gca,'FontSize',fontsize_use);
subplot(p_row,p_col,1+pcol);pcol=pcol+1;
tmp_parameter = struct('type','parameter','percent_threshold_',[tmp_prct]);
tmp_parameter = isosurface_f_x_u_1(tmp_parameter,a_p123_pole_x_u_xxx_); tmp_parameter = rmfield(tmp_parameter,'vval_');
title(sprintf('a_p123_pole_x_u_xxx_: %.1f%%',tmp_prct),'Interpreter','none');
%title(sprintf('B: %.1f%%',tmp_prct),'Interpreter','none');
set(gca,'FontSize',fontsize_use);
set(gcf,'Position',1+[0,0,1024*2,512]); drawnow();
%%%%;
fname_fig_pre = sprintf('%s_jpg/test_heterogeneity_spurious_clathrin1l_p123_pole_p%.4d_FIGD',dir_pm,round(100*tmp_prct));
...
%%%%%%%%;
a_p123_pole_k_Y_yk__ = zeros(n_lm_max,n_k_p_r);
for nk_p_r=0:n_k_p_r-1;
tmp_index_ = n_lm_csum_(1+nk_p_r) + (0:n_lm_(1+nk_p_r)-1);
a_p123_pole_k_Y_yk__(1:n_lm_(1+nk_p_r),1+nk_p_r) = a_p123_pole_k_Y_yk_(1+tmp_index_);
...
,l_max_max ...
,n_k_p_r ...
,reshape(a_p123_pole_k_Y_yk__,[n_lm_max,n_k_p_r]) ...
,viewing_k_eq_d/k_p_r_max ...
,template_k_eq_d ...
,n_w_max ...
);
tmp_t = toc(tmp_t); if (verbose); disp(sprintf(' %% pm_template_2: %0.6fs',tmp_t)); end;
S_p123_pole_k_p_wkS__ = reshape(S_p123_pole_k_p_wkS__,[n_w_max*n_k_p_r,n_S]);
S_p123_pole_k_q_wkS__ = zeros(n_w_sum,n_S);
for nS=0:n_S-1;
S_p123_pole_k_q_wkS__(:,1+nS) = interp_p_to_q(n_k_p_r,n_w_,n_w_sum,S_p123_pole_k_p_wkS__(:,1+nS));
end;%for nS=0:n_S-1;
%%%%%%%%;
