...
if ~isfield(parameter,'n_source_gaussian'); parameter.n_source_gaussian = 8; end;
...
if flag_disp;
n_contour = 32;
N_x_c_reco_lim_ = max(real(tmp_N_x_c_reco_),[],'all')*[-1,+1];
...
subplot_{1+np} = subplot(p_row,p_col,1+np);np=np+1; imagesc_p(n_k_p_r,k_p_r_/k_p_r_max,n_w_,n_w_sum,real(tmp_R_x_p_form_));axis image;axisnotick;
subplot_{1+np} = subplot(p_row,p_col,1+np);np=np+1; imagesc_p(n_k_p_r,k_p_r_,n_w_,n_w_sum,real(tmp_N_k_p_form_));axis image;axisnotick;
...
subplot_{1+np} = subplot(p_row,p_col,1+np);np=np+1; imagesc_p(n_k_p_r,k_p_r_/k_p_r_max,n_w_,n_w_sum,real(tmp_R_x_p_pre_));axis image;axisnotick;
subplot_{1+np} = subplot(p_row,p_col,1+np);np=np+1; imagesc_p(n_k_p_r,k_p_r_,n_w_,n_w_sum,real(tmp_O_k_p_pre_));axis image;axisnotick;
