...
quad_k_c_qd__ = [ quad_k_c_0_shell_ , quad_k_c_1_shell_ , quad_k_c_2_shell_ ];
%%%%;
Ylm__ = get_Ylm__(1+l_max,0:l_max,quad_n_shell,quad_azimu_b_shell_,quad_polar_a_shell_);
Ylm_yq__ = zeros(n_lm,quad_n_shell);
nml=0;
for l_val=0:l_max;
...
Ylm_w_yq__ = Ylm_yq__ * sparse(1:quad_n_shell,1:quad_n_shell,quad_weight_shell_,quad_n_shell,quad_n_shell);
%%%%;
[ ...
 data_k_p_polar_a__ ...
,data_k_p_azimu_b__ ...
...
quad_from_data_qwM__ = sparse(1+index_quad_from_data_,1:n_w*n_M,1,quad_n_shell,n_w*n_M);
...
n_k_sphere = 0;
quad_n_shell_csum_ = zeros(1+n_k_p_r,1);
a_k_p_kq__ = cell(n_k_p_r,1);
for nk_p_r=0:n_k_p_r-1;
n_lm = n_lm_(1+nk_p_r);
n_w = n_w_(1+nk_p_r);
...
a_k_p_kq__{1+nk_p_r} = quad_from_data_T_CTF_normalized_q_;
n_k_sphere = n_k_sphere + quad_n_shell;
quad_n_shell_csum_(1+nk_p_r+1) = quad_n_shell_csum_(1+nk_p_r+0) + quad_n_shell;
a_k_Y_(1+index_Y_) = conj(Ylm_w_yq__)*quad_from_data_T_CTF_normalized_q_;
%%%%%%%%;
end;%for nk_p_r=0:n_k_p_r-1;

a_k_p_ = zeros(n_k_sphere,1);
for nk_p_r=0:n_k_p_r-1;
tmp_index_ = [quad_n_shell_csum_(1+nk_p_r):quad_n_shell_csum_(1+nk_p_r+1)-1];
a_k_p_(1+tmp_index_) = a_k_p_kq__{1+nk_p_r};
end;%for nk_p_r=0:n_k_p_r-1;
