...
%%%%%%%%;
% SS_xxz___ <-- < rotation of CTF*S , rotation of CTF*S > ;
%%%%%%%%;
%%%%;
...
%%%%;
nx0 = 0; nx1 = 0;
SS_D = sum(CTF_S_flip_x_c_xx__(1+nx0+[0:n_x_u_pack-1],1+nx1+[0:n_x_u_pack-1]).*CTF_S_flip_x_c_xx__,'all');
SS_C = SS_xxz___(1+nx0+floor(n_x_u_pack/2)-1,1+nx1+floor(n_x_u_pack/2)-1,1+0);
disp(sprintf(' %% SS_D vs SS_C: %0.16f',fnorm(SS_D-SS_C)/fnorm(SS_D)));
%%%%;
nx0 = 3; nx1 = -7;
tmp_n2 = floor(n_x_u_pack/2);
SS_D = sum(CTF_S_flip_pad_x_c_xx__(n_x_u_pack+1+nx0+[0:n_x_u_pack-1],n_x_u_pack+1+nx1+[0:n_x_u_pack-1]).*CTF_S_flip_x_c_xx__,'all');
SS_C = SS_xxz___(1+nx0+floor(n_x_u_pack/2)-1,1+nx1+floor(n_x_u_pack/2)-1,1+0);
disp(sprintf(' %% SS_D vs SS_C: %0.16f',fnorm(SS_D-SS_C)/fnorm(SS_D)));
%%%%;
nx0 = -13; nx1 = +17;
...
%%%%%%%%;
% Compare R1S1_B_xxz___ to R1S1_C_xxz___ via CoC2 and CoC3 ;
%%%%%%%%;
...
OS_xxz___ = R1S1_pad_B_xxz___(n_x_u_pack + nx0-(tmp_n2-1-dnx)+[1:n_x_u_pack],n_x_u_pack + nx1-(tmp_n2-1-dnx)+[1:n_x_u_pack],:);
%CoC2 = corr( reshape(SS_xxz___(:,:,1+0),[n_x_u_pack^2,1]) , reshape(OS_xxz___(:,:,1+0),[n_x_u_pack^2,1]) );
%CoC3 = corr( reshape(SS_xxz___,[n_x_u_pack^2*n_gamma_z,1]) , reshape(OS_xxz___,[n_x_u_pack^2*n_gamma_z,1]) );
CoC2 = dot( reshape(SS_xxz___(:,:,1+0),[n_x_u_pack^2,1]) , reshape(OS_xxz___(:,:,1+0),[n_x_u_pack^2,1]) );
CoC3 = dot( reshape(SS_xxz___,[n_x_u_pack^2*n_gamma_z,1]) , reshape(OS_xxz___,[n_x_u_pack^2*n_gamma_z,1]) );
CoC2_B_xx__(1+nx0,1+nx1) = CoC2;
CoC3_B_xx__(1+nx0,1+nx1) = CoC3;
%%%%;
OS_xxz___ = R1S1_pad_C_xxz___(n_x_u_pack + nx0-(tmp_n2-1-dnx)+[1:n_x_u_pack],n_x_u_pack + nx1-(tmp_n2-1-dnx)+[1:n_x_u_pack],:);
%CoC2 = corr( reshape(SS_xxz___(:,:,1+0),[n_x_u_pack^2,1]) , reshape(OS_xxz___(:,:,1+0),[n_x_u_pack^2,1]) );
%CoC3 = corr( reshape(SS_xxz___,[n_x_u_pack^2*n_gamma_z,1]) , reshape(OS_xxz___,[n_x_u_pack^2*n_gamma_z,1]) );
CoC2 = dot( reshape(SS_xxz___(:,:,1+0),[n_x_u_pack^2,1]) , reshape(OS_xxz___(:,:,1+0),[n_x_u_pack^2,1]) );
CoC3 = dot( reshape(SS_xxz___,[n_x_u_pack^2*n_gamma_z,1]) , reshape(OS_xxz___,[n_x_u_pack^2*n_gamma_z,1]) );
...
%%%%%%%%;
K2_xx__ = SS_xxz___(:,:,1+0)*n_x_u_pack*n_x_u_pack/(n_x_u_pack*n_x_u_pack*dx_0*dx_1);
