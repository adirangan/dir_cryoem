...
deltafunct_lim_ = prctile(a_deltafunct_I_I_k_p_quad_,95)*[-1,+1];
...
title('a_deltafunct_I_I_k_p_quad_','Interpreter','none');
subplot(p_row,p_col,1+np);np=np+1;
deltafunct_llim_ = [-3,log10(prctile(a_deltafunct_I_I_k_p_quad_,99.95))];
flag_2d_vs_3d = 0;
k_p_r_max = 1;
imagesc_polar_a_azimu_b_0( ...
 polar_a_shell_ ... 
,azimu_b_shell_ ... 
,log10(abs(a_deltafunct_I_I_k_p_quad_)) ... 
,deltafunct_llim_ ... 
,colormap_beach() ... 
,flag_2d_vs_3d ...
,k_p_r_max ...
...
axisnotick3d; axis vis3d;
title('log10(a_deltafunct_I_I_k_p_quad_)','Interpreter','none');
subplot(p_row,p_col,1+np);np=np+1;
plot(polar_a_shell_,log10(abs(a_deltafunct_I_I_k_p_quad_)),'.');
xlabel('polar_a_shell_','Interpreter','none');
ylabel('log10(abs(a_deltafunct_I_I_k_p_quad_))','Interpreter','none');
grid on;
end;%if (flag_disp);
%%%%%%%%;
% Now visualize the effects of mollifying the deltafunct. ;
%%%%%%%%;
flag_calc=1;
if flag_calc;
sigma_kernel_max = 0.25;
n_sigma_kernel = 12; sigma_kernel_ = transpose(linspace(0,sigma_kernel_max,n_sigma_kernel));
figure(1+nf);nf=nf+1;clf;figmed;
p_row = 3; p_col = ceil(n_sigma_kernel/p_row); np=0;
for nsigma_kernel=0:n_sigma_kernel-1;
sigma_kernel = sigma_kernel_(1+nsigma_kernel);
% typical mollification: exp(-l*(1+l)*0.5*sigma_kernel^2)*sqrt(2l+1)*sqrt(4*pi);
tmp_a_mollify_k_Y_lm_ = exp(-Y_l_val_.*(Y_l_val_+1).*0.5.*sigma_kernel^2);
tmp_a_deltafunct_mollify_k_Y_lm_ = tmp_a_mollify_k_Y_lm_.*sqrt(2*pi).*sqrt(4*pi).*sqrt(1+2*Y_l_val_).*(Y_m_val_==0);
[ ...
 tmp_a_deltafunct_mollify_k_p_quad_ ...
] = ...
convert_spharm_to_k_p_4( ...
 flag_verbose ...
,n_shell ...
...
,weight_3d_k_p_r_ ...
,l_max ...
,tmp_a_deltafunct_mollify_k_Y_lm_ ...
,Ylm_uklma___ ...
,k_p_azimu_b_sub_uka__ ...
,k_p_polar_a_sub_uka__ ...
,l_max_uk_ ...
...
subplot(p_row,p_col,1+np);np=np+1;
plot(polar_a_shell_,log10(abs(tmp_a_deltafunct_mollify_k_p_quad_)),'.');
xlabel('polar_a_shell_','Interpreter','none');
ylabel('log10(abs(a))','Interpreter','none');
title(sprintf('$\\sigma$ %0.2f $[l(l+1)\\sigma^2/2]$ %0.2f',sigma_kernel,l_max*(l_max+1)*0.5*sigma_kernel^2),'Interpreter','latex');
grid on;
end;%for nsigma_kernel=0:n_sigma_kernel-1;
end;%if flag_calc;
%%%%%%%%;
% Now visualize the error associated with: ;
% deltafunct_k_Y --> mollify --> convert to k_p --> crop at 1e-6 --> convert to k_Y --> demollify. ;
%%%%%%%%;
flag_calc=1;
if flag_calc;
tmp_a_deltafunct_k_Y_lm_ = sqrt(2*pi).*sqrt(4*pi).*sqrt(1+2*Y_l_val_).*(Y_m_val_==0);
tolerance_k_p = 1e-3;
sigma_kernel_max = 0.25;
n_sigma_kernel = 12; sigma_kernel_ = transpose(linspace(0,sigma_kernel_max,n_sigma_kernel));
figure(1+nf);nf=nf+1;clf;figmed;
...
% typical mollification: exp(-l*(1+l)*0.5*sigma_kernel^2)*sqrt(2l+1)*sqrt(4*pi);
tmp_a_mollify_k_Y_lm_ = exp(-Y_l_val_.*(Y_l_val_+1).*0.5.*sigma_kernel^2);
tmp_a_restore_k_Y_lm_ = exp(+Y_l_val_.*(Y_l_val_+1).*0.5.*sigma_kernel^2);
tmp_a_deltafunct_mollify_k_Y_lm_ = tmp_a_mollify_k_Y_lm_.*tmp_a_deltafunct_k_Y_lm_;
[ ...
 tmp_a_deltafunct_mollify_k_p_quad_ ...
] = ...
convert_spharm_to_k_p_4( ...
...
tmp_a_deltafunct_crop_k_p_quad_ = (abs(tmp_a_deltafunct_mollify_k_p_quad_)>tolerance_k_p);
tmp_f_crop = sum(tmp_a_deltafunct_crop_k_p_quad_)/numel(tmp_a_deltafunct_crop_k_p_quad_);
tmp_a_deltafunct_mollify_crop_k_p_quad_ = tmp_a_deltafunct_mollify_k_p_quad_.*tmp_a_deltafunct_crop_k_p_quad_;
[ ...
 tmp_a_deltafunct_mollify_crop_k_Y_quad_ ...
] = ...
...
,weight_3d_k_p_r_ ...
,l_max ...
,tmp_a_deltafunct_mollify_crop_k_p_quad_ ...
,Ylm_uklma___ ...
,k_p_azimu_b_sub_uka__ ...
,k_p_polar_a_sub_uka__ ...
,l_max_uk_ ...
...
tmp_a_deltafunct_mollify_crop_restore_k_Y_quad_ = tmp_a_deltafunct_mollify_crop_k_Y_quad_ .* tmp_a_restore_k_Y_lm_;
subplot(p_row,p_col,1+np);np=np+1;
plot(Y_l_val_,log10(abs(tmp_a_deltafunct_k_Y_lm_ - tmp_a_deltafunct_mollify_crop_restore_k_Y_quad_)),'.');
xlabel('Y_l_val_','Interpreter','none');
ylabel('log10(abs(a_k_Y_ error))','Interpreter','none');
title(sprintf('$\\sigma$ %0.2f $\\mu$ %0.2f $f$ %0.2f',sigma_kernel,l_max*(l_max+1)*0.5*sigma_kernel^2,tmp_f_crop),'Interpreter','latex');
grid on;
end;%for nsigma_kernel=0:n_sigma_kernel-1;
end;%if flag_calc;

%%%%%%%%;
sigma_kernel_crit = 0.0758;
%%%%%%%%;
flag_calc=0;
if flag_calc;
%%%%%%%%;
...
u_sml_norm_ = u_sml_ / max(1e-12,chebfun_kernel_crit_avg) ;
%%%%;
...
n% factor_scaling_min = smallest scaling-factor. ;
%%%%
...
%for l_val=0:l_max;
%nc_use = max(0,min(n_c_use-1,floor(n_c_use*l_val/l_max)));
%plot(log(1.0-z_),chebleg_d_{1+l_val}(z_),'-','Color',c_use__(1+nc_use,:));
%end;%for l_val=0:l_max;
plot(log(1.0-z_),chebfun_kernel_crit_(z_)/chebfun_kernel_crit_avg,'-','Color','k','LineWidth',linewidth_use);
plot(log(1-0-z_),chebfun_kernel_qpro_(z_)/chebfun_kernel_qpro_avg,'-','Color',0.85*[1,1,1],'LineWidth',linewidth_use);
hold off;
...
%%%%;
scali_norm_ = kappa_norm_;
%scali_norm_ = u_sml_norm_;
kernel_qpro_k_Y_form_ = zeros(n_lm,1);
kernel_qpro_k_Y_form_(1+l_val_.^2 + l_val_) = scali_norm_;
%%%%;
deconvolve_lm_ = (sqrt(4*pi)*sqrt(1+2*Y_l_val_))./max(1e-12,scali_norm_(1+Y_l_val_));
if flag_disp;
figure(1+nf);nf=nf+1;clf;figsml;
linewidth_use = 3;
...
[tmp_ij_] = knnsearch(qref_k_c_qd__,reshape(tmp_I_pole_,[1,3]),'K',n_nearest);
tmp_index_keep_ = tmp_ij_-1; tmp_index_drop_ = setdiff(0:qref_n_shell-1,tmp_index_keep_);
...
[tmp_ij_] = knnsearch(qref_k_c_qd__,reshape(tmp_R_pole_,[1,3]),'K',n_nearest);
tmp_index_keep_ = tmp_ij_-1; tmp_index_drop_ = setdiff(0:qref_n_shell-1,tmp_index_keep_);
...
%%%%%%%%;
% Now we test the first-derivative. ;
%%%%%%%%;
tmp_I__ = eye(3,3);
tmp_I_pole_ = tmp_I__*[0;0;1];
tmp_I_d2_ = (qref_k_c_0_shell_ - tmp_I_pole_(1+0)).^2 + (qref_k_c_1_shell_ - tmp_I_pole_(1+1)).^2 + (qref_k_c_2_shell_ - tmp_I_pole_(1+2)).^2 ;
...
%%%%%%%%;
% test the same reconstruction using kappa_qpro_0. ;
%%%%%%%%;
parameter = struct('type','parameter'); parameter.flag_verbose = 1; parameter.flag_disp = 1;
parameter.kernel_qpro_polar_a_pole_north = 3*pi/24;
...
%%%%%%%%;
% Now try to find a reconstruction that is accurate out to l_max = 49. ;
%%%%%%%%;
l_ext = ceil(1.25*l_max);
parameter = struct('type','parameter'); parameter.flag_verbose = 1; parameter.flag_disp = 1;
