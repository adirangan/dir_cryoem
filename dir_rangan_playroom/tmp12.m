...
if (flag_verbose>0); disp(sprintf(' %% a_deltafunct_I_I_k_Y_lm_ vs a_deconvolve_I_I_k_Y_lm_: %0.16f',fnorm(a_deltafunct_I_I_k_Y_lm_ - a_deconvolve_I_I_crop_k_Y_lm_)/fnorm(a_deltafunct_I_I_k_Y_lm_))); end;
...
subplot(p_row,p_col,1+np);np=np+1;
plot(Y_l_val_,abs(a_deltafunct_I_R_k_Y_lm_-a_deconvolve_I_R_crop_k_Y_lm_),'.');
xlabel('Y_l_val_','Interpreter','none');
ylabel('abs(delta-decon)','Interpreter','none');
grid on;
end;%if (flag_disp);
