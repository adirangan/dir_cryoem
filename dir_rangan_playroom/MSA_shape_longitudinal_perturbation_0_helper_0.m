...
%%%%%%%%%%%%%%%%;
drawnow(); %error('stop');
fname_fig_pre = sprintf('%s/MSA_shape_demonstration_ns%.2d_%s',dir_manuscript_jpg,n_source_gaussian,str_ntype);
...
axisnotick;
drawnow(); %error('stop');
fname_fig_pre = sprintf('%s/MSA_shape_demonstration_ns%.2d_disc',dir_manuscript_jpg,n_source_gaussian);
fname_fig_jpg = sprintf('%s.jpg',fname_fig_pre);
if ( flag_replot | ~exist(fname_fig_jpg,'file') );
sgtitle(fname_fig_pre,'Interpreter','none');
disp(sprintf(' %% writing %s',fname_fig_pre));
