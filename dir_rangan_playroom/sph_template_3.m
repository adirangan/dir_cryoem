...
% a_k_Y_lmk__ = complex array of size (n_lm,n_k_p_r). ;
...
if (nargin<1+na); a_k_Y_lmk__=[]; end; na=na+1;
...
if (isempty(a_k_Y_lmk__)); a_k_Y_lmk__ = zeros(n_lm,1); end;
if (verbose>2); fprintf(1,' %% \t\n'); darray_printf_margin(real(a_k_Y_lmk__),n_lm,n_k_p_r,' %% a_k_Y_lmk_real___: '); end;
if (verbose>2); fprintf(1,' %% \t\n'); darray_printf_margin(imag(a_k_Y_lmk__),n_lm,n_k_p_r,' %% a_k_Y_lmk_imag___: '); end;
...
%%%%%%%%;
% Now we unroll the a_k_Y_lmk__. ;
%%%%%%%%;
tmp_t = tic();
a_k_Y_lmk___ = zeros(1+l_max_max,n_m_max,n_k_p_r);
for nk_p_r=0:n_k_p_r-1;
tmp_a_k_Y_lm_ = a_k_Y_lmk__(:,1+nk_p_r);
tmp_a_k_Y_lm__ = zeros(1+l_max_max,n_m_max);
l_max = l_max_(1+nk_p_r);
na=0;
for l_val=0:l_max;
