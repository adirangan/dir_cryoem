...
%%%%%%%%;
% Here we attempt 'limiting' the interpolation grid (i.e., local nodes) by the poles (i.e., boundaries of polar_a_). ;
%%%%%%%%;
...
str_thisfunction = 'shell_k_p_scatter_from_tensor_interpolate_n_7';
...
%%%%%%%%;
n_scat = 1024*4;
polar_a_scat_ = 1*pi*rand(n_scat,1) - 0*pi; polar_a_scat_(1+0) = 0; polar_a_scat_(1+1) = pi;
azimu_b_scat_ = 2*pi*rand(n_scat,1) - 0*pi;
l_max = 49;
...
for m_val=m_sub_;
tmp_Y_ = scatter_from_tensor_sba__*transpose(Ylm_grid__{1+l_val}(1+l_val+m_val,:));
Ylm_pint__{1+l_val}(1+l_val+m_val,:) = tmp_Y_;
Ylm_pint_ = [Ylm_pint_;tmp_Y_(:)];
tmp_Y_ = Ylm_scat__{1+l_val}(1+l_val+m_val,:);
Ylm_scat_ = [Ylm_scat_;tmp_Y_(:)];
...
subplot(1,2,2);
plot(Ylm_scat_,Ylm_pint_,'.');
xlabel('Ylm_scat_','Interpreter','none');
ylabel('Ylm_pint_','Interpreter','none');
title(sprintf('l_max %d',l_max),'Interpreter','none');
end;%if flag_disp;
%%%%;
...
polar_a_scatter_rescale_ = max(0,min(n_polar_a-1,polar_a_scatter_periodize_rescale_/spacing_polar_a)); %<-- confine to box if not already confined. ;
%%%%;
if (flag_verbose>0);
...
polar_a_scatter_start_ = max(0,min(n_polar_a-1-n_order,floor(polar_a_scatter_rescale_)-n_part_order));
...
tmp_index_col_polar_a_x_(1+tmp_index_) = 0; 
tmp_index_ = efind(tmp_index_col_polar_a_x_> (n_polar_a-1)); 
tmp_index_col_polar_a_x_(1+tmp_index_) = n_polar_a-1;
%%%%;
...
tmp_azimu_b_pos_xx__ = periodize(tmp_azimu_b_pos_xx__,0,2*pi);
...
tmp_col_azimu_b_pos_xx__ = periodize(tmp_col_azimu_b_xx__,0,n_azimu_b);
...
index_col_azimu_b_xs__(1+tmp_index_) = index_col_azimu_b_xs__(1+tmp_index_) + n_azimu_b;
%%%%%%%%;
% cap polar_a at poles. ;
%%%%%%%%;
tmp_index_ = efind(index_col_polar_a_xs__< 0        ); 
index_col_polar_a_xs__(1+tmp_index_) = 0; 
tmp_index_ = efind(index_col_polar_a_xs__> (n_polar_a-1)); 
index_col_polar_a_xs__(1+tmp_index_) = n_polar_a-1;
%%%%;
if (flag_verbose>0); disp(sprintf(' %% n_azimu_b %d: index_col_azimu_b_xs__ in [%.4d,%.4d]',n_azimu_b,min(index_col_azimu_b_xs__,[],'all'),max(index_col_azimu_b_xs__,[],'all'))); end;
if (flag_verbose>0); disp(sprintf(' %% n_polar_a %d: index_col_polar_a_xs__ in [%.4d,%.4d]',n_polar_a,min(index_col_polar_a_xs__,[],'all'),max(index_col_polar_a_xs__,[],'all'))); end;
%%%%%%%%;
...
tmp_col_xx__ = periodize(tmp_col_azimu_b_xx__,0,n_azimu_b) + max(0,min(n_polar_a-1,tmp_col_polar_a_xx__))*n_azimu_b ;
