%%%%%%%%;
% This is basically the same as test_basis_0.m ;
% but with a slightly different choice of test-function. ;
%%%%%%%%;
clear;
...
%%%%%%%%;
k_p_r_max = 48/(2*pi)*f_res; k_eq_d = 1.0/(2*pi);
[ ...
 n_k_p_r ...
,k_p_r_ ...
,weight_3d_k_p_r_ ...
...
if (verbose); disp(sprintf(' %% sum(weight_3d_k_p_r_)*4*pi - volume: %0.16f',sum(weight_3d_k_p_r_)*4*pi - 4/3*pi*k_p_r_max^3)); end;
%%%%%%%%;
...
if (verbose); disp(sprintf(' %% sum(weight_2d_k_p_r_) - area %0.16f',sum(weight_2d_k_p_r_) - pi*k_p_r_max^2)); end;
...
if (verbose); disp(sprintf(' %% M_x_c_l2 vs N_x_c_l2: %0.16f',fnorm(M_x_c_l2 - N_x_c_l2)/fnorm(M_x_c_l2))); end;
M_k_p_l2 = sum(abs(M_k_p_).^2 .* weight_2d_k_all_,'all')*(4*pi^2);
if (verbose); disp(sprintf(' %% M_x_c_l2 vs M_k_p_l2: %0.16f',fnorm(M_x_c_l2 - M_k_p_l2)/fnorm(M_x_c_l2))); end;
M_k_q_ = interp_p_to_q(n_k_p_r,n_w_,n_w_sum,M_k_p_);
M_k_q_l2 = sum(abs(M_k_q_).^2 .* weight_2d_k_all_,'all')*(4*pi^2);
if (verbose); disp(sprintf(' %% M_x_c_l2 vs M_k_q_l2: %0.16f',fnorm(M_x_c_l2 - M_k_q_l2)/fnorm(M_x_c_l2))); end;
N_k_p_ = interp_q_to_p(n_k_p_r,n_w_,n_w_sum,M_k_q_);
N_k_p_l2 = sum(abs(N_k_p_).^2 .* weight_2d_k_all_,'all')*(4*pi^2);
if (verbose); disp(sprintf(' %% M_x_c_l2 vs N_k_p_l2: %0.16f',fnorm(M_x_c_l2 - N_k_p_l2)/fnorm(M_x_c_l2))); end;
end;%if nl==0;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
if nl==1;
...
imagesc_c(n_x_M_u,x_u_0_,n_x_M_u,x_u_1_,real(M_x_c_),clim_,colormap_pm.^gamma_use); axis image;
...
imagesc_p(n_k_p_r,k_p_r_,n_w_,n_w_sum,real(M_k_p_),clim_,colormap_pm.^gamma_use); axis image;
...
imagesc_q(n_k_p_r,k_p_r_,n_w_,n_w_sum,real(M_k_q_),clim_,colormap_pm.^gamma_use); axisnotick;
...
%%%%;
fname_fig_pre = sprintf('/%s/rangan/dir_cryoem/dir_CryoBIFE_MD/dir_CryoLike_fig/pm_fig_image_basis_nl%d_FIGA',string_root,nl);
