c$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
      subroutine test_innerproduct_svdread_batch_ver1(verbose
...
c$$$                            allows test_innerproduct_svdread_batch_ver1.f
...
c$$$                                               + gamma_z
c$$$                          - See test_innerproduct_svdread_batch_ver1_dr.f for
c$$$                            an example.
c$$$
c$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
      implicit none
...
c$$$      parameters for timing
      ...
      if (verbose.gt.0) then
         write(6,'(A)')
     $        '[entering test_innerproduct_svdread_batch_ver1]'
      end if      
      if (verbose.gt.0) then
     ...
     $        ,' too large in test_innerproduct_svdread_batch_ver1'
      end if
     ...
     $        ,' too small in test_innerproduct_svdread_batch_ver1'
      end if
      if (.false.) then
     ...
     $        ' Allocating array Z_q_ to hold the innerproucts'
      ...
      do ns=0,n_S-1
         call cp1_c16(ld_S,S_p_(ns*ld_S),S_p)
         call innerproduct_p(n_r,grid_p_,n_w_,n_A,S_p,S_p,C_S_(ns))
     ...
     $              ,svd_s_,svd_V_r_,n_r,grid_p_,n_w_,n_A,S_q ,M_q_
     $              ,Z_svds_)
               timing_toc_2 = second()
     ...
     $              ,n_r,grid_p_,n_w_,n_A,C_Z,S_p,M_q_,Z_q_)
               timing_toc_3 = second()
               timing_tot_3 = timing_tot_3 + (timing_toc_3 -
     $              timing_tic_3)
     ...
     $           ,n_gamma_z,Z_q_,ndx_max,ndy_max,ngz_max,C_Z_max)
            ...
            if (verbose.gt.2) then
               write(6,'(A,I0,A,I0,A,I2,1X,I2,1X,I2,A,2F8.3)')
     $              'M_q_(' , nm , ') match to S_q_(',ns
     ...
     $        'transforming M_p_,S_p_ to M_q_,S_q_: total time '
     $        ,timing_tot_1 ,'; time_per ',timing_tot_1/max(1,n_S*n_M)
         write(6,'(A,F8.5,A,F8.5)')
     ...
     $        ,'; time_per ',timing_tot_4/max(1,n_S*n_M)
      ...
      if (verbose.gt.0) then
         write(6,'(A)')
     $        '[finished test_innerproduct_svdread_batch_ver1]'
      end if
