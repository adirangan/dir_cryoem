...
snr_per_A = 0.0025; %<-- decrease to increase noise. ;
f_rand_0in = 0.00;
n_M_factor_0in = 8; %<-- n_M = n_w_max * n_M_factor_0in. ;
k_p_r_max_factor_0in = 1; %<-- k_p_r_max = 48 * k_p_r_max_factor_0in. ;
...
subplot(1,2,1);imagesc_c_mask(n_x_u,x_u_0_,n_x_u,x_u_1_,a_rho_x_u_,alim_); axis image; axisnotick;
subplot(1,2,2);imagesc_c_mask(n_x_u,x_u_0_,n_x_u,x_u_1_,b_rho_x_u_,alim_); axis image; axisnotick;
...
subplot(1,2,1); imagesc_c_mask(n_x_u,x_u_0_,n_x_u,x_u_1_,a_x_u_,alim_); title('a'); axis image; axisnotick;
subplot(1,2,2); imagesc_c_mask(n_x_u,x_u_0_,n_x_u,x_u_1_,b_rho_x_u_,alim_); title('b'); axis image; axisnotick;
...
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1+nf);nf=nf+1;clf;figmed;figbeach();
subplot(1,3,1);imagesc_c_mask(n_x_u,x_u_0_,n_x_u,x_u_1_,a_x_u_base_,[0,1]); axis image; axisnotick;
title('a_x_u_base_','Interpreter','none');
subplot(1,3,2);imagesc_c_mask(n_x_u,x_u_0_,n_x_u,x_u_1_,real(a_x_u_reco_),[0,1]); axis image; axisnotick;
title('a_x_u_reco_','Interpreter','none');
subplot(1,3,3);imagesc_c_mask(n_x_u,x_u_0_,n_x_u,x_u_1_,real(a_x_u_flip_),[0,1]); axis image; axisnotick;
...
if (snr_per_A>0); sigma_noise_per_Pixel = sqrt(a_k_p_quad_l2)/max(1e-12,snr_per_A)/sqrt(n_x_u)/sqrt(Pixel_A); end;
...
%%%%;
b_x_u_temp_ = a_x_u_base_ + sigma_noise_per_Pixel*randn(n_x_u,n_x_u);
b_k_p_temp_ = ...
interp_x_c_to_k_p_xxnufft( ...
 n_x_u ...
,diameter_x_c ...
