      recursive subroutine tesselation_find_depth(diameter_min
     $     ,nlevel__in,v0_i,v1_i,v2_i,parity,n_L,L_,nlevel_out)
      implicit none
      integer *4 verbose
      data verbose / 0 /
      real *8 diameter_min
      integer *4 nlevel__in,n_L,nlevel_out
      ...
      real *8 aa,a0,a1,a2
      logical isin
      integer *4 nlevel_child_0
      integer *4 nlevel_child_1
      integer *4 nlevel_child_2
      integer *4 nlevel_child_3
      integer *4, allocatable :: nlevel_child_(:)
      integer *4 max_i4_f
     ...
     $        '[entering tesselation_find_depth]: ',nlevel__in
     ...
     $        ,'; n_L_sub: ',n_L_sub
         do nL=0,n_L_sub-1
            write(6,'(A,A,I0,3F8.4)') trim(prefix),'L_sub: ',nL,
     $           L_sub_(0+3*nL), L_sub_(1+3*nL), L_sub_(2+3*nL)
      ...
      if (n_L_sub.gt.1 .and. diameter.gt.diameter_min) then
         ...
         call tesselation_find_depth(diameter_min,nlevel__in+1,v0_k,m2
     $        ,m1,parity,n_L_sub,L_sub_,nlevel_child_0)
         call tesselation_find_depth(diameter_min,nlevel__in+1,v1_k,m0
     $        ,m2,parity,n_L_sub,L_sub_,nlevel_child_1)
         call tesselation_find_depth(diameter_min,nlevel__in+1,v2_k,m1
     $        ,m0,parity,n_L_sub,L_sub_,nlevel_child_2)
         call tesselation_find_depth(diameter_min,nlevel__in+1,m0,m1,m2,
     $        .not. parity,n_L_sub,L_sub_,nlevel_child_3)
         allocate(nlevel_child_(0:3))
         nlevel_child_(0) = nlevel_child_0
         nlevel_child_(1) = nlevel_child_1
         nlevel_child_(2) = nlevel_child_2
         nlevel_child_(3) = nlevel_child_3
         if (verbose.gt.1) then
            write(6,'(A,A,I0,1X,I0,1X,I0,1X,I0)') trim(prefix) ,
     $           'nlevel_child_: ' , nlevel_child_0 , nlevel_child_1 ,
     $           nlevel_child_2 , nlevel_child_3
         end if ! if (verbose.gt.1) then
         nlevel_out = max_i4_f(4,nlevel_child_)
