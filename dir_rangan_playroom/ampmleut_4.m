...
% index_nCTF_from_nM_: integer array of size n_M. index_nCTF_from_nM_(1+nM) is the CTF_index used for image M_k_p__(:,1+nM). ;
...
verbose=2;
if (verbose); disp(sprintf(' %% [entering ampmleut_4]')); end;
...
if (~isempty(VSCTF_Mc__)); VSCTF_Mc__ = VSCTF_Mc__(:,1+n_CTF_rank_use); end; %<-- replace original. ;
n_CTF_rank = n_CTF_rank_use; %<-- replace original. ;
...
if (n_CTF_rank<=0 | isempty(index_nCTF_from_nM_));
n_CTF_rank = 1;
...
end;%if (n_CTF_rank<=0 | isempty(index_nCTF_from_nM_));
...
%%%%%%%%;
if ( (~flag_alternate_MS_vs_SM) & (flag_MS_vs_SM==1) & (order_limit_MS>-1) & (niteration<n_iteration/2) );
for nneighborhood=0:n_neighborhood-1;
a_UCTF_UX_Y_0qbp_yncx___(:,:,1+nneighborhood) = a_UCTF_UX_Y_0qbp_yncx___(:,:,1+nneighborhood).*repmat(Y_lm_cut_,[1,n_CTF_rank]);
end;%for nneighborhood=0:n_neighborhood-1;
end;%if ( (~flag_alternate_MS_vs_SM) & (flag_MS_vs_SM==1) & (order_limit_MS>-1) & (niteration<n_iteration/2) );
%%%%%%%%;
...
,reshape(a_UCTF_UX_Y_0qbp_yncx___(:,:,1:n_neighborhood),[pm_n_lm_max,pm_n_UX_rank*n_CTF_rank*n_neighborhood]) ...
...
if (verbose); disp(sprintf(' %% [finished ampmleut_4]')); end;
