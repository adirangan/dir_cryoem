function [X_best,polar_a_best,azimu_b_best,gamma_z_best,delta_best_,a_k_Y_best_,b_k_Y_best_] = test_spharm_15(n_k_p_r,k_p_r_,k_p_r_max,weight_k_p_r_,N_wavelength,l_max_,a_k_Y_,b_k_Y_);
...
b_k_Y_ = modsph_B_ori_;
%%%%%%%%;
if (verbose); disp(sprintf(' %% translating to form b_k_Y_bake_ = T_(delta_bake_) * b_k_Y_')); end;
z_target = N_wavelength; delta_p_r_max = z_target/(2*pi*k_p_r_max);
delta_bake_ = delta_p_r_max*[+0.1,-0.4,+0.2]; assert(fnorm(delta_bake_)<delta_p_r_max);
if (verbose); disp(sprintf(' %% delta_bake_: %+0.6f , %+0.6f , %+0.6f',delta_bake_)); end;
l_max_max = max(l_max_); dWtdkd__l_max_max = 2*l_max_max;
dWtdkd__ = dwignertdkd__(dWtdkd__l_max_max);
delta_bake_p_r = fnorm(delta_bake_);
Wt___ = expm_dwignertdkd__(dWtdkd__,n_k_p_r,k_p_r_,l_max_,delta_bake_p_r);
Wt_ = wignert_ODE_0(dWtdkd__,Wt___,n_k_p_r,k_p_r_,l_max_,delta_bake_p_r);
delta_z_c_ = transpose(delta_bake_);
...
b_k_Y_bake_ = tmp_Y_form_;
b_k_Y_ = b_k_Y_bake_;
%%%%%%%%;
if (verbose); disp(sprintf(' %% rotating to form a_k_Y_bake_ = R_(euler_bake_) * a_k_Y_:')); end;
azimu_b_bake = +pi/4; polar_a_bake = -pi/3; gamma_z_bake = +pi/5;
euler_bake_ = [+azimu_b_bake,+polar_a_bake,+gamma_z_bake];
if (verbose); disp(sprintf(' %% euler_bake_: %+0.6f , %+0.6f , %+0.6f',euler_bake_)); end;
W_beta_bake__ = wignerd_b(l_max_max,euler_bake_(1+1));
tmp_Y_form_ = a_k_Y_;
tmp_Y_form_ = rotate_spharm_to_spharm_2(0,W_beta_bake__,n_k_p_r,k_p_r_,l_max_,tmp_Y_form_,euler_bake_);
a_k_Y_bake_ = tmp_Y_form_;
a_k_Y_ = a_k_Y_bake_;
%%%%%%%%;
end;%if nargin<8;
...
if (verbose>1); disp(sprintf(' %% Now step through the polar_a_, translating b_k_Y_ by each of the delta_node_')); end;
for npolar_a=1:n_polar_a;
polar_a = polar_a_(npolar_a);
if (mod(npolar_a,8)==0); if (verbose); disp(sprintf(' %% npolar_a %d/%d polar_a %0.2f',npolar_a,n_polar_a,polar_a)); end; end;
