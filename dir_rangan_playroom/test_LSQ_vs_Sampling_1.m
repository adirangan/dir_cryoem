...
tmp = 1/(1*K_hi*sqrt(2*pi));
for nk=1:2*K_hi+1;
k_val = K_al_(nk);
r__k(nk) = tmp*(randn(1) + i*randn(1)) / max(1,exp(gamma*abs(k_val))) ;
...
xlabel('x__s','Interpreter','none'); ylabel('r'); xlim([-pi,pi]); title('rho');
subplot(2,2,2);plot(x__s,real(G_al*r__k),'r-',x__s,imag(G_al*r__k),'b-');
xlabel('x__s','Interpreter','none'); ylabel('r'); xlim([-pi,pi]); title('int rho');
subplot(2,2,[3,4]);
...
for nk=1:2*K_hi+1;
k_val = K_al_(nk);
y__k(nk) = randn(1) + i*randn(1) / max(1,exp(gamma*abs(k_val))) ;
...
c_i1 = 2*eta*ctranspose(F__n_lo)*y__b;
...
c_i2 = 2*eta*ctranspose(F__n_lo)*y__b;
...
c_i3 = 2*eta*ctranspose(F__n_lo)*y__b;
