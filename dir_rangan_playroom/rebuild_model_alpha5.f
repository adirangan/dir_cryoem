C***********************************************************************
      subroutine rebuild_model_alpha5(cslices,imagesize,nimages, alpha5
     $     ,icstart,nlats,itypep,xnodesr,ngridc,nlow,ncur, lmod
...
c     alpha5(3,*)    image parameters: 
c                          alpha5(1,*) is first Euler angle (alpha)
c                          alpha5(2,*) is second Euler angle (beta)
c                          alpha5(3,*) is third Euler angle  (gamma_z)
c                          alpha5(4,*) is x-offset of center (delta_x)
c                          alpha5(5,*) is y-offset of center (delta_y)
c
...
C***********************************************************************
      ...
      if (verbose.gt.0) then
         write(6,*) '[entering rebuild_model_alpha5]'
      end if
...
c
      do is = nlow,ncur
         ...
         call get_lsqdata_alpha5(cslices,imagesize,nimages,icstart(is),
     $        xnodesr(is),ngridc(is),alpha5,phiout,thetas,phis,nout)
...
c
     ...
     $     '[entering rebuild_model_alpha5]'
      ...
      subroutine get_lsqdata_alpha5(ximagesk,imagesize,nimages,icstart,
     $     xnodesr,ngridc,alpha5,phiout,thetas,phis,nout)
      implicit none
      integer verbose
      data verbose / 0 /
      ...
      complex *16 M_p_1_(0:ngridc-1)
      integer nw
      if (verbose.gt.0) write(6,*)
     $     '[entering get_lsqdata_alpha5]'
c
      allocate(xc(ngridc))
      allocate(yc(ngridc))
      allocate(zc(ngridc))
