...
tmp_error_20 = 0;
for nl=0:n_l;
tmp_error_01 = tmp_error_01 + fnorm(W0_{1+nl}-W1_{1+nl});
tmp_error_12 = tmp_error_12 + fnorm(W1_{1+nl}-W2_{1+nl});
tmp_error_20 = tmp_error_20 + fnorm(W2_{1+nl}-W0_{1+nl});
end;%for nl=0:n_l;
if (flag_verbose>0); disp(sprintf(' %% tmp_error_01: %0.16f',tmp_error_01)); end;
if (flag_verbose>0); disp(sprintf(' %% tmp_error_12: %0.16f',tmp_error_12)); end;
if (flag_verbose>0); disp(sprintf(' %% tmp_error_20: %0.16f',tmp_error_20)); end;
%%%%%%%%;
dbeta = pi*(1e-4);
[d0W_pos_] = dwignerdda_b(n_l,beta+(+1)*dbeta);
[d0W_mid_,d1W_mid_,d2W_mid_] = dwignerdda_b(n_l,beta+(+0)*dbeta);
[d0W3_mid_,~,~,d1W3_mid_,d2W3_mid_] = wignerd_c(n_l,beta+(+0)*dbeta);
[d0W_neg_] = dwignerdda_b(n_l,beta+(-1)*dbeta);
%%%%;
tmp_error_bvsc_d0 = 0;
tmp_error_bvsc_d1 = 0;
tmp_error_bvsc_d2 = 0;
for nl=0:n_l;
tmp_error_bvsc_d0 = tmp_error_bvsc_d0 + fnorm(d0W3_mid_{1+nl}-d0W_mid_{1+nl});
tmp_error_bvsc_d1 = tmp_error_bvsc_d1 + fnorm(d1W3_mid_{1+nl}-d1W_mid_{1+nl});
tmp_error_bvsc_d2 = tmp_error_bvsc_d2 + fnorm(d2W3_mid_{1+nl}-d2W_mid_{1+nl});
end;%for nl=0:n_l;
if (flag_verbose>0); disp(sprintf(' %% tmp_error_bvsc_d0: %0.16f',tmp_error_bvsc_d0)); end;
if (flag_verbose>0); disp(sprintf(' %% tmp_error_bvsc_d1: %0.16f',tmp_error_bvsc_d1)); end;
if (flag_verbose>0); disp(sprintf(' %% tmp_error_bvsc_d2: %0.16f',tmp_error_bvsc_d2)); end;
%%%%;
tmp_error_fvsd_d1 = 0;
tmp_error_fvsd_d2 = 0;
for nl=0:n_l;
...
tmp_error_fvsd_d1 = tmp_error_fvsd_d1 + fnorm(d1W_dif_{1+nl}-d1W_mid_{1+nl})/max(1e-12,fnorm(d1W_dif_{1+nl}));
tmp_error_fvsd_d2 = tmp_error_fvsd_d2 + fnorm(d2W_lap_{1+nl}-d2W_mid_{1+nl})/max(1e-12,fnorm(d2W_lap_{1+nl}));
end;%for nl=0:n_l;
if (flag_verbose>0); disp(sprintf(' %% tmp_error_fvsd_d1: %0.16f',tmp_error_fvsd_d1)); end;
if (flag_verbose>0); disp(sprintf(' %% tmp_error_fvsd_d2: %0.16f',tmp_error_fvsd_d2)); end;
...
%%%%%%%%%%%%%%%%;
if (flag_verbose); disp(sprintf(' %% setting W(%d,%d) <-- %f (using %s %f %f %f)',nl+nmp,nl+nmn,d0_tmp,str_tmp,W1,W2,W3)); end;
...
S__ = s_*transpose(s_);
d0W_{1+nl} = d0W_{1+nl}.*S__;
d1W_{1+nl} = d1W_{1+nl}.*S__;
d2W_{1+nl} = d2W_{1+nl}.*S__;
end;%for nl=1:n_l;
