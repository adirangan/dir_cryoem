function [b_] = rotate_spharm_to_spharm_0(verbose,n_k,k_,l_max_,a_,alpha_);
...
% alpha_ equals array of euler angles; [alpha, beta, gamma] ;
% ;
...
b_ = zeros(size(a_));
for nk=1:n_k;
l_max_max = l_max_(nk); n_lm = n_lm_(nk); ix_base = sum(n_lm_(1:nk-1));
a_k_ = a_(ix_base + (1:n_lm)); b_k_ = zeros(size(a_k_));
W_beta_ = wignerd_b(l_max_max,-alpha_(2));
...
b_k_tmp = W_gamma*W_beta_{1+l_val}*W_alpha*a_k_tmp;
