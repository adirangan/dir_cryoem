...
azimu_b_all_ = 2*pi*transpose(linspace(0.0,1.0,n_all)); %<-- throw in a few meridian entries just to check. ;
polar_a_all_ = 1*pi*transpose(linspace(0.0,1.0,n_all)); %<-- throw in a few polar entries just to check. ;
polar_a_all_(1:n_all/2) = polar_a_all_(n_all/2+1:end); %<-- test nonunique polar_a_all_. ;
%%%%;
flag_flip = 0;
tmp_t = tic();
[~,Ylm_2__,~,~,sqrt_2lp1_,sqrt_2mp1_,sqrt_rat0_m_,sqrt_rat3_lm__,sqrt_rat4_lm__] = get_Ylm__2([],n_l,l_val_,n_all,azimu_b_all_,polar_a_all_);
tmp_t = toc(tmp_t); if (flag_verbose>-1); disp(sprintf(' %% get_Ylm__2 (not precomputation): %0.6fs',tmp_t)); end;
tmp_t = tic();
[~,Ylm_2__] = get_Ylm__2([],n_l,l_val_,n_all,azimu_b_all_,polar_a_all_,[],sqrt_2lp1_,sqrt_2mp1_,sqrt_rat0_m_,sqrt_rat3_lm__,sqrt_rat4_lm__);
tmp_t = toc(tmp_t); if (flag_verbose>-1); disp(sprintf(' %% get_Ylm__2 (yes precomputation): %0.6fs',tmp_t)); end;
%%%%;
flag_flip = 1;
tmp_t = tic();
[~,Ylm_2__] = get_Ylm__2([],n_l,l_val_,n_all,azimu_b_all_,polar_a_all_,flag_flip,sqrt_2lp1_,sqrt_2mp1_,sqrt_rat0_m_,sqrt_rat3_lm__,sqrt_rat4_lm__);
tmp_t = toc(tmp_t); if (flag_verbose>-1); disp(sprintf(' %% get_Ylm__2 (yes precomputation): %0.6fs',tmp_t)); end;
%%%%;
disp(sprintf(' %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%; '));
...
n_d = 3;
for nd=0:n_d-1;
if nd==0; tmp_str_pre = 'd0Y_mid_'; tmp_pre = d0Y_mid_lmj___; end;
if nd==1; tmp_str_pre = 'd1Y_mid_'; tmp_pre = d1Y_mid_lmj___; end;
if nd==2; tmp_str_pre = 'd2Y_mid_'; tmp_pre = d2Y_mid_lmj___; end;
tmp_error=0;
for nl=0:n_l-1;
l_val = l_val_(1+nl);
tmp_str_pos = sprintf('%s_%d_mj__',tmp_str_pre,l_val);
tmp_pos = tmp_pre{1+nl};
tmp_str_ascii = sprintf('d%dY_mid_%d_mj__',nd,l_val);
...
tmp_ascii = reshape(cell2mat(textscan(fid,'(%f)')),size(tmp_pos));
fclose(fid);
...
disp(sprintf(' %% tmp_error: %0.16f %%<-- should be <1e-3',tmp_error_numerator/max(1e-12,tmp_error_denomator)));
if (flag_verbose>1);
...
tmp_error_yespole_numerator = 0; tmp_error_yespole_denomator = 0;
tmp_error_notpole_numerator = 0; tmp_error_notpole_denomator = 0;
tmp_index_notpole_ = efind(abs(sin(polar_a_all_))>1e-3); %<-- stay away from poles. ;
for nl=0:n_l-1;
tmp_error_yespole_numerator = tmp_error_yespole_numerator + fnorm(d2Y_dif_lmj___{1+nl} - d2Y_mid_lmj___{1+nl});
tmp_error_yespole_denomator = tmp_error_yespole_denomator + fnorm(d2Y_dif_lmj___{1+nl});
tmp_error_notpole_numerator = tmp_error_notpole_numerator + fnorm(d2Y_dif_lmj___{1+nl}(1+tmp_index_notpole_) - d2Y_mid_lmj___{1+nl}(1+tmp_index_notpole_));
tmp_error_notpole_denomator = tmp_error_notpole_denomator + fnorm(d2Y_dif_lmj___{1+nl}(1+tmp_index_notpole_));
end;%for nl=0:n_l-1;
disp(sprintf(' %% tmp_error_yespole: %0.16f %%<-- can be large due to lazy/simple derivative near pole',tmp_error_yespole_numerator/max(1e-12,tmp_error_yespole_denomator)));
disp(sprintf(' %% tmp_error_notpole: %0.16f %%<-- should be <1e-3',tmp_error_notpole_numerator/max(1e-12,tmp_error_notpole_denomator)));
if (flag_verbose>1);
figure(1+nf);nf=nf+1;clf;figmed;
subplot(1,2,1);
hold on;
...
flag_precomputation = ...
  ( ...
    ...
    | isempty(sqrt_2lp1_) ...
    | isempty(sqrt_2mp1_) ...
    | isempty(sqrt_rat0_m_) ...
    | isempty(sqrt_rat3_lm__) ...
    | isempty(sqrt_rat4_lm__) ...
    );
...
%%%%;
tmp_t = toc(tmp_t); if (flag_verbose>0); disp(sprintf(' %% ylgndr_2 (flag_precomputation %d): %0.6fs',flag_precomputation,tmp_t)); end;
...
tmp_t = tic();
if flag_d0;
tmp_d0Y_lmj___ = permute(d0y_jlm___,1+[1,2,0]); %<-- permutation before inflation is faster. ;
tmp_d0Y_lmj___ = bsxfun(@times,tmp_d0Y_lmj___(:,1+abs(-l_max:+l_max),ij_return_),reshape(expimb_mj__,[1,1+2*l_max,n_all]))/sqrt(4*pi);
%tmp_d0Y_lmj___ = permute(d0y_jlm___(ij_return_,:,1+abs(-l_max:+l_max)),1+[1,2,0]); %<-- permutation after inflation is slower. ;
...
%%%%;
if flag_d1;
tmp_d1Y_lmj___ = ((-1)^flag_flip)^1 * permute(bsxfun(@times,d1y_jlm___,-sin(polar_a_unique_)),1+[1,2,0]);
...
%%%%;
if flag_d2;
tmp_d2Y_lmj___ = ((-1)^flag_flip)^2 * permute(bsxfun(@times,d2y_jlm___,(-sin(polar_a_unique_)).^2) + bsxfun(@times,d1y_jlm___,-cos(polar_a_unique_)),1+[1,2,0]);
