function [map,s,hdr,extraHeader]=ReadMRC_0(filename,startSlice, numSlices,test);
...
% listed. ;
% If the numSlices argument is <=0, no data are read; if it is negative, the file ;
...
%   end;
%   fclose(h);
% If a third output argument is specified, the header (as a vector of int32) ;
% is returned.  For non-extended files it has 256 entries. ;
% ;
...
% Added fields s.mx, s.my and s.mz (unit cell dimensions), s.org (origin offset) ; 
% along with s.pixA the pixel (voxel) size, which is computed as ;
