c$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
      subroutine test_innerproduct_batch_stage_0a(verbose
     $     ,svd_calculation_type,n_r,grid_p_,n_w_,max_x_c,n_S,ld_S,S_p_
     $     ,n_M,ld_M,M_p_,n_CTF,ld_CTF,CTF_p_,delta_x_est_,delta_y_est_
     $     ,gamma_z_est_,l2_norm_est_,ctf_ind_est_,eps_target
     $     ,N_pixels_in,displacement_max,n_delta_x,n_delta_y,n_gamma_z
...
c$$$                            M_p_(j + nm*ld_M).
c$$$
c$$$      integer *4 n_CTF      Number of ctfs stored within CTF_p_
c$$$
c$$$      integer *4 ld_CTF     Leading dimension of the array storing the
c$$$                            ctfs. (can be larger than a single image).
c$$$
c$$$      complex *16 CTF_p_    Array of size n_CTF*ld_CTF storing n_CTF ctfs.
c$$$                            Each ctf should be represented in
c$$$                            fourier space on a quasi-uniform polar-grid
c$$$                            (i.e., of form CTF_k_p_)
c$$$                          - We use the first n_A elements of each ctf.
c$$$                          - This subroutine assumes that ctfs are
c$$$                            stored sequentially, using a stride of ld_CTF.
c$$$                          - More specifically, we assume that CTF_p_ is 
c$$$                            indexed from 0 to n_CTF*ld_CTF-1, and that
c$$$                            the jth point in ctf nc is at 
c$$$                            CTF_p_(j + nc*ld_CTF).
c$$$
...
c$$$
c$$$      real *8 l2_norm_est_  Array of size n_M storing the current estimates
c$$$                            of the template-based l2_norm associated with
c$$$                            each image. For any given image this l2_norm
c$$$                            is computed via:
c$$$                            L = C_M * c / C_S, where
c$$$                            C_M is the norm of the image,
c$$$                            C_S is the norm of the best matching template
c$$$                            (after transformation and ctf), and
c$$$                            c is the pearson's correlation between the image
c$$$                            and the best matching template.
c$$$
c$$$      real *8 ctf_ind_est_ Array of size n_M storing the index nc (from
c$$$                           zero to n_ctf-1) of the ctf-function associated
c$$$                           with each image. I.e., ctf_ind_est_(nm) stores
c$$$                           the ctf_index associated with image nm.
c$$$
...
c$$$                            with a magnitude less than displacement_max.
c$$$
c$$$      complex *16 C_M_      Array of size n_M.
c$$$                          - Assuming this array is indexed from 0
c$$$                            to n_M-1, C_M_(nm) holds the l2_norm of image
c$$$                            number nm.
c$$$
c$$$      complex *16 C_S_max_  Array of size n_S*n_M.
c$$$                          - Assuming this array is indexed from 0
c$$$                            to n_S*n_M-1, C_S_max_(ns + nm*n_S) 
c$$$                            holds the (complex) l2_norm_squared of the
c$$$                            template that corresponds to the best 
c$$$                            alignment between template ns and image nm.
c$$$                          - Note that this l2_norm takes into account
c$$$                            the image-specific ctf-function applied to
c$$$                            the template (with the appropriate gamma_z).
c$$$                          - Note that this "best alignment" is
c$$$                            restricted to have a displacement vector
c$$$                            with a magnitude less than displacement_max.
c$$$                            
...
c$$$                          - Note that C_Z is a pearson's correlation,
c$$$                            going between -1 and 1.
c$$$                          - Note that this "best alignment" is
c$$$                            restricted to have a displacement vector
c$$$                            with a magnitude less than displacement_max.
...
c$$$      parameters for timing
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[entering test_innerproduct_batch_stage_0a]'
      end if      
      if (verbose.gt.0) then
     ...
     $        ,' too large in test_innerproduct_batch_stage_0a'
      end if
     ...
     $        ,' too small in test_innerproduct_batch_stage_0a'
      end if
      if (.false.) then
...
c$OMP DO 
     ...
     $           ,' with thread: ',omp_get_thread_num()
         ...
         call test_innerproduct_batch_stage_1a(verbose
     $        ,svd_calculation_type,n_r,grid_p_,n_w_,max_x_c,n_S,ld_S
     $        ,S_p_,n_tmp,ld_M,M_p_(ld_M*i_tmp),n_CTF,ld_CTF,CTF_p_
     $        ,delta_x_est_(i_tmp),delta_y_est_(i_tmp)
     $        ,gamma_z_est_(i_tmp),l2_norm_est_(i_tmp)
...
c$OMP END PARALLEL
     ...
     $        timing_toc-timing_tic
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[finished test_innerproduct_batch_stage_0a]'
      end if
