...
% <R(+gamma_z)*T(+delta)*M_k_p_wk_,S_k_p_wk_>, or ;
% <T(+delta)*M_k_p_wk_,R(-gamma_z)*S_k_p_wk_>. ;
...
% TM_RS_form_w_ stores the result of the analytical formula. ;
% TM_RS_quad_w_ stores the result calculated using fourier-bessel qudarature. ;
% TM_RS_brut_w_ stores the direct calculation (using quadrature on the polar-grid). ;
%%%%%%%%;
...
TM_k_p_wk_ = transf_p_to_p(n_k_p_r,k_p_r_,n_w_,n_w_sum,M_k_p_wk_,+delta_fix_(1+0),+delta_fix_(1+1));
TM_k_q_wk_ = interp_p_to_q(n_k_p_r,n_w_,n_w_sum,TM_k_p_wk_);
S_k_p_wk_ = S_k_p_wkS__(:,1+nS);
S_k_q_wk_ = interp_p_to_q(n_k_p_r,n_w_,n_w_sum,S_k_p_wk_);
TM_RS_quad_w_ = ifft(sum(bsxfun(@times,reshape(conj(TM_k_q_wk_).*S_k_q_wk_,[n_w_max,n_k_p_r]),reshape(weight_2d_k_p_r_,[1,n_k_p_r])),2));
TM_RS_brut_w_ = zeros(n_w_max,1);
for nw=0:n_w_max-1;
gamma_z = gamma_z_(1+nw);
TM_RS_brut_w_(1+nw) = sum(conj(rotate_p_to_p_fftw(n_k_p_r,n_w_,n_w_sum,TM_k_p_wk_,+gamma_z)).*S_k_p_wk_.*weight_2d_wk_)*(2*pi)^2;
end;%for nw=0:n_w_max-1;
fnorm_disp(flag_verbose,'TM_RS_brut_w_',TM_RS_brut_w_,'TM_RS_quad_w_',TM_RS_quad_w_,' %%<-- should be zero');
%%%%%%%%;
...
TM_RS_I = I_xPPx_0(k_p_r_max,CTF_phi,delta_S_2_,CTF_phi,delta_M_2_);
TM_RS_form = TM_RS_form + TM_RS_I;
end;%for nsource_M=0:n_source-1;
end;%for nsource_S=0:n_source-1;
TM_RS_form_w_(1+nw) = TM_RS_form;
end;%for nw=0:n_w_max-1;
fnorm_disp(flag_verbose,'TM_RS_form_w_',TM_RS_form_w_,'TM_RS_brut_w_',TM_RS_brut_w_,' %%<-- should be small');
fnorm_disp(flag_verbose,'TM_RS_form_w_',TM_RS_form_w_,'TM_RS_quad_w_',TM_RS_quad_w_,' %%<-- should be small');
%%%%%%%%;
