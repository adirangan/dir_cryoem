...
disp(sprintf(' %% Warning, replacing rlnImageName with job_569_model_1.mrcs'));
for nimage=0:numel(star_blockData{1}.rlnImageName)-1;
star_blockData{1}.rlnImageName{1+nimage} = sprintf('%d@Particles_Stacks_Model_1/job_569_model_1.mrcs',1+nimage);
end;%for nimage=0:numel(star_blockData{1}.rlnImageName)-1;
...
%%%%%%%%;
% Now try restricting the angles to lie on a ring. ;
% Verdict: Helps a little, but not too much (0.45-->0.51 correlation). ;
%%%%%%%%;

verbose=1;
if (verbose); disp(sprintf(' %% entering test_pm_LetB1')); end;
date_diff_threshold = 0.25;
flag_force_create_mat=0;flag_force_create_tmp=0;
delta_sigma_use = delta_sigma;
dat_rseed_ = [0:2]; n_dat_rseed = numel(dat_rseed_);
...
image_delta_y_true_ = +image_delta_y_true_(1+(0:n_M-1));
%%%%%%%%;
ndat_rseed=2;%for ndat_rseed=0:n_dat_rseed-1;
dat_rseed = dat_rseed_(1+ndat_rseed);
ndelta_r_max_factor=3;%for ndelta_r_max_factor=0:n_delta_r_max_factor-1;
delta_r_max_factor = delta_r_max_factor_(1+ndelta_r_max_factor);
delta_r_max_use = delta_r_max_factor * delta_sigma_use*sqrt(log(20^2));
delta_r_max_upb = 2.0 * delta_sigma_use*sqrt(log(20^2)); %<-- allow large accumulated translations. ;
ndat_n_UX_rank=9;%for ndat_n_UX_rank=0:n_dat_n_UX_rank-1;
dat_n_UX_rank = dat_n_UX_rank_(1+ndat_n_UX_rank);
flag_alternate_MS_vs_SM = [1];%for flag_alternate_MS_vs_SM = [0:1];
...
parameter.euler_polar_a_restrict_band = pi/4;
if (numel(unique(CTF_index_))>=n_M/8);
disp(sprintf(' %% Warning, setting flag_CTF_index_unused==1'));
parameter.flag_CTF_index_unused = 1;
end;%if (numel(unique(CTF_index_))>=n_M/8);
...
XC_fname_mat = sprintf('%s_mat/X_2d_xcor_d0_restrict4_%st%.4dn%.2dr%d.mat',dir_pm,str_strategy,floor(1000*delta_r_max),dat_n_UX_rank,rseed);
XC_fname_snapshot_pre = sprintf('%s_mat/X_2d_xcor_d0_restrict4_%st%.4dn%.2dr%d_snapshot',dir_pm,str_strategy,floor(1000*delta_r_max),dat_n_UX_rank,rseed);
XC_fname_snapshot_jpg = sprintf('%s.jpg',XC_fname_snapshot_pre);
XC_fname_compare_image_rank_pre = sprintf('%s_mat/X_2d_xcor_d0_restrict4_%st%.4dn%.2dr%d_compare_image_rank',dir_pm,str_strategy,floor(1000*delta_r_max),dat_n_UX_rank,rseed);
...
if ( ...
     ...
     &  exist(XC_fname_snapshot_jpg,'file') ...
     &  exist(XC_fname_compare_image_rank_jpg,'file') ...
   ); flag_skip_all = 1; end;
%%%%;
if ~flag_skip_all;
%%%%;
...
XC_fname_pre = sprintf('%s_mat/X_2d_xcor_d0_restrict4_%st%.4dn%.2dr%d',dir_pm,str_strategy,floor(1000*delta_r_max),dat_n_UX_rank,rseed);
[XC_flag_skip,XC_fname_mat] = open_fname_tmp(XC_fname_pre,date_diff_threshold,flag_force_create_mat,flag_force_create_tmp);
if ~XC_flag_skip;
%%%%%%%%;
% First use the final step from XB to recalculate the principal-modes. ;
%%%%%%%%;
...
tmp_ = load(fname_XB_k_Y_mat); XB_a_k_Y_reco_ = tmp_.a_k_Y_reco_; clear tmp_;
XB_a_k_Y_reco__ = zeros(n_lm_max,n_k_p_r);
for nk_p_r=0:n_k_p_r-1;
tmp_index_ = n_lm_csum_(1+nk_p_r) + (0:n_lm_(1+nk_p_r)-1);
XB_a_k_Y_reco__(1:n_lm_(1+nk_p_r),1+nk_p_r) = XB_a_k_Y_reco_(1+tmp_index_);
end;%for nk_p_r=0:n_k_p_r-1;
%%%%%%%%;
[ ...
 X_2d_xcor_d0_restrict4__ ...
,X_2d_xcor_d0_restrict4_weight_r_ ...
] = ...
...
[UX_2d_xcor_d0_restrict4__,SX_2d_xcor_d0_restrict4__,VX_2d_xcor_d0_restrict4__] = svds(X_2d_xcor_d0_restrict4__,n_UX_rank); SX_2d_xcor_d0_restrict4_ = diag(SX_2d_xcor_d0_restrict4__);
if (verbose); disp(sprintf(' %% cumsum(SX_2d_xcor_d0_restrict4_): ')); disp(sprintf(' %% %0.4f',cumsum(SX_2d_xcor_d0_restrict4_/sum(SX_2d_xcor_d0_restrict4_)))); end;
%%%%%%%%;
try;
disp(sprintf(' %% %s not found, creating',XC_fname_mat));
XC_parameter = struct('type','parameter');
XC_parameter = parameter; %<-- copy parameter and all its fields. ;
XC_parameter.delta_r_upb = delta_r_upb;
XC_parameter.delta_r_max = delta_r_max;
XC_parameter.rseed = rseed;
XC_parameter.fname_pre = XC_fname_pre;
XC_parameter.flag_alternate_MS_vs_SM = flag_alternate_MS_vs_SM;
XC_parameter.flag_local_exclusion = flag_local_exclusion;
XC_parameter.flag_qbp_vs_lsq = flag_qbp_vs_lsq;
...
%%%%%%%%;
if (verbose); disp(sprintf(' %% %s found, aligning',XC_fname_mat)); end;
...
%%%%%%%%;
if ~exist('UX_2d_xcor_d0_restrict4__','var');
tmp_ = load(fname_XB_k_Y_mat); XB_a_k_Y_reco_ = tmp_.a_k_Y_reco_; clear tmp_;
XB_a_k_Y_reco__ = zeros(n_lm_max,n_k_p_r);
for nk_p_r=0:n_k_p_r-1;
tmp_index_ = n_lm_csum_(1+nk_p_r) + (0:n_lm_(1+nk_p_r)-1);
...
[UX_2d_xcor_d0_restrict4__,SX_2d_xcor_d0_restrict4__,VX_2d_xcor_d0_restrict4__] = svds(X_2d_xcor_d0_restrict4__,n_UX_rank); SX_2d_xcor_d0_restrict4_ = diag(SX_2d_xcor_d0_restrict4__);
end;%if ~exist('UX_2d_xcor_d0_restrict4__','var');
%%%%%%%%;
% First form a_CTF_avg_UX_2d_xcor_d0_restrict4_Y_quad__ ;
%%%%%%%%;
a_CTF_avg_UX_2d_xcor_d0_restrict4_Y_quad__ = zeros(n_lm_max,n_UX_rank);
for nUX_rank=0:n_UX_rank-1;
for nk_p_r=0:n_k_p_r-1;
tmp_l_max = l_max_(1+nk_p_r);
tmp_n_lm = (tmp_l_max+1).^2;
tmp_index_ = n_lm_csum_(1+nk_p_r) + (0:tmp_n_lm-1);
a_CTF_avg_UX_2d_xcor_d0_restrict4_Y_quad__(1:tmp_n_lm,1+nUX_rank) = ...
a_CTF_avg_UX_2d_xcor_d0_restrict4_Y_quad__(1:tmp_n_lm,1+nUX_rank) + ...
UX_2d_xcor_d0_restrict4__(1+nk_p_r,1+nUX_rank)*X_2d_xcor_d0_restrict4_weight_r_(1+nk_p_r)*a_k_Y_true_(1+tmp_index_)*CTF_avg_k_p_r_(1+nk_p_r); ...
%<-- use average CTF here, under the assumption that a_CTF_UX_2d_xcor_d0_restrict4_Y_quad_ will be used alone. ;
end;%for nk_p_r=0:n_k_p_r-1;
end;%for nUX_rank=0:n_UX_rank-1;
%%%%%%%%;
...
,l_max_max ...
,dat_n_UX_rank ...
,reshape(a_CTF_avg_UX_2d_xcor_d0_restrict4_Y_quad__(:,1:dat_n_UX_rank),[n_lm_max*dat_n_UX_rank,1]) ...
,n_M ...
,euler_polar_a_true_ ...
,euler_azimu_b_true_ ...
,euler_gamma_z_true_ ...
...
,image_delta_y_true_ ...
,[] ...
,tmp_XC_.a_CTF_avg_UX_Y__ ...
,tmp_XC_.euler_polar_a__ ...
,tmp_XC_.euler_azimu_b__ ...
,tmp_XC_.euler_gamma_z__ ...
,tmp_XC_.image_delta_x_acc__ + tmp_XC_.image_delta_x_upd__ ...
,tmp_XC_.image_delta_y_acc__ + tmp_XC_.image_delta_y_upd__ ...
);
...
%%%%%%%%;
tmp_XC_euler_polar_a_ = tmp_XC_.euler_polar_a__(:,end);
tmp_XC_euler_azimu_b_ = tmp_XC_.euler_azimu_b__(:,end);
tmp_XC_euler_gamma_z_ = tmp_XC_.euler_gamma_z__(:,end);
tmp_XC_image_delta_x_ = tmp_XC_.image_delta_x_acc__(:,end) + tmp_XC_.image_delta_x_upd__(:,end);
tmp_XC_image_delta_y_ = tmp_XC_.image_delta_y_acc__(:,end) + tmp_XC_.image_delta_y_upd__(:,end);
...
if (verbose); disp(sprintf(' %% %s not found, creating',fname_XC_k_Y_mat)); end;
a_k_Y_reco_ = ...
cg_lsq_4( ...
 cg_lsq_n_order ...
,n_k_p_r ...
...
%tmp_ = load(fname_XC_k_Y_mat); XC_a_k_Y_reco_ = tmp_.a_k_Y_reco_; clear tmp_; %<-- not used. ;
%%%%%%%%;
...
%%%%%%%%;
if (verbose) disp(sprintf(' %% %s not found, creating',XC_fname_snapshot_jpg)); end;
tmp_XC_ = load(XC_fname_mat);
...
disp(sprintf(' %% Warning, %s not found in ampmut_wrap_wrap_4',tmp_XC_fname_mat));
flag_found = 0;
...
%%%%%%%%;
if (verbose) disp(sprintf(' %% %s not found, creating',XC_fname_compare_image_rank_jpg)); end;
tmp_XC_parameter = parameter;
[ ...
 tmp_XC_parameter ...
] = ...
...
%%%%%%%%;
% Now check ampmut_compare_image_rank. ;
%%%%%%%%;

fname_prefix_pre4 = '/data/rangan/dir_cryoem/dir_LetB1/dir_pm_mat';
fname_prefix_ver4 = '/data/rangan/dir_cryoem/dir_LetB1/dir_pm_mat/X_2d_xcor_d0_a1t0073n20r2';
ver4_ = struct('type','collect');
ver4_.X_fname_mat = sprintf('%s.mat',fname_prefix_ver4);
ver4_.X_fname_compare_image_rank_pre = sprintf('%s_compare_image_rank_deleteme.mat',fname_prefix_ver4);
ver4_.X_TM_ = [];
tmp_ver4_ = load(sprintf('%s/a_k_p_quad_.mat',fname_prefix_pre4));
ver4_.a_k_p_quad_ = tmp_ver4_.a_k_p_quad_;
ver4_.k_eq_d = tmp_ver4_.k_eq_d;
ver4_.k_p_r_ = tmp_ver4_.k_p_r_;
ver4_.k_p_r_max = tmp_ver4_.k_p_r_max;
ver4_.n_k_p_r = tmp_ver4_.n_k_p_r;
ver4_.weight_3d_k_p_r_ = tmp_ver4_.weight_3d_k_p_r_;
clear tmp_ver4_;
tmp_ver4_ = load(sprintf('%s/a_k_Y_quad_.mat',fname_prefix_pre4));
ver4_.a_k_Y_quad_ = tmp_ver4_.a_k_Y_quad_;
ver4_.l_max_ = tmp_ver4_.l_max_;
ver4_.l_max_max = tmp_ver4_.l_max_max;
clear tmp_ver4_;
tmp_ver4_ = load(sprintf('%s/a_CTF_avg_UX_Y_reco__.mat',fname_prefix_pre4));
ver4_.euler_polar_a_true_ = tmp_ver4_.euler_polar_a__(:,end);
ver4_.euler_azimu_b_true_ = tmp_ver4_.euler_azimu_b__(:,end);
ver4_.euler_gamma_z_true_ = tmp_ver4_.euler_gamma_z__(:,end);
ver4_.image_delta_x_true_ = tmp_ver4_.image_delta_x_acc__(:,end) + tmp_ver4_.image_delta_x_upd__(:,end);
ver4_.image_delta_y_true_ = tmp_ver4_.image_delta_y_acc__(:,end) + tmp_ver4_.image_delta_y_upd__(:,end);
ver4_.image_X_value_true_ = tmp_ver4_.image_X_value__(:,end-1);
clear tmp_ver4_;
tmp_ver4_ = load(sprintf('%s/CTF_k_p__.mat',fname_prefix_pre4));
ver4_.CTF_k_p__ = tmp_ver4_.CTF_k_p__;
ver4_.CTF_k_p_r__ = tmp_ver4_.CTF_k_p_r__;
ver4_.CTF_k_p_r_xavg__ = tmp_ver4_.CTF_k_p_r_xavg__;
ver4_.CTF_k_p_r_xcor__ = tmp_ver4_.CTF_k_p_r_xcor__;
ver4_.n_CTF = tmp_ver4_.n_CTF;
ver4_.n_CTF_rank = tmp_ver4_.n_CTF_rank;
ver4_.CTF_avg_k_p_ = tmp_ver4_.CTF_avg_k_p_;
ver4_.CTF_avg_k_p_r_ = tmp_ver4_.CTF_avg_k_p_r_;
ver4_.CTF_index_ = tmp_ver4_.CTF_index_;
ver4_.index_nCTF_from_nM_ = ver4_.CTF_index_;
clear tmp_ver4_;
tmp_ver4_ = load(sprintf('%s/S_k_p__.mat',fname_prefix_pre4));
ver4_.n_w_ = tmp_ver4_.n_w_;
ver4_.n_w_max = tmp_ver4_.n_w_max;
ver4_.template_k_eq_d = tmp_ver4_.template_k_eq_d;
ver4_.viewing_k_eq_d = tmp_ver4_.viewing_k_eq_d;
ver4_.weight_2d_k_p_r_ = tmp_ver4_.weight_2d_k_p_r_;
clear tmp_ver4_;
tmp_ver4_ = load(sprintf('%s_a_k_Y_.mat',fname_prefix_ver4));
ver4_.a_k_Y_reco_ = tmp_ver4_.a_k_Y_reco_;
clear tmp_ver4_;
tmp_ver4_ = load(sprintf('%s_align_a_k_Y_.mat',fname_prefix_ver4));
ver4_.X_best_ = tmp_ver4_.X_best_;
ver4_.X_best_flag_flip_ = tmp_ver4_.X_best_flag_flip_;
ver4_.polar_a_best_ = tmp_ver4_.polar_a_best_;
ver4_.azimu_b_best_ = tmp_ver4_.azimu_b_best_;
ver4_.gamma_z_best_ = tmp_ver4_.gamma_z_best_;
ver4_.delta_best__ = tmp_ver4_.delta_best__;
clear tmp_ver4_;
tmp_ver4_ = load(sprintf('%s.mat',fname_prefix_ver4));
ver4_.parameter = tmp_ver4_.parameter;
ver4_.euler_polar_a__ = tmp_ver4_.euler_polar_a__;
ver4_.euler_azimu_b__ = tmp_ver4_.euler_azimu_b__;
ver4_.euler_gamma_z__ = tmp_ver4_.euler_gamma_z__;
ver4_.image_delta_x_acc__ = tmp_ver4_.image_delta_x_acc__;
ver4_.image_delta_y_acc__ = tmp_ver4_.image_delta_y_acc__;
ver4_.image_delta_x_upd__ = tmp_ver4_.image_delta_x_upd__;
ver4_.image_delta_y_upd__ = tmp_ver4_.image_delta_y_upd__;
ver4_.image_I_value__ = tmp_ver4_.image_I_value__;
ver4_.image_X_value__ = tmp_ver4_.image_X_value__;
ver4_.image_S_index__ = tmp_ver4_.image_S_index__;
clear tmp_ver4_;

flag_center_image = 0;
fname_prefix_pre5 = '/data/rangan/dir_cryoem/dir_LetB1_x0/dir_pm_mat';
fname_prefix_ver5 = '/data/rangan/dir_cryoem/dir_LetB1_x0/dir_pm_mat/X_2d_xcor_d0_a1t0074n20r2';
ver5_ = struct('type','collect');
ver5_.X_fname_mat = sprintf('%s.mat',fname_prefix_ver5);
ver5_.X_fname_compare_image_rank_pre = sprintf('%s_compare_image_rank_deleteme.mat',fname_prefix_ver5);
ver5_.X_TM_ = [];
tmp_ver5_ = load(sprintf('%s/a_k_p_quad_.mat',fname_prefix_pre5));
ver5_.a_k_p_quad_ = tmp_ver5_.a_k_p_quad_;
ver5_.k_eq_d = tmp_ver5_.k_eq_d;
ver5_.k_p_r_ = tmp_ver5_.k_p_r_;
ver5_.k_p_r_max = tmp_ver5_.k_p_r_max;
ver5_.n_k_p_r = tmp_ver5_.n_k_p_r;
ver5_.weight_3d_k_p_r_ = tmp_ver5_.weight_3d_k_p_r_;
clear tmp_ver5_;
tmp_ver5_ = load(sprintf('%s/a_k_Y_quad_.mat',fname_prefix_pre5));
ver5_.a_k_Y_quad_ = tmp_ver5_.a_k_Y_quad_;
ver5_.l_max_ = tmp_ver5_.l_max_;
ver5_.l_max_max = tmp_ver5_.l_max_max;
clear tmp_ver5_;
if flag_center_image==0; tmp_ver5_ = load(sprintf('%s/a_k_Y_reco_from_M__.mat',fname_prefix_pre5)); end;
if flag_center_image==1; tmp_ver5_ = load(sprintf('%s/a_k_Y_reco_from_N__.mat',fname_prefix_pre5)); end;
ver5_.euler_polar_a_true_ = tmp_ver5_.euler_polar_a_Mi__(:,end);
ver5_.euler_azimu_b_true_ = tmp_ver5_.euler_azimu_b_Mi__(:,end);
ver5_.euler_gamma_z_true_ = tmp_ver5_.euler_gamma_z_Mi__(:,end);
ver5_.image_delta_x_true_ = tmp_ver5_.image_delta_x_acc_Mi__(:,end) + tmp_ver5_.image_delta_x_upd_Mi__(:,end);
ver5_.image_delta_y_true_ = tmp_ver5_.image_delta_y_acc_Mi__(:,end) + tmp_ver5_.image_delta_y_upd_Mi__(:,end);
ver5_.image_X_value_true_ = tmp_ver5_.image_X_value_Mi__(:,end-1);
clear tmp_ver5_;
tmp_ver5_ = load(sprintf('%s/CTF_k_p_wkC__.mat',fname_prefix_pre5));
ver5_.CTF_k_p_wkC__ = tmp_ver5_.CTF_k_p_wkC__;
ver5_.CTF_k_p_r_kC__ = tmp_ver5_.CTF_k_p_r_kC__;
ver5_.CTF_k_p_r_xavg__ = tmp_ver5_.CTF_k_p_r_xavg__;
ver5_.CTF_k_p_r_xcor__ = tmp_ver5_.CTF_k_p_r_xcor__;
ver5_.n_CTF = tmp_ver5_.n_CTF;
ver5_.n_CTF_rank = tmp_ver5_.n_CTF_rank;
ver5_.CTF_avg_k_p_wk_ = tmp_ver5_.CTF_avg_k_p_wk_;
ver5_.CTF_avg_k_p_r_k_ = tmp_ver5_.CTF_avg_k_p_r_k_;
ver5_.index_nCTF_from_nM_ = tmp_ver5_.index_nCTF_from_nM_;
clear tmp_ver5_;
tmp_ver5_ = load(sprintf('%s/S_k_p__.mat',fname_prefix_pre5));
ver5_.n_w_ = tmp_ver5_.n_w_;
ver5_.n_w_max = tmp_ver5_.n_w_max;
ver5_.template_k_eq_d = tmp_ver5_.template_k_eq_d;
ver5_.viewing_k_eq_d = tmp_ver5_.viewing_k_eq_d;
ver5_.weight_2d_k_p_r_ = tmp_ver5_.weight_2d_k_p_r_;
clear tmp_ver5_;
tmp_ver5_ = load(sprintf('%s_a_k_Y_.mat',fname_prefix_ver5));
ver5_.a_k_Y_reco_ = tmp_ver5_.a_k_Y_reco_;
clear tmp_ver5_;
tmp_ver5_ = load(sprintf('%s_align_a_k_Y_.mat',fname_prefix_ver5));
ver5_.X_best_ = tmp_ver5_.X_best_;
ver5_.X_best_flag_flip_ = tmp_ver5_.X_best_flag_flip_;
ver5_.polar_a_best_ = tmp_ver5_.polar_a_best_;
ver5_.azimu_b_best_ = tmp_ver5_.azimu_b_best_;
ver5_.gamma_z_best_ = tmp_ver5_.gamma_z_best_;
ver5_.delta_best__ = tmp_ver5_.delta_best__;
clear tmp_ver5_;
tmp_ver5_ = load(sprintf('%s.mat',fname_prefix_ver5));
ver5_.parameter = tmp_ver5_.parameter;
ver5_.euler_polar_a_Mi__ = tmp_ver5_.euler_polar_a_Mi__;
ver5_.euler_azimu_b_Mi__ = tmp_ver5_.euler_azimu_b_Mi__;
ver5_.euler_gamma_z_Mi__ = tmp_ver5_.euler_gamma_z_Mi__;
ver5_.image_delta_x_acc_Mi__ = tmp_ver5_.image_delta_x_acc_Mi__;
ver5_.image_delta_y_acc_Mi__ = tmp_ver5_.image_delta_y_acc_Mi__;
ver5_.image_delta_x_upd_Mi__ = tmp_ver5_.image_delta_x_upd_Mi__;
ver5_.image_delta_y_upd_Mi__ = tmp_ver5_.image_delta_y_upd_Mi__;
ver5_.image_I_value_Mi__ = tmp_ver5_.image_I_value_Mi__;
ver5_.image_X_value_Mi__ = tmp_ver5_.image_X_value_Mi__;
ver5_.image_S_index_Mi__ = tmp_ver5_.image_S_index_Mi__;
clear tmp_ver5_;
...
,ver4_.k_p_r_ ...
,ver4_.k_p_r_max ...
,ver4_.weight_3d_k_p_r_ ...
,ver4_.weight_2d_k_p_r_ ...
,ver4_.n_w_ ...
,n_M ...
,M_k_p__ ...
,ver4_.n_CTF ...
,ver4_.n_CTF_rank ...
,ver4_.index_nCTF_from_nM_ ...
,ver4_.CTF_k_p__ ...
,ver4_.l_max_ ...
,ver4_.a_k_Y_quad_ ...
,ver4_.euler_polar_a_true_ ...
,ver4_.euler_azimu_b_true_ ...
,ver4_.euler_gamma_z_true_ ...
,ver4_.image_delta_x_true_ ...
,ver4_.image_delta_y_true_ ...
,ver4_.X_TM_ ...
,ver4_.X_fname_mat ...
,ver4_.X_fname_compare_image_rank_pre ...
);
...
,ver5_.k_p_r_ ...
,ver5_.k_p_r_max ...
,ver5_.weight_3d_k_p_r_ ...
,ver5_.weight_2d_k_p_r_ ...
,ver5_.n_w_ ...
,n_M ...
,M_k_p__ ...
,ver5_.n_CTF ...
,ver5_.n_CTF_rank ...
,ver5_.index_nCTF_from_nM_ ...
,ver5_.CTF_k_p_wkC__ ...
,ver5_.l_max_ ...
,ver5_.a_k_Y_quad_ ...
,ver5_.euler_polar_a_true_ ...
,ver5_.euler_azimu_b_true_ ...
,ver5_.euler_gamma_z_true_ ...
,ver5_.image_delta_x_true_ ...
,ver5_.image_delta_y_true_ ...
,ver5_.X_TM_ ...
,ver5_.X_fname_mat ...
,ver5_.X_fname_compare_image_rank_pre ...
);

ver5_.X_fname_compare_image_rank_pre = '/data/rangan/dir_cryoem/dir_LetB1_x0/dir_pm_mat/X_2d_xcor_d0_a1t0074n20r2_compare_image_rank_deleteme2.mat';
ampmut_compare_image_rank_0b( ...
 ver5_.parameter ...
,ver5_.n_k_p_r ...
,ver5_.k_p_r_ ...
,ver5_.k_p_r_max ...
...
% plot(ver4_.image_X_value_true_,ver5_.image_X_value_true_,'.',[0,1],[0,1],'k-'); axis equal; grid on;

%%%%%%%%;
% Verdict, still a mystery. ;
%%%%%%%%;
