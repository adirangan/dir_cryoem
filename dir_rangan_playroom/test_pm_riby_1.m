...
verbose=1;
if (verbose); disp(sprintf(' %% [entering test_pm_riby_1]')); end;
...
tmp_ij_0_ = floor(n_O_x_u_pack_(1+0)*1.0/4) + [1:n_Q_sub_x_u_pack_(1+0)];
tmp_ij_1_ = floor(n_O_x_u_pack_(1+1)*2.5/4) + [1:n_Q_sub_x_u_pack_(1+1)];
Q_x_u_pack_ = conv2(P_x_u_pack_,PSF_avg_x_u_xx__,'same');
Q_sub_x_u_pack_ = Q_x_u_pack_(tmp_ij_0_,tmp_ij_1_);
flag_disp=0;
if flag_disp;
tmp_Q_x_u_pack_ = Q_x_u_pack_;
tmp_p = prctile(tmp_Q_x_u_pack_,100,'all');
tmp_Q_x_u_pack_(min(tmp_ij_0_),tmp_ij_1_) = tmp_p;
tmp_Q_x_u_pack_(max(tmp_ij_0_),tmp_ij_1_) = tmp_p;
tmp_Q_x_u_pack_(tmp_ij_0_,min(tmp_ij_1_)) = tmp_p;
tmp_Q_x_u_pack_(tmp_ij_0_,max(tmp_ij_1_)) = tmp_p;
figure(1+nf);nf=nf+1;clf;figmed;
...
if ~exist('scatter_from_tensor_wswk___','var'); scatter_from_tensor_wswk___ = []; end;
nS=0;
S_k_p_wk_ = S_k_p_wkS__(:,1+nS);
...
S_x_u_avg = sum(S_x_u_xx__.*M_x_u_xx__,'all')*dx_pack*dx_pack;
S_x_u_nrm_xx__ = (S_x_u_xx__ - S_x_u_avg).*M_x_u_xx__;
S_x_u_var = sum(S_x_u_nrm_xx__.^2,'all')*dx_pack*dx_pack;
...
subplot(1,3,2); imagesc(Q1M_A_x_u_xxw___(:,:,1)); axis image; axisnotick; title('A'); colorbar;
subplot(1,3,3); imagesc(Q1M_B_x_u_xxw___(:,:,1)); axis image; axisnotick; title('B'); colorbar;
figure(1+nf);nf=nf+1;clf;figmed;
n_h = 128; np=0;
hlim_CA_ = [ ...
 min(min(Q1M_C_x_u_xx__,[],'all'),min(Q1M_A_x_u_xxw___(:,:,1),[],'all')) ...
 max(max(Q1M_C_x_u_xx__,[],'all'),max(Q1M_A_x_u_xxw___(:,:,1),[],'all')) ...
];
h2_CA__ = hist2d_0(Q1M_C_x_u_xx__,Q1M_A_x_u_xxw___(:,:,1),n_h,n_h,hlim_CA_,hlim_CA_);
subplot(2,2,1+np);np=np+1;
imagesc(log2(1+h2_CA__)); axis image; axisnotick; colorbar; set(gca,'ydir','normal');
title('Q1M_C_x_u_xx__ vs Q1M_A_x_u_xxw___(:,:,1)','Interpreter','none');
subplot(2,2,1+np);np=np+1;
plot(Q1M_C_x_u_xx__(:),reshape(Q1M_A_x_u_xxw___(:,:,1),[prod(n_Q_sub_x_u_pack_),1]),'.');
axisnotick; xlim(hlim_CA_);ylim(hlim_CA_); axis square;
title('Q1M_C_x_u_xx__ vs Q1M_A_x_u_xxw___(:,:,1)','Interpreter','none');
hlim_CB_ = [ ...
 min(min(Q1M_C_x_u_xx__,[],'all'),min(Q1M_B_x_u_xxw___(:,:,1),[],'all')) ...
 max(max(Q1M_C_x_u_xx__,[],'all'),max(Q1M_B_x_u_xxw___(:,:,1),[],'all')) ...
];
h2_CB__ = hist2d_0(Q1M_C_x_u_xx__,Q1M_B_x_u_xxw___(:,:,1),n_h,n_h,hlim_CB_,hlim_CB_);
subplot(2,2,1+np);np=np+1;
imagesc(log2(1+h2_CB__)); axis image; axisnotick; colorbar; set(gca,'ydir','normal');
title('Q1M_C_x_u_xx__ vs Q1M_B_x_u_xxw___(:,:,1)','Interpreter','none');
subplot(2,2,1+np);np=np+1;
plot(Q1M_C_x_u_xx__(:),reshape(Q1M_B_x_u_xxw___(:,:,1),[prod(n_Q_sub_x_u_pack_),1]),'.');
...
subplot(1,3,2); imagesc(Q1S_A_x_u_xxw___(:,:,1)); axis image; axisnotick; title('A'); colorbar;
subplot(1,3,3); imagesc(Q1S_B_x_u_xxw___(:,:,1)); axis image; axisnotick; title('B'); colorbar;
figure(1+nf);nf=nf+1;clf;figmed;
n_h = 128; np=0;
hlim_CA_ = [ ...
 min(min(Q1S_C_x_u_xx__,[],'all'),min(Q1S_A_x_u_xxw___(:,:,1),[],'all')) ...
 max(max(Q1S_C_x_u_xx__,[],'all'),max(Q1S_A_x_u_xxw___(:,:,1),[],'all')) ...
];
h2_CA__ = hist2d_0(Q1S_C_x_u_xx__,Q1S_A_x_u_xxw___(:,:,1),n_h,n_h,hlim_CA_,hlim_CA_);
subplot(2,2,1+np);np=np+1;
imagesc(log2(1+h2_CA__)); axis image; axisnotick; colorbar; set(gca,'ydir','normal');
title('Q1S_C_x_u_xx__ vs Q1S_A_x_u_xxw___(:,:,1)','Interpreter','none');
subplot(2,2,1+np);np=np+1;
plot(Q1S_C_x_u_xx__(:),reshape(Q1S_A_x_u_xxw___(:,:,1),[prod(n_Q_sub_x_u_pack_),1]),'.');
axisnotick; xlim(hlim_CA_);ylim(hlim_CA_); axis square;
title('Q1S_C_x_u_xx__ vs Q1S_A_x_u_xxw___(:,:,1)','Interpreter','none');
hlim_CB_ = [ ...
 min(min(Q1S_C_x_u_xx__,[],'all'),min(Q1S_B_x_u_xxw___(:,:,1),[],'all')) ...
 max(max(Q1S_C_x_u_xx__,[],'all'),max(Q1S_B_x_u_xxw___(:,:,1),[],'all')) ...
];
h2_CB__ = hist2d_0(Q1S_C_x_u_xx__,Q1S_B_x_u_xxw___(:,:,1),n_h,n_h,hlim_CB_,hlim_CB_);
subplot(2,2,1+np);np=np+1;
imagesc(log2(1+h2_CB__)); axis image; axisnotick; colorbar; set(gca,'ydir','normal');
title('Q1S_C_x_u_xx__ vs Q1S_B_x_u_xxw___(:,:,1)','Interpreter','none');
subplot(2,2,1+np);np=np+1;
plot(Q1S_C_x_u_xx__(:),reshape(Q1S_B_x_u_xxw___(:,:,1),[prod(n_Q_sub_x_u_pack_),1]),'.');
axisnotick; xlim(hlim_CB_);ylim(hlim_CB_); axis square;
title('Q1S_C_x_u_xx__ vs Q1S_B_x_u_xxw___(:,:,1)','Interpreter','none');
...
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% innerproduct_O_x_u_vs_S_x_u_2: %0.2fs',tmp_t)); end;
%%%%;
Q1_avg_x_u_xxw___ = Q1M_x_u_xxw___/M_sum;
Q1_var_x_u_xxw___ = Q2M_x_u_xxw___/M_sum - real(Q1_avg_x_u_xxw___).^2;
Q1_std_x_u_xxw___ = sqrt(Q1_var_x_u_xxw___);
% With these terms we can compute: ;
% Q1_avg = <Q1,M>/M_sum ;
% Q1_var = <Q2,M>/M_sum - Q1_avg^2 ;
% Q1_std = sqrt(Q1_var) ;
...
verbose=1;
if (verbose); disp(sprintf(' %% [finished test_pm_riby_1]')); end;
