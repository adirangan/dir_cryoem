...
sample_sphere_7(verbose,k_p_r_max,k_eq_d,str_T_vs_L_{1},flag_uniform_over_n_k_p_r,flag_uniform_over_polar_a);
...
[n_k_all_csum] = sample_shell_6(k_p_r,k_eq_d_use,str_T_vs_L,flag_uniform_over_polar_a) ;
...
weight_3d_k_all_(1+ij_) = weight_k_sub_ * weight_3d_k_p_r_(1+nk_p_r) / max(1e-12,k_p_r).^2;
...
disp(sprintf(' %% sample_sphere_7: k_p_r_max: %0.2f; k_eq_d %0.2f; n_k_all %.8d n_k_shell %.6d flag_uniform_over_n_k_p_r %d flag_uniform_over_polar_a %d str_T_vs_L %s; error %0.16f',k_p_r_max,k_eq_d,n_k_all,n_k_all_csum_(end)-n_k_all_csum_(end-1),flag_uniform_over_n_k_p_r,flag_uniform_over_polar_a,str_T_vs_L,fnorm(Ix-Iq)/max(1e-12,fnorm(Ix)))); 
end;%if (verbose>0); 
