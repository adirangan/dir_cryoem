...
% TorL: string either 'T' for tschebycheff or 'L' [default] for legendre quadrature in the polar_a-direction (errors are comparable). ;
% ;
% Outputs: ;
...
sample_sphere_7(verbose,k_p_r_max,k_eq_d,'T');
sample_sphere_7(verbose,k_p_r_max,k_eq_d,'L');
end;%for nk_eq_d = 1:n_k_eq_d;
disp('returning'); return;
end;%if (nargin<1);

if (nargin<4); TorL = []; end;
...
[n_k_all_csum] = sample_shell_5(k_p_r,k_eq_d,TorL) ;
...
for nk_p_r=0:n_k_p_r-1;
k_p_r = k_p_r_(1+nk_p_r);
[n_k_all_csum,k_p_azimu_b_sub_,k_p_polar_a_sub_,weight_k_sub_] = sample_shell_5(k_p_r,k_eq_d,TorL) ;
...
disp(sprintf(' %% sample_sphere_7: k_p_r_max: %0.2f; k_eq_d %0.2f; n_k_all %d n_k_shell %d TorL %s; error %0.16f',k_p_r_max,k_eq_d,n_k_all,n_k_all_csum_(end)-n_k_all_csum_(end-1),TorL,fnorm(Ix-Iq)/fnorm(Ix))); 
end;%if (verbose>0); 
