function [ FA_x_c , X_1_ , X_2_ , X_3_ ]  = spharm_to_fig_4(n_k,k_,n_l_,a_,res,alpha_,delta_);
...
[FA_x_c_nu3d1,ier] = nufft3d1(numel(FA_k_c_p),pi*K_1_p_(:)/res,pi*K_2_p_(:)/res,pi*K_3_p_(:)/res,FA_k_c_p(:),-1,1e-12,m3d1,m3d1,m3d1);
FA_x_c_nu3d1 = real(permute(reshape(FA_x_c_nu3d1,m3d1,m3d1,m3d1),[2,1,3]))*numel(FA_k_c_p); FA_x_c_nu3d1 = FA_x_c_nu3d1(ij_tmp,ij_tmp,ij_tmp);
...
[FA_x_c_nu3d3,ier] = nufft3d3(n_all,kx_all_resampled_,ky_all_resampled_,kz_all_resampled_,MOL_A_K_P_,-1,1e-12,n_m,M_X_(:),M_Y_(:),M_Z_(:));
FA_x_c_nu3d3 = real(reshape(FA_x_c_nu3d3,res,res,res));

if (verbose>-1); disp(sprintf(' %% res %d: numel FA_k_c_p %d , numel MOL_A_K_P_ %d , FA_x_c/nu_3d1 %f , nu_3d3/FA_x_c %f , nu_3d3/nu_3d1 %f',res,numel(FA_k_c_p),numel(MOL_A_K_P_), mean(FA_x_c(:))./mean(FA_x_c_nu3d1(:)) , mean(FA_x_c_nu3d3(:))./mean(FA_x_c(:)) , mean(FA_x_c_nu3d3(:))./mean(FA_x_c_nu3d1(:)) )); end;

[FA_x_c_nu3d3,ier] = nufft3d3(numel(FA_k_c_p),pi*K_1_p_(:)/res,pi*K_2_p_(:)/res,pi*K_3_p_(:)/res,FA_k_c_p(:),-1,1e-12,n_m,M_X_(:),M_Y_(:),M_Z_(:));
FA_x_c_nu3d3 = real(reshape(FA_x_c_nu3d3,res,res,res));
if (verbose>-1); disp(sprintf(' %% res %d: numel FA_k_c_p %d , numel MOL_A_K_P_ %d , FA_x_c/nu_3d1 %f , nu_3d3/FA_x_c %f , nu_3d3/nu_3d1 %f',res,numel(FA_k_c_p),numel(MOL_A_K_P_), mean(FA_x_c(:))./mean(FA_x_c_nu3d1(:)) , mean(FA_x_c_nu3d3(:))./mean(FA_x_c(:)) , mean(FA_x_c_nu3d3(:))./mean(FA_x_c_nu3d1(:)) )); end;
...
for nv=length(v_):-1:1;
v = v_(nv);
hpatch = patch(isosurface(X_1_,X_2_,X_3_,FA_x_c_nu3d1,v)); isonormals(X_1_,X_2_,X_3_,FA_x_c_nu3d1,hpatch);
nc = max(1,min(ncra,floor(ncra*nv/length(v_))));
hpatch.FaceColor = cra(nc,:);
hpatch.EdgeColor = 'none';
hpatch.FaceAlpha = (nv/length(v_)).^4 * 1.0;
...
for nv=length(v_):-1:1;
v = v_(nv);
hpatch = patch(isosurface(X_1_,X_2_,X_3_,FA_x_c_nu3d3,v)); isonormals(X_1_,X_2_,X_3_,FA_x_c_nu3d3,hpatch);
nc = max(1,min(ncra,floor(ncra*nv/length(v_))));
hpatch.FaceColor = cra(nc,:);
hpatch.EdgeColor = 'none';
hpatch.FaceAlpha = (nv/length(v_)).^4 * 1.0;
