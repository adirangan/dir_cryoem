...
%%%%%%%%;
% The final formula are tagged with:
% [TAG_NEGATIVE_LOG_LIKELIHOOD_MU_COMPLEX] ; %<-- negative-log-likelihood after marginalization over complex mu, real nu and positive sigma. ;
% [TAG_SIGMA_OPT_MU_COMPLEX] ; %<-- optimal sigma after marginalization over complex mu, real nu. ;
% [TAG_NEGATIVE_LOG_LIKELIHOOD_MU_REAL] ; %<-- negative-log-likelihood after marginalization over real mu, real nu and positive sigma. ;
% [TAG_SIGMA_OPT_MU_REAL] ; %<-- optimal sigma after marginalization over real mu, real nu. ;
% [TAG_SCALEPLANE_REAL] ; %<-- marginalization over two scaling factors. ;
% [TAG_SCALEHYPER_REAL] ; %<-- marginalization over a hyperbolic scaling factor. ;
% [TAG_SUBSPHERE_REAL] ; %<-- marginalization over a real offset and hyperbolic scaling factor. ;
%%%%%%%%;
...
ssnll_nu_function_0 = @(nu_) ... %<-- one version of the ssnll_nu_function. ;
  0.5*(I_A*nu_.^2 -2*I_2B*nu_ + I_C)./max(1e-12,I_11) ...
  ;
%%%%%%%%;
...
%%%%%%%%;
likelihood_mu_nu_sigma_function_0 = @(mu_r_,mu_i_,nu_,sigma) exp(-ssnll_mu_nu_function_0(mu_r_,mu_i_,nu_)./max(1e-12,sigma.^2)) .* (2*pi*sigma.^2).^(-n_pixel/2) ; %<-- here is the standard version of the likelihood_mu_nu_sigma_function. ;
...
%%%%%%%%;
likelihood_nu_sigma_function_0 = @(nu_,sigma_) exp(-ssnll_nu_function_0(nu_)./max(1e-12,sigma_.^2)) .* (2*pi*sigma_.^2).^(-n_pixel/2) .* (2*pi*sigma_.^2./max(1e-12,I_11)) ; %<-- Here is one version of the likelihood_nu_sigma_function. ;
likelihood_nu_sigma_function_1 = @(nu_,sigma_) exp(-ssnll_nu_function_1(nu_)./max(1e-12,sigma_.^2)) .* (2*pi*sigma_.^2).^(-n_pixel/2) .* (2*pi*sigma_.^2./max(1e-12,I_11)) ; %<-- Here is a slightly different version of the likelihood_nu_sigma_function which uses the more expressive ssnll_nu_function. ;
...
likelihood_sigma_form_1 = sqrt(2*pi*sigma^2*I_11/max(1e-12,I_A))*exp((I_2B^2/max(1e-12,2*sigma^2*I_11*I_A)))*exp(-I_C/max(1e-12,2*sigma^2*I_11)) .* (2*pi*sigma.^2).^(-n_pixel/2) .* (2*pi*sigma.^2./max(1e-12,I_11));
likelihood_sigma_form_2 = sqrt(2*pi*sigma^2/max(1e-12,I_xcenxcen))*exp(-ssnll_2/max(1e-12,sigma^2)) .* (2*pi*sigma.^2).^(-n_pixel/2) .* (2*pi*sigma.^2./max(1e-12,I_11));
%%%%%%%%;
...
%%%%%%%%;
likelihood_sigma_function_2 = @(sigma_) sqrt(2*pi*sigma_.^2./max(1e-12,I_xcenxcen)).*exp(-ssnll_2./max(1e-12,sigma_.^2)) .* (2*pi*sigma_.^2).^(-n_pixel/2) .* (2*pi*sigma_.^2./max(1e-12,I_11)); %<-- Here is an expressive version of likelihood_sigma. ;
...
likelihood_form_2 = (2*pi)^(3/2-n_pixel/2) * (I_xcenxcen).^(-1/2) * 0.5 * (ssnll_2).^(2 - n_pixel/2) * gamma(n_pixel/2 - 2) / max(1e-12,I_11);
%%%%%%%%;
% We compare the results of numerical-quadrature to the analytical formulae. ;
%%%%%%%%;
...
ssnll_2 = 0.5*I_ycenycen*I_sinthetasquared ; %<-- a simple representation of the ssnll_function. ;
%%%%%%%%;
% Now we set up a linear-quadrature grid and weights for mu_ := mu_r_. ;
%%%%%%%%;
mu_lim_ = 5.0*[-1,+1];
n_mu_q = 256*8;
[mu_value_q_,mu_weight_q_] = legpts(n_mu_q,mu_lim_);
%%%%%%%%;
likelihood_mu_nu_sigma_function_0 = @(mu_r_,nu_,sigma) exp(-ssnll_mu_nu_function_0(mu_r_,nu_)./max(1e-12,sigma.^2)) .* (2*pi*sigma.^2).^(-n_pixel/2) ; %<-- here is the standard version of the likelihood_mu_nu_sigma_function. ;
...
%%%%%%%%;
likelihood_nu_sigma_function_1 = @(nu_,sigma_) exp(-ssnll_nu_function_1(nu_)./max(1e-12,sigma_.^2)) .* (2*pi*sigma_.^2).^(-n_pixel/2) .* sqrt(2*pi*sigma_.^2./max(1e-12,I_11)) ; %<-- Here is a slightly different version of the likelihood_nu_sigma_function which uses the more expressive ssnll_nu_function. ;
...
%%%%%%%%;
likelihood_sigma_quad_1 = sum(bsxfun(@times,likelihood_nu_sigma_function_1(nu_value_q_,sigma),reshape(nu_weight_q_,[n_nu_q,1])),'all');
likelihood_sigma_form_2 = sqrt(2*pi*sigma^2/max(1e-12,I_xcenxcen))*exp(-ssnll_2/max(1e-12,sigma^2)) .* (2*pi*sigma.^2).^(-n_pixel/2) .* sqrt(2*pi*sigma.^2./max(1e-12,I_11));
%%%%%%%%;
fnorm_disp(flag_verbose,'likelihood_sigma_form_2',likelihood_sigma_form_2,'likelihood_sigma_quad_1',likelihood_sigma_quad_1,' %<-- should be small');
%%%%%%%%;
sigma_lim_ = 16*[0,+1];
...
[sigma_value_q_,sigma_weight_q_] = legpts(n_sigma_q,sigma_lim_);
%%%%%%%%;
likelihood_sigma_function_2 = @(sigma_) sqrt(2*pi*sigma_.^2./max(1e-12,I_xcenxcen)).*exp(-ssnll_2./max(1e-12,sigma_.^2)) .* (2*pi*sigma_.^2).^(-n_pixel/2) .* sqrt(2*pi*sigma_.^2./max(1e-12,I_11)); %<-- Here is an expressive version of likelihood_sigma. ;
likelihood_quad_2 = sum(bsxfun(@times,likelihood_sigma_function_2(sigma_value_q_),reshape(sigma_weight_q_,[n_sigma_q,1])),'all');
likelihood_form_2 = (2*pi)^(2/2-n_pixel/2) * (I_xcenxcen).^(-1/2) * 0.5 * (ssnll_2).^(3/2 - n_pixel/2) * gamma(n_pixel/2 - 3/2) / max(1e-12,sqrt(I_11));
%%%%%%%%;
fnorm_disp(flag_verbose,'likelihood_form_2',likelihood_form_2,'likelihood_quad_2',likelihood_quad_2,' %<-- should be small');
%%%%%%%%;
...
% We also make sure to construct these functions so that both the image and the ctf*template are real when represented in real-space. ;
% More specifically, negating k_c_ (or rotating k_p_ by pi/2) should flip the sign of the imaginary-component: ;
%  M_k_p_wk__(1+nw+1*n_w_(1+nk_p_r)/2,1+nk_p_r) = conj(M_k_p_wk__(1+nw+0*n_w_(1+nk_p_r)/2,1+nk_p_r. ;
%%%%%%%%;
rng(2);
K = k_p_r_max;
phi_S = 2*pi*rand();
...
CTF_k_p_wk_ = i*2*k_p_r_wk_.*cos(k_p_w_wk_ - phi_S);
T_k_p_wk_ = CTF_k_p_wk_ .* S_k_p_wk_ ;
M_k_p_wk_ = i*2*k_p_r_wk_.*cos(k_p_w_wk_ - phi_M).*exp(2*pi*i*k_p_r_wk_.*delta_M.*cos(k_p_w_wk_ - omega_M));
...
ssnll_nu_function = @(nu_,sigma_) ... %<-- This is the expressive version of ssnll_nu. ;
  ...
  ;
likelihood_nu_sigma_function = @(nu_,sigma_) exp(-ssnll_nu_function(nu_,sigma_)./max(1e-12,sigma_.^2)) .* (2*pi*max(1e-12,sigma_.^2)).^(-n_pixel/2) .* (2*pi*sigma_.^2./max(1e-12,I_E_E_form)) ; %<-- This is the expressive version of likelihood_nu_sigma. ;
%%%%%%%%;
...
%%%%%%%%;
n_sigma_q_sub = 32; [sigma_value_q_sub_,sigma_weight_q_sub_] = legpts(n_sigma_q_sub,[0,1]);
n_sigma_max = 128*8; sigma_lim_ = n_sigma_max*[0,+1];
...
%%%%%%%%;
% We can examine the formula for likelihood_sigma and find the maximizing sigma-value. ;
% To demonstrate this we simply re-use the functions set up above.
% [TAG_SIGMA_OPT_MU_COMPLEX] ;
%%%%%%%%;
tmp_M = n_pixel - 3; %<-- note that tmp_M is different than the real-case below. ;
sigma_opt = sqrt(2*ssnll_function/max(1,tmp_M)); %<-- note that the ssnll_function is different than the real-case below. ;
if flag_disp;
plot(sigma_value_q_,likelihood_sigma_function(sigma_value_q_),'ko-',sigma_opt,likelihood_sigma_function(sigma_opt),'rx');
xlim([0,8]); xlabel('sigma');
ylabel('likelihood_sigma','Interpreter','none');
end;%if flag_disp;

%%%%%%%%;
% We can sketch out the calculation in likelihood.py (20250316). ;
%%%%%%%%;
WST_Iss = I_E_E_form;
WST_Isy = I_E_T_form;
WST_Isx = I_E_M_form;
...
WST_Ixy = I_M_T_form;
WST_Iyy = I_T_T_form;
WST_A = - abs(WST_Isx).^2 + WST_Ixx * WST_Iss;
WST_B = - real(WST_Isx) * real(WST_Isy) - imag(WST_Isx) * imag(WST_Isy) + WST_Ixy * WST_Iss;
WST_C = + abs(WST_Isy).^2 - WST_Iyy * WST_Iss;
WST_D = - (WST_B.^2/max(1e-12,WST_A) + WST_C);
WST_p = n_pixel / 2.0 - 2.0;
WST_constant = (3.0 - n_pixel) / 2.0 * log(2 * pi) - log(2) - 0.5 * log(WST_Iss) + gammaln(n_pixel / 2.0 - 2.0) + WST_p * log(2 * WST_Iss);
WST_log_likelihood = -WST_p * log(WST_D) - 0.5 * log(WST_A) + WST_constant;
WST_negative_log_likelihood = -WST_log_likelihood;
fnorm_disp(flag_verbose,'negative_log_likelihood_form',negative_log_likelihood_form,'WST_negative_log_likelihood',WST_negative_log_likelihood,'%<-- should be zero');

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
% Now we set up a similar test, but integrate over only real mu. ;
% We re-use the settings established above for the image-template test. ;
% Note that we can keep the same image, template, CTF and mask used above, ;
% since can still integrate over nu real, ;
% and T_x_c_xx__, M_x_c_xx__ and E_x_c_xx__ should all be real. ;
...
ssnll_nu_function = @(nu_,sigma_) ... %<-- This is the expressive version of ssnll_nu. ;
  0.5*(I_Tcen_Tcen_form + nu_.^2*I_Mcen_Mcen_form - nu_*I_Tcen_Mcen_form - nu_*I_Mcen_Tcen_form) ...
  ;
likelihood_nu_sigma_function = @(nu_,sigma_) exp(-ssnll_nu_function(nu_,sigma_)./max(1e-12,sigma_.^2)) .* (2*pi*max(1e-12,sigma_.^2)).^(-n_pixel/2) .* sqrt(2*pi*sigma_.^2./max(1e-12,I_E_E_form)) ; %<-- This is the expressive version of likelihood_nu_sigma. ;
%%%%%%%%;
sigma_value = 1.3; nu_value = 0.8;
...
%%%%;
likelihood_nu_sigma_quad = sum(bsxfun(@times,likelihood_mu_nu_sigma_q_,reshape(mu_weight_q_,[n_mu_q,1])),'all') ;
likelihood_nu_sigma_form = likelihood_nu_sigma_function(nu_value,sigma_value);
fnorm_disp(flag_verbose,'likelihood_nu_sigma_form',likelihood_nu_sigma_form,'likelihood_nu_sigma_quad',likelihood_nu_sigma_quad,' %<-- should be small as long as quadrature includes support');
%%%%%%%%;
nu_lim_ = 16*[-1,+1];
...
%%%%%%%%;
n_sigma_q_sub = 32; [sigma_value_q_sub_,sigma_weight_q_sub_] = legpts(n_sigma_q_sub,[0,1]);
n_sigma_max = 32*8; sigma_lim_ = n_sigma_max*[0,+1];
n_sigma_q = n_sigma_q_sub*n_sigma_max;
sigma_value_q_ = reshape(bsxfun(@plus,sigma_value_q_sub_,[0:n_sigma_max-1]),[n_sigma_q,1]);
sigma_weight_q_ = repmat(sigma_weight_q_sub_,[1,n_sigma_max]);
%%%%%%%%;
...
% We can examine the formula for likelihood_sigma and find the maximizing sigma-value. ;
% To demonstrate this we simply re-use the functions set up above.
% [TAG_SIGMA_OPT_MU_REAL] ;
%%%%%%%%;
tmp_M = n_pixel - 2; %<-- note that tmp_M is different than the complex-case above. ;
sigma_opt = sqrt(2*ssnll_function/max(1,tmp_M)); %<-- note that the ssnll_function is different than the complex-case above. ;
if flag_disp;
plot(sigma_value_q_,likelihood_sigma_function(sigma_value_q_),'ko-',sigma_opt,likelihood_sigma_function(sigma_opt),'rx');
xlim([0,8]); xlabel('sigma');
ylabel('likelihood_sigma','Interpreter','none');
...
%%%%%%%%;
% Now we test 2-dimensional intensity marginalization over nu, ku. ;
% Here the image and signal are strictly real. ;
% [TAG_SCALEPLANE_REAL] ;
%%%%%%%%;
n_pixel = 7;
%%%%%%%%;
y_ = 0.25*randn(n_pixel,1); %<-- signal. ;
y0 = y_(1+0);
y1 = y_(1+1);
y2 = y_(1+2);
y3 = y_(1+3);
...
nu = 0.2; ku = 0.5;
%%%%%%%%;
ssnll_nu_ku_function_0 = @(nu_,ku_) ... %<-- a standard definition for the ssnll_nu_ku_function. ;
  + 0.5*conj(ku_*y0 - nu_*x0).*(ku_*y0 - nu_*x0) ...
  + 0.5*conj(ku_*y1 - nu_*x1).*(ku_*y1 - nu_*x1) ...
  + 0.5*conj(ku_*y2 - nu_*x2).*(ku_*y2 - nu_*x2) ...
  + 0.5*conj(ku_*y3 - nu_*x3).*(ku_*y3 - nu_*x3) ...
  + 0.5*conj(ku_*y4 - nu_*x4).*(ku_*y4 - nu_*x4) ...
  + 0.5*conj(ku_*y5 - nu_*x5).*(ku_*y5 - nu_*x5) ...
  + 0.5*conj(ku_*y6 - nu_*x6).*(ku_*y6 - nu_*x6) ...
  ;
%%%%%%%%;
I_yy = sum(conj(y_).*y_,'all');
I_xy = sum(conj(x_).*y_,'all');
...
I_xx = sum(conj(x_).*x_,'all');
%%%%%%%%;
ynrm_ = y_/max(1e-12,sqrt(I_yy)); %<-- normalized signal. ;
xnrm_ = x_/max(1e-12,sqrt(I_xx)); %<-- normalized image. ;
I_ynrmxnrm = sum(conj(ynrm_).*xnrm_,'all');
I_xnrmynrm = sum(conj(xnrm_).*ynrm_,'all');
%%%%%%%%;
I_costheta = 0.5*(I_xnrmynrm + I_ynrmxnrm); %<-- similarity measure. ;
I_sinthetasquared = 1 - I_costheta.^2; %<-- dis-similarity measure. ;
I_sintheta = sqrt(I_sinthetasquared);
%%%%%%%%;
ssnll_0 = sqrt(I_yy)*sqrt(I_xx)*I_sintheta ; %<-- a simple representation of the ssnll_function. ;
%%%%%%%%;
nu_lim_ = 16*[-1,+1];
n_nu_q = 256;
[nu_value_q_,nu_weight_q_] = legpts(n_nu_q,nu_lim_);
ku_lim_ = 16*[-1,+1];
n_ku_q = 256;
[ku_value_q_,ku_weight_q_] = legpts(n_ku_q,ku_lim_);
[nu_value_qq__,ku_value_qq__] = ndgrid(nu_value_q_,ku_value_q_);
nu_ku_weight_qq__ = reshape(nu_weight_q_,[n_nu_q,1])*reshape(ku_weight_q_,[1,n_ku_q]);
ssnll_nu_ku_qq__ = ssnll_nu_ku_function_0(nu_value_qq__,ku_value_qq__);
likelihood_nu_ku_sigma_qq__ = (2*pi*sigma.^2).^(-n_pixel/2) .* exp(-ssnll_nu_ku_qq__./max(1e-12,sigma.^2));
likelihood_sigma_quad_0 = sum(likelihood_nu_ku_sigma_qq__.*nu_ku_weight_qq__,'all');
likelihood_sigma_form_0 = (2*pi*sigma.^2).^(-(n_pixel-2)/2) ./ max(1e-12,ssnll_0);
%%%%%%%%;
fnorm_disp(flag_verbose,'likelihood_sigma_form_0',likelihood_sigma_form_0,'likelihood_sigma_quad_0',likelihood_sigma_quad_0,' %<-- should be small');
%%%%%%%%;

%%%%%%%%;
% Now we test hyperbolic intensity marginalization over nu. ;
% Here the image and signal are strictly real. ;
% [TAG_SCALEHYPER_REAL] ;
%%%%%%%%;
n_pixel = 7;
...
nu = 0.2;
%%%%%%%%;
ssnll_nu_function_0 = @(nu_) ... %<-- a standard definition for the ssnll_nu_function. ;
  + 0.5*conj((1.0./nu_)*y0 - nu_*x0).*((1.0./nu_)*y0 - nu_*x0) ...
  + 0.5*conj((1.0./nu_)*y1 - nu_*x1).*((1.0./nu_)*y1 - nu_*x1) ...
  + 0.5*conj((1.0./nu_)*y2 - nu_*x2).*((1.0./nu_)*y2 - nu_*x2) ...
  + 0.5*conj((1.0./nu_)*y3 - nu_*x3).*((1.0./nu_)*y3 - nu_*x3) ...
  + 0.5*conj((1.0./nu_)*y4 - nu_*x4).*((1.0./nu_)*y4 - nu_*x4) ...
  + 0.5*conj((1.0./nu_)*y5 - nu_*x5).*((1.0./nu_)*y5 - nu_*x5) ...
  + 0.5*conj((1.0./nu_)*y6 - nu_*x6).*((1.0./nu_)*y6 - nu_*x6) ...
  ;
ssnll_un_function_0 = @(un_) ... %<-- a standard definition for the ssnll_un_function. ;
  + 0.5*conj(un_*y0 - (1.0./un_)*x0).*(un_*y0 - (1.0./un_)*x0) ...
  + 0.5*conj(un_*y1 - (1.0./un_)*x1).*(un_*y1 - (1.0./un_)*x1) ...
  + 0.5*conj(un_*y2 - (1.0./un_)*x2).*(un_*y2 - (1.0./un_)*x2) ...
  + 0.5*conj(un_*y3 - (1.0./un_)*x3).*(un_*y3 - (1.0./un_)*x3) ...
  + 0.5*conj(un_*y4 - (1.0./un_)*x4).*(un_*y4 - (1.0./un_)*x4) ...
  + 0.5*conj(un_*y5 - (1.0./un_)*x5).*(un_*y5 - (1.0./un_)*x5) ...
  + 0.5*conj(un_*y6 - (1.0./un_)*x6).*(un_*y6 - (1.0./un_)*x6) ...
  ;
%%%%%%%%;
I_yy = sum(conj(y_).*y_,'all');
I_xy = sum(conj(x_).*y_,'all');
...
I_costheta = 0.5*(I_xnrmynrm + I_ynrmxnrm); %<-- similarity measure. ;
%%%%%%%%;
ssnll_0 = sqrt(I_yy)*sqrt(I_xx)*(1-I_costheta) ; %<-- a simple representation of the ssnll_function. ;
%%%%%%%%;
nu_lim_ = 4*[1e-3,+1];
n_nu_q = 256;
[nu_value_q_,nu_weight_q_] = legpts(n_nu_q,nu_lim_);
un_lim_ = 4*[1e-3,+1];
n_un_q = 256;
[un_value_q_,un_weight_q_] = legpts(n_un_q,un_lim_);
ssnll_nu_q_ = ssnll_nu_function_0(nu_value_q_);
likelihood_nu_sigma_q_ = (2*pi*sigma.^2).^(-n_pixel/2) .* exp(-ssnll_nu_q_./max(1e-12,sigma.^2));
ssnll_un_q_ = ssnll_un_function_0(un_value_q_);
likelihood_un_sigma_q_ = (2*pi*sigma.^2).^(-n_pixel/2) .* exp(-ssnll_un_q_./max(1e-12,sigma.^2));
likelihood_sigma_quad_0 = sum(likelihood_nu_sigma_q_.*reshape(nu_weight_q_,[n_nu_q,1]),'all') + sum(likelihood_un_sigma_q_.*reshape(un_weight_q_,[n_un_q,1]),'all');
likelihood_sigma_form_0 = (2*pi*sigma.^2).^(-(n_pixel-1)/2) .* 0.5 .* (1./max(1e-12,sqrt(I_yy)) + 1./max(1e-12,sqrt(I_xx))) .* exp(-ssnll_0/max(1e-12,sigma.^2));
%%%%%%%%;
fnorm_disp(flag_verbose,'likelihood_sigma_form_0',likelihood_sigma_form_0,'likelihood_sigma_quad_0',likelihood_sigma_quad_0,' %<-- should be small');
%%%%%%%%;

%%%%%%%%;
% Now we test marginalization over real mu and hyperbolic intensity marginalization over nu. ;
% Here the image and signal are strictly real. ;
% [TAG_SUBSPHERE_REAL] ;
%%%%%%%%;
n_pixel = 7;
...
e6 = e_(1+6);
%%%%%%%%;
ssnll_mu_nu_function_0 = @(mu_,nu_) ... %<-- a standard definition for the ssnll_mu_nu_function. ;
  + 0.5*conj((1.0./nu_)*y0 - nu_*x0 - (mu_)*e0).*((1.0./nu_)*y0 - nu_*x0 - (mu_)*e0) ...
  + 0.5*conj((1.0./nu_)*y1 - nu_*x1 - (mu_)*e1).*((1.0./nu_)*y1 - nu_*x1 - (mu_)*e1) ...
  + 0.5*conj((1.0./nu_)*y2 - nu_*x2 - (mu_)*e2).*((1.0./nu_)*y2 - nu_*x2 - (mu_)*e2) ...
  + 0.5*conj((1.0./nu_)*y3 - nu_*x3 - (mu_)*e3).*((1.0./nu_)*y3 - nu_*x3 - (mu_)*e3) ...
  + 0.5*conj((1.0./nu_)*y4 - nu_*x4 - (mu_)*e4).*((1.0./nu_)*y4 - nu_*x4 - (mu_)*e4) ...
  + 0.5*conj((1.0./nu_)*y5 - nu_*x5 - (mu_)*e5).*((1.0./nu_)*y5 - nu_*x5 - (mu_)*e5) ...
  + 0.5*conj((1.0./nu_)*y6 - nu_*x6 - (mu_)*e6).*((1.0./nu_)*y6 - nu_*x6 - (mu_)*e6) ...
  ;
ssnll_mu_ou_function_0 = @(mu_,ou_) ... %<-- a standard definition for the ssnll_mu_ou_function. ;
  + 0.5*conj(ou_*y0 - (1.0./ou_)*x0 - (mu_)*e0).*(ou_*y0 - (1.0./ou_)*x0 - (mu_)*e0) ...
  + 0.5*conj(ou_*y1 - (1.0./ou_)*x1 - (mu_)*e1).*(ou_*y1 - (1.0./ou_)*x1 - (mu_)*e1) ...
  + 0.5*conj(ou_*y2 - (1.0./ou_)*x2 - (mu_)*e2).*(ou_*y2 - (1.0./ou_)*x2 - (mu_)*e2) ...
  + 0.5*conj(ou_*y3 - (1.0./ou_)*x3 - (mu_)*e3).*(ou_*y3 - (1.0./ou_)*x3 - (mu_)*e3) ...
  + 0.5*conj(ou_*y4 - (1.0./ou_)*x4 - (mu_)*e4).*(ou_*y4 - (1.0./ou_)*x4 - (mu_)*e4) ...
  + 0.5*conj(ou_*y5 - (1.0./ou_)*x5 - (mu_)*e5).*(ou_*y5 - (1.0./ou_)*x5 - (mu_)*e5) ...
  + 0.5*conj(ou_*y6 - (1.0./ou_)*x6 - (mu_)*e6).*(ou_*y6 - (1.0./ou_)*x6 - (mu_)*e6) ...
  ;
%%%%%%%%;
I_yy = sum(conj(y_).*y_,'all');
I_xy = sum(conj(x_).*y_,'all');
...
I_11 = sum(conj(e_).*e_,'all');
%%%%%%%%;
ybar_ = I_1y*e_/max(1e-12,I_11); %<-- signal average. ;
xbar_ = I_1x*e_/max(1e-12,I_11); %<-- image average. ;
ycen_ = y_ - ybar_; %<-- value-centered signal. ;
xcen_ = x_ - xbar_; %<-- value-centered image. ;
I_ycenycen = sum(conj(ycen_).*ycen_,'all');
I_xcenycen = sum(conj(xcen_).*ycen_,'all');
I_ycenxcen = sum(conj(ycen_).*xcen_,'all');
I_xcenxcen = sum(conj(xcen_).*xcen_,'all');
%%%%%%%%;
ynrm_ = ycen_/max(1e-12,sqrt(I_ycenycen)); %<-- normalized signal. ;
xnrm_ = xcen_/max(1e-12,sqrt(I_xcenxcen)); %<-- normalized image. ;
I_ynrmxnrm = sum(conj(ynrm_).*xnrm_,'all');
I_xnrmynrm = sum(conj(xnrm_).*ynrm_,'all');
%%%%%%%%;
...
I_sinthetasquared = 1 - I_costheta.^2; %<-- dis-similarity measure. ;
%%%%%%%%;
ssnll_0 = sqrt(I_ycenycen)*sqrt(I_xcenxcen)*(1-I_costheta) ; %<-- a simple representation of the ssnll_function. ;
%%%%%%%%;
mu_lim_ = 16*[-1,+1];
n_mu_q = 512;
[mu_value_q_,mu_weight_q_] = legpts(n_mu_q,mu_lim_);
...
ou_lim_ = 4*[1e-3,+1];
n_ou_q = 256;
[ou_value_q_,ou_weight_q_] = legpts(n_ou_q,ou_lim_);
[mu_value_mn__,nu_value_mn__] = ndgrid(mu_value_q_,nu_value_q_);
mu_nu_weight_mn__ = reshape(mu_weight_q_,[n_mu_q,1])*reshape(nu_weight_q_,[1,n_nu_q]);
[mu_value_mo__,ou_value_mo__] = ndgrid(mu_value_q_,ou_value_q_);
mu_ou_weight_mo__ = reshape(mu_weight_q_,[n_mu_q,1])*reshape(ou_weight_q_,[1,n_ou_q]);
ssnll_mu_nu_mn__ = ssnll_mu_nu_function_0(mu_value_mn__,nu_value_mn__);
likelihood_mu_nu_sigma_mn__ = (2*pi*sigma.^2).^(-n_pixel/2) .* exp(-ssnll_mu_nu_mn__./max(1e-12,sigma.^2));
ssnll_mu_ou_mo__ = ssnll_mu_ou_function_0(mu_value_mo__,ou_value_mo__);
likelihood_mu_ou_sigma_mo__ = (2*pi*sigma.^2).^(-n_pixel/2) .* exp(-ssnll_mu_ou_mo__./max(1e-12,sigma.^2));
likelihood_sigma_quad_0 = sum(likelihood_mu_nu_sigma_mn__.*mu_nu_weight_mn__,'all') + sum(likelihood_mu_ou_sigma_mo__.*mu_ou_weight_mo__,'all');
likelihood_sigma_form_0 = (2*pi*sigma.^2).^(-(n_pixel-2)/2) .* 0.5 .* (1./max(1e-12,sqrt(I_ycenycen)) + 1./max(1e-12,sqrt(I_xcenxcen))) .* exp(-ssnll_0/max(1e-12,sigma.^2)) / max(1e-12,sqrt(I_11));
%%%%%%%%;
fnorm_disp(flag_verbose,'likelihood_sigma_form_0',likelihood_sigma_form_0,'likelihood_sigma_quad_0',likelihood_sigma_quad_0,' %<-- should be small');
%%%%%%%%;
