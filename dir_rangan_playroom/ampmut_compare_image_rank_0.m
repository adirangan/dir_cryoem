...
verbose=1;
if (verbose); disp(sprintf(' %% [entering ampmut_compare_image_rank_0]')); end;
...
image_X_value_Mi__ = tmp_.image_X_value__;
n_iteration_use = size(image_X_value_Mi__,2)-1;
R_k_p_l2_Mi__ = zeros(n_M,n_iteration_use);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
for niteration_use=0:n_iteration_use-1;
tmp_euler_polar_a_true_ = tmp_.euler_polar_a__(:,1+niteration_use+1);
tmp_euler_azimu_b_true_ = tmp_.euler_azimu_b__(:,1+niteration_use+1);
tmp_euler_gamma_z_true_ = tmp_.euler_gamma_z__(:,1+niteration_use+1);
tmp_image_delta_x_true_ = tmp_.image_delta_x_acc__(:,1+niteration_use+1) + tmp_.image_delta_x_upd__(:,1+niteration_use+1);
tmp_image_delta_y_true_ = tmp_.image_delta_y_acc__(:,1+niteration_use+1) + tmp_.image_delta_y_upd__(:,1+niteration_use+1);
tmp_image_X_value_true_ = tmp_.image_X_value__(:,1+niteration_use+0);
...
if (verbose); disp(sprintf(' %% [finished ampmut_compare_image_rank_0]')); end;
