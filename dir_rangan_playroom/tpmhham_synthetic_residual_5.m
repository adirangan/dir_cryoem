...
disp(sprintf(' %% tmp_a_k_Y_quad_mavg_ vs syn_a_k_Y_lsq0_: correlation %+0.6f',tmp_X_best));
 %}
%%%%%%%%;
% now try to run function to calculate loadings. ;
%%%%%%%%;
tmp_n_residual_loading = 3; tmp_n_residual_iteration = 32;
tmp_M_loading__ = get_loading_0(tmp_n_residual_loading,tmp_n_residual_iteration,syn_n_order,tmp_n_k_p_r,tmp_weight_k_p_r_,tmp_l_max_,tmp_n_w_,syn_n_M,tmp_M_k_p__,tmp_euler_polar_a_,tmp_euler_azimu_b_,tmp_euler_gamma_z_);
flag_plot=1;
if flag_plot;
fname_fig = sprintf('%s_jpg/tpmhham_synthetic_5_manual_s%.4dr%.3d_FIGA',dir_trunk,floor(1000*syn_snr),nUX_rank);
if (~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
clf;
c_ = colormap_beach(); n_c = size(c_,1);
nc_ = max(0,min(n_c-1,floor(n_c*syn_M_molecule_type_/n_molecule)));
scatter(tmp_M_loading__(1,:),tmp_M_loading__(2,:),64,c_(1+nc_,:),'filled','MarkerEdgeColor','k'); 
axisnotick;
figbig;
