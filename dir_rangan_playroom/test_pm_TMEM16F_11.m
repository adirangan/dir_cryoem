...
dir_pm = sprintf('/%s/rangan/dir_cryoem/dir_TMEM16F/dir_pm',string_root);
...
dir_relion = sprintf('/%s/rangan/dir_cryoem/dir_TMEM16F/dir_relion',string_root);
...
dir_data_star = sprintf('/%s/rangan/dir_cryoem/dir_TMEM16F',string_root);
Pixel_Spacing = 1.059; %<-- in angstroms, from https://www.ebi.ac.uk/pdbe/emdb/empiar/entry/10278/ ;
fname_nopath_volume = 'emd_20244.map';
fname_nopath_star = 'All_8192.star';
...
%%%%;
tmp_XB_align_a_k_Y_ = [];
tmp_XB_fname_pre = sprintf('/%s/rangan/dir_cryoem/dir_TMEM16F_x0/dir_pm_mat/X_2d_xcor_d0_a1t0065n18r1_align_a_k_Y_',string_root);
...
%%%%;
tmp_XB_a_k_Y_ = [];
tmp_XB_fname_pre = sprintf('/%s/rangan/dir_cryoem/dir_TMEM16F_x0/dir_pm_mat/X_2d_xcor_d0_a1t0065n18r1_a_k_Y_',string_root);
tmp_XB_fname_mat = sprintf('%s.mat',tmp_XB_fname_pre);
if (~exist(tmp_XB_fname_mat,'file'));
disp(sprintf(' %% Warning, %s not found',tmp_XB_fname_mat));
end;%if (~exist(tmp_XB_fname_mat,'file'));
...
%%%%%%%%;
% Now try croping, recentering, aligning and converting. ;
% Verdict: Not worth it!;
%%%%%%%%;
cor2_crop_ = zeros(n_crop,1);
for ncrop=0:n_crop-1;
if (verbose); disp(sprintf(' %% ncrop %d/%d',ncrop,n_crop)); end;
r_crop = 1.0*ncrop/(n_crop-1);
tmp_c_x_u_reco_ = tmp_b_x_u_reco_.*reshape(x_u_r___<=r_crop,[n_xxx_u,1]);
%%%%%%%%;
% Calculate moments. ;
%%%%%%%%;
tmp_c_x_u_reco_ = reshape(tmp_c_x_u_reco_,[n_x_u_pack,n_x_u_pack,n_x_u_pack]);
tmp_c_x_u_reco_ = tmp_c_x_u_reco_ + min(tmp_c_x_u_reco_,[],'all');
tmp_c_x_c_0_avg = sum(x_u_0___.^1.*tmp_c_x_u_reco_/sum(tmp_c_x_u_reco_,'all'),'all');
tmp_c_x_c_1_avg = sum(x_u_1___.^1.*tmp_c_x_u_reco_/sum(tmp_c_x_u_reco_,'all'),'all');
tmp_c_x_c_2_avg = sum(x_u_2___.^1.*tmp_c_x_u_reco_/sum(tmp_c_x_u_reco_,'all'),'all');
tmp_c_x_c_0_std = sum((x_u_0___ - tmp_c_x_c_0_avg).^2.*tmp_c_x_u_reco_/sum(tmp_c_x_u_reco_,'all'),'all');
tmp_c_x_c_1_std = sum((x_u_1___ - tmp_c_x_c_1_avg).^2.*tmp_c_x_u_reco_/sum(tmp_c_x_u_reco_,'all'),'all');
tmp_c_x_c_2_std = sum((x_u_2___ - tmp_c_x_c_2_avg).^2.*tmp_c_x_u_reco_/sum(tmp_c_x_u_reco_,'all'),'all');
tmp_c_x_c_avg_ = [tmp_c_x_c_0_avg ; tmp_c_x_c_1_avg ; tmp_c_x_c_2_avg];
tmp_c_x_c_std_ = [tmp_c_x_c_0_std ; tmp_c_x_c_1_std ; tmp_c_x_c_2_std];
disp(sprintf(' %% tmp_c_x_c_std_ vs tmp_c_x_c_avg_: %0.2f',fnorm(tmp_c_x_c_std_)/fnorm(tmp_c_x_c_avg_)));
if (max(abs(tmp_c_x_c_avg_))> diameter_x_c/n_x_u_pack);
disp(sprintf(' %% Warning! molecule may not be well centered. Consider recentering.'));
end;%if (max(abs(tmp_c_x_c_avg_))> diameter_x_c/n_x_u_pack);
%%%%%%%%;
% re-center. ;
%%%%%%%%;
k_u_0_ = periodize(0:n_x_u_pack-1,-n_x_u_pack/2,+n_x_u_pack/2)/2; %<-- box has diameter 2. ;
...
tmp_d_x_u_reco_ = real(ifftn(fftn(tmp_c_x_u_reco_).*exp(+i*2*pi*(k_u_0___*tmp_c_x_c_0_avg + k_u_1___*tmp_c_x_c_1_avg + k_u_2___*tmp_c_x_c_2_avg))));
tmp_d_x_c_0_avg = sum(x_u_0___.^1.*tmp_d_x_u_reco_/sum(tmp_d_x_u_reco_,'all'),'all');
tmp_d_x_c_1_avg = sum(x_u_1___.^1.*tmp_d_x_u_reco_/sum(tmp_d_x_u_reco_,'all'),'all');
tmp_d_x_c_2_avg = sum(x_u_2___.^1.*tmp_d_x_u_reco_/sum(tmp_d_x_u_reco_,'all'),'all');
tmp_d_x_c_0_std = sum((x_u_0___ - tmp_d_x_c_0_avg).^2.*tmp_d_x_u_reco_/sum(tmp_d_x_u_reco_,'all'),'all');
tmp_d_x_c_1_std = sum((x_u_1___ - tmp_d_x_c_1_avg).^2.*tmp_d_x_u_reco_/sum(tmp_d_x_u_reco_,'all'),'all');
tmp_d_x_c_2_std = sum((x_u_2___ - tmp_d_x_c_2_avg).^2.*tmp_d_x_u_reco_/sum(tmp_d_x_u_reco_,'all'),'all');
tmp_d_x_c_avg_ = [tmp_d_x_c_0_avg ; tmp_d_x_c_1_avg ; tmp_d_x_c_2_avg];
tmp_d_x_c_std_ = [tmp_d_x_c_0_std ; tmp_d_x_c_1_std ; tmp_d_x_c_2_std];
disp(sprintf(' %% tmp_d_x_c_std_ vs tmp_d_x_c_avg_: %0.2f',fnorm(tmp_d_x_c_std_)/fnorm(tmp_d_x_c_avg_)));
%%%%%%%%;
% convert and align. ;
%%%%%%%%;
eta = pi/x_p_r_max; tmp_t = tic;
tmp_d_k_p_reco_ = xxnufft3d3(n_xxx_u,x_u_0___(:)*eta,x_u_1___(:)*eta,x_u_2___(:)*eta,tmp_d_x_u_reco_(:).*xxx_u_weight_(:),-1,1e-12,n_k_all,2*pi*k_c_0_all_/eta,2*pi*k_c_1_all_/eta,2*pi*k_c_2_all_/eta)/sqrt(2*pi)/sqrt(2*pi)/sqrt(2*pi);
tmp_t = toc(tmp_t); disp(sprintf(' %% xxnufft3d3: tmp_d_k_p_reco_ time %0.2fs',tmp_t));
tmp_t = tic;
[tmp_d_k_Y_reco_] = convert_k_p_to_spharm_1(verbose,n_k_all,n_k_all_csum_,k_p_r_all_,k_p_azimu_b_all_,k_p_polar_a_all_,weight_3d_k_all_,weight_shell_k_,n_k_p_r,k_p_r_,weight_3d_k_p_r_,l_max_,tmp_d_k_p_reco_);
tmp_t = toc(tmp_t); disp(sprintf(' %% tmp_d_k_Y_reco_ time %0.2fs',tmp_t));
tmp_t = tic;
[ ... 
 tmp_e_k_Y_reco_ ...
,tmp_X_e_k_Y_best ...
...
tmp_t = toc(tmp_t); disp(sprintf(' %% tmp_e_k_Y_reco_ time %0.2fs',tmp_t));
tmp_t = tic();
[ ... 
  tmp_e_x_u_reco_ ...
] = ...
...
tmp_t = toc(tmp_t); disp(sprintf(' %% tmp_e_x_u_reco_ time %0.2fs',tmp_t));
tmp_X_e_x_u_best = real(corr(tmp_e_x_u_reco_(:),a_x_u_reco_(:)));
tmp_X_c_x_u_best = real(corr(tmp_c_x_u_reco_(:),a_x_u_reco_(:)));
disp(sprintf(' %% tmp_X_e_k_Y_best %0.3f',tmp_X_e_k_Y_best));
disp(sprintf(' %% tmp_X_e_x_u_best %0.3f',tmp_X_e_x_u_best));
disp(sprintf(' %% tmp_X_c_x_u_best %0.3f',tmp_X_c_x_u_best));
cor0_crop_(1+ncrop) = tmp_X_c_k_Y_best;
cor1_crop_(1+ncrop) = tmp_X_e_k_Y_best;
cor2_crop_(1+ncrop) = tmp_X_e_x_u_best;
end;%for ncrop=0:n_crop-1;
