...
verbose=1;
snr_per_A_ = 10.^[0:-0.5:-5]; n_snr_per_A = numel(snr_per_A_);
n_M_factor_ = 1:5; n_n_M_factor = numel(n_M_factor_);
n_pick_ = 1:10; n_n_pick = numel(n_pick_);
f_rand_ = [0.00,0.05]; n_f_rand = numel(f_rand_);
local_rseed_ = 0:2; n_local_rseed = numel(local_rseed_);
...
if (nargin<1+na);,snr_per_A=[]; end; na=na+1;
if (nargin<1+na);,n_M_factor_0in=[]; end; na=na+1;
if (nargin<1+na);,n_pick_0in=[]; end; na=na+1;
if (nargin<1+na);,f_rand_0in=[]; end; na=na+1;
if (nargin<1+na);,local_rseed=[]; end; na=na+1;
if isempty(verbose); verbose = 1; end;
if isempty(snr_per_A); snr_per_A = 0.005; end;
if isempty(n_M_factor_0in); n_M_factor_0in = 4; end;
if isempty(n_pick_0in); n_pick_0in = 2; end;
if isempty(f_rand_0in); f_rand_0in = 0.00; end;
if isempty(local_rseed); local_rseed = 1; end;
...
rng(global_rseed);
Pixel_A = 4.0;
%snr_per_A = 0.005; %<-- decrease to increase noise. ;
%n_M_factor_0in = 4; %<-- n_M = n_w_max * n_M_factor_0in. ;
%n_pick_0in = 2; %<-- strong overlap at n_pick_0in = ceil(sqrt(n_w_max)). ;
%f_rand_0in = 0.0;
%local_rseed = 1;
...
[fname_SR0A_skip,fname_SR0A_mat] = open_fname_tmp(fname_SR0A_pre);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
if ~fname_SR0A_skip;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
...
tmp_ = [ ...
	      blockletter_0('F',[6,1,1]) ...
             ,zeros(6*7,3) ...
	     ,blockletter_0('I',[6,1,1]) ...
             ,zeros(6*7,3) ...
	     ,blockletter_0('N',[6,1,1]) ...
             ,zeros(6*7,3) ...
	     ,blockletter_0('Y',[6,1,1]) ...
             ,zeros(6*7,3) ...
	     ,blockletter_0('U',[6,1,1]) ...
	 ];
...
x_sigma = 0.15;
a_x_u_ = 1/(sqrt(2*pi)*x_sigma).^2 * exp(-x_u_r__.^2/(2*x_sigma^2));
end;%if flag_gaussian;
...
subplot(1,2,1);imagesc(a_rho_x_u_,[0,1]); axis image; axisnotick;
subplot(1,2,2);imagesc(b_rho_x_u_,[0,1]); axis image; axisnotick;
end;%if flag_plot;
%%%%%%%%;
a_x_u_base_ = a_x_u_;
...
subplot(1,2,1); imagesc(a_x_u_,[-1,1]); title('a'); axis image; axisnotick;
subplot(1,2,2); imagesc(b_rho_x_u_,[-1,1]); title('b'); axis image; axisnotick;
...
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1+nf);nf=nf+1;clf;figmed;figbeach();
subplot(1,3,1);imagesc(a_x_u_base_,[0,1]); axis image; axisnotick;
title('a_x_u_base_','Interpreter','none');
subplot(1,3,2);imagesc(real(a_x_u_reco_),[0,1]); axis image; axisnotick;
title('a_x_u_reco_','Interpreter','none');
subplot(1,3,3);imagesc(real(a_x_u_flip_),[0,1]); axis image; axisnotick;
...
for nUX_rank=0:n_UX_rank-1;
tmp_M_pM_ = reshape(UX_M_k_p_pnM___(:,1+nUX_rank,:),[n_pick*n_M,1]);
UX_a_k_q_reco_wn__(:,1+nUX_rank) = a_from_M_qpM__ * tmp_M_pM_;
end;%for nUX_rank=0:n_UX_rank-1;
...
X_in__ = zeros(global_n_iteration,pm_n_UX_rank_max);
kld_in__ = zeros(global_n_iteration,pm_n_UX_rank_max);
nw_Min___ = zeros(n_M,global_n_iteration,pm_n_UX_rank_max);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
for pm_n_UX_rank=1:pm_n_UX_rank_max;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;

if (verbose>0); disp(sprintf(' %% %s: pm_n_UX_rank %d/%d',fname_SR0A_pre,pm_n_UX_rank,pm_n_UX_rank_max)); end;
...
UX_a_k_q_reco_wni___ = zeros(n_w_max,pm_n_UX_rank,n_iteration);
UX_a_k_p_reco_wni___ = zeros(n_w_max,pm_n_UX_rank,n_iteration);
...
M_from_a_pMq__ = reshape(M_from_a_pMq___,[n_pick*n_M,n_w_max]);
a_from_M_qpM__ = pinv(M_from_a_pMq__,1e-6);
UX_a_k_q_reco_wn__ = zeros(n_w_max,pm_n_UX_rank);
for nUX_rank=0:pm_n_UX_rank-1;
tmp_M_pM_ = reshape(UX_M_k_p_pnM___(:,1+nUX_rank,:),[n_pick*n_M,1]);
UX_a_k_q_reco_wn__(:,1+nUX_rank) = a_from_M_qpM__ * tmp_M_pM_;
end;%for nUX_rank=0:pm_n_UX_rank-1;
%%%%;
UX_a_k_p_reco_wn__ = reshape(interp_q_to_p(pm_n_UX_rank,n_w_max*ones(pm_n_UX_rank,1),n_w_max*pm_n_UX_rank,UX_a_k_q_reco_wn__(:)),[n_w_max,pm_n_UX_rank]);
%%%%;
[ ...
 parameter ...
,nw_aM_M_ ...
...
,n_w_max ...
,pm_n_UX_rank ...
,UX_a_k_p_reco_wn__(:,1+[0:pm_n_UX_rank-1]) ...
,n_pick ...
,n_M ...
...
%%%%;
UX_a_k_p_reco_sub_l2 = sum(abs(UX_a_k_p_reco_wn__(:,1+[0:pm_n_UX_rank-1])).^2,'all') / n_w_max;
tmp_X_0_q_ = sum(conj(UX_a_k_q_quad_wn__(:,1+[0:pm_n_UX_rank-1])).*UX_a_k_q_reco_wn__,2)/n_w_max;
tmp_X_0_w_ = ifft(tmp_X_0_q_)*n_w_max;
tmp_X_1_q_ = sum(conj(UX_a_k_q_quad_wn__(:,1+[0:pm_n_UX_rank-1])).*reshape(image_q_flip_0(pm_n_UX_rank,n_w_max,UX_a_k_q_reco_wn__),[n_w_max,pm_n_UX_rank]),2)/n_w_max;
...
%%%%;
X_i_(1+niteration) = tmp_X / sqrt(UX_a_k_p_quad_sub_l2*UX_a_k_p_reco_sub_l2);
UX_a_k_q_reco_wni___(:,:,1+niteration) = UX_a_k_q_reco_wn__;
UX_a_k_p_reco_wni___(:,:,1+niteration) = UX_a_k_p_reco_wn__;
...
if flag_plot;
%%%%%%%%;
fname_fig = sprintf('%s_jpg/gamma_z_Mi_',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
%%%%%%%%;
figure(1+nf);nf=nf+1;clf;figbig;
subplot(2,2,1);plot(1:n_iteration,X_i_,'k.-','LineWidth',2,'MarkerSize',16);
subplot(2,2,2);imagesc(gamma_z_Mi__);
xlabel('iteration');ylabel('nM');
n_h = 64;
h__ = zeros(n_h,n_iteration);
for niteration=0:n_iteration-1;
h__(:,1+niteration) = hist(gamma_z_Mi__(:,1+niteration),linspace(0,2*pi,n_h));
end;%for niteration=0:n_iteration-1;
subplot(2,2,3);imagesc(h__); xlabel('iteration'); ylabel('gamma_z','Interpreter','none');
subplot(2,2,4);hist(gamma_z_Mi__(:,end));
%%%%%%%%;
print('-djpeg',sprintf('%s.jpg',fname_fig));
print('-depsc',sprintf('%s.eps',fname_fig));
close(gcf);
...
X_in__(:,pm_n_UX_rank) = X_i_;
kld_in__(:,pm_n_UX_rank) = kld_i_;
nw_Min___(:,:,pm_n_UX_rank) = nw_Mi__;
...
close_fname_tmp(fname_SR0A_pre);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
end;%if ~fname_SR0A_skip;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
