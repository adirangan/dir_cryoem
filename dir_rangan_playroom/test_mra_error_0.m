...
plot(sigma_eps_,sigma_gam_equi_lob_,'k-','LineWidth',2);
plot(sigma_eps_use_,sigma_gam_emp_,'ro','MarkerFaceColor','r');
xlabel('sigma_eps','Interpreter','none');
ylabel('sigma_gam','Interpreter','none');
title('error');
xlim([0,0.4]); ylim([0,0.2]); grid on;
subplot(1,2,2);
hold on;
%plot(sigma_eps_,sigma_gam_equi_upb_,'k-');
plot(log(sigma_eps_),log(sigma_gam_equi_lob_),'k-','LineWidth',2);
plot(log(sigma_eps_use_),log(sigma_gam_emp_),'ro','MarkerFaceColor','r');
xlabel('log(sigma_eps)','Interpreter','none');
ylabel('log(sigma_gam)','Interpreter','none');
title('error (log-scale)');
xlim([-6,log(0.4)]); ylim([-6,log(0.2)]); grid on;
set(gcf,'Position',1+[0,0,1024,512]);
end;%if flag_plot;
fname_fig = sprintf('./test_mra_error_N%d_0.jpg',n_M);
print('-djpeg',fname_fig);
fname_fig = sprintf('./test_mra_error_N%d_0.eps',n_M);
