...
str_am_init = sprintf('am_init_%s_nUX%d',str_cost,nUX_rank);
...
,n_w_uni_sum ...
,dat_M_k_p__(:,1+nM) ...
,+1.0*delta_read_x_(1+nM) ...
,+1.0*delta_read_y_(1+nM) ...
);
dat_M_k_q__(:,1+nM) = ...
interp_p_to_q( ...
 n_k_p_r ...
...
fname_mat = sprintf('%s_mat/%s_A.mat',dir_trunk,str_am_init);
if ( exist(fname_mat,'file')); disp(sprintf(' %% %s found, not loading',fname_mat)); end;
...
fname_mat = sprintf('%s_mat/%s_B.mat',dir_trunk,str_am_init);
if ( exist(fname_mat,'file')); disp(sprintf(' %% %s found, not loading',fname_mat)); end;
if (~exist(fname_mat,'file')); 
%%%%%%%%;
...
fname_mat = sprintf('%s_mat/%s_C.mat',dir_trunk,str_am_init);
if ( exist(fname_mat,'file')); disp(sprintf(' %% %s found, not loading',fname_mat)); end;
if (~exist(fname_mat,'file')); 
...
flag_plot=1;
if flag_plot;
fname_fig = sprintf('%s_jpg/%s_FIGA',dir_trunk,str_am_init);
...
flag_plot=1;
if flag_plot;
fname_fig = sprintf('%s_jpg/%s_FIGB',dir_trunk,str_am_init);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
%%%%%%%%;
...
flag_plot=1;
if flag_plot;
fname_fig = sprintf('%s_jpg/%s_FIGC',dir_trunk,str_am_init);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
%%%%%%%%;
% plot viewing-angle-spread for those viewing-angles that are close. ;
...
for ninit_Y_lm_cut_threshold=0:n_init_Y_lm_cut_threshold-1;
init_Y_lm_cut_threshold = init_Y_lm_cut_threshold_(1+ninit_Y_lm_cut_threshold);
fname_fig = sprintf('%s_jpg/%s_FIGD_ncut%d',dir_trunk,str_am_init,ninit_Y_lm_cut_threshold);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
%%%%%%%%;
