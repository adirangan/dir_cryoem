...
dir_pm = sprintf('/%s/rangan/dir_cryoem/dir_MlaFEDB/dir_pm',string_root);
...
dir_relion = sprintf('/%s/rangan/dir_cryoem/dir_MlaFEDB/dir_relion',string_root);
...
dir_data_star = sprintf('/%s/rangan/dir_cryoem/dir_MlaFEDB',string_root);
Pixel_Spacing = 0.832; %<-- in angstroms, from https://www.ebi.ac.uk/pdbe/emdb/empiar/entry/10536/ ;

%%%%%%%%;
% First load emd_22116.map volume. ;
%%%%%%%%;
fname_mat = sprintf('%s_mat/a_x_u_pack_.mat',dir_pm);
if (flag_recalc | ~exist(fname_mat,'file'));
disp(sprintf(' %% %s not found, creating',fname_mat));
fname_emd = sprintf('%s/emd_22116.map',dir_data_star);
...
disp(sprintf(' %% %s not found, creating',fname_mat));
n_M = 1024;
fname_nopath_star = 'Empiar_10536_00_to_23.star';
[ ...
 M_x_c___ ...
,index_nCTF_from_nM_ ...
,index_nM_from_nCTF_ ...
...
%%%%%%%%;
disp(sprintf(' %% Warning, removing edge-artefacts'));
...
disp(sprintf(' %% Warning, replacing CTF_Voltage_kV=%0.2f with 300, see elifesciences.org/articles/62518',CTF_Voltage_kV)); CTF_Voltage_kV = 300;
...
disp(sprintf(' %% Warning, replacing tmp_Voltage %0.2f with 300',tmp_Voltage)); tmp_Voltage = 300;
