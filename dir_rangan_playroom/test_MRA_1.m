...
figure(1);clf;figsml;
c_beach__ = colormap_beach(); n_c_beach = size(c_beach__,1);
str_symbol_ = {'o','s'}; n_str_symbol=numel(str_symbol_);
hold on;
%%%%%%%%;
for nq=0:n_q-1;
...
r_true_q = r_true_q_(1+nq);
w_true_q = w_true_q_(1+nq);
plot(r_true_q.*cos(w_true_q),r_true_q.*sin(w_true_q),str_symbol,'Color','k');
plot(r_true_q.*cos(gamma_),r_true_q.*sin(gamma_),'k-');
end;%for nq=0:n_q-1;
%%%%%%%%;
for nM=0:n_M-1;
...
w_samp_q = w_samp_q_(1+nq);
q = q_(1+nq);
plot(r_samp_q.*[cos(w_samp_q + q.*gamma_true)],r_samp_q.*[sin(w_samp_q + q.*gamma_true)],str_symbol,'MarkerFaceColor',c_beach__(1+nc_beach,:));
plot(r_samp_q.*cos(gamma_),r_samp_q.*sin(gamma_),'-','Color',c_beach__(1+nc_beach,:));
end;%for nq=0:n_q-1;
end;%for nM=0:n_M-1;
%%%%%%%%;
...
sigma_ = 2.^[0:0.5:5]; n_sigma = numel(sigma_);
r_factor_ = 2.^[-1:0.125:+1]; n_r_factor = numel(r_factor_);
