function [X0_,n_op,n_mult] = register_spharm_to_spharm_angle_parfor_3(verbose,n_k,k_,n_l_,a_,b_,n_beta,beta_);
...
[X0_,n_op,n_mult] = register_spharm_to_spharm_angle_parfor_3(verbose,n_k,k_,n_l_,a_,b_,n_beta,beta_);
end;%for n_l=2.^[4:7];
...
t_out = toc(t_0in);
disp(sprintf(' %% register time: single volume pair: loop-over-beta: time %0.6f',t_out));
%%%%%%%%;
n_op = 0;
n_mult = 0;
t_0in = tic;
n_vol = 16; 
n_pair = n_vol^2;
a____ = zeros(n_vol,n_m_max,n_k,1+n_l_max);
tmp_a___ = reshape(a____,n_vol*n_m_max,n_k,1+n_l_max);
ab____ = zeros(n_vol*n_m_max,n_vol*n_m_max,1+n_l_max);
for nl=0:n_l_max;
tmp_a__ = squeeze(tmp_a___(:,:,1+nl));
ab____(:,:,1+nl) = tmp_a__*transpose(tmp_a__);
clear tmp_a__;
end;%for nl=0:n_l_max;
clear tmp_a___;
ab_p___ = reshape(permute(reshape(ab____,n_vol,n_m_max,n_vol,n_m_max,1+n_l_max),[1,3,5,2,4]),n_pair,1+n_l_max,n_m_max^2);
clear ab____;
t_out = toc(t_0in);
disp(sprintf(' %% correlation_time: multiple volume pair: loop-over-nl: time %0.6f = %0.6f per pair',t_out,t_out/n_pair));
d_p___ = zeros(1+n_l_max,n_beta,n_m_max^2);
d_p___ = reshape(permute(d___,[3,4,1,2]),1+n_l_max,n_beta,n_m_max^2);
C___ = zeros(n_pair,n_beta,n_m_max^2);
t_0in = tic;
for nmnmp=0:n_m_max^2-1;
C___(:,:,1+nmnmp) = ab_p___(:,:,1+nmnmp)*d_p___(:,:,1+nmnmp);
n_op = n_op + n_pair*n_beta*(1+n_l_max);
n_mult = n_mult + n_pair*n_beta*(1+n_l_max);
end;%for nmnmp=0:n_m_max^2-1;
C_p____ = reshape(permute(C___,[3,1,2]),n_m_max,n_m_max,n_pair,n_beta);
X0____ = conj(fft(fft(C_p____,[],1),[],2));
t_out = toc(t_0in);
disp(sprintf(' %% register time: multiple volume pair: loop-over-(mn,mp): time %0.6f = %0.6f per pair',t_out,t_out/n_pair));
%%%%%%%%;
disp(sprintf(' %% mem usage %0.2f GB',monitor_memory_whos));
