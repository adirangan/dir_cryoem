...
for nexperiment=4;%for nexperiment=(randperm(n_experiment)-1);
na=0;
...
% global_parameter=[];fname_prefix='p28hRPT1_x0';dir_nopath_data_star='p28hRP';Pixel_Spacing=0.98;fname_nopath_volume='emd_8674.map';fname_nopath_star='T1.star';
...
verbose=1;
if (verbose); disp(sprintf(' %% [entering test_pm_sfig_24]')); end;
...
% Also construct RS_k_p__ (templates not shifted to align with images, but are rotated). ;
% And construct TM_k_p__ (images shifted to align with templates, but not rotated). ;
%%%%%%%%;
n_S = n_viewing_all;
RS_k_p__ = zeros(n_w_sum,n_M);
TM_k_p__ = zeros(n_w_sum,n_M);
X_RSTM_ = zeros(n_M,1);
...
S_k_p_ = S_k_p__(:,1+nS);
RS_k_p_ = rotate_p_to_p_fftw(n_k_p_r,n_w_,n_w_sum,S_k_p_,+tmp_euler_gamma_z);
%RS_k_p_ = transf_p_to_p(n_k_p_r,k_p_r_,n_w_,n_w_sum,RS_k_p_,-tmp_image_delta_x,-tmp_image_delta_y);
RS_k_p_ = sparse(1:n_w_sum,1:n_w_sum,CTF_k_p_wkC__(:,1+index_nCTF_from_nM_(1+nM)),n_w_sum,n_w_sum)*RS_k_p_;
RS_k_p__(:,1+nM) = RS_k_p_;
TM_k_p_ = transf_p_to_p(n_k_p_r,k_p_r_,n_w_,n_w_sum,M_k_p_,+tmp_image_delta_x,+tmp_image_delta_y);
%TM_k_p_ = rotate_p_to_p_fftw(n_k_p_r,n_w_,n_w_sum,TM_k_p_,-tmp_euler_gamma_z);
TM_k_p__(:,1+nM) = TM_k_p_;
tmp_RSRS = innerproduct_p_quad(n_k_p_r,k_p_r_,weight_2d_k_p_r_,n_w_,n_w_sum,RS_k_p_,RS_k_p_);
tmp_TMTM = innerproduct_p_quad(n_k_p_r,k_p_r_,weight_2d_k_p_r_,n_w_,n_w_sum,TM_k_p_,TM_k_p_);
tmp_RSTM = innerproduct_p_quad(n_k_p_r,k_p_r_,weight_2d_k_p_r_,n_w_,n_w_sum,RS_k_p_,TM_k_p_);
X_RSTM_(1+nM) = real(tmp_RSTM)/sqrt(tmp_RSRS*tmp_TMTM);
end;%for nM=0:n_M-1;
%%%%%%%%;
...
tmp_index_ = n_x_M_u/2 + [-tmp_n_x:+tmp_n_x];
...
T_k_p_ = sparse(1:n_w_sum,1:n_w_sum,CTF_k_p_wkC__(:,1+index_nCTF_from_nM_(1+nM)),n_w_sum,n_w_sum)*T_k_p_;
tmp_S_x_c__ = real(interp_k_p_to_x_c_xxnufft(n_x_M_u,diameter_x_c,n_x_M_u,diameter_x_c,n_k_p_r,k_p_r_,n_w_,T_k_p_));
...
verbose=1;
if (verbose); disp(sprintf(' %% [finished test_pm_sfig_24]')); end;
