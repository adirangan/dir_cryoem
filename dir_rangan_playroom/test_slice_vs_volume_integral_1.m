...
str_thisfunction = 'test_slice_vs_volume_integral_1';
verbose=1;
...
if (verbose); disp(sprintf(' %% fnorm(M_avg_k_p_wkS__ - mean(M_k_p_wkSM___,3)): %0.16f',fnorm(M_avg_k_p_wkS__ - mean(M_k_p_wkSM___,3)))); end;
tmp_t = toc(tmp_t); disp(sprintf(' %% pm_template_2: %0.2fs',tmp_t));
...
%%%%%%%%;
if (verbose); disp(sprintf(' %% Assuming uniform distribution of viewing-angles: ')); end;
if (verbose); disp(sprintf(' %% (multi volume to ensure accurate variance calculation): ')); end;
...
if (verbose); disp(sprintf(' %% Assuming uniform distribution of viewing-angles (single volume): ')); end;
if (verbose); disp(sprintf(' %% (warning, variance inaccurately calculated: ')); end;
%%%%;
...
L2_2d_avg_k_Y = sum( bsxfun(@times,abs(bsxfun(@minus,c_k_Y_quad_yk_,a_k_Y_base_)).^2,weight_Y_2d_) , 'all' );
if (verbose); disp(sprintf(' %% L2_2d_avg_k_p %0.6f vs L2_2d_avg_k_Y %0.6f: %0.16f',L2_2d_avg_k_p,L2_2d_avg_k_Y,fnorm(L2_2d_avg_k_p - L2_2d_avg_k_Y)/fnorm(L2_2d_avg_k_Y))); end;
%%%%;
...
if (verbose); disp(sprintf(' %% sum(jl_ab_all_.*viewing_weight_all_)/(k_p_r_max^2): %0.16f (should be 1)',sum(jl_ab_all_.*viewing_weight_all_)/(k_p_r_max^2))); end;
...
if (verbose); disp(sprintf(' %% jl_hk_lm__(1,1+l_max)*sqrt(4*pi): %0.16f (should be within 2-3 digits of 1)',jl_hk_lm__(1,1+l_max)*sqrt(4*pi))); end;
%%%%%%%%;
qbp_eps = 1e-3;
[ ...
 j1_hk_Y_ ...
...
if (verbose); disp(sprintf(' %% j1_hk_lm__(1,1+l_max)*sqrt(4*pi): %0.16f (should be 1)',j1_hk_lm__(1,1+l_max)*sqrt(4*pi))); end;
%%%%%%%%;
jl_hk_k_all_ = ...
...
if (verbose); disp(sprintf(' %% sum(real(jl_hk_k_all_).*viewing_weight_all_/k_p_r_max^2: %0.16f (should be within 2-3 digits of 1)',sum(real(jl_hk_k_all_).*viewing_weight_all_/k_p_r_max^2))); end;
%%%%%%%%;
j1_hk_k_all_ = ...
convert_spharm_to_k_p_2( ...
 0 ...
...
if (verbose); disp(sprintf(' %% sum(real(j1_hk_k_all_).*viewing_weight_all_/k_p_r_max^2: %0.16f (should be 1)',sum(real(j1_hk_k_all_).*viewing_weight_all_/k_p_r_max^2))); end;
%%%%%%%%;
...
%%%%%%%%;
if (verbose); disp(sprintf(' %% jl_hk_k_all_')); end;
[jl_hk_k_all_] = ...
convert_spharm_to_k_p_2( ...
 0*verbose ...
,n_k_all ...
...
%%%%%%%%;
if (verbose); disp(sprintf(' %% j1_hk_k_all_')); end;
[j1_hk_k_all_] = ...
convert_spharm_to_k_p_2( ...
 0*verbose ...
,n_k_all ...
...
%%%%%%%%;
if (verbose); disp(sprintf(' %% a_k_p_quad_k_all_')); end;
[a_k_p_base_k_all_] = ...
convert_spharm_to_k_p_2( ...
 0*verbose ...
,n_k_all ...
...
%%%%%%%%;
if (verbose); disp(sprintf(' %% c_k_p_quad_k_all_')); end;
[c_k_p_quad_k_all_] = ...
convert_spharm_to_k_p_2( ...
 0*verbose ...
,n_k_all ...
...
for nM=0:n_M-1;
if (verbose); disp(sprintf(' %% nM %d/%d b_k_p_quad_k_all_kM__',nM,n_M)); end;
...
if ( exist(fname_mat,'file'));
if (verbose); disp(sprintf(' %% %s found, not creating',fname_mat)); end;
load(fname_mat);
end;%if ( exist(fname_mat,'file'));
...
if (verbose); disp(sprintf(' %% k_p_r %0.6f: sum(weight_3d_k_all_(1+tmp_index_))/(weight_3d_k_p_r * 4*pi): %0.16f',k_p_r,sum(weight_3d_k_all_(1+tmp_index_))/(4*pi*weight_3d_k_p_r))); end;
weight_3d_riesz_k_all_(1+tmp_index_) = weight_3d_k_all_(1+tmp_index_) * weight_2d_k_p_r / max(1e-16,weight_3d_k_p_r);
if (verbose); disp(sprintf(' %% k_p_r %0.6f: sum(weight_3d_riesz_k_all_(1+tmp_index_))/(weight_2d_k_p_r * 4*pi): %0.16f',k_p_r,sum(weight_3d_riesz_k_all_(1+tmp_index_))/(4*pi*weight_2d_k_p_r))); end;
end;%for nk_p_r=0:n_k_p_r-1;
%%%%;
...
if (verbose); disp(sprintf(' %% L2_2d_MvS_k_p %0.6f vs L2_2d_bva_k_p %0.6f: %0.16f',L2_2d_MvS_k_p,L2_2d_bva_k_p,fnorm(L2_2d_MvS_k_p - L2_2d_bva_k_p)/fnorm(L2_2d_MvS_k_p))); end;
%%%%;
...
if (verbose); disp(sprintf(' %% (uniform weight) L2_2d_MvS_k_p %0.6f vs L2_2d_bva_k_p %0.6f: %0.16f',L2_2d_MvS_k_p,L2_2d_bva_k_p,fnorm(L2_2d_MvS_k_p - L2_2d_bva_k_p)/fnorm(L2_2d_MvS_k_p))); end;
%%%%;
if (verbose); disp(sprintf(' %% Assuming non-uniform distribution of viewing-angles (multi volume): ')); end;
if (verbose); disp(sprintf(' %% (multi volume to ensure accurate variance calculation): ')); end;
...
if (verbose); disp(sprintf(' %% (nonuniform weight) L2_2d_MvS_k_p %0.6f vs L2_2d_bva_k_p %0.6f: %0.16f (should be good to 2-3 digits)',L2_2d_MvS_k_p,L2_2d_bva_k_p,fnorm(L2_2d_MvS_k_p - L2_2d_bva_k_p)/fnorm(L2_2d_MvS_k_p))); end;
%%%%;
...
if (verbose); disp(sprintf(' %% (nonuniform weight) L2_2d_MvS_avg_k_p %0.6f vs L2_2d_bva_avg_k_p %0.6f: %0.16f (should be good to 2-3 digits)',L2_2d_MvS_avg_k_p,L2_2d_bva_avg_k_p,fnorm(L2_2d_MvS_avg_k_p - L2_2d_bva_avg_k_p)/fnorm(L2_2d_MvS_avg_k_p))); end;
%%%%;
...
if (verbose); disp(sprintf(' %% L2_2d_MvS_var_k_p %0.6f vs L2_2d_bva_var_k_p %0.6f: %0.16f (should be good to 2-3 digits)',L2_2d_MvS_var_k_p,L2_2d_bva_var_k_p,fnorm(L2_2d_MvS_var_k_p - L2_2d_bva_var_k_p)/fnorm(L2_2d_bva_var_k_p))); end;
%%%%;
