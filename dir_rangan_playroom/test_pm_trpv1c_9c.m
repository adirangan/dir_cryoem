...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
fname_sheres_pre = sprintf('%s_mat/%s_a_k_Y_sher_%s%s_from_quad_',dir_pm,str_xvM,str_xfix,str_nls);
...
fname_fig = sprintf('%s_jpg/%s_a_k_Y_sher_%s%s_from_quad_snapshot',dir_pm,str_xvM,str_xfix,str_nls);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
...
if flag_compute;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
rseed_ = [0:6]; n_rseed = numel(rseed_);
%sigma_sheres_ = exp([-5:0.5:0]); n_sigma_sheres = numel(sigma_sheres_);
%sigma_sheres_ = exp([-3:0.5:-1.5]); n_sigma_sheres = numel(sigma_sheres_);
sigma_sheres_ = exp([-2.0]); n_sigma_sheres = numel(sigma_sheres_);
flag_xcor_vs_Memp = 0;
flag_rank_vs_tolerance=0;
delta_r_max = 0.1;
tolerance_pm = tolerance_master;
...
parameter.a_x_u_base_ = a_x_u_pack_;
...
n_M_relion_ = 1024*[1,2,4,8,16,32]; n_M_relion_ = [n_M_relion_,numel(star_blockData{1}.rlnVoltage)]; n_n_M_relion = numel(n_M_relion_);
...
,l_max_max ...
,n_k_p_r ...
,zeros(n_lm_max,n_k_p_r) ...
,template_viewing_k_eq_d ...
,-1 ...
,n_w_max ...
);
%%%%%%%%;
R2_base_wSM___ = 2 - 2*X_base_wSM___;
R2_weight_wSM___ = repmat(reshape(template_viewing_weight_all_,[1,n_S,1]),[n_w_max,1,n_M]);
sigma_sheres_0in_ = exp(-5:0.25:0); n_sigma_sheres_0in = numel(sigma_sheres_0in_);
sigma_sheres_out_ = zeros(n_sigma_sheres_0in,1);
for nsigma_sheres_0in=0:n_sigma_sheres_0in-1;
if (mod(nsigma_sheres_0in,8)==0); disp(sprintf(' %% nsigma_sheres_0in %d/%d',nsigma_sheres_0in,n_sigma_sheres_0in)); end;
sigma_sheres_0in = sigma_sheres_0in_(1+nsigma_sheres_0in);
sigma_sheres_out = sigma_me_0(R2_base_wSM___,R2_weight_wSM___,sigma_sheres_0in);
sigma_sheres_out_(1+nsigma_sheres_0in) = sigma_sheres_out;
end;%for nsigma_sheres_0in=0:n_sigma_sheres_0in-1;
%%%%%%%%; 
figure(1+nf);nf=nf+1;clf;figsml;
plot(sigma_sheres_0in_,sigma_sheres_out_,'ko');
xlabel('sigma_sheres_0in_','Interpreter','none');
ylabel('sigma_sheres_out_','Interpreter','none');
title('sigma_me_0','Interpreter','none');

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
% Verdict: temperature is O(1), even with the factor of 0.5 in the iteration.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;

sigma_sheres_ = exp([-5:0.5:0]); n_sigma_sheres = numel(sigma_sheres_);
n_iteration = 16;
X_best_sher_alig_is__ = zeros(n_iteration,n_sigma_sheres);
for nsigma_sheres=0:n_sigma_sheres-1;
sigma_sheres = sigma_sheres_(1+nsigma_sheres);
str_nls = sprintf('nls%.2d',round(10*-log(sigma_sheres)));
fname_sheres_mat = sprintf('%s_mat/X_2d_Memp_d1_%st0100p20r0.mat',dir_pm,str_nls);
if  exist(fname_sheres_mat,'file');
tmp_ = load(fname_sheres_mat);
X_best_sher_alig_is__(:,1+nsigma_sheres) = tmp_.X_best_sher_alig_i_;
clear tmp_;
end;%if  exist(fname_sheres_mat,'file');
end;%for nsigma_sheres=0:n_sigma_sheres-1;
 
fname_fig = sprintf('%s_jpg/X_best_sher_alig_is__FIGA',dir_pm);
str_legend_ = cell(n_sigma_sheres,1);
for nsigma_sheres=0:n_sigma_sheres-1;
sigma_sheres = sigma_sheres_(1+nsigma_sheres);
str_legend_{1+nsigma_sheres} = sprintf('sigma = exp(%+2.1f)',log(sigma_sheres));
end;%for nsigma_sheres=0:n_sigma_sheres-1;
figure(1);clf;figsml;
...
hold on;
for nsigma_sheres=0:n_sigma_sheres-1;
nc_use = max(0,min(n_c_use-1,floor(n_c_use*nsigma_sheres/n_sigma_sheres)));
%nc_use = nsigma_sheres;
X_best_sher_alig_i_ = X_best_sher_alig_is__(:,1+nsigma_sheres);
plot(1:n_iteration,X_best_sher_alig_i_,'ko-','LineWidth',linewidth_use,'MarkerSize',markersize_use,'MarkerFaceColor',c_use__(1+nc_use,:));
end;%for nsigma_sheres=0:n_sigma_sheres-1;
hold off;
xlim([0,n_iteration+1]);
ylim([0.50,1.00]); grid on;
xlabel('iteration');
ylabel('correlation (no mask)');
title('shere-style qbp for trpv1');
