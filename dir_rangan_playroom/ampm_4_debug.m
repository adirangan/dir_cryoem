...
%%%%%%%%;
a_UX_Y_0lsq_ = spharm_normalize_0(pm_n_k_p_r,pm_k_p_r_,pm_weight_k_p_r_,pm_l_max_,a_UX_Y_0lsq_);
%%%%%%%%;
if (nargout>1);
...
%%%%%%%%;
nM_ = round(linspace(1,n_M,12))-1;
for nM=nM_;
tmp_C = sqrt(UX_M_l2_dM__(1,1+nM))/sqrt(UX_S_l2_(1+image_S_index_(1+nM))) * image_I_value_(1+nM) ;
tmp_X = UX_S_l2_(1+image_S_index_(1+nM))*(tmp_C^2-1) ;
disp(sprintf(' %% image %.4d: X %+0.6f a %+0.2f b %+0.2f c %+0.2f dx %+0.2f dy %+0.2f I %+0.6f <-- C %+0.6f --> X %0.6f S %.3d', ...
	      nM ...
	     ,image_X_value_(1+nM) ...
	     ,euler_polar_a_(1+nM) ...
	     ,euler_azimu_b_(1+nM) ...
