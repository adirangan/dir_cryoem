...
snr_per_A = 0.20; %<-- decrease to increase noise. ;
f_rand_0in = 0.0;
n_M_factor_0in = 8; %<-- n_M = n_w_max * n_M_factor_0in. ;
n_w_max_factor_0in = 4; %<-- n_w_max = n_k_p_r * n_w_max_factor_0in. ;
n_x_u_factor_0in = 2; %<-- n_x_u = 64 * n_x_u_factor_0in. ;
global_flag_Ma_vs_aM = 1; %<-- 1 to enforce uniform viewing angles. ;
...
if (global_flag_Ma_vs_aM);
string_infix = sprintf('%sa1',string_infix);
end;%if (global_flag_Ma_vs_aM);
...
n_w_max = n_w_max_factor_0in*(n_k_p_r);
...
parameter = struct('type','parameter');
parameter.f_rand = f_rand_0in;
parameter.flag_Ma_vs_aM = global_flag_Ma_vs_aM;
%%%%%%%%;
...
%%%%;
sgtitle(sprintf('%s: pm_n_UX_rank %d',string_infix,pm_n_UX_rank),'Interpreter','none');
%%%%;
end;%if flag_vis;
