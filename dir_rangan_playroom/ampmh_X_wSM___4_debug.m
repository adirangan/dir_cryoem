...
% and delta_ = [ FTK.delta_x_(1+delta_j) ; FTK.delta_y_(1+delta_j) ], ;
% where delta_j = delta_j_wSM___(1+nw,1+nS,1+nM). ;
% Put more succinctly: ;
...
if (verbose); disp(sprintf(' %% [testing ampmh_X_wSM___4_debug.m]')); end;
%%%%%%%%;
tmp_t = tic();
[ ...
 n_k_all ...
...
%%%%%%%%;
% Calculate ampmh_X_wSS___4_debug. ;
%%%%%%%%;
tmp_t = tic();
...
%%%%%%%%;
% Finally check ampmh_X_wSM___4_debug to see how the rotations and translations are implemented. ;
%%%%%%%%;
...
verbose = 1;
if (verbose>0); disp(sprintf(' %% [entering ampmh_X_wSM___4_debug]')); end;
%n_w_max = max(n_w_);
...
if (n_S_sub>0);
tmp_t = tic();
if (~issparse(SS_k_q__)); SS_k_q__ = sparse(SS_k_q__); end;
%%%%%%%%;
%tmp_0_ = reshape(svd_USVUXM_SMwl____,[n_S_rank,n_M_sub*n_w_max*FTK.n_svd_l]);
%tmp_1_ = SS_k_q__*tmp_0_;
%tmp_2_ = VS_k_q__(1+S_ij_,:)*tmp_1_;
%tmp_3_ = reshape(tmp_2_,[n_S_sub,n_M_sub,n_w_max,FTK.n_svd_l]);
%tmp_4_ = permute(tmp_3_,[4,3,1,2]);
%%%%;
%disp(sprintf(' %% sizes: %d,%d %d,%d %d,%d,%d,%d',size(VS_k_q__(1+S_ij_,:)),size(SS_k_q__),size(svd_USVUXM_SMwl____)));
%%%%;
...
svd_SVUXM_lwSM____ = permute(reshape(VSSS_k_q__(1+S_ij_,:)*reshape(svd_USVUXM_SMwl____,[n_S_rank,n_M_sub*n_w_max*FTK.n_svd_l]),[n_S_sub,n_M_sub,n_w_max,FTK.n_svd_l]),[4,3,1,2]);
%%%%%%%%;
%tmp_5_ = reshape(svd_SVUXM_lwSM____,[FTK.n_svd_l,n_w_max*n_S_sub*n_M_sub]);
%tmp_6_ = FTK.svd_U_d_expiw_s__*tmp_5_;
%tmp_7_ = reshape(tmp_6_,[FTK.n_delta_v,n_w_max,n_S_sub,n_M_sub]);
%%%%;
...
if (verbose>1); disp(sprintf(' %% [finished ampmh_X_wSM___4_debug]')); end;
