...
verbose=1;
if (verbose); disp(sprintf(' %% [entering test_pm_riby_3]')); end;
...
%%%%%%%%;
fname_mat = sprintf('%s_mat/test_pm_riby_ibp_xx__.mat',dir_pm);
if (flag_recalc | ~exist(fname_mat,'file'));
disp(sprintf(' %% %s not found, creating',fname_mat));
%%%%%%%%;
...
% illustrate minimum. ;
%%%%;
tmp_inten_ = tmp_inten.*linspace(0.99,1.01,1+32);
tmp_backg_ = tmp_backg.*linspace(0.99,1.01,1+48);
[tmp_inten__,tmp_backg__] = ndgrid(tmp_inten_,tmp_backg_);
% Note: error: |Q - (inten*S+backg)|^2 ;
tmp_err__ = tmp_Q2M1_zS__(1+tmp_index_max) + tmp_inten__.^2*Q0S2_S_(1+tmp_index_nS_max) + tmp_backg__.^2*Q0M1_S_(1+tmp_index_nS_max) - 2*tmp_inten__.*tmp_Q1S1_zS__(1+tmp_index_max) - 2*tmp_backg__.*tmp_Q1M1_zS__(1+tmp_index_max) + 2*tmp_inten__.*tmp_backg__.*Q0S1_S_(1+tmp_index_nS_max);
figure(1+nf);nf=nf+1;clf;figsml; fig80s;
imagesc(tmp_err__);axisnotick;axis image; xlabel('backg');ylabel('inten');
end;%if flag_check;
%%%%;
...
% illustrate minimum. ;
%%%%;
tmp_sigma_ = tmp_sigma.*linspace(0.99,1.01,1+64);
% Note: llike: -tmp_M*(0.5*log(2*pi)+log(sigma)) - (0.5/sigma^2)*tmp_err ;
tmp_lli_ = -Q0M1_S_(1+tmp_index_nS_max)*(0.5*log(2*pi)+log(tmp_sigma_)) - 0.5./max(1e-12,tmp_sigma_.^2)*tmp_err;
figure(1+nf);nf=nf+1;clf;figsml; fig80s;
plot(tmp_sigma_,tmp_lli_,'.-');xlabel('sigma');ylabel('log-likelihood');
end;%if flag_check;
%%%%;
tmp_nll_f = @(ibs_) ...
...
tmp_nll = tmp_nll_f([tmp_inten;tmp_backg;tmp_sigma]);
[tmp_ibs_opt_,tmp_nll_opt] = fminsearch(tmp_nll_f,[tmp_inten;tmp_backg;tmp_sigma]);
%%%%;
...
inten_sum_xx__(1+nx0,1+nx1) = tmp_ibs_opt_(1+0);
backg_sum_xx__(1+nx0,1+nx1) = tmp_ibs_opt_(1+1);
sigma_sum_xx__(1+nx0,1+nx1) = tmp_ibs_opt_(1+2);
nll_sum_xx__(1+nx0,1+nx1) = tmp_nll_opt;
clear tmp_inten tmp_backg tmp_sigma tmp_ibs_opt_ tmp_nll_opt tmp_nll_f ;
...
disp(sprintf(' %% %s found, not creating',fname_mat));
load(fname_mat);
fname_rel_mat = sprintf('%s_mat/test_pm_riby_nll_rel_xx__.mat',dir_pm);
if (flag_recalc | ~exist(fname_rel_mat,'file'));
disp(sprintf(' %% %s not found, creating',fname_rel_mat));
nll_rel_sum_xx__ = zeros(n_Q_0,n_Q_1);
tmp_t_start = tic(); nxx=0;
for nx0=0:n_Q_0-1; for nx1=0:n_Q_1-1;
if (verbose>-1);
if (mod(nxx,8)==0); 
...
tmp_Q1M1_zS__ = reshape(Q1M1_xxzS____(1+nx0,1+nx1,:,:),[1,1,n_gamma_z,n_S]);
tmp_Q2M1_zS__ = reshape(Q2M1_xxzS____(1+nx0,1+nx1,:,:),[1,1,n_gamma_z,n_S]);
tmp_inten = inten_sum_xx__(1+nx0,1+nx1);
tmp_backg = backg_sum_xx__(1+nx0,1+nx1);
tmp_sigma = sigma_sum_xx__(1+nx0,1+nx1);
[ ...
 tmp_nll_raw_sum ...
,tmp_nll_rel_sum ...
] = ...
...
nll_rel_sum_xx__(1+nx0,1+nx1) = tmp_nll_rel_sum;
clear tmp_inten tmp_backg tmp_sigma tmp_nll_sum tmp_nll_rel_sum ;
clear tmp_X_QS_zS__ tmp_Q1S1_zS__ tmp_Q1M1_zS__ tmp_Q2M1_zS__ ;
nxx=nxx+1;
end;end;%for nx0=0:n_Q_0-1; for nx1=0:n_Q_1-1;
save(fname_rel_mat ...
...
end;%if (~exist(fname_rel_mat,'file'));
if ( exist(fname_rel_mat,'file'));
disp(sprintf(' %% %s found, not creating',fname_rel_mat));
load(fname_rel_mat);
end;%if ( exist(fname_rel_mat,'file'));
end;%if ( exist(fname_mat,'file'));
%%%%%%%%;

fname_fig = sprintf('%s_jpg/test_pm_riby_ibp_xx_FIGA',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1+nf);nf=nf+1;clf;figbig;
...
Q_lsigm = @(sigma_coef_) reshape(exp(Q_xxp__*sigma_coef_),[n_Q_0,n_Q_1,1,1]); %<-- log sigma. ;
Q_inten = @(inten_coef_) reshape(   (Q_xxp__*inten_coef_),[n_Q_0,n_Q_1,1,1]); %<-- raw inten. ;
Q_backg = @(backg_coef_) reshape(   (Q_xxp__*backg_coef_),[n_Q_0,n_Q_1,1,1]); %<-- raw backg. ;
lsigm_coef_ = zeros(n_coef,1);
inten_coef_ = zeros(n_coef,1);
backg_coef_ = zeros(n_coef,1);
[~,tmp_ij_nll_rel_min] = min(nll_rel_sum_xx__,[],'all','linear'); tmp_index_nll_rel_min = tmp_ij_nll_rel_min-1;
n_coef = 6;
lsigm_coef_(1+0) = log(sigma_sum_xx__(1+tmp_index_nll_rel_min));
inten_coef_(1+0) =    (inten_sum_xx__(1+tmp_index_nll_rel_min));
backg_coef_(1+0) =    (backg_sum_xx__(1+tmp_index_nll_rel_min));
...
tmp_Q1M1_zS__ = reshape(Q1M1_xxzS____(1+nx0,1+nx1,:,:),[1,1,n_gamma_z,n_S]);
tmp_Q2M1_zS__ = reshape(Q2M1_xxzS____(1+nx0,1+nx1,:,:),[1,1,n_gamma_z,n_S]);
tmp_sigma = exp(Q_xxp__(1+nx0+nx1*n_Q_0,:)*lsigm_coef_);
tmp_inten =    (Q_xxp__(1+nx0+nx1*n_Q_0,:)*inten_coef_);
tmp_backg =    (Q_xxp__(1+nx0+nx1*n_Q_0,:)*backg_coef_);
[ ...
 tmp_nll_raw_sum ...
,tmp_nll_rel_sum ...
] = ...
...
verbose=1;
if (verbose); disp(sprintf(' %% [finished test_pm_riby_3]')); end;
