...
title('$B^{\mbox{signal}}(\vec{x}) + B^{\mbox{noise}}(\vec{x})$','Interpreter','Latex');
axis image; axisnotick;
%%%%;
subplot(p_row,p_col,1+ns); ns=ns+1;
S_x_c_lim_ = mean(S_x_c_nMB_,'all') + std(S_x_c_nMB_,1,'all')*2.5*[-1,+1];
imagesc(S_x_c_nMB_,S_x_c_lim_);
title('$B^{\mbox{signal}}(\vec{x})$','Interpreter','Latex');
axis image; axisnotick;
%%%%;
subplot(p_row,p_col,1+ns); ns=ns+1;
imagesc_p(n_k_p_r,k_p_r_,n_w_,n_w_sum,real(U_k_p_nMB_),[],colormap_80s);
title('$\hat{B}^{\mbox{signal}}(\vec{k}) + \hat{B}^{\mbox{noise}}(\vec{k})$','Interpreter','Latex');
axis image; axisnotick;
%%%%;
subplot(p_row,p_col,1+ns); ns=ns+1;
imagesc_p(n_k_p_r,k_p_r_,n_w_,n_w_sum,real(T_k_p_nMB_),[],colormap_80s);
title('$\hat{B}^{\mbox{signal}}(\vec{k})$','Interpreter','Latex');
axis image; axisnotick;
%%%%;
...
grid on;
title(sprintf('$C^{signal}(\\vec{u})$ for the first %d ranks',n_svd),'Interpreter','latex');
disp(sprintf(' %% writing %s',fname_fig));
print('-djpeg',sprintf('%s.jpg',fname_fig));
print('-depsc',sprintf('%s.eps',fname_fig));
close(gcf);
...
for tmp_nM=[0,1];
nM=nMA; tmp_str = 'A'; if (tmp_nM==1); nM=nMB; tmp_str = 'B'; end;
...
set(gca,'YTick',[]);
title(sprintf('$\\langle \\hat{%s} , CTF\\odot\\left[ R_{\\gamma}\\circ\\hat{S}\\right] \\rangle$ for the first %d ranks',tmp_str,n_svd),'Interpreter','latex');
...
title(sprintf('$A^{signal}_{%d}(\\vec{x})$',1+nM),'Interpreter','Latex');
axis image; axisnotick;
end;%for nnM_sub=0:n_nM_sub-1;
%%%%;
disp(sprintf(' %% writing %s',fname_fig));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
% Repeat using empirical principal-modes. ;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
fname_mat = sprintf('%s_mat/pm_fig_X_2d_Memp_cluster_d1_FIGJ__.mat',dir_pm);
if (flag_recalc | ~exist(fname_mat,'file'));
disp(sprintf(' %% %s not found, creating',fname_mat));
%%%%%%%%;
% First cluster the CTF based on tolerance_cluster. ;
...
fname_mat = sprintf('%s_mat/pm_fig_X_2d_Memp_cluster_d1_timing_FIGJ__.mat',dir_pm);
if (flag_recalc | ~exist(fname_mat,'file'));
disp(sprintf(' %% %s not found, creating',fname_mat));
%%%%%%%%;
tmp_t_mult_ = zeros(n_tolerance_pm,1);
...
fname_fig = sprintf('%s_jpg/pm_fig_X_2d_Memp_cluster_d1_FIGJ__',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
