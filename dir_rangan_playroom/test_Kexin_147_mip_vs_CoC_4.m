...
% For now we exclude data-points which are above the 98th-percentile. ;
%%%%%%%%;
CDF_threshold = 0.02; %<-- used only on positive end below. ;

%%%%%%%%;
% Now we fit the 'middle' of the mip-distribution with an extreme-value-distribution (gev). ;
% Note that we constrain gev_zet to lie in [0,1], ensuring that the support includes long positive tails and has finite mean. ;
% Thus, our parametrization is a simple sigmoid: gev_zet = exp(gev_sze)/(1+exp(gev_sze));
% Our parametrization for gev_sig is: gev_sig = exp(gev_lsi). ;
%%%%%%%%;
[~,rank_mip_10_r_] = sort(mip_10_r_,'ascend'); [~,rank_mip_10_r_] = sort(rank_mip_10_r_,'ascend'); rank_mip_10_r_ = (rank_mip_10_r_-0.5)/max(1,numel(mip_10_r_)); pro_mip_10_r_ = sqrt(2.0)*erfinv(2.0*rank_mip_10_r_ - 1.0);
tmp_index_mip_ = efind(rank_mip_10_r_<=1-CDF_threshold); %<-- exclude upper percentile. ;
...
tmp_gev_mip_cdf = @(x_,gev_mip_sze,gev_mip_lsi) gevcdf(x_,exp(gev_mip_sze)./(1+exp(gev_mip_sze)),exp(gev_mip_lsi),gev_mip_med_mu0(exp(gev_mip_sze)./(1+exp(gev_mip_sze)),exp(gev_mip_lsi)));
tmp_gev_mip_ks = @(gev_mip_sze,gev_mip_lsi) sum(abs(tmp_rank_mip_10_r_ - tmp_gev_mip_cdf(tmp_mip_10_r_,gev_mip_sze,gev_mip_lsi)),'all');
n_gev_mip_sze = 64; gev_mip_sze_ = linspace(-5.0,+5.0,n_gev_mip_sze);
n_gev_mip_lsi = 96; gev_mip_lsi_ = log(std(tmp_mip_10_r_,1)) + linspace(-0.5,+0.5,n_gev_mip_lsi);
gev_mip_ks_zs__ = zeros(n_gev_mip_sze,n_gev_mip_lsi); %<-- kolmogorov-smirnov makes more sense here. ;
for ngev_mip_sze=0:n_gev_mip_sze-1;
if mod(ngev_mip_sze,4)==0; disp(sprintf(' %% ngev_mip_sze %d/%d',ngev_mip_sze,n_gev_mip_sze)); end;
for ngev_mip_lsi=0:n_gev_mip_lsi-1;
gev_mip_ks_zs__(1+ngev_mip_sze,1+ngev_mip_lsi) = tmp_gev_mip_ks(gev_mip_sze_(1+ngev_mip_sze),gev_mip_lsi_(1+ngev_mip_lsi));
end;%for ngev_mip_lsi=0:n_gev_mip_lsi-1;
end;%for ngev_mip_sze=0:n_gev_mip_sze-1;
%%%%;
[~,tmp_ij] = min(gev_mip_ks_zs__,[],'all','linear');
tmp_index=tmp_ij-1; tmp_index_sze = mod(tmp_index,n_gev_mip_sze); tmp_index_lsi = floor(tmp_index/n_gev_mip_sze);
tmp_gev_mip_ks_sze = gev_mip_sze_(1+tmp_index_sze); tmp_gev_mip_ks_zet = exp(tmp_gev_mip_ks_sze)./(1+exp(tmp_gev_mip_ks_sze));
tmp_gev_mip_ks_lsi = gev_mip_lsi_(1+tmp_index_lsi); tmp_gev_mip_ks_sig = exp(tmp_gev_mip_ks_lsi);
...
%%%%%%%%;
[~,rank_CoC_10_r_] = sort(CoC_10_r_,'ascend'); [~,rank_CoC_10_r_] = sort(rank_CoC_10_r_,'ascend'); rank_CoC_10_r_ = (rank_CoC_10_r_-0.5)/max(1,numel(CoC_10_r_)); pro_CoC_10_r_ = sqrt(2.0)*erfinv(2.0*rank_CoC_10_r_ - 1.0);
tmp_index_CoC_ = efind(rank_CoC_10_r_<=1-CDF_threshold); %<-- exclude upper percentile. ;
...
tmp_gev_CoC_cdf = @(x_,gev_CoC_sze,gev_CoC_lsi) gevcdf(x_,exp(gev_CoC_sze)./(1+exp(gev_CoC_sze)),exp(gev_CoC_lsi),gev_CoC_med_mu0(exp(gev_CoC_sze)./(1+exp(gev_CoC_sze)),exp(gev_CoC_lsi)));
tmp_gev_CoC_ks = @(gev_CoC_sze,gev_CoC_lsi) sum(abs(tmp_rank_CoC_10_r_ - tmp_gev_CoC_cdf(tmp_CoC_10_r_,gev_CoC_sze,gev_CoC_lsi)),'all');
n_gev_CoC_sze = 64; gev_CoC_sze_ = linspace(-5.0,+5.0,n_gev_CoC_sze);
n_gev_CoC_lsi = 96; gev_CoC_lsi_ = log(std(tmp_CoC_10_r_,1)) + linspace(-0.5,+0.5,n_gev_CoC_lsi);
gev_CoC_ks_zs__ = zeros(n_gev_CoC_sze,n_gev_CoC_lsi); %<-- kolmogorov-smirnov makes more sense here. ;
for ngev_CoC_sze=0:n_gev_CoC_sze-1;
if mod(ngev_CoC_sze,4)==0; disp(sprintf(' %% ngev_CoC_sze %d/%d',ngev_CoC_sze,n_gev_CoC_sze)); end;
for ngev_CoC_lsi=0:n_gev_CoC_lsi-1;
gev_CoC_ks_zs__(1+ngev_CoC_sze,1+ngev_CoC_lsi) = tmp_gev_CoC_ks(gev_CoC_sze_(1+ngev_CoC_sze),gev_CoC_lsi_(1+ngev_CoC_lsi));
end;%for ngev_CoC_lsi=0:n_gev_CoC_lsi-1;
end;%for ngev_CoC_sze=0:n_gev_CoC_sze-1;
%%%%;
[~,tmp_ij] = min(gev_CoC_ks_zs__,[],'all','linear');
tmp_index=tmp_ij-1; tmp_index_sze = mod(tmp_index,n_gev_CoC_sze); tmp_index_lsi = floor(tmp_index/n_gev_CoC_sze);
tmp_gev_CoC_ks_sze = gev_CoC_sze_(1+tmp_index_sze); tmp_gev_CoC_ks_zet = exp(tmp_gev_CoC_ks_sze)./(1+exp(tmp_gev_CoC_ks_sze));
tmp_gev_CoC_ks_lsi = gev_CoC_lsi_(1+tmp_index_lsi); tmp_gev_CoC_ks_sig = exp(tmp_gev_CoC_ks_lsi);
...
legend({'emp','ks'},'Location','NorthWest');
set(gca,'FontSize',fontsize_use);
%%%%;
subplot(1,2,2);
hold on;
...
set(gca,'FontSize',fontsize_use);
%%%%;
fname_fig_pre = sprintf('%s/test_Kexin_147_mip_vs_CoC_3_2_ks_zetapos_FIGK',dir_jpg);
fname_fig_jpg = sprintf('%s.jpg',fname_fig_pre);
if (flag_replot | ~exist(fname_fig_jpg));
disp(sprintf(' %% Writing %s',fname_fig_pre));
sgtitle(fname_fig_pre,'Interpreter','none');
...
set(gca,'FontSize',fontsize_use);
%%%%%%%%;
fname_fig_pre = sprintf('%s/test_Kexin_147_mip_vs_CoC_3_2_ks_zetapos_FIGL',dir_jpg);
fname_fig_jpg = sprintf('%s.jpg',fname_fig_pre);
if (flag_replot | ~exist(fname_fig_jpg));
disp(sprintf(' %% Writing %s',fname_fig_pre));
sgtitle(fname_fig_pre,'Interpreter','none');
...
set(gca,'FontSize',fontsize_use);
%%%%%%%%;
fname_fig_pre = sprintf('%s/test_Kexin_147_mip_vs_CoC_3_2_10_ks_zetapos_FIGM',dir_jpg);
fname_fig_jpg = sprintf('%s.jpg',fname_fig_pre);
if (flag_replot | ~exist(fname_fig_jpg));
disp(sprintf(' %% Writing %s',fname_fig_pre));
sgtitle(fname_fig_pre,'Interpreter','none');
