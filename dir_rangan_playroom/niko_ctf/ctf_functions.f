...
c     compute the envelope function
      do rr = 1,ngridr
         envelop(rr) = exp(-(theta(rr)**2/theta0**2))
      enddo
...
c     box size D
      ...
      rad = sqrt(rad)
      angle = rad*thetatr
      angspt=atan2(real(m),real(l))
      c1=2.0d0*pi*angle*angle/(2.0d0*lambda)
      c2=-c1*Cs*angle*angle/2.0d0
      angdif=angspt-angast
      ccos=cos(2.0d0*angdif)
      df = 0.5d0*(df1+df2+ccos*(df1-df2))
      chi=c1*df+c2

      ctfv=-w1*sin(chi)-w2*cos(chi)
...
c     compute the envelope function
      do rr = 1,ngridr
         Bth(rr) = exp(-(theta(rr)**2/theta0**2))
      enddo
