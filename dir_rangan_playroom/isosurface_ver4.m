function isosurface_ver4(n_,dirname,tname) ;
% for use with test_ver6.f, which has model_A and model_B ;
figure;
...
if (ncur==0);
F_x_c = MDA_read_c16(sprintf('%s/S_A_ori_.mda',dirname)); F_x_c = real(permute(F_x_c,[2,1,3]));
...
elseif (ncur==1);
F_x_c = MDA_read_c16(sprintf('%s/S_B_ori_.mda',dirname)); F_x_c = real(permute(F_x_c,[2,1,3]));
X = MDA_read_r8(sprintf('%s/x_pre_.mda',dirname)); X = permute(X,[2,1,3]);
Y = MDA_read_r8(sprintf('%s/y_pre_.mda',dirname)); Y = permute(Y,[2,1,3]);
Z = MDA_read_r8(sprintf('%s/z_pre_.mda',dirname)); Z = permute(Z,[2,1,3]);
elseif (ncur==2);
F_A_x_c = MDA_read_c16(sprintf('%s/S_A_ori_.mda',dirname)); 
F_B_x_c = MDA_read_c16(sprintf('%s/S_B_ori_.mda',dirname)); 
