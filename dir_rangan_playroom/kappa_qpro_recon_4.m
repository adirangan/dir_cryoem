...
str_thisfunction = 'kappa_qpro_recon_4';
...
parameter.kernel_qpro_l_max_band = floor(24/2)-1;%parameter.kernel_qpro_l_max_band = +Inf;
...
parameter.kernel_qpro_polar_a_pole_north=12*pi/24;
parameter.kernel_qpro_polar_a_pole_south=12*pi/24;
...
parameter.flag_recalc_dtau_dtau_qref_from_data=0;
parameter.kernel_qpro_l_max_use = 24;
parameter.kernel_qpro_l_max_band = floor(24/2)-1;
parameter.kernel_qpro_l_max_ext = ceil(1.25*parameter.kernel_qpro_l_max_use);
parameter.flag_kernel_full = 1; %<-- if set to 1 then use full brute-force kernel. ;
parameter.kernel_qpro_polar_a_pole_north=12*pi/24;
parameter.kernel_qpro_polar_a_pole_south=12*pi/24;
...
R_neg = Rz(b-1*db);
DR_mid = dRz(b+0*db);
DR_dif = (R_pos - R_neg)/max(1e-12,2*db);
fnorm_disp(flag_verbose,'DR_dif',DR_dif,'DR_mid',DR_mid);
%%%%;
a = 1*pi*rand(); da = 1e-3; 
R_mid = Ry(a+0*da);
R_pos = Ry(a+1*da);
R_neg = Ry(a-1*da);
DR_mid = dRy(a+0*da);
DR_dif = (R_pos - R_neg)/max(1e-12,2*da);
fnorm_disp(flag_verbose,'DR_dif',DR_dif,'DR_mid',DR_mid);
%%%%;

%%%%%%%%;
if isempty(l_max); l_max = kernel_qpro_l_max_use; end;
l_val_ = transpose([0:l_max]);
%%%%%%%%;
% Defining legendre polynomials via chebfun. ;
%%%%%%%%;
...
%%%%;
k_p_r_max = l_max/(2*pi); k_eq_d = 0.5/(2*pi);
...
%%%%%%%%;
a_deltafunct_band_I_I_k_Y_lm_ = sqrt(2*pi)*sqrt(4*pi)*sqrt(1+2*Y_l_val_).*(Y_m_val_==0).*(Y_l_val_<=kernel_qpro_l_max_band);
if ~exist('Ylm_uklma___','var'); Ylm_uklma___=[]; end;
if ~exist('k_p_azimu_b_sub_uka__','var'); k_p_azimu_b_sub_uka__ = []; end;
if ~exist('k_p_polar_a_sub_uka__','var'); k_p_polar_a_sub_uka__ = []; end;
if ~exist('l_max_uk_','var'); l_max_uk_ = []; end;
...
%%%%;
deconvolve_lm_ = (sqrt(4*pi)*sqrt(1+2*Y_l_val_).*(Y_l_val_<=kernel_qpro_l_max_band))./max(1e-12,kappa_norm_(1+Y_l_val_));
if flag_disp;
...
fnorm_disp(flag_verbose,'chebfun_kernel_norm_qpro_(cos(polar_a_shell_))',chebfun_kernel_norm_qpro_(cos(polar_a_shell_)),'kernel_qpro_k_p_quad_*sqrt(2*pi)',kernel_qpro_k_p_quad_*sqrt(2*pi));
[ ...
...
fnorm_disp(flag_verbose,'kernel_qpro_k_Y_form_',kernel_qpro_k_Y_form_,'kernel_qpro_k_Y_quad_',kernel_qpro_k_Y_quad_);
if flag_disp;
figure(1+nf);nf=nf+1;clf;figsml;
...
%%%%;
n_lm_use = (kernel_qpro_l_max_use+1).^2;
n_lm_ext = (kernel_qpro_l_max_ext+1).^2;
...
fnorm_disp(flag_verbose,'tmp_R_ker_',tmp_R_ker_,'chebfun_kernel_norm_qpro_(1-tmp_R_d2_/2)',chebfun_kernel_norm_qpro_(1-tmp_R_d2_/2));
...
fnorm_disp(flag_verbose,'a_I_I_crop_k_Y_lm_',a_I_I_crop_k_Y_lm_,'a_I_I_k_Y_lm_',a_I_I_k_Y_lm_);
fnorm_disp(flag_verbose,'a_R_I_crop_k_Y_lm_',a_R_I_crop_k_Y_lm_,'a_R_I_k_Y_lm_',a_R_I_k_Y_lm_);
fnorm_disp(flag_verbose,'a_I_R_crop_k_Y_lm_',a_I_R_crop_k_Y_lm_,'a_I_R_k_Y_lm_',a_I_R_k_Y_lm_);
fnorm_disp(flag_verbose,'a_R_R_crop_k_Y_lm_',a_R_R_crop_k_Y_lm_,'a_R_R_k_Y_lm_',a_R_R_k_Y_lm_);
fnorm_disp(flag_verbose,'a_R_I_crop_k_Y_lm_',a_R_I_crop_k_Y_lm_,'a_I_R_crop_k_Y_lm_',a_I_R_crop_k_Y_lm_);
fnorm_disp(flag_verbose,'a_I_I_crop_k_Y_lm_',a_I_I_crop_k_Y_lm_,'a_R_R_crop_k_Y_lm_',a_R_R_crop_k_Y_lm_);
%%%%;
...
fnorm_disp(flag_verbose,'a_deltafunct_band_I_I_k_Y_lm_',a_deltafunct_band_I_I_k_Y_lm_,'a_deconvolve_I_I_crop_k_Y_lm_',a_deconvolve_I_I_crop_k_Y_lm_);
fnorm_disp(flag_verbose,'a_deltafunct_band_I_R_k_Y_lm_',a_deltafunct_band_I_R_k_Y_lm_,'a_deconvolve_I_R_crop_k_Y_lm_',a_deconvolve_I_R_crop_k_Y_lm_);
fnorm_disp(flag_verbose,'a_deltafunct_band_R_I_k_Y_lm_',a_deltafunct_band_R_I_k_Y_lm_,'a_deconvolve_R_I_crop_k_Y_lm_',a_deconvolve_R_I_crop_k_Y_lm_);
fnorm_disp(flag_verbose,'a_deltafunct_band_R_R_k_Y_lm_',a_deltafunct_band_R_R_k_Y_lm_,'a_deconvolve_R_R_crop_k_Y_lm_',a_deconvolve_R_R_crop_k_Y_lm_);
%%%%%%%%;
[ ...
...
fnorm_disp(flag_verbose,'a_I_dR_dif_crop_k_Y_lm_',a_I_dR_dif_crop_k_Y_lm_,'a_I_dR_mid_crop_k_Y_lm_',a_I_dR_mid_crop_k_Y_lm_);
%%%%;
...
fnorm_disp(flag_verbose,'a_deconvolve_I_dR_dif_crop_k_Y_lm_',a_deconvolve_I_dR_dif_crop_k_Y_lm_,'a_deconvolve_I_dR_mid_crop_k_Y_lm_',a_deconvolve_I_dR_mid_crop_k_Y_lm_);
%%%%%%%%;
...
%%%%%%%%;
% reconstructing with kappa_qpro_apply_3. ;
%%%%%%%%;
KAPPA = [];
n_w_max = n_w;
...
[ ...
 parameter ...
,KAPPA ...
,a_restore_mid_C2M0_k_Y_yk__ ...
,a_restore_mid_C1M1_k_Y_yk__ ...
,a_restore_mid_C0M2_k_Y_yk__ ...
,dtau_a_restore_mid_C2M0_k_Y_yk__ ...
,dtau_a_restore_mid_C1M1_k_Y_yk__ ...
,dtau_a_restore_mid_C0M2_k_Y_yk__ ...
,dtau_dtau_a_restore_mid_C2M0_k_Y_yk__ ...
,dtau_dtau_a_restore_mid_C1M1_k_Y_yk__ ...
,dtau_dtau_a_restore_mid_C0M2_k_Y_yk__ ...
] = ...
kappa_qpro_apply_3( ...
 parameter ...
,KAPPA ...
...
fnorm_disp(flag_verbose,'T_restore_k_Y_lm_',T_restore_k_Y_lm_,'a_restore_mid_C1M1_k_Y_yk__*n_w_max/(2*pi)',a_restore_mid_C1M1_k_Y_yk__*n_w_max/(2*pi));
%%%%%%%%;

%%%%%%%%;
% Converting each volume. ;
%%%%%%%%;
clear qref_Ylm_uklma___;
clear qref_k_p_azimu_b_sub_uka__;
clear qref_k_p_polar_a_sub_uka__;
clear qref_l_max_uk_;
clear qref_index_nu_n_k_per_shell_from_nk_p_r_;
clear qref_index_k_per_shell_uka__;
for ntype=0:9-1;
na=0;
if ntype==na; tmp_k_Y_yk__ = a_restore_mid_C2M0_k_Y_yk__; end; na=na+1;
if ntype==na; tmp_k_Y_yk__ = a_restore_mid_C1M1_k_Y_yk__; end; na=na+1;
if ntype==na; tmp_k_Y_yk__ = a_restore_mid_C0M2_k_Y_yk__; end; na=na+1;
if ntype==na; tmp_k_Y_yk__ = dtau_a_restore_mid_C2M0_k_Y_yk__; end; na=na+1;
if ntype==na; tmp_k_Y_yk__ = dtau_a_restore_mid_C1M1_k_Y_yk__; end; na=na+1;
if ntype==na; tmp_k_Y_yk__ = dtau_a_restore_mid_C0M2_k_Y_yk__; end; na=na+1;
if ntype==na; tmp_k_Y_yk__ = dtau_dtau_a_restore_mid_C2M0_k_Y_yk__; end; na=na+1;
if ntype==na; tmp_k_Y_yk__ = dtau_dtau_a_restore_mid_C1M1_k_Y_yk__; end; na=na+1;
if ntype==na; tmp_k_Y_yk__ = dtau_dtau_a_restore_mid_C0M2_k_Y_yk__; end; na=na+1;
...
na=0;
if ntype==na; a_restore_mid_C2M0_k_p_qk__ = tmp_k_p_qk__; end;; na=na+1;
if ntype==na; a_restore_mid_C1M1_k_p_qk__ = tmp_k_p_qk__; end;; na=na+1;
if ntype==na; a_restore_mid_C0M2_k_p_qk__ = tmp_k_p_qk__; end;; na=na+1;
if ntype==na; dtau_a_restore_mid_C2M0_k_p_qk__ = tmp_k_p_qk__; end;; na=na+1;
if ntype==na; dtau_a_restore_mid_C1M1_k_p_qk__ = tmp_k_p_qk__; end;; na=na+1;
if ntype==na; dtau_a_restore_mid_C0M2_k_p_qk__ = tmp_k_p_qk__; end;; na=na+1;
if ntype==na; dtau_dtau_a_restore_mid_C2M0_k_p_qk__ = tmp_k_p_qk__; end;; na=na+1;
if ntype==na; dtau_dtau_a_restore_mid_C1M1_k_p_qk__ = tmp_k_p_qk__; end;; na=na+1;
if ntype==na; dtau_dtau_a_restore_mid_C0M2_k_p_qk__ = tmp_k_p_qk__; end;; na=na+1;
end;%for ntype=0:9-1;

%%%%%%%%;
% Now reconstructing with kappa_qpro_apply_4. ;
%%%%%%%%;
tmp_parameter = parameter;
tmp_parameter.flag_recalc_knn = 1;
tmp_parameter.flag_check = 1;
[ ...
 ~ ...
,~ ...
,b_restore_mid_C2M0_k_p_qk__ ...
,b_restore_mid_C1M1_k_p_qk__ ...
,b_restore_mid_C0M2_k_p_qk__ ...
,dtau_b_restore_mid_C2M0_k_p_qk__ ...
,dtau_b_restore_mid_C1M1_k_p_qk__ ...
,dtau_b_restore_mid_C0M2_k_p_qk__ ...
,dtau_dtau_b_restore_mid_C2M0_k_p_qk__ ...
,dtau_dtau_b_restore_mid_C1M1_k_p_qk__ ...
,dtau_dtau_b_restore_mid_C0M2_k_p_qk__ ...
] = ...
kappa_qpro_apply_4( ...
 tmp_parameter ...
,[] ...
...
%%%%%%%%;
fnorm_disp(flag_verbose,'a_restore_mid_C2M0_k_p_qk__',a_restore_mid_C2M0_k_p_qk__,'b_restore_mid_C2M0_k_p_qk__',b_restore_mid_C2M0_k_p_qk__);
fnorm_disp(flag_verbose,'a_restore_mid_C1M1_k_p_qk__',a_restore_mid_C1M1_k_p_qk__,'b_restore_mid_C1M1_k_p_qk__',b_restore_mid_C1M1_k_p_qk__);
fnorm_disp(flag_verbose,'a_restore_mid_C0M2_k_p_qk__',a_restore_mid_C0M2_k_p_qk__,'b_restore_mid_C0M2_k_p_qk__',b_restore_mid_C0M2_k_p_qk__);
fnorm_disp(flag_verbose,'dtau_a_restore_mid_C2M0_k_p_qk__',dtau_a_restore_mid_C2M0_k_p_qk__,'dtau_b_restore_mid_C2M0_k_p_qk__',dtau_b_restore_mid_C2M0_k_p_qk__);
fnorm_disp(flag_verbose,'dtau_a_restore_mid_C1M1_k_p_qk__',dtau_a_restore_mid_C1M1_k_p_qk__,'dtau_b_restore_mid_C1M1_k_p_qk__',dtau_b_restore_mid_C1M1_k_p_qk__);
fnorm_disp(flag_verbose,'dtau_a_restore_mid_C0M2_k_p_qk__',dtau_a_restore_mid_C0M2_k_p_qk__,'dtau_b_restore_mid_C0M2_k_p_qk__',dtau_b_restore_mid_C0M2_k_p_qk__);
fnorm_disp(flag_verbose,'dtau_dtau_a_restore_mid_C2M0_k_p_qk__',dtau_dtau_a_restore_mid_C2M0_k_p_qk__,'dtau_dtau_b_restore_mid_C2M0_k_p_qk__',dtau_dtau_b_restore_mid_C2M0_k_p_qk__);
fnorm_disp(flag_verbose,'dtau_dtau_a_restore_mid_C1M1_k_p_qk__',dtau_dtau_a_restore_mid_C1M1_k_p_qk__,'dtau_dtau_b_restore_mid_C1M1_k_p_qk__',dtau_dtau_b_restore_mid_C1M1_k_p_qk__);
fnorm_disp(flag_verbose,'dtau_dtau_a_restore_mid_C0M2_k_p_qk__',dtau_dtau_a_restore_mid_C0M2_k_p_qk__,'dtau_dtau_b_restore_mid_C0M2_k_p_qk__',dtau_dtau_b_restore_mid_C0M2_k_p_qk__);
...
[ ...
 ~ ...
,~ ...
,a_restore_pos_C2M0_k_Y_yk__ ...
,a_restore_pos_C1M1_k_Y_yk__ ...
,a_restore_pos_C0M2_k_Y_yk__ ...
] = ...
kappa_qpro_apply_3( ...
 parameter ...
,KAPPA ...
...
[ ...
 ~ ...
,~ ...
,a_restore_neg_C2M0_k_Y_yk__ ...
,a_restore_neg_C1M1_k_Y_yk__ ...
,a_restore_neg_C0M2_k_Y_yk__ ...
] = ...
kappa_qpro_apply_3( ...
 parameter ...
,KAPPA ...
...
dtau_a_restore_dif_C1M1_k_Y_yk__ = (a_restore_pos_C1M1_k_Y_yk__ - a_restore_neg_C1M1_k_Y_yk__)/max(1e-12,2*dtau);
fnorm_disp(flag_verbose,'dtau_a_restore_dif_C1M1_k_Y_yk__',dtau_a_restore_dif_C1M1_k_Y_yk__,'dtau_a_restore_mid_C1M1_k_Y_yk__',dtau_a_restore_mid_C1M1_k_Y_yk__);
%%%%;
if flag_disp;
figure(1+nf);nf=nf+1;clf;figmed;
subplot(1,2,1);
plot(Y_l_val_,log10(abs(dtau_a_restore_dif_C1M1_k_Y_yk__-dtau_a_restore_mid_C1M1_k_Y_yk__)),'.');
xlim([0,l_max]); xlabel('Y_l_val_','Interpreter','none');
ylabel('log10(abs(error))','Interpreter','none');
title('log10(abs(dtau error))','Interpreter','none');
subplot(1,2,2);
plot(Y_l_val_,log10(abs(dtau_a_restore_dif_C1M1_k_Y_yk__-dtau_a_restore_mid_C1M1_k_Y_yk__)./abs(dtau_a_restore_dif_C1M1_k_Y_yk__)),'.');
xlim([0,l_max]); xlabel('Y_l_val_','Interpreter','none');
...
hold on;
rlim_dif_ = prctile(real(dtau_a_restore_dif_C1M1_k_Y_yk__(1+tmp_index_)),[  0,100]); rlim_dif_ = mean(rlim_dif_) + 0.5*1.25*diff(rlim_dif_)*[-1,+1]; if diff(rlim_dif_)==0; rlim_dif_=[-1,+1]; end;
rlim_mid_ = prctile(real(dtau_a_restore_mid_C1M1_k_Y_yk__(1+tmp_index_)),[  0,100]); rlim_mid_ = mean(rlim_mid_) + 0.5*1.25*diff(rlim_mid_)*[-1,+1]; if diff(rlim_mid_)==0; rlim_mid_=[-1,+1]; end;
rlim_ = [min([rlim_dif_,rlim_mid_]),max([rlim_dif_,rlim_mid_])];
ilim_dif_ = prctile(imag(dtau_a_restore_dif_C1M1_k_Y_yk__(1+tmp_index_)),[  0,100]); ilim_dif_ = mean(ilim_dif_) + 0.5*1.25*diff(ilim_dif_)*[-1,+1]; if diff(ilim_dif_)==0; ilim_dif_=[-1,+1]; end;
ilim_mid_ = prctile(imag(dtau_a_restore_mid_C1M1_k_Y_yk__(1+tmp_index_)),[  0,100]); ilim_mid_ = mean(ilim_mid_) + 0.5*1.25*diff(ilim_mid_)*[-1,+1]; if diff(ilim_mid_)==0; ilim_mid_=[-1,+1]; end;
...
plot(real(dtau_a_restore_dif_C1M1_k_Y_yk__(1+tmp_index_)),real(dtau_a_restore_mid_C1M1_k_Y_yk__(1+tmp_index_)),'ro');
plot(imag(dtau_a_restore_dif_C1M1_k_Y_yk__(1+tmp_index_)),imag(dtau_a_restore_mid_C1M1_k_Y_yk__(1+tmp_index_)),'bx');
hold off;
...
%%%%%%%%;
dtau_dtau_a_restore_dif_C1M1_k_Y_yk__ = (a_restore_pos_C1M1_k_Y_yk__ - 2*a_restore_mid_C1M1_k_Y_yk__ + a_restore_neg_C1M1_k_Y_yk__)/max(1e-12,dtau*dtau);
fnorm_disp(flag_verbose,'dtau_dtau_a_restore_dif_C1M1_k_Y_yk__',dtau_dtau_a_restore_dif_C1M1_k_Y_yk__,'dtau_dtau_a_restore_mid_C1M1_k_Y_yk__',dtau_dtau_a_restore_mid_C1M1_k_Y_yk__);
%%%%;
if flag_disp;
figure(1+nf);nf=nf+1;clf;figmed;
subplot(1,2,1);
plot(Y_l_val_,log10(abs(dtau_dtau_a_restore_dif_C1M1_k_Y_yk__-dtau_dtau_a_restore_mid_C1M1_k_Y_yk__)),'.');
xlim([0,l_max]); xlabel('Y_l_val_','Interpreter','none');
ylabel('log10(abs(error))','Interpreter','none');
title('log10(abs(dtau_dtau error))','Interpreter','none');
subplot(1,2,2);
plot(Y_l_val_,log10(abs(dtau_dtau_a_restore_dif_C1M1_k_Y_yk__-dtau_dtau_a_restore_mid_C1M1_k_Y_yk__)./abs(dtau_dtau_a_restore_dif_C1M1_k_Y_yk__)),'.');
xlim([0,l_max]); xlabel('Y_l_val_','Interpreter','none');
ylabel('log10(abs(relative error))','Interpreter','none');
...
tmp_index_ = efind(Y_l_val_==l_val);
hold on;
rlim_dif_ = prctile(real(dtau_dtau_a_restore_dif_C1M1_k_Y_yk__(1+tmp_index_)),[  0,100]); rlim_dif_ = mean(rlim_dif_) + 0.5*1.25*diff(rlim_dif_)*[-1,+1]; if diff(rlim_dif_)==0; rlim_dif_=[-1,+1]; end;
rlim_mid_ = prctile(real(dtau_dtau_a_restore_mid_C1M1_k_Y_yk__(1+tmp_index_)),[  0,100]); rlim_mid_ = mean(rlim_mid_) + 0.5*1.25*diff(rlim_mid_)*[-1,+1]; if diff(rlim_mid_)==0; rlim_mid_=[-1,+1]; end;
rlim_ = [min([rlim_dif_,rlim_mid_]),max([rlim_dif_,rlim_mid_])];
ilim_dif_ = prctile(imag(dtau_dtau_a_restore_dif_C1M1_k_Y_yk__(1+tmp_index_)),[  0,100]); ilim_dif_ = mean(ilim_dif_) + 0.5*1.25*diff(ilim_dif_)*[-1,+1]; if diff(ilim_dif_)==0; ilim_dif_=[-1,+1]; end;
ilim_mid_ = prctile(imag(dtau_dtau_a_restore_mid_C1M1_k_Y_yk__(1+tmp_index_)),[  0,100]); ilim_mid_ = mean(ilim_mid_) + 0.5*1.25*diff(ilim_mid_)*[-1,+1]; if diff(ilim_mid_)==0; ilim_mid_=[-1,+1]; end;
ilim_ = [min([ilim_dif_,ilim_mid_]),max([ilim_dif_,ilim_mid_])];
dlim_ = [min([rlim_,ilim_]),max([rlim_,ilim_])];
plot(dlim_,dlim_,'-','Color',0.85*[1,1,1]);
plot(real(dtau_dtau_a_restore_dif_C1M1_k_Y_yk__(1+tmp_index_)),real(dtau_dtau_a_restore_mid_C1M1_k_Y_yk__(1+tmp_index_)),'ro');
plot(imag(dtau_dtau_a_restore_dif_C1M1_k_Y_yk__(1+tmp_index_)),imag(dtau_dtau_a_restore_mid_C1M1_k_Y_yk__(1+tmp_index_)),'bx');
hold off;
xlim(dlim_); ylim(dlim_);
axisnotick;
xlabel('dif'); ylabel('mid')
...
fnorm_disp(flag_verbose,'a_restore_mid_C2M0_k_p_quad_',a_restore_mid_C2M0_k_p_quad_,'a_recover_mid_C2M0_k_p_quad_',a_recover_mid_C2M0_k_p_quad_);
fnorm_disp(flag_verbose,'a_restore_mid_C2M0_k_Y_yk__',a_restore_mid_C2M0_k_Y_yk__,'a_recover_mid_C2M0_k_Y_yk__',a_recover_mid_C2M0_k_Y_yk__);
%%%%;
[ ...
 a_restore_mid_C1M1_k_p_quad_ ...
] = ...
convert_spharm_to_k_p_4( ...
 flag_verbose ...
,n_shell ...
...
fnorm_disp(flag_verbose,'a_restore_mid_C1M1_k_p_quad_',a_restore_mid_C1M1_k_p_quad_,'a_recover_mid_C1M1_k_p_quad_',a_recover_mid_C1M1_k_p_quad_);
fnorm_disp(flag_verbose,'a_restore_mid_C1M1_k_Y_yk__',a_restore_mid_C1M1_k_Y_yk__,'a_recover_mid_C1M1_k_Y_yk__',a_recover_mid_C1M1_k_Y_yk__);
%%%%;
[ ...
 a_restore_mid_C0M2_k_p_quad_ ...
] = ...
convert_spharm_to_k_p_4( ...
 flag_verbose ...
,n_shell ...
...
fnorm_disp(flag_verbose,'a_restore_mid_C0M2_k_p_quad_',a_restore_mid_C0M2_k_p_quad_,'a_recover_mid_C0M2_k_p_quad_',a_recover_mid_C0M2_k_p_quad_);
fnorm_disp(flag_verbose,'a_restore_mid_C0M2_k_Y_yk__',a_restore_mid_C0M2_k_Y_yk__,'a_recover_mid_C0M2_k_Y_yk__',a_recover_mid_C0M2_k_Y_yk__);
%%%%;
tmp_a_a_k_p_quad_ = a_restore_mid_C0M2_k_p_quad_ .* a_restore_mid_C1M1_k_p_quad_ ;
[ ...
 tmp_a_a_k_Y_quad_yk__ ...
] = ...
convert_k_p_to_spharm_4( ...
 flag_verbose ...
,n_shell ...
...
fnorm_disp(flag_verbose,'tmp_a_a_k_p_quad_',tmp_a_a_k_p_quad_,'tmp_a_a_k_p_reco_',tmp_a_a_k_p_reco_);
fnorm_disp(flag_verbose,'tmp_a_a_k_Y_quad_yk__',tmp_a_a_k_Y_quad_yk__,'tmp_a_a_k_Y_reco_yk__',tmp_a_a_k_Y_reco_yk__);
if flag_disp;
figure(1+nf);nf=nf+1;clf;figbig;
markersize_use = 12;
subplot(3,1,1);
hold on;
plot(Y_l_val_,abs(a_restore_mid_C0M2_k_Y_yk__),'o','MarkerSize',markersize_use,'MarkerFaceColor','c');
plot(kernel_qpro_l_max_band*[1,1],[0,max(abs(a_restore_mid_C0M2_k_Y_yk__))],'k-','LineWidth',2);
xlim([0,l_max]); xlabel('l_val'); title('a_restore_mid_C0M2_k_Y_yk__','Interpreter','none');
subplot(3,1,2);
hold on;
plot(Y_l_val_,abs(a_restore_mid_C1M1_k_Y_yk__),'o','MarkerSize',markersize_use,'MarkerFaceColor','c');
plot(kernel_qpro_l_max_band*[1,1],[0,max(abs(a_restore_mid_C1M1_k_Y_yk__))],'k-','LineWidth',2);
xlim([0,l_max]); xlabel('l_val'); title('a_restore_mid_C1M1_k_Y_yk__','Interpreter','none');
subplot(3,1,3);
hold on;
plot(Y_l_val_,abs(tmp_a_a_k_Y_quad_yk__),'o','MarkerSize',markersize_use,'MarkerFaceColor','c');
plot(1*kernel_qpro_l_max_band*[1,1],[0,max(abs(tmp_a_a_k_Y_quad_yk__))],'k-','LineWidth',2);
plot(2*kernel_qpro_l_max_band*[1,1],[0,max(abs(tmp_a_a_k_Y_quad_yk__))],'k-','LineWidth',2);
xlim([0,l_max]); xlabel('l_val'); title('tmp_a_a_k_Y_quad_yk__','Interpreter','none');
sgtitle('degree doubling','Interpreter','none');
end;%if flag_disp;
