...
verbose=0;
if (verbose); disp(sprintf(' %% [entering image_center_1]')); end;
...
while flag_continue;
M_x_c_ = interp_k_p_to_x_c_xxnufft(n_x,diameter_x_c,n_x,diameter_x_c,n_k_p_r,k_p_r_,n_w_,M_k_p_.*weight_2d_k_all_*(2*pi)^2)*sqrt(n_x^2) * n_w_sum;
if isempty(M_x_c_0in_); M_x_c_0in_ = M_x_c_; end;
...
if (verbose); disp(sprintf(' %% [finished image_center_1]')); end;
