...
% Setting up figure for illustration. ;
%%%%%%%%;
...
lambda = 1; rseed  = 1; sigma = 0.5;
rng(rseed);rseed=rseed+1;
A_q_ = randn(n_q,1) + i*randn(n_q,1);
A_w_ = F_wq__*A_q_;

sigma_true = 0.5; str_sigma_true = sprintf('s%.3d',round(1000*sigma_true));

if flag_disp>1;
figure(1);clf;figbig;
for rseed=0:48-1;
rng(rseed); A_q_ = randn(n_q,1) + i*randn(n_q,1); A_w_ = F_wq__*A_q_;
subplot(6,8,1+rseed); plot(A_w_,'.'); axisnotick;
end;%for rseed=0:48-1;
end;%if flag_disp>1;
...
%%%%%%%%;
rseed = 20;
str_rseed = sprintf('r%.2d',rseed);
str_xfix = sprintf('%s%s%s%s%s',str_q_max,str_n_M,str_sigma_true,str_n_iteration,str_rseed);

rng(rseed);
A_q_ = randn(n_q,1) + i*randn(n_q,1); A_w_ = F_wq__*A_q_;
%%%%%%%%;
rng(rseed);rseed=rseed+1;
...
if flag_disp>0;
nf_base = nf;
figure(1+nf_base);nf=nf+1;clf;set(gcf,'Position',1+[0,0,1024,1024+128]);
fontsize_use = 12;
hold on;
...
B_init_q_ = lambda*A_q_ + (1-lambda)*fnorm(A_q_)*rand_q_;
B_init_w_ = F_wq__*B_init_q_;
B_sher_q_ = B_init_q_;
B_sher_w_ = B_init_w_;
B_zero_w_ = B_init_w_;
B_emax_w_ = B_init_w_;
...
%plot(real(B_init_w_),imag(B_init_w_),'c-','LineWidth',linewidth_big);
xlabel('real'); ylabel('imag'); grid on;
end;%for ns=1:3-1;
end;%if flag_disp>0;
%%%%;
...
subplot(2,2,4); cla;
hold on;
%plot(1+[0:niteration],log10(Error_sher_k1_i_(1+[0:niteration])),'k.-');
plot(1+[0:niteration],(Error_sher_k1_i_(1+[0:niteration])).^2/n_gamma,'k.-');
%plot(1+[0:niteration],log10(Error_zero_k1_i_(1+[0:niteration])),'bx');
%plot(1+[0:niteration],log10(Error_emax_k1_i_(1+[0:niteration])),'gx');
%plot(1+[0:niteration],log10(Error_alte_k1_i_(1+[0:niteration])),'.-','Color',0.85*[1,1,1]);
plot(1+[0:niteration],(Error_alte_k1_i_(1+[0:niteration])).^2/n_gamma,'.-','Color',0.85*[1,1,1]);
hold off;
%xlabel('iteration'); ylabel('log10(Error)','Interpreter','none'); title('log10(Error_k1)','Interpreter','none');
xlabel('iteration'); ylabel('Error','Interpreter','none'); title('Error_k1','Interpreter','none');
xlim([1,n_iteration]);
grid on;
%%%%;
...
fname_fig_pre = sprintf('../dir_jpg/test_MSA_sheres_illustration_%s_FIGA',str_xfix);
