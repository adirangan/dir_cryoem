...
parameter.flag_local_exclusion = 0;
str_strategy = ''; 
if (parameter.flag_alternate_MS_vs_SM); str_strategy = sprintf('%sa1',str_strategy); end;
if (parameter.flag_local_exclusion); str_strategy = sprintf('%se1',str_strategy); end;
...
fname_mat = sprintf('%s.mat',XA_fname_pre);
if ( exist(fname_mat,'file')); disp(sprintf(' %% %s found, not deleting',fname_mat)); end;
fname_tmp = sprintf('%s.tmp',XA_fname_pre);
if ( exist(fname_tmp,'file')); disp(sprintf(' %% %s found, deleting',fname_tmp)); delete(fname_tmp); end;
ampmut_wrap_wrap_4( ...
 parameter ...
,n_k_p_r ...
,k_p_r_ ...
...
parameter.flag_qbp_vs_lsq = flag_qbp_vs_lsq;
ampmut_wrap_wrap_4( ...
 parameter ...
,n_k_p_r ...
,k_p_r_ ...
...
nstrategy = flag_local_exclusion + 2*flag_alternate_MS_vs_SM + 4*flag_qbp_vs_lsq;
str_strategy_{1+nstrategy} = str_strategy;
end;%for flag_local_exclusion = [0:1];
end;%for flag_alternate_MS_vs_SM = [0:1];
end;%for flag_qbp_vs_lsq_ = [0:1];
%%%%%%%%;
...
figure(1);clf;figbig;
c_beach__ = colormap_beach(); n_c_beach = size(c_beach__,1); c_beach__(1,:) = 0.85*[1,1,1];
colormap(c_beach__);
