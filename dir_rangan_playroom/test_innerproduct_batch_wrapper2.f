      subroutine test_innerproduct_batch_wrapper_2(verbose,ncur,nlats_
     $    ,grid_k_p_,max_x_c,n_S,ld_S,S_k_p_,n_M,ld_M,M_k_p_,alpha_est_
     $    ,eps_target,N_pixels_in,displacement_max,n_delta_x,n_delta_y
     $    ,n_gamma_z,svd_calculation_type,delta_x_max_,delta_y_max_
     $    ,gamma_z_max_,C_Z_max_)
...
c$$$      parameters for timing
      ...
      if (verbose.gt.0) then
          write(6,'(A)')
     $        '[entering test_innerproduct_batch_wrapper_2]: '
       end if
...
c$$$      calculating all innerproducts
      timing_tic = second()
      call test_innerproduct_batch(verbose,svd_calculation_type,n_r
     $    ,grid_k_p_,n_w_,max_x_c,n_S,ld_S,S_k_p_,n_M,ld_M,M_k_p_
     ...
     $        ,(timing_toc -timing_tic)/(n_S*n_M*n_delta_x*n_delta_y)
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[finished test_innerproduct_batch_wrapper_2]'
      end if
