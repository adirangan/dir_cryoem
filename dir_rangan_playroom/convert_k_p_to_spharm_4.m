...
if (nargin<1+na); a_k_all_=[]; end; na=na+1;
...
n_lm_ = (l_max_+1).^2;
if (flag_verbose); disp(sprintf(' %% [entering %s] n_k_all %d, n_lm_sum %d',str_thisfunction,n_k_all,sum(n_lm_))); end;
a_k_Y_ = zeros(sum(n_lm_),1);
ix=0;
for nk_p_r=0:n_k_p_r-1;
k_p_r = k_p_r_(1+nk_p_r); 
n_k_all_csum = n_k_all_csum_(1+nk_p_r);
if (flag_verbose>1); disp(sprintf(' %% nk_p_r %d/%d k_p_r %0.2f, n_k_all_csum %d --> %0.2f%%',nk_p_r,n_k_p_r,k_p_r,n_k_all_csum,n_k_all_csum/n_k_all)); end;
if (nk_p_r<n_k_p_r-1); n_sub = n_k_all_csum_(1+nk_p_r+1) - n_k_all_csum_(1+nk_p_r); else n_sub = n_k_all - n_k_all_csum ; end;
index_sub_ = n_k_all_csum + transpose(0:n_sub-1);
...
weight_shell_k_sub_ = reshape(weight_shell_k_(1+index_sub_)/max(1e-12,k_p_r^2),[n_sub,1]);
...
%%%%;
tmp_t = tic();
a_k_all_sub_ = reshape(a_k_all_(1+index_sub_),[n_sub,1]);
a_k_Y_sub_ = zeros(n_lm,1);
a_k_Y_sub_ = conj(Ylm_lma__)*(a_k_all_sub_.*weight_shell_k_sub_);
a_k_Y_(1+ix+[0:n_lm-1]) = a_k_Y_sub_;
ix = ix+n_lm;
clear Ylm_lma__;
tmp_t = toc(tmp_t); if (flag_verbose); disp(sprintf(' %% nk_p_r %d/%d a_k_all_sub_: %0.6fs',nk_p_r,n_k_p_r,tmp_t)); end;
end;%for nk_p_r=0:n_k_p_r-1;
if (flag_verbose); disp(sprintf(' %% [finished %s] n_k_all %d, n_lm_sum %d',str_thisfunction,n_k_all,sum(n_lm_))); end;
