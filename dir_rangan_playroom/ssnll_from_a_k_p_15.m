...
if ~exist('dvol_a_R_k_p_Rqk__','var'); a_R_k_p_Rqk__=[]; end;
...
str_thisfunction = 'ssnll_from_a_k_p_15';
...
if (n_w_sum~=n_w_max*n_k_p_r); disp(sprintf(' %% Warning, n_w_ nonuniform in %s: n_w_sum (%d)~=n_w_max (%d)*n_k_p_r (%d)',str_thisfunction,n_w_sum,n_w_max,n_k_p_r)); end;
...
tmp_t = toc(tmp_t); if (flag_verbose>0); disp(sprintf(' %% interpolate_template_7 (flag_ssnll): %0.6fs',tmp_t)); end;
%%%%%%%%;
...
tmp_t = toc(tmp_t); if (flag_verbose>0); disp(sprintf(' %% interpolate_template_7 (flag_dtau_ssnll): %0.6fs',tmp_t)); end;
%%%%%%%%;
end;%if isempty(S_k_p_wkS__);
%%%%%%%%%%%%%%%%;
end;%if  flag_ssnll &  flag_dtau_ssnll & ~flag_dtau_dtau_ssnll ;
...
tmp_t = toc(tmp_t); if (flag_verbose>0); disp(sprintf(' %% interpolate_template_7 (flag_dtau_dtau_ssnll): %0.6fs',tmp_t)); end;
%%%%%%%%;
end;%if isempty(S_k_p_wkS__);
%%%%%%%%%%%%%%%%;
end;%if  flag_ssnll &  flag_dtau_ssnll &  flag_dtau_dtau_ssnll ;
