...
MSA_shape_3(parameter,q_max,n_gamma);
...
if strcmp(str_shape,'flower7'); n_gamma = (14*4)*ceil(n_gamma/(14*4)); end;
...
C_q_ = zeros(n_q,1);
%%%%%%%%;
if strcmp(str_shape,'flower7');
%%%%%%%%;
% 14 segments to the path. ;
% 7 petals per segment, each with speed 1.; 
%%%%;
...
ng=0;
index_segment_00_ = ng + index_d14_; ng = ng + n_gamma_d14; A_00_w_ = exp(2*pi*i*(0+0.5)/14)*(+0 + sin(2*gamma_d14_) + i*(+0 + 0.125*sin(4*gamma_d14_))) ;
index_segment_01_ = ng + index_d14_; ng = ng + n_gamma_d14; A_01_w_ = exp(2*pi*i*(1+0.5)/14)*(+0 + sin(2*gamma_d14_) + i*(+0 + 0.125*sin(4*gamma_d14_))) ;
index_segment_02_ = ng + index_d14_; ng = ng + n_gamma_d14; A_02_w_ = exp(2*pi*i*(2+0.5)/14)*(+0 + sin(2*gamma_d14_) + i*(+0 + 0.125*sin(4*gamma_d14_))) ;
index_segment_03_ = ng + index_d14_; ng = ng + n_gamma_d14; A_03_w_ = exp(2*pi*i*(3+0.5)/14)*(+0 + sin(2*gamma_d14_) + i*(+0 + 0.125*sin(4*gamma_d14_))) ;
index_segment_04_ = ng + index_d14_; ng = ng + n_gamma_d14; A_04_w_ = exp(2*pi*i*(4+0.5)/14)*(+0 + sin(2*gamma_d14_) + i*(+0 + 0.125*sin(4*gamma_d14_))) ;
index_segment_05_ = ng + index_d14_; ng = ng + n_gamma_d14; A_05_w_ = exp(2*pi*i*(5+0.5)/14)*(+0 + sin(2*gamma_d14_) + i*(+0 + 0.125*sin(4*gamma_d14_))) ;
index_segment_06_ = ng + index_d14_; ng = ng + n_gamma_d14; A_06_w_ = exp(2*pi*i*(6+0.5)/14)*(+0 + sin(2*gamma_d14_) + i*(+0 + 0.125*sin(4*gamma_d14_))) ;
index_segment_07_ = ng + index_d14_; ng = ng + n_gamma_d14; A_07_w_ = conj(A_00_w_);
index_segment_08_ = ng + index_d14_; ng = ng + n_gamma_d14; A_08_w_ = conj(A_01_w_);
index_segment_09_ = ng + index_d14_; ng = ng + n_gamma_d14; A_09_w_ = conj(A_02_w_);
index_segment_10_ = ng + index_d14_; ng = ng + n_gamma_d14; A_10_w_ = conj(A_03_w_);
index_segment_11_ = ng + index_d14_; ng = ng + n_gamma_d14; A_11_w_ = conj(A_04_w_);
index_segment_12_ = ng + index_d14_; ng = ng + n_gamma_d14; A_12_w_ = conj(A_05_w_);
index_segment_13_ = ng + index_d14_; ng = ng + n_gamma_d14; A_13_w_ = conj(A_06_w_);
assert(ng==n_gamma);
A_w_pre_ =  [ ...
A_00_w_ ; ...
A_01_w_ ; ...
...
C_w_pre_(1 + 1*n_gamma_d02 + index_d02_) = conj(C_w_pre_(1 + 0*n_gamma_d02 + index_d02_));
if t_diffuse>0;
C_w_pre_ = MSA_shape_diffuse_0(n_gamma,gamma_w_,C_w_pre_,t_diffuse);
end;%if t_diffuse>0;
C_q_ = F_inv_qw__*C_w_pre_;
%%%%%%%%;
end;%if strcmp(str_shape,'flower7');
%%%%%%%%;
...
if ntype==1; tmp_w_pre_ = B_w_pre_; tmp_str = 'B';  end;
if ntype==2; tmp_w_pre_ = C_w_pre_; tmp_str = 'C';  end;
dir_jpg = sprintf('%s/dir_jpg',pwd); if ~exist(dir_jpg,'dir'); mkdir(dir_jpg); end;
fname_fig_pre = sprintf('%s/MSA_%s_%s_complexplane_pre_FIGA',dir_jpg,str_shape,tmp_str);
fname_fig_jpg = sprintf('%s.jpg',fname_fig_pre);
figure(1+nf);nf=nf+1;clf;figbig;
fontsize_use = 12;
linewidth_use = 8;
subplot_{1} = subplot(1,1,1);
...
if strcmp(str_shape,'figure8'); x_max_use = x_max; end;
if strcmp(str_shape,'flower7'); x_max_use = 1.250; end;
xlim(x_max_use*[-1,+1]);
ylim(x_max_use*[-1,+1]);
set(gca,'XTick',-x_max_use:0.1:+x_max_use);
set(gca,'YTick',-x_max_use:0.1:+x_max_use);
set(gca,'FontSize',fontsize_use);
xtickangle(90); grid on;
axis square;
title(sprintf('image ring for %s',tmp_str));
colormap(subplot_{1},colormap('hsv'));
...
if ntype==1; tmp_w_ = B_w_; tmp_str = 'B'; tmp__ = B__;   end;
if ntype==2; tmp_w_ = C_w_; tmp_str = 'C'; tmp__ = C__;   end;
dir_jpg = sprintf('%s/dir_jpg',pwd); if ~exist(dir_jpg,'dir'); mkdir(dir_jpg); end;
fname_fig_pre = sprintf('%s/MSA_%s_%s_complexplane_FIGA',dir_jpg,str_shape,tmp_str);
fname_fig_jpg = sprintf('%s.jpg',fname_fig_pre);
...
dir_jpg = sprintf('%s/dir_jpg',pwd); if ~exist(dir_jpg,'dir'); mkdir(dir_jpg); end;
fname_fig_pre = sprintf('%s/MSA_%s_%s_display_FIGA',dir_jpg,str_shape,tmp_str);
fname_fig_jpg = sprintf('%s.jpg',fname_fig_pre);
figure(1+nf);nf=nf+1;clf;set(gcf,'Position',1+[0,0,2*1024,768]);
fontsize_use = 12;
p_row = 1; p_col = 3; ns=0;
%%%%;
tmp_k_p_lim_ = prctile(abs(tmp_k_p_wk__(:)),[95])*[-1,+1];
tmp_x_c_lim_ = prctile(abs(tmp_x_c_xx__(:)),[95])*[-1,+1];
subplot(p_row,p_col,1+ns);ns=ns+1; imagesc_c(n_x_c,x_c_0_,n_x_c,x_c_1_,real(tmp_x_c_xx__(:)),tmp_x_c_lim_,colormap_beach); axis image; axisnotick;
title(sprintf('real(%s_x_c_xx__(:))',tmp_str),'Interpreter','none'); set(gca,'FontSize',fontsize_use);
subplot(p_row,p_col,1+ns);ns=ns+1; imagesc_p(n_k_p_r,k_p_r_,n_w_,n_w_sum,real(tmp_k_p_wk__(:)),tmp_k_p_lim_,colormap_80s); axis image; axisnotick;
title(sprintf('real(%s_k_p_wk__(:))',tmp_str),'Interpreter','none'); set(gca,'FontSize',fontsize_use);
subplot(p_row,p_col,1+ns);ns=ns+1; imagesc_p(n_k_p_r,k_p_r_,n_w_,n_w_sum,imag(tmp_k_p_wk__(:)),tmp_k_p_lim_,colormap_80s); axis image; axisnotick;
title(sprintf('imag(%s_k_p_wk__(:))',tmp_str),'Interpreter','none'); set(gca,'FontSize',fontsize_use);
%subplot(p_row,p_col,1+ns);ns=ns+1; imagesc_c(n_x_c,x_c_0_,n_x_c,x_c_1_,imag(tmp_x_c_xx__(:)),tmp_x_c_lim_,colormap_beach); axis image; axisnotick;
%title(sprintf('imag(%s_x_c_xx__(:))',tmp_str),'Interpreter','none'); set(gca,'FontSize',fontsize_use);
%%%%;
sgtitle(tmp_str,'Interpreter','none');
disp(sprintf(' %% writing %s',fname_fig_jpg));
print('-djpeg',fname_fig_jpg);
close(gcf);
end;%for ntype=0:n_type-1;
...
%%%%%%%%;
dir_jpg = sprintf('%s/dir_jpg',pwd); if ~exist(dir_jpg,'dir'); mkdir(dir_jpg); end;
fname_fig_pre = sprintf('%s/MSA_%s_ABC_crosssection_FIGA',dir_jpg,str_shape);
fname_fig_jpg = sprintf('%s.jpg',fname_fig_pre);
...
fname_fig_pre = sprintf('%s/MSA_%s_%s_isosurface_FIGA',dir_jpg,str_shape,tmp_str);
fname_fig_jpg = sprintf('%s.jpg',fname_fig_pre);
figure(1+nf);nf=nf+1;clf;figbig;
fontsize_use = 12;
prctile_ = [98.5];%prctile_ = [97.5,98.5,99.5];
subplot(1,1,1); isosurface_f_x_u_1(struct('percent_threshold_',prctile_),tmp_x_c_xxx___);
title(sprintf('%s_x_c_xxx___',tmp_str),'Interpreter','none');
%xlabel('x'); ylabel('y'); zlabel('z'); axis vis3d;
xlabel(''); ylabel(''); zlabel(''); axis vis3d;
set(gca,'FontSize',fontsize_use);
disp(sprintf(' %% writing %s',fname_fig_jpg));
print('-djpeg',fname_fig_jpg);
close(gcf);
