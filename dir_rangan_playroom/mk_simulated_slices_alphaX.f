c***********************************************************************
      subroutine mk_simulated_slices_alphaX(density,ngrid,boxsize,eps
     $     ,ngridr,ityper,ngridc,rkmax,n_alpha,alphaX,nslices
     $     ,nslice_size ,cslices)
c***********************************************************************
c$$$      From density in physical space, generate slices of Fourier 
c$$$      transform on 2D grids at requested parameters :
c$$$      alphaX -> (cos(beta==theta),alpha==phi,gamma_z,delta_x,delta_y).
c$$$      
...
c     n_alpha       leading dimension of alphaX
c     alphaX        n_alpha x nimage array of parameters for each simulated image
...
c-----------------------------------------------------------------------
     ...
     $     '[entering mk_simulated_slices_alphaX]'
c
...
c
      if (verbose.gt.1) write(6,*)
     $     '[finished mk_simulated_slices_alphaX]'
      return
      end
