function [ FA_x_c , X_1_ , X_2_ , X_3_ ]  = spharm_to_fig_2(n_k,k_,n_l_,a_,res,alpha_,delta_);
...
FA_x_c = real(recenter3(fftn(recenter3(FA_k_c_p))));
sample_rate = res / k_max;
kx_all_resampled_ = pi*kx_all_/res ;
ky_all_resampled_ = pi*ky_all_/res ;
kz_all_resampled_ = pi*kz_all_/res ;
mx = sample_rate*res;
%[FA_x_c_nufft,ier] = nufft3d1(numel(MOL_A_K_),pi*K_1_(:)/res,pi*K_2_(:)/res,pi*K_3_(:)/res,MOL_A_K_(:),-1,1e-12,mx,mx,mx);
%FA_x_c_nufft = real(permute(reshape(FA_x_c_nufft,mx,mx,mx),[2,1,3]))*numel(MOL_A_K_);
[FA_x_c_nufft,ier] = nufft3d1(n_all,kx_all_resampled_,ky_all_resampled_,kz_all_resampled_,FA_k_p,-1,1e-12,mx,mx,mx);
FA_x_c_nufft = real(permute(reshape(FA_x_c_nufft,mx,mx,mx),[2,1,3]))*n_all;
z_ = round(linspace(1/4*res,3/4*res,6));
for nz=1:length(z_);
z = z_(nz);
%subplot(2,length(z_),nz+0*length(z_)); imagesc(real(squeeze(FA_x_c(:,:,z))),[-0.2,1.0]);
%subplot(2,length(z_),nz+1*length(z_)); imagesc(real(squeeze(FA_x_c_nufft(:,:,z))),[-0.2,1.0]);
 subplot(2,length(z_),nz+0*length(z_)); imagesc(real(squeeze(FA_x_c(:,:,z)))); colorbar;
 subplot(2,length(z_),nz+1*length(z_)); imagesc(real(squeeze(FA_x_c_nufft(:,:,z)))); colorbar;
...
%FA_x_c_nufft = real(permute(reshape(FA_x_c_nufft,mx,mx,mx),[2,1,3]))*numel(MOL_A_K_);
[FA_x_c_nufft,ier] = nufft3d1(n_all,kx_all_resampled_,ky_all_resampled_,kz_all_resampled_,FA_k_p,-1,1e-12,mx,mx,mx);
FA_x_c_nufft = real(permute(reshape(FA_x_c_nufft,mx,mx,mx),[2,1,3]))*n_all/(2*pi);
ij_tmp = (1:res) - floor(res/2) + floor(sample_rate*res/2);
...
subplot(1,3,3);
cra = colormap('spring'); cra = cra(end:-1:1,:);  ncra = size(cra,1);
F_tmp = FA_x_c_nufft(ij_tmp,ij_tmp,ij_tmp);
v_avg = mean(F_tmp(:)); v_std = std(F_tmp(:)); v_min = min(F_tmp(:)); v_max = max(F_tmp(:));
vlim = v_avg + 2.5*v_std*[-1,1];
v_ = linspace(vlim(1),vlim(2),7);
for nv=length(v_):-1:1;
v = v_(nv);
hpatch = patch(isosurface(X_1_,X_2_,X_3_,FA_x_c_nufft,v)); isonormals(X_1_,X_2_,X_3_,FA_x_c_nufft,hpatch);
nc = max(1,min(ncra,floor(ncra*nv/length(v_))));
hpatch.FaceColor = cra(nc,:);
hpatch.EdgeColor = 'none';
hpatch.FaceAlpha = (nv/length(v_)).^4 * 1.0;
...
title(sprintf('FA_x nufft, r_max %d',k_max));
% end plot;
end;%if plot_flag;
