...
tmp_t = tic;
a_x_u_reco__(:,nk_p_r) = nufft3d3(tmp_n_k_all,2*pi*k_c_0_all_(tmp_ij_)*eta,2*pi*k_c_1_all_(tmp_ij_)*eta,2*pi*k_c_2_all_(tmp_ij_)*eta,a_k_p_quad_(tmp_ij_).*(2*pi)^3.*weight_k_all_(tmp_ij_),+1,1e-12,n_X_u,X_u_0_(:)/eta,X_u_1_(:)/eta,X_u_2_(:)/eta)/sqrt(2*pi)/sqrt(2*pi)/sqrt(2*pi);
...
figbig;
fname_pre = sprintf('/data/rangan/dir_cryoem/dir_rangan_playroom/dir_principled_marching_jpg/test_principled_marching_trpv1_4_FIGB');
...
for nk_p_r=0:n_k_p_r-1;
tmp_ij_ = n_w_csum_(1+nk_p_r) + (1:n_w_(1+nk_p_r));
weight_2d_k_all_(tmp_ij_) = weight_2d_k_p_r_(1+nk_p_r) / max(1,n_w_(1+nk_p_r)) / (2*pi)^2;
end;%for nk_p_r=0:n_k_p_r-1;
...
for nk_p_r=1:n_k_p_r;
tmp_ij_ = n_w_csum_(nk_p_r) + (1:n_w_(nk_p_r));
CTF_avg_k_p_r_(nk_p_r) = mean(CTF_avg_k_p_(tmp_ij_));
end;%for nk_p_r=1:n_k_p_r;
...
[tmp_k_p_polar_a__,tmp_k_p_azimu_b__] = cg_rhs_1(n_viewing_all,n_w_(1+nk_p_r),viewing_polar_a_all_,viewing_azimu_b_all_,zeros(n_viewing_all,1));
tmp_l_max = l_max_(1+nk_p_r);
tmp_n_polar_a = max(15,1+2*tmp_l_max); tmp_n_azimu_b = 1+2*tmp_n_polar_a;
legendre_evaluate_ljm___ = legendre_evaluate_ljm___0(tmp_l_max,cos(linspace(0,pi,tmp_n_polar_a)));
legendre_evaluate_mlj___ = permute(legendre_evaluate_ljm___,[3,1,2]);
tmp_n_l_val = 1+1*tmp_l_max;
tmp_n_m_val = 1+2*tmp_l_max;
expil = exp(+i*2*pi*tmp_l_max/tmp_n_azimu_b); 
expil__ = sparse(1:tmp_n_azimu_b,1:tmp_n_azimu_b,expil.^transpose([0:tmp_n_azimu_b-1]),tmp_n_azimu_b,tmp_n_azimu_b);
expi = exp(-i*2*pi/tmp_n_azimu_b); 
expi__ = expi.^(transpose([0:tmp_n_m_val-1])*[0:tmp_n_azimu_b-1]);
tmp_n_order = 5;
tensor_to_scatter__ = cg_interpolate_n_1(tmp_n_order,tmp_n_polar_a,tmp_n_azimu_b,numel(tmp_S_k_p__),tmp_k_p_polar_a__(:),tmp_k_p_azimu_b__(:));
...
tmp_a_k_Y_0lsq_ = pcg(tmp_AtAn__,tmp_At__(tmp_S_k_p__(:)));
tmp_a_k_Y_quad_ = a_k_Y_quad_(1+n_lm_csum_(1+nk_p_r)+[0:n_lm_(1+nk_p_r)-1]);
...
%%%%%%%%;
% Now test out alternating minimization for one of the shells. ;
%%%%%%%%;
nk_p_r = 12;
tmp_S_k_p__ = S_k_p__(1+n_w_csum_(1+nk_p_r)+[0:n_w_(1+nk_p_r)-1],:);
qwer
...
X1_ = zeros(n_w_max,1);
tmp_T_q1_ = innerproduct_q_k_stretch_quad_0(n_k_p_r,k_p_r_,weight_2d_k_p_r_/(2*pi),n_w_,n_w_sum,tmp_S_k_q_,tmp_M_k_q_);
X1_ = ifft(tmp_T_q1_)*n_w_max; %<-- multiplication by n_w_max not needed in fortran fftw_plan_back. ;
%%%%%%%%;
...
polar_a = 0;
X_ori_(nk_p_r_1,nk_p_r_2) = register_spharm_to_spharm_2(verbose,1,1,1,tmp_l_max,a_k_Y_quad_(tmp_ij_1_)*CTF_avg_k_p_r_(nk_p_r_1),a_k_Y_quad_(tmp_ij_2_)*CTF_avg_k_p_r_(nk_p_r_2));
X_ori_(nk_p_r_2,nk_p_r_1) = conj(X_ori_(nk_p_r_1,nk_p_r_2));
tmp_sum = 0;
for npolar_a=1:n_polar_a;
polar_a = polar_a_(npolar_a);
[tmp_X_tau__] = register_spharm_to_spharm_single_beta_2(verbose,1,1,1,tmp_l_max,a_k_Y_quad_(tmp_ij_1_)*CTF_avg_k_p_r_(nk_p_r_1),a_k_Y_quad_(tmp_ij_2_)*CTF_avg_k_p_r_(nk_p_r_2),polar_a,n_m_max,azimu_b_,n_m_max,gamma_z_,[],[],[],[]);
...
for nUX_rank=1:n_UX_rank;
for nk_p_r=1:n_k_p_r;
tmp_l_max = l_max_(nk_p_r);
tmp_n_lm = (tmp_l_max+1).^2;
tmp_ij_ = n_lm_csum_(nk_p_r) + (1:tmp_n_lm);
a_UX_Y_quad__(1:tmp_n_lm,nUX_rank) = a_UX_Y_quad__(1:tmp_n_lm,nUX_rank) + UX_(nk_p_r,nUX_rank)*a_k_Y_quad_(tmp_ij_)*CTF_avg_k_p_r_(nk_p_r);
end;%for nk_p_r=1:n_k_p_r;
end;%for nUX_rank=1:n_UX_rank;
%%%%%%%%;
...
if flag_check;
for nUX_rank=1:n_UX_rank;
[tmp_X,tmp_X_ori,tmp_X_tau,tmp_weight_so3] = principled_marching_cost_0(verbose,n_m_max,l_max_max,a_UX_Y_quad__(:,nUX_rank),a_UX_Y_quad__(:,nUX_rank));
tmp_Z = transpose(UX_(:,nUX_rank))*X_*(UX_(:,nUX_rank));
...
figbig;
fname_pre = sprintf('/data/rangan/dir_cryoem/dir_rangan_playroom/dir_principled_marching_jpg/test_principled_marching_trpv1_4_FIGC');
disp(sprintf(' %% writing %s',fname_pre));
print('-djpeg',sprintf('%s.jpg',fname_pre));
print('-depsc',sprintf('%s.eps',fname_pre));
end;%if flag_plot;
...
subplot(3,n_plot,1 + (nplot-1) + 0*n_plot); imagesc(real(b_k_p_quad_),quad_lim_); axisnotick; title(sprintf('real nk_p_r: %d, real(quad)',nk_p_r),'Interpreter','none');
subplot(3,n_plot,1 + (nplot-1) + 1*n_plot); imagesc(imag(b_k_p_quad_),quad_lim_); axisnotick; title(sprintf('imag nk_p_r: %d, imag(quad)',nk_p_r),'Interpreter','none');
subplot(3,n_plot,1 + (nplot-1) + 2*n_plot); imagesc( abs(b_k_p_quad_),quad_lim_); axisnotick; title(sprintf(' abs nk_p_r: %d,  abs(quad)',nk_p_r),'Interpreter','none');
end;%for nplot=1:n_plot;
colormap(colormap_beach());
figbig;
fname_pre = sprintf('/data/rangan/dir_cryoem/dir_rangan_playroom/dir_principled_marching_jpg/test_principled_marching_trpv1_4_FIGD');
disp(sprintf(' %% writing %s',fname_pre));
print('-djpeg',sprintf('%s.jpg',fname_pre));
print('-depsc',sprintf('%s.eps',fname_pre));
end;%if flag_plot;
...
colormap(colormap_beach());
fname_pre = sprintf('/data/rangan/dir_cryoem/dir_rangan_playroom/dir_principled_marching_jpg/test_principled_marching_trpv1_4_FIGE');
disp(sprintf(' %% writing %s',fname_pre));
print('-djpeg',sprintf('%s.jpg',fname_pre));
print('-depsc',sprintf('%s.eps',fname_pre));
end;%if flag_plot;
...
figbig;
fname_pre = sprintf('/data/rangan/dir_cryoem/dir_rangan_playroom/dir_principled_marching_jpg/test_principled_marching_trpv1_4_FIGF');
disp(sprintf(' %% writing %s',fname_pre));
print('-djpeg',sprintf('%s.jpg',fname_pre));
print('-depsc',sprintf('%s.eps',fname_pre));
...
for nk_p_r=1:n_k_p_r;
tmp_UX_M_k_p_ = tmp_UX_M_k_p_ + UX_(nk_p_r,nUX_rank)*N_k_p__(:,nk_p_r);
end;%for nk_p_r=1:n_k_p_r;
UX_M_k_p___(:,1+nimage_sub,nUX_rank) = tmp_UX_M_k_p_;
UX_M_k_q___(:,1+nimage_sub,nUX_rank) = interp_p_to_q(1,n_w_max,n_w_max,tmp_UX_M_k_p_);
end;%for nUX_rank=1:n_UX_rank;
end;%for nimage_sub=0:n_image_sub-1;
flag_plot=0;
if flag_plot;
colormap(colormap_beach());
nUX_rank=1; clim_ = 1.5*std(real(UX_M_k_p___(:,:,nUX_rank)),1,'all')*[-1,+1];
nUX_rank= 1; subplot(2,3,1); imagesc(real(squeeze(UX_M_k_p___(:,:,nUX_rank))),clim_); xlabel('nimage'); ylabel('gamma'); title(sprintf('real(rank==%d) [%0.2f,%0.2f]',nUX_rank,clim_));
nUX_rank= 2; subplot(2,3,2); imagesc(real(squeeze(UX_M_k_p___(:,:,nUX_rank))),clim_); xlabel('nimage'); ylabel('gamma'); title(sprintf('real(rank==%d) [%0.2f,%0.2f]',nUX_rank,clim_));
nUX_rank= 4; subplot(2,3,3); imagesc(real(squeeze(UX_M_k_p___(:,:,nUX_rank))),clim_); xlabel('nimage'); ylabel('gamma'); title(sprintf('real(rank==%d) [%0.2f,%0.2f]',nUX_rank,clim_));
nUX_rank= 8; subplot(2,3,4); imagesc(real(squeeze(UX_M_k_p___(:,:,nUX_rank))),clim_); xlabel('nimage'); ylabel('gamma'); title(sprintf('real(rank==%d) [%0.2f,%0.2f]',nUX_rank,clim_));
nUX_rank=16; subplot(2,3,5); imagesc(real(squeeze(UX_M_k_p___(:,:,nUX_rank))),clim_); xlabel('nimage'); ylabel('gamma'); title(sprintf('real(rank==%d) [%0.2f,%0.2f]',nUX_rank,clim_));
nUX_rank=32; subplot(2,3,6); imagesc(real(squeeze(UX_M_k_p___(:,:,nUX_rank))),clim_); xlabel('nimage'); ylabel('gamma'); title(sprintf('real(rank==%d) [%0.2f,%0.2f]',nUX_rank,clim_));
figbig;
fname_pre = sprintf('/data/rangan/dir_cryoem/dir_rangan_playroom/dir_principled_marching_jpg/test_principled_marching_trpv1_4_FIGG');
disp(sprintf(' %% writing %s',fname_pre));
print('-djpeg',sprintf('%s.jpg',fname_pre));
print('-depsc',sprintf('%s.eps',fname_pre));
end;%if flag_plot;
...
if flag_plot;
colormap(colormap_beach());
nUX_rank= 1; subplot(2,3,1); imagesc(log10(abs(squeeze(UX_M_k_q___(:,:,nUX_rank)))),[-5,0]); xlabel('nimage'); ylabel('q'); title(sprintf('log10(abs(rank %d)) [-5,0]',nUX_rank));
nUX_rank= 2; subplot(2,3,2); imagesc(log10(abs(squeeze(UX_M_k_q___(:,:,nUX_rank)))),[-5,0]); xlabel('nimage'); ylabel('q'); title(sprintf('log10(abs(rank %d)) [-5,0]',nUX_rank));
nUX_rank= 4; subplot(2,3,3); imagesc(log10(abs(squeeze(UX_M_k_q___(:,:,nUX_rank)))),[-5,0]); xlabel('nimage'); ylabel('q'); title(sprintf('log10(abs(rank %d) [-5,0])',nUX_rank));
nUX_rank= 8; subplot(2,3,4); imagesc(log10(abs(squeeze(UX_M_k_q___(:,:,nUX_rank)))),[-5,0]); xlabel('nimage'); ylabel('q'); title(sprintf('log10(abs(rank %d)) [-5,0]',nUX_rank));
nUX_rank=16; subplot(2,3,5); imagesc(log10(abs(squeeze(UX_M_k_q___(:,:,nUX_rank)))),[-5,0]); xlabel('nimage'); ylabel('q'); title(sprintf('log10(abs(rank %d)) [-5,0]',nUX_rank));
nUX_rank=32; subplot(2,3,6); imagesc(log10(abs(squeeze(UX_M_k_q___(:,:,nUX_rank)))),[-5,0]); xlabel('nimage'); ylabel('q'); title(sprintf('log10(abs(rank %d)) [-5,0]',nUX_rank));
figbig;
fname_pre = sprintf('/data/rangan/dir_cryoem/dir_rangan_playroom/dir_principled_marching_jpg/test_principled_marching_trpv1_4_FIGGq');
disp(sprintf(' %% writing %s',fname_pre));
print('-djpeg',sprintf('%s.jpg',fname_pre));
print('-depsc',sprintf('%s.eps',fname_pre));
end;%if flag_plot;
...
n_plot=6;
for nplot=1:n_plot;
tmp_ = log10(abs(svds(reshape(permute(UX_M_k_q___(:,:,1:nplot),[1,3,2]),[n_w_max*nplot,n_image_sub]),n_w_max)));
subplot(2,3,nplot);
plot(tmp_,'o'); xlim([1,n_w_max]); ylim([-7+tmp_(1),tmp_(1)+1]);
xlabel('pc'); ylabel('log10(sigma)');
title(sprintf('spectrum of UX_M_k_q___(:,:,nUX_rank==1:%d)',nplot),'Interpreter','none');
end;%for nplot=1:n_plot;
figbig;
fname_pre = sprintf('/data/rangan/dir_cryoem/dir_rangan_playroom/dir_principled_marching_jpg/test_principled_marching_trpv1_4_FIGH');
disp(sprintf(' %% writing %s',fname_pre));
print('-djpeg',sprintf('%s.jpg',fname_pre));
print('-depsc',sprintf('%s.eps',fname_pre));
end;%if flag_plot;
