...
c
      ...
      if (verbose.gt.1) then
         write(6,'(A)') '[entering test_ver5dI]'
      end if
...
c$$$      enddo
      ...
      d_(0) = 3
      d_(1) = n_source_use
      write(fname,'(A)') './dir_mda5dI/source_xyz_.mda'
...
c     create physical space grid and sample function on it.
     ...
     1     fgauss,ff_ori,fnorm)
      ...
      d_(1) = ngrid
      d_(2) = ngrid
      write(fname,'(A)') './dir_mda5dI/x_pre_.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(fname)
      call MDA_write_r8(3,d_,x,fname)
      write(fname,'(A)') './dir_mda5dI/y_pre_.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(fname)
      call MDA_write_r8(3,d_,y,fname)
      write(fname,'(A)') './dir_mda5dI/z_pre_.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(fname)
      call MDA_write_r8(3,d_,z,fname)
      write(fname,'(A)') './dir_mda5dI/S_ori_.mda'
...
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      write(fname,'(A)') './dir_jpg5dI/Fig_ctf_c'
...
c$$$      n_S_sample_max = maximum number of templates to use at each step.
      n_S_sample_max = n_M_sample_max*4
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         d_(0) = n_alpha
         d_(1) = n_M_sample
         write(fname,'(A,I0,A)') './dir_mda5dI/alpha_tru_',ncur,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : ',trim(fname)
         call MDA_write_r8(2,d_,alpha_tru_,fname)
         d_(0) = n_alpha
         d_(1) = n_M_sample
         write(fname,'(A,I0,A)') './dir_mda5dI/alpha_est_',ncur,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : ',trim(fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         write(fname,'(A,I0)') './dir_jpg5dI/Fig_MNC_',ncur
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $              ,S_sample_(nS_sample*nimage_size))
               S_alpha_polar_a_sample_(nS_sample) = alpha2d_ori_(1
     $              +nalpha_polar_a,1+ns)
               S_alpha_azimu_b_sample_(nS_sample) = alpha2d_ori_(1
     $              +nalpha_azimu_b,1+ns)
            enddo               !do nS_sample=0,n_S_sample-1
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         d_(0) = n_S_sample
         d_(1) = n_M_sample
         write(fname,'(A,I0,A)') './dir_mda5dI/C_Z_max_',ncur,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : ',trim(fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         write(fname,'(A,I0)') './dir_jpg5dI/Fig_MTS_c_',ncur
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
 ...
 10   continue
      if (verbose.gt.1) then
         write(6,'(A)') '[finished test_ver5dI]'
      end if
      stop
      end
