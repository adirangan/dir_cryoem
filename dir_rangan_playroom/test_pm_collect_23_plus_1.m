...
%%%%%%%%%%%%%%%%%%%%%%%;
fname_fig = sprintf('%s/test_pm_collect_plus_a0t_FIGF',dir_jpg);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
dlim_ = [0,0.15];
rlim_ = [2,20];
Xlim_ = [0.35,+1.0];
figure(1+nf);nf=nf+1;clf;set(gcf,'Position',1+[0,0,2*1024,768*3]); fig80s;
c_80s__ = colormap_80s; n_c_80s = size(c_80s__,1);
markersize_use = 8;
%str_symbol_ = {'o','^','s','p','h'};
str_symbol_ = {'.-','.-'};
p_row = 3; p_col = ceil(1*n_u_experiment/p_row); np=0;
%%%%%%%%;
for nu_experiment=0:n_u_experiment-1;
dir_nopath_data_star = u_dir_nopath_data_star_{1+nu_experiment};
...
%%%%%%%%%%%%%%%%%%%%%%%;
fname_fig = sprintf('%s/test_pm_collect_plus_a1t_FIGF',dir_jpg);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
dlim_ = [0,0.15];
rlim_ = [2,20];
...
%{
subplot(p_row,p_col,1+np);np=np+1;
n_a = size(collect_aX_.X_best_a1t_ia__,2);
%[~,tmp_index_] = sort(collect_aX_.delta_sigma_use_a1t_a_,'ascend'); tmp_index_ = tmp_index_ - 1;
[tmp_rank_pm_,tmp_index_] = sort(collect_aX_.rank_pm_a1t_a_,'ascend'); tmp_index_ = tmp_index_ - 1;
imagesc(collect_aX_.X_best_a1t_ia__(1+0*n_iteration+[0:n_iteration-1],1+tmp_index_),Xlim_);
axis image;
ylabel('iteration');
set(gca,'XTick',1:n_a,'XTickLabel',tmp_rank_pm_); xtickangle(90);
xlabel('rank');
