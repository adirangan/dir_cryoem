...
if (nargin<1+na); ori_k_c_0_all_=[]; end; na=na+1;
if (nargin<1+na); ori_k_c_1_all_=[]; end; na=na+1;
if (nargin<1+na); ori_k_c_2_all_=[]; end; na=na+1;
...
if isempty(ori_k_c_0_all_); ori_k_c_0_all_ = k_p_r_all_.*cos(k_p_azimu_b_all_).*sin(k_p_polar_a_all_); end;
if isempty(ori_k_c_1_all_); ori_k_c_1_all_ = k_p_r_all_.*sin(k_p_azimu_b_all_).*sin(k_p_polar_a_all_); end;
if isempty(ori_k_c_2_all_); ori_k_c_2_all_ = k_p_r_all_.*cos(k_p_polar_a_all_); end;
...
%%%%;
ori_a_k_p_quad_ = zeros(ori_n_k_all,1);
for ori_nk_p_r=0:ori_n_k_p_r-1;
ori_tmp_index_ = ori_n_k_all_csum_(1+ori_nk_p_r+0):ori_n_k_all_csum_(1+ori_nk_p_r+1)-1;
for nk_p_r=0:n_k_p_r-1;
tmp_index_ = n_k_all_csum_(1+nk_p_r+0):n_k_all_csum_(1+nk_p_r+1)-1;
ori_a_k_p_quad_(1+ori_tmp_index_) = ori_a_k_p_quad_(1+ori_tmp_index_) + ori_from_pm_UX_kn__(1+ori_nk_p_r,1+nk_p_r)/max(1e-12,ori_X_weight_r_(1+ori_nk_p_r))*a_k_p_quad_(1+tmp_index_);
end;%for nk_p_r=0:n_k_p_r-1;
end;%for ori_nk_p_r=0:ori_n_k_p_r-1;
%%%%;
eta = pi/k_p_r_max; tmp_t = tic;
a_x_u_quad_ = xxnufft3d3(ori_n_k_all,2*pi*ori_k_c_0_all_*eta,2*pi*ori_k_c_1_all_*eta,2*pi*ori_k_c_2_all_*eta,ori_a_k_p_quad_.*(2*pi)^3.*ori_weight_3d_k_all_,+1,1e-12,n_xxx_u,x_u_0___(:)/eta,x_u_1___(:)/eta,x_u_2___(:)/eta)/sqrt(2*pi)/sqrt(2*pi)/sqrt(2*pi);
...
%%%%;
ori_v_k_p_use_ = zeros(ori_n_k_all,1);
for ori_nk_p_r=0:ori_n_k_p_r-1;
ori_tmp_index_ = ori_n_k_all_csum_(1+ori_nk_p_r+0):ori_n_k_all_csum_(1+ori_nk_p_r+1)-1;
for nk_p_r=0:n_k_p_r-1;
tmp_index_ = n_k_all_csum_(1+nk_p_r+0):n_k_all_csum_(1+nk_p_r+1)-1;
ori_v_k_p_use_(1+ori_tmp_index_) = ori_v_k_p_use_(1+ori_tmp_index_) + ori_from_pm_UX_kn__(1+ori_nk_p_r,1+nk_p_r)/max(1e-12,ori_X_weight_r_(1+ori_nk_p_r))*v_k_p_use_(1+tmp_index_);
end;%for nk_p_r=0:n_k_p_r-1;
end;%for ori_nk_p_r=0:ori_n_k_p_r-1;
%%%%;
eta = pi/k_p_r_max; tmp_t = tic;
v_x_u_use_ = xxnufft3d3(ori_n_k_all,2*pi*ori_k_c_0_all_*eta,2*pi*ori_k_c_1_all_*eta,2*pi*ori_k_c_2_all_*eta,v_k_p_use_.*(2*pi)^3.*ori_weight_3d_k_all_,+1,1e-12,n_xxx_u,x_u_0___(:)/eta,x_u_1___(:)/eta,x_u_2___(:)/eta)/sqrt(2*pi)/sqrt(2*pi)/sqrt(2*pi);
