...
verbose=1;
if (verbose); disp(sprintf(' %% [entering rlnImageName_from_star_1]')); end;
...
%%%%%%%%;
if (n_block>2);
disp(sprintf(' %% Warning, n_block %d in rlnImageName_from_star_1',n_block));
end;%if (n_block>2);
%%%%%%%%;
if (n_block==1);
...
n_image = min(n_image,numel(star_blockData{1+index_particles}.rlnImageName));
rlnImageName_ = star_blockData{1+index_particles}.rlnImageName(1:n_image);
Voltage_M_ = zeros(n_image,1); if isfield(star_blockData{1+index_particles},'rlnVoltage'); Voltage_M_(1:n_image) = star_blockData{1+index_particles}.rlnVoltage(1:n_image); end; if isfield(star_blockData{1+index_optics},'rlnVoltage'); Voltage_M_(1:n_image) = star_blockData{1+index_optics}.rlnVoltage(1); end;
DefocusU_M_ = zeros(n_image,1); if isfield(star_blockData{1+index_particles},'rlnDefocusU'); DefocusU_M_(1:n_image) = star_blockData{1+index_particles}.rlnDefocusU(1:n_image); end; if isfield(star_blockData{1+index_optics},'rlnDefocusU'); DefocusU_M_(1:n_image) = star_blockData{1+index_optics}.rlnDefocusU(1); end;
DefocusV_M_ = zeros(n_image,1); if isfield(star_blockData{1+index_particles},'rlnDefocusV'); DefocusV_M_(1:n_image) = star_blockData{1+index_particles}.rlnDefocusV(1:n_image); end; if isfield(star_blockData{1+index_optics},'rlnDefocusV'); DefocusV_M_(1:n_image) = star_blockData{1+index_optics}.rlnDefocusV(1); end;
DefocusAngle_M_ = zeros(n_image,1); if isfield(star_blockData{1+index_particles},'rlnDefocusAngle'); DefocusAngle_M_(1:n_image) = star_blockData{1+index_particles}.rlnDefocusAngle(1:n_image); end; if isfield(star_blockData{1+index_optics},'rlnDefocusAngle'); DefocusAngle_M_(1:n_image) = star_blockData{1+index_optics}.rlnDefocusAngle(1); end;
SphericalAberration_M_ = zeros(n_image,1); if isfield(star_blockData{1+index_particles},'rlnSphericalAberration'); SphericalAberration_M_(1:n_image) = star_blockData{1+index_particles}.rlnSphericalAberration(1:n_image); end; if isfield(star_blockData{1+index_optics},'rlnSphericalAberration'); SphericalAberration_M_(1:n_image) = star_blockData{1+index_optics}.rlnSphericalAberration(1); end;
AmplitudeContrast_M_ = zeros(n_image,1); if isfield(star_blockData{1+index_particles},'rlnAmplitudeContrast'); AmplitudeContrast_M_(1:n_image) = star_blockData{1+index_particles}.rlnAmplitudeContrast(1:n_image); end; if isfield(star_blockData{1+index_optics},'rlnAmplitudeContrast'); AmplitudeContrast_M_(1:n_image) = star_blockData{1+index_optics}.rlnAmplitudeContrast(1); end;
end;%if (n_block==2);
%%%%%%%%;
...
if (verbose); disp(sprintf(' %% [finished rlnImageName_from_star_1]')); end;
