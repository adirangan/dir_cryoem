...
if (nargin<1+na);,a_k_Y__=[]; end; na=na+1;
...
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% a_k_Y___: %0.3fs',tmp_t)); end;
...
%%%%%%%%;
tmp_t = tic();
[I_pos__,I_neg_] = pm_delta_integral_2(n_k_p_r,k_p_r_,delta_sigma,l_max_max,pm_delta_integral_tolerance);
I_pos__ = I_pos__/(2*pi);
I_neg__ = (I_neg_*transpose(I_neg_))/(2*pi)^2;
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% I_pos__ and I_neg_: %0.3fs',tmp_t)); end;
%%%%%%%%;
tmp_t = tic();
...
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% X_weight_r_: %0.3fs',tmp_t)); end;
%%%%%%%%;
tmp_t = tic();
...
%%%%%%%%;
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% X__: %0.3fs',tmp_t)); end;
