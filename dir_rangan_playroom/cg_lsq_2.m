function a_k_Y_ = cg_lsq_2(n_order,n_k_p_r,l_max_,n_w_,n_M,M_k_p__,CTF_avg_k_p_r_,euler_polar_a_,euler_azimu_b_,euler_gamma_z_);
% simply applies conjugate-gradient to the least-squares problem to solve for a_k_Y_. ;
% uses CTF_avg_k_p_r_ for all images. ;
...
CTF_avg_k_p_r = CTF_avg_k_p_r_(1+nk_p_r);
...
An__ = @(a_k_Y_) tensor_to_scatter__*reshape(cg_evaluate_n_1(l_max,convert_spharm_to_spharm__0(l_max,CTF_avg_k_p_r*a_k_Y_),n_polar_a,n_azimu_b,legendre_evaluate_ljm___),[n_polar_a*n_azimu_b,1]);
At__ = @(a_k_X_) CTF_avg_k_p_r*convert_spharm__to_spharm_0(l_max,cg_evaluate_t_1(n_polar_a,n_azimu_b,reshape(scatter_to_tensor__*a_k_X_,[n_polar_a,n_azimu_b]),l_max,legendre_evaluate_mlj___,expil__,expi__));
