...
c     
      ...
      real *8 distance_req
c$$$      parameter (distance_req=1.5d0)
...
c$$$      enddo
      ...
      d_(0) = 3
      d_(1) = n_source_use
      write(fname,'(A)') './dir_mda5d/source_xyz_.mda'
...
c     create physical space grid and sample function on it.
     ...
     1     fgauss,ff_ori,fnorm)
      ...
      d_(1) = ngrid
      d_(2) = ngrid
      write(fname,'(A)') './dir_mda5d/x_pre_.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(fname)
      call MDA_write_r8(3,d_,x,fname)
      write(fname,'(A)') './dir_mda5d/y_pre_.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(fname)
      call MDA_write_r8(3,d_,y,fname)
      write(fname,'(A)') './dir_mda5d/z_pre_.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(fname)
      call MDA_write_r8(3,d_,z,fname)
      write(fname,'(A)') './dir_mda5d/S_ori_.mda'
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      write(fname,'(A)') './dir_jpg5d/Fig_ctf_c'
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         d_(0) = n_alpha
         d_(1) = n_M_sample
         write(fname,'(A,I0,A)') './dir_mda5d/alpha_tru_',ncur,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : ',trim(fname)
         call MDA_write_r8(2,d_,alpha_tru_,fname)
         d_(0) = n_alpha
         d_(1) = n_M_sample
         write(fname,'(A,I0,A)') './dir_mda5d/alpha_est_',ncur,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : ',trim(fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         write(fname,'(A,I0)') './dir_jpg5d/Fig_MNC_c_',ncur
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        ,eps,npts,x,y,z,ff_tru,ier)
         ...
         d_(1) = ngrid
         d_(2) = ngrid
         write(fname,'(A,I0,A)') './dir_mda5d/S_tru_',ncur,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : ',trim(fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        ,eps,npts,x,y,z,ff_est,ier)
         ...
         d_(1) = ngrid
         d_(2) = ngrid
         write(fname,'(A,I0,A)') './dir_mda5d/S_est_',ncur,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : ',trim(fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         d_(0) = n_S_sample
         d_(1) = n_M_sample
         write(fname,'(A,I0,A)') './dir_mda5d/C_Z_max_',ncur,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : ',trim(fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         write(fname,'(A,I0)') './dir_jpg5d/Fig_MTS_c_',ncur
