...
%%%%%%%%;
fname_mat = sprintf('%s/%s_%s_%s.mat',dir_mat,fname_0,fname_1,fname_2);
tmp_ = load(fname_mat);
...
dir_jpg_sub = sprintf('%s/dir_%s',dir_jpg,fname_0);
tmp_ij = strfind(dir_jpg_sub,'_n1024');
if (tmp_ij>2); dir_jpg_sub = dir_jpg_sub(1:tmp_ij-1); end;
if (~exist(dir_jpg_sub,'dir')); sprintf(' %% mkdir %s',dir_jpg_sub); mkdir(dir_jpg_sub); end;

fname_fig = sprintf('%s/%s_%s_%s_euler_angle_FIGA',dir_jpg_sub,fname_0,fname_1,fname_2);
...
figure(3); clf;
markersize_use = 15;
p_row = 3; p_col = ceil((1+n_iteration)/p_row); np=0;
figbeach();
figbig;
subplot(p_row,p_col,1+np); np=np+1;
hold on;
patch(2*pi*[+0;+1;+1;+0],1*pi*[+0;+0;+1;+1],0.65*[1,1,1],'EdgeColor','none');
scatter(0*pi+euler_angle_marina_(2,1:n_M),1*pi-euler_angle_marina_(1,1:n_M),markersize_use,c2d__(1:n_M,:),'filled');
hold off;
axisnotick;
...
hold on;
patch(2*pi*[+0;+1;+1;+0],1*pi*[+0;+0;+1;+1],0.65*[1,1,1],'EdgeColor','none');
scatter(0*pi+tmp_euler_azimu_b_,1*pi-tmp_euler_polar_a_,markersize_use,c2d__(1:n_M,:),'filled');
hold off;
axisnotick;
axis([0,2*pi,0,1*pi]); axis equal; xlabel('azimu_b'); ylabel('polar_a');
title(sprintf('%s %d (flip %d)',fname_1,niteration,flip_flag));
%imagesc(log2(1+hist2d_0(0*pi+tmp_euler_azimu_b_,1*pi-tmp_euler_polar_a_,2*n_h,n_h,[0,2*pi],[0,1*pi])),lh_lim_);
%set(gca,'Ydir','normal');
%axisnotick; axis image;
drawnow();
end;%for niteration=0:n_iteration-1;
sgtitle(sprintf('%s_%s_%s',fname_0,fname_1,fname_2),'Interpreter','none');
figbig;
...
hold on;
patch([-1;+1;+1;-1],[-1;-1;+1;+1],'k');
scatter(mat_image_delta_x__(:,1+niteration),mat_image_delta_y__(:,1+niteration),markersize_use,c2d__(1:n_M,:),'filled');
hold off;
axisnotick;
axis([-1,+1,-1,+1]); axis square;
%xlabel('dx'); ylabel('dy');
