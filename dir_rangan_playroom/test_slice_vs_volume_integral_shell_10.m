...
%%%%%%%%;
% construct a denser set of templates along the equator. ;
%%%%%%%%;
n_N_from_M = 4;
n_N = n_N_from_M*n_M;
euler_azimu_b_N_ = linspace(0,2*pi,1+n_N); euler_azimu_b_N_ = transpose(euler_azimu_b_N_(1:n_N));
euler_polar_a_N_ = (pi/2)*ones(n_N,1);
[ ...
 N_k_p_wkN__ ...
] = ...
pm_template_2( ...
...
N_k_p_wkN__ = reshape(N_k_p_wkN__,[n_w_sum,n_N]);
if (flag_verbose>0); disp(sprintf(' %% fnorm(N_k_p_wkN__(:,1:n_N_from_M:end) - M_k_p_wkM__(:,1:1:end)): %0.16f',fnorm(N_k_p_wkN__(:,1:n_N_from_M:end) - M_k_p_wkM__(:,1:1:end)))); end;
%%%%%%%%;
% Now convert N to M. ;
%%%%%%%%;
n_M = n_N;
M_k_p_wkM__ = N_k_p_wkN__;
euler_polar_a_M_ = euler_polar_a_N_;
euler_azimu_b_M_ = euler_azimu_b_N_;
euler_gamma_z_M_ = zeros(n_M,1);
