function [X_best_,a_k_Y_0lsq__,euler_polar_a__,euler_azimu_b__,euler_gamma_z__] = am_2(rseed,n_iteration,n_iteration_register,viewing_k_eq_d,n_order,n_k_p_r,k_p_r_,k_p_r_max,weight_k_p_r_,weight_2d_k_p_r_,n_w_,n_M,M_k_p__,M_k_q__,CTF_index_,CTF_k_p__,l_max_,a_k_Y_true_,euler_polar_a_,euler_azimu_b_,euler_gamma_z_,flag_MS_vs_SM,f_rand);
%%%%%%%%;
...
% a_k_Y_true_: complex array of size n_lm_sum = sum((1+l_max_).^2). The 'ground-truth' to which a_k_Y_0lsq_ is compared. ;
...
tmp_t = tic();
for nM=0:n_M-1;
tmp_X_S_ = max(squeeze(real(X_wSM___(:,:,1+nM))),[],1);
tmp_ij_ = find(tmp_X_S_>=prctile(tmp_X_S_,100-100*f_rand)) - 1;
tmp_ij_index = max(0,min(numel(tmp_ij_)-1,floor(numel(tmp_ij_)*rand())));
nS_best = tmp_ij_(1+tmp_ij_index);
[~,nw_best] = max(squeeze(real(X_wSM___(:,1+nS_best,1+nM)))); nw_best = nw_best - 1;
euler_polar_a_(1+nM) = viewing_polar_a_all_(1+nS_best);
euler_azimu_b_(1+nM) = viewing_azimu_b_all_(1+nS_best);
euler_gamma_z_(1+nM) = 2*pi*nw_best/n_w_max;
end;%for nM=0:n_M-1;
