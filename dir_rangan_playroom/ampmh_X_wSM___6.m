...
if (verbose); disp(sprintf(' %% [testing ampmh_X_wSM___6.m]')); end;
%%%%%%%%;
tmp_t = tic();
[ ...
 n_k_all ...
...
%%%%%%%%;
% Finally, check ampmh_X_wSM___6 to see how the rotations and translations are implemented. ;
%%%%%%%%;
...
verbose = 0;
if (verbose>0); disp(sprintf(' %% [entering ampmh_X_wSM___6]')); end;
...
if (~isfield(parameter,'n_w_max_use')); parameter.n_w_max_use = 0; end;
if (~isfield(parameter,'svd_eps_use')); parameter.svd_eps_use = 0; end;
if (~isfield(parameter,'n_svd_l_use')); parameter.n_svd_l_use = 0; end;
if (~isfield(parameter,'n_delta_v_use')); parameter.n_delta_v_use = 0; end;
if (~isfield(parameter,'pm_n_UX_rank_use')); parameter.pm_n_UX_rank_use = 0; end;
if (~isfield(parameter,'flag_optimize_over_gamma_z')); parameter.flag_optimize_over_gamma_z = 0; end;
...
X_wSM___ = zeros(n_w_max_use,n_S,n_M);
delta_x_wSM___ = zeros(n_w_max_use,n_S,n_M);
delta_y_wSM___ = zeros(n_w_max_use,n_S,n_M);
if (nargout>3); gamma_z_wSM___ = zeros(n_w_max_use,n_S,n_M); end;
if (nargout>4); I_value_wSM___ = zeros(n_w_max_use,n_S,n_M); end;
...
if (nargout>3); gamma_z_SM__ = zeros(n_S,n_M); end;
if (nargout>4); I_value_SM__ = zeros(n_S,n_M); end;
end;%if (flag_optimize_over_gamma_z == 1);

flag_compute_I = 0;
if (nargout>4); flag_compute_I = 1; end;
...
UX_M_l2_dM_use__ = zeros(FTK_n_delta_v,n_M);
tmp_UX_M_k_q_dwnM____ = reshape(FTK_svd_U_d_expiw_s__(:,1+FTK_index_svd_s_sort_from_all_)*reshape(svd_VUXM_lwnM____(1+FTK_index_svd_s_sort_from_all_,1+index_nw_,1:pm_n_UX_rank_use,:),[FTK_n_svd_l,n_w_max_use*pm_n_UX_rank_use*n_M]),[FTK_n_delta_v,n_w_max_use,pm_n_UX_rank_use,n_M]);
UX_M_l2_dM_use__(:,:) = sum(reshape(permute(abs(tmp_UX_M_k_q_dwnM____).^2,[2,3,1,4]),[n_w_max_use*pm_n_UX_rank_use,FTK_n_delta_v,n_M]),1) * n_w_max_use;
...
innerproduct_p_quad( ...
 pm_n_UX_rank_use ...
,ones(pm_n_UX_rank_use,1) ...
,ones(pm_n_UX_rank_use,1)/(2*pi) ...
,n_w_max_use*ones(pm_n_UX_rank_use,1) ...
,n_w_max_use*pm_n_UX_rank_use ...
,reshape(CTF_UX_S_k_q_wnS___(1+index_nw_,1:pm_n_UX_rank_use,1+nS),[n_w_max_use*pm_n_UX_rank_use,1]) ...
,reshape(CTF_UX_S_k_q_wnS___(1+index_nw_,1:pm_n_UX_rank_use,1+nS),[n_w_max_use*pm_n_UX_rank_use,1]) ...
);
end;%for nS=0:n_S-1;
...
CTF_UX_S_k_q_nSw___ = permute(CTF_UX_S_k_q_wnS___(1+index_nw_,1:pm_n_UX_rank_use,:),[2,3,1]);

if (verbose & n_w_max_use<n_w_max);
disp(sprintf('CTF_UX_S_k_q_wnS___: total %0.16f retained %0.16f discarded %0.16f',fnorm(CTF_UX_S_k_q_wnS___),fnorm(CTF_UX_S_k_q_wnS___(1+index_nw_,:,:)),fnorm(CTF_UX_S_k_q_wnS___(setdiff(1:n_w_max,1+index_nw_),:,:))));
disp(sprintf('svd_VUXM_lwnM____: total %0.16f retained %0.16f discarded %0.16f',fnorm(svd_VUXM_lwnM____),fnorm(svd_VUXM_lwnM____(:,1+index_nw_,:,:)),fnorm(svd_VUXM_lwnM____(:,setdiff(1:n_w_max,1+index_nw_),:,:))));
end;%if (verbose & n_w_max_use<n_w_max);
...
if (nargout>3); tmp_str = 'gamma_z_wSM___'; disp(sprintf(' %% memory: %s --> %0.6f GB',tmp_str,whos(tmp_str).bytes/1e9)); end;
if (nargout>4); tmp_str = 'I_value_wSM___'; disp(sprintf(' %% memory: %s --> %0.6f GB',tmp_str,whos(tmp_str).bytes/1e9)); end;
end;%if (flag_optimize_over_gamma_z == 0);
if (flag_optimize_over_gamma_z == 1);
...
if (nargout>3); tmp_str = 'gamma_z_SM__'; disp(sprintf(' %% memory: %s --> %0.6f GB',tmp_str,whos(tmp_str).bytes/1e9)); end;
if (nargout>4); tmp_str = 'I_value_SM__'; disp(sprintf(' %% memory: %s --> %0.6f GB',tmp_str,whos(tmp_str).bytes/1e9)); end;
end;%if (flag_optimize_over_gamma_z == 1);
...
if (n_M_sub>0);
tmp_t = tic();
svd_VUXM_nMwl____ = zeros(pm_n_UX_rank_use,n_M_sub,n_w_max_use,FTK_n_svd_l);
svd_VUXM_nMwl____ = permute(svd_VUXM_lwnM____(1+FTK_index_svd_s_sort_from_all_,1+index_nw_,1:pm_n_UX_rank_use,1+index_M_in_Mbatch_),[3,4,2,1]);
svd_SVUXM_SMwl____ = zeros(n_S,n_M_sub,n_w_max_use,FTK_n_svd_l);
for nl=0:FTK_n_svd_l-1;
for nw=0:n_w_max_use-1;
svd_SVUXM_SMwl____(:,:,1+nw,1+nl) = ctranspose(CTF_UX_S_k_q_nSw___(:,:,1+nw))*svd_VUXM_nMwl____(:,:,1+nw,1+nl);
end;%for nw=0:n_w_max_use-1;
end;%for nl=0:FTK_n_svd_l-1;
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% svd_SVUXM_SMwl____: %0.6f',tmp_t)); end;
tmp_t = tic();
svd_SVUXM_lwSM____ = permute(ifft(permute(svd_SVUXM_SMwl____,[3,4,1,2]),[],1)*n_w_max_use,[2,1,3,4]);
...
%%%%%%%%;
svd_USESVUXM_dwSM____ = real(reshape(FTK_svd_U_d_expiw_s__(:,1+FTK_index_svd_s_sort_from_all_)*reshape(svd_SVUXM_lwsM____,[FTK_n_svd_l,n_w_max_use*n_S_sub*n_M_sub]),[FTK_n_delta_v,n_w_max_use,n_S_sub,n_M_sub]));
%%%%%%%%;
l2_dSM___ = permute(reshape(reshape(sqrt(CTF_UX_S_l2_use_(1+index_S_in_Sbatch_)),[n_S_sub,1])*reshape(sqrt(UX_M_l2_dM_use__(:,1+index_M_in_Mbatch_)),[1,FTK_n_delta_v*n_M_sub]),[n_S_sub,FTK_n_delta_v,n_M_sub]),[2,1,3]);
n2_dSM___ = 1./max(1e-14,l2_dSM___);
f2_dSM___ = permute(reshape(reshape(sqrt(CTF_UX_S_l2_use_(1+index_S_in_Sbatch_)),[n_S_sub,1])*reshape(1./max(1e-14,sqrt(UX_M_l2_dM_use__(:,1+index_M_in_Mbatch_))),[1,FTK_n_delta_v*n_M_sub]),[n_S_sub,FTK_n_delta_v,n_M_sub]),[2,1,3]);
...
tmp_t = tic();
X_sub_dwSM____ = repmat(reshape(n2_dSM___,[FTK_n_delta_v,1,n_S_sub,n_M_sub]),[1,n_w_max_use,1,1]) .* svd_USESVUXM_dwSM____; %<-- correlation. ;
%X_sub_dwSM____ = bsxfun(@times,reshape(real(n2_dSM___),[FTK_n_delta_v,1,n_S_sub,n_M_sub]),real(svd_USESVUXM_dwSM____)); %<-- correlation. ;
if (flag_compute_I);
I_value_sub_dwSM____ = repmat(reshape(f2_dSM___,[FTK_n_delta_v,1,n_S_sub,n_M_sub]),[1,n_w_max_use,1,1]) .* X_sub_dwSM____; %<-- I_value. ;
I_value_use_dwSM____ = max(0,real(I_value_sub_dwSM____));
end;%if (flag_compute_I);
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% X_sub_dwSM____: %0.6f',tmp_t)); end;
%%%%%%%%;
flag_check=0;
if flag_check;
[tmp_X_wSM___,tmp_delta_ij___] = max(reshape(X_sub_dwSM____,[FTK_n_delta_v,n_w_max_use*n_S_sub*n_M_sub]),[],1); %<-- maximize correlation. ;
tmp_X_wSM___ = reshape(tmp_X_wSM___,[n_w_max_use,n_S_sub,n_M_sub]);
tmp_delta_ij___ = reshape(tmp_delta_ij___,[n_w_max_use,n_S_sub,n_M_sub]);
[tmp_X_SM__,tmp_dw_ij__] = max(reshape(X_sub_dwSM____,[FTK_n_delta_v*n_w_max_use,n_S_sub*n_M_sub]),[],1); %<-- maximize correlation. ;
[tmp_delta_ij__,tmp_gamma_ij__] = ind2sub([FTK_n_delta_v,n_w_max_use],tmp_dw_ij__);
...
tmp_t = tic();
[tmp_X_wSM___,tmp_delta_ij___] = max(reshape(X_sub_dwSM____,[FTK_n_delta_v,n_w_max_use*n_S_sub*n_M_sub]),[],1); %<-- maximize correlation. ;
assert(min(tmp_delta_ij___)>=1); assert(max(tmp_delta_ij___)<=FTK_n_delta_v);
tmp_X_wSM___ = reshape(tmp_X_wSM___,[n_w_max_use,n_S_sub,n_M_sub]);
tmp_delta_ij___ = reshape(tmp_delta_ij___,[n_w_max_use,n_S_sub,n_M_sub]);
tmp_delta_x___ = FTK_delta_x_(tmp_delta_ij___);
tmp_delta_y___ = FTK_delta_y_(tmp_delta_ij___);
tmp_gamma_z___ = 2*pi*(0:n_w_max_use-1)/n_w_max_use;
X_wSM___(:,1+index_S_in_Sbatch_,1+index_M_in_Mbatch_) = tmp_X_wSM___;
delta_x_wSM___(:,1+index_S_in_Sbatch_,1+index_M_in_Mbatch_) = reshape(tmp_delta_x___,[n_w_max_use,n_S_sub,n_M_sub]);
delta_y_wSM___(:,1+index_S_in_Sbatch_,1+index_M_in_Mbatch_) = reshape(tmp_delta_y___,[n_w_max_use,n_S_sub,n_M_sub]);
gamma_z_wSM___(:,1+index_S_in_Sbatch_,1+index_M_in_Mbatch_) = repmat(tmp_gamma_z___(:),[1,n_S_sub,n_M_sub]);
if (flag_compute_I);
tmp_I_value_use_dwSM__ = reshape(I_value_use_dwSM____,[FTK_n_delta_v,n_w_max_use*n_S_sub*n_M_sub]);
tmp_I_value_use_wSM_ = zeros(n_w_max_use*n_S_sub*n_M_sub,1);
tmp_t2=tic();
for nl=0:n_w_max_use*n_S_sub*n_M_sub-1;
tmp_I_value_use_wSM_(1+nl) = tmp_I_value_use_dwSM__(tmp_delta_ij___(1+nl),1+nl);
end;%for nl=0:n_w_max_use*n_S_sub*n_M_sub-1;
I_value_wSM___(:,1+index_S_in_Sbatch_,1+index_M_in_Mbatch_) = reshape(tmp_I_value_use_wSM_,[n_w_max_use,n_S_sub,n_M_sub]);
...
[tmp_X_SM__,tmp_dw_ij__] = max(reshape(X_sub_dwSM____,[FTK_n_delta_v*n_w_max_use,n_S_sub*n_M_sub]),[],1); %<-- maximize correlation. ;
[tmp_delta_ij__,tmp_gamma_ij__] = ind2sub([FTK_n_delta_v,n_w_max_use],tmp_dw_ij__);
assert(min(tmp_delta_ij__)>=1); assert(max(tmp_delta_ij__)<=FTK_n_delta_v);
assert(min(tmp_gamma_ij__)>=1); assert(max(tmp_gamma_ij__)<=n_w_max_use);
tmp_X_SM__ = reshape(tmp_X_SM__,[n_S_sub,n_M_sub]);
tmp_delta_ij__ = reshape(tmp_delta_ij__,[n_S_sub,n_M_sub]);
tmp_gamma_ij__ = reshape(tmp_gamma_ij__,[n_S_sub,n_M_sub]);
tmp_delta_x__ = FTK_delta_x_(tmp_delta_ij__);
tmp_delta_y__ = FTK_delta_y_(tmp_delta_ij__);
tmp_gamma_z__ = 2*pi*(tmp_gamma_ij__-1)/n_w_max_use;
...
if (flag_compute_I);
tmp_I_value_use_dwSM___ = reshape(I_value_use_dwSM____,[FTK_n_delta_v,n_w_max_use,n_S_sub*n_M_sub]);
...
if (flag_optimize_over_gamma_z == 1);
if (nargout>0); X_wSM___ = X_SM__; end;
if (nargout>1); delta_x_wSM___ = delta_x_SM__; end;
if (nargout>2); delta_y_wSM___ = delta_y_SM__; end;
if (nargout>3); gamma_z_wSM___ = gamma_z_SM__; end;
if (nargout>4); I_value_wSM___ = I_value_SM__; end;
end;%if (flag_optimize_over_gamma_z == 1);

if (verbose>1); disp(sprintf(' %% [finished ampmh_X_wSM___6]')); end;
