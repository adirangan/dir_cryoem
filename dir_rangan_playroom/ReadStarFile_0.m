function [blockNames,blockData,ok]=ReadStarFile_0(name,n_Line)
...
% n_Line (optional) places a limit on the number of lines read. ;
% 
% >> blockNames{1} =
%   1×12 char array
% data_general
...
while P<=numel(C) % loop through all the entries
	...
	tmp_n_C = numel(C);
	if (tmp_n_C< P); disp(sprintf(' %% Warning, tmp_n_C %d vs P %d in ReadStarFile',tmp_n_C,P)); end;
	tmp_n_C_P = numel(C{P});
	if (tmp_n_C_P< 1); disp(sprintf(' %% Warning, tmp_n_C_P %d in ReadStarFile',tmp_n_C_P)); end;
        while (numel(C)>=P) & (numel(C{P})>=1) & (numel(C{P}{1})>=nFields)
            nRows=nRows+1;
	    tmp_n_C_P_1 = numel(C{P}{1});
	    if (tmp_n_C_P_1~=nFields); disp(sprintf(' %% Warning, tmp_n_C_P_1 %d vs nFields %d in ReadStarFile; P %d nRows %d',tmp_n_C_P_1,nFields,P,nRows)); end;
