...
c$$$      enddo
      ...
      d_(0) = 3
      d_(1) = n_source_use
      write(fname,'(A)') './dir_mda4b/source_xyz_.mda'
...
c     create physical space grid and sample function on it.
     ...
     1     fgauss,ff_ori,fnorm)
      ...
      d_(1) = ngrid
      d_(2) = ngrid
      write(fname,'(A)') './dir_mda4b/x_pre_.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(fname)
      call MDA_write_r8(3,d_,x,fname)
      write(fname,'(A)') './dir_mda4b/y_pre_.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(fname)
      call MDA_write_r8(3,d_,y,fname)
      write(fname,'(A)') './dir_mda4b/z_pre_.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(fname)
      call MDA_write_r8(3,d_,z,fname)
      write(fname,'(A)') './dir_mda4b/S_ori_.mda'
...
c-----------------------------------------------------------------------
      ...
      n_ctf = 1
      allocate(CTF_k_p_(0:nimage_size-1))
...
c-----------------------------------------------------------------------
c     6) TEST 1: generate templates; should match slices since we took slices
...
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $           *nM_sample))
         ...
         d_(0) = 5
         d_(1) = n_M_sample
         write(fname,'(A,I0,A)') './dir_mda4b/alpha_tru_',ncur,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : ',trim(fname)
         call MDA_write_r8(2,d_,alpha_tru_,fname)
         d_(0) = 5
         d_(1) = n_M_sample
         write(fname,'(A,I0,A)') './dir_mda4b/alpha_est_',ncur,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : ',trim(fname)
...
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        ,wtsth_tmp,ngridps_tmp,phsteps_tmp,nspherebig_tmp)
         call rebuild_model_alpha2d(M_sample_,nimage_size,n_M_sample
     $        ,1,nimage_size,CTF_k_p_,I_ctf_,5,alpha_tru_
     $        ,icstart ,nlats,itypep,xnodesr,ngridc,nlow,ncur
     $        ,xnodesth_tmp,wtsth_tmp,nsphstore ,isph_start,nterms_sph
     $        ,oversamp,kord,modsph_tru)

c$$$         call rebuild_model_alphaX(M_sample_,nimage_size,n_M_sample ,5
c$$$     $        ,alpha_tru_,icstart,nlats,itypep,xnodesr,ngridc,nlow,ncur
c$$$     $        ,nsphstore,isph_start,nterms_sph,oversamp,kord,eps
c$$$     $        ,modsph_tru)
     ...
     $        ,eps,npts,x,y,z,ff_tru,ier)
         ...
         d_(1) = ngrid
         d_(2) = ngrid
         write(fname,'(A,I0,A)') './dir_mda4b/S_tru_',ncur,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : ',trim(fname)
...
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        ,wtsth_tmp,ngridps_tmp,phsteps_tmp,nspherebig_tmp)
         call rebuild_model_alpha2d(M_sample_,nimage_size,n_M_sample
     $        ,1,nimage_size,CTF_k_p_,I_ctf_,5,alpha_est_
     $        ,icstart ,nlats,itypep,xnodesr,ngridc,nlow,ncur
     $        ,xnodesth_tmp,wtsth_tmp,nsphstore ,isph_start,nterms_sph
     $        ,oversamp,kord,modsph_est)

c$$$         call rebuild_model_alphaX(M_sample_,nimage_size,n_M_sample ,5
c$$$     $        ,alpha_est_,icstart,nlats,itypep,xnodesr,ngridc,nlow,ncur
c$$$     $        ,nsphstore,isph_start,nterms_sph,oversamp,kord,eps
c$$$     $        ,modsph_est)
     ...
     $        ,eps,npts,x,y,z,ff_est,ier)
         ...
         d_(1) = ngrid
         d_(2) = ngrid
         write(fname,'(A,I0,A)') './dir_mda4b/S_est_',ncur,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : ',trim(fname)
...
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        *n_S_sample,') innerproducts'
         ...
         call test_innerproduct_batch_wrapper_2(0,ncur,nlats,xnodesr
     $       ,max_x_c,n_S_sample,ntemplatesize,S_sample_,n_M_sample
     $       ,nimage_size,M_sample_,alpha_est_,0.1d0,N_pixels_in
     $       ,displacement_max,n_delta_x,n_delta_y,max(32,2 *ncur), -1
     $       ,delta_x_max_,delta_y_max_,gamma_z_max_,C_Z_max_)
         timing_toc = omp_get_wtime()
         if (verbose.gt.0) then
            write(6,'(A,A,F8.3)') 'test_innerproduct_batch_wrapper_2:'
     $           ,' total_time ',timing_toc-timing_tic
         end if
         d_(0) = n_S_sample
         d_(1) = n_M_sample
         write(fname,'(A,I0,A)') './dir_mda4b/C_Z_max_',ncur,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : ',trim(fname)
         call MDA_write_c16(2,d_,C_Z_max_,fname)
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
c$$$         sort results
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         call test_innerproduct_batch_wrapper_3(0,n_S_sample,n_M_sample
     $     ,delta_x_max_,delta_y_max_,gamma_z_max_,C_Z_max_
...
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         write(fname,'(A,I0)') './dir_jpg4b/Fig_MTS_',ncur
         call Fig_gen_ver1(ncur,nlats,xnodesr,n_S_sample,ntemplatesize
     $       ,S_sample_,n_M_sample,nimage_size,M_sample_,alpha_est_
     $       ,delta_x_sort_SM_,delta_y_sort_SM_,gamma_z_sort_SM_
     $       ,I_permute_SM_,min(8,n_M_sample),fname)
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
c$$$         update alphas
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         call test_innerproduct_batch_wrapper_4(0,n_S_sample,n_M_sample
     $        ,delta_x_sort_SM_,delta_y_sort_SM_,gamma_z_sort_SM_
     $        ,C_Z_sort_SM_,I_permute_SM_,I_inverse_SM_
     $        ,delta_x_sort_MS_,delta_y_sort_MS_,gamma_z_sort_MS_
     $        ,C_Z_sort_MS_,I_permute_MS_,I_inverse_MS_,alpha_0_sample_
     $        ,alpha_1_sample_,alpha_est_)         
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
