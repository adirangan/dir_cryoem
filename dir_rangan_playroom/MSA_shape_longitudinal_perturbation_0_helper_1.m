...
%%%%;
figure(1);clf;figmed;
fontsize_use = 12; linewidth_use = 2;
p_row = 1; p_col = 2; np=0;
c_use__ = colormap('cool'); n_c_use = size(c_use__,1);
%%;
subplot(p_row,p_col,1+np);np=np+1;
plot(gamma_z_,g_dilate(gamma_z_),'k-','LineWidth',linewidth_use);
...
title('perturbation $g(\gamma)$','Interpreter','latex');
set(gca,'FontSize',fontsize_use);
%%;
subplot(p_row,p_col,1+np);np=np+1;
n_omega = 64; omega_pre_ = linspace(0,2*pi,n_omega+1); omega_pre_ = transpose(omega_pre_(1:n_omega));
omega_pos_ = f_dilate(omega_pre_);
hold on;
for nomega=0:n_omega-1;
n_psi = 32; psi_ = linspace(omega_pre_(1+nomega),omega_pos_(1+nomega),n_psi);
r_0 = 0.0; r_1 = 1.0;
psi_pre_ = 0.5*[psi_(1) , psi_(1:end-1)] + 0.5*psi_;
psi_pos_ = 0.5*psi_ + 0.5*[psi_(2:end) , psi_(end)];
tmp_x_ = [ r_0*cos(psi_pre_) ; r_1*cos(psi_pre_) ; r_1*cos(psi_pos_) ; r_0*cos(psi_pos_) ];
tmp_y_ = [ r_0*sin(psi_pre_) ; r_1*sin(psi_pre_) ; r_1*sin(psi_pos_) ; r_0*sin(psi_pos_) ];
tmp_c__ = c_use__(round(linspace(1,n_c_use,n_psi)),:);
patch(tmp_x_,tmp_y_,reshape(tmp_c__,[1,n_psi,3]),'LineStyle','none');
end;%for nomega=0:n_omega-1;
hold on;
xlabel('$\cos(\gamma)$','Interpreter','latex'); xlim([-1,+1]); set(gca,'XTick',[-1,0,+1]);
ylabel('$\sin(\gamma)$','Interpreter','latex'); ylim([-1,+1]); set(gca,'YTick',[-1,0,+1]);
title('$\gamma$ old (blue) $\rightarrow$ $\gamma$ new (pink) ','Interpreter','latex');
set(gca,'FontSize',fontsize_use);
axis square;
%%;
fname_fig_pre = sprintf('%s/MSA_shape_longitudinal_perturbation_illustration_dt%.3d',dir_manuscript_jpg,floor(1000*dt));
