...
str_thisfunction = 'qbp_sheres_1';

a_k_Y_zero_yk_ = []; %<-- zero-temperature limit. ;
a_k_Y_zero_yk__ = [];
a_k_Y_0qbp_yk_ = []; %<-- standard quadratur-backpropagation. ;
a_k_Y_0qbp_yk__ = [];
a_k_Y_sher_yk_ = []; %<-- sheres-style bayesian-inference. ;
a_k_Y_sher_yk__ = [];
X_base_wSM___ = []; %<-- in principle this could be passed in (and not recomputed later). ;
delta_x_base_wSM___ = []; %<-- in principle this could be passed in (and not recomputed later). ;
delta_y_base_wSM___ = []; %<-- in principle this could be passed in (and not recomputed later). ;
gamma_z_base_wSM___ = []; %<-- in principle this could be passed in (and not recomputed later). ;
I_value_base_wSM___ = []; %<-- in principle this could be passed in (and not recomputed later). ;
euler_polar_a_base_M_=[]; %<-- in principle this could be passed in (and not recomputed later). ;
euler_azimu_b_base_M_=[]; %<-- in principle this could be passed in (and not recomputed later). ;
euler_gamma_z_base_M_=[]; %<-- in principle this could be passed in (and not recomputed later). ;
image_delta_x_base_M_=[]; %<-- in principle this could be passed in (and not recomputed later). ;
image_delta_y_base_M_=[]; %<-- in principle this could be passed in (and not recomputed later). ;
image_I_value_base_M_=[]; %<-- in principle this could be passed in (and not recomputed later). ;
frac_qk__ = [];
quad_from_data_T_CTF_normalized_qk__ = [];
expR2_sum_M_ = [];
numerator_qkM___ = [];
...
l_max_max = n_w_max/2 - 1; %<-- assumes n_w_max = 2*(l_max_max+1) ;
...
R2_base_SM__ = 2 - 2*X_base_SM__; %<-- residual assumes that images and templates are normalized (similar to the assumption within X_base_SM__). ;
R2_min_M_ = reshape(min(R2_base_SM__,[],1),[n_M,1]); %<-- minimum residual for each image. Used to offset exponent below. ;
...
Ylm_w_yq__ = Ylm_yq__ * sparse(1:quad_n_all,1:quad_n_all,quad_weight_all_,quad_n_all,quad_n_all); %<-- Ylm conversion operator along with the appropriate quadrature weights. ;
%%%%;
% now we set the viewing_gamma_z_ to be zero, orienting the data_k_c_?_wS__ to the typical templates we generate. ;
%%%%;
[ ...
...
%%%%%%%%;
quad_from_data_T_CTF_numerator_qk__ = zeros(quad_n_all,n_k_p_r); %<-- will hold the translated images multiplied by the associated CTF value. ;
quad_from_data_T_CTF_denomator_qk__ = zeros(quad_n_all,n_k_p_r); %<-- will hold the sum of the squares of the associated CTF values. ;
%%%%%%%%%%%%%%%%;
for ncluster=0:n_cluster-1;
%%%%%%%%%%%%%%%%;
index_nM_from_ncluster_ = index_nM_from_ncluster__{1+ncluster};
...
%%%%%%%%;
T_k_p_sub_wkM__ = zeros(n_w_sum,tmp_n_M); %<-- will hold the translated version of each image (reconstructed from the FTK and svd_VUXM_lwnM____). ;
for nM_sub=0:tmp_n_M-1;
nM = index_nM_from_ncluster_(1+nM_sub);
image_delta_index = knnsearch([FTK.delta_x_,FTK.delta_y_],[+image_delta_x_sub_M_(1+nM_sub),+image_delta_y_sub_M_(1+nM_sub)],'K',1)-1; %<-- for each image, we find the nearest listed translation from the FTK structure. ;
...
if sigma_sheres> 0;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
% Now put together sheres-style qbp if the temperature is positive. ;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
...
%%%%%%%%;
UX_kn__ = UX_knc___(:,1:pm_n_UX_rank,1+ncluster); %<-- principal-modes for this particular cluster. ;
X_weight_r_ = X_weight_rc__(:,1+ncluster);
%%%%%%%%;
CTF_k_p_r_xavg_k_ = CTF_k_p_r_xavg_kc__(:,1+ncluster);
CTF_UX_S_k_q_wnS__ = zeros(pm_n_w_sum,n_S); %<-- holds principal-templates. ;
...
%%%%%%%%;
tmp_X_sub_wSM___ = zeros(n_w_max,n_S_sub,tmp_n_M_sub); %<-- holds the correlations between principal-images and principal-templates for this batch (maximized over translations). ;
...
tmp_X_sub_dwSM____ = zeros(FTK.n_delta_v,n_w_max,n_S_sub,tmp_n_M_sub); %<-- holds the correlations between principal-images and principal-templates for this batch, this time spanning translations. (Note that this storage can be quite memory intensive). ;
%%%%%%%%;
tmp_t = tic();
[ ...
 parameter ...
...
%%%%%%%%;
tmp_t = tic();
tmp_R2_wSdM____ = bsxfun(@minus,permute(2 - 2*tmp_X_sub_dwSM____,[2,3,1,4]),reshape(R2_sub_min_M_(1+index_tmp_nM_in_Mbatch_),[1,1,1,tmp_n_M_sub])); %<-- note that R2_sub_min_M_ is used as an offset. ;
tmp_expR2_p_wSdM____ = exp(-tmp_R2_wSdM____/(2*sigma_sheres^2)); %<-- these are the unnormalized probabilities associated with each image. ;
tmp_p_p_wSdMk_____ = ...
  bsxfun(@times ...
	 ,reshape(tmp_expR2_p_wSdM____,[n_w_max,n_S_sub,FTK.n_delta_v,tmp_n_M_sub,1]) ...
	 ,reshape(CTF_k_p_r_xavg_k_,[1,1,1,1,n_k_p_r]) ...
	 ) ...
  /sigma_sheres^2;
%<-- tmp_p_q_wSdMk_____ now holds the product of the (unnormalized) probabilites and the CTF for each k-ring (all divided by sigma_sheres^2). ;
%<-- this corresponds to the $P(...)\cdot\CTF_{ij}$ from Sheres equation 3. ;
tmp_p_q_wSdMk_____ = ...
  reshape( ...
	   interp_p_to_q_block_0( ...
				  n_S_sub*FTK.n_delta_v*tmp_n_M_sub*n_k_p_r ...
				  ,n_w_max*ones(n_S_sub*FTK.n_delta_v*tmp_n_M_sub*n_k_p_r,1) ...
				  ,n_w_max*n_S_sub*FTK.n_delta_v*tmp_n_M_sub*n_k_p_r ...
				  ,tmp_p_p_wSdMk_____ ...
				  ) ...
	   ,[n_w_max,n_S_sub,FTK.n_delta_v,tmp_n_M_sub,n_k_p_r] ...
	   );
%<-- tmp_p_q_wSdMk_____ now holds the fourier-transform (i.e., bessel-coefficients) of tmp_p_p_wSdMk_____. ;
%<-- this will be used below to calculate the inner-product between a principal-image and tmp_p_p_wSdMk_____ across all in-plane rotations gamma. ;
expR2_sub_sum_M_(1+index_tmp_nM_in_Mbatch_) = ...
  expR2_sub_sum_M_(1+index_tmp_nM_in_Mbatch_) ...
  + reshape( ...
	     sum( ...
		  reshape( ...
			   tmp_expR2_p_wSdM____ ...
			   ,[n_w_max*n_S_sub*FTK.n_delta_v,tmp_n_M_sub] ...
			   ) ...
		  ,1 ...
		  ) ...
	     ,[tmp_n_M_sub,1] ...
	     );
%<-- expR2_sub_sum_M_ now holds the summed (unnormalized) probabilites associated with each image. ;
%<-- these will be used to create the $\Gamma$ from Sheres equation 6. ;
tmp_numerator_qdMk____ = ...
  reshape( ...
	   quad_from_data_sub_qwS__*reshape( ...
					     ifft( ...
						   reshape( ...
							    bsxfun(@times ...
								   ,reshape( ...
									     reshape( ...
										      reshape( ...
											       permute( ...
													reshape( ...
														 FTK.svd_U_d_expiw_s__*reshape( ...
																		svd_VUXM_sub_lwnM____ ...
																		,[FTK.n_svd_l,n_w_max*pm_n_UX_rank*tmp_n_M_sub] ...
																		) ...
														 ,[FTK.n_delta_v,n_w_max,pm_n_UX_rank,tmp_n_M_sub] ...
														 )*n_w_max ...
													,[2,1,4,3] ...
													) ...
											       ,[n_w_max*FTK.n_delta_v*tmp_n_M_sub,pm_n_UX_rank] ...
											       )*transpose(UX_kn__)*diag(X_weight_r_.^(-1)) ...
										      ,[n_w_max,FTK.n_delta_v,tmp_n_M_sub,n_k_p_r] ...
										      ) ...
									     ,[n_w_max,1,FTK.n_delta_v,tmp_n_M_sub,n_k_p_r] ...
									     ) ...
								   ,reshape( ...
									     conj(tmp_p_q_wSdMk_____) ...
									     ,[n_w_max,n_S_sub,FTK.n_delta_v,tmp_n_M_sub,n_k_p_r] ...
									     ) ...
								   ) ...
							    ,[n_w_max,n_S_sub*FTK.n_delta_v*tmp_n_M_sub*n_k_p_r] ...
	   ...
	   ,[quad_n_all,FTK.n_delta_v,tmp_n_M_sub,n_k_p_r] ...
	   );
%<-- tmp_numerator_wSdMk_____ would hold the collection of inner-products between the various tranlsated principal-images and rotated versions of tmp_p_p_wSdMk_____ (for different in-plane gamma). ;
%<-- tmp_numerator_qdMk____ now holds the accumulation of tmp_numerator_wSdMk_____ onto each quadrature node. ;
tmp_denomator_qdMk____ = ...
  reshape( ...
	   quad_from_data_sub_qwS__ * reshape( ...
					       bsxfun(@times ...
						      ,reshape(CTF_k_p_r_xavg_k_,[1,1,1,1,n_k_p_r]) ...
						      ,repmat(sum(tmp_p_p_wSdMk_____,1),[n_w_max,1,1,1,1]) ...
						      ) ...
					       ,[n_w_max*n_S_sub,FTK.n_delta_v*tmp_n_M_sub*n_k_p_r] ...
					       ) ...
	   ,[quad_n_all,FTK.n_delta_v,tmp_n_M_sub,n_k_p_r] ...
	   );
%<-- tmp_denomator_wSdMk_____ would hold the product of the (unnormalized) probabilties and the squared CTF for each k-ring (all divided by sigma_sheres^2). ;
%<-- tmp_denomator_qdMk____ now holds the accumulation of tmp_denomator_wSdMk_____ onto each quadrature node. ;
...
%<-- frac_qk__ now accounts for the terms shown in equation 3 in Sheres. ;
%<-- Note that the $sigma$ is fixed and $\tau_{kl}$ is ignored. ;
