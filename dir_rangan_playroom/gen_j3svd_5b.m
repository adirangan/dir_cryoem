function [n_svd_r,svd_r_,svd_r_m,svd_r_c,svd_r_w_,svd_r_Jv_,n_svd_d,svd_d_,svd_d_m,svd_d_c,svd_d_w_,svd_d_Jv_,n_svd_l,svd_l_,svd_U_d_,svd_s_,svd_V_r_] = gen_j3svd_5b(K_max,N_pixel,eps_target,l_max,a_K,b_K);
...
disp(sprintf(' %% testing gen_j3svd_5b with K_max %d N_pixel %0.2f eps_target %0.2f and radial weight function',K_max,N_pixel,eps_target));
[n_svd_r,svd_r_,svd_r_m,svd_r_c,svd_r_w_,svd_r_Jv_,n_svd_d,svd_d_,svd_d_m,svd_d_c,svd_d_w_,svd_d_Jv_,n_svd_l,svd_l_,svd_U_d_,svd_s_,svd_V_r_] = gen_j3svd_5b(K_max,N_pixel,eps_target,l_max,n_r_degree,n_d_degree);
...
J_ = besselj(l_tmp + 0.5,transpose(delta_value_)*r_value_) .* sqrt(pi./2) ./ sqrt(transpose(delta_value_)*r_value_);
H_ = zeros(size(J_));
for ij=ij_tmp_;
...
tmp_2 = sum(sum(abs(J_).*(svd_d_w_*transpose(svd_r_w_))));
...
if (verbose>1); 
disp(sprintf(' %% [entering gen_j3svd_5b] N_pixel %0.2f l_max %d, a_K %d, b_K %d, eps_target %0.6f',N_pixel,l_max,a_K,b_K,eps_target)); 
end;%if (verbose>1);
