...
%%%%%%%%;
% debugging tphtam_experimental_12. ;
%%%%%%%%;
clear;
...
%%%%%%%%;
dir_data = sprintf('/%s/rangan/dir_cryoem/dir_trpv1/data_nosym',string_root);
...
%%%%%%%%;
%dir_svd = sprintf('/%s/rangan/dir_cryoem/dir_rangan_playground/dir_gen_Jsvd_6',string_root);
...
if flag_check;
%%%%%%%%;
% compare principal-images to principal-templates. ;
% compare the log-likelihood to the correlations. ;
%%%%%%%%;
tmp_n_UX_rank = 2;
pm_n_UX_rank = tmp_n_UX_rank;
pm_n_k_p_r = pm_n_UX_rank;
...
tmp_t = toc(tmp_t); if (verbose); disp(sprintf(' %% n_M_sub %d L_wSM___: %0.3fs',n_M_sub,tmp_t)); end;
%%%%%%%%;
% Now compare the log-likelihoods to the correlations. ;
%%%%%%%%;
n_iteration = 6;
for niteration=0:n_iteration-1;
nM = max(0,min(n_M_sub-1,floor(n_M_sub*rand())));
nS = max(0,min(n_S-1,floor(n_S*rand())));
nw = max(0,min(n_w_max-1,floor(n_w_max*rand())));
L_1 = L_wSM___(1+nw,1+nS,1+nM);
delta_j = delta_j_wSM___(1+nw,1+nS,1+nM);
I_value = I_value_wSM___(1+nw,1+nS,1+nM);
tmp_image_delta_x = FTK.delta_x_(1+delta_j); tmp_image_delta_y = FTK.delta_y_(1+delta_j);
[UX_M_k_q_wn__,UX_M_k_p_wn__] = ampmh_UX_M_k_p_wnM___0(FTK,n_w_,pm_n_UX_rank,1,svd_VUXM_lwnM____(:,:,:,1+nM),tmp_image_delta_x,tmp_image_delta_y);
UX_S_CTF_k_q_ = US_k_q__*SS_k_q__*ctranspose(VS_k_q__(1+nS,:));
X_0 = innerproduct_p_quad(pm_n_k_p_r,pm_k_p_r_,pm_weight_2d_k_p_r_/(2*pi),pm_n_w_,pm_n_w_sum,rotate_q_to_q(pm_n_k_p_r,pm_n_w_,pm_n_w_sum,UX_S_CTF_k_q_,2*pi*nw/n_w_max),UX_M_k_q_wn__(:)) / sqrt(UX_S_l2_(1+nS) * UX_M_l2_dM__(1+delta_j,1+nM));
L_0 = UX_S_l2_(1+nS)*(X_0^2-1);
disp(sprintf(' %% L_0 vs L_1: %0.16f',fnorm(L_0-L_1)/fnorm(L_0)));
I_0 = innerproduct_p_quad(pm_n_k_p_r,pm_k_p_r_,pm_weight_2d_k_p_r_/(2*pi),pm_n_w_,pm_n_w_sum,rotate_q_to_q(pm_n_k_p_r,pm_n_w_,pm_n_w_sum,UX_S_CTF_k_q_,2*pi*nw/n_w_max),UX_M_k_q_wn__(:)) / UX_M_l2_dM__(1+delta_j,1+nM);
disp(sprintf(' %% I_0 vs I_value: %0.16f',fnorm(I_0-I_value)/fnorm(I_0)));
end;%for niteration=0:n_iteration-1;
%%%%%%%%;
% Now repeat without translations. ;
% Check to see if MS optimization is the same. ;
%%%%%%%%;

FTK = ampmh_FTK_1(n_k_p_r,k_p_r_,k_p_r_max,0,svd_eps,1);
%%%%%%%%;
n_M = n_image_sub;
M_k_q__ = zeros(n_w_sum,n_M);
for nM=0:n_M-1;
...
delta_j = delta_j_wSM___(1+nw,1+nS,1+nM);
I_value = I_value_wSM___(1+nw,1+nS,1+nM),;
I_value_use = max(0,real(I_value));
tmp_image_delta_x = FTK.delta_x_(1+delta_j); tmp_image_delta_y = FTK.delta_y_(1+delta_j);
[UX_M_k_q_wn__,UX_M_k_p_wn__] = ampmh_UX_M_k_p_wnM___0(FTK,n_w_,pm_n_UX_rank,1,svd_VUXM_lwnM____(:,:,:,1+nM),tmp_image_delta_x,tmp_image_delta_y);
UX_S_CTF_k_q_ = US_k_q__*SS_k_q__*ctranspose(VS_k_q__(1+nS,:));
tmp_MS = real(innerproduct_p_quad(pm_n_k_p_r,pm_k_p_r_,pm_weight_2d_k_p_r_/(2*pi),pm_n_w_,pm_n_w_sum,rotate_q_to_q(pm_n_k_p_r,pm_n_w_,pm_n_w_sum,UX_S_CTF_k_q_,2*pi*nw/n_w_max),UX_M_k_q_wn__(:)));
X_0 = tmp_MS / sqrt(UX_S_l2_(1+nS) * UX_M_l2_dM__(1+delta_j,1+nM));
disp(sprintf(' %% X_0 vs X_1: %0.16f',fnorm(X_0-X_1)/fnorm(X_0)));
% L = -I*I*<M,M> + 2*I*<M,S> - <S,S> ;
L_0 = -I_value_use.^2*real(UX_M_l2_dM__(1+delta_j,1+nM)) + 2*I_value_use*tmp_MS - UX_S_l2_(1+nS);
L_2 = UX_S_l2_(1+nS)*(real(X_0).^2 - 1);
disp(sprintf(' %% I_value %+.16f <-- I_value_use %+.16f: L_0 vs L_2: %0.16f',I_value,I_value_use,fnorm(L_0-L_2)/fnorm(L_0)));
disp(sprintf(' %% L_0 vs L_1: %0.16f',fnorm(L_0-L_1)/fnorm(L_0)));
I_0 = innerproduct_p_quad(pm_n_k_p_r,pm_k_p_r_,pm_weight_2d_k_p_r_/(2*pi),pm_n_w_,pm_n_w_sum,rotate_q_to_q(pm_n_k_p_r,pm_n_w_,pm_n_w_sum,UX_S_CTF_k_q_,2*pi*nw/n_w_max),UX_M_k_q_wn__(:)) / UX_M_l2_dM__(1+delta_j,1+nM);
disp(sprintf(' %% I_0 vs I_value: %0.16f',fnorm(I_0-I_value)/fnorm(I_0)));
end;%for niteration=0:n_iteration-1;
%%%%%%%%;
% Now check ordering/ranking of X_wSM___ vs L_wSM___. ;
% Note that matches with negative correlation are indicated as having high likelihood. ;
% This is because negative intensities I_value are allowed. ;
%%%%%%%%;
tmp_E = 0;
tmp_A = 0;
for nS=0:n_S-1;
tmp_UX_S_l2 = UX_S_l2_(1+nS);
for nw=0:pm_n_w_max-1;
tmp_X_ = reshape(real(X_wSM___(1+nw,1+nS,:)),[n_M_sub,1]);
tmp_L1_ = reshape(real(L_wSM___(1+nw,1+nS,:)),[n_M_sub,1]);
[~,tmp_X_ij_] = sort(tmp_X_,'ascend');
[~,tmp_L1_ij_] = sort(tmp_L1_,'ascend');
tmp_E = tmp_E + numel(find(tmp_X_ij_~=tmp_L1_ij_));
tmp_A = tmp_A + numel(tmp_X_ij_);
end;%for nw=0:pm_n_w_max-1;
end;%for nS=0:n_S-1;
disp(sprintf(' %% tmp_E/tmp_A %0.2f/%0.2f = %0.2f',tmp_E,tmp_A,tmp_E/tmp_A));
%%%%%%%%;
end;%if flag_check;
...
%%%%%%%%;
fname_mat = sprintf('%s_mat/test_principled_marching_trpv1_17_d0_UXI_CTF_S_k_p__.mat',dir_trunk);
if (~exist(fname_mat,'file'));
disp(sprintf(' %% %s not found, creating',fname_mat));
%%%%%%%%;
...
a_UXI_Y_quad__ = zeros(pm_n_lm_max,n_UXI_rank);
for nUXI_rank=0:n_UXI_rank-1;
for nk_p_r=0:n_k_p_r-1;
tmp_l_max = l_max_(1+nk_p_r);
tmp_n_lm = (tmp_l_max+1).^2;
tmp_index_ = n_lm_csum_(1+nk_p_r) + (0:tmp_n_lm-1);
a_UXI_Y_quad__(1:tmp_n_lm,1+nUXI_rank) = a_UXI_Y_quad__(1:tmp_n_lm,1+nUXI_rank) + UXI__(1+nk_p_r,1+nUXI_rank)*X_weight_r_(1+nk_p_r)*a_k_Y_quad_(1+tmp_index_)*CTF_avg_k_p_r_(1+nk_p_r); %<-- use average CTF here, under the assumption that a_UXI_Y_quad_ will be used alone. ;
end;%for nk_p_r=0:n_k_p_r-1;
...
% l2_signal = sqrt(mean(abs(d0_UXI_CTF_S_k_p__).^2 * reshape(pm_viewing_weight_all_,[pm_n_viewing_all,1]) / (4*pi))). ;
...
%%%%%%%%;
fname_fig = sprintf('%s_jpg/test_principled_marching_trpv1_17_d0_UXI_CTF_S_k_p__',dir_trunk);
if (~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
clf;
n_plot=6;
...
imagesc_p(pm_n_k_p_r,1:pm_n_k_p_r,pm_n_w_,sum(pm_n_w_),real(d0_UXI_CTF_S_k_p__(:,1+nviewing_all)),[],colormap_beach());
axis image; axisnotick; title(sprintf('d0_UXI_CTF_S_k_p__(:,1+nv) %d',nviewing_all),'Interpreter','none');
subplot(6,5,1+5*nplot+1);
imagesc_p(pm_n_k_p_r,1:pm_n_k_p_r,pm_n_w_,sum(pm_n_w_),real(da_UXI_CTF_S_k_p__(:,1+nviewing_all)),[],colormap_beach());
axis image; axisnotick; title(sprintf('da_UXI_CTF_S_k_p__(:,1+nv) %d',nviewing_all),'Interpreter','none');
subplot(6,5,1+5*nplot+2);
imagesc_p(pm_n_k_p_r,1:pm_n_k_p_r,pm_n_w_,sum(pm_n_w_),real(db_UXI_CTF_S_k_p__(:,1+nviewing_all)),[],colormap_beach());
axis image; axisnotick; title(sprintf('db_UXI_CTF_S_k_p__(:,1+nv) %d',nviewing_all),'Interpreter','none');
subplot(6,5,1+5*nplot+3);
imagesc_p(pm_n_k_p_r,1:pm_n_k_p_r,pm_n_w_,sum(pm_n_w_),real(dc_UXI_CTF_S_k_p__(:,1+nviewing_all)),[],colormap_beach());
axis image; axisnotick; title(sprintf('dc_UXI_CTF_S_k_p__(:,1+nv) %d',nviewing_all),'Interpreter','none');
subplot(6,5,1+5*nplot+4);
imagesc_p(pm_n_k_p_r,1:pm_n_k_p_r,pm_n_w_,sum(pm_n_w_),real(dt_UXI_CTF_S_k_p__(:,1+nviewing_all)),[],colormap_beach());
axis image; axisnotick; title(sprintf('dt_UXI_CTF_S_k_p__(:,1+nv) %d',nviewing_all),'Interpreter','none');
end;%for nplot=0:n_plot-1;
figbig;
disp(sprintf(' %% writing %s',fname_fig));
print('-djpeg',sprintf('%s.jpg',fname_fig));
...
%%%%%%%%;
fname_fig = sprintf('%s_jpg/test_principled_marching_trpv1_17_d0_UXI_CTF_S_grad_FIGA',dir_trunk);
if (~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
%%%%;
...
imagesc_polar_a_azimu_b_0(pm_viewing_polar_a_all_,pm_viewing_azimu_b_all_,eta_equi_upb___(1+nl2_snr,1+nk_p_r,:),[0,pi],c__);
...
% Now calculate the typical directional-derivative (with respect to viewing-angle) of the template-operator applied to the true molecule. ;
%%%%%%%%;
fname_mat = sprintf('%s_mat/test_principled_marching_trpv1_17_d0_UX_CTF_S_k_p__.mat',dir_trunk);
if (~exist(fname_mat,'file'));
disp(sprintf(' %% %s not found, creating',fname_mat));
%%%%%%%%;
pm_n_UX_rank = 9;
...
%%%%%%%%;
fname_fig = sprintf('%s_jpg/test_principled_marching_trpv1_17_d0_UX_CTF_S_k_p__',dir_trunk);
if (~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
clf;
n_plot=6;
...
nviewing_all = max(0,min(pm_n_viewing_all-1,round(pm_n_viewing_all*nplot/n_plot)));
subplot(6,5,1+5*nplot+0);
imagesc(real(reshape(d0_UX_CTF_S_k_p__(:,1+nviewing_all),[pm_n_w_max,pm_n_k_p_r]))); colormap(colormap_beach());
axisnotick; title(sprintf('d0_UX_CTF_S_k_p__(:,1+nv) %d',nviewing_all),'Interpreter','none');
subplot(6,5,1+5*nplot+1);
imagesc(real(reshape(da_UX_CTF_S_k_p__(:,1+nviewing_all),[pm_n_w_max,pm_n_k_p_r]))); colormap(colormap_beach());
axisnotick; title(sprintf('da_UX_CTF_S_k_p__(:,1+nv) %d',nviewing_all),'Interpreter','none');
subplot(6,5,1+5*nplot+2);
imagesc(real(reshape(db_UX_CTF_S_k_p__(:,1+nviewing_all),[pm_n_w_max,pm_n_k_p_r]))); colormap(colormap_beach());
axisnotick; title(sprintf('db_UX_CTF_S_k_p__(:,1+nv) %d',nviewing_all),'Interpreter','none');
subplot(6,5,1+5*nplot+3);
imagesc(real(reshape(dc_UX_CTF_S_k_p__(:,1+nviewing_all),[pm_n_w_max,pm_n_k_p_r]))); colormap(colormap_beach());
axisnotick; title(sprintf('dc_UX_CTF_S_k_p__(:,1+nv) %d',nviewing_all),'Interpreter','none');
subplot(6,5,1+5*nplot+4);
imagesc(real(reshape(dt_UX_CTF_S_k_p__(:,1+nviewing_all),[pm_n_w_max,pm_n_k_p_r]))); colormap(colormap_beach());
axisnotick; title(sprintf('dt_UX_CTF_S_k_p__(:,1+nv) %d',nviewing_all),'Interpreter','none');
end;%for nplot=0:n_plot-1;
figbig;
disp(sprintf(' %% writing %s',fname_fig));
print('-djpeg',sprintf('%s.jpg',fname_fig));
...
%%%%%%%%;
fname_fig = sprintf('%s_jpg/test_principled_marching_trpv1_17_d0_UX_CTF_S_grad_FIGA',dir_trunk);
if (~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
%%%%;
l2_snr_ = [0 , 2.^[-2:-0.5:-4]]; n_l2_snr=numel(l2_snr_);
...
delta_r_max_factor_ = [0,1,sqrt(2)]; n_delta_r_max_factor = numel(delta_r_max_factor_);
%%%%%%%%;
...
dat_infix = sprintf('%s_I_%s',str_combine,str_delta);
...
tmp_t = tic;
tmp_e_k_Y_reco_ = cg_lsq_4(tmp_n_order,n_k_p_r,k_p_r_,l_max_,n_w_,tmp_n_M,M_k_p__,CTF_idx_,CTF_k_p__,tmp_euler_polar_a_,tmp_euler_azimu_b_,tmp_euler_gamma_z_,tmp_image_delta_x_,tmp_image_delta_y_);
...
%%%%%%%%;
fname_fig = sprintf('%s_jpg/test_principled_marching_trpv1_17_X_compare_C',dir_trunk);
if (1 | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
clf;
figure(1);clf;
