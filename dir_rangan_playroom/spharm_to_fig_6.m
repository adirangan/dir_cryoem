function [ FA_x_c_ , X_1_ , X_2_ , X_3_ ]  = spharm_to_fig_6(n_k,k_,n_l_,a_,res,alpha_,delta_);
...
if nargin<7; delta_ = 1*0.125*[1,2,3]; end;
if nargin<6; alpha_ = [1*pi/4,1*pi/4,1*pi/6]; end;
...
FA_k_c_ = MOL_A_K_; FA_l_k_c = abs(FA_k_c_); FA_w_k_c = angle(FA_k_c_);
...
v = v_(nv);
hpatch = patch(isosurface(K_1_,K_2_,K_3_,F_tmp_,v)); isonormals(K_1_,K_2_,K_3_,F_tmp_,hpatch);
...
FA_rk_c_ = zeros(sample_rate*res,sample_rate*res,sample_rate*res);
FA_rk_c_(ij_tmp,ij_tmp,ij_tmp) = FA_k_c_;
...
FA_x_c_ = real(recenter3(fftn(recenter3(FA_rk_c_)))); FA_x_c_ = FA_x_c_(ij_tmp,ij_tmp,ij_tmp);
...
for nv=length(v_):-1:1;
v = v_(nv);
hpatch = patch(isosurface(X_1_,X_2_,X_3_,FA_x_c_,v)); isonormals(X_1_,X_2_,X_3_,FA_x_c_,hpatch);
nc = max(1,min(ncra,floor(ncra*nv/length(v_))));
hpatch.FaceColor = cra(nc,:);
hpatch.EdgeColor = 'none';
hpatch.FaceAlpha = (nv/length(v_)).^4 * 1.0;
...
for nk=1:n_k;
k = k_(nk); 
n_all_(nk) =  sample_shell_0(k,sample_d,'L');
end%for nk=1:n_k;
n_all = sum(n_all_);
k_all_ = zeros(n_all,1); theta_all_ = zeros(n_all,1); phi_all_ = zeros(n_all,1); weight_all_ = zeros(n_all,1);
MOL_A_K_P_ = zeros(n_all,1);

n_sub = 0;
for nk=1:n_k;
k = k_(nk); 
[length_sub,theta_sub_,phi_sub_,weight_sub_] = sample_shell_0(k,sample_d,'L');
n_l = n_l_(nk); n_lm = n_lm_(nk); ix_base = sum(n_lm_(1:nk-1));
a_k_ = a_(ix_base + (1:n_lm)); 
l_ = []; m_ = []; for nl=0:n_l; l_ = [l_ , nl*ones(1,2*nl+1) ]; m_ = [m_ , [-nl:+nl] ]; end;%for nl=0:n_l;
...
l_val = nl;
if (verbose>2); disp(sprintf(' %% nk %d k %d/%d: nl %d l_val %d',nk,k,k_max,nl,l_val)); end;
Llm__=legendre(l_val,cos(phi_sub_),'unnorm');
...
%s=1; % original phase ;
if (length_sub>0); 
Ylm_ = s*c*Llm_.*exp(+i*m_val*transpose(theta_sub_)); 
...
theta_all_(1 + n_sub + (0:length_sub-1)) = theta_sub_;
phi_all_(1 + n_sub + (0:length_sub-1)) = phi_sub_;
...
ij_tmp = (1:res) - floor(res/2) + floor(sample_rate*res/2);
sample_rate = res / k_max; m3d1 = sample_rate*res;
[FA_x_c_nu3d1_,ier] = nufft3d1(numel(FA_rk_c_),pi*K_1_p_(:)/res,pi*K_2_p_(:)/res,pi*K_3_p_(:)/res,FA_rk_c_(:),-1,1e-12,m3d1,m3d1,m3d1);
FA_x_c_nu3d1_ = real(permute(reshape(FA_x_c_nu3d1_,m3d1,m3d1,m3d1),[2,1,3]))*numel(FA_rk_c_); FA_x_c_nu3d1_ = FA_x_c_nu3d1_(ij_tmp,ij_tmp,ij_tmp);
...
[FA_x_c_nu3d3_,ier] = nufft3d3(n_all,kx_all_resampled_,ky_all_resampled_,kz_all_resampled_,MOL_A_K_P_.*weight_all_,-1,1e-12,n_m,M_X_(:),M_Y_(:),M_Z_(:));
FA_x_c_nu3d3_ = real(reshape(FA_x_c_nu3d3_,res,res,res));

if (verbose>-1); disp(sprintf(' %% res %d: numel FA_rk_c_ %d , numel MOL_A_K_P_ %d , FA_x_c_/nu_3d1 %f , nu_3d3/FA_x_c_ %f , nu_3d3/nu_3d1 %f',res,numel(FA_rk_c_),numel(MOL_A_K_P_), mean(FA_x_c_(:))./mean(FA_x_c_nu3d1_(:)) , mean(FA_x_c_nu3d3_(:))./mean(FA_x_c_(:)) , mean(FA_x_c_nu3d3_(:))./mean(FA_x_c_nu3d1_(:)) )); end;
...
subplot(2,2,3);
cra = colormap('spring'); cra = cra(end:-1:1,:);  ncra = size(cra,1);
F_tmp_ = FA_x_c_nu3d1_;
v_avg = mean(F_tmp_(:)); v_std = std(F_tmp_(:)); v_min = min(F_tmp_(:)); v_max = max(F_tmp_(:));
vlim = v_avg + 2.5*v_std*[-1,1];
v_ = linspace(vlim(1),vlim(2),7);
for nv=length(v_):-1:1;
v = v_(nv);
hpatch = patch(isosurface(X_1_,X_2_,X_3_,FA_x_c_nu3d1_,v)); isonormals(X_1_,X_2_,X_3_,FA_x_c_nu3d1_,hpatch);
nc = max(1,min(ncra,floor(ncra*nv/length(v_))));
hpatch.FaceColor = cra(nc,:);
hpatch.EdgeColor = 'none';
hpatch.FaceAlpha = (nv/length(v_)).^4 * 1.0;
...
title(sprintf('FA_x nu3d1, r_max %d',k_max));
% end plot;
end;%if plot_flag;
