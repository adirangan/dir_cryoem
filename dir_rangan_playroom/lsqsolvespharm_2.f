...
c     For PCGN: T S alpha = f -> conjg(S') T' D T S alpha = conjg(S') T' D f
c                      
c     solving neq:   
c
c      call prinf(' calling mkshinterp nout is *',nout,1)
      call mkshinterp(thetas,phis,nout,k,nquad,nquadm,values,icols)
      call getquadwtsirreg(nout,k,nquad,nquadm,icols,
     $                     wts,qwts)
      do i = 1,nout
         qwts(i) = 1.0d0
      enddo
      do i = 1,nout
         phivals(i) = qwts(i)*phiout(i)    
      enddo
