...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
% test_principled_marching_heterogeneous_helper_alternating_minimization_synthetic_7. ;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
...
fname_mat = sprintf('%s_mat/tpmhhamsux_%s_M_k_p___n%.3ds%.4dr%.3d.mat',dir_trunk,syn_infix,syn_n_M,floor(1000*syn_snr),syn_n_UX_rank);
...
%%%%%%%%;
fname_fig = sprintf('%s_mat/dir_jpg/tpmhhamsux_%s_M_k_p___n%.3ds%.4dr%.3d',dir_trunk,syn_infix,syn_n_M,floor(1000*syn_snr),syn_n_UX_rank);
if (~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
clf;
colormap(colormap_beach());
nUX_rank=1-1; clim_ = 1.5*std(real(syn_UX_M_k_p___(:,:,1+nUX_rank)),1,'all')*[-1,+1];
nUX_rank= 1-1; subplot(1,2,1); imagesc(real(squeeze(syn_UX_M_k_p___(:,:,1+nUX_rank))),clim_); xlabel('nimage'); ylabel('gamma'); title(sprintf('real(rank==%d) [%0.2f,%0.2f]',1+nUX_rank,clim_));
nUX_rank= 2-1; subplot(1,2,2); imagesc(real(squeeze(syn_UX_M_k_p___(:,:,1+nUX_rank))),clim_); xlabel('nimage'); ylabel('gamma'); title(sprintf('real(rank==%d) [%0.2f,%0.2f]',1+nUX_rank,clim_));
figbig;
...
%%%%%%%%;
fname_fig = sprintf('%s_mat/dir_jpg/tpmhhamsux_%s_M_k_p___A_n%.3ds%.4dr%.3d',dir_trunk,syn_infix,syn_n_M,floor(1000*syn_snr),syn_n_UX_rank);
if (~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
clf;
...
%%%%%%%%;
for nUX_rank=0:syn_n_UX_rank-1;
MS_fname_pre = sprintf('%s_mat/tpmhhamsux_%s_Y_n%.3ds%.4dr%.3d_allshell_MS_rng%.3dnUX%.3d',dir_trunk,syn_infix,syn_n_M,floor(1000*syn_snr),syn_n_UX_rank,syn_rseed,nUX_rank);
...
%%%%%%%%;
SM_fname_pre = sprintf('%s_mat/tpmhhamsux_%s_Y_n%.3ds%.4dr%.3d_allshell_SM_rng%.3dnUX%.3d',dir_trunk,syn_infix,syn_n_M,floor(1000*syn_snr),syn_n_UX_rank,syn_rseed,nUX_rank);
