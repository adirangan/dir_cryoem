...
svd_VUX0CTFM_lwnM____ = tpmh_VUXM_lwnM____3(FTK,n_k_p_r,n_w_,n_M,CTF_M_k_q_wkM__,pm_n_UX_rank,UX0_kn__,X_weight_r_);
%%%%%%%%;
...
if nd==na; tmp_str_ascii = 'svd_VUXCTFM_lwnM____'; tmp_local = svd_VUX0CTFM_lwnM____; tab_p=1; end; na=na+1;
...
if nd==na; svd_VUX1CTFM_lwnM____ = reshape(tmp_ascii,size(svd_VUX0CTFM_lwnM____)); end; na=na+1;
...
%%;
subplot(p_row,p_col,1+np);np=np+1;cla;
tmp_n_ = squeeze(sum(abs(svd_VUX1CTFM_lwnM____ - svd_VUX0CTFM_lwnM____).^2,1+[0,1,3]));
plot(log10(SX0_n_),log10(tmp_n_),'ko-');
xlabel('log10(SX0_n_)','Interpreter','none');
ylabel('log10(sum(abs(svd_VUX1CTFM_lwnM____ - svd_VUX0CTFM_lwnM____).^2,1+[0,2,3]))','Interpreter','none');
