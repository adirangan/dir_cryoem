...
str_thisfunction = 'imagesc_S_k_p_3d_1';
...
if ~isfield(parameter,'n_contour'); parameter.n_contour = 2; end;
...
vval_ = linspace(min(vlim_),max(vlim_),n_contour); dvval = mean(diff(vval_));
...
[contour_sub__] = contourc(transpose(abs(real(S_k_c__)-val)),0.5*dvval*[1,1]);
[n_item_sub,val_sub_i_,length_sub_i_,k_c_0_sub_il__,k_c_1_sub_il__] = cell_from_contour_0(contour_sub__);

subplot(1,2,1);plot(val_sub_i_,'.'); subplot(1,2,2);plot(length_sub_i_,'.'); error('stopping');

for nitem_sub=0:n_item_sub-1;
tmp_length = length_sub_i_(1+nitem_sub);
tmp_k_c_0_ = k_c_0_sub_il__{1+nitem_sub}; tmp_k_c_0_ = k_p_r_max*(tmp_k_c_0_ - n_k_c/2 - 0.5)*2/n_k_c;
tmp_k_c_1_ = k_c_1_sub_il__{1+nitem_sub}; tmp_k_c_1_ = k_p_r_max*(tmp_k_c_1_ - n_k_c/2 - 0.5)*2/n_k_c;
