      subroutine Fig_gen_ver1a(ncur,nlats_,grid_k_p_,n_S,ld_S,S_k_p_,n_M
...
c$$$         Display template with perturbation but no ctf
     ...
     $        delta_x_sort_(n_S-1 + nm*n_S)
         delta_y = alpha_est_(nalpha_delta_y + nm*n_alpha) +
     $        delta_y_sort_(n_S-1 + nm*n_S)
...
c$$$      Printing figure file for image
     ...
     $     ,trim(fname_fig),trim(fname_jpg)
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[finished Fig_gen_ver1a]'
      end if
