      subroutine test_innerproduct_batch_excerpt_00a(n_gamma_z,gamma_z_
     $     ,fftw_plan_frwd_,fftw_in1_,fftw_out_,fftw_plan_back_last
     $     ,fftw_in1_last_,fftw_out_last_,n_r,grid_p_,n_w_,n_A,S_p_
     $     ,M_p_,Z_q_)
c$$$      %%%%%%%%%%%%%%%%%%%%%%%
c$$$      Calculates : 
c$$$      Z_q_(ngz) = \| M_p_ .* R ( S_p_ ) \|_{L^{2}} , 
c$$$      where:
c$$$      ".*" is a pointwise product, 
c$$$      M_p_ is a CTF-function given on nonuniform polar grid,
...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%
      ...
      complex *16 Z_q
      complex *16 S_p_(0:0),M_p_(0:0),Z_q_(0:0)
     ...
     $     ,n_A,fftw_in1_,fftw_out_,S_q_,S_q_)
      call xc1_c16(n_A,M_p_,M_p_,M_q_)
      call interp_p_to_q_fftw(n_r,fftw_plan_frwd_,n_w_
     $     ,n_A,fftw_in1_,fftw_out_,M_q_,M_q_)
