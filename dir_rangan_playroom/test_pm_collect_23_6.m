...
dir_drgn = sprintf('/%s/rangan/cryodrgn/dir_%s',string_root,dir_nopath_data_star);
if strcmp(dir_nopath_data_star,'precatalytic_spliceosome'); dir_drgn = sprintf('/%s/rangan/cryodrgn/dir_%s',string_root,'ps1'); end;
dir_aspi = sprintf('/%s/rangan/dir_aspire/dir_%s',string_root,dir_nopath_data_star);
...
X_best_ai__ = cell(n_X_2d_xcor_d0_a1t_align,1);
...
X_Memp_best_ai__ = [];
fname_mat = sprintf('%s/test_pm_collect_X_Memp_best_ai__.mat',dir_pm_mat);
if ( flag_recoll | ~exist(fname_mat,'file'));
%%%%%%%%;
disp(sprintf(' %% %s not found, creating',fname_mat));
%%%%;
tmp_X_2d_Memp_d1_a1t_align_ = ls(sprintf('%s/X_2d_Memp_d1_a1t*_align_a_k_Y_.mat',dir_pm_mat));
tmp_index_start_ = strfind(tmp_X_2d_Memp_d1_a1t_align_,sprintf('/%s',string_root))-1;
tmp_index_final_ = strfind(tmp_X_2d_Memp_d1_a1t_align_,'.mat')+4-1;
n_X_2d_Memp_d1_a1t_align = min(numel(tmp_index_start_),numel(tmp_index_final_));
%%%%;
X_Memp_best_ai__ = cell(n_X_2d_Memp_d1_a1t_align,1);
X_Memp_best_a_ = zeros(n_X_2d_Memp_d1_a1t_align,1);
for na=0:n_X_2d_Memp_d1_a1t_align-1;
tmp_fname_align_a_k_Y_mat = tmp_X_2d_Memp_d1_a1t_align_(1+tmp_index_start_(1+na)+0:1+tmp_index_final_(1+na)-1);
if (tmp_fname_align_a_k_Y_mat(1)==sprintf('\n')); tmp_fname_align_a_k_Y_mat = tmp_fname_align_a_k_Y_mat(2:end); end;
tmp_ = load(tmp_fname_align_a_k_Y_mat,'X_best_'); tmp_X_best_ = tmp_.X_best_; clear tmp_;
tmp_X_best = tmp_X_best_(end);
clear tmp_;
%%%%;
X_Memp_best_ai__{1+na} = tmp_X_best_;
X_Memp_best_a_(1+na)=tmp_X_best;
%%%%;
end;%for na=0:n_X_2d_Memp_d1_a1t_align-1;
%%%%;
save(fname_mat ...
,'X_Memp_best_ai__' ...
);
...
X_xcor_best_ai__ = [];
fname_mat = sprintf('%s/test_pm_collect_X_xcor_best_ai__.mat',dir_pm_mat);
if ( flag_recoll | ~exist(fname_mat,'file'));
%%%%%%%%;
disp(sprintf(' %% %s not found, creating',fname_mat));
%%%%;
...
n_X_2d_xcor_d0_a1t_align = min(numel(tmp_index_start_),numel(tmp_index_final_));
%%%%;
X_xcor_best_ai__ = cell(n_X_2d_xcor_d0_a1t_align,1);
X_xcor_best_a_ = zeros(n_X_2d_xcor_d0_a1t_align,1);
for na=0:n_X_2d_xcor_d0_a1t_align-1;
tmp_fname_align_a_k_Y_mat = tmp_X_2d_xcor_d0_a1t_align_(1+tmp_index_start_(1+na)+0:1+tmp_index_final_(1+na)-1);
if (tmp_fname_align_a_k_Y_mat(1)==sprintf('\n')); tmp_fname_align_a_k_Y_mat = tmp_fname_align_a_k_Y_mat(2:end); end;
tmp_ = load(tmp_fname_align_a_k_Y_mat,'X_best_'); tmp_X_best_ = tmp_.X_best_; clear tmp_;
...
clear tmp_;
%%%%;
X_xcor_best_ai__{1+na} = tmp_X_best_;
X_xcor_best_a_(1+na)=tmp_X_best;
%%%%;
end;%for na=0:n_X_2d_xcor_d0_a1t_align-1;
%%%%;
save(fname_mat ...
...
X_aspi_best = []; X_aspi_mask=[];
fname_aspi_mat = sprintf('%s/dir_pm_mat/X_aspi.mat',dir_aspi);
if ( exist(fname_aspi_mat,'file'));
load(fname_aspi_mat);
end;%if ( exist(fname_aspi_mat,'file'));
X_drgn_best = []; X_drgn_mask=[];
fname_drgn_mat = sprintf('%s/dir_pm_mat/X_drgn.mat',dir_drgn);
...
collect__{1+nexperiment}.X_Memp_best_ai__ = X_Memp_best_ai__;
collect__{1+nexperiment}.X_xcor_best_ai__ = X_xcor_best_ai__;
collect__{1+nexperiment}.X_aspi_mask = X_aspi_mask;
collect__{1+nexperiment}.X_drgn_mask = X_drgn_mask;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
...
linewidth_use = 0.25;
tmp_horiz_gap = 0.125;
facecolor_drgn_ = [0.50,0.00,0.00];
facecolor_aspi_ = [0.00,0.50,0.00];
...
% nu_experiment = 5; %<-- precatalytic_spliceosome. ;

cat_nu_experiment_a_ = [];
cat_X_ampm_a_ = [];
cat_X_Memp_ampm_ai__ = [];
cat_X_xcor_ampm_ai__ = [];
cat_tolerance_pm_a_ = [];
cat_delta_sigma_use_a_ = [];
cat_flag_rank_vs_tolerance_a_ = [];
X_ampm_avg_e_ = zeros(n_u_experiment,1);
X_ampm_av2_e_ = zeros(n_u_experiment,1);
X_ampm_var_e_ = zeros(n_u_experiment,1);
X_ampm_std_e_ = zeros(n_u_experiment,1);
X_ampm_st2_e_ = zeros(n_u_experiment,1);
cat_X_ampm_avg_a_ = [];
cat_X_Memp_ampm_avg_a_ = [];
cat_X_xcor_ampm_avg_a_ = [];
cat_X_ampm_var_a_ = [];
cat_X_ampm_std_a_ = [];
cat_X_Memp_ampm_std_a_ = [];
cat_X_xcor_ampm_std_a_ = [];
...
tmp_collect_drgn = -Inf;
tmp_collect_aspi = -Inf;
tmp_collect_reli = -Inf;
tmp_collect_corr_OAM_stab = -Inf;
tmp_collect_corr_OBI_stab = -Inf;
na_ampm_tot=0;
tmp_nu_experiment_a_ = [];
tmp_X_ampm_a_ = [];
tmp_X_Memp_ampm_ai__ = [];
tmp_X_xcor_ampm_ai__ = [];
tmp_tolerance_pm_a_ = [];
...
tmp_nu_experiment_a_(1 + tmp_index_) = nu_experiment;
tmp_X_ampm_a_(1 + tmp_index_) = tmp_X_;
for na=0:n_a-1;
tmp_n_i = numel(collect_.X_Memp_best_ai__{1+na});
tmp_X_Memp_ampm_ai__(1 + na_ampm_tot + na,1:tmp_n_i) = flip(collect_.X_Memp_best_ai__{1+na}); %<-- align the final iteration. ;
end;%for na=0:n_a-1;
for na=0:n_a-1;
tmp_n_i = numel(collect_.X_xcor_best_ai__{1+na});
tmp_X_xcor_ampm_ai__(1 + na_ampm_tot + na,1:tmp_n_i) = flip(collect_.X_xcor_best_ai__{1+na}); %<-- align the final iteration. ;
end;%for na=0:n_a-1;
...
if ~isempty(collect_.X_drgn_mask); tmp_collect_drgn = max(tmp_collect_drgn,collect_.X_drgn_mask); end;
if ~isempty(collect_.X_aspi_mask); tmp_collect_aspi = max(tmp_collect_aspi,collect_.X_aspi_mask); end;
...
disp(sprintf(' %% %% tmp_collect_drgn %0.6f',tmp_collect_drgn));
disp(sprintf(' %% %% tmp_collect_aspi %0.6f',tmp_collect_aspi));
...
%%%%%%%%;
cat_nu_experiment_a_ = [ cat_nu_experiment_a_(:) ; tmp_nu_experiment_a_(:) ];
cat_X_ampm_a_ = [ cat_X_ampm_a_(:) ; tmp_X_ampm_a_(:) ];
cat_X_Memp_ampm_ai__ = [ cat_X_Memp_ampm_ai__ ; tmp_X_Memp_ampm_ai__ ];
cat_X_xcor_ampm_ai__ = [ cat_X_xcor_ampm_ai__ ; tmp_X_xcor_ampm_ai__ ];
cat_tolerance_pm_a_ = [ cat_tolerance_pm_a_(:) ; tmp_tolerance_pm_a_(:) ];
cat_delta_sigma_use_a_ = [ cat_delta_sigma_use_a_(:) ; tmp_delta_sigma_use_a_(:) ];
cat_flag_rank_vs_tolerance_a_ = [ cat_flag_rank_vs_tolerance_a_(:) ; tmp_flag_rank_vs_tolerance_a_(:) ];
%%%%%%%%;
tmp_X_ampm_use_srt_ = sort(tmp_X_ampm_a_(1+tmp_index_ampm_),'ascend');
tmp_X_Memp_ampm_use_srt_ = sort(tmp_X_Memp_ampm_ai__(1+tmp_index_ampm_,1),'ascend');
tmp_X_xcor_ampm_use_srt_ = sort(tmp_X_xcor_ampm_ai__(1+tmp_index_ampm_,1),'ascend');
tmp_X_sher_use_srt_ = sort(tmp_X_sher_a_(1+tmp_index_sher_),'ascend');
%%%%%%%%;
X_ampm_avg_e_(1+nu_experiment) = mean(tmp_X_ampm_use_srt_);
X_Memp_ampm_avg_e_(1+nu_experiment) = mean(tmp_X_Memp_ampm_use_srt_);
X_xcor_ampm_avg_e_(1+nu_experiment) = mean(tmp_X_xcor_ampm_use_srt_);
X_ampm_var_e_(1+nu_experiment) = var(tmp_X_ampm_use_srt_,1);
X_ampm_std_e_(1+nu_experiment) = std(tmp_X_ampm_use_srt_,1);
X_Memp_ampm_std_e_(1+nu_experiment) = std(tmp_X_Memp_ampm_use_srt_,1);
X_xcor_ampm_std_e_(1+nu_experiment) = std(tmp_X_xcor_ampm_use_srt_,1);
cat_X_ampm_avg_a_ = [cat_X_ampm_avg_a_(:) ; X_ampm_avg_e_(1+nu_experiment)*ones(n_a_ampm_tot,1)];
cat_X_Memp_ampm_avg_a_ = [cat_X_Memp_ampm_avg_a_(:) ; X_Memp_ampm_avg_e_(1+nu_experiment)*ones(n_a_ampm_tot,1)];
cat_X_xcor_ampm_avg_a_ = [cat_X_xcor_ampm_avg_a_(:) ; X_xcor_ampm_avg_e_(1+nu_experiment)*ones(n_a_ampm_tot,1)];
cat_X_ampm_var_a_ = [cat_X_ampm_var_a_(:) ; X_ampm_var_e_(1+nu_experiment)*ones(n_a_ampm_tot,1)];
cat_X_ampm_std_a_ = [cat_X_ampm_std_a_(:) ; X_ampm_std_e_(1+nu_experiment)*ones(n_a_ampm_tot,1)];
cat_X_Memp_ampm_std_a_ = [cat_X_Memp_ampm_std_a_(:) ; X_Memp_ampm_std_e_(1+nu_experiment)*ones(n_a_ampm_tot,1)];
cat_X_xcor_ampm_std_a_ = [cat_X_xcor_ampm_std_a_(:) ; X_xcor_ampm_std_e_(1+nu_experiment)*ones(n_a_ampm_tot,1)];
%%%%%%%%;

fname_fig_pre = sprintf('%s/test_pm_collect_%s_FIGK',dir_jpg,u_dir_nopath_data_star);
...
str_xtick_ = {};
%%%%;
tmp_width = 1; tmp_horiz = xtick_pre; tmp_verti = tmp_collect_drgn;
patch(tmp_horiz + tmp_width*[0;0;1;1;0],tmp_verti*[0;1;1;0;0],0,'LineWidth',linewidth_use,'EdgeColor','k','FaceColor',facecolor_drgn_);
str_xtick_{1+nx} = 'DRG';
...
nx=nx+1;nh = nh+1;
%%%%;
tmp_width = 1; tmp_horiz = xtick_pre; tmp_verti = tmp_collect_aspi;
patch(tmp_horiz + tmp_width*[0;0;1;1;0],tmp_verti*[0;1;1;0;0],0,'LineWidth',linewidth_use,'EdgeColor','k','FaceColor',facecolor_aspi_);
str_xtick_{1+nx} = 'ASP';
xtick_min_(1+nx) = xtick_pre;
xtick_max_(1+nx) = xtick_pre + tmp_width;
xtick_pre = xtick_max_(1+nx) + tmp_horiz_gap;
...
fname_fig_jpg_strip = sprintf('%s/test_pm_collect_%s_FIGK_strip.jpg',tmp_dir,u_dir_nopath_data_star);
...
cat_index_ampm_ = efind( ...
 cat_X_ampm_a_~=0 ...
& cat_tolerance_pm_a_<=10^-1.75 ...
& cat_delta_sigma_use_a_> 0.00 ...
& cat_flag_rank_vs_tolerance_a_==flag_rank_vs_tolerance_use ...
);

disp('returning');return;

%%%%%%%%;
% plot sensitivity to iteration number. ;
%%%%%%%%;
cat_Z_ampm_bi__ = bsxfun(@rdivide,bsxfun(@minus,[fliplr(cat_X_Memp_ampm_ai__(1+cat_index_ampm_,:)),fliplr(cat_X_xcor_ampm_ai__(1+cat_index_ampm_,:))],cat_X_xcor_ampm_ai__(1+cat_index_ampm_,1+0)),max(1e-12,cat_X_xcor_ampm_std_a_(1+cat_index_ampm_))); %<-- relative to final iteration. ;
%cat_Z_ampm_bi__ = bsxfun(@rdivide,bsxfun(@minus,[fliplr(cat_X_Memp_ampm_ai__(1+cat_index_ampm_,:)),fliplr(cat_X_xcor_ampm_ai__(1+cat_index_ampm_,:))],cat_X_xcor_ampm_avg_a_(1+cat_index_ampm_)),max(1e-12,cat_X_xcor_ampm_std_a_(1+cat_index_ampm_))); %<-- relative to average final iteration. ;
figure(1+nf);nf=nf+1;clf;figsml;
linewidth_sml = 1;
linewidth_med = 3;
linewidth_lrg = 5;
fontsize_use = 12;
tmp_x_ = 1:64;
tmp_index_ = tmp_x_([2:2:end])-1;
hold on;
plot(tmp_x_(1+tmp_index_),prctile(cat_Z_ampm_bi__(:,1+tmp_index_),50,1),'k-','LineWidth',linewidth_lrg);
plot(tmp_x_(1+tmp_index_),prctile(cat_Z_ampm_bi__(:,1+tmp_index_),15,1),'k-','LineWidth',linewidth_med);
plot(tmp_x_(1+tmp_index_),prctile(cat_Z_ampm_bi__(:,1+tmp_index_), 5,1),'k-','LineWidth',linewidth_sml);
%plot(tmp_x_(1+tmp_index_),prctile(cat_Z_ampm_bi__(:,1+tmp_index_),85,1),'k-','LineWidth',linewidth_med);
%plot(tmp_x_(1+tmp_index_),prctile(cat_Z_ampm_bi__(:,1+tmp_index_),95,1),'k-','LineWidth',linewidth_sml);
plot(tmp_x_,0*tmp_x_,'k:','LineWidth',linewidth_sml);
plot( 2*[1,1],[-10,+1],'k:','LineWidth',linewidth_sml);
plot(32*[1,1],[-10,+1],'k:','LineWidth',linewidth_sml);
plot(33*[1,1],[-10,+1],'k:','LineWidth',linewidth_sml);
plot(64*[1,1],[-10,+1],'k:','LineWidth',linewidth_sml);
xlim([0,65]); set(gca,'XTick',[4:4:64]);
ylim([-10,+0.5]);
xlabel('iteration');
ylabel('Z (normalized)');
legend({'50%','15%',' 5%'},'Location','SouthEast');
title(sprintf('iteration vs Z (normalized)'),'Interpreter','latex');
set(gca,'FontSize',fontsize_use);
%%%%%%%%;
tmp_dir = sprintf('/%s/rangan/dir_cryoem/dir_ampm_manuscript/dir_ampm_fig_collect',string_root);
fname_fig_jpg_strip = sprintf('%s/test_pm_collect_sensitivity_iteration_FIGC_strip.jpg',tmp_dir);
fname_fig_eps_strip = sprintf('%s/test_pm_collect_sensitivity_iteration_FIGC_strip.eps',tmp_dir);
if ( flag_replot | ~exist(fname_fig_jpg_strip,'file') );
disp(sprintf(' %% writing %s',fname_fig_jpg_strip));
print('-djpeg',sprintf('%s',fname_fig_jpg_strip));
print('-depsc',sprintf('%s',fname_fig_eps_strip));
end;%if ( flag_replot | ~exist(fname_fig_jpg_strip,'file') );
%%%%%%%%;

%%%%%%%%;
% Now plot sensitivity to: ;
% tolerance_pm_a_ ;
% delta_sigma_use_a_ ;
%%%%%%%%;
cat_Z_ampm_b_ = (cat_X_ampm_a_(1+cat_index_ampm_)-cat_X_ampm_avg_a_(1+cat_index_ampm_))./max(1e-12,cat_X_ampm_std_a_(1+cat_index_ampm_));
figure(1+nf);nf=nf+1;clf;figmed;
markersize_use = 16;
linewidth_use = 5;
fontsize_use = 12;
%%%%%%%%;
cat_tolerance_pm_b_ = cat_tolerance_pm_a_(1+cat_index_ampm_);
tmp_x_b_ = cat_tolerance_pm_b_;
tmp_x2 = sum(tmp_x_b_.^2);
tmp_x1 = sum(tmp_x_b_.^1);
tmp_x0 = sum(tmp_x_b_.^0);
tmp_xy = sum(tmp_x_b_.*cat_Z_ampm_b_);
tmp_1y = sum(cat_Z_ampm_b_);
tmp_p_ = [tmp_x2 , tmp_x1 ; tmp_x1 , tmp_x0] \ [tmp_xy ; tmp_1y];
tmp_xlim_ = [min(tmp_x_b_),max(tmp_x_b_)];
tmp_Y_ampm_b_ = tmp_p_(1+1) + tmp_p_(1+0)*tmp_x_b_;
subplot(1,2,1);
hold on;
plot(tmp_x_b_,cat_Z_ampm_b_,'k.','MarkerSize',markersize_use);
plot(tmp_xlim_,tmp_p_(1+1) + tmp_p_(1+0)*tmp_xlim_,'r-','LineWidth',linewidth_use);
hold off;
tmp_R2 = 1.0 - sum((cat_Z_ampm_b_ - tmp_Y_ampm_b_).^2) / max(1e-12,sum(cat_Z_ampm_b_.^2));
disp(sprintf(' %% tmp_R2: %0.16f',tmp_R2));
xlabel('$\epsilon$','Interpreter','latex'); xlim(mean(tmp_xlim_) + 0.5*1.125*diff(tmp_xlim_)*[-1,+1]);
set(gca,'XTick',[0.001,0.005,0.010,0.015],'XTickLabel',{'0.001','0.005','0.010','0.015'}); %xtickangle(90);
ylabel('z-score'); ylim([-5,+5]); grid on;
title(sprintf('$\\epsilon$ vs z-score: $R^2$ %0.3f',tmp_R2),'Interpreter','latex');
set(gca,'FontSize',fontsize_use);
%%%%%%%%;
cat_delta_sigma_use_b_ = cat_delta_sigma_use_a_(1+cat_index_ampm_);
tmp_x_b_ = cat_delta_sigma_use_b_;
tmp_x2 = sum(tmp_x_b_.^2);
tmp_x1 = sum(tmp_x_b_.^1);
tmp_x0 = sum(tmp_x_b_.^0);
tmp_xy = sum(tmp_x_b_.*cat_Z_ampm_b_);
...
tmp_R2 = 1.0 - sum((cat_Z_ampm_b_ - tmp_Y_ampm_b_).^2) / max(1e-12,sum(cat_Z_ampm_b_.^2));
disp(sprintf(' %% tmp_R2: %0.16f',tmp_R2));
xlabel('$\delta$','Interpreter','latex'); xlim(mean(tmp_xlim_) + 0.5*1.125*diff(tmp_xlim_)*[-1,+1]);
set(gca,'XTick',[0.01,0.05,0.10,0.15],'XTickLabel',{'0.01','0.05','0.10','0.15'}); %xtickangle(90);
ylabel('z-score'); ylim([-5,+5]); grid on;
title(sprintf('$\\delta_{l}$ vs z-score: $R^2$ %0.3f',tmp_R2),'Interpreter','latex');
set(gca,'FontSize',fontsize_use);
%%%%%%%%;
tmp_dir = sprintf('/%s/rangan/dir_cryoem/dir_ampm_manuscript/dir_ampm_fig_collect',string_root);
fname_fig_jpg_strip = sprintf('%s/test_pm_collect_sensitivity_tolerance_sigma_FIGAB_strip.jpg',tmp_dir);
fname_fig_eps_strip = sprintf('%s/test_pm_collect_sensitivity_tolerance_sigma_FIGAB_strip.eps',tmp_dir);
if ( flag_replot | ~exist(fname_fig_jpg_strip,'file') );
disp(sprintf(' %% writing %s',fname_fig_jpg_strip));
print('-djpeg',sprintf('%s',fname_fig_jpg_strip));
print('-depsc',sprintf('%s',fname_fig_eps_strip));
