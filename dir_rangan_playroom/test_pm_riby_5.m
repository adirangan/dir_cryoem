...
verbose=1;
if (verbose); disp(sprintf(' %% [entering test_pm_riby_5]')); end;
...
%%%%%%%%;
n_R_sub_x_u_pack_ = floor(n_O_x_u_pack_/8);
R_sub_ij_0_ = floor(n_O_x_u_pack_(1+0)*1.0/4) + [1:n_R_sub_x_u_pack_(1+0)];
R_sub_ij_1_ = floor(n_O_x_u_pack_(1+1)*2.5/4) + [1:n_R_sub_x_u_pack_(1+1)];
...
subplot(1,2,1); imagesc(tmp_R_x_u_pack_); axis image; axisnotick; title('R','Interpreter','none');
subplot(1,2,2); imagesc(R_sub_x_u_pack_); axis image; axisnotick; title('R_sub','Interpreter','none');
sgtitle(fname_fig,'Interpreter','none');
disp(sprintf(' %% writing %s',fname_fig));
print('-djpeg',sprintf('%s.jpg',fname_fig));
print('-depsc',sprintf('%s.eps',fname_fig));
...
%%%%%%%%;
fname_mat = sprintf('%s_mat/test_pm_riby_R_ibp_nll_rel_sum_xx__.mat',dir_pm);
if (flag_recalc | ~exist(fname_mat,'file'));
disp(sprintf(' %% %s not found, creating',fname_mat));
%%%%%%%%;
...
fname_fig = sprintf('%s_jpg/test_pm_riby_R_ibp_rel_sum_xx_FIGA',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1+nf);nf=nf+1;clf;figbig;
...
fname_fig = sprintf('%s_jpg/test_pm_riby_R_ibp_rel_sum_xx_FIGB',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1+nf);nf=nf+1;clf;figbig;
p_row = 3;p_col = 4; np=0;
...
verbose=1;
if (verbose); disp(sprintf(' %% [finished test_pm_riby_5]')); end;
