% trying to find a regime where alternating-minimization produces a ;
% concentration of estimated angles. ;
% verdict: could not find one yet (tried small n_M and high noise). ;
...
% define images. ;
%%%%%%%%;
n_M = 1024*/8; %<-- note that, as long as n_M is sufficiently large, the error is determined by \partial_gamma_F_ and the snr. ;
...
hold on;
nc = max(0,min(n_c-1,floor(n_c*niteration/n_iteration)));
plot(sort(gamma_shift_),'Color',c_(1+nc,:));
xlabel('order');ylabel('gamma');
xlim([0,n_M+1]); ylim([0,2*pi]);
hold off;
if (mod(niteration,1)==0); drawnow(); end;
end;%if flag_plot;
