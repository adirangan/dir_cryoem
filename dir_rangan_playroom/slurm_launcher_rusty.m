function slurm_launcher_rusty(command_string,nnodes,tpn,memdecl,walltime);
% launches an instance of matlab set to execute 'command_string';
% nnodes (number of nodes -- integer);
% tpn (tasks-per-node -- integer);
% memdecl (memory request in GB -- integer);
% walltime (runtime in hours -- integer);
...
fprintf(fid_sh,'#SBATCH --partition=ccm\n');
fprintf(fid_sh,'\n');
fprintf(fid_sh,'module load matlab\n');
