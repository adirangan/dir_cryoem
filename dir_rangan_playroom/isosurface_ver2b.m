function isosurface_ver2b(nmax,infix)
figure;

nlow = 4;
%nmax = 22;
n_ = [0,[nlow:2:nmax]];
prows = 3; pcols = ceil(length(n_)/prows);
for nF=1:length(n_);
ncur = n_(nF);

if (ncur==0);
F_x_c = MDA_read_c16('./dir_mda/S_ori_.mda'); F_x_c = real(permute(F_x_c,[2,1,3]));
X = MDA_read_r8('./dir_mda/x_pre_.mda'); X = permute(X,[2,1,3]);
Y = MDA_read_r8('./dir_mda/y_pre_.mda'); Y = permute(Y,[2,1,3]);
Z = MDA_read_r8('./dir_mda/z_pre_.mda'); Z = permute(Z,[2,1,3]);
elseif (ncur>0);
F_x_c = MDA_read_c16(sprintf('./dir_mda/%s%d_.mda',infix,ncur)); F_x_c = real(permute(F_x_c,[2,1,3]));
X = MDA_read_r8('./dir_mda/x_pos_.mda'); X = permute(X,[2,1,3]);
Y = MDA_read_r8('./dir_mda/y_pos_.mda'); Y = permute(Y,[2,1,3]);
Z = MDA_read_r8('./dir_mda/z_pos_.mda'); Z = permute(Z,[2,1,3]);
end;% if;
...
title(sprintf('F_x(x) %s%d',infix,ncur),'interpreter','none');
