...
[n_all,azimu_b_all_,polar_a_all_,weight_all_] = sample_shell_6(1,sample_d,'L',flag_tensor_vs_adap) ;
for nk=1:b_K;
k = k_(nk);
...
Iq = k_w_*Iq_;
Ix = 4*pi*(1/delta^3)*( sin(K_max*delta) - (K_max*delta)*cos(K_max*delta) );
disp(sprintf(' %% flag_tensor_vs_adap %d sample_d = %0.6f n_all %.4d b_K %.2d (Ix-Iq)/Ix = %+0.16f/%+0.16f = %+0.16f',flag_tensor_vs_adap,sample_d,n_all,b_K,Ix-Iq,Ix,(Ix-Iq)/Ix));
...
%%%%;
[n_all,azimu_b_all_,polar_a_all_,weight_all_] = sample_shell_6(r,d,'T',flag_tensor_vs_adap) ;
f_all_ = feval(@f_test,polar_a_all_); g_all_ = feval(@g_test,azimu_b_all_); h_all_ = f_all_.*g_all_;
It = dot(weight_all_,h_all_);
[n_all,azimu_b_all_,polar_a_all_,weight_all_] = sample_shell_6(r,d,'C',flag_tensor_vs_adap) ;
f_all_ = feval(@f_test,polar_a_all_); g_all_ = feval(@g_test,azimu_b_all_); h_all_ = f_all_.*g_all_;
Ic = dot(weight_all_,h_all_);
[n_all,azimu_b_all_,polar_a_all_,weight_all_] = sample_shell_6(r,d,'L',flag_tensor_vs_adap) ;
f_all_ = feval(@f_test,polar_a_all_); g_all_ = feval(@g_test,azimu_b_all_); h_all_ = f_all_.*g_all_;
...
disp(sprintf(' %% flag_tensor_vs_adap %d n_all %d Ix %0.3f It %0.3f error %0.16f (%0.2f) Ic %0.3f error %0.16f (%0.2f) Il %0.3f error %0.16f (%0.2f)',flag_tensor_vs_adap,n_all,Ix,It,et,let,Ic,ec,lec,Il,el,lel));
...
if (nargin<4); flag_tensor_vs_adap = []; end;
...
if flag_tensor_vs_adap==0; n_azimu_b = 3+round(2*pi*spolar_a*r/max(1e-12,d)); end;
if flag_tensor_vs_adap==1; n_azimu_b = n_azimu_b_max; end;
