function nf = test_CryoLike_k_eq_d_20241002(k_eq_d_double,k_int,n_x_u,nf);
%%%%%%%%;
...
%     Doing so will narrow the support of the spatial-gaussian, increasing the resolution at the cost of integration-accuracy.
...
str_thisfunction = 'test_CryoLike_k_eq_d_20241002';
...
nf = test_CryoLike_k_eq_d_20241002(k_eq_d_double,[],[],nf);
...
if (flag_verbose>0); disp(sprintf(' %% setting k_int = %0.6f ',k_int)); end;
if (flag_verbose>0); disp(sprintf(' %% setting n_x_u = %0.6f ',n_x_u)); end;
...
%%%%%%%%;
if (flag_verbose>0); disp(sprintf(' %% %% %% %% %% %% %% %% ')); end;
if (flag_verbose>0); disp(sprintf(' %% generating random volume:')); end;
n_source = 128;
...
parameter.vval_ = prctile(abs(j_x_u_form_),99.75,'all');
%%%%;
subplot(p_row,p_col,1+np);np=np+1; hold on;
