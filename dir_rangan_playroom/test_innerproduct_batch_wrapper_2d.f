      subroutine test_innerproduct_batch_wrapper_2d(verbose,ncur,nlats_
     $     ,grid_k_p_,max_x_c,n_S,ld_S,S_k_p_,distance_req
     $     ,S_alpha_polar_a_ ,S_alpha_azimu_b_,n_M,ld_M,M_k_p_,n_CTF
     ...
     $     ,C_M_,C_S_max_ ,C_Z_max_,S_used_all)
...
c$$$      parameters for timing
      ...
      if (verbose.gt.0) then
          write(6,'(A)')
     $        '[entering test_innerproduct_batch_wrapper_2d]: '
       end if
...
c$$$      calculating all innerproducts
     ...
     $        -timing_tic)/max(1,S_used_all),'; time_per S,M,dx,dy: '
     $        ,(timing_toc -timing_tic)/(S_used_all*n_delta_x*n_delta_y)
      end if
      if (verbose.gt.1) then
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[finished test_innerproduct_batch_wrapper_2d]'
      end if
