...
%title(sprintf('AMPM %.3f (Z %0.2f)',tmp_percent_threshold,corr_full_reco_vs_crop_ampm_x_(1+ncrop)));
title(sprintf('AMPM (Z %0.2f)',corr_full_reco_vs_crop_ampm_x_(1+ncrop)));
...
%title(sprintf('AMPM (ring) %.3f (Z %0.2f)',tmp_percent_threshold,corr_full_reco_vs_crop_ampm_ring_x_(1+ncrop)));
title(sprintf('AMPM (ring) (Z %0.2f)',corr_full_reco_vs_crop_ampm_ring_x_(1+ncrop)));
xlabel('');ylabel('');zlabel(''); set(gca,'FontSize',fontsize_use);
%%%%;
...
sgtitle('');
tmp_dir = sprintf('/%s/rangan/dir_cryoem/dir_ampm_manuscript/dir_ampm_fig_ring_vol',string_root);
tmp_str = fname_ring_vol_fig_pre; tmp_str = tmp_str(strfind(tmp_str,'X_2d'):end);
fname_fig_jpg_strip = sprintf('%s/LetB1_%s_strip.jpg',tmp_dir,tmp_str);
