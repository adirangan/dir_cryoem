c     Compile with: gfortran -fcray-pointer -w -O2 -o test_innerproduct_svdread_batch_ver1_dr.out test_innerproduct_svdread_batch_ver1_dr.f -lfftw3 ; 
c     Run with: ./test_innerproduct_svdread_batch_ver1_dr.out 1 48 0.1 3.0 17 17 48 -1 ; 
c$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
      program test_innerproduct_svdread_batch_ver1_dr
c$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
c$$$      This program is a driver which tests the subroutine
c$$$      test_innerproduct_svdread_batch_ver1_dr.
...
c$$$         (see test_innerproduct_svdread_batch_ver1_pos.jpg).
...
c$$$      range of displacement and rotation parameters to measure
      ...
      real *8 delta_x_sort,delta_y_sort,delta_x_tmp,delta_y_tmp
...
c$$$      function for template generation
      external get_F3_x_c
      real *8 param_1,param_2,param_3
...
c$$$      list: verbose ncur eps_target N_pixels_in n_delta_x n_delta_y n_gamma_z svd_calculation_type
      ...
      if (verbose.gt.0) then
          write(6,'(A)')
     $        '[entering test_innerproduct_svdread_batch_ver1_dr]: '
       end if
...
c$$$      generate templates
     ...
     $        ,' templates in total,'
         write(6,*) 'each using the function get_F3_x_c'
...
c$$$      Calculating x-space template on regular cartesian-grid
         call get_F3_x_c_(n_r,grid_x_c_,max_x_c,n_r ,grid_x_c_,max_x_c
     $        ,S_x_c_(ns*n_r*n_r),get_F3_x_c,param_1,param_2,param_3)
...
c$$$      Calculating x-space image on regular cartesian-grid
      ...
      do nm=0,n_M-1
         ns = nm/3
         delta_x_tru_(nm) = 4.0*(mod(nm,3)-1)/n_r*max_x_c
         delta_y_tru_(nm) = 6.0*(mod(nm,3)-1)/n_r*max_x_c
...
c$$$      do nm=0,n_M-1
c$$$         delta_x_est_(nm) = delta_x_tru_(nm)
c$$$         delta_y_est_(nm) = delta_y_tru_(nm)
c$$$         gamma_z_est_(nm) = gamma_z_tru_(nm)
c$$$      enddo
      if (verbose.gt.1) then
         write(6,'(A)') ''
...
c$$$      calculating all innerproducts
      ...
      timing_tic = second()
      call test_innerproduct_svdread_batch_ver1(verbose
     ...
     $        ,' and rotation parameters.'
      ...
      do nm=0,n_M-1
         delta_x_sort = delta_x_sort_(n_S-1+nm*n_S)
         delta_y_sort = delta_y_sort_(n_S-1+nm*n_S)
...
c$$$         delta_x_upd_(nm) = delta_x_est_(nm) + delta_x_sort
c$$$         delta_y_upd_(nm) = delta_y_est_(nm) + delta_y_sort
     ...
     $    'gamma_z_err_:', max_r8_f(n_M,gamma_z_err_)
      ...
      unitnumber = 7
      write(fname_pre,'(A)') 'test_innerproduct_svdread_batch_ver1_pre'
...
c$$$     $        ,y_side)
     ...
     $        ,'test_innerproduct_svdread_batch_ver1_pre.jpg"'
      end if
...
c$$$      Initializing figure file for image
      unitnumber = 7
      write(fname_pre,'(A)') 'test_innerproduct_svdread_batch_ver1_pos'
      write(fname_fig,'(A,A)') trim(fname_pre),'.fig'
      write(fname_jpg,'(A,A)') trim(fname_pre),'.jpg'
      open(unitnumber,file=fname_fig,status='replace' ,form='formatted')
      call Fig_header(unitnumber);
...
c$$$      printing templates and images
     ...
     $        ,y_side)
      ...
      if (verbose.gt.1) then
         ...
         write(6,'(A)') 'We write the figure file to disk.'
         write(6,'(A,A)') 'try: '
     $        ,'"gthumb test_innerproduct_svdread_batch_ver1_pos.jpg"'
      end if
c$$$      Printing figure file for image
      close(unitnumber,status='keep')
      write(fig2dev_system_call,'(A,A,1X,A)') 'fig2dev -Ljpeg -q 10 '
 ...
 10   continue ! if plot_flag.eqv..false.
      if (verbose.gt.0) then
         write(6,'(A)')
     $        '[finished test_innerproduct_svdread_batch_ver1_dr]'
      end if
      ...
      include 'get_F3_x_c.f'
      include 'get_F3_x_c_.f'
      ...
      include 'get_interchange_delta.f'
      include 'test_innerproduct_svdread_batch_ver1.f'
