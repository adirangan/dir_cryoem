function tensor_to_scatter__ = cg_interpolate_n_1(n_order,n_polar_a,n_azimu_b,n_scatter,polar_a_scatter_,azimu_b_scatter_);
...
for n_order=[3,5,7];
tensor_to_scatter__ = cg_interpolate_n_1(n_order,n_polar_a,n_azimu_b,n_scat,polar_a_scat_,azimu_b_scat_);
...
index_col_polar_a__(:,1+nscatter) = polar_a_scatter_floor_(1+nscatter) + transpose([0:n_order-1]);
...
index_col_azimu_b__(:,1+nscatter) = periodize(azimu_b_scatter_floor_(1+nscatter) + transpose([0:n_order-1]),0,n_azimu_b); %<-- loop boundary here. ;
...
index_col__(:,1+nscatter) = reshape( repmat(index_col_polar_a__(:,1+nscatter),1,n_order) + repmat(transpose(index_col_azimu_b__(:,1+nscatter)),n_order,1)*n_polar_a , [n_order^2,1]);
end;%for nscatter=0:n_scatter-1;
