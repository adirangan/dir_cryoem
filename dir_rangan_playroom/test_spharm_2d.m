function test_spharm_2d(n_l,a_,res_);
% when no inputs are passed we import a spherical harmonic representation (generated by kspacegrid_to_model), ;
% then pick a shell (e.g., ncur = 10), and display: ;
% Fig 1: the original shell ;
% Fig 2: A rotated version constructed by brute-force ;
% Fig 3: A rotated version constructed by wigner-d matrix ;
% ;
% Note that the 'condon-shortley phase factor' is accounted for within both wignerd_b.m and wignerd_lsq_b.m. ;
% ;
% ;
% n_l = highest order of expansion ;
...
%{
  test_spharm_2d();
  % or something like: ;
  n_l =  2; a_ = [+0,+0,+0,+0,+0,+0,+1,+0,+0]; res_ = [96,96]; test_spharm_2d(n_l,a_,res_);
  n_l =  1; a_ = [+0,+0,+0,+1]; res_ = [96,96]; test_spharm_2d(n_l,a_,res_);
  %}
...
%alpha = +3*pi/4; beta = -2.5*pi/6; gamma = -5*pi/3;
%alpha = 0*pi/3; beta = 1*pi/4; gamma = 0*pi/6;
alpha = 0*pi/3; beta = 0*pi/8; gamma = 1*pi/6;
%W_ = wignerd_lsq_b(n_l,beta);
W_ = wignerd_b(n_l,beta);
b_ = zeros(size(a_));
for nl=0:n_l;
