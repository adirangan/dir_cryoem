...
kernel_l_max_use = 49; kernel_l_max_ext = ceil(1.25*49);
parameter.kernel_l_max_use = kernel_l_max_use;
parameter.kernel_l_max_ext = kernel_l_max_ext;
...
[tmp_ij_north_] = knnsearch(qref_k_c_qc__,reshape(+tmp_I_pole_,[1,3]),'K',n_nearest_north);
[tmp_ij_south_] = knnsearch(qref_k_c_qc__,reshape(-tmp_I_pole_,[1,3]),'K',n_nearest_south);
...
[tmp_ij_north_] = knnsearch(qref_k_c_qc__,reshape(+tmp_R_pole_,[1,3]),'K',n_nearest_north);
[tmp_ij_south_] = knnsearch(qref_k_c_qc__,reshape(-tmp_R_pole_,[1,3]),'K',n_nearest_south);
tmp_index_keep_ = union(tmp_ij_north_,tmp_ij_south_)-1; tmp_index_drop_ = setdiff(0:qref_n_shell-1,tmp_index_keep_);
...
%%%%%%%%;
[tmp_ij_north_wMn__,tmp_d1_north_wMn__] = knnsearch(qref_k_c_qc__,+data_k_c_wMd__,'K',n_nearest_north); %<-- obtain tmp_d1_south_wMn__. ;
...
%qref_k_eq_d = 1.00*sqrt(4*pi./max(1,n_lm_ext)); %<-- older setting from qbp_6. consider changing. ;
 qref_k_eq_d = 0.50*sqrt(4*pi./max(1,n_lm_ext)); %<-- one half the older setting from qbp_6. increased density of quadrature points. ;
%qref_k_eq_d = 0.25*sqrt(4*pi./max(1,n_lm_ext)); %<-- one quarter the older setting from qbp_6. increased density of quadrature points. ;
...
tmp_t = tic();
Ylm_ext_yq__ = zeros(n_lm_ext,qref_n_shell);
...
Ylm_ext_yq__(1+nml,:) = Ylm_ext__{1+l_val_ext}(1+l_val_ext+m_val_ext,:);
nml=nml+1;
end;%for m_val_ext=-l_val_ext:+l_val_ext;
end;%for l_val_ext=0:l_max_ext;
Ylm_ext_weight_yq__ = Ylm_ext_yq__ * sparse(1:qref_n_shell,1:qref_n_shell,qref_weight_shell_,qref_n_shell,qref_n_shell);
...
KAPPA.Ylm_ext_weight_yq__ = Ylm_ext_weight_yq__;
...
KAPPA.Ylm_use_weight_yq__ = KAPPA.Ylm_ext_weight_yq__(1:n_lm_use,:);
