...
% Built off of test_pm_trpv1c_9b.m, aiming for 10 angstrom resolution. ;
% Starting with k_p_r_max = 64/(2*pi) and n_M = 1024. ;
% Verdict: fsc --> 30A? ;
% Next trying with k_p_r_max = 64/(2*pi) and n_M = 1024*4. ;
% Verdict: Now maybe 15A? ;
% Next trying with n_x_u_pack = 128, k_p_r_max = 96/(2*pi), and n_M = 1024*8. ;
% Verdict: Looks like 10A, or even finer (since fsc hits 0.5 at k_p_r_max). ;
% Note that this calculation does not take into account any mask. ;
% Thus, the masked fsc could be higher. ;
clear;
...
dir_pm = sprintf('/%s/rangan/dir_cryoem/dir_trpv1f/dir_pm',string_root);
...
dir_relion = sprintf('/%s/rangan/dir_cryoem/dir_trpv1f/dir_relion',string_root);
...
disp(sprintf(' %% %s not found, creating',fname_mat));
tmp_t = tic;
k_p_r_max = 96/(2*pi); k_eq_d = 1.0/(2*pi);
[ ...
 n_k_all ...
,n_k_all_csum_ ...
,k_p_r_all_ ...
...
%%%%%%%%
fname_fig = sprintf('%s_jpg/fsc_a_vs_c_k_',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
%%%%%%%%;
%%%%%%%%;
...
clf; figmed;
subplot(1,2,1);
v_p_r_ = (2*k_p_r_)/(n_x_M_u * Pixel_Spacing);
v_p_r_max = (2*k_p_r_max)/(n_x_M_u * Pixel_Spacing);
hold on;
plot(v_p_r_,real(fsc_r0_k_),'r-',v_p_r_,0.5*ones(size(k_p_r_)),'k:','LineWidth',3);
xlim([0,v_p_r_max]);
ylim([0,1]);
xlabel('k'); ylabel('fsc');
hold off;
subplot(1,2,2);
l_p_r_ = 1./max(1e-12,v_p_r_);
hold on;
plot(log10(l_p_r_),real(fsc_r0_k_),'r-',log10(l_p_r_),0.5*ones(size(k_p_r_)),'k:','LineWidth',3);
xlim([0,+2]);
ylim([0,1]);
xlabel('log10(l = 1/k)'); ylabel('fsc');
hold off;
index_fsc_r0 = min( efind( (fsc_r0_k_(1:end-1)>=0.5) & (fsc_r0_k_(2:end)< 0.5) ) );
sgtitle(sprintf('c_x_u_: corr %0.4f ; v_fsc %0.4f l_fsc %0.4f',X_best_reco,v_p_r_(1+index_fsc_r0),l_p_r_(1+index_fsc_r0)),'Interpreter','none');
disp(sprintf(' %% writing %s',fname_fig));
print('-djpeg',sprintf('%s.jpg',fname_fig));
print('-depsc',sprintf('%s.eps',fname_fig));
close(gcf);
...
flag_compute = 0 & ( strcmp(platform,'access1') | strcmp(platform,'rusty') );
if flag_compute;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
