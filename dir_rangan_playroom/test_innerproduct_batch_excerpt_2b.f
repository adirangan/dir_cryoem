      subroutine test_innerproduct_batch_excerpt_2b(n_delta_x,delta_x_
     $     ,n_delta_y,delta_y_,n_gamma_z,gamma_z_,fftw_plan_frwd_
     $     ,fftw_in1_,fftw_out_,fftw_plan_back_last ,fftw_in1_last_
     $     ,fftw_out_last_,n_r,grid_p_,n_w_,n_A,C_M,C_S_,S_p_,M_q_,Z_q_)
      implicit none
      ...
      complex *16 C_M,C_S_(0:0),Z_q,C_Z_use
     ...
     $              ,Z_q)
               ...
               if (zabs(C_M*C_S_(ngz)).le.1.0d-15) then
                  C_Z_use = 1.0d0
               else
                  C_Z_use = C_M*C_S_(ngz)
               end if
