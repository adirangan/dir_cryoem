...
/* The helper function */
/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
void transpose_ps_block1_omp_helper
(
 float *A
 ,float *B
 ,const int n_row
 ...
 )
{
  ...
  for(nc=ncol_start; nc<=ncol_final; nc+=block_size) {
    for(nr=nrow_start; nr<=nrow_final; nr+=block_size) {
      ...
      for(nc2=nc; nc2<max_nc2; nc2+=4) {
	for(nr2=nr; nr2<max_nr2; nr2+=4) {
	  transpose_ps_block0(&A[nc2*lda +nr2], &B[nr2*ldb + nc2], lda, ldb);
	  /* for(nr2=nr; nr2<max_nr2; nr2+=4) { } */}
	/* for(nc2=nc; nc2<max_nc2; nc2+=4) { } */}
      /* for(nr=nrow_start; nr<=nrow_final; nr+=block_size) { } */}
    /* for(nc=ncol_start; nc<=ncol_final; nc+=block_size) { } */}
...
)
{
  ...
  if (flag_omp==0){
	...
	transpose_ps_block1_omp_helper
	  (
	   A
	   ,B
	   ,n_row
