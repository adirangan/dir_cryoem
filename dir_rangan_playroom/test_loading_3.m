%%%%%%%%;
% testing loading calculation after using pm_template_2. ;
%%%%%%%%;
clear;
...
str_thisfunction = 'test_loading_3';
...
if (verbose); disp(sprintf(' %% [%s]',str_thisfunction)); end;
%%%%%%%%;
...
%%%%%%%%;
k_p_r_max = 48/(2*pi); k_eq_d = 1.0/max(1e-12,k_p_r_max); TorL = 'L'; flag_unif_vs_adap = 1;
%%%%%%%%;
tmp_t = tic();
[ ...
 n_k_all ...
...
tmp_prct_ = 90; %tmp_prct_ = [90,95,99];
for nmolecule=0:n_molecule-1;
subplot(1,n_molecule,1+nmolecule);
isosurface_f_x_u_0(reshape(a_x_u_reco_xm__(:,1+nmolecule),x_u_res,x_u_res,x_u_res),tmp_prct_);
...
tmp_t = toc(tmp_t); disp(sprintf(' %% a_k_Y_form_ym__: time %0.2fs',tmp_t));
%%%%%%%%;
...
%%%%%%%%;
% restructure a_k_Y_form_ym__. ;
%%%%%%%%;
a_k_Y_form_ykm___ = zeros(n_lm_max,n_k_p_r,n_molecule);
for nmolecule=0:n_molecule-1;
for nk_p_r=0:n_k_p_r-1;
tmp_index_ = n_lm_csum_(1+nk_p_r) + (0:n_lm_(1+nk_p_r)-1);
a_k_Y_form_ykm___(1:n_lm_(1+nk_p_r),1+nk_p_r,1+nmolecule) = a_k_Y_form_ym__(1+tmp_index_,1+nmolecule);
end;%for nk_p_r=0:n_k_p_r-1;
end;%for nmolecule=0:n_molecule-1;
a_k_Y_form_avg_yk__ = mean(a_k_Y_form_ykm___,3);
%%%%%%%%;
...
%%%%%%%%;
% compare get_template_1 (above) with pm_template_2 (below). ;
%%%%%%%%;
...
,tmp_viewing_weight_all_ ...
,~ ...
,~ ...
,~ ...
,tmp_n_viewing_polar_a ...
,tmp_viewing_polar_a_ ...
,tmp_n_viewing_azimu_b_ ...
] = ...
sample_shell_5( ...
 k_p_r_max ...
,viewing_k_eq_d ...
...
tmp_n_viewing_azimu_b_sum = sum(tmp_n_viewing_azimu_b_);
tmp_n_viewing_azimu_b_csum_ = cumsum([0;tmp_n_viewing_azimu_b_]);
if (verbose); disp(sprintf(' %% tmp_n_viewing_all %d tmp_n_viewing_polar_a %d tmp_n_viewing_azimu_b_sum %d',tmp_n_viewing_all,tmp_n_viewing_polar_a,tmp_n_viewing_azimu_b_sum)); end;
[ ...
 tmp_n_w_ ...
,tmp_weight_2d_k_p_r_ ...
,tmp_weight_2d_k_all_ ...
] = ...
get_weight_2d_1( ...
 0*verbose ...
,n_k_p_r ...
...
if (verbose); disp(sprintf(' %% tmp_n_w_max %d tmp_n_w_sum %d',tmp_n_w_max,tmp_n_w_sum)); end;
clear tmp_S_k_p_wkSm___;
for nmolecule=0:n_molecule-1;
tmp_t = tic();
[ ...
 tmp_S_k_p_wkSm___(:,:,:,1+nmolecule) ...
,~ ...
,tmp_n_S ...
,~ ...
,~ ...
...
,l_max_max ...
,n_k_p_r ...
,reshape(a_k_Y_form_ykm___(:,:,1+nmolecule),[n_lm_max,n_k_p_r]) ...
,viewing_k_eq_d/k_p_r_max ...
,template_k_eq_d ...
,n_w_max ...
);
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% pm_template_2: %0.2fs',tmp_t)); end;
if (verbose); disp(sprintf(' %% tmp_n_S %d',tmp_n_S)); end;
end;%for nmolecule=0:n_molecule-1;
tmp_S_k_p_wkSm___ = reshape(tmp_S_k_p_wkSm___,[n_w_max*n_k_p_r,tmp_n_S,n_molecule]);
if (verbose); disp(sprintf(' %% S_k_p_wnSm___ vs tmp_S_k_p_wkSm___: %0.16f',fnorm(S_k_p_wnSm___ - tmp_S_k_p_wkSm___)/fnorm(S_k_p_wnSm___))); end;
%%%%%%%%;
flag_plot=1;
if flag_plot;
...
%%%%%%%%;
a_CTF_avg_UX_Y_form_avg_yn__ = zeros(n_lm_max,n_UX_rank);
for nUX_rank=0:n_UX_rank-1;
for nk_p_r=0:n_k_p_r-1;
tmp_l_max = l_max_(1+nk_p_r);
tmp_n_lm = (tmp_l_max+1).^2;
tmp_index_ = n_lm_csum_(1+nk_p_r) + (0:tmp_n_lm-1);
a_CTF_avg_UX_Y_form_avg_yn__(1:tmp_n_lm,1+nUX_rank) = a_CTF_avg_UX_Y_form_avg_yn__(1:tmp_n_lm,1+nUX_rank) + UX__(1+nk_p_r,1+nUX_rank)*X_weight_r_(1+nk_p_r)*a_k_Y_form_avg_y_(1+tmp_index_)*CTF_avg_k_p_r_(1+nk_p_r); %<-- use average CTF here, under the assumption that a_CTF_avg_UX_Y_form_ will be used alone. ;
end;%for nk_p_r=0:n_k_p_r-1;
end;%for nUX_rank=0:n_UX_rank-1;
%%%%%%%%;
% Calculate a few principal-templates for visualization using get_template_1. ;
%%%%%%%%;
tmp_t = tic();
clear UX_CTF_avg_S_k_p_avg_wnS__;
[ ...
 UX_CTF_avg_S_k_p_avg_wnS__ ...
,~ ...
,~ ...
,~ ...
,tmp_n_S ...
...
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% get_template_1: %0.2fs',tmp_t)); end;
%%%%%%%%;
% compare against pm_template_2. ;
%%%%%%%%;
tmp_t = tic();
clear tmp_UX_CTF_avg_S_k_p_avg_wnS__;
[ ...
 tmp_UX_CTF_avg_S_k_p_avg_wnS__ ...
,~ ...
,tmp_n_S ...
,~ ...
,~ ...
...
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% pm_template_2: %0.2fs',tmp_t)); end;
tmp_UX_CTF_avg_S_k_p_avg_wnS__ = reshape(tmp_UX_CTF_avg_S_k_p_avg_wnS__,[n_w_max*n_UX_rank,tmp_n_S]);
if (verbose); disp(sprintf(' %% UX_CTF_avg_S_k_p_avg_wnS__ vs tmp_UX_CTF_avg_S_k_p_avg_wnS__: %0.16f',fnorm(UX_CTF_avg_S_k_p_avg_wnS__ - tmp_UX_CTF_avg_S_k_p_avg_wnS__)/fnorm(UX_CTF_avg_S_k_p_avg_wnS__))); end;
%%%%%%%%;
UX_CTF_avg_S_k_q_avg_wnS__ = zeros(n_w_max*n_UX_rank,tmp_n_S);
for nS=0:tmp_n_S-1;
UX_CTF_avg_S_k_q_avg_wnS__(:,1+nS) = ...
interp_p_to_q( ...
 n_UX_rank ...
,n_w_max*ones(n_UX_rank,1) ...
,n_w_max*n_UX_rank ...
,UX_CTF_avg_S_k_p_avg_wnS__(:,1+nS) ...
);
...
rng(0);
euler_polar_a_M_ = periodize(+euler_polar_a_true_ + 0.125*2*pi*rand(n_M,1),0,1*pi);
euler_azimu_b_M_ = periodize(+euler_azimu_b_true_ + 0.250*2*pi*rand(n_M,1),0,2*pi);
euler_gamma_z_M_ = periodize(+euler_gamma_z_true_ + 0.250*2*pi*rand(n_M,1),0,2*pi);
image_delta_x_acc_M_ = +image_delta_x_true_ + 0.125*delta_sigma*(2*rand(n_M,1)-1);
image_delta_y_acc_M_ = +image_delta_y_true_ + 0.125*delta_sigma*(2*rand(n_M,1)-1);
...
n_CTF = n_ctf;
n_cluster = n_CTF;
index_ncluster_from_nCTF_ = transpose([0:n_cluster-1]);
index_nCTF_from_nM_ = CTF_index_;
CTF_k_p_r_kC__ = CTF_k_p_r__;
pm_n_UX_rank_c_ = pm_n_UX_rank*ones(n_cluster,1);
UX_knc___ = repmat(UX__,[1,1,n_cluster]);
X_weight_rc__ = repmat(X_weight_r_,[1,n_cluster]);
tmp_t = tic;
[ ...
 parameter ...
,flag_image_delta_upd_M_ ...
...
%%%%%%%%;
% Now compare a_k_Y_reco_yki__ with a_k_Y_form_avg_y_. ;
%%%%%%%%;
N_wavelength = 0;
[ ...
 X_best ...
...
,l_max_ ...
,a_k_Y_form_avg_y_ ...
,a_k_Y_reco_yki__(:,end) ...
);
if (verbose); disp(sprintf(' %% X_best %0.2f',X_best)); end;
%%%%%%%%;
end;%if flag_check;
%%%%%%%%;
...
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% pm_template_2: %0.2fs',tmp_t)); end;
tmp_S_k_p_avg_wnS__ = reshape(tmp_S_k_p_avg_wnS__,[n_w_max*n_k_p_r,tmp_n_S]);
if (verbose); disp(sprintf(' %% S_k_p_avg_wnS__ vs tmp_S_k_p_avg_wnS__: %0.16f',fnorm(S_k_p_avg_wnS__ - tmp_S_k_p_avg_wnS__)/fnorm(S_k_p_avg_wnS__))); end;
%%%%;
tmp_error = 0;
for nM=0:n_M-1;
nS = nM;
...
%%%%%%%%;
% Visualize differences between full and principal-loadings. ;
%%%%%%%%;
...
parameter.flag_SV_lsq_vs_dot = 0;
[ ...
 parameter ...
,SV_0a_Ml__ ...
] = ...
...
tmp_t = toc(tmp_t); if (verbose>-1); disp(sprintf(' %% get_loading_qbp_2 flag_loading_svd_vs_iteration = 0a: %0.2fs',tmp_t)); end;
%%%%;
tmp_t = tic();
parameter.n_loading = n_loading;
parameter.n_loading_iteration = n_loading_iteration;
...
tmp_t = toc(tmp_t); if (verbose>-1); disp(sprintf(' %% get_loading_qbp_2 flag_loading_svd_vs_iteration = 0b: %0.2fs',tmp_t)); end;
%%%%;
tmp_t = tic();
parameter.n_loading = n_loading;
parameter.n_loading_iteration = n_loading_iteration;
...
tmp_t = toc(tmp_t); if (verbose>-1); disp(sprintf(' %% get_loading_qbp_2 flag_loading_svd_vs_iteration = 0c: %0.2fs',tmp_t)); end;
%%%%;
tmp_t = tic();
parameter.n_loading = n_loading;
...
tmp_t = toc(tmp_t); if (verbose>-1); disp(sprintf(' %% get_loading_qbp_2 flag_loading_svd_vs_iteration = 1: %0.2fs',tmp_t)); end;
%%%%;
parameter.n_loading = n_loading;
parameter.flag_loading_svd_vs_iterate = -1;
[ ...
...
%%%%;
parameter.n_loading = n_loading;
parameter.flag_loading_svd_vs_iterate = 2;
[ ...
 parameter ...
,SV_2_Ml__ ...
] = ...
...
%%%%;
subplot(p_row,p_col,1+np);np=np+1;
scatter3(SV_0a_Ml__(:,1),SV_0a_Ml__(:,2),SV_0a_Ml__(:,3),8,mod(0:n_M-1,3),'filled');axis vis3d; title('0a');
%%%%;
subplot(p_row,p_col,1+np);np=np+1;
scatter3(SV_0b_Ml__(:,1),SV_0b_Ml__(:,2),SV_0b_Ml__(:,3),8,mod(0:n_M-1,3),'filled');axis vis3d; title('0b');
%%%%;
subplot(p_row,p_col,1+np);np=np+1;
scatter3(SV_0c_Ml__(:,1),SV_0c_Ml__(:,2),SV_0c_Ml__(:,3),8,mod(0:n_M-1,3),'filled');axis vis3d; title('0c');
%%%%;
subplot(p_row,p_col,1+np);np=np+1;
scatter3(SV_1_Ml__(:,1),SV_1_Ml__(:,2),SV_1_Ml__(:,3),8,mod(0:n_M-1,3),'filled');axis vis3d; title('1');
%%%%;
subplot(p_row,p_col,1+np);np=np+1;
plot(SV_n_Ml__(:,1),mod(0:n_M-1,3),'ko'); title('n');
%%%%;
subplot(p_row,p_col,1+np);np=np+1;
scatter3(SV_2_Ml__(:,1),SV_2_Ml__(:,2),SV_2_Ml__(:,3),8,mod(0:n_M-1,3),'filled');axis vis3d; title('2');
%%%%;
