...
dir_pm = sprintf('/%s/rangan/dir_cryoem/dir_ps1/dir_pm',string_root);
...
dir_relion = sprintf('/%s/rangan/dir_cryoem/dir_ps1/dir_relion',string_root);
...
dir_data_star = sprintf('/%s/rangan/dir_cryoem/dir_precatalytic_spliceosome',string_root);
Pixel_Spacing = 1.699; %<-- in angstroms, from https://www.ebi.ac.uk/pdbe/emdb/empiar/entry/10180/ ;
...
N_k_p_nrm__(:,1+nM) = N_k_p_/sqrt(tmp_l2);
...
%%%%%%%%;
fname_fig = sprintf('%s_jpg/loading_het_FIGB',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
...
for nvolume=0:n_volume-1;
nc = max(0,min(n_c_beach-1,floor(n_c_beach*nvolume/n_volume)));
tmp_index_ = efind(index_nvolume_from_nM_==nvolume);
subplot(2,n_volume,1+nvolume+0*n_volume);
...
subplot(2,n_volume,1+nvolume+1*n_volume);
...
%%%%%%%%;
fname_fig = sprintf('%s_jpg/loading_het_FIGC',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1);clf;figbig;figbeach();
c_beach__ = colormap_beach(); n_c_beach = size(c_beach__,1); c_beach__(1,:) = [1,1,1];
