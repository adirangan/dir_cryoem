      subroutine test_innerproduct_batch_wrapper1(verbose,ncur,nlats_
...
c$$$      parameters for timing
      ...
      if (verbose.gt.0) then
          write(6,'(A)')
     $        '[entering test_innerproduct_batch_wrapper1]: '
       end if
...
c$$$      calculating all innerproducts
     ...
     $     ,delta_y_sort_,gamma_z_sort_,C_Z_sort_,I_permute_,I_inverse_)
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[finished test_innerproduct_batch_wrapper1]'
      end if
