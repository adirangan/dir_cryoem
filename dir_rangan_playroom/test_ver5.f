...
c$$$      enddo
      ...
      d_(0) = 3
      d_(1) = n_source_use
      write(fname,'(A)') './dir_mda5/source_xyz_.mda'
...
c     create physical space grid and sample function on it.
     ...
     1     fgauss,ff_ori,fnorm)
      ...
      d_(1) = ngrid
      d_(2) = ngrid
      write(fname,'(A)') './dir_mda5/x_pre_.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(fname)
      call MDA_write_r8(3,d_,x,fname)
      write(fname,'(A)') './dir_mda5/y_pre_.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(fname)
      call MDA_write_r8(3,d_,y,fname)
      write(fname,'(A)') './dir_mda5/z_pre_.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(fname)
      call MDA_write_r8(3,d_,z,fname)
      write(fname,'(A)') './dir_mda5/S_ori_.mda'
...
c$$$               delta_tmp = rand()*displacement_max
               angle_tmp = 2*pi*(1.0d0*nnn)/(1.0d0*numonsphere(ngridr))
               delta_tmp = displacement_max*(2*(1.0d0*nnn)/(1.0d0
     $              *numonsphere(ngridr))-1.0d0)
...
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      write(fname,'(A)') './dir_jpg5/Fig_ctf_a'
...
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         d_(0) = n_alpha
         d_(1) = n_M_sample
         write(fname,'(A,I0,A)') './dir_mda5/alpha_tru_',ncur,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : ',trim(fname)
         call MDA_write_r8(2,d_,alpha_tru_,fname)
         d_(0) = n_alpha
         d_(1) = n_M_sample
         write(fname,'(A,I0,A)') './dir_mda5/alpha_est_',ncur,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : ',trim(fname)
...
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         write(fname,'(A,I0)') './dir_jpg5/Fig_MNC_a_',ncur
...
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        ,eps,npts,x,y,z,ff_tru,ier)
         ...
         d_(1) = ngrid
         d_(2) = ngrid
         write(fname,'(A,I0,A)') './dir_mda5/S_tru_',ncur,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : ',trim(fname)
...
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        ,eps,npts,x,y,z,ff_est,ier)
         ...
         d_(1) = ngrid
         d_(2) = ngrid
         write(fname,'(A,I0,A)') './dir_mda5/S_est_',ncur,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : ',trim(fname)
...
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         d_(0) = n_S_sample
         d_(1) = n_M_sample
         write(fname,'(A,I0,A)') './dir_mda5/C_Z_max_',ncur,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : ',trim(fname)
...
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         write(fname,'(A,I0)') './dir_jpg5/Fig_MTS_a_',ncur
