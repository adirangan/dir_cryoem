...
rng(global_rseed);
Pixel_A = 4.0;
snr_per_A = 0.5; %<-- decrease to increase noise. ;
f_rand_0in = 0.0;
n_M_factor_0in = 8; %<-- n_M = n_w_max * n_M_factor_0in. ;
n_pick_0in = 10; %<-- strong overlap at n_pick_0in = ceil(sqrt(n_w_max)). ;
local_rseed = 0;
...
end;%if flag_volume_type==0;
%%%%%%%%;
if flag_volume_type==1;
a_x_u_ = zeros(n_x_u,n_x_u);
tmp_ = [ ...
...
end;%if flag_volume_type==1;
%%%%%%%%;
if flag_volume_type==2;
x_sigma = 4*1/n_x_u;
a_x_u_ = zeros(n_x_u,n_x_u);
n_source = 128;
...
subplot(1,2,1); imagesc(a_x_u_,alim_); title('a'); axis image; axisnotick;
subplot(1,2,2); imagesc(b_rho_x_u_,alim_); title('b'); axis image; axisnotick;
...
p_row = 4; p_col = ceil((4+global_n_iteration)/p_row); np=0;
subplot(p_row,p_col,1+np);np=np+1;
imagesc(real(a_x_u_base_)); axis image; axisnotick;
end;%if flag_vis;
...
imagesc(real(a_x_u_reco_xx__),alim_); axis image; axisnotick; title(sprintf('ni %d',niteration));
drawnow();
end;%if flag_vis;
%%%%;
...
%%%%;
tmp_Y_0_q_ = sum(conj(reshape(a_k_q_quad_,[n_w_max,n_k_p_r])).*reshape(a_k_q_reco_wk__(:),[n_w_max,n_k_p_r]).*reshape(weight_2d_k_all_,[n_w_max,n_k_p_r]),2) * (2*pi)^2;
tmp_Y_0_w_ = ifft(tmp_Y_0_q_)*n_w_max;
tmp_Y_1_q_ = sum(conj(reshape(a_k_q_flip_,[n_w_max,n_k_p_r])).*reshape(a_k_q_reco_wk__(:),[n_w_max,n_k_p_r]).*reshape(weight_2d_k_all_,[n_w_max,n_k_p_r]),2) * (2*pi)^2;
tmp_Y_1_w_ = ifft(tmp_Y_0_q_)*n_w_max;
...
Y_i_(1+niteration) = tmp_Y / sqrt(a_k_p_quad_l2*a_k_p_reco_l2);
...
%%%%;
sgtitle(sprintf('pm_n_UX_rank %d',pm_n_UX_rank),'Interpreter','none');
%%%%;
end;%if flag_vis;
