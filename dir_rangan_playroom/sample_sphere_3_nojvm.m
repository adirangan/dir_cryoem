function [n_all,theta_all_,phi_all_,weight_all_] = sample_sphere_3_nojvm(r,d,TorL) ;
% generate theta_ and phi_ arrays sampled on sphere of radius r at density d ;
...
setup;
 ...
 [n_all,theta_all_,phi_all_,weight_all_] = sample_sphere_3_nojvm(1,sample_d,'L') ;
 for nk=1:b_K;
 k = k_(nk);
...
for nd=1:length(d_);
d = d_(nd);
disp(sprintf(' %% sampling at density d %0.3f',d));
r=3.0;
...
[n_all,theta_all_,phi_all_,weight_all_] = sample_sphere_3_nojvm(r,d,'T') ;
...
[n_all,theta_all_,phi_all_,weight_all_] = sample_sphere_3_nojvm(r,d,'L') ;
%weight_avg = mean(weight_all_); weight_std = std(weight_all_);
%disp(sprintf(' %% n_all %d %d %d',n_all,length(theta_all_),length(phi_all_)));
%x_ = cos(theta_all_).*sin(phi_all_);
%y_ = sin(theta_all_).*sin(phi_all_);
