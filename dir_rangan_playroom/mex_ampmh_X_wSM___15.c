 ...
 * gcc -fPIC -w -O2 -DWITHOUT_MEX -D_AVX -mavx -D_CBLAS -D_FMA -mfma -fopenmp mex_ampmh_X_wSM___15.c -lm -lfftw3_omp -lfftw3 -lfftw3f_omp -lfftw3f -lgslcblas ; ./a.out;
	...
	if (n_S_sub>0){
	  ...
	  if (flag_dump){
	    ...
	    if (flag_fast){
	      ...
	      MDA_write_r4(MDA_n_dim,MDA_dim_,f_svd_SVUXM_lwsM_real____,MDA_fname);
	      sprintf(MDA_fname,"mex_f_svd_SVUXM_lwsM_imag____.mat");
	      MDA_write_r4(MDA_n_dim,MDA_dim_,f_svd_SVUXM_lwsM_imag____,MDA_fname);
	      /* if (flag_fast){ } */}
	    /* if (flag_dump){ } */}
	  /* %%%% */
	  /* svd_USESVUXM_dwSM____ = real(reshape(FTK.svd_U_d_expiw_s__*reshape(svd_SVUXM_lwsM____,[FTK.n_svd_l,n_w_max*n_S_sub*n_M_sub]),[FTK.n_delta_v,n_w_max,n_S_sub,n_M_sub])); */
	  ...
	  if (flag_dump && flag_fast){
	    ...
	    MDA_write_r4(MDA_n_dim,MDA_dim_,f_svd_USESVUXM_dwSM_real____,MDA_fname);
	    sprintf(MDA_fname,"mex_f_svd_USESVUXM_dwSM_imag____.mat");
	    MDA_write_r4(MDA_n_dim,MDA_dim_,f_svd_USESVUXM_dwSM_imag____,MDA_fname);
	    /* if (flag_dump){ } */}
	  /* %%%% */
	  /* 
	     l2_dSM___ = permute(reshape(reshape(sqrt(CTF_UX_S_l2_use_(1+index_S_in_Sbatch_)),[n_S_sub,1])*reshape(sqrt(UX_M_l2_dM_use__(:,1+index_M_in_Mbatch_)),[1,FTK.n_delta_v*n_M_sub]),[n_S_sub,FTK.n_delta_v,n_M_sub]),[2,1,3]);
...
/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
/* The computational routine (slow vs fast) */
/* backup before attempting omp */
/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
void mex_ampmh_X_wSM___15_bkp
(
  int n_M_per_Mbatch
 ,int n_S_per_Sbatch
 ,int flag_optimize_over_gamma_z
...
#include "mex_ampmh_X_wSM___15_omp.c"
...
int main()
{
  ...
  local_toc(0,t_start_,t_final_,d_start_,d_final_,l_msec_,l_ssec_,l_usec_,elct_,elrt_,1,verbose+1,"mex_ampmh_X_wSM___15: ");
  /* /\* %%%%; *\/ */
  /* local_tic(0,t_start_,d_start_); */
...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
  ...
  int na=0,n_a;
  mwSize n_out=0;
  if (verbose){ printf(" %% [entering gateway mex_ampmh_X_wSM___15]\n");}
  /* %%%%%%%%%%%%%%%% */
  if(nrhs!=18) { mexErrMsgIdAndTxt("MyToolbox:mex_ampmh_X_wSM___15:nrhs","18 0in required.");}
  if(nlhs<  4) { mexErrMsgIdAndTxt("MyToolbox:mex_ampmh_X_wSM___15:nlhs"," 4 out required.");}
  /* %%%%%%%%%%%%%%%% */
  na=0;
  /* %%%% */
  ...
  /* %%%%%%%%%%%%%%%% */
  if (verbose){ printf(" %% [finished gateway mex_ampmh_X_wSM___15]\n");}
}
#endif
