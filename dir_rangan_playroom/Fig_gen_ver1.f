      subroutine Fig_gen_ver1(ncur,nlats_,grid_k_p_,n_S,ld_S,S_k_p_,n_M
     $    ,ld_M,M_k_p_,alpha_est_,delta_x_sort_,delta_y_sort_
     $    ,gamma_z_sort_,I_permute_,n_M_sample,prefix)
c$$$      Reads in a few templates and images and plots a comparison
      ...
      data verbose / 0 /
      logical plot_flag
      integer *4 ncur,nlats_(0:ncur-1),n_S,ld_S,n_M,ld_M,n_M_sample
...
c$$$         Display template without perturbation
     ...
     $        ,font_type,font_size,x_loc,y_loc,x_offset,y_offset,x_side
     $        ,y_side)
c$$$         Display template with perturbation
         ns = I_permute_(n_S-1 + nm*n_S)
         delta_x = alpha_est_(3 + nm*5) + delta_x_sort_(n_S-1 + nm*n_S)
         delta_y = alpha_est_(4 + nm*5) + delta_y_sort_(n_S-1 + nm*n_S)
         gamma_z = alpha_est_(2 + nm*5) + gamma_z_sort_(n_S-1 + nm*n_S)
         call cp1_c16(n_A,S_k_p_(ns*ld_S),T_k_p_)
...
c$$$      Printing figure file for image
     ...
     $     ,trim(fname_fig),trim(fname_jpg)
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[finished Fig_gen_ver1]'
      end if
