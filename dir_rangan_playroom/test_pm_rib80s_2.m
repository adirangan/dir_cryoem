...
dir_pm = sprintf('/%s/rangan/dir_cryoem/dir_rib80s/dir_pm',string_root);
...
dir_relion = sprintf('/%s/rangan/dir_cryoem/dir_rib80s/dir_relion',string_root);
...
dir_data_star = sprintf('/%s/rangan/dir_cryoem/dir_rib80s',string_root);
Pixel_Spacing = 1.34; %<-- in angstroms, from https://www.ebi.ac.uk/pdbe/emdb/empiar/entry/10028/ ;

%%%%%%%%;
% First load emd_5778.mrc volume. ;
%%%%%%%%;
fname_mat = sprintf('%s_mat/a_x_u_pack_.mat',dir_pm);
if (flag_recalc | ~exist(fname_mat,'file'));
disp(sprintf(' %% %s not found, creating',fname_mat));
fname_emd = sprintf('%s/emd_2660.mrc',dir_data_star);
...
CTF_lambda_per_box = CTF_lambda/(n_x_u*CTF_Object_Pixel_Size);% n_x_u*CTF_Object_Pixel_Size is the box size in Angstroms ;
%%%%;
na=0;
for nk = 0:n_k_p_r-1;
...
if ( exist(fname_ut_align_a_CTF_avg_UX_Y_mat,'file'));
...
end;%if ( exist(fname_ut_align_a_CTF_avg_UX_Y_mat,'file'));
fname_ut_align_a_k_Y_mat = sprintf('%s_align_a_k_Y_.mat',ut_fname_pre);
if ( exist(fname_ut_align_a_k_Y_mat,'file'));
...
end;%if ( exist(fname_ut_align_a_k_Y_mat,'file'));
...
if ( exist(fname_vt_align_a_CTF_avg_UX_Y_mat,'file'));
...
end;%if ( exist(fname_vt_align_a_CTF_avg_UX_Y_mat,'file'));
fname_vt_align_a_k_Y_mat = sprintf('%s_align_a_k_Y_.mat',vt_fname_pre);
if ( exist(fname_vt_align_a_k_Y_mat,'file'));
...
end;%if ( exist(fname_vt_align_a_k_Y_mat,'file'));
%%%%;
end;%for ndat_n_UX_rank=0:n_dat_n_UX_rank-1;
end;%for ndelta_r_max_factor=0:n_delta_r_max_factor-1;
end;%for ndat_rseed=0:n_dat_rseed-1;
...
str_symbol_ = {'o-','^-','s-','p-'};
markersize_use_ = round(linspace(8,16,n_delta_r_max_factor));
for ndat_n_UX_rank=0:n_dat_n_UX_rank-1;
dat_n_UX_rank = dat_n_UX_rank_(1+ndat_n_UX_rank);
...
dat_rseed = dat_rseed_(1+ndat_rseed);
for ndelta_r_max_factor=0:n_delta_r_max_factor-1;
gamma = 2*(1-ndelta_r_max_factor/n_delta_r_max_factor);
markersize_use = markersize_use_(1+ndelta_r_max_factor);
delta_r_max_factor = delta_r_max_factor_(1+ndelta_r_max_factor);
delta_r_max_use = delta_r_max_factor * delta_sigma_use*sqrt(log(20^2));
delta_r_max_upb = 2.0 * delta_sigma_use*sqrt(log(20^2)); %<-- allow large accumulated translations. ;
nc = max(0,min(n_c_80s-1,floor(n_c_80s*ndat_n_UX_rank/(n_dat_n_UX_rank-1))));
str_symbol = str_symbol_{1+ndelta_r_max_factor};
plot(1:2*n_iteration,xt_align_a_k_Y____(:,1+ndat_rseed,1+ndelta_r_max_factor,1+ndat_n_UX_rank),str_symbol,'MarkerSize',markersize_use,'Color',c_80s__(1+nc,:),'MarkerFaceColor',c_80s__(1+nc,:).^(gamma),'MarkerEdgeColor','k','LineWidth',2);
end;%for ndelta_r_max_factor=0:n_delta_r_max_factor-1;
end;%for ndat_rseed=0:n_dat_rseed-1;
%%%%%%%%;
%{
