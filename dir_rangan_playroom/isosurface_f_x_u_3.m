...
if ~isfield(parameter,'flag_version'); parameter.flag_version = 0; end;
flag_version = parameter.flag_version;
if ~isfield(parameter,'n_graph_laplacian'); parameter.n_graph_laplacian = 2; end;
...
n_face_g = size(tmp_faces_g_,1);
n_vertex_g = size(tmp_vertices_g_,1);
%%%%;
if flag_version==0; [~,d_v_] = distance_from_isosurface_0(struct('n_graph_laplacian',n_graph_laplacian),tmp_faces_g_,tmp_vertices_g_,tmp_faces_f_,tmp_vertices_f_); end;
if flag_version==1; [ij_v3__,d_v3__] = knnsearch(tmp_vertices_g_,tmp_vertices_f_,'K',3); d_v_ = d_v3__(:,1); end;
...
alpha(spatch,v_alpha);
tmp_vertices_f_2_ = bsxfun(@plus,[0,0,min(x_2_lim_)],bsxfun(@times,[+1,+1,-1e-3],tmp_vertices_f_));
