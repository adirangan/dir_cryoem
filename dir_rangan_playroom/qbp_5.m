...
if (nargin<13); image_delta_x_ = []; end;
if (nargin<14); image_delta_y_ = []; end;
if (nargin<15); image_I_value_ = []; end;
...
if isempty(CTF_index_); CTF_index_ = zeros(n_M,1); end;
if isempty(CTF_k_p__); CTF_k_p__ = ones(n_w_sum,1); end;
if (qbp_eps>1); qbp_eps = max(1e-12,0.1^(qbp_eps)); end;

verbose=0;
if (verbose>0); disp(sprintf(' %% [entering qbp_5]')); end;

n_w_sum = sum(n_w_);
T_k_p__ = M_k_p__;
if ( (fnorm(image_delta_x_)>0) | (fnorm(image_delta_y_)>0) | (fnorm(image_I_value_-ones(n_M,1))>0) );
for nM=0:n_M-1;
T_k_p__(:,1+nM) = image_I_value_(1+nM) * transf_p_to_p(n_k_p_r,k_p_r_,n_w_,n_w_sum,M_k_p__(:,1+nM),+image_delta_x_(1+nM),+image_delta_y_(1+nM));
end;%for nM=0:n_M-1;
...
CTF_wMn__ = reshape(permute(reshape(CTF_k_p__(:,1+CTF_index_),[n_w,n_k_p_r,n_M]),[1,3,2]),[n_w*n_M,n_k_p_r]);
...
n_quad_from_data_q_ = quad_from_data_qwM__*ones(n_w*n_M,1); %<-- number of data-points per quadrature-point. ;
data_from_quad_wMq__ = bsxfun(@rdivide,transpose(quad_from_data_qwM__),max(1,transpose(n_quad_from_data_q_)));
CTF_wM_ = reshape(CTF_k_p__(1+index_nw_,1+CTF_index_),[n_w*n_M,1]);
CTF2_q_ = quad_from_data_qwM__*abs(CTF_wM_).^2;
%%%%;
end;%end;%if flag_unique_n==1;
%%%%%%%%;
...
if (verbose>0); disp(sprintf(' %% [finished qbp_5]')); end;
