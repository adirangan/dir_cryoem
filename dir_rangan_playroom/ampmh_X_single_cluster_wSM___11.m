...
verbose = 0;
if (verbose>0); disp(sprintf(' %% [entering ampmh_X_single_cluster_wSM___11]')); end;
...
if (~isfield(parameter,'flag_X_dwSM____')); parameter.flag_X_dwSM____ = nargout>=7; end; %<-- parameter_bookmark. ;
%%%%%%%%;
...
flag_X_dwSM____ = parameter.flag_X_dwSM____;
if (nargout< 7); flag_X_dwSM____ = 0; end;
if (nargout>=7); flag_X_dwSM____ = 1; end;
parameter.flag_X_dwSM____ = flag_X_dwSM____;
...
X_wSM___ = zeros(n_S,n_M);
delta_x_wSM___ = zeros(n_S,n_M);
delta_y_wSM___ = zeros(n_S,n_M);
gamma_z_wSM___ = zeros(n_S,n_M);
if (flag_compute_I_value); I_value_wSM___ = zeros(n_S,n_M); end;
end;%if flag_optimize_over_gamma_z==1;

if flag_X_dwSM____;
n_X_dwSM = FTK.n_delta_v*n_w_max*n_S*n_M;
n_X_dwSM_GB = n_X_dwSM*8/1e9;
if (verbose); tmp_str = 'X_dwSM____'; disp(sprintf(' %% memory: %s --> %0.6f GB',tmp_str,n_X_dwSM_GB)); end;
if (n_X_dwSM_GB> 1.0);
disp(sprintf(' %% Warning, n_X_dwSM_GB %0.2f',n_X_dwSM_GB));
X_dwSM____ = [];
flag_X_dwSM____ = 0;
end;%if (n_X_dwSM_GB> 1.0);
if (n_X_dwSM_GB<=1.0);
X_dwSM____ = zeros(FTK.n_delta_v,n_w_max,n_S,n_M);
flag_X_dwSM____ = 1;
end;%if (n_X_dwSM_GB<=1.0);
end;%if flag_X_dwSM____;
...
tmp_t = tic();
if ndims(CTF_UX_S_k_q_wnS___)==2;
CTF_UX_S_k_q_wnS___ = reshape(CTF_UX_S_k_q_wnS___,[n_w_max,pm_n_UX_rank,n_S]);
end;%if ndims(CTF_UX_S_k_q_wnS___)==2;
CTF_UX_S_k_q_nSw___ = permute(CTF_UX_S_k_q_wnS___,[2,3,1]);
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% CTF_UX_S_k_q_nSw___: %0.6f',tmp_t)); end;
if (verbose>0);
...
parameter = parameter_timing_update(parameter,'ampmh_X_single_cluster_wSM___11: svd_VUXM_nMwl____',tmp_t);
%%%%%%%%;
tmp_t = tic();
svd_SVUXM_SMwl____ = zeros(n_S,n_M_sub,n_w_max,FTK.n_svd_l);
for nl=0:FTK.n_svd_l-1;
for nw=0:n_w_max-1;
svd_SVUXM_SMwl____(:,:,1+nw,1+nl) = svd_SVUXM_SMwl____(:,:,1+nw,1+nl) + ctranspose(CTF_UX_S_k_q_nSw___(:,:,1+nw))*svd_VUXM_nMwl____(:,:,1+nw,1+nl);
end;%for nw=0:n_w_max-1;
end;%for nl=0:FTK.n_svd_l-1;
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% svd_SVUXM_SMwl____: %0.6f',tmp_t)); end;
parameter = parameter_timing_update(parameter,'ampmh_X_single_cluster_wSM___11: svd_SVUXM_SMwl____',tmp_t);
tmp_t = tic();
...
parameter = parameter_timing_update(parameter,'ampmh_X_single_cluster_wSM___11: svd_SVUXM_lwSM____',tmp_t);
%%%%%%%%;
...
l2_dSM___(:,:,1+nM_sub) = reshape(sqrt(UX_M_l2_dM__(:,1+index_M_in_Mbatch_(1+nM_sub))),[FTK.n_delta_v,1])*reshape(sqrt(CTF_UX_S_l2_S_(1+index_S_in_Sbatch_)),[1,n_S_sub]);
end;%for nM_sub=0:n_M_sub-1;
n2_dSM___ = 1./max(1e-14,l2_dSM___);
f2_dSM___ = zeros(FTK.n_delta_v,n_S_sub,n_M_sub);
for nM_sub=0:n_M_sub-1;
f2_dSM___(:,:,1+nM_sub) = reshape( 1./max(1e-12,sqrt(UX_M_l2_dM__(:,1+index_M_in_Mbatch_(1+nM_sub)))) , [FTK.n_delta_v,1] ) * reshape(sqrt(CTF_UX_S_l2_S_(1+index_S_in_Sbatch_)),[1,n_S_sub]);
end;%for nM_sub=0:n_M_sub-1;
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% svd_USESVUXM_dwSM____: %0.6f',tmp_t)); end;
parameter = parameter_timing_update(parameter,'ampmh_X_single_cluster_wSM___11: svd_USESVUXM_dwSM____',tmp_t);
...
parameter = parameter_timing_update(parameter,'ampmh_X_single_cluster_wSM___11: X_sub_dwSM____',tmp_t);
%%%%%%%%;
if flag_X_dwSM____;
...
parameter = parameter_timing_update(parameter,'ampmh_X_single_cluster_wSM___11: X_wSM___',tmp_t);
end;%if flag_optimize_over_gamma_z==0;
%%%%%%%%;
if flag_optimize_over_gamma_z==1;
tmp_t = tic();
...
if (verbose>1); disp(sprintf(' %% [finished ampmh_X_single_cluster_wSM___11]')); end;
