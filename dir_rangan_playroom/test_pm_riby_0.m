...
verbose=1;
if (verbose); disp(sprintf(' %% [entering test_pm_riby_0]')); end;
...
for nS=0:n_S-1;
S_x_u_xx_ = interp_k_p_to_x_c_xxnufft(n_x_u_pack,diameter_x_c,n_x_u_pack,diameter_x_c,n_k_p_r,k_p_r_,n_w_,S_k_p_wkS__(:,1+nS).*weight_2d_wk_*(2*pi)^2)*sqrt(n_x_u_pack*n_x_u_pack) * n_w_sum;
S_x_u_xxS___(:,:,1+nS) = real(reshape(S_x_u_xx_,[n_x_u_pack,n_x_u_pack]));
end;%for nS=0:n_S-1;
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% S_x_u_xxS___: %0.2fs',tmp_t)); end;
%%%%;
S_mask_threshold = 0.05;
M_x_u_xxS___ = zeros(n_x_u_pack,n_x_u_pack,n_S); %<-- template masks. ;
M_k_p_wkS__ = zeros(n_w_sum,n_S);
for nS=0:n_S-1;
S_x_u_xx__ = S_x_u_xxS___(:,:,1+nS);
Slim_ = [min(S_x_u_xx__,[],'all'),max(S_x_u_xx__,[],'all')];
Scut = min(Slim_) + diff(Slim_)*S_mask_threshold;
M_x_u_xx__ = cast(S_x_u_xx__>Scut,'double');
...
%%%%%%%%;
n_Q_x_u_pack_ = floor(n_O_x_u_pack_/4);
tmp_ij_0_ = floor(n_O_x_u_pack_(1+0)*1.0/4) + [1:n_Q_x_u_pack_(1+0)];
tmp_ij_1_ = floor(n_O_x_u_pack_(1+1)*2.5/4) + [1:n_Q_x_u_pack_(1+1)];
...
M_x_u_xx__ = real( interp_k_p_to_x_c_xxnufft(n_x_u_pack,diameter_x_c,n_x_u_pack,diameter_x_c,n_k_p_r,k_p_r_,n_w_,M_k_p_wk_.*weight_2d_wk_*(2*pi)^2)*sqrt(n_x_u_pack*n_x_u_pack) * n_w_sum );
M_sum = sum(M_x_u_xx__,'all')*dx_pack*dx_pack;
S_x_u_xx__ = S_x_u_xx__.*M_x_u_xx__;
S_x_u_avg = sum(S_x_u_xx__,'all')*dx_pack*dx_pack;
...
verbose=1;
if (verbose); disp(sprintf(' %% [finished test_pm_riby_0]')); end;
