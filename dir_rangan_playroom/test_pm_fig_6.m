...
% also makes figs for Beamer_empm_2.tex. ;
%%%%%%%%;
...
%%%%%%%%;
fname_fig = sprintf('%s_jpg/empm_fig_Spindle_Example_FIGA_',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1+nf);nf=nf+1;figbig;
fontsize_use = 18;
subplot(2,3,4);
tmp_index_ = 128 + [-64:+64]; isosurface_f_x_u_0(a_x_c_zavg_(1+tmp_index_,1+tmp_index_,1+tmp_index_),90); axisnotick; xlabel('');ylabel('');zlabel('');
title(sprintf('$G(\\vec{x}) \\ \\ \\ 90\\%%$'),'Interpreter','Latex'); set(gca,'FontSize',fontsize_use);
subplot(2,3,1);
tmp_index_ = 128 + [-64:+64]; isosurface_f_x_u_0(a_x_c_load_(1+tmp_index_,1+tmp_index_,1+tmp_index_),95); axisnotick; xlabel('');ylabel('');zlabel('');
title(sprintf('$F(\\vec{x}) \\ \\ \\ 95\\%%$'),'Interpreter','Latex'); set(gca,'FontSize',fontsize_use);
%%%%;
subplot(2,3,5);
tmp_index_ = 0:n_k_c-1; val_a_k_c = isosurface_f_x_u_0( abs(a_k_c_zavg_(1+tmp_index_,1+tmp_index_,1+tmp_index_)),95.0); axisnotick; xlabel('');ylabel('');zlabel('');
title(sprintf('$\\| \\hat G(\\vec{k}) \\| \\ \\ \\ 95\\%%$'),'Interpreter','Latex');set(gca,'FontSize',fontsize_use);
subplot(2,3,2);
tmp_index_ = 0:n_k_c-1; val_a_k_c = isosurface_f_x_u_0( abs(a_k_c_load_(1+tmp_index_,1+tmp_index_,1+tmp_index_)),[],val_a_k_c); axisnotick; xlabel('');ylabel('');zlabel('');
title(sprintf('$\\| \\hat F(\\vec{k}) \\| \\ \\ \\ 95\\%%$'),'Interpreter','Latex');set(gca,'FontSize',fontsize_use);
%%%%;
subplot(2,3,6);
tmp_index_ = 0:n_k_c-1; val_a_k_c = isosurface_f_x_u_0( abs(a_k_c_zavg_(1+tmp_index_,1+tmp_index_,1+tmp_index_)),99.5); axisnotick; xlabel('');ylabel('');zlabel('');
title(sprintf('$\\| \\hat G(\\vec{k}) \\| \\ \\ \\ 99.5\\%%$'),'Interpreter','Latex');set(gca,'FontSize',fontsize_use);
subplot(2,3,3);
tmp_index_ = 0:n_k_c-1; val_a_k_c = isosurface_f_x_u_0( abs(a_k_c_load_(1+tmp_index_,1+tmp_index_,1+tmp_index_)),[],val_a_k_c); axisnotick; xlabel('');ylabel('');zlabel('');
title(sprintf('$\\| \\hat F(\\vec{k}) \\| \\ \\ \\ 99.5\\%%$'),'Interpreter','Latex');set(gca,'FontSize',fontsize_use);
%%%%;
...
%%%%;
nMC=index_nM_target_(1+7); %<-- third image. ;
euler_polar_a_true_nMC = euler_polar_a_true_(1+nMC);
euler_azimu_b_true_nMC = euler_azimu_b_true_(1+nMC);
euler_gamma_z_true_nMC = euler_gamma_z_true_(1+nMC);
image_delta_x_true_nMC = image_delta_x_true_(1+nMC);
image_delta_y_true_nMC = image_delta_y_true_(1+nMC);
%%%%;
nMD=index_nM_target_(1+8); %<-- fourth image. ;
euler_polar_a_true_nMD = euler_polar_a_true_(1+nMD);
euler_azimu_b_true_nMD = euler_azimu_b_true_(1+nMD);
euler_gamma_z_true_nMD = euler_gamma_z_true_(1+nMD);
image_delta_x_true_nMD = image_delta_x_true_(1+nMD);
image_delta_y_true_nMD = image_delta_y_true_(1+nMD);
%%%%;
...
fname_fig = sprintf('%s_jpg/empm_fig_image_sample_FIGA_',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
...
fname_fig = sprintf('%s_jpg/empm_fig_SliceTheorem_2d_FIGA_',dir_pm);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
...
tmp_sigma_g = 2.0/(2*pi); tmp_g_2d_ = (1/sqrt(2*pi) / tmp_sigma_g) * exp(-(k_c_1_2d_.^2)/(2*tmp_sigma_g.^2)); tmp_g_2d_ = tmp_g_2d_/max(1e-12,max(tmp_g_2d_));
tmp_sigma_g = 2.0/(2*pi); tmp_g_2d_ = abs(k_c_1_2d_)<=tmp_sigma_g;
tmp_bot = min(T_x_c_lim_) + (2/64)*diff(T_x_c_lim_);
tmp_T_k_p_nMA_ = max(tmp_bot,real(T_k_p_nMA_)) .* tmp_g_2d_ + min(T_x_c_lim_).*(1-tmp_g_2d_);
imagesc_p(n_k_p_r,k_p_r_,n_w_,n_w_sum,tmp_T_k_p_nMA_,T_x_c_lim_,colormap_80s_b__);
...
title(sprintf('${\\cal R} [ \\hat{A}_{%d}^{\\mbox{signal}}(\\vec{k}) + \\hat{A}_{%d}^{\\mbox{noise}}(\\vec{k}) ]$',1+nMA,1+nMA),'Interpreter','Latex');
axis image; axisnotick; set(gca,'FontSize',fontsize_use);
%%%%;
subplot(p_row,p_col,1+ns); ns=ns+1;
imagesc_p(n_k_p_r,k_p_r_,n_w_,n_w_sum,real(T_k_p_nMA_),[],colormap_80s);
title(sprintf('${\\cal R} [ \\hat{B}_{%d} := \\hat{A}_{%d}^{\\mbox{signal}}(\\vec{k}) ]$',1+nMA,1+nMA),'Interpreter','Latex');
axis image; axisnotick; set(gca,'FontSize',fontsize_use);
%%%%;
%%%%%%%%;
%%%%;
...
title(sprintf('${\\cal R} [ \\hat{A}_{%d}^{\\mbox{signal}}(\\vec{k}) + \\hat{A}_{%d}^{\\mbox{noise}}(\\vec{k}) ]$',1+nMB,1+nMB),'Interpreter','Latex');
axis image; axisnotick; set(gca,'FontSize',fontsize_use);
%%%%;
subplot(p_row,p_col,1+ns); ns=ns+1;
imagesc_p(n_k_p_r,k_p_r_,n_w_,n_w_sum,real(T_k_p_nMB_),[],colormap_80s);
title(sprintf('${\\cal R} [ \\hat{B}_{%d} := \\hat{A}_{%d}^{\\mbox{signal}}(\\vec{k}) ]$',1+nMB,1+nMB),'Interpreter','Latex');
axis image; axisnotick; set(gca,'FontSize',fontsize_use);
%%%%;
%%%%%%%%;
disp(sprintf(' %% writing %s',fname_fig));
