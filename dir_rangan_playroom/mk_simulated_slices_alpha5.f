c***********************************************************************
      subroutine mk_simulated_slices_alpha5(density,ngrid,boxsize,eps
     $     ,ngridr,ityper,ngridc,rkmax,alpha5,nslices,nslice_size
     $     ,cslices)
c***********************************************************************
c$$$      From density in physical space, generate slices of Fourier 
c$$$      transform on 2D grids at requested parameters :
c$$$      alpha5 -> (cos(beta==theta),alpha==phi,gamma_z,delta_x,delta_y).
c$$$      
...
c     alpha5        5 x nimage array of parameters for each simulated image
...
c-----------------------------------------------------------------------
     ...
     $     '[entering mk_simulated_slices_alpha5]'
c
...
c
      if (verbose.gt.1) write(6,*)
     $     '[finished mk_simulated_slices_alpha5]'
      return
      end
