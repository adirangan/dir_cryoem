...
n_svd_l = FTK.n_svd_l;
svd_l_ = FTK.svd_l_;
svd_U_d_chebcoef_ = FTK.svd_U_d_chebcoef_;
svd_s_ = FTK.svd_s_;
svd_V_r_chebcoef_ = FTK.svd_V_r_chebcoef_;
...
svd_U_d_chebcoef_bl__ = reshape(svd_U_d_chebcoef_,[n_svd_d,n_svd_l]);
svd_V_r_chebcoef_al__ = reshape(svd_V_r_chebcoef_,[n_svd_r,n_svd_l]);
%%%%%%%%;
b_b_ = zeros(n_b,1);
for nb=0:n_b-1;
...
l = svd_l_(1+ns);
S = svd_s_(1+ns);
U_d = dot(svd_U_d_chebcoef_bl__(:,1+ns),b_b_);
V_r_ = transpose(sum(bsxfun(@times,svd_V_r_chebcoef_al__(:,1+ns),a_ak__),1));
