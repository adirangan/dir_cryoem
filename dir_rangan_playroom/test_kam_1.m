...
%%%%%%%%;
noment_0_lm0_ = reshape(sum(S_k_q_qlmS___(1+0,:,:).*repmat(reshape(viewing_weight_all_,[1,1,n_S]),[1,n_lm_sum,1]),3),[n_lm_sum,1]);
noment_1_lm0lm1g___ = zeros(n_lm_sum,n_lm_sum,n_gamma);
for nS=0:n_S-1;
for ngamma1=0:n_gamma-1;
...
noment_1_lm0lm1g___(:,:,1+ngamma1) = noment_1_lm0lm1g___(:,:,1+ngamma1) + reshape(conj(transpose(reshape(S_k_q_qlmS___(:,:,1+nS),[n_w_sum,n_lm_sum])))*expiqgS_qlm__,[n_lm_sum,n_lm_sum,1])*viewing_weight_all_(1+nS);
end;%for ngamma1=0:n_gamma-1;
end;%for nS=0:n_S-1;
noment_2_lm1lm2lm0gg_____ = zeros(n_lm_sum,n_lm_sum,n_lm_sum,n_gamma,n_gamma);
for nS=0:n_S-1;
for ngamma1=0:n_gamma-1;
...
noment_2_lm1lm2lm0gg_____(:,:,:,1+ngamma1,1+ngamma2) = noment_2_lm1lm2lm0gg_____(:,:,:,1+ngamma1,1+ngamma2) + permute(reshape(reshape(conj(S_k_q_qlmS___(1+nq0,:,1+nS)),[n_lm_sum,1])*reshape(expiqg1Sg2S_lm0lm1__,[1,n_lm_sum^2]),[n_lm_sum,n_lm_sum,n_lm_sum,1,1]),[2,3,1,4,5])*viewing_weight_all_(1+nS);
...
%%%%%%%%;
disp(sprintf(' %% moment_0_lm0_ vs noment_0_lm0_: %0.16f',fnorm(moment_0_lm0_-noment_0_lm0_)/fnorm(moment_0_lm0_)));
disp(sprintf(' %% moment_1_lm0lm1g___ vs noment_1_lm0lm1g___: %0.16f',fnorm(moment_1_lm0lm1g___-noment_1_lm0lm1g___)/fnorm(moment_1_lm0lm1g___)));
disp(sprintf(' %% moment_2_lm1lm2lm0gg_____ vs noment_2_lm1lm2lm0gg_____: %0.16f',fnorm(moment_2_lm1lm2lm0gg_____-noment_2_lm1lm2lm0gg_____)/fnorm(moment_2_lm1lm2lm0gg_____)));
%%%%%%%%;
...
subplot(2,2,2); hist(kappa_0_,linspace(0,10,128)); title(sprintf('\\kappa_0 --> mean %0.2f',mean(kappa_0_)));
subplot(2,2,3); hist(kappa_1_,linspace(0,10,128)); title(sprintf('\\kappa_1 --> mean %0.2f',mean(kappa_1_)));
subplot(2,2,4); hist(kappa_2_,linspace(0,10,128)); title(sprintf('\\kappa_2 --> mean %0.2f',mean(kappa_2_)));
figbig;
