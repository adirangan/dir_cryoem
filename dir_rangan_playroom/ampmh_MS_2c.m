...
tmp_n_S = n_S;
index_nS_permutation_ = randperm(tmp_n_S)-1;
end;%if flag_euler_polar_a_restrict==0; %<-- any polar_a allowed. ;
if flag_euler_polar_a_restrict==1; %<-- only equatorial polar_a allowed. ;
tmp_n_S = n_polar_a_retain;
tmp_index_nS_permutation_ = randperm(tmp_n_S)-1;
index_nS_permutation_ = index_polar_a_retain_(1+tmp_index_nS_permutation_);
end;%if flag_euler_polar_a_restrict==1; %<-- only equatorial polar_a allowed. ;
%%%%%%%%;
...
tmp_nS=tmp_nS+1; if (tmp_nS>=tmp_n_S); tmp_nS=0; end;
nM_sum=nM_sum+1;
