...
sgtitle(sprintf('v_eig_dvol_yk_: %s niteration %d lambda %0.2f = exp(%+0.2f) index_lambda %d/%d: %s ',str_title,niteration,lambda_use,log(lambda_use),index_lambda,niteration,str_vv),'Interpreter','none');
end;%if flag_disp;
%%%%;
if flag_disp;
...
imagesc(squeeze(real(v_x_u_use___(end/2,:,:))));
axis image; axisnotick; colorbar;
sgtitle(sprintf('%s niteration %d lambda %0.2f = exp(%+0.2f) index_lambda %d/%d: %s ',str_title,niteration,lambda_use,log(lambda_use),index_lambda,niteration,str_vv),'Interpreter','none');
end;%if flag_disp;
%%%%%%%%;
