...
Micrograph_mip_9_2_ = ReadMRC_0(fname_mip_9_2_mrc);
...
Micrograph_CoC_9_2_ = ReadMRC_0(fname_CoC_9_2_mrc);
...
Micrograph_mip_9_3_ = ReadMRC_0(fname_mip_9_3_mrc);
...
Micrograph_CoC_9_3_ = ReadMRC_0(fname_CoC_9_3_mrc);
...
Micrograph_mip_5_2_ = ReadMRC_0(fname_mip_5_2_mrc);
...
Micrograph_CoC_5_2_ = ReadMRC_0(fname_CoC_5_2_mrc);
...
Micrograph_mip_5_3_ = ReadMRC_0(fname_mip_5_3_mrc);
...
Micrograph_CoC_5_3_ = ReadMRC_0(fname_CoC_5_3_mrc);
...
imagesc(Micrograph_mip_9_2_);axis image; axisnotick; title(infix_mip_9_2,'Interpreter','none'); set(gca,'FontSize',fontsize_use);
subplot(p_row,p_col,1+np);np=np+1;cla;
imagesc(Micrograph_CoC_9_2_);axis image; axisnotick; title(infix_CoC_9_2,'Interpreter','none'); set(gca,'FontSize',fontsize_use);
subplot(p_row,p_col,1+np);np=np+1;cla;
imagesc(Micrograph_mip_9_3_);axis image; axisnotick; title(infix_mip_9_3,'Interpreter','none'); set(gca,'FontSize',fontsize_use);
subplot(p_row,p_col,1+np);np=np+1;cla;
imagesc(Micrograph_CoC_9_3_);axis image; axisnotick; title(infix_CoC_9_3,'Interpreter','none'); set(gca,'FontSize',fontsize_use);
subplot(p_row,p_col,1+np);np=np+1;cla;
imagesc(Micrograph_mip_5_2_);axis image; axisnotick; title(infix_mip_5_2,'Interpreter','none'); set(gca,'FontSize',fontsize_use);
subplot(p_row,p_col,1+np);np=np+1;cla;
imagesc(Micrograph_CoC_5_2_);axis image; axisnotick; title(infix_CoC_5_2,'Interpreter','none'); set(gca,'FontSize',fontsize_use);
subplot(p_row,p_col,1+np);np=np+1;cla;
imagesc(Micrograph_mip_5_3_);axis image; axisnotick; title(infix_mip_5_3,'Interpreter','none'); set(gca,'FontSize',fontsize_use);
subplot(p_row,p_col,1+np);np=np+1;cla;
imagesc(Micrograph_CoC_5_3_);axis image; axisnotick; title(infix_CoC_5_3,'Interpreter','none'); set(gca,'FontSize',fontsize_use);
...
tmp_mip_error = tmp_mip_error + abs(mip_50_r_(1+tmp_index_1) - Micrograph_mip_9_3_(1+tmp_nx,1+tmp_ny));
tmp_CoC_error = tmp_CoC_error + abs(CoC_50_r_(1+tmp_index_1) - Micrograph_CoC_9_3_(1+tmp_nx,1+tmp_ny));
...
%%%%%%%%;
[n_x,n_y] = size(Micrograph_mip_9_3_);
...
tmp_mip__ = reshape(Micrograph_mip_9_3_(tmp_index_),[1+2*tmp_r,1+2*tmp_r]);
tmp_CoC__ = reshape(Micrograph_CoC_9_3_(tmp_index_),[1+2*tmp_r,1+2*tmp_r]);
...
%%%%%%%%;
% recapitulate relevant points. ;
%%%%%%%%;
[n_x,n_y] = size(Micrograph_mip_9_3_);
filter_rad_ = [50:-1:0];n_filter_rad = numel(filter_rad_);
n_xx_fr_ = zeros(n_filter_rad,1);
CoC_xx_fr__ = cell(n_filter_rad,1);
mip_xx_fr__ = cell(n_filter_rad,1);
nx_xx_fr__ = cell(n_filter_rad,1);
ny_xx_fr__ = cell(n_filter_rad,1);
lab_xx_fr__ = cell(n_filter_rad,1);
pro_CoC_xx_fr__ = cell(n_filter_rad,1);
pro_mip_xx_fr__ = cell(n_filter_rad,1);
p1q_xx_fr__ = cell(n_filter_rad,1);
A_mip_f_ = zeros(n_filter_rad,1);
A_CoC_f_ = zeros(n_filter_rad,1);
A_p1q_f_ = zeros(n_filter_rad,1);
%%%%;
%%%%%%%%;
%%%%;
tmp_index_1_ = efind(lab_50_r_==1);
for nfilter_rad=0:n_filter_rad-1;
filter_rad = filter_rad_(1+nfilter_rad);
disp(sprintf(' %% %% %% %% %% %% %% %% %% '));
disp(sprintf(' %% nfilter_rad %d/%d',nfilter_rad,n_filter_rad));
disp(sprintf(' %% %% %% %% %% %% %% %% %% '));
%%%%;
tmp_index_ = efind(max_filter_2d_imdilate_0(Micrograph_mip_9_3_,filter_rad)==Micrograph_mip_9_3_);
n_xx_r = numel(tmp_index_);
n_xx_fr_(1+nfilter_rad) = n_xx_r;
tmp_index_x_ = mod(tmp_index_,n_y);
nx_xx_r_ = tmp_index_x_;
tmp_index_y_ = floor(tmp_index_/n_y);
ny_xx_r_ = tmp_index_y_;
mip_xx_r_ = Micrograph_mip_9_3_(1+tmp_index_);
CoC_xx_r_ = Micrograph_CoC_9_3_(1+tmp_index_);
mip_xx_fr__{1+nfilter_rad} = mip_xx_r_;
CoC_xx_fr__{1+nfilter_rad} = CoC_xx_r_;
nx_xx_fr__{1+nfilter_rad} = nx_xx_r_;
ny_xx_fr__{1+nfilter_rad} = ny_xx_r_;
[tmp_ij_] = knnsearch([nx_xx_r_,ny_xx_r_],[nx_50_r_(1+tmp_index_1_),ny_50_r_(1+tmp_index_1_)]);
lab_xx_r_ = zeros(n_xx_r,1);
lab_xx_r_(tmp_ij_) = 1;
lab_xx_fr__{1+nfilter_rad} = lab_xx_r_;
clear tmp_ij_;
%%%%;
[~,rank_mip_xx_r_] = sort(mip_xx_r_,'ascend');
[~,rank_mip_xx_r_] = sort(rank_mip_xx_r_,'ascend');
rank_mip_xx_r_ = (rank_mip_xx_r_-0.5)/max(1,numel(mip_xx_r_));
pro_mip_xx_r_ = sqrt(2.0)*erfinv(2.0*rank_mip_xx_r_ - 1.0);
[~,rank_CoC_xx_r_] = sort(CoC_xx_r_,'ascend');
[~,rank_CoC_xx_r_] = sort(rank_CoC_xx_r_,'ascend');
rank_CoC_xx_r_ = (rank_CoC_xx_r_-0.5)/max(1,numel(CoC_xx_r_));
pro_CoC_xx_r_ = sqrt(2.0)*erfinv(2.0*rank_CoC_xx_r_ - 1.0);
pro_mip_xx_fr__{1+nfilter_rad} = pro_mip_xx_r_;
pro_CoC_xx_fr__{1+nfilter_rad} = pro_CoC_xx_r_;
%%%%;
tmp_ = [pro_mip_xx_r_,pro_CoC_xx_r_];
Cinv_xx__ = transpose(tmp_)*tmp_/n_xx_r;
[Uinv_xx__,Dinv_xx_] = eigs(Cinv_xx__); Dinv_xx_ = diag(Dinv_xx_);
C_xx__ = inv(Cinv_xx__);
[U_xx__,D_xx_] = eigs(C_xx__); D_xx_ = diag(D_xx_);
g_xx__ = rho_g2(pro_x__,pro_y__,C_xx__);
clear tmp_;
%%%%;
% calculated p-value. ;
%%%%;
pq1_xx_r_ = zeros(n_xx_r,1);
tmp_a = max(sqrt(Dinv_xx_)); tmp_b = min(sqrt(Dinv_xx_)); tmp_w = pi/4; %<-- assumes correlated with major-axis at pi/4. ;
tmp_gamma = atan2(1.0,0.5*sin(2*tmp_w)*(tmp_b/tmp_a - tmp_a/tmp_b)); %<-- angular formula only valid in 2-dimensions. ;
for nxx_r=0:n_xx_r-1;
if flag_verbose & (mod(nxx_r,1024*4)==0); disp(sprintf(' %% nxx_r %d/%d',nxx_r,n_xx_r)); end;
pro_mip = pro_mip_xx_r_(1+nxx_r);
pro_CoC = pro_CoC_xx_r_(1+nxx_r);
p1q_equ = 1.0;
if (pro_mip> 0) & (pro_CoC> 0);
tmp_x_0 = +cos(tmp_w)*pro_mip + sin(tmp_w)*pro_CoC;
tmp_x_1 = -sin(tmp_w)*pro_mip + cos(tmp_w)*pro_CoC;
...
p1q_xx_r_(1+nxx_r) = p1q_equ;
end;%for nxx_r=0:n_xx_r-1;
p1q_xx_fr__{1+nfilter_rad} = p1q_xx_r_;
%%%%;
tmp_index_D_ = efind(lab_xx_r_==1);
tmp_index_X_ = efind(lab_xx_r_==0);
A_mip = auc_0(mip_xx_r_(1+tmp_index_X_),mip_xx_r_(1+tmp_index_D_));
A_CoC = auc_0(CoC_xx_r_(1+tmp_index_X_),CoC_xx_r_(1+tmp_index_D_));
A_p1q = auc_0(-log(p1q_xx_r_(1+tmp_index_X_)),-log(p1q_xx_r_(1+tmp_index_D_)));
A_mip_f_(1+nfilter_rad) = A_mip;
A_CoC_f_(1+nfilter_rad) = A_CoC;
A_p1q_f_(1+nfilter_rad) = A_p1q;
%%%%;
end;%for nfilter_rad=0:n_filter_rad-1;
%%%%;
%%%%%%%%;
%%%%;
...
subplot(1,2,1);cla;
hold on;
plot(filter_rad_,A_CoC_f_,'ko-','LineWidth',linewidth_use,'MarkerSize',markersize_use,'MarkerFaceColor','g');
plot(filter_rad_,A_mip_f_,'ko-','LineWidth',linewidth_use,'MarkerSize',markersize_use,'MarkerFaceColor','r');
plot(filter_rad_,A_p1q_f_,'ko-','LineWidth',linewidth_use,'MarkerSize',markersize_use,'MarkerFaceColor','c');
hold off;
legend({'CoC','mip','p1q'},'Location','NorthEast');
xlabel('filter radius'); ylabel('AUC'); title('AUC vs filter-radius');
xlim([-1,51]);set(gca,'XTick',[0:5:50]); ylim([0.985,1.005]);set(gca,'YTick',[0.985:0.005:1.005]); grid on;
set(gca,'FontSize',fontsize_use);
%%%%;
subplot(1,2,2);cla;
hold on;
plot(filter_rad_,-log(1-A_CoC_f_),'ko-','LineWidth',linewidth_use,'MarkerSize',markersize_use,'MarkerFaceColor','g');
plot(filter_rad_,-log(1-A_mip_f_),'ko-','LineWidth',linewidth_use,'MarkerSize',markersize_use,'MarkerFaceColor','r');
plot(filter_rad_,-log(1-A_p1q_f_),'ko-','LineWidth',linewidth_use,'MarkerSize',markersize_use,'MarkerFaceColor','c');
hold off;
legend({'CoC','mip','p1q'},'Location','NorthEast');
xlabel('filter radius'); ylabel('-log(1-AUC)'); title('-log(1-AUC) vs filter-radius');
xlim([-1,51]);set(gca,'XTick',[0:5:50]); ylim([4,11]);set(gca,'YTick',[4:0.5:11]); grid on;
set(gca,'FontSize',fontsize_use);
%%%%;
fname_fig_pre = sprintf('%s/test_Kexin_AUC_9_3_xx_FIGL',dir_jpg);
fname_fig_jpg = sprintf('%s.jpg',fname_fig_pre);
if (flag_replot | ~exist(fname_fig_jpg));
disp(sprintf(' %% Writing %s',fname_fig_pre));
sgtitle(fname_fig_pre,'Interpreter','none');
