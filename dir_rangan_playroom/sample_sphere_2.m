function [n_all,theta_all_,phi_all_,weight_all_] = sample_sphere_2(r,d,TorL) ;
...
[n_all,theta_all_,phi_all_,weight_all_] = sample_sphere_2(1,sample_d,'L') ;
for nk=1:b_K;
k = k_(nk);
...
[n_all,theta_all_,phi_all_,weight_all_] = sample_sphere_2(r,d,'T') ;
...
[n_all,theta_all_,phi_all_,weight_all_] = sample_sphere_2(r,d,'L') ;
weight_avg = mean(weight_all_); weight_std = std(weight_all_);
disp(sprintf(' %% n_all %d %d %d',n_all,length(theta_all_),length(phi_all_)));
x_ = cos(theta_all_).*sin(phi_all_);
y_ = sin(theta_all_).*sin(phi_all_);
