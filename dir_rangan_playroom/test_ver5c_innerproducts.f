...
c     
      ...
      complex *16, allocatable :: modsph_tr2(:)
      ...
      parameter (normalization_flag=.true.)
      logical ctf_flag
      parameter (ctf_flag=.true.)
      logical template_flag
      parameter (template_flag=.false.)
...
c
      ...
      if (verbose.gt.1) then
         write(6,'(A)') '[entering test_ver5cI]'
      end if
...
c$$$      enddo
      ...
      d_(0) = 3
      d_(1) = n_source_use
      write(fname,'(A)') './dir_mda5cI/source_xyz_.mda'
...
c     create physical space grid and sample function on it.
     ...
     1     fgauss,ff_ori,fnorm)
      ...
      d_(1) = ngrid
      d_(2) = ngrid
      write(fname,'(A)') './dir_mda5cI/x_pre_.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(fname)
      call MDA_write_r8(3,d_,x,fname)
      write(fname,'(A)') './dir_mda5cI/y_pre_.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(fname)
      call MDA_write_r8(3,d_,y,fname)
      write(fname,'(A)') './dir_mda5cI/z_pre_.mda'
      if (verbose.gt.1) write(6,'(A,A)') 'Writing r8 : ',trim(fname)
      call MDA_write_r8(3,d_,z,fname)
      write(fname,'(A)') './dir_mda5cI/S_ori_.mda'
...
c
      ...
      allocate(modsph_tr2(0:nsphstore-1))
...
c      
      ...
      allocate(alpha2d_ori_(n_alpha,nimages))
...
c$$$            original parameters
            alpha2d_ori_(1+nalpha_polar_a,nnn) = dacos(ctheta)
            alpha2d_ori_(1+nalpha_azimu_b,nnn) = phi
            alpha2d_ori_(1+nalpha_gamma_z,nnn) = 0.0d0
c$$$            alpha2d_ori_(1+nalpha_gamma_z,nnn) = (1-2*rand())*pi
            alpha2d_ori_(1+nalpha_delta_x,nnn) = 0.0d0/rmax
            alpha2d_ori_(1+nalpha_delta_y,nnn) = 0.0d0/rmax
            alpha2d_ori_(1+nalpha_l2_norm,nnn) = 1.0d0
            alpha2d_ori_(1+nalpha_ctf_ind,nnn) = 0.0d0
c$$$            true parameters used to deform image
...
c$$$     $           -1)
            alpha2d_est_(1+nalpha_delta_x,nnn) = 0.125d0/rmax*(mod(nnn+2
     $              ,5)-1)
            alpha2d_est_(1+nalpha_delta_y,nnn) = 0.25d0/rmax*(mod(nnn+1
     $              ,3)-1)
...
c
     ...
     $     icstart)
      if (verbose.gt.0) write(6,*) '  ntemplatesize is ',ntemplatesize
      if (verbose.gt.0) write(6,*) '  ntemplate_size is ',ntemplate_size
      if (verbose.gt.0) write(6,*) '  nimage_size is ',nimage_size
...
c
     ...
     $     ,cslices)
      call mk_simulated_slices_alpha2d(ff_ori,ngrid,boxsize,eps,ngridr,
     $     ityper,ngridc,rmax,n_alpha,alpha2d_ori_,nimages,nimage_size
     $     ,templates)
...
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      write(fname,'(A)') './dir_jpg5cI/Fig_ctf_c'
...
c$$$      n_M_sample_max = maximum number of images to use at each step.
      ...
      allocate(alpha_ori_(0:n_alpha*n_M_sample_max-1))
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         ...
         do nM_sample=0,n_M_sample-1
            nm = I_M_sample_(nM_sample)
            call cp1_r8(n_alpha,alpha2d_ori_(1,1+nm),alpha_ori_(0
     $           +n_alpha*nM_sample))
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         d_(0) = n_alpha
         d_(1) = n_M_sample
         write(fname,'(A,I0,A)') './dir_mda5cI/alpha_tru_',ncur,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : ',trim(fname)
         call MDA_write_r8(2,d_,alpha_tru_,fname)
         d_(0) = n_alpha
         d_(1) = n_M_sample
         write(fname,'(A,I0,A)') './dir_mda5cI/alpha_est_',ncur,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : ',trim(fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         write(fname,'(A,I0)') './dir_jpg5cI/Fig_MNC_c_',ncur
     ...
     $        ,alpha_est_,min(16,n_M_sample),fname)
         ...
         if (template_flag.eqv..true.) then

c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
c$$$  generate templates based on original model
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $           ,' templates'
            timing_tic = second()
            call template_gen(modsph_ori,nsphstore,isph_start,
     $           nterms_sph,ngridr,ngridc,ntemplatesize,icstart, nlats
     $           ,itypep,ncur,numonsphere_cur,templates)
            timing_toc = second()
            if (verbose.gt.0) then
               write(6,'(A,A,F8.3)') 'template_gen:' ,' total_time '
     $              ,timing_toc-timing_tic
            end if              ! if verbose
            call getspheregrid(nlats(ncur),itypep,xnodesth_tmp,
     $           sthetas_tmp,wtsth_tmp,ngridps_tmp,phsteps_tmp
     $           ,nspherebig_tmp)
            nnn = 0
            do kk = 1,nlats(ncur)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $              ,S_sample_(nS_sample*ntemplatesize))
            ...
            enddo               !do nS_sample=0,n_S_sample-1

         end if                 !if (template_flag.eqv..true.) then

         if (template_flag.eqv..false.) then

c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
c$$$  copy templates from original templates
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
            n_S_sample = min(nimages,n_S_sample_max)
            if (verbose.gt.0) write(6,'(A,I0,A)') 'selecting '
     $           ,n_S_sample,' templates'
            do nS_sample=0,n_S_sample-1
               if (n_S_sample.lt.nimages) then
                  I_S_sample_(nS_sample) = min(nimages-1 ,floor(nimages
     $                 *1.0d0*nS_sample/(n_S_sample -1)))
               else
                  I_S_sample_(nS_sample) = nS_sample
               end if
               ns = I_S_sample_(nS_sample)
               call cp1_c16(nimage_size,templates(1,1+ns)
     $              ,S_sample_(nS_sample*nimage_size))
               alpha_polar_a_sample_(nS_sample) = alpha2d_ori_(1
     $              +nalpha_polar_a,1+ns)
               alpha_azimu_b_sample_(nS_sample) = alpha2d_ori_(1
     $              +nalpha_azimu_b,1+ns)
            enddo               !do nS_sample=0,n_S_sample-1
            
         end if                 !if (template_flag.eqv..false.) then
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        *n_S_sample,') innerproducts'
         ...
         call test_innerproduct_batch_wrapper_2c(0,ncur,nlats,xnodesr
     ...
     $        ,n_delta_y,max(32,2*ncur), svd_calculation_type
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         d_(0) = n_S_sample
         d_(1) = n_M_sample
         write(fname,'(A,I0,A)') './dir_mda5cI/C_Z_max_',ncur,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : ',trim(fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         write(fname,'(A,I0)') './dir_jpg5cI/Fig_MTS_c_',ncur
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      ...
      enddo                     ! do ncur = ngridr,ngridr


      if (verbose.gt.0) write(6,*)
     $     'copying nalpha_polar_a: alpha2d-->alpha'
      do nM_sample=0,n_M_sample-1
         nm = I_M_sample_(nM_sample)
         alpha_ori_(nM_sample) = alpha2d_ori_(1+nalpha_polar_a,1+nm)
         alpha_tru_(nM_sample) = alpha2d_tru_(1+nalpha_polar_a,1+nm)
         alpha_est_(nM_sample) = alpha2d_est_(1+nalpha_polar_a,1+nm)
      enddo                     !do nM_sample=0,n_M_sample-1
      write(format_string,'(A,I0,A)') '(A,' , n_M_sample , '(F8.4,1X))'
      write(6,format_string) 'nalpha_polar_a ori: '
     $     ,(alpha_ori_(nM_sample),nM_sample=0,n_M_sample-1)
      write(6,format_string) 'nalpha_polar_a tru: '
     $     ,(alpha_tru_(nM_sample),nM_sample=0,n_M_sample-1)
      write(6,format_string) 'nalpha_polar_a est: '
     $     ,(alpha_est_(nM_sample),nM_sample=0,n_M_sample-1)

      if (verbose.gt.0) write(6,*)
     $     'copying nalpha_azimu_b: alpha2d-->alpha'
      do nM_sample=0,n_M_sample-1
         nm = I_M_sample_(nM_sample)
         alpha_ori_(nM_sample) = alpha2d_ori_(1+nalpha_azimu_b,1+nm)
         alpha_tru_(nM_sample) = alpha2d_tru_(1+nalpha_azimu_b,1+nm)
         alpha_est_(nM_sample) = alpha2d_est_(1+nalpha_azimu_b,1+nm)
      enddo                     !do nM_sample=0,n_M_sample-1
      write(format_string,'(A,I0,A)') '(A,' , n_M_sample , '(F8.4,1X))'
      write(6,format_string) 'nalpha_azimu_b ori: '
     $     ,(alpha_ori_(nM_sample),nM_sample=0,n_M_sample-1)
      write(6,format_string) 'nalpha_azimu_b tru: '
     $     ,(alpha_tru_(nM_sample),nM_sample=0,n_M_sample-1)
      write(6,format_string) 'nalpha_azimu_b est: '
     $     ,(alpha_est_(nM_sample),nM_sample=0,n_M_sample-1)

      if (verbose.gt.0) write(6,*)
     $     'copying nalpha_gamma_z: alpha2d-->alpha'
      do nM_sample=0,n_M_sample-1
         nm = I_M_sample_(nM_sample)
         alpha_ori_(nM_sample) = alpha2d_ori_(1+nalpha_gamma_z,1+nm)
         alpha_tru_(nM_sample) = alpha2d_tru_(1+nalpha_gamma_z,1+nm)
         alpha_est_(nM_sample) = alpha2d_est_(1+nalpha_gamma_z,1+nm)
      enddo                     !do nM_sample=0,n_M_sample-1
      write(format_string,'(A,I0,A)') '(A,' , n_M_sample , '(F8.4,1X))'
      write(6,format_string) 'nalpha_gamma_z ori: '
     $     ,(alpha_ori_(nM_sample),nM_sample=0,n_M_sample-1)
      write(6,format_string) 'nalpha_gamma_z tru: '
     $     ,(alpha_tru_(nM_sample),nM_sample=0,n_M_sample-1)
      write(6,format_string) 'nalpha_gamma_z est: '
     $     ,(alpha_est_(nM_sample),nM_sample=0,n_M_sample-1)

      if (verbose.gt.0) write(6,*)
     $     'copying nalpha_delta_x: alpha2d-->alpha'
      do nM_sample=0,n_M_sample-1
         nm = I_M_sample_(nM_sample)
         alpha_ori_(nM_sample) = alpha2d_ori_(1+nalpha_delta_x,1+nm)
         alpha_tru_(nM_sample) = alpha2d_tru_(1+nalpha_delta_x,1+nm)
         alpha_est_(nM_sample) = alpha2d_est_(1+nalpha_delta_x,1+nm)
      enddo                     !do nM_sample=0,n_M_sample-1
      write(format_string,'(A,I0,A)') '(A,' , n_M_sample , '(F8.4,1X))'
      write(6,format_string) 'nalpha_delta_x ori: '
     $     ,(alpha_ori_(nM_sample),nM_sample=0,n_M_sample-1)
      write(6,format_string) 'nalpha_delta_x tru: '
     $     ,(alpha_tru_(nM_sample),nM_sample=0,n_M_sample-1)
      write(6,format_string) 'nalpha_delta_x est: '
     $     ,(alpha_est_(nM_sample),nM_sample=0,n_M_sample-1)

      if (verbose.gt.0) write(6,*)
     $     'copying nalpha_delta_y: alpha2d-->alpha'
      do nM_sample=0,n_M_sample-1
         nm = I_M_sample_(nM_sample)
         alpha_ori_(nM_sample) = alpha2d_ori_(1+nalpha_delta_y,1+nm)
         alpha_tru_(nM_sample) = alpha2d_tru_(1+nalpha_delta_y,1+nm)
         alpha_est_(nM_sample) = alpha2d_est_(1+nalpha_delta_y,1+nm)
      enddo                     !do nM_sample=0,n_M_sample-1
      write(format_string,'(A,I0,A)') '(A,' , n_M_sample , '(F8.4,1X))'
      write(6,format_string) 'nalpha_delta_y ori: '
     $     ,(alpha_ori_(nM_sample),nM_sample=0,n_M_sample-1)
      write(6,format_string) 'nalpha_delta_y tru: '
     $     ,(alpha_tru_(nM_sample),nM_sample=0,n_M_sample-1)
      write(6,format_string) 'nalpha_delta_y est: '
     $     ,(alpha_est_(nM_sample),nM_sample=0,n_M_sample-1)

      if (verbose.gt.0) write(6,*)
     $     'copying nalpha_l2_norm: alpha2d-->alpha'
      do nM_sample=0,n_M_sample-1
         nm = I_M_sample_(nM_sample)
         alpha_ori_(nM_sample) = alpha2d_ori_(1+nalpha_l2_norm,1+nm)
         alpha_tru_(nM_sample) = alpha2d_tru_(1+nalpha_l2_norm,1+nm)
         alpha_est_(nM_sample) = alpha2d_est_(1+nalpha_l2_norm,1+nm)
      enddo                     !do nM_sample=0,n_M_sample-1
      write(format_string,'(A,I0,A)') '(A,' , n_M_sample , '(F8.4,1X))'
      write(6,format_string) 'nalpha_l2_norm ori: '
     $     ,(alpha_ori_(nM_sample),nM_sample=0,n_M_sample-1)
      write(6,format_string) 'nalpha_l2_norm tru: '
     $     ,(alpha_tru_(nM_sample),nM_sample=0,n_M_sample-1)
      write(6,format_string) 'nalpha_l2_norm est: '
     $     ,(alpha_est_(nM_sample),nM_sample=0,n_M_sample-1)
      
      if (verbose.gt.0) write(6,*)
     $     'copying nalpha_ctf_ind: alpha2d-->alpha'
      do nM_sample=0,n_M_sample-1
         nm = I_M_sample_(nM_sample)
         alpha_ori_(nM_sample) = alpha2d_ori_(1+nalpha_ctf_ind,1+nm)
         alpha_tru_(nM_sample) = alpha2d_tru_(1+nalpha_ctf_ind,1+nm)
         alpha_est_(nM_sample) = alpha2d_est_(1+nalpha_ctf_ind,1+nm)
      enddo                     !do nM_sample=0,n_M_sample-1
      write(format_string,'(A,I0,A)') '(A,' , n_M_sample , '(F8.4,1X))'
      write(6,format_string) 'nalpha_ctf_ind ori: '
     $     ,(alpha_ori_(nM_sample),nM_sample=0,n_M_sample-1)
      write(6,format_string) 'nalpha_ctf_ind tru: '
     $     ,(alpha_tru_(nM_sample),nM_sample=0,n_M_sample-1)
      write(6,format_string) 'nalpha_ctf_ind est: '
     $     ,(alpha_est_(nM_sample),nM_sample=0,n_M_sample-1)
         
 10   continue
      if (verbose.gt.1) then
         write(6,'(A)') '[finished test_ver5cI]'
      end if
      stop
      end
