...
str_thisfunction = 'convert_k_p_to_spharm_3';
...
weight_k_sub_ = reshape(weight_shell_k_(1+index_sub_)/k_p_r^2,[n_sub,1]);
...
a_k_Y_sub_ = conj(Ylm_lma__)*(a_k_all_sub_.*weight_k_sub_);
...
tmp_t = toc(tmp_t); disp(sprintf(' %% convert_k_p_to_spharm_3 (pre): %0.6fs',tmp_t));
disp(sprintf(' %% a_k_Y_quad2_ vs a_k_Y_quad1_: %0.16f',fnorm(a_k_Y_quad2_ - a_k_Y_quad1_)/fnorm(a_k_Y_quad1_)));
disp(sprintf(' %% a_k_Y_quad2_ vs a_k_Y_quad_: %0.16f',fnorm(a_k_Y_quad2_ - a_k_Y_quad_)/fnorm(a_k_Y_quad_)));
%%%%%%%%;
