c$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
      subroutine test_innerproduct_batch_stage_1a(verbose
...
c$$$      parameters for timing
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[entering test_innerproduct_batch_stage_1a]'
      end if      
      if (verbose.gt.0) then
...
c$$$      enddo
     ...
     $        ,' too large in test_innerproduct_batch_stage_1a'
      end if
     ...
     $        ,' too small in test_innerproduct_batch_stage_1a'
      end if
...
c$$$      end if
     ...
     $        *ld_M),C_M)
         ...
         call rotate_p_to_p(n_r,n_w_,n_A,T_p_,+gamma_z,T_p_)
         call transf_p_to_p(n_r,grid_p_,n_w_,n_A,T_p_,+delta_x,+delta_y
     $        ,T_p_)
         call interp_p_to_q_fftw(n_r,fftw_plan_frwd_,n_w_,n_A,fftw_in1_
     $        ,fftw_out_,T_p_,M_q_)
         timing_toc_1 = second()
     ...
     $              timing_tic_2)
               timing_tic_3 = second()
               call test_innerproduct_batch_excerpt_0a(n_delta_x
     ...
     $              *n_gamma_z + nctf*n_gamma_z*n_S),Z_q_)
               timing_toc_3 = second()
     ...
     $              timing_tic_2)
               timing_tic_3 = second()
               call test_innerproduct_batch_excerpt_1a(n_delta_x
     $              ,n_delta_y,n_gamma_z,gamma_z_,n_w_max,n_svd_l
     ...
     $              timing_tic_3)
               ...
               timing_tic_3 = second()
               call test_innerproduct_batch_excerpt_2a(n_delta_x
     ...
     $              nctf*n_gamma_z*n_S),S_p_(ns*ld_S),M_q_,Z_q_)
               timing_toc_3 = second()
               timing_tot_3 = timing_tot_3 + (timing_toc_3 -
     $              timing_tic_3)
            end if !svd_calculation_type
            timing_tic_4 = second()
            call test_innerproduct_batch_excerpt_4a(delta_x_est_(nm)
     ...
     $           ,ngz_max ,C_Z_max)
            ...
            C_S_max_(ns + nm*n_S) = C_S_(ngz_max + ns*n_gamma_z + nctf
     $           *n_gamma_z*n_S)
...
c$$$      include './dir_gen_Jsvd_1/gen_Jsvd_svdfree.txt'
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[finished test_innerproduct_batch_stage_1a]'
      end if
