...
c     
      ...
      integer n_M_sample,nM_sample,nM_sample_sub,n_S_sample,nS_sample
      integer *4, allocatable :: n_M_sample_sub_(:)
      integer *4, allocatable :: I_M_sample_sub_(:)
      ...
      do kk = 1,nlats(ngridr)
         ...
         do ll = 1,ngridps(kk)
            ...
            if (displacement_flag) then
               ...
               alpha5_tru_(4,nnn) = cos(angle_tmp)*delta_tmp
               alpha5_tru_(5,nnn) = sin(angle_tmp)*delta_tmp
            else
               alpha5_tru_(4,nnn) = 0.0d0/rmax
               alpha5_tru_(5,nnn) = 0.0d0/rmax
            end if
...
c$$$      n_M_sample_max = nimages
      n_M_sample_max = 512
      allocate(n_M_sample_sub_(0:n_M_sample_max-1))
      allocate(I_M_sample_sub_(0:n_M_sample_max-1))
...
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        ,modsph_tru)
         ...
         if (verbose.gt.0) then
            write(6,'(A,A,F8.3)') 'rebuild_model_alpha5 (tru):'
...
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        ,modsph_est)
         timing_toc = second()
         if (verbose.gt.0) then
            write(6,'(A,A,F8.3)') 'rebuild_model_alpha5 (est):'
     $           ,' total_time ',timing_toc-timing_tic
         end if
...
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
c$$$         generate templates based on current model
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
...
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        *n_S_sample,') innerproducts'
         timing_tic = second()
         n_M_sample_sub = 8
         do nM_sample_sub=0,n_M_sample_sub-1
            if (nM_sample_sub.eq.0) then
               n_M_sample_sub_(0) = n_M_sample/n_M_sample_sub
               I_M_sample_sub_(0) = 0
            else if (nM_sample_sub.gt.0 .and.
     $              nM_sample_sub.lt.n_M_sample_sub-1) then
               n_M_sample_sub_(nM_sample_sub) = n_M_sample
     $              /n_M_sample_sub
               I_M_sample_sub_(nM_sample_sub) =
     $              I_M_sample_sub_(nM_sample_sub-1) +
     $              n_M_sample_sub_(nM_sample_sub-1)
            else if (nM_sample_sub.eq.n_M_sample_sub-1) then
               I_M_sample_sub_(n_M_sample_sub-1) =
     $              I_M_sample_sub_(n_M_sample_sub-2) +
     $              n_M_sample_sub_(n_M_sample_sub-2)
               n_M_sample_sub_(n_M_sample_sub-1) = n_M_sample -
     $              I_M_sample_sub_(n_M_sample_sub-1)
            end if
         enddo
         if (verbose.gt.0) then
            write(format_string,'(A,I0,A)') '(A,',n_M_sample_sub
     $           ,'(I0,1X))'
            write(6,format_string) 'n_M_sample_sub_: '
     $           ,(n_M_sample_sub_(nM_sample_sub),nM_sample_sub=0
     $           ,n_M_sample_sub-1)
            write(6,format_string) 'I_M_sample_sub_: '
     $           ,(I_M_sample_sub_(nM_sample_sub),nM_sample_sub=0
     $           ,n_M_sample_sub-1)
         end if
         wtime = omp_get_wtime()
c$OMP PARALLEL PRIVATE(n_tmp,i_tmp)
c$OMP DO 
         do nM_sample_sub=0,n_M_sample_sub-1
            n_tmp = n_M_sample_sub_(nM_sample_sub)
            i_tmp = I_M_sample_sub_(nM_sample_sub)
            call test_innerproduct_batch_wrapper1(0,ncur,nlats,xnodesr
     $           ,max_x_c,n_S_sample,ntemplatesize,S_sample_,n_tmp
     $           ,nimage_size,M_sample_(nimage_size*i_tmp),alpha_est_(5
     $           *i_tmp),0.1d0,N_pixels_in,n_delta_x,n_delta_y,max(32,2
     $           *ncur), -1 ,delta_x_sort_(n_S_sample*i_tmp)
     $           ,delta_y_sort_(n_S_sample*i_tmp)
     $           ,gamma_z_sort_(n_S_sample*i_tmp),C_Z_sort_(n_S_sample
     $           *i_tmp),I_permute_(n_S_sample*i_tmp)
     $           ,I_inverse_(n_S_sample*i_tmp))
         enddo
c$OMP END DO
c$OMP END PARALLEL
         wtime = omp_get_wtime() - wtime
         write(6,'(A,G14.6)') 'wtime: ', wtime

c$$$         call test_innerproduct_batch_wrapper1(0,ncur,nlats
c$$$     $        ,xnodesr,max_x_c,n_S_sample,ntemplatesize,S_sample_
c$$$     $        ,n_M_sample,nimage_size,M_sample_,alpha_est_,0.1d0,0.5d0
c$$$     $        ,5,5,max(32,2*ncur),-1,delta_x_sort_,delta_y_sort_
...
c$$$     $        ,gamma_z_sort_,C_Z_sort_,I_permute_,I_inverse_)
         ...
         d_(0) = n_S_sample
         d_(1) = n_M_sample
         write(fname,'(A,I0,A)') './dir_mda/C_Z_sort_',ncur,'_.mda'
         if (verbose.gt.1) write(6,'(A,A)') 'Writing c16 : ',trim(fname)
         call MDA_write_c16(2,d_,C_Z_sort_,fname)
         timing_toc = second()
         if (verbose.gt.0) then
            write(6,'(A,A,F8.3)') 'test_innerproduct_batch_wrapper1:'
     $           ,' total_time ',timing_toc-timing_tic
         end if
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
c$$$         print out a subset of image-template pairs
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         write(fname,'(A,I0)') './dir_jpg/Fig_MTS_',ncur
         call Fig_gen_ver1(ncur,nlats,xnodesr,n_S_sample,ntemplatesize
     $        ,S_sample_,n_M_sample,nimage_size,M_sample_,alpha_est_
     $        ,delta_x_sort_,delta_y_sort_,gamma_z_sort_,I_permute_
     $        ,min(8,n_M_sample),fname)
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
c$$$         update alphas
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         if (verbose.gt.0) write(6,*) 'update alphas'
         if (displacement_max.le.0) then
            do nm=0,n_M_sample-1
               n_use = floor((1 - rand()**4)*n_S_sample)
               n_use = max(0,min(n_S_sample-1,n_use))
               ns = I_permute_(n_use + nm*n_S_sample)
               alpha_est_(0 + nm*5) = alpha_0_sample_(ns)
               alpha_est_(1 + nm*5) = alpha_1_sample_(ns)
               alpha_est_(2 + nm*5) = alpha_est_(2 + nm*5) +
     $              gamma_z_sort_(n_use + nm*n_S_sample)
               alpha_est_(3 + nm*5) = alpha_est_(3 + nm*5) +
     $              delta_x_sort_(n_use + nm*n_S_sample)
               alpha_est_(4 + nm*5) = alpha_est_(4 + nm*5) +
     $              delta_y_sort_(n_use + nm*n_S_sample)
            enddo
         else !if (displacement_max.gt.0) then
c$$$            fix later
            do nm=0,n_M_sample-1
               n_use = floor((1 - rand()**4)*n_S_sample)
               n_use = max(0,min(n_S_sample-1,n_use))
     ...
     $              delta_x_sort_(n_use + nm*n_S_sample)
               alpha_est_(4 + nm*5) = alpha_est_(4 + nm*5) +
     $              delta_y_sort_(n_use + nm*n_S_sample)
               delta_tmp = dsqrt(alpha_est_(3 + nm*5)**2 + alpha_est_(3
     $              + nm*5)**2)
c$$$            fix later -- should account for displacement boundary
c$$$            within innerproduct routine, then again outside.
               if (delta_tmp.gt.displacement_max) then
                  alpha_est_(0 + nm*5) = alpha_0_sample_(ns)
                  alpha_est_(1 + nm*5) = alpha_1_sample_(ns)
                  alpha_est_(2 + nm*5) = 2*pi*rand()
                  alpha_est_(3 + nm*5) = 0.0d0
                  alpha_est_(4 + nm*5) = 0.0d0
               end if
            enddo
            
         end if !displacement_max
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
