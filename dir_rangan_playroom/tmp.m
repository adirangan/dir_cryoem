%%%%%%%%;
% comparing test_tfpmut_wrap_6: ;
%%%%%%%%;
flag_verbose=1; flag_disp=1;nf=0;
prefix_matlab = '/data/rangan/dir_cryoem/dir_FINYU_nx64s003nM1024nCTF50p40V300D25000r050A010/dir_pm_mat/test_tfpmut_wrap_6_XB_from_matlab';
prefix_python = '/data/rangan/dir_cryoem/dir_FINYU_nx64s003nM1024nCTF50p40V300D25000r050A010/dir_pm_mat/test_tfpmut_wrap_6_XB_from_python';

fname_matlab_0 = sprintf('%s_stage_0.mat',prefix_matlab);
fname_python_0 = sprintf('%s_stage_0.mat',prefix_python);
if exist(fname_matlab_0,'file') & exist(fname_python_0,'file');
tmp_matlab_ = load(fname_matlab_0);
tmp_python_ = load(fname_python_0);
fnorm_disp(flag_verbose,'tmp_matlab_.n_w_',tmp_matlab_.n_w_,'double(tmp_python_.n_w_)',double(tmp_python_.n_w_));
fnorm_disp(flag_verbose,'tmp_matlab_.n_w_csum_',tmp_matlab_.n_w_csum_,'double(tmp_python_.n_w_csum_)',double(tmp_python_.n_w_csum_));
fnorm_disp(flag_verbose,'tmp_matlab_.n_y_',tmp_matlab_.n_y_,'double(tmp_python_.n_y_)',double(tmp_python_.n_y_));
fnorm_disp(flag_verbose,'tmp_matlab_.n_y_csum_',tmp_matlab_.n_y_csum_,'double(tmp_python_.n_y_csum_)',double(tmp_python_.n_y_csum_));
end;%if exist(fname_matlab_0,'file') & exist(fname_python_0,'file');

fname_matlab_1 = sprintf('%s_stage_1.mat',prefix_matlab);
fname_python_1 = sprintf('%s_stage_1.mat',prefix_python);
if exist(fname_matlab_1,'file') & exist(fname_python_1,'file');
tmp_matlab_ = load(fname_matlab_1);
tmp_python_ = load(fname_python_1);
fnorm_disp(flag_verbose,'tmp_matlab_.k_p_r_',tmp_matlab_.k_p_r_,'double(tmp_python_.k_p_r_)',double(tmp_python_.k_p_r_));
fnorm_disp(flag_verbose,'tmp_matlab_.weight_2d_k_p_r_',tmp_matlab_.weight_2d_k_p_r_,'double(tmp_python_.weight_2d_k_p_r_)',double(tmp_python_.weight_2d_k_p_r_));
fnorm_disp(flag_verbose,'tmp_matlab_.CTF_k_p_r_kC__',tmp_matlab_.CTF_k_p_r_kC__,'double(tmp_python_.CTF_k_p_r_kC__)',double(tmp_python_.CTF_k_p_r_kC__));
fnorm_disp(flag_verbose,'tmp_matlab_.n_index_nM_from_ncluster_',tmp_matlab_.n_index_nM_from_ncluster_,'double(tmp_python_.n_index_nM_from_ncluster_)',double(tmp_python_.n_index_nM_from_ncluster_));
fnorm_disp(flag_verbose,'tmp_matlab_.index_ncluster_from_nCTF_',tmp_matlab_.index_ncluster_from_nCTF_,'double(tmp_python_.index_ncluster_from_nCTF_)',double(tmp_python_.index_ncluster_from_nCTF_));
flag_disp=0;
if flag_disp;
n_cluster = numel(tmp_matlab_.index_nM_from_ncluster__);
for ncluster=0:n_cluster-1;
fnorm_disp(flag_verbose,'tmp_matlab_.index_nM_from_ncluster__{1+ncluster}',tmp_matlab_.index_nM_from_ncluster__{1+ncluster},'double(tmp_python_.index_nM_from_ncluster__{1+ncluster})',double(tmp_python_.index_nM_from_ncluster__{1+ncluster}));
end;%for ncluster=0:n_cluster-1;
end;%if flag_disp;
end;%if exist(fname_matlab_1,'file') & exist(fname_python_1,'file');

fname_matlab_2 = sprintf('%s_stage_2.mat',prefix_matlab);
fname_python_2 = sprintf('%s_stage_2.mat',prefix_python);
if exist(fname_matlab_2,'file') & exist(fname_python_2,'file');
tmp_matlab_ = load(fname_matlab_2);
tmp_python_ = load(fname_python_2);
fnorm_disp(flag_verbose,'tmp_matlab_.pm_UX_knc___',tmp_matlab_.pm_UX_knc___,'double(tmp_python_.pm_UX_knc___)',double(tmp_python_.pm_UX_knc___));
fnorm_disp(flag_verbose,'tmp_matlab_.pm_SX_kc__',tmp_matlab_.pm_SX_kc__,'double(tmp_python_.pm_SX_kc__)',double(tmp_python_.pm_SX_kc__));
fnorm_disp(flag_verbose,'tmp_matlab_.pm_n_UX_rank_c_',tmp_matlab_.pm_n_UX_rank_c_,'double(tmp_python_.pm_n_UX_rank_c_)',double(tmp_python_.pm_n_UX_rank_c_));
%%%%;
flag_disp=0;
if flag_disp;
figure(1+nf);nf=nf+1;clf;figbig;
n_cluster = size(tmp_matlab_.pm_UX_knc___,1+2);
p_row = 6; p_col = ceil(n_cluster/p_row); np=0;
for ncluster=0:n_cluster-1;
subplot(p_row,p_col,1+np);np=np+1;cla;
imagesc(tmp_matlab_.pm_UX_knc___(:,:,1+ncluster)); axisnotick; drawnow();
end;%for ncluster=0:n_cluster-1;
end;%if flag_disp;
%%%%;
if flag_disp;
figure(1+nf);nf=nf+1;clf;figbig;
n_cluster = size(tmp_python_.pm_UX_knc___,1+2);
p_row = 6; p_col = ceil(n_cluster/p_row); np=0;
for ncluster=0:n_cluster-1;
subplot(p_row,p_col,1+np);np=np+1;cla;
imagesc(tmp_python_.pm_UX_knc___(:,:,1+ncluster)); axisnotick; drawnow();
end;%for ncluster=0:n_cluster-1;
end;%if flag_disp;
%%%%;
end;%if exist(fname_matlab_2,'file') & exist(fname_python_2,'file');

fname_matlab_3 = sprintf('%s_stage_3.mat',prefix_matlab);
fname_python_3 = sprintf('%s_stage_3.mat',prefix_python);
if exist(fname_matlab_3,'file') & exist(fname_python_3,'file');
tmp_matlab_ = load(fname_matlab_3);
tmp_python_ = load(fname_python_3);
fnorm_disp(flag_verbose,'tmp_matlab_.n_k_p_r',tmp_matlab_.n_k_p_r,'double(tmp_python_.n_k_p_r)',double(tmp_python_.n_k_p_r));
fnorm_disp(flag_verbose,'tmp_matlab_.k_p_r_',tmp_matlab_.k_p_r_,'double(tmp_python_.k_p_r_)',double(tmp_python_.k_p_r_));
fnorm_disp(flag_verbose,'tmp_matlab_.k_p_r_max',tmp_matlab_.k_p_r_max,'double(tmp_python_.k_p_r_max)',double(tmp_python_.k_p_r_max));
fnorm_disp(flag_verbose,'tmp_matlab_.weight_3d_k_p_r_',tmp_matlab_.weight_3d_k_p_r_,'double(tmp_python_.weight_3d_k_p_r_)',double(tmp_python_.weight_3d_k_p_r_));
fnorm_disp(flag_verbose,'tmp_matlab_.weight_2d_k_p_r_',tmp_matlab_.weight_2d_k_p_r_,'double(tmp_python_.weight_2d_k_p_r_)',double(tmp_python_.weight_2d_k_p_r_));
fnorm_disp(flag_verbose,'tmp_matlab_.n_w_',tmp_matlab_.n_w_,'double(tmp_python_.n_w_)',double(tmp_python_.n_w_));
fnorm_disp(flag_verbose,'tmp_matlab_.weight_2d_k_p_wk_',tmp_matlab_.weight_2d_k_p_wk_,'double(tmp_python_.weight_2d_k_p_wk_)',double(tmp_python_.weight_2d_k_p_wk_));
fnorm_disp(flag_verbose,'tmp_matlab_.l_max_',tmp_matlab_.l_max_,'double(tmp_python_.l_max_)',double(tmp_python_.l_max_));
fnorm_disp(flag_verbose,'tmp_matlab_.n_CTF',tmp_matlab_.n_CTF,'double(tmp_python_.n_CTF)',double(tmp_python_.n_CTF));
fnorm_disp(flag_verbose,'tmp_matlab_.CTF_k_p_wkC__',tmp_matlab_.CTF_k_p_wkC__,'double(tmp_python_.CTF_k_p_wkC__)',double(tmp_python_.CTF_k_p_wkC__));
fnorm_disp(flag_verbose,'tmp_matlab_.index_nCTF_from_nM_',tmp_matlab_.index_nCTF_from_nM_,'double(tmp_python_.index_nCTF_from_nM_)',double(tmp_python_.index_nCTF_from_nM_));
fnorm_disp(flag_verbose,'tmp_matlab_.n_M',tmp_matlab_.n_M,'double(tmp_python_.n_M)',double(tmp_python_.n_M));
fnorm_disp(flag_verbose,'tmp_matlab_.M_k_p_wkM__',tmp_matlab_.M_k_p_wkM__,'double(tmp_python_.M_k_p_wkM__)',double(tmp_python_.M_k_p_wkM__));
fnorm_disp(flag_verbose,'tmp_matlab_.n_cluster',tmp_matlab_.n_cluster,'double(tmp_python_.n_cluster)',double(tmp_python_.n_cluster));
fnorm_disp(flag_verbose,'tmp_matlab_.index_ncluster_from_nCTF_',tmp_matlab_.index_ncluster_from_nCTF_,'double(tmp_python_.index_ncluster_from_nCTF_)',double(tmp_python_.index_ncluster_from_nCTF_));
fnorm_disp(flag_verbose,'tmp_matlab_.pm_n_UX_rank_c_',tmp_matlab_.pm_n_UX_rank_c_,'double(tmp_python_.pm_n_UX_rank_c_)',double(tmp_python_.pm_n_UX_rank_c_));
fnorm_disp(flag_verbose,'tmp_matlab_.pm_UX_knc___',tmp_matlab_.pm_UX_knc___,'double(tmp_python_.pm_UX_knc___)',double(tmp_python_.pm_UX_knc___));
fnorm_disp(flag_verbose,'tmp_matlab_.pm_X_weight_rc__',tmp_matlab_.pm_X_weight_rc__,'double(tmp_python_.pm_X_weight_rc__)',double(tmp_python_.pm_X_weight_rc__));
fnorm_disp(flag_verbose,'tmp_matlab_.euler_polar_a_M_',tmp_matlab_.euler_polar_a_M_,'double(tmp_python_.euler_polar_a_M_)',double(tmp_python_.euler_polar_a_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.euler_azimu_b_M_',tmp_matlab_.euler_azimu_b_M_,'double(tmp_python_.euler_azimu_b_M_)',double(tmp_python_.euler_azimu_b_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.euler_gamma_z_M_',tmp_matlab_.euler_gamma_z_M_,'double(tmp_python_.euler_gamma_z_M_)',double(tmp_python_.euler_gamma_z_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_x_acc_M_',tmp_matlab_.image_delta_x_acc_M_,'double(tmp_python_.image_delta_x_acc_M_)',double(tmp_python_.image_delta_x_acc_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_y_acc_M_',tmp_matlab_.image_delta_y_acc_M_,'double(tmp_python_.image_delta_y_acc_M_)',double(tmp_python_.image_delta_y_acc_M_));
end;%if exist(fname_matlab_3,'file') & exist(fname_python_3,'file');

fname_matlab_4 = sprintf('%s_stage_4.mat',prefix_matlab);
fname_python_4 = sprintf('%s_stage_4.mat',prefix_python);
if exist(fname_matlab_4,'file') & exist(fname_python_4,'file');
tmp_matlab_ = load(fname_matlab_4);
tmp_python_ = load(fname_python_4);
fnorm_disp(flag_verbose,'tmp_matlab_.flag_verbose',tmp_matlab_.flag_verbose,'double(tmp_python_.flag_verbose)',double(tmp_python_.flag_verbose));
fnorm_disp(flag_verbose,'tmp_matlab_.tolerance_master',tmp_matlab_.tolerance_master,'double(tmp_python_.tolerance_master)',double(tmp_python_.tolerance_master));
fnorm_disp(flag_verbose,'tmp_matlab_.flag_gpu',tmp_matlab_.flag_gpu,'double(tmp_python_.flag_gpu)',double(tmp_python_.flag_gpu));
fnorm_disp(flag_verbose,'tmp_matlab_.rseed',tmp_matlab_.rseed,'double(tmp_python_.rseed)',double(tmp_python_.rseed));
fnorm_disp(flag_verbose,'tmp_matlab_.n_iteration',tmp_matlab_.n_iteration,'double(tmp_python_.n_iteration)',double(tmp_python_.n_iteration));
fnorm_disp(flag_verbose,'tmp_matlab_.n_delta_v_requested',tmp_matlab_.n_delta_v_requested,'double(tmp_python_.n_delta_v_requested)',double(tmp_python_.n_delta_v_requested));
fnorm_disp(flag_verbose,'tmp_matlab_.flag_MS_vs_SM',tmp_matlab_.flag_MS_vs_SM,'double(tmp_python_.flag_MS_vs_SM)',double(tmp_python_.flag_MS_vs_SM));
fnorm_disp(flag_verbose,'tmp_matlab_.order_limit_MS',tmp_matlab_.order_limit_MS,'double(tmp_python_.order_limit_MS)',double(tmp_python_.order_limit_MS));
fnorm_disp(flag_verbose,'tmp_matlab_.delta_r_max',tmp_matlab_.delta_r_max,'double(tmp_python_.delta_r_max)',double(tmp_python_.delta_r_max));
fnorm_disp(flag_verbose,'tmp_matlab_.delta_r_upb',tmp_matlab_.delta_r_upb,'double(tmp_python_.delta_r_upb)',double(tmp_python_.delta_r_upb));
fnorm_disp(flag_verbose,'tmp_matlab_.template_viewing_k_eq_d',tmp_matlab_.template_viewing_k_eq_d,'double(tmp_python_.template_viewing_k_eq_d)',double(tmp_python_.template_viewing_k_eq_d));
fnorm_disp(flag_verbose,'tmp_matlab_.flag_alternate_MS_vs_SM',tmp_matlab_.flag_alternate_MS_vs_SM,'double(tmp_python_.flag_alternate_MS_vs_SM)',double(tmp_python_.flag_alternate_MS_vs_SM));
fnorm_disp(flag_verbose,'tmp_matlab_.svd_eps',tmp_matlab_.svd_eps,'double(tmp_python_.svd_eps)',double(tmp_python_.svd_eps));
fnorm_disp(flag_verbose,'tmp_matlab_.n_w_',tmp_matlab_.n_w_,'double(tmp_python_.n_w_)',double(tmp_python_.n_w_));
fnorm_disp(flag_verbose,'tmp_matlab_.n_w_sum',tmp_matlab_.n_w_sum,'double(tmp_python_.n_w_sum)',double(tmp_python_.n_w_sum));
fnorm_disp(flag_verbose,'tmp_matlab_.n_w_max',tmp_matlab_.n_w_max,'double(tmp_python_.n_w_max)',double(tmp_python_.n_w_max));
fnorm_disp(flag_verbose,'tmp_matlab_.n_w_csum_',tmp_matlab_.n_w_csum_,'double(tmp_python_.n_w_csum_)',double(tmp_python_.n_w_csum_));
fnorm_disp(flag_verbose,'tmp_matlab_.l_max_max',tmp_matlab_.l_max_max,'double(tmp_python_.l_max_max)',double(tmp_python_.l_max_max));
fnorm_disp(flag_verbose,'tmp_matlab_.n_y_',tmp_matlab_.n_y_,'double(tmp_python_.n_y_)',double(tmp_python_.n_y_));
fnorm_disp(flag_verbose,'tmp_matlab_.n_y_max',tmp_matlab_.n_y_max,'double(tmp_python_.n_y_max)',double(tmp_python_.n_y_max));
fnorm_disp(flag_verbose,'tmp_matlab_.n_y_sum',tmp_matlab_.n_y_sum,'double(tmp_python_.n_y_sum)',double(tmp_python_.n_y_sum));
fnorm_disp(flag_verbose,'tmp_matlab_.n_y_csum_',tmp_matlab_.n_y_csum_,'double(tmp_python_.n_y_csum_)',double(tmp_python_.n_y_csum_));
fnorm_disp(flag_verbose,'tmp_matlab_.n_cluster',tmp_matlab_.n_cluster,'double(tmp_python_.n_cluster)',double(tmp_python_.n_cluster));
fnorm_disp(flag_verbose,'tmp_matlab_.index_ncluster_from_nM_',tmp_matlab_.index_ncluster_from_nM_,'double(tmp_python_.index_ncluster_from_nM_)',double(tmp_python_.index_ncluster_from_nM_));
%fnorm_disp(flag_verbose,'tmp_matlab_.index_nM_from_ncluster__',tmp_matlab_.index_nM_from_ncluster__,'double(tmp_python_.index_nM_from_ncluster__)',double(tmp_python_.index_nM_from_ncluster__));
fnorm_disp(flag_verbose,'tmp_matlab_.n_index_nM_from_ncluster_',tmp_matlab_.n_index_nM_from_ncluster_,'double(tmp_python_.n_index_nM_from_ncluster_)',double(tmp_python_.n_index_nM_from_ncluster_));
fnorm_disp(flag_verbose,'tmp_matlab_.CTF_k_p_wkC__',tmp_matlab_.CTF_k_p_wkC__,'double(tmp_python_.CTF_k_p_wkC__)',double(tmp_python_.CTF_k_p_wkC__));
fnorm_disp(flag_verbose,'tmp_matlab_.n_CTF',tmp_matlab_.n_CTF,'double(tmp_python_.n_CTF)',double(tmp_python_.n_CTF));
fnorm_disp(flag_verbose,'tmp_matlab_.CTF_k_p_r_kC__',tmp_matlab_.CTF_k_p_r_kC__,'double(tmp_python_.CTF_k_p_r_kC__)',double(tmp_python_.CTF_k_p_r_kC__));
fnorm_disp(flag_verbose,'tmp_matlab_.CTF_k_p_wkM__',tmp_matlab_.CTF_k_p_wkM__,'double(tmp_python_.CTF_k_p_wkM__)',double(tmp_python_.CTF_k_p_wkM__));
fnorm_disp(flag_verbose,'tmp_matlab_.CTF_k_p_r_xavg_kc__',tmp_matlab_.CTF_k_p_r_xavg_kc__,'double(tmp_python_.CTF_k_p_r_xavg_kc__)',double(tmp_python_.CTF_k_p_r_xavg_kc__));
fnorm_disp(flag_verbose,'tmp_matlab_.n_delta_v',tmp_matlab_.n_delta_v,'double(tmp_python_.n_delta_v)',double(tmp_python_.n_delta_v));
fnorm_disp(flag_verbose,'tmp_matlab_.n_svd_l',tmp_matlab_.n_svd_l,'double(tmp_python_.n_svd_l)',double(tmp_python_.n_svd_l));
fnorm_disp(flag_verbose,'tmp_matlab_.euler_polar_a_Mi__',tmp_matlab_.euler_polar_a_Mi__,'double(tmp_python_.euler_polar_a_Mi__)',double(tmp_python_.euler_polar_a_Mi__));
fnorm_disp(flag_verbose,'tmp_matlab_.euler_azimu_b_Mi__',tmp_matlab_.euler_azimu_b_Mi__,'double(tmp_python_.euler_azimu_b_Mi__)',double(tmp_python_.euler_azimu_b_Mi__));
fnorm_disp(flag_verbose,'tmp_matlab_.euler_gamma_z_Mi__',tmp_matlab_.euler_gamma_z_Mi__,'double(tmp_python_.euler_gamma_z_Mi__)',double(tmp_python_.euler_gamma_z_Mi__));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_x_acc_Mi__',tmp_matlab_.image_delta_x_acc_Mi__,'double(tmp_python_.image_delta_x_acc_Mi__)',double(tmp_python_.image_delta_x_acc_Mi__));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_y_acc_Mi__',tmp_matlab_.image_delta_y_acc_Mi__,'double(tmp_python_.image_delta_y_acc_Mi__)',double(tmp_python_.image_delta_y_acc_Mi__));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_x_upd_Mi__',tmp_matlab_.image_delta_x_upd_Mi__,'double(tmp_python_.image_delta_x_upd_Mi__)',double(tmp_python_.image_delta_x_upd_Mi__));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_y_upd_Mi__',tmp_matlab_.image_delta_y_upd_Mi__,'double(tmp_python_.image_delta_y_upd_Mi__)',double(tmp_python_.image_delta_y_upd_Mi__));
fnorm_disp(flag_verbose,'tmp_matlab_.flag_image_delta_upd_Mi__',tmp_matlab_.flag_image_delta_upd_Mi__,'double(tmp_python_.flag_image_delta_upd_Mi__)',double(tmp_python_.flag_image_delta_upd_Mi__));
fnorm_disp(flag_verbose,'tmp_matlab_.image_I_value_Mi__',tmp_matlab_.image_I_value_Mi__,'double(tmp_python_.image_I_value_Mi__)',double(tmp_python_.image_I_value_Mi__));
fnorm_disp(flag_verbose,'tmp_matlab_.image_X_value_Mi__',tmp_matlab_.image_X_value_Mi__,'double(tmp_python_.image_X_value_Mi__)',double(tmp_python_.image_X_value_Mi__));
fnorm_disp(flag_verbose,'tmp_matlab_.image_S_index_Mi__',tmp_matlab_.image_S_index_Mi__,'double(tmp_python_.image_S_index_Mi__)',double(tmp_python_.image_S_index_Mi__));
fnorm_disp(flag_verbose,'tmp_matlab_.a_k_Y_reco_yki__',tmp_matlab_.a_k_Y_reco_yki__,'double(tmp_python_.a_k_Y_reco_yki__)',double(tmp_python_.a_k_Y_reco_yki__));
fnorm_disp(flag_verbose,'tmp_matlab_.rseed',tmp_matlab_.rseed,'double(tmp_python_.rseed)',double(tmp_python_.rseed));
fnorm_disp(flag_verbose,'tmp_matlab_.euler_polar_a_M_',tmp_matlab_.euler_polar_a_M_,'double(tmp_python_.euler_polar_a_M_)',double(tmp_python_.euler_polar_a_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.euler_azimu_b_M_',tmp_matlab_.euler_azimu_b_M_,'double(tmp_python_.euler_azimu_b_M_)',double(tmp_python_.euler_azimu_b_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.euler_gamma_z_M_',tmp_matlab_.euler_gamma_z_M_,'double(tmp_python_.euler_gamma_z_M_)',double(tmp_python_.euler_gamma_z_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_x_acc_M_',tmp_matlab_.image_delta_x_acc_M_,'double(tmp_python_.image_delta_x_acc_M_)',double(tmp_python_.image_delta_x_acc_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_y_acc_M_',tmp_matlab_.image_delta_y_acc_M_,'double(tmp_python_.image_delta_y_acc_M_)',double(tmp_python_.image_delta_y_acc_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_x_upd_M_',tmp_matlab_.image_delta_x_upd_M_,'double(tmp_python_.image_delta_x_upd_M_)',double(tmp_python_.image_delta_x_upd_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_y_upd_M_',tmp_matlab_.image_delta_y_upd_M_,'double(tmp_python_.image_delta_y_upd_M_)',double(tmp_python_.image_delta_y_upd_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_x_bit_M_',tmp_matlab_.image_delta_x_bit_M_,'double(tmp_python_.image_delta_x_bit_M_)',double(tmp_python_.image_delta_x_bit_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_y_bit_M_',tmp_matlab_.image_delta_y_bit_M_,'double(tmp_python_.image_delta_y_bit_M_)',double(tmp_python_.image_delta_y_bit_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.flag_image_delta_upd_M_',tmp_matlab_.flag_image_delta_upd_M_,'double(tmp_python_.flag_image_delta_upd_M_)',double(tmp_python_.flag_image_delta_upd_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_I_value_M_',tmp_matlab_.image_I_value_M_,'double(tmp_python_.image_I_value_M_)',double(tmp_python_.image_I_value_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.M_pert_k_p_wkM__',tmp_matlab_.M_pert_k_p_wkM__,'double(tmp_python_.M_pert_k_p_wkM__)',double(tmp_python_.M_pert_k_p_wkM__));
fnorm_disp(flag_verbose,'tmp_matlab_.flag_precompute_UX_CTF_S_k_q_wnS__',tmp_matlab_.flag_precompute_UX_CTF_S_k_q_wnS__,'double(tmp_python_.flag_precompute_UX_CTF_S_k_q_wnS__)',double(tmp_python_.flag_precompute_UX_CTF_S_k_q_wnS__));
fnorm_disp(flag_verbose,'tmp_matlab_.flag_precompute_UX_CTF_S_l2_S_',tmp_matlab_.flag_precompute_UX_CTF_S_l2_S_,'double(tmp_python_.flag_precompute_UX_CTF_S_l2_S_)',double(tmp_python_.flag_precompute_UX_CTF_S_l2_S_));
end;%if both exist;

fname_matlab_5_0 = sprintf('%s_stage_5_0.mat',prefix_matlab);
fname_python_5_0 = sprintf('%s_stage_5_0.mat',prefix_python);
if exist(fname_matlab_5_0,'file') & exist(fname_python_5_0,'file');
tmp_matlab_ = load(fname_matlab_5_0);
tmp_python_ = load(fname_python_5_0);
fnorm_disp(flag_verbose,'tmp_matlab_.index_nM_to_update_',tmp_matlab_.index_nM_to_update_,'double(tmp_python_.index_nM_to_update_)',double(tmp_python_.index_nM_to_update_));
fnorm_disp(flag_verbose,'tmp_matlab_.tmp_n_M',tmp_matlab_.tmp_n_M,'double(tmp_python_.tmp_n_M)',double(tmp_python_.tmp_n_M));
fnorm_disp(flag_verbose,'tmp_matlab_.flag_MS_vs_SM',tmp_matlab_.flag_MS_vs_SM,'double(tmp_python_.flag_MS_vs_SM)',double(tmp_python_.flag_MS_vs_SM));
fnorm_disp(flag_verbose,'tmp_matlab_.M_pert_k_p_wkM__',tmp_matlab_.M_pert_k_p_wkM__,'double(tmp_python_.M_pert_k_p_wkM__)',double(tmp_python_.M_pert_k_p_wkM__));
fnorm_disp(flag_verbose,'tmp_matlab_.M_orig_k_p_wkM__',tmp_matlab_.M_orig_k_p_wkM__,'double(tmp_python_.M_orig_k_p_wkM__)',double(tmp_python_.M_orig_k_p_wkM__));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_x_acc_M_',tmp_matlab_.image_delta_x_acc_M_,'double(tmp_python_.image_delta_x_acc_M_)',double(tmp_python_.image_delta_x_acc_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_y_acc_M_',tmp_matlab_.image_delta_y_acc_M_,'double(tmp_python_.image_delta_y_acc_M_)',double(tmp_python_.image_delta_y_acc_M_));
end;%if both exist;

fname_matlab_6_0 = sprintf('%s_stage_6_0.mat',prefix_matlab);
fname_python_6_0 = sprintf('%s_stage_6_0.mat',prefix_python);
if exist(fname_matlab_6_0,'file') & exist(fname_python_6_0,'file');
tmp_matlab_ = load(fname_matlab_6_0);
tmp_python_ = load(fname_python_6_0);
fnorm_disp(flag_verbose,'tmp_matlab_.a_k_Y_reco_yk_',tmp_matlab_.a_k_Y_reco_yk_,'double(tmp_python_.a_k_Y_reco_yk_)',double(tmp_python_.a_k_Y_reco_yk_));
end;%if both exist;

fname_matlab_7_0 = sprintf('%s_stage_7_0.mat',prefix_matlab);
fname_python_7_0 = sprintf('%s_stage_7_0.mat',prefix_python);
if exist(fname_matlab_7_0,'file') & exist(fname_python_7_0,'file');
tmp_matlab_ = load(fname_matlab_7_0);
tmp_python_ = load(fname_python_7_0);
fnorm_disp(flag_verbose,'tmp_matlab_.a_k_Y_reco_yk_',tmp_matlab_.a_k_Y_reco_yk_,'double(tmp_python_.a_k_Y_reco_yk_)',double(tmp_python_.a_k_Y_reco_yk_));
end;%if both exist;

fname_matlab_8_0 = sprintf('%s_stage_8_0.mat',prefix_matlab);
fname_python_8_0 = sprintf('%s_stage_8_0.mat',prefix_python);
if exist(fname_matlab_8_0,'file') & exist(fname_python_8_0,'file');
tmp_matlab_ = load(fname_matlab_8_0);
tmp_python_ = load(fname_python_8_0);
fnorm_disp(flag_verbose,'tmp_matlab_.S_k_p_wkS__',tmp_matlab_.S_k_p_wkS__,'double(tmp_python_.S_k_p_wkS__)',double(tmp_python_.S_k_p_wkS__));
fnorm_disp(flag_verbose,'tmp_matlab_.n_S',tmp_matlab_.n_S,'double(tmp_python_.n_S)',double(tmp_python_.n_S));
fnorm_disp(flag_verbose,'tmp_matlab_.viewing_azimu_b_S_',tmp_matlab_.viewing_azimu_b_S_,'double(tmp_python_.viewing_azimu_b_S_)',double(tmp_python_.viewing_azimu_b_S_));
fnorm_disp(flag_verbose,'tmp_matlab_.viewing_polar_a_S_',tmp_matlab_.viewing_polar_a_S_,'double(tmp_python_.viewing_polar_a_S_)',double(tmp_python_.viewing_polar_a_S_));
end;%if both exist;

fname_matlab_9_0 = sprintf('%s_stage_9_0.mat',prefix_matlab);
fname_python_9_0 = sprintf('%s_stage_9_0.mat',prefix_python);
if exist(fname_matlab_9_0,'file') & exist(fname_python_9_0,'file');
tmp_matlab_ = load(fname_matlab_9_0);
tmp_python_ = load(fname_python_9_0);
fnorm_disp(flag_verbose,'tmp_matlab_.n_k_p_r',tmp_matlab_.n_k_p_r,'double(tmp_python_.n_k_p_r)',double(tmp_python_.n_k_p_r));
fnorm_disp(flag_verbose,'tmp_matlab_.k_p_r_',tmp_matlab_.k_p_r_,'double(tmp_python_.k_p_r_)',double(tmp_python_.k_p_r_));
fnorm_disp(flag_verbose,'tmp_matlab_.k_p_r_max',tmp_matlab_.k_p_r_max,'double(tmp_python_.k_p_r_max)',double(tmp_python_.k_p_r_max));
fnorm_disp(flag_verbose,'tmp_matlab_.n_w_',tmp_matlab_.n_w_,'double(tmp_python_.n_w_)',double(tmp_python_.n_w_));
...
fnorm_disp(flag_verbose,'tmp_matlab_.pm_n_UX_rank_c_',tmp_matlab_.pm_n_UX_rank_c_,'double(tmp_python_.pm_n_UX_rank_c_)',double(tmp_python_.pm_n_UX_rank_c_));
fnorm_disp(flag_verbose,'tmp_matlab_.pm_UX_knc___',tmp_matlab_.pm_UX_knc___,'double(tmp_python_.pm_UX_knc___)',double(tmp_python_.pm_UX_knc___));
fnorm_disp(flag_verbose,'abs(tmp_matlab_.pm_UX_knc___(:,1:5,:))',abs(tmp_matlab_.pm_UX_knc___(:,1:5,:)),'abs(double(tmp_python_.pm_UX_knc___(:,1:5,:)))',abs(double(tmp_python_.pm_UX_knc___(:,1:5,:))));
fnorm_disp(flag_verbose,'tmp_matlab_.pm_X_weight_rc__',tmp_matlab_.pm_X_weight_rc__,'double(tmp_python_.pm_X_weight_rc__)',double(tmp_python_.pm_X_weight_rc__));
fnorm_disp(flag_verbose,'tmp_matlab_.Z_SM__',tmp_matlab_.Z_SM__,'double(tmp_python_.Z_SM__)',double(tmp_python_.Z_SM__));
fnorm_disp(flag_verbose,'tmp_matlab_.UX_CTF_S_l2_S_',tmp_matlab_.UX_CTF_S_l2_S_,'double(tmp_python_.UX_CTF_S_l2_S_)',double(tmp_python_.UX_CTF_S_l2_S_));
fnorm_disp(flag_verbose,'tmp_matlab_.UX_T_M_l2_SM__',tmp_matlab_.UX_T_M_l2_SM__,'double(tmp_python_.UX_T_M_l2_SM__)',double(tmp_python_.UX_T_M_l2_SM__));
fnorm_disp(flag_verbose,'tmp_matlab_.X_SM__',tmp_matlab_.X_SM__,'double(tmp_python_.X_SM__)',double(tmp_python_.X_SM__));
fnorm_disp(flag_verbose,'tmp_matlab_.delta_x_SM__',tmp_matlab_.delta_x_SM__,'double(tmp_python_.delta_x_SM__)',double(tmp_python_.delta_x_SM__));
fnorm_disp(flag_verbose,'tmp_matlab_.delta_y_SM__',tmp_matlab_.delta_y_SM__,'double(tmp_python_.delta_y_SM__)',double(tmp_python_.delta_y_SM__));
fnorm_disp(flag_verbose,'tmp_matlab_.gamma_z_SM__',tmp_matlab_.gamma_z_SM__,'double(tmp_python_.gamma_z_SM__)',double(tmp_python_.gamma_z_SM__));
fnorm_disp(flag_verbose,'tmp_matlab_.index_sub_SM__',tmp_matlab_.index_sub_SM__,'double(tmp_python_.index_sub_SM__)',double(tmp_python_.index_sub_SM__));
%fnorm_disp(flag_verbose,'tmp_matlab_.index_nM_from_ncluster__',tmp_matlab_.index_nM_from_ncluster__,'double(tmp_python_.index_nM_from_ncluster__)',double(tmp_python_.index_nM_from_ncluster__));
fnorm_disp(flag_verbose,'tmp_matlab_.n_index_nM_from_ncluster_',tmp_matlab_.n_index_nM_from_ncluster_,'double(tmp_python_.n_index_nM_from_ncluster_)',double(tmp_python_.n_index_nM_from_ncluster_));
fnorm_disp(flag_verbose,'tmp_matlab_.M_pert_k_p_wkM__',tmp_matlab_.M_pert_k_p_wkM__,'double(tmp_python_.M_pert_k_p_wkM__)',double(tmp_python_.M_pert_k_p_wkM__));
fnorm_disp(flag_verbose,'tmp_matlab_.M_k_q_wkM__',tmp_matlab_.M_k_q_wkM__,'double(tmp_python_.M_k_q_wkM__)',double(tmp_python_.M_k_q_wkM__));
fnorm_disp(flag_verbose,'tmp_matlab_.UX_T_M_l2_dM__',tmp_matlab_.UX_T_M_l2_dM__,'double(tmp_python_.UX_T_M_l2_dM__)',double(tmp_python_.UX_T_M_l2_dM__));
fnorm_disp(flag_verbose,'tmp_matlab_.UX_M_l2_M_',tmp_matlab_.UX_M_l2_M_,'double(tmp_python_.UX_M_l2_M_)',double(tmp_python_.UX_M_l2_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.svd_V_UX_M_lwnM____',tmp_matlab_.svd_V_UX_M_lwnM____,'double(tmp_python_.svd_V_UX_M_lwnM____)',double(tmp_python_.svd_V_UX_M_lwnM____));
fnorm_disp(flag_verbose,'tmp_matlab_.UX_CTF_S_k_q_wnS__',tmp_matlab_.UX_CTF_S_k_q_wnS__,'double(tmp_python_.UX_CTF_S_k_q_wnS__)',double(tmp_python_.UX_CTF_S_k_q_wnS__));
end;%if both exist;

fname_matlab_10_0 = sprintf('%s_stage_10_0.mat',prefix_matlab);
fname_python_10_0 = sprintf('%s_stage_10_0.mat',prefix_python);
if exist(fname_matlab_10_0,'file') & exist(fname_python_10_0,'file');
tmp_matlab_ = load(fname_matlab_10_0);
tmp_python_ = load(fname_python_10_0);
fnorm_disp(flag_verbose,'tmp_matlab_.euler_polar_a_M_',tmp_matlab_.euler_polar_a_M_,'double(tmp_python_.euler_polar_a_M_)',double(tmp_python_.euler_polar_a_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.euler_azimu_b_M_',tmp_matlab_.euler_azimu_b_M_,'double(tmp_python_.euler_azimu_b_M_)',double(tmp_python_.euler_azimu_b_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.euler_gamma_z_M_',tmp_matlab_.euler_gamma_z_M_,'double(tmp_python_.euler_gamma_z_M_)',double(tmp_python_.euler_gamma_z_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_x_bit_M_',tmp_matlab_.image_delta_x_bit_M_,'double(tmp_python_.image_delta_x_bit_M_)',double(tmp_python_.image_delta_x_bit_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_y_bit_M_',tmp_matlab_.image_delta_y_bit_M_,'double(tmp_python_.image_delta_y_bit_M_)',double(tmp_python_.image_delta_y_bit_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_I_value_M_',tmp_matlab_.image_I_value_M_,'double(tmp_python_.image_I_value_M_)',double(tmp_python_.image_I_value_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_X_value_M_',tmp_matlab_.image_X_value_M_,'double(tmp_python_.image_X_value_M_)',double(tmp_python_.image_X_value_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_S_index_M_',tmp_matlab_.image_S_index_M_,'double(tmp_python_.image_S_index_M_)',double(tmp_python_.image_S_index_M_));
end;%if both exist;

fname_matlab_11_0 = sprintf('%s_stage_11_0.mat',prefix_matlab);
fname_python_11_0 = sprintf('%s_stage_11_0.mat',prefix_python);
if exist(fname_matlab_11_0,'file') & exist(fname_python_11_0,'file');
tmp_matlab_ = load(fname_matlab_11_0);
tmp_python_ = load(fname_python_11_0);
fnorm_disp(flag_verbose,'tmp_matlab_.delta_r_upd_threshold',tmp_matlab_.delta_r_upd_threshold,'double(tmp_python_.delta_r_upd_threshold)',double(tmp_python_.delta_r_upd_threshold));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_x_tot_M_',tmp_matlab_.image_delta_x_tot_M_,'double(tmp_python_.image_delta_x_tot_M_)',double(tmp_python_.image_delta_x_tot_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_y_tot_M_',tmp_matlab_.image_delta_y_tot_M_,'double(tmp_python_.image_delta_y_tot_M_)',double(tmp_python_.image_delta_y_tot_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_r_tot_M_',tmp_matlab_.image_delta_r_tot_M_,'double(tmp_python_.image_delta_r_tot_M_)',double(tmp_python_.image_delta_r_tot_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_x_nrm_M_',tmp_matlab_.image_delta_x_nrm_M_,'double(tmp_python_.image_delta_x_nrm_M_)',double(tmp_python_.image_delta_x_nrm_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_y_nrm_M_',tmp_matlab_.image_delta_y_nrm_M_,'double(tmp_python_.image_delta_y_nrm_M_)',double(tmp_python_.image_delta_y_nrm_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.tmp_index_',tmp_matlab_.tmp_index_,'double(tmp_python_.tmp_index_)',double(tmp_python_.tmp_index_));
fnorm_disp(flag_verbose,'tmp_matlab_.flag_image_delta_upd_M_',tmp_matlab_.flag_image_delta_upd_M_,'double(tmp_python_.flag_image_delta_upd_M_)',double(tmp_python_.flag_image_delta_upd_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_r_upd_prenorm_M_',tmp_matlab_.image_delta_r_upd_prenorm_M_,'double(tmp_python_.image_delta_r_upd_prenorm_M_)',double(tmp_python_.image_delta_r_upd_prenorm_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_r_upd_posnorm_M_',tmp_matlab_.image_delta_r_upd_posnorm_M_,'double(tmp_python_.image_delta_r_upd_posnorm_M_)',double(tmp_python_.image_delta_r_upd_posnorm_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_x_acc_M_',tmp_matlab_.image_delta_x_acc_M_,'double(tmp_python_.image_delta_x_acc_M_)',double(tmp_python_.image_delta_x_acc_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_y_acc_M_',tmp_matlab_.image_delta_y_acc_M_,'double(tmp_python_.image_delta_y_acc_M_)',double(tmp_python_.image_delta_y_acc_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_x_upd_M_',tmp_matlab_.image_delta_x_upd_M_,'double(tmp_python_.image_delta_x_upd_M_)',double(tmp_python_.image_delta_x_upd_M_));
fnorm_disp(flag_verbose,'tmp_matlab_.image_delta_y_upd_M_',tmp_matlab_.image_delta_y_upd_M_,'double(tmp_python_.image_delta_y_upd_M_)',double(tmp_python_.image_delta_y_upd_M_));
