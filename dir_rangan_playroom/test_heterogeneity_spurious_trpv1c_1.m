...
if flag_disp;
figure(1+nf);nf=nf+1;clf;figmed;
subplot(1,2,1); plot(sort(viewing_polar_a_all_(tmp_ij_(1:tmp_n_S_use))),'.'); 
ylim([0,pi]); xlabel('index'); ylabel('viewing_polar_a_all_','Interpreter','none');
title('lo nllr');
subplot(1,2,2); plot(sort(viewing_polar_a_all_(tmp_ij_(tmp_n_S_use+1:end))),'.'); 
ylim([0,pi]); xlabel('index'); ylabel('viewing_polar_a_all_','Interpreter','none');
title('hi nllr');
end;% if flag_disp;
%%%%%%%%;
...
%%%%%%%%;
cap_theta = min(abs(pi/2 - sort(viewing_polar_a_all_(tmp_ij_(tmp_n_S_use+1:end)))));
