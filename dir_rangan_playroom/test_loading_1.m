...
if (nmolecule==0); delta_orig_ = 0.15*randn(3,1); end;
if (nmolecule> 0);
delta_orig_ = delta_orig_dsm___(:,1+ndelta_orig,1+0);
delta_orig_ = delta_orig_ + (nmolecule/n_molecule)*0.1*sin(2*pi*ndelta_orig/(n_delta_orig-1))*[1;0;0];
end;%if (nmolecule> 0);
...
a_x_u_reco_xm__(:,1+nmolecule) = nufft3d3(n_k_all,2*pi*k_c_0_all_*tmp_eta,2*pi*k_c_1_all_*tmp_eta,2*pi*k_c_2_all_*tmp_eta,a_k_p_orig_km__(:,1+nmolecule).*(2*pi)^3.*weight_3d_k_all_,+1,1e-12,n_X_u,X_u_0_(:)/tmp_eta,X_u_1_(:)/tmp_eta,X_u_2_(:)/tmp_eta)/sqrt(2*pi)/sqrt(2*pi)/sqrt(2*pi);
