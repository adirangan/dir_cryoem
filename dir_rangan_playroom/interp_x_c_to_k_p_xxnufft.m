...
[x_c_0__,x_c_1__] = ndgrid(x_c_0_,x_c_1_); n_xx_M_u = n_x_M_u^2; xx_M_u_weight = (2*x_p_r_max/n_x_M_u)^3;
...
a_x_c_quad_ = interp_k_p_to_x_c_xxnufft(n_x_M_u,diameter_x_c,n_x_M_u,diameter_x_c,n_k_p_r,k_p_r_,n_w_,a_k_p_form_.*weight_2d_k_all_*(2*pi)^2)*sqrt(n_x_M_u^2) * n_w_sum;
a_x_c_quad_l2 = sum(abs(a_x_c_quad_).^2,'all')*dx^2;
b_k_p_form_l2 = sum(abs(b_k_p_form_).^2.*weight_2d_k_all_)*(2*pi)^2;
b_x_c_quad_ = interp_k_p_to_x_c_xxnufft(n_x_M_u,diameter_x_c,n_x_M_u,diameter_x_c,n_k_p_r,k_p_r_,n_w_,b_k_p_form_.*weight_2d_k_all_*(2*pi)^2)*sqrt(n_x_M_u^2) * n_w_sum;
...
tmp_M_x_c_reco_ = interp_k_p_to_x_c_xxnufft(n_x_M_u,diameter_x_c,n_x_M_u,diameter_x_c,n_k_p_r,k_p_r_,n_w_,tmp_M_k_p_.*weight_2d_k_all_*(2*pi)^2)*sqrt(n_x_M_u^2) * n_w_sum;
