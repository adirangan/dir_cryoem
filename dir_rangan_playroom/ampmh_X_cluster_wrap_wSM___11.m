...
if (verbose); disp(sprintf(' %% [testing ampmh_X_cluster_wrap_wSM___11.m]')); end;
%%%%%%%%;
tmp_t = tic();
[ ...
 n_k_all ...
...
%%%%%%%%;
% Calculate ampmh_X_cluster_wrap_wSM___11. ;
%%%%%%%%;
...
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% X_wSM_calc___: %0.3fs',tmp_t)); end;

%%%%%%%%;
% Calculate true landscape of innerproducts for the same set of translations. ;
%%%%%%%%;
X_wSM_form__ = zeros(n_w_max,n_viewing_all,n_M);
X_wSM_quad__ = zeros(n_w_max,n_viewing_all,n_M);
...
cc = cos(+gamma_z); sc = sin(+gamma_z);
Rz = [+cc , -sc ; +sc , +cc];
delta_x = delta_x_wSM_calc___(1+nw,1+nS,1+nM);
delta_y = delta_y_wSM_calc___(1+nw,1+nS,1+nM);
...
X_wSM_form___(1+nw,1+nS,1+nM) = real(X_form) / (S_l2_quad*M_l2_quad) ;
%%%%;
X_quad = sum(reshape(conj(S_k_p_0use_).*M_k_p_0use_,[n_w_max,n_k_p_r])*weight_2d_k_p_r_)/n_w_max;
X_wSM_quad___(1+nw,1+nS,1+nM) = real(X_quad) / (S_l2_quad*M_l2_quad) ;
%%%%;
end;%for nw=0:n_w_max-1;
end;%for nM=0:n_M-1;
end;%for nS=0:n_viewing_all-1;
%%%%%%%%;
disp(sprintf(' %% X_wSM_form___ vs X_wSM_calc___: %0.16f',fnorm(X_wSM_form___-X_wSM_calc___)/fnorm(X_wSM_form___)));
disp(sprintf(' %% X_wSM_quad___ vs X_wSM_calc___: %0.16f',fnorm(X_wSM_quad___-X_wSM_calc___)/fnorm(X_wSM_quad___)));
%%%%%%%%;
...
verbose = 0;
if (verbose>0); disp(sprintf(' %% [entering ampmh_X_cluster_wrap_wSM___11]')); end;
...
[ ...
 parameter ...
,X_wSM___(:,:,1+index_nM_from_ncluster_) ...
,delta_x_wSM___(:,:,1+index_nM_from_ncluster_) ...
,delta_y_wSM___(:,:,1+index_nM_from_ncluster_) ...
,gamma_z_wSM___(:,:,1+index_nM_from_ncluster_) ...
,I_value_wSM___(:,:,1+index_nM_from_ncluster_) ...
] = ...
...
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% ampmh_X_single_cluster_wSM___11: %0.3fs',tmp_t)); end;
parameter = parameter_timing_update(parameter,'ampmh_X_single_cluster_wSM___11',tmp_t);
%%%%%%%%;
flag_check=0;
if (flag_check);
n_test=9;
...
disp(sprintf(' %% tmp_X_wSm__ vs X_wSM___(:,:,1+tmp_nM): %0.16f',fnorm(tmp_X_wSm__ - X_wSM___(:,:,1+tmp_nM))/fnorm(tmp_X_wSm__)));
disp(sprintf(' %% tmp_delta_x_wSm__ vs delta_x_wSM___(:,:,1+tmp_nM): %0.16f',fnorm(tmp_delta_x_wSm__ - delta_x_wSM___(:,:,1+tmp_nM))/fnorm(tmp_delta_x_wSm__)));
disp(sprintf(' %% tmp_delta_y_wSm__ vs delta_y_wSM___(:,:,1+tmp_nM): %0.16f',fnorm(tmp_delta_y_wSm__ - delta_y_wSM___(:,:,1+tmp_nM))/fnorm(tmp_delta_y_wSm__)));
disp(sprintf(' %% tmp_gamma_z_wSm__ vs gamma_z_wSM___(:,:,1+tmp_nM): %0.16f',fnorm(tmp_gamma_z_wSm__ - gamma_z_wSM___(:,:,1+tmp_nM))/fnorm(tmp_gamma_z_wSm__)));
disp(sprintf(' %% tmp_I_value_wSm__ vs I_value_wSM___(:,:,1+tmp_nM): %0.16f',fnorm(tmp_I_value_wSm__ - I_value_wSM___(:,:,1+tmp_nM))/fnorm(tmp_I_value_wSm__  )));
end;%for ntest=0:n_test-1;
end;% if (flag_check);
%%%%%%%%;
...
if ( (nargout>5) & (isempty(I_value_wSM___)) ); I_value_wSM___ = ones(n_w_max,n_S,n_M); end;

if (verbose>0); disp(sprintf(' %% [finished ampmh_X_cluster_wrap_wSM___11]')); end;
