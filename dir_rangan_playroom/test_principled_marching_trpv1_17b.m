...
c_ = colormap_beach(); n_c = size(c_,1);
subplot(2,2,1);
stairfall_0(X_1M_from_d1_sigma__,0:n_k_p_r,[0,tru_X_d1_S_(1)],repmat(colormap_beach(),[4,1]));
xlim([0,n_k_p_r]); ylim([0,tru_X_d1_S_(1)]);
xlabel('rank'); ylabel('sigma');
title('X_1M_from_d1_sigma__','Interpreter','none');
subplot(2,2,2); 
stairfall_0(X_d1_from_1M_sigma__,0:n_k_p_r,[0,emp_X_1M_S_(1)],repmat(colormap_beach(),[4,1]));
xlim([0,n_k_p_r]); ylim([0,emp_X_1M_S_(1)]);
xlabel('rank'); ylabel('sigma');
title('X_d1_from_1M_sigma__','Interpreter','none');
subplot(2,2,3); 
stairfall_0(log10(X_1M_from_d1_sigma__),log10(1:1+n_k_p_r),log10(tru_X_d1_S_(1)) + [-6,0],repmat(colormap_beach(),[4,1]));
xlim([0,log10(1+n_k_p_r)]); ylim(log10(tru_X_d1_S_(1)) + [-6,0]);
xlabel('log10(rank)'); ylabel('log10(sigma)');
title('X_1M_from_d1_sigma__','Interpreter','none');
subplot(2,2,4); 
stairfall_0(log10(X_d1_from_1M_sigma__),log10(1:1+n_k_p_r),log10(emp_X_1M_S_(1)) + [-6,0],repmat(colormap_beach(),[4,1]));
xlim([0,log10(1+n_k_p_r)]); ylim(log10(emp_X_1M_S_(1)) + [-6,0]);
xlabel('log10(rank)'); ylabel('log10(sigma)');
title('X_d1_from_1M_sigma__','Interpreter','none');
figbig;
disp(sprintf(' %% writing %s',fname_fig));
