function [map,s,hdr,extraHeader]=NegateMRC_0(filename_0in,startSlice, numSlices,test,filename_out);
...
tmp_ij = strfind(filename_out,'.mrc');
if (~isempty(tmp_ij));
filename_out = sprintf('%s_negated%s',filename_out(1:tmp_ij-1),filename_out(tmp_ij:end));
...
if (verbose); disp(sprintf(' %% writing negated map')); end;
fwrite(f_out,-map,string);
