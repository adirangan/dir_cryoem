...
c     eps       CG tolerance
c     numit     max number of CG iterations
c
c     OUTPUT:
c
...
c Doc updated ahb 1/3/16
     ...
     1           nquad,k,eps,numit)
      implicit none
      integer nterms,nquad,nquadm,nitermax,nout,k,numit,ier,niter,np
      ...
      external multaha
c
      write(6,*) ' nquad is ',nquad
      write(13,*) ' nquad is ',nquad
      nquadm = 2*nquad
...
ccc      call prinf(' calling shevalspherep_adj nout is *',nout,1)
      call sheval_spheregrid_adj(nterms,nquad,nquadm,xnodes,ugrid,y)
...
c     note: wts are not used
      implicit none
      integer n,nterms,nquad,nquadm,nout,k
...
ccc      call prinf(' calling shevalspherep *',n,1)
      call sheval_spheregrid(x,ugrid,nterms,nquad,nquadm,xnodes)
