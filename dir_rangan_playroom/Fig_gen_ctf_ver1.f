      subroutine Fig_gen_ctf_ver1(ncur,nlats_,grid_k_p_,n_M,ld_M,M_k_p_
     $    ,N_k_p_,ld_CTF,CTF_k_p_,n_alpha,alpha_est_,n_M_sample,prefix)
c$$$      Reads in a few images and plots them before and after
c$$$      convolution with their ctf-function
      ...
      data verbose / 0 /
      logical plot_flag
      integer *4 ncur,nlats_(0:ncur-1),n_M,ld_M,ld_CTF,n_M_sample
     $     ,n_alpha
      real *8 grid_k_p_(0:0)
      real *8 alpha_est_(0:0)
      complex *16 M_k_p_(0:0),N_k_p_(0:0),CTF_k_p_(0:0)
...
c$$$      printing images
     ...
     $        ,font_type,font_size,x_loc,y_loc,x_offset,y_offset,x_side
     $        ,y_side)
c$$$         Print image after ctf-convolution
...
c$$$         Print ctf-function associated with image
         ns = alpha_est_(n_alpha-1 + nm*n_alpha)
         if (verbose.gt.1) then
            write(6,'(A,I0,A,I0)') 'ctf for image ',nm,' is ',ns
         end if
c$$$         Display ctf-function
         call cp1_c16(n_A,CTF_k_p_(ns*ld_CTF),T_k_p_)
     ...
     $        ,y_side)
         write(fig_title,'(A,I0,A,I4,A)') 'R(C_k_p(',ns,')) '
     $        ,nint(dlog(F_std)),'    '
         text_length = 18
         color_index = 0
         font_type = 0
     ...
     $        ,T_k_p_,F_min,F_max,x_offset,y_offset,x_side
     $        ,y_side)
         write(fig_title,'(A,I0,A,I4,A)') 'I(C_k_p(',ns,')) '
     $        ,nint(dlog(F_std)),'    '
         text_length = 18
         color_index = 0
         font_type = 0
...
c$$$      Printing figure file for image
     ...
     $     ,trim(fname_fig),trim(fname_jpg)
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[finished Fig_gen_ctf_ver1]'
      end if
