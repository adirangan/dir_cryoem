...
pm_tot_t_ = zeros(n_UX_rank,1);
pm_n_op_ = zeros(n_UX_rank,1);
pm_n_mult_ = zeros(n_UX_rank,1);
...
tmp_t = toc(tmp_t); if (verbose>1); disp(sprintf(' %% X_ww__: %0.3fs',tmp_t)); end;
pm_E = fnorm(X_quad_ - pm_X_quad_)/fnorm(X_quad_);
pm_E_(1+tmp_pm_n_UX_rank) = pm_E;
pm_tot_t_(1+tmp_pm_n_UX_rank) = tmp_t;
pm_n_op_(1+tmp_pm_n_UX_rank) = pm_n_op;
pm_n_mult_(1+tmp_pm_n_UX_rank) = pm_n_mult;
end;%for tmp_pm_n_UX_rank=0:n_UX_rank-1;

%%%%%%%%;
% Verdict: O(K^4) computation to calculate a+b array using all O(K) radial quadrature-nodes. ;
% followed by another O(K^4) computation to calculate innerproducts for all O(K) beta-values. ;
% Total is only improved by a factor of 2 or so using radial-principal-modes. ;
% Of course, we can also try order-principal-modes. ;
% See test_pm_vfig_0.m. ;
%%%%%%%%;
