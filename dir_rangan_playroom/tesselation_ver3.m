...
%n_points = 8192;
n_points = 128;
hL_ = HandleObject(randn(3,n_points)); % List of points ;
for npoints=1:n_points; hL_.O(:,npoints) = hL_.O(:,npoints)/norm(hL_.O(:,npoints)); end;
hLT = HandleObject(1:n_points); % List-index for each point ;
hTL = HandleObject(zeros(1,n_points)); % This will eventually store the tesselation-index for each element of hL_ (leaves only) ;
...
hT_LT_.O(index__input) = 0; if (n_L_sub_length==1); hT_LT_.O(index__input)=LT_sub(1); hTL.O(LT_sub(1))=index__input; end;
...
if (verbose); disp(sprintf('%s error, empty list',prefix)); end;
