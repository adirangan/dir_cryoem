...
%'LetB1' , 'LetB1' , 1.31 , 'emd_20993.map' , 'job_569_model_1_350MB.star' ; ...
...
n_M_ = [256,512,768]; n_n_M = numel(n_M_);
for nn_M=0:n_n_M-1;
n_M = n_M_(1+nn_M);
...
global_parameter.rank_pm_0in_ = [16,18];
global_parameter.tolerance_pm_0in_ = 0.1.^[2:0.5:3];
fname_prefix=sprintf('ps1_x%dto%d',floor(global_parameter.nM_start/1024),floor(global_parameter.nM_final/1024));
...
global_parameter.flag_center_image = 1;
global_parameter.nM_start = 1024*nx;
global_parameter.nM_final = global_parameter.nM_start + 1024 - 1;
global_parameter.n_x_u_pack_0in = 64;
global_parameter.k_p_r_max_0in = 48/(2*pi);
global_parameter.delta_sigma_use_0in = 0.05;
global_parameter.dat_rseed_0in_ = [0:2];
global_parameter.rank_pm_0in_ = [16,18];
global_parameter.tolerance_pm_0in_ = 0.1.^[2:0.5:3];
fname_prefix=sprintf('ps1_x%d',ceil(global_parameter.nM_start/1024));
dir_nopath_data_star='precatalytic_spliceosome';
Pixel_Spacing=1.699;
fname_nopath_volume='consensus_half1_class001.mrc';
fname_nopath_star='consensus_data.star';
...
verbose=1; nf=0;
if (verbose); disp(sprintf(' %% [entering test_pm_24]')); end;
...
x_p_r_max = 1.0;
n_x_u_pack = n_x_u_pack_0in; %<-- default: 64. ;
...
disp(sprintf(' %% %s not found, creating',fname_mat));
tmp_t = tic;
k_p_r_max = k_p_r_max_0in; %<-- default: 48/(2*pi). ;
k_eq_d = 1.0/(2*pi);
[ ...
 n_k_all ...
,n_k_all_csum_ ...
...
%%%%%%%%;
n_M = 1 + nM_final - nM_start; %<-- default: 1024. ;
image_center_1_wrap_2;
%%%%%%%%;
[ ...
 M_x_c___ ...
,index_nCTF_from_nM_ ...
...
verbose=1;
if (verbose); disp(sprintf(' %% [finished test_pm_24]')); end;
