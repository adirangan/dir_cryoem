      subroutine test_innerproduct_batch_wrapper_2c(verbose,ncur,nlats_
...
c$$$      parameters for timing
      ...
      if (verbose.gt.0) then
          write(6,'(A)')
     $        '[entering test_innerproduct_batch_wrapper_2c]: '
       end if
...
c$$$      calculating all innerproducts
      timing_tic = second()
      call test_innerproduct_batch_stage_0c(verbose
     ...
     $        ,(timing_toc -timing_tic)/(n_S*n_M*n_delta_x*n_delta_y)
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[finished test_innerproduct_batch_wrapper_2c]'
      end if
