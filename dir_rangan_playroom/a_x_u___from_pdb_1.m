...
str_thisfunction = 'a_x_u___from_pdb_1';
...
if ~isfield(parameter,'sigma_angstrom'); parameter.sigma_angstrom=16.0; end;
...
sigma_gaussian = 1.0/max(1,n_x_u) * sigma_angstrom/max(1e-12,Pixel_Spacing);
g = @(x0,x1,x2) 1/sqrt(2*pi)^3 / max(1e-12,sigma_gaussian^3) * exp( -( (x_u_0___ - x0).^2 + (x_u_1___ - x1).^2 + (x_u_2___ - x2).^2 )/max(1e-12,2*sigma_gaussian^3));
%%%%;
[ ...
 parameter ...
,n_Atom ...
