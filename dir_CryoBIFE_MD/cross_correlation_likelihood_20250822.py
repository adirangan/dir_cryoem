...
from EMPM.microscopy import get_possible_displacements_grid, translation_kernel_fourier
from EMPM.stacks import Templates
...
### TODO NOTE: We are currently rolling the quadrature weighting into the
### translation kernel. We only use the non-FB representation of the
### sqrtweighted-fourier-templates for computing the template norm,
### so it doesn't (shouldn't) matter which displacement we take, but
### know that the root of the quadrature weights has ALREADY been applied.
### This may cause an issue?
def _compute_template_norms(
    n_inplanes: int, # for the inverse fft
    sqrtweighted_fourier_templates_mnw: torch.Tensor,
    ctf_snw: torch.Tensor,
):
    # TODO REMOVE
    assert len(sqrtweighted_fourier_templates_mnw.shape) == 3
    ## NOTE: as of right now this has the quadrature weights baked in
    ## (it need not be this way)
    # TODO: change ordering when we fix it globally
    t_sq_fourier_bessel_csnq = fourier_bessel_transform(
        sqrtweighted_fourier_templates_mnw.conj() * sqrtweighted_fourier_templates_mnw,
        axis=-1
    ).unsqueeze(0)
    ctfs_sq_fourier_bessel_csnq = fourier_bessel_transform(
        ctf_snw.conj() * ctf_snw,
        axis=-1
    ).conj().unsqueeze(1)

    # ctf index corresponds to image index
    # we are using m here correctly to refer to image
    ctf_templates_msq = (t_sq_fourier_bessel_csnq * ctfs_sq_fourier_bessel_csnq).sum(2)
    ctf_template_norms_msw = torch.fft.irfft(
        ctf_templates_msq,
        n=n_inplanes,
        dim=2,
        norm="forward"
...
def _cross_images_and_templates(
    ...
    Ixx_smdw = _compute_template_norms(
        n_inplanes,
        sqrtweighted_fourier_templates_mnw,
        ctf_snw
    ).unsqueeze(2)
    Iyy_smdw = torch.sum(
        absq(images_fourier_snw) * integration_weights_points,
        dim = (1,2)
    ...
    cross_correlation_smdw = Ixy_smdw / torch.sqrt(Ixx_smdw * Iyy_smdw)
...
class CrossCorrelationLikelihood:
    ...
    Attributes:
        ...
        n_displacements (int): number of displacements considered in checking optimal displacement.
            Should be the length of the x_displacement and y_displacement vectors.
