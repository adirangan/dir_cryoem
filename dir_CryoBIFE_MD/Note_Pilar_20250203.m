...
if strcmp(platform,'access1'); run(sprintf('/%s/rangan/dir_cryoem/dir_rangan_playroom/setup_access1',string_root)); end;
if strcmp(platform,'evale'); run(sprintf('/%s/rangan/dir_cryoem/dir_rangan_playroom/setup_eval1',string_root)); end;
cd(sprintf('/%s/rangan/dir_cryoem/dir_CryoBIFE_MD',string_root));
...
[~,tmp_ij] = max(P_div_max_nrm_e_,[],'all','linear');
...
%%%%%%%%;
[~,ij_opt] = max(logP_ee__,[],'all','linear');
index_opt = ij_opt - 1;
...
%%%%%%%%;
nc0 = 0; nc1 = 1; nc2 = 2; %<-- pick three models. ;
n_LL_b = 1; LL_b_ = transpose(linspace(1.00,1.15,n_LL_b))*max(LL_rc__,[],'all'); %<-- essentially no variation for this example, so just pick one. ;
