...
dir_jpg = '~/dir_cryoem/dir_CryoBIFE_MD';
...
logP_max = max(logP_ee__,[],'all','linear');
...
LL_b = 1.15*max(LL_rc__,[],'all','linear'); %<-- arbitrary. ;
...
%%%%%%%%;
nc0 = 0; nc1 = 1; nc2 = 2; %<-- pick three models. ;
n_LL_b = 1; LL_b_ = transpose(linspace(1.00,1.15,n_LL_b))*max(LL_rc__,[],'all','linear'); %<-- no variation w.r.t LL_b, so just pick one. ;
