...
int WritePNMfile_color(double *double_array_,int rows,int cols,double min,double max,char *filename)
{
  ...
  ud_ = (unsigned char *) wkspace_all0c(3*rows*cols);
  for (nr=0;nr<rows;nr++){ for (nc=0;nc<cols;nc++){
      d = double_array_[nr+nc*rows]; colorscale(d,min,max,&rcolor,&gcolor,&bcolor);
      ud_[0+nc*3+nr*3*cols] = (unsigned char)(UCHAR_MAX*rcolor); 
      ud_[1+nc*3+nr*3*cols] = (unsigned char)(UCHAR_MAX*gcolor); 
      ud_[2+nc*3+nr*3*cols] = (unsigned char)(UCHAR_MAX*bcolor);
      /* for (nr=0;nr<rows;nr++){ for (nc=0;nc<cols;nc++){ }} */}}
  fwrite(ud_,sizeof(unsigned char),3*rows*cols,fp);
