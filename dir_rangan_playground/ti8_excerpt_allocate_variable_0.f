...
c$$$         do nothing
         ...
         if (n_M_0_sub_0in.ne.n_M) then
            write(6,'(A,A,I0,A)')
     $           ' Because flag_tesselation.eqv..true.: '
     $           ,' Setting n_M_0_sub_0in = n_M: ' , n_M ,
     $           ' and n_M_1_sub_0in = 1 '
         end if !if (n_M_0_sub_0in.ne.n_M) then
         n_M_0_sub_0in = n_M
         n_M_1_sub_0in = 1
...
c$$$      flag_time_Zstore = .false. ! read from input. ;
...
c$$$      Calculating template size using 'get_template_size'
      allocate(n_w_(0:1+n_k_p_r_cur-1))
      call cs1_i4(n_k_p_r_cur,n_w_)
      d_memory_estimate = d_memory_estimate + 4.0d0*n_k_p_r_cur
      allocate(n_w_csum_(0:1+n_k_p_r_cur-1))
      call cs1_i4(n_k_p_r_cur,n_w_csum_)
      d_memory_estimate = d_memory_estimate + 4.0d0*n_k_p_r_cur
      if (verbose.gt.1) then
         write(6,'(A,I0,A,A)') ' n_k_p_r_cur = ',n_k_p_r_cur
     $        ,' calling get_template_size to'
     $        ,' determine n_w_ and n_w_sum'
      end if !if (verbose.gt.1) then
      call get_template_size(n_polar_a_,n_k_p_r_cur,n_w_sum,n_w_
     $     ,n_w_csum_)
      if (verbose.gt.1) then
         write(6,'(A,I0)') ' n_w_sum = ',n_w_sum
         call print_all_i4(n_k_p_r_cur,n_w_,' n_w_: ')
      end if !if (verbose.gt.1) then
...
c$$$      indices
      ...
      n_w_max = n_w_(nr-1)
      if (verbose.gt.1) then
         write(6,'(A,I0,A,I0)') ' n_w_max ',n_w_max,'; n_w_sum ',n_w_sum
      end if !if (verbose.gt.1) then
     ...
     $        ' It is usually reasonable for n_gamma_z (i.e,. the '
      ...
      if (verbose.gt.1) then
         write(6,'(A)') ' Setting up workspace for nufft1df77.'
         write(6,'(A)') ' This is used if gamma_z_ is not uniformly '
         write(6,'(A)') ' sampled or if n_gamma_z.neq.n_w_max. '
         write(6,'(A)') ' '
      end if !if (verbose.gt.1) then
      call nufft1df77_1d2_lused(1.0d-12,n_gamma_z,nufft1df77_lused)
      nufft1df77_lw = nufft1df77_lused
      allocate(nufft1df77_fw_(0:1+nufft1df77_lw-1))
      call cs1_r8(nufft1df77_lw,nufft1df77_fw_)
      d_memory_estimate = d_memory_estimate + 8.0d0*nufft1df77_lw
      if (verbose.gt.1) then
         write(6,'(A,I0)') ' nufft1df77_lw: ' , nufft1df77_lw
      end if !if (verbose.gt.1) then
      if (verbose.gt.1) then
         write(6,'(A)') ' Setting up omp workspace for nufft1df77.'
         write(6,'(A)') ' This is used if gamma_z_ is not uniformly '
         write(6,'(A)') ' sampled or if n_gamma_z.neq.n_w_max. '
         write(6,'(A)') ' '
      end if !if (verbose.gt.1) then
      allocate(nufft1df77_lused_(0:1+n_omp_sub_0in-1))
      call cs1_i4(n_omp_sub_0in,nufft1df77_lused_)
      allocate(nufft1df77_lw_(0:1+n_omp_sub_0in-1))
      call cs1_i4(n_omp_sub_0in,nufft1df77_lw_)
      do nomp_sub_0in=0,n_omp_sub_0in-1
         nufft1df77_lused_(nomp_sub_0in) = nufft1df77_lw
         nufft1df77_lw_(nomp_sub_0in) = nufft1df77_lw
      enddo !do nomp_sub_0in=0,n_omp_sub_0in-1
      allocate(nufft1df77_fw__(0:1+n_omp_sub_0in*nufft1df77_lw-1))
      call cs1_r8(n_omp_sub_0in*nufft1df77_lw,nufft1df77_fw__)
      d_memory_estimate = d_memory_estimate + 8.0d0*n_omp_sub_0in
     $     *nufft1df77_lused + 4.0d0*2.0d0*n_omp_sub_0in
     ...
     $           ' = n_svd_l = ' , n_svd_l
      ...
      allocate(S_p_(0:1+n_w_sum-1))
      call cs1_c16(n_w_sum,S_p_)
      allocate(S_q_(0:1+n_w_sum-1))
      call cs1_c16(n_w_sum,S_q_)
      allocate(S_p_omp__(0:1+n_w_sum*n_omp_sub_0in-1))
      call cs1_c16(n_w_sum*n_omp_sub_0in,S_p_omp__)
      allocate(S_q_omp__(0:1+n_w_sum*n_omp_sub_0in-1))
      call cs1_c16(n_w_sum*n_omp_sub_0in,S_q_omp__)
      allocate(M_p_(0:1+n_w_sum-1))
      call cs1_c16(n_w_sum,M_p_)
      allocate(M_q_(0:1+n_w_sum-1))
      call cs1_c16(n_w_sum,M_q_)
      allocate(M_p_omp__(0:1+n_w_sum*n_omp_sub_0in-1))
      call cs1_c16(n_w_sum*n_omp_sub_0in,M_p_omp__)
      allocate(M_q_omp__(0:1+n_w_sum*n_omp_sub_0in-1))
      call cs1_c16(n_w_sum*n_omp_sub_0in,M_q_omp__)
      allocate(CTF_p_(0:1+n_w_sum-1))
      call cs1_c16(n_w_sum,CTF_p_)
      allocate(CTF_q_(0:1+n_w_sum-1))
      call cs1_c16(n_w_sum,CTF_q_)
      allocate(CTF_p_omp__(0:1+n_w_sum*n_omp_sub_0in-1))
      call cs1_c16(n_w_sum*n_omp_sub_0in,CTF_p_omp__)
      allocate(CTF_q_omp__(0:1+n_w_sum*n_omp_sub_0in-1))
      call cs1_c16(n_w_sum*n_omp_sub_0in,CTF_q_omp__)
      n_Z_x = max(max(n_w_sum,n_gamma_z),n_w_max)
      allocate(Z_p_(0:1+n_Z_x-1))
      call cs1_c16(n_Z_x,Z_p_)
      allocate(Z_q_(0:1+n_Z_x-1))
      call cs1_c16(n_Z_x,Z_q_)
      allocate(Z_p_omp__(0:1+n_Z_x*n_omp_sub_0in-1))
      call cs1_c16(n_Z_x*n_omp_sub_0in,Z_p_omp__)
      allocate(Z_q_omp__(0:1+n_Z_x*n_omp_sub_0in-1))
      call cs1_c16(n_Z_x*n_omp_sub_0in,Z_q_omp__)
      d_memory_estimate = d_memory_estimate + 16.0d0*n_Z_x*(8.0d0 +
     $     8.0d0*n_omp_sub_0in)
      allocate(ZZ__(0:1+n_delta_v*n_gamma_z-1))
      call cs1_c16(n_delta_v*n_gamma_z,ZZ__)
      allocate(ZZ_omp__(0:1+n_delta_v*n_gamma_z*n_omp_sub_0in
     $     -1))
      call cs1_c16(n_delta_v*n_gamma_z*n_omp_sub_0in,ZZ_omp__)
      d_memory_estimate = d_memory_estimate + 16.0d0*n_delta_v*1.0d0
     $     *n_gamma_z*(1.0d0 + n_omp_sub_0in)
      allocate(ZZ_sub_(0:1+n_w_max-1))
      call cs1_c16(n_w_max,ZZ_sub_)
      allocate(ZZ_sub_omp__(0:1+n_w_max*n_omp_sub_0in-1))
      call cs1_c16(n_w_max*n_omp_sub_0in,ZZ_sub_omp__)
      d_memory_estimate = d_memory_estimate + 16.0d0*n_w_max*(1.0d0 +
     $     n_omp_sub_0in)
      allocate(ZZ_pos_(0:1+n_gamma_z-1))
      call cs1_c16(n_gamma_z,ZZ_pos_)
      allocate(ZZ_pos_omp__(0:1+n_gamma_z*n_omp_sub_0in-1))
      call cs1_c16(n_gamma_z*n_omp_sub_0in,ZZ_pos_omp__)
     ...
     $     n_omp_sub_0in)
      ...
      allocate(CTF_R_S_sub_omp__(0:1+n_gamma_z*n_omp_sub_0in-1))
      call cs1_c16(n_gamma_z*n_omp_sub_0in,CTF_R_S_sub_omp__)
      d_memory_estimate = d_memory_estimate + 16.0d0*n_gamma_z*(1.0d0 +
     $     n_omp_sub_0in)
      ...
      n_S_0_sub_use = min(n_S_0_sub_0in,n_S)
     ...
     $     *n_CTF*1.0d0*nS_0_per_max
      ...
      n_S_1_sub_use = min(n_S_1_sub_0in,nS_0_per_max)
     ...
     $        nS_1_per_max
      ...
      if (flag_memory_estimate.eqv..false.) then
         ...
         n_C_trn0_omp = nS_1_per_max*n_transf*n_omp_sub_0in
         allocate(C_trn0_omp__(0:1+nS_1_per_max*n_transf*n_omp_sub_0in
     $        -1))
         call cs1_c16(nS_1_per_max*n_transf*n_omp_sub_0in,C_trn0_omp__)
      end if !if (flag_memory_estimate.eqv..false.) then
      d_memory_estimate = d_memory_estimate + 16.0d0*(1.0d0
     $     *nS_1_per_max)*(1.0d0*n_transf)*(1.0d0 +n_omp_sub_0in)

      if (flag_tesselation.eqv..true.) then
         d_mem = 0.0d0
         d_mem = d_mem + 1.0d0*n_gamma_z*1.0d0*n_CTF*1.0d0*nS_1_per_max
     $        *1.0d0*n_omp_sub_0in*16.0d0
         if (verbose.gt.1 .or. verbose_mem.gt.0) then
     ...
     $           *nS_1_per_max*n_omp_sub_0in
            allocate(CTF_R_S_local_omp__(0:1+n_gamma_z*n_CTF
     $           *nS_1_per_max*n_omp_sub_0in-1))
            call cs1_c16(n_gamma_z*n_CTF*nS_1_per_max*n_omp_sub_0in
     $           ,CTF_R_S_local_omp__)
         end if !if (flag_memory_estimate.eqv..false.) then
         d_memory_estimate = d_memory_estimate + 16.0d0*(1.0d0
     ...
     $        *n_omp_sub_0in)
      ...
      if (flag_tesselation.eqv..true.) then
         if (flag_memory_estimate.eqv..false.) then
            allocate(vp_input_omp__(0:1+3*n_omp_sub_0in-1))
            call cs1_r8(3*n_omp_sub_0in,vp_input_omp__)
            allocate(flag_S_use_omp__(0:1+nS_0_per_max*n_omp_sub_0in-1))
            call cs1_l2(nS_0_per_max*n_omp_sub_0in,flag_S_use_omp__)
            allocate(LT_omp__(0:1+2*nS_0_per_max*n_omp_sub_0in-1))
            call cs1_i4(2*nS_0_per_max*n_omp_sub_0in,LT_omp__)
            n_S_alpha_S_index_local_omp = nS_1_per_max
     $           *n_omp_sub_0in
            allocate(S_alpha_S_index_local_omp__(0:1+nS_1_per_max
     $           *n_omp_sub_0in-1))
            call cs1_r8(nS_1_per_max*n_omp_sub_0in
     ...
     $           *n_omp_sub_0in-1))
            call cs1_i4(nS_1_per_max*n_omp_sub_0in
     $           ,I_S_sample_local_omp__)
         end if                 !if (flag_memory_estimate.eqv..false.) then
         d_memory_estimate = d_memory_estimate + 8.0d0*3*n_omp_sub_0in
     ...
     $              *n_omp_sub_0in-1))
               call cs1_c16(n_r*nS_1_per_max*n_w_max*n_omp_sub_0in
     $              ,O_S_q_local_omp__)
            end if              !if (flag_memory_estimate.eqv..false.) then
            d_memory_estimate = d_memory_estimate + 16.0d0*n_r*1.0d0
     $           *nS_1_per_max*1.0d0*n_w_max*1.0d0*n_omp_sub_0in
         end if !if (flag_tesselation.eqv..true.) then
     ...
     $           *1.0d0*n_w_max*1.0d0*n_omp_sub_0in*16.0d0
            if (verbose.gt.1 .or. verbose_mem.gt.0) then
     ...
     $              *n_w_max*n_omp_sub_0in-1))
               call cs1_c16(n_r*n_transf*nS_1_per_max*n_w_max
     $              *n_omp_sub_0in,Z_S_q_local_omp__)
            end if              !if (flag_memory_estimate.eqv..false.) then
            d_memory_estimate = d_memory_estimate + 16.0d0*n_r*1.0d0
     $           *n_transf*1.0d0*nS_1_per_max*1.0d0*n_w_max*1.0d0
     $           *n_omp_sub_0in
      ...
      n_M_0_sub_use = min(n_M_0_sub_0in,n_M)
     ...
     $        *nM_0_per_max
      ...
      allocate(alpha_0in_(0:1+n_alpha-1))
      call cs1_r8(n_alpha,alpha_0in_)
      allocate(alpha_0in_omp__(0:1+n_alpha*n_omp_sub_0in-1))
      call cs1_r8(n_alpha*n_omp_sub_0in,alpha_0in_omp__)
      d_memory_estimate = d_memory_estimate + 8.0d0*n_alpha*(1.0d0 +
     $     n_omp_sub_0in)
      if (flag_MS_vs_SM.eqv..true.) then
      allocate(n_MS_omp__(0:1+n_S*n_omp_sub_0in-1))
      call cs1_i4(n_S*n_omp_sub_0in,n_MS_omp__)
      allocate(alpha_MS_omp__(0:1+n_alpha*n_MS_max*n_S*n_omp_sub_0in-1))
      call cs1_r8(n_alpha*n_MS_max*n_S*n_omp_sub_0in,alpha_MS_omp__)
      d_memory_estimate = d_memory_estimate + 4.0d0*n_S*n_omp_sub_0in +
     $     8.0d0*n_alpha*n_MS_max*n_S*n_omp_sub_0in
     ...
     $              *n_omp_sub_0in-1))
               call cs1_c16(n_r*n_w_max*n_omp_sub_0in
     $              ,O_T_R_CTF_M_q_local_omp__)
               n_O_T_R_CTF_M_q_local_omp__ = n_r*n_w_max*n_omp_sub_0in
            end if              !if (flag_memory_estimate.eqv..false.) then
            d_memory_estimate = d_memory_estimate + 16.0d0*n_r*1.0d0
     $           *n_w_max*1.0d0*n_omp_sub_0in
         end if                 !if (flag_tesselation.eqv..false.) then
     ...
     $              *n_w_max*n_omp_sub_0in-1))
               call cs1_c16(n_r*n_transf*n_w_max*n_omp_sub_0in
     ...
     $           *n_omp_sub_0in
      ...
      n_M_1_sub_use = min(n_M_1_sub_0in,nM_0_per_max)
     ...
     $           *1.0d0*nS_1_per_max*1.0d0*nM_1_per_max*1.0d0*n_w_max
            ...
            d_mem = 0.0d0
            d_mem = d_mem + 1.0d0*n_transf*1.0d0*nS_1_per_max*1.0d0
     $           *nM_1_per_max*1.0d0*n_w_max*1.0d0*n_omp_sub_0in*16.0d0
            if (verbose.gt.1 .or. verbose_mem.gt.0) then
               write (6,'(A,2(F16.8,A))')
     ...
     $              *nS_1_per_max*nM_1_per_max*n_w_max*n_omp_sub_0in-1))
               call cs1_c16(n_transf*nS_1_per_max*nM_1_per_max*n_w_max
     $              *n_omp_sub_0in,S_T_T_R_CTF_M_q_local_omp__)
               n_S_T_T_R_CTF_M_q_local_omp__ = n_transf*nS_1_per_max
     $              *nM_1_per_max*n_w_max*n_omp_sub_0in
            end if              !if (flag_memory_estimate.eqv..false.) then
            d_memory_estimate = d_memory_estimate + 16.0d0*n_transf
     $           *1.0d0*nS_1_per_max*1.0d0*nM_1_per_max*1.0d0*n_w_max
     $           *1.0d0*n_omp_sub_0in
     ...
     $              *n_transf*nM_1_per_max*n_w_max*n_omp_sub_0in-1))
               call cs1_c16(nS_1_per_max*n_transf*nM_1_per_max*n_w_max
     $              *n_omp_sub_0in,S_T_T_R_CTF_M_q_local_omp__)
     ...
     $              *nS_1_per_max*nM_1_per_max*n_w_max*n_omp_sub_0in-1))
               call cs1_c16(n_transf*nS_1_per_max*nM_1_per_max*n_w_max
     $              *n_omp_sub_0in,S_Z_T_R_CTF_M_q_local_omp__)
     ...
     $           *1.0d0*nS_1_per_max*1.0d0*nM_1_per_max*1.0d0*n_w_max
            ...
            d_mem = 0.0d0
            d_mem = d_mem + 1.0d0*n_delta_v*1.0d0*nS_1_per_max
     $           *1.0d0*nM_1_per_max*1.0d0*n_w_max*1.0d0*n_omp_sub_0in
     $           *16.0d0
            if (verbose.gt.1 .or. verbose_mem.gt.0) then
               write (6,'(A,2(F16.8,A))')
     $              ' S_T_T_R_CTF_M_q_local_omp__ requires ' ,d_mem*1.0d
     ...
     $           *1.0d0*n_omp_sub_0in
      ...
      allocate(n_S_9_per_(0:1+n_omp_sub_0in-1))
      call cs1_i4(n_omp_sub_0in,n_S_9_per_)
      allocate(n_S_9_sum_(0:1+n_omp_sub_0in-1))
      call cs1_i4(n_omp_sub_0in,n_S_9_sum_)
      allocate(n_M_9_per_(0:1+n_omp_sub_0in-1))
      call cs1_i4(n_omp_sub_0in,n_M_9_per_)
      allocate(n_M_9_sum_(0:1+n_omp_sub_0in-1))
      call cs1_i4(n_omp_sub_0in,n_M_9_sum_)
      d_memory_estimate = d_memory_estimate + 4.0d0*4.0d0*n_omp_sub_0in

      n_S_use = 0
      n_S_use_sum = 0
      allocate(n_S_use_sum_(0:1+n_omp_sub_0in-1))
      call cs1_i4(n_omp_sub_0in,n_S_use_sum_)
