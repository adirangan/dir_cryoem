...
c     OUTPUT:
c
c     xnodesth       nodes in [-1,1] corresponding to dcos(theta_j)
c                    where theta_j are equispaced at classical 
c                    Chebyshev nodes
c     sthetas        corresponding dsin(theta) values
...
c----------------------------------------------------------------------
      ...
      real *8 wtsth(ngridt)
c
c    Chebyshev quadrature in theta (incorporates dsin(theta) weight)
...
c
      ...
      enddo
      ffhatnorm = dsqrt(ffhatnorm)/(dsqrt(2.0d0*pi)**3)
      return
      end
c
c
