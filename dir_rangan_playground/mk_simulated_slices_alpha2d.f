...
!>      alpha2d -> (dcos(beta==theta),alpha==phi,gamma_z,delta_x,delta_y). ;\n
!>       ;\n
...
c$$$      alpha2d -> (dcos(beta==theta),alpha==phi,gamma_z,delta_x,delta_y).
c$$$      
...
c
     ...
     $        ,ii),+delta_x,+delta_y,cslices(1,ii))
         ...
         call rotate_p_to_p_fftw_plan_0on(ngridr,ngridc,nslice_size
     $        ,cslices(1,ii),gamma_z,cslices(1,ii))
      enddo
      t2 = omp_get_wtime()
