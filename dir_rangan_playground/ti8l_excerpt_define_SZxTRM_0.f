...
!> Calculates S_T_T_R_CTF_M_q_local_omp__. ;\n
     ...
     $        ,S_Z_T_R_CTF_M_q_local_omp__(p_S_Z_T_R_CTF_M_q_local))
      ...
      do nw=0,n_w_max-1
         ...
         nx4 = n_r*n_transf*nS_1_per_max*n_w_max*n_omp_sub_0in
     ...
     $           ' max: ' , nx4-1
         ...
         nx4 = n_transf*nS_1_per_max*n_w_max*n_omp_sub_0in
     ...
     $        *dcmplx(1.0d0,0.0d0),Z_S_q_local_omp__(nx1+p_Z_S_q_local)
     $        ,n_r,O_T_R_CTF_M_q_local_omp__(nx2+p_O_T_R_CTF_M_q_local)
     $        ,n_r ,0.0d0 *dcmplx(1.0d0,0.0d0)
...
c$$$      %%%%%%%%%%%%%%%%
     ...
     $     ,fpm_0in__(n_F_tmp),fpm_out__(n_F_tmp) ,fpm_back_(nM_9_sub)
...
c$$$      %%%%%%%%%%%%%%%%
     ...
     $     ,S_T_T_R_CTF_M_q_local_omp__(p_S_T_T_R_CTF_M_q_local))
      timing_tic = omp_get_wtime()
      do nw=0,n_w_max-1
         n_Y_tmp = n_delta_v*nS_1_per_max*nw
         n_X_tmp = n_transf*nS_1_per_max*nw
         call zgemm('T','N',n_delta_v,nS_1_per_max*1 ,n_svd_l ,0.5d0
     $        *dcmplx(1.0d0,0.0d0),Z_S_svdd_,n_svd_l
     $        ,S_Z_T_R_CTF_M_q_local_omp__(n_X_tmp
     $        +p_S_Z_T_R_CTF_M_q_local) ,n_svd_l,0.0d0*dcmplx(1.0d0
     $        ,0.0d0) ,S_T_T_R_CTF_M_q_local_omp__(n_Y_tmp
...
c$$$      %%%%%%%%%%%%%%%%
      ...
      if (flag_test) then
         if (verbose.gt.1) then
            write(6,'(A)')
     $           ' calling ti7_check_SZxTRM_4 to test. '
            write(6,'(A)') ' '
         end if !if (verbose.gt.1) then
         call ti8_check_SZxTRM_4(verbose-1
     $        ,svd_d_max
     $        ,n_svd_d
     $        ,svd_d_
     $        ,svd_r_max
