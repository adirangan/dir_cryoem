function vlim_ = isosurface_ver8(n_,dirname,infix,vlim_) ;
...
X = MDA_read_r8(sprintf('%s/grid_x0_c___.mda',dirname)); X = permute(X,[2,1,3]);
Y = MDA_read_r8(sprintf('%s/grid_x1_c___.mda',dirname)); Y = permute(Y,[2,1,3]);
Z = MDA_read_r8(sprintf('%s/grid_x2_c___.mda',dirname)); Z = permute(Z,[2,1,3]);
elseif (ncur==-2);
...
subplot(prows,pcols,nF); cla;
cra = colormap_beach();
%cra = colormap('spring'); cra = cra(end:-1:1,:); 
%cra = colormap('cool'); 
ncra = size(cra,1);
v_avg = mean(F_x_c(:)); v_std = std(F_x_c(:)); v_min = min(F_x_c(:)); v_max = max(F_x_c(:));
if (nargin<4 | isempty(vlim_)); vlim_ = v_avg + 2.5*v_std*[-1,1]; end;
%vlim_ = [v_min , v_max];
if numel(vlim_)>1; v_ = linspace(vlim_(1),vlim_(2),7); end;
if numel(vlim_)==1; v_ = vlim_(1); end;
...
if length(n_)>2; n_start = min(n_(find(n_>0))); fname = sprintf('%s/f_x_c_%s___%d_%d_%d.jpg',dirname,infix,n_start,round(mean(diff(n_(2:end)))),n_(end)); end;
