...
c$$$      assumes uniform grid and periodic boundary conditions
      ...
      integer nx,ny,nc
      real *8 dx,dy
      C_c = dcmplx(0.0d0,0.0d0)
c$$$      C_T = dcmplx(0.0d0,0.0d0)
c$$$      C_M = dcmplx(0.0d0,0.0d0)
      ...
      do ny=0,n_y-1
         do nx=0,n_x-1
c$$$            nc = nx + ny*n_x
            C_c = C_c + dconjg(T_c_(nc))*M_c_(nc)
c$$$            C_T = C_T + dconjg(T_c_(nc))*T_c_(nc)
c$$$            C_M = C_M + dconjg(M_c_(nc))*M_c_(nc)
            nc = nc+1  
         enddo
      enddo
