...
!> Upgraded to ignore frequencies of magnitude n_w_(nr)/2 or larger. ; ;\n
      subroutine ti8_build_Z_T_R_CTF_M_q_2(
     $     verbose
     $     ,svd_r_max
     $     ,n_svd_r
     $     ,svd_r_
...
c$$$         enddo !do nl=0,n_svd_l-1         
     ...
     $        ,M_p__(I_M_sample_(nm)*ld_M),C_M)
         ...
         if (verbose.gt.2) then
            ...
            call print_sub_c16(n_w_sum,M_q_,'original M_p_: ')
            call innerproduct_p(n_r,grid_p_,n_w_,n_w_sum,M_p_,M_p_
     $           ,Z_tmp)
            Z_tmp = zsqrt(Z_tmp)
         ...
         if (verbose.gt.2) then
            write(6,'(A,I0,A,I0,A,I0)') ' nm: ' , nm 
            call print_sub_c16(n_w_sum,M_q_,'  CTF *  M_p_: ')
            call innerproduct_p(n_r,grid_p_,n_w_,n_w_sum,M_p_,M_p_
     $           ,Z_tmp)
            Z_tmp = zsqrt(Z_tmp)
     ...
     $        ,n_w_,n_w_sum,fftw_0in_,fftw_out_,M_p_,-gamma_z_est,M_p_)
         if (verbose.gt.2) then
            write(6,'(A,I0,A,I0,A,I0)') ' nm: ' , nm 
            call print_sub_c16(n_w_sum,M_q_,'  R(CTF*M_p_): ')
            call innerproduct_p(n_r,grid_p_,n_w_,n_w_sum,M_p_,M_p_
     $           ,Z_tmp)
            Z_tmp = zsqrt(Z_tmp)
     ...
     $        -delta_y_est,M_p_)
         if (verbose.gt.2) then
            write(6,'(A,I0,A,I0,A,I0)') ' nm: ' , nm 
            call print_sub_c16(n_w_sum,M_q_,'T(R(CTF*M_p_)):')
            call innerproduct_p(n_r,grid_p_,n_w_,n_w_sum,M_p_,M_p_
     $           ,Z_tmp)
            Z_tmp = zsqrt(Z_tmp)
