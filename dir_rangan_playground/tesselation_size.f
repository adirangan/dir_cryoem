...
!> Finds size of the tesselation-tree. ;\n
      recursive subroutine tesselation_size(tradius_min ,nl_0in,v0_i
     $     ,v1_i,v2_i,parity,n_L,L_,LT_,nl_out,nm_out,ll_out)
      implicit none
      integer *4 verbose
      data verbose / 0 /
      real *8 tradius_min
      integer *4 nl_0in,n_L,nl_out,nm_out,ll_out
     ...
     $        '[entering tesselation_size]: ',nl_0in
     ...
     $           +3*nL) , L_(1+3*nL) , L_(2+3*nL)
      ...
      if (n_L_sub.ge.1) then
         ...
         if (.not. lf) then
            ...
            call tesselation_size(tradius_min,nl_0in+1,v0_k,m2 ,m1
     $           ,parity,n_L_sub,L_sub_,LT_sub_,nl_child_0,nm_child_0
     $           ,ll_child_0)
            call tesselation_size(tradius_min,nl_0in+1,v1_k,m0 ,m2
     $           ,parity,n_L_sub,L_sub_,LT_sub_,nl_child_1,nm_child_1
     $           ,ll_child_1)
            call tesselation_size(tradius_min,nl_0in+1,v2_k,m1 ,m0
     $           ,parity,n_L_sub,L_sub_,LT_sub_,nl_child_2,nm_child_2
     $           ,ll_child_2)
            call tesselation_size(tradius_min,nl_0in+1,m0,m1,m2, .not.
