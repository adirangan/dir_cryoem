!> Doxygen comment: ;\n
!> checks memory allocation for test_ver16.f ;\n
      if (verbose.gt.1) then
         write(6,'(A)') '[entering test_ver16_checkset]'
     ...
     $     ,flag_memory_checkset)
      call cxs_i4(n_k_p_upperbound,ngridc,'ngridc'
     $     ,flag_memory_checkset)
      call cxs_i4(n_k_p_upperbound,n_w_,'n_w_',flag_memory_checkset)
      call cxs_i4(n_k_p_upperbound,ngridc_tmp,'ngridc_tmp'
     $     ,flag_memory_checkset)
      call cxs_i4(n_k_p_max,icstart,'icstart',flag_memory_checkset)
      call cxs_i4(n_k_p_max,n_w_csum_,'n_w_csum_',flag_memory_checkset)
      call cxs_i4(n_k_p_upperbound,ngridps,'ngridps'
     $     ,flag_memory_checkset)
      call cxs_i4(n_k_p_upperbound,n_azimu_b_,'n_azimu_b_'
     $     ,flag_memory_checkset)
      call cxs_r8(n_k_p_max,grid_k_p_,'grid_k_p_'
     $     ,flag_memory_checkset)
      call cxs_r8(n_k_p_max,weight_k_p_,'weight_k_p_'
     $     ,flag_memory_checkset)
      call cxs_r8(n_k_p_upperbound,xnodesth,'xnodesth'
     $     ,flag_memory_checkset)
      call cxs_r8(n_k_p_upperbound,grid_cos_polar_a_
     $     ,'grid_cos_polar_a_',flag_memory_checkset)
      call cxs_r8(n_k_p_upperbound,sthetas,'sthetas'
     $     ,flag_memory_checkset)
      call cxs_r8(n_k_p_upperbound,grid_sin_polar_a_
     $     ,'grid_sin_polar_a_',flag_memory_checkset)
      call cxs_r8(n_k_p_upperbound,weight_cos_polar_a_
     $     ,'weight_cos_polar_a_',flag_memory_checkset)
      call cxs_r8(n_k_p_upperbound,azimu_b_step_,'azimu_b_step_'
     $     ,flag_memory_checkset)
      n = 3*n_source_max
...
c$$$  store timing. ;
     ...
     $     ,flag_memory_checkset)
      ...
      if (verbose.gt.1) then
         write(6,'(A)') '[finished test_ver16_checkset]'
      end if !if (verbose.gt.1) then
