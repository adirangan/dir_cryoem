...
!>        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;\n
!>        Generally speaking, this driver first generates three molecules ;\n
...
!>        in k-space polar coordinates (denoted by n_k_p_max). ;\n
...
!>        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;\n
!>        try: ;\n
!>        make -f test_ver16.make ; ./test_ver16.out 20 0.4 0.3 0.2 0.1 100.0d0 1 ; 
!>        make -f test_ver16.make ; ./test_ver16.out 20 0.4 0.3 0.2 0.1 10.0d0 1 ; 
!>        make -f test_ver16.make ; ./test_ver16.out 20 0.4 0.3 0.2 0.1 1.0d0 1 ; 
!>        make -f test_ver16.make ; ./test_ver16.out 20 0.4 0.3 0.2 0.1 0.1d0 1 ; 
!>        make -f test_ver16.make ; ./test_ver16.out 20 0.4 0.3 0.2 0.1 0.01d0 1 ; 
...
c$$$      parameter (n_k_p_max=16) ! maximum number of k-values (in k-space polar coordinates). sometimes named ngridr. ; read from cmd_K ;
      ...
      parameter (n_x_c_max=60) ! maximum number of x-values (in x-space cartesian coordinates) used to generate initial molecule. sometimes named ngrvdid. ;
...
c$$$      svd_calculation_type == 0 --> multiply, then fft ;
...
c$$$      svd_calculation_type == 2 --> brute force displacements ;
      ...
      parameter (flag_heterogeneity=.true.)
      ...
      parameter (n_M_sample_max_set=0)
...
c$$$      parameter (n_M_1_sub_0in=4)
      ...
      parameter (d_memory_limit=3.0d9)
...
c$$$      parameter (n_MS_max=0)
      logical flag_skip_marching !determines whether to start iteration at n_k_low = n_k_max (.true.) or 2 (.false.). ;
...
c$$$      or temporary. ;
c$$$      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
      integer n !temporary index for allocating size. ;
...
c$$$      parameter (n_k_p_lowerbound=4*n_k_p_max) ! lower bound on the number of k-values (in k-space polar coordintes). sometimes named ntmax. ;
...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
      ...
      real *8, allocatable :: xnodesth(:) !temporary: values for dcos(polar_a) associated with n_polar_a_(nk) in k-space polar coordinates for a particular grid_k_p_(nk). sometimes called grid_cos_polar_a_(nk). ;
      real *8, allocatable :: grid_cos_polar_a_(:) !temporary: values for dcos(polar_a) associated with n_polar_a_(nk) in k-space polar coordinates for a particular grid_k_p_(nk). sometimes called xnodesth(nk). ;
      real *8, allocatable :: sthetas(:) !temporary: values for dsin(polar_a) associated with n_polar_a_(nk) in k-space polar coordinates for a particular grid_k_p_(nk). sometimes called grid_sin_polar_a_(nk). ;
      real *8, allocatable :: grid_sin_polar_a_(:) !temporary: values for dsin(polar_a) associated with n_polar_a_(nk) in k-space polar coordinates for a particular grid_k_p_(nk). sometimes called sthetas(nk). ;
...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
      ...
      character(len=1024) str_k,str_RTXX,str_displacement,str_svd
     ...
     $     ,str_mem
      ...
      integer n_Y_lm_csum_cur !temporary: total number of basis functions used in spherical harmonic expansion (in k-space polar coordinates) out to a particular radius k. sometimes called n_Y_lm_csum_cur
...
c$$$      command line input variables
      ...
      integer ncmd
      real *8 cmd_K,cmd_A,cmd_B,cmd_C,cmd_D,cmd_snr
      integer *4 cmd_rseed
...
c     defocus (in Angstroms) and angle of astigmatism
      ...
      real *8 kgridx2,kgridy2,ctfv, ctfv1
      real *8 lambda,stepr,thetatr
      real *8, allocatable :: envelope(:)
      complex *16, allocatable :: ctfw(:)

      real *8 max_df, min_df, max_ast, min_ast,diff
      integer cn,rr,ll
      real *8 d,theta
...
c      call srand(time())
      ...
      allocate(Voltage(num_ctf))
      allocate(DefocusU(num_ctf))
      allocate(DefocusV(num_ctf))
      allocate(DefocusAngle(num_ctf))
      allocate(SphericalAberration(num_ctf))
      allocate(DetectorPixelSize(num_ctf))
      allocate(Magnification(num_ctf))
      allocate(AmplitudeContrast(num_ctf))
...
c     generate a value between the min and max defocus
         df2 = adi_rand_f(rseed)*(max_df-min_df)
         df2 = df2 + min_df
c         write(6,*) df1
c     generate a value between max and min astigmatism
         diff = adi_rand_f(rseed)*(max_ast-min_ast)
         diff = diff + min_ast
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      n_k_p_max = nint(cmd_K)
      n_k_p_lowerbound = (4*n_k_p_max)
...
c$$$      %%%%%%%%%%%%%%%%
      write(dir_base,'(14(A))') './dir' ,trim(str_k)
...
c-----------------------------------------------------------------------
      allocate(nlats(1+n_k_p_max))
      call cs1_i4(n_k_p_max,nlats)
      allocate(n_polar_a_(0:1+n_k_p_max-1))
      call cs1_i4(n_k_p_max,n_polar_a_)
      allocate(numonsphere(1+n_k_p_max))
      call cs1_i4(n_k_p_max,numonsphere)
      allocate(n_azimu_b_polar_a_sum_(0:1+n_k_p_max-1))
      call cs1_i4(n_k_p_max,n_azimu_b_polar_a_sum_)
      allocate(nterms_sph(1+n_k_p_max))
      call cs1_i4(n_k_p_max,nterms_sph)
      allocate(n_Y_l_(0:1+n_k_p_max-1))
      call cs1_i4(n_k_p_max,n_Y_l_)
      allocate(isph_start(1+n_k_p_max))
      call cs1_i4(n_k_p_max,isph_start)
      allocate(n_Y_lm_csum_(0:1+n_k_p_max-1))
      call cs1_i4(n_k_p_max,n_Y_lm_csum_)
      allocate(ngridc(1+n_k_p_lowerbound))
      call cs1_i4(n_k_p_lowerbound,ngridc)
      allocate(n_w_(0:1+n_k_p_lowerbound-1))
      call cs1_i4(n_k_p_lowerbound,n_w_)
      allocate(ngridc_tmp(1+n_k_p_lowerbound))
      call cs1_i4(n_k_p_lowerbound,ngridc_tmp)
      allocate(icstart(1+n_k_p_max))
      call cs1_i4(n_k_p_max,icstart)
      allocate(n_w_csum_(0:1+n_k_p_max-1))
      call cs1_i4(n_k_p_max,n_w_csum_)
      allocate(ngridps(1+n_k_p_lowerbound))
      call cs1_i4(n_k_p_lowerbound,ngridps)
      allocate(n_azimu_b_(0:1+n_k_p_lowerbound-1))
      call cs1_i4(n_k_p_lowerbound,n_azimu_b_)
      allocate(grid_k_p_(0:1+n_k_p_max-1))
      call cs1_r8(n_k_p_max,grid_k_p_)
      allocate(weight_k_p_(0:1+n_k_p_max-1))
      call cs1_r8(n_k_p_max,weight_k_p_)
      allocate(xnodesth(1+n_k_p_lowerbound))
      call cs1_r8(n_k_p_lowerbound,xnodesth)
      allocate(grid_cos_polar_a_(0:1+n_k_p_lowerbound-1))
      call cs1_r8(n_k_p_lowerbound,grid_cos_polar_a_)
      allocate(sthetas(1+n_k_p_lowerbound))
      call cs1_r8(n_k_p_lowerbound,sthetas)
      allocate(grid_sin_polar_a_(0:1+n_k_p_lowerbound-1))
      call cs1_r8(n_k_p_lowerbound,grid_sin_polar_a_)
      allocate(weight_cos_polar_a_(0:1+n_k_p_lowerbound-1))
      call cs1_r8(n_k_p_lowerbound,weight_cos_polar_a_)
      allocate(azimu_b_step_(0:1+n_k_p_lowerbound-1))
      call cs1_r8(n_k_p_lowerbound,azimu_b_step_)
      allocate(source_x1_c_x2_c_x3_c_A__(0:3-1,0:1+n_source_max-1))
      n = 3*n_source_max
      call cs1_r8(n,source_x1_c_x2_c_x3_c_A__)
      if (flag_heterogeneity) then
         allocate(source_x1_c_x2_c_x3_c_B__(0:3-1,0:1+n_source_max-1))
         n = 3*n_source_max
         call cs1_r8(n,source_x1_c_x2_c_x3_c_B__)
         allocate(source_x1_c_x2_c_x3_c_C__(0:3-1,0:1+n_source_max-1))
         n = 3*n_source_max
         call cs1_r8(n,source_x1_c_x2_c_x3_c_C__)
      end if !if (flag_heterogeneity) then
      ...
      allocate(grid_x3_c___(n_x_c_max,n_x_c_max,1+n_x_c_max))
      n = n_x_c_max*n_x_c_max*n_x_c_max
      call cs1_r8(n,grid_x3_c___)
...
c-----------------------------------------------------------------------
      ...
      do nsource=0,n_source_use-1
         ...
         source_x1_c_x2_c_x3_c_A__(1-1,nsource) = source_r*dcos(source_w)
         source_x1_c_x2_c_x3_c_A__(2-1,nsource) = source_r*dsin(source_w)
...
c
     ...
     $     ,n_azimu_b_polar_a_k_p_sum
      if (verbose.gt.1) call prinf(' isph_start *',isph_start
     $     ,n_k_p_max)
      if (verbose.gt.1) call prinf(' n_Y_lm_csum_ *',n_Y_lm_csum_
     $     ,n_k_p_max)
      if (verbose.gt.1) call prinf(' nterms_sph *',nterms_sph
     $     ,n_k_p_max)
      if (verbose.gt.1) call prinf(' n_Y_l_ *',n_Y_l_,n_k_p_max)
      if (verbose.gt.1) call prinf(' n_Y_lm_sum is *'
     $     ,n_Y_lm_sum,1)
c
      allocate(grid_k1_c_(0:1+n_azimu_b_polar_a_k_p_sum-1))
      n = n_azimu_b_polar_a_k_p_sum
      call cs1_r8(n,grid_k1_c_)
      allocate(grid_k2_c_(0:1+n_azimu_b_polar_a_k_p_sum-1))
      n = n_azimu_b_polar_a_k_p_sum
      call cs1_r8(n,grid_k2_c_)
      allocate(grid_k3_c_(0:1+n_azimu_b_polar_a_k_p_sum-1))
      n = n_azimu_b_polar_a_k_p_sum
      call cs1_r8(n,grid_k3_c_)
      allocate(weight_k_c_(0:1+n_azimu_b_polar_a_k_p_sum-1))
      n = n_azimu_b_polar_a_k_p_sum
      call cs1_r8(n,weight_k_c_)
      allocate(f_k_c_A_(0:1+n_azimu_b_polar_a_k_p_sum-1))
      n = n_azimu_b_polar_a_k_p_sum
      call cs1_c16(n,f_k_c_A_)
      if (flag_heterogeneity) then
         allocate(f_k_c_B_(0:1+n_azimu_b_polar_a_k_p_sum-1))
         n = n_azimu_b_polar_a_k_p_sum
         call cs1_c16(n,f_k_c_B_)
         allocate(f_k_c_C_(0:1+n_azimu_b_polar_a_k_p_sum-1))
         n = n_azimu_b_polar_a_k_p_sum
         call cs1_c16(n,f_k_c_C_)
      end if !if (flag_heterogeneity) then
      allocate(f_k_c_tru_(0:1+n_azimu_b_polar_a_k_p_sum-1))
      n = n_azimu_b_polar_a_k_p_sum
      call cs1_c16(n,f_k_c_tru_)
      allocate(f_k_c_est_(0:1+n_azimu_b_polar_a_k_p_sum-1))
      n = n_azimu_b_polar_a_k_p_sum
...
c
     ...
     $     ,ier)
      ...
      if (verbose.gt.1) then
         ...
         call print_all_i4(n_k_p_max,numonsphere,'numonsphere: ')
         call print_all_i4(n_k_p_max,numonsphere
...
c-----------------------------------------------------------------------
     ...
     $     ,n_azimu_b_ ,azimu_b_step_ ,n_azimu_b_polar_a_sum_tmp)
      ...
      if (verbose.gt.0) then
         call print_all_i4(n_k_p_max,n_w_
     $        ,' n_w_: ')
         call print_all_i4(n_k_p_max,n_azimu_b_polar_a_sum_
     $        ,' n_azimu_b_polar_a_sum_: ')
      end if! if (verbose.gt.0) then
...
c
     ...
     $     icstart)
      call cp1_i4(n_k_p_max,icstart,n_w_csum_)
...
c
     ...
     $     ,alpha2d_tru__,n_M ,ld_M,Y_slice_A__)
c-----------------------------------------------------------------------
...
c
     ...
     $        ,n_w_,k_p_max,alpha2d_tru__,n_M ,ld_M
     $        ,Y_slice_B__)
         diameter_x_c = 2.0d0
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        ,  ' n_heterogeneity_D: ' , n_heterogeneity_D
      ...
      write(6,*) ' checking 0'
      n = n_alpha*n_M
      call cxs_r8(n,alpha2d_tru__,13,'alpha2d_tru__'
     $     ,flag_memory_checkset)
      n = n_alpha*n_M
      call cxs_r8(n,alpha2d_est__,13,'alpha2d_est__'
     $     ,flag_memory_checkset)
      write(6,*) ' finished checking '
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      allocate(ctfw(0:num_ctf*ld_M-1))
      do cn = 1,num_ctf
c     spherical aberation of the lens in mm
         Cs = SphericalAberration(cn)
c     into Angstroms
         Cs=Cs*(10.0d0**7.0d0)
c     voltage in Kvolts
         Kv = Voltage(cn)
...
c     electron wavelength in Angstroms
         lambda = 12.2643247/dsqrt(Kv+Kv**2*0.978466d-6)
c     defocus values (in Angstroms)
         df1 = DefocusU(cn)
         df2 = DefocusV(cn)
c     angle of astigmatism
         angast = DefocusAngle(cn)
c     into radians
         angast = angast*pi/180.0d0
c     Amplitude Contrast
         AmpCnst = AmplitudeContrast(cn)
...
c      call envelope_fxn(ngridr,xnodesr/pi,D,envelope)
      ...
      do nk = 0,n_k_p_max-1
         d = grid_k_p_(nk)
         do ll = 0,n_w_(nk)-1
            theta = 2.0d0*pi*ll/n_w_(nk)
            kgridx2 = d*dcos(theta)
            kgridy2 = d*dsin(theta)
            call niko_ctf(Cs,lambda,w1,w2,df1,df2,angast,
     1           thetatr,kgridx2/pi,kgridy2/pi,ctfv)
            ctfw((cn-1)*ld_CTF+(n_w_csum_(nk)-1)+ll) = -ctfv
         enddo
      enddo
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      ...
      allocate(S_k_p__(0:1+ld_S*n_azimu_b_polar_a_k_p_sum-1))
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $           ,n_k_p_max
            call Fig_gen_ctf_ver3(n_k_p_max,n_polar_a_,grid_k_p_
...
c$$$  store timing. ;
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      allocate(timing_rebuild_tru_(0:1+n_k_p_max + 4 - 1)) ! stores timing. ;
      n = n_k_p_max+4
      call cs1_r8(n,timing_rebuild_tru_)
      allocate(timing_residual_tru_(0:1+n_k_p_max + 4 - 1)) ! stores timing. ;
      n = n_k_p_max+4
      call cs1_r8(n,timing_residual_tru_)
      allocate(timing_rebuild_est_(0:1+n_k_p_max + 4 - 1)) ! stores timing. ;
      n = n_k_p_max+4
      call cs1_r8(n,timing_rebuild_est_)
      allocate(timing_residual_est_(0:1+n_k_p_max + 4 - 1)) ! stores timing. ;
      n = n_k_p_max+4
      call cs1_r8(n,timing_residual_est_)
      allocate(timing_template_create_(0:1+n_k_p_max + 4 - 1)) ! stores timing. ;
      n = n_k_p_max+4
      call cs1_r8(n,timing_template_create_)
      allocate(timing_innerproduct_(0:1+n_k_p_max + 4 - 1)) ! stores timing. ;
      n = n_k_p_max+4
      call cs1_r8(n,timing_innerproduct_)
      allocate(timing_alpha_update_(0:1+n_k_p_max + 4 - 1)) ! stores timing. ;
      n = n_k_p_max+4
...
c     
      ...
      if (n_k_low.gt.3) then
         write(6,'(A,I0)') 'Note: n_k_low starting at: ' , n_k_low
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         ...
         timing_tic = omp_get_wtime()
         call rebuild_model_2(n_M_sample,I_M_sample_,ld_M,Y_slice__
     $        ,n_ctf ,ld_CTF,CTF_k_p__,alpha_tru__ ,n_w_csum_
     $        ,n_polar_a_,quadrature_type_azimu_b,grid_k_p_,n_w_,n_k_low
     $        ,n_k_cur,n_Y_lm_csum_,n_Y_l_ ,lsq_oversample
     $        ,lsq_interpolation_order,eps_default ,Y_tru_)
         timing_toc = omp_get_wtime()
         timing_tot = timing_toc-timing_tic
         if (verbose.gt.0) write(6,'(A,F8.4,A)') ' timing: ' ,
     $        timing_tot ,' <-- rebuild_tru '
         timing_rebuild_tru_(n_k_cur) = timing_tot
         if (verbose.gt.0) then
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */         
         if (flag_residual) then
            ...
            if (n_k_cur.lt.n_k_p_max) then
               n_Y_lm_csum_cur = n_Y_lm_csum_(n_k_cur+1-1)
            end if !if (n_k_cur.lt.n_k_p_max) then
            call test_residual_5(rseed,n_M_sample,I_M_sample_,ld_M
     $           ,Y_slice__,n_ctf,ld_CTF,CTF_k_p__,alpha_tru__
     $           ,n_w_csum_,n_polar_a_,quadrature_type_azimu_b,grid_k_p_
     $           ,n_w_,n_k_1,n_k_cur,n_Y_lm_csum_cur ,n_Y_lm_csum_ ,n_Y_l_
     ...
     $           timing_tot ,' <-- residual_tru '
            timing_residual_tru_(n_k_cur) = timing_tot
         end if !if (flag_residual) then
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $           ,I_M_sample_,ld_M,Y_slice__,M_residual_sample__
     $           ,alpha_tru__,min(16 ,n_M_sample) ,tmp_fname)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
         ...
         if (flag_tesselation) then
            tesselation_distance_req = 2.0d0 / max(1,n_k_cur-16)
...
c$$$      svd_calculation_type == 2 --> account for displacements via brute-force. ;
     ...
     $      ,timing_rebuild_est_(n_k_cur) ! time to rebuild Y_est_. ;
     $      ,timing_template_create_(n_k_cur) ! time to create templates. ;
     $      ,timing_innerproduct_(n_k_cur) ! time to calculate innerproducts. ;
     $      )
         timing_toc = omp_get_wtime()
         timing_tot = timing_toc-timing_tic
         if (verbose.gt.0) write(6,'(A,F8.4,A)') ' timing: ' ,
     $        timing_tot ,' <-- alpha_update '
         timing_alpha_update_(n_k_cur) = timing_tot
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */         
         if (flag_residual) then
            ...
            call test_residual_5(rseed,n_M_sample,I_M_sample_,ld_M
     $           ,Y_slice__,n_ctf,ld_CTF,CTF_k_p__,alpha_bkp__
     $           ,n_w_csum_,n_polar_a_,quadrature_type_azimu_b,grid_k_p_
     $           ,n_w_,n_k_1,n_k_cur,n_Y_lm_csum_cur ,n_Y_lm_csum_ ,n_Y_l_
     ...
     $           timing_tot ,' <-- residual_est '
            timing_residual_est_(n_k_cur) = timing_tot
         end if !if (flag_residual) then
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $           '/Fig_residual_est_',n_k_cur
            call Fig_R_ver1(n_k_cur,n_polar_a_,grid_k_p_,n_M_sample
     $           ,I_M_sample_,ld_M,Y_slice__,M_residual_sample__
     $           ,alpha_bkp__,min(16 ,n_M_sample) ,tmp_fname)
         end if !if (flag_residual .and. flag_fig) then
