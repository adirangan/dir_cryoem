...
!> Build index list for tesselation-tree. ;\n
     ...
     $ tradius_min , nl_max , nl_0in ,
     $ v0_i , v1_i , v2_i ,
     $ parity ,
     $ n_L , L_ , LT_ , TL_ , 
     $ index_parent , index_0input , ls_0input ,
     $ T_nl_ , 
     $ T_up_ , 
     $ T_id_ , 
     $ T_pa_ , 
     ...
     $ ls_output)
      ...
      data verbose / 0 /
      real *8 tradius_min
      integer *4 nl_max,nl_0in,n_L
      integer *4 index_parent
      integer *4 index_0input,index_output
      ...
      write(format_string,'(A,I0,A)') '(A',nl_0in+1,')'
      write(prefix,format_string) '%'      
      if (verbose.gt.0) then
         write(6,'(A,A,I0)') trim(prefix) ,
     $        '[entering tesselation_index]: ',nl_0in
     ...
     $           +3*nL) , L_(1+3*nL) , L_(2+3*nL)
      ...
      if (n_L_sub.ge.1) then
         ...
         T_nl_(1*index_0input) = nl_0in
         T_up_(1*index_0input) = parity
         T_id_(1*index_0input) = index_0input
         T_pa_(1*index_0input) = index_parent
         call cp1_r8(3,v0_k,T_v0_(3*index_0input))
         call cp1_r8(3,v1_k,T_v1_(3*index_0input))
         call cp1_r8(3,v2_k,T_v2_(3*index_0input))
         call cp1_r8(3,vm,T_vm_(3*index_0input))
         call cp1_r8(3,m0,T_m0_(3*index_0input))
         call cp1_r8(3,m1,T_m1_(3*index_0input))
         call cp1_r8(3,m2,T_m2_(3*index_0input))
         call cp1_r8(3,e0,T_e0_(3*index_0input))
         call cp1_r8(3,e1,T_e1_(3*index_0input))
         call cp1_r8(3,e2,T_e2_(3*index_0input))
         call cp1_r8(3,n0,T_n0_(3*index_0input))
         call cp1_r8(3,n1,T_n1_(3*index_0input))
         call cp1_r8(3,n2,T_n2_(3*index_0input))
         call cp1_r8(3,nn,T_nn_(3*index_0input))
         T_tr_(1*index_0input) = tradius
         T_ll_(1*index_0input) = n_L_sub
         T_lf_(1*index_0input) = lf
         T_c0_(1*index_0input) = -1
         T_c1_(1*index_0input) = -1
         T_c2_(1*index_0input) = -1
         T_c3_(1*index_0input) = -1
         T_ls_(1*index_0input) = -1
         index_output = index_0input + 1
         ls_output = ls_0input
         if (lf) then
            if (verbose.gt.0) then
     ...
     $              'leaf, copying LT_sub to T_LT_ '
            ...
            T_ls_(1*index_0input) = ls_0input
            do nL_sub=0,n_L_sub-1
               T_LT_(ls_0input + nL_sub) = LT_sub_(nL_sub)
               TL_(LT_sub_(nL_sub)) = index_0input
            enddo !do nL_sub=0,n_L_sub-1
            ls_output = ls_0input + n_L_sub
         end if !if (lf) then
         if (.not. lf) then
            if (verbose.gt.1) then
     ...
     $           tradius_min , nl_max , nl_0in + 1 ,
     $           v0_k , m2 , m1 ,
     $           parity ,
     $           n_L_sub , L_sub_ , LT_sub_ , TL_ , 
     $           T_id_(index_0input) , index_output , ls_output ,
     $           T_nl_ , 
     $           T_up_ , 
     $           T_id_ , 
     $           T_pa_ , 
     ...
     $           ls_tempA)
            ...
            if (index_output.gt.index_tempB) then
               T_c0_(index_0input) = index_tempB
     ...
     $           ls_tempA)
            ...
            ls_output = ls_tempA
            if (index_output.gt.index_tempB) then
               T_c1_(index_0input) = index_tempB
            end if !if (index_output.gt.index_tempB) then
     ...
     $           ls_tempA)
            ...
            ls_output = ls_tempA
            if (index_output.gt.index_tempB) then
               T_c2_(index_0input) = index_tempB
            end if !if (index_output.gt.index_tempB) then
     ...
     $           ls_tempA)
            ...
            ls_output = ls_tempA
            if (index_output.gt.index_tempB) then
               T_c3_(index_0input) = index_tempB
            end if !if (index_output.gt.index_tempB) then
