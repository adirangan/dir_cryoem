!> Doxygen comment: ;\n
!> check memory allocations for test_alpha_update_8.f ;\n
      if (verbose.gt.1) then
         write(6,'(A)')
     $        '[entering test_alpha_update_8_excerpt_checkset]'
      end if !if (verbose.gt.1) then
      
      flag_memory_checkset = .true.

      call cxs_c16(n_M_sample,C_M_,'C_M_',flag_memory_checkset)
      call cxs_r8(n_azimu_b__sum_cur,S_alpha_S_index_all_,
     $     'S_alpha_S_index_all_',flag_memory_checkset)
      call cxs_r8(n_azimu_b__sum_cur,S_alpha_polar_a_all_,
     $     'S_alpha_polar_a_all_',flag_memory_checkset)
      call cxs_r8(n_azimu_b__sum_cur,S_alpha_azimu_b_all_,
     $     'S_alpha_azimu_b_all_',flag_memory_checkset)
      call cxs_r8(n_k_p_r_upperbound,grid_cos_polar_a_tmp_,
     $     'grid_cos_polar_a_tmp_',flag_memory_checkset)
      call cxs_r8(n_k_p_r_upperbound,grid_sin_polar_a_tmp_,
     $     'grid_sin_polar_a_tmp_',flag_memory_checkset)
      call cxs_r8(n_k_p_r_upperbound,weight_cos_polar_a_tmp_,
     $     'weight_cos_polar_a_tmp_',flag_memory_checkset)
      call cxs_r8(n_k_p_r_upperbound,azimu_b_step_tmp_,
     $     'azimu_b_step_tmp_',flag_memory_checkset)
      call cxs_i4(n_k_p_r_upperbound,n_azimu_b_tmp_,'n_azimu_b_tmp_'
     ...
     $     ,flag_memory_checkset)
      ...
      if (verbose.gt.1) then
         write(6,'(A)')
     $        '[finished test_alpha_update_8_excerpt_checkset]'
      end if !if (verbose.gt.1) then
