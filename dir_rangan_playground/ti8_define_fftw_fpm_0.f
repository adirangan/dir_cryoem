...
!> Generates the fftw_many_plan for use with multiple omp sub-blocks. ;\n
     ...
     $        ,' MB, ' , d_mem*1.0d-9 , ' GB'
      ...
      d_mem = 0.0d0
      d_mem = d_mem + 1.0d0*n_fpm*1.0d0*n_omp_sub_0in*16.0d0
      if (verbose.gt.1 .or. verbose_mem.gt.0) then
         write (6,'(A,2(F16.8,A))') ' fpm_0in__ requires ' , d_mem*1.0d
     $        -6,' MB, ' , d_mem*1.0d-9 , ' GB'
      end if !if (verbose.gt.0) then
      allocate(fpm_0in__(0:n_fpm*n_omp_sub_0in-1))
      allocate(fpm_out__(0:n_fpm*n_omp_sub_0in-1))     
      allocate(fpm_back_(0:n_omp_sub_0in-1))
      timing_tic = omp_get_wtime()
c$$$      We do not actually use the forward plan
      do nomp_sub=0,n_omp_sub_0in-1      
         call dfftw_plan_many_dft(fpm_back_(nomp_sub),fpm_rank
     $        ,fpm_n__(nomp_sub),fpm_howmany,fpm_0in__(n_fpm*nomp_sub)
     ...
     $        ,fpm_ostride ,fpm_odist ,FFTW_BACKWARD,FFTW_MEASURE)
      ...
      gnump_tot = (1.0d0*n_fpm)*(1.0d0*n_omp_sub_0in)
