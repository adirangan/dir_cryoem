...
!> Upgraded to use radial quadrature weight. ;\n
      subroutine innerproduct_q_k_svdr_quad_polyval_off_0(flag_S_vs_M
     $     ,svd_r_max,n_svd_r,svd_r_,n_svd_l,svd_l_,svd_s_,svd_V_r_
     $     ,svd_polyval_V_r_,n_r,grid_p_,weight_p_,n_w_,n_A ,T_q_,M_q_
     $     ,C_q_)
...
c$$$      Upgraded to use radial quadrature weight. ;\n
      ...
      if (verbose.gt.0) then
         write(6,'(A,A,AI0)') ' % [entering ' ,
     $        'innerproduct_q_k_svdr_quad_polyval_off_0' , '] n_r ',n_r
      end if
...
c$$$         We assume that the fourier basis is orthonormal (not merely orthogonal)
     ...
     $                 *T_q_(ic))*M_q_(ict)
      ...
      if (verbose.gt.0) then
         write(6,'(A)')
     $        ' % [finished innerproduct_q_k_svdr_quad_polyval_off_0]'
      end if
      end
