...
!> Generating fftw_plans for omp sub-blocks. ;\n
      ...
      allocate(fftw_0in__(0:n_A*n_omp_sub_0in-1))
      allocate(fftw_out__(0:n_A*n_omp_sub_0in-1))
     ...
     $           ,n_w_(nr),fftw_0in__(n_A_tmp+na),fftw_out__(n_A_tmp
     $           +na),FFTW_FORWARD,FFTW_ESTIMATE) 
            call dfftw_plan_dft_1d_(fftw_plan_back__(n_r_tmp+nr)
     $           ,n_w_(nr),fftw_out__(n_A_tmp+na),fftw_0in__(n_A_tmp
     $           +na),FFTW_BACKWARD,FFTW_ESTIMATE) 
      ...
      gnump_tot = (1.0d0*n_A)*(1.0d0*n_omp_sub_0in)
