...
!> Tests S_T_T_R_CTF_M_q_ calculation directly. ;\n
      subroutine ti8_check_SZxTRM_4(verbose
     $     ,svd_d_max
     $     ,n_svd_d
     $     ,svd_d_
     $     ,svd_r_max
...
c$$$         Define C_q0_
         do ndv=0,n_delta_v-1
            ...
            call transf_p_to_p(n_r,grid_p_,n_w_ ,n_w_sum,S_p_,+delta_x,
     $           +delta_y,S_p_)
     ...
     $           ,n_w_,n_w_sum,fftw_0in_,fftw_out_,M_p_,-gamma_z_est
     $           ,M_p_)
            call transf_p_to_p(n_r,grid_p_,n_w_,n_w_sum,M_p_,
     $           -delta_x_est,-delta_y_est,M_p_)
            do nw=0,n_w_max-1
               gamma_z = (2.0d0*pi*nw)/(1.0d0*n_w_max)
               call rotate_p_to_p_fftw(n_r,fftw_plan_frwd_
     $              ,fftw_plan_back_,n_w_,n_w_sum,fftw_0in_,fftw_out_
     $              ,S_p_, +gamma_z,S_q_)
               call innerproduct_p_quad(n_r,grid_p_,weight_p_,n_w_
     $              ,n_w_sum,S_q_,M_p_,C_q0_(nw + n_w_max*ndv))
...
c$$$         Define C_q1_
     ...
     $           ,n_w_,n_w_sum,S_q_,M_q_,C_q1_(n_w_max*ndv))
            call cp1_c16(n_w_max,C_q1_(n_w_max*ndv),fftw_out_last_)
            call dfftw_execute_(fftw_plan_back_last)
            call cp1_c16(n_w_max,fftw_0in_last_,C_q1_(n_w_max*ndv))
         enddo                  !do ndv=0,n_delta_v-1
c$$$         Define C_q2_
         do ndv=0,n_delta_v-1
            delta_x = delta_x_(ndv)
     ...
     $           ,svd_U_d_,svd_s_,svd_V_r_,n_r,grid_p_,n_w_,n_w_sum,S_q_
     $           ,delta_x,delta_y,S_q_)
            delta_x_est = +delta_x_est_(nm)
            delta_y_est = +delta_y_est_(nm)
            gamma_z_est = +gamma_z_est_(nm)
            nctf = nint(ctf_ind_est_(nm))
     ...
     $           ,fftw_0in_,fftw_out_,M_p_,M_q_)
            call innerproduct_q_k_stretch_quad_0(n_r,grid_p_ ,weight_p_
     $           ,n_w_,n_w_sum,S_q_,M_q_,C_q2_(n_w_max*ndv))
            call cp1_c16(n_w_max,C_q2_(n_w_max*ndv),fftw_out_last_)
            call dfftw_execute_(fftw_plan_back_last)
            call cp1_c16(n_w_max,fftw_0in_last_,C_q2_(n_w_max*ndv))
         enddo                  !do ndv=0,n_delta_v-1
c$$$         Define S_q
         call cp1_c16(n_w_sum,S_p__(I_S_sample_(ns)*ld_S),S_p_)
         call interp_p_to_q_fftw(n_r,fftw_plan_frwd_,n_w_ ,n_w_sum
