...
!> Tests S_T_T_R_CTF_M_q_ calculation directly. ;\n
      subroutine ti8_check_SxZTRM_4(verbose
     $     ,svd_d_max
     $     ,n_svd_d
     $     ,svd_d_
     $     ,svd_r_max
...
c$$$      %%%%%%%% ;
      MDA_n_d = 1
      MDA_d_(0) = 1
      write(MDA_string,'(A)') './dir_tmp/n_w_sum.mda'
      call MDA_write_i4(MDA_n_d,MDA_d_,n_w_sum,MDA_string)
c$$$      %%%%%%%% ;
      MDA_n_d = 1
      MDA_d_(0) = 1
...
c$$$      %%%%%%%% ;
      end if !if (flag_dump.eqv..true.) then
...
c$$$      %%%%%%%% ;
      ...
      allocate(T_p_(0:n_w_sum-1))
      call cl1_c16(n_w_sum,T_p_)
      allocate(T_q_(0:n_w_sum-1))
      call cl1_c16(n_w_sum,T_q_)
...
c$$$         Combine S_q and M_q into Z_S_svdr_
     ...
     $        ,n_svd_r,svd_r_,n_svd_l,svd_l_,svd_s_ ,svd_V_r_,n_r
     $        ,grid_p_,weight_p_,n_w_,n_w_sum,S_q_,M_q_ ,Z_S_svdr_)
c$$$         %%%%%%%%
         if (flag_dump.eqv..true.) then
         if ((ns.eq.ns_dump) .and. (nm.eq.nm_dump)) then
            MDA_n_d = 1
...
c$$$         fftw each term in Z_S_svdr_
     ...
     $           ,fftw_out_last_,1)
            ...
            call cps_c16(n_w_max,fftw_0in_last_,1,Z_S_svdr_(nsvd_l)
     $           ,n_svd_l)
...
c$$$         %%%%%%%%
     ...
     $        ,n_r,grid_p_,weight_p_,n_w_,n_w_sum,S_q_,M_q_ ,Z_M_svdr_)
c$$$         %%%%%%%%
         if (flag_dump.eqv..true.) then
         if ((ns.eq.ns_dump) .and. (nm.eq.nm_dump)) then
            MDA_n_d = 1
...
c$$$         fftw each term in Z_M_svdr_
     ...
     $           ,fftw_out_last_,1)
            call dfftw_execute_(fftw_plan_back_last)
            call cps_c16(n_w_max,fftw_0in_last_,1,Z_M_svdr_(nsvd_l)
     $           ,n_svd_l)
         enddo                  !do svd_l=0,n_svd_l-1
c$$$         %%%%%%%%
         if (flag_dump.eqv..true.) then
...
c$$$  nC1 = nw + ndv*n_w_max
               C_S_q_(nC1) = dcmplx(0.0d0,0.0d0)
               C_M_q_(nC1) = dcmplx(0.0d0,0.0d0)
               nC2 = ndv*n_svd_l
               nC3 = nw*n_svd_l
               do nsvd_l=0,n_svd_l-1
...
c$$$         %%%%%%%%
         do ndv=0,n_delta_v-1
            ...
            do nw=0,n_w_max-1
               ...
               call cp1_c16(n_w_sum,S_q_,T_q_)
               call transf_svd_q_to_q_polyval_0on_0(svd_r_max,n_svd_r
     $              ,svd_r_,svd_d_max,n_svd_d,svd_d_,n_svd_l,svd_l_
     $              ,svd_U_d_,svd_s_,svd_V_r_,n_r,grid_p_,n_w_,n_w_sum
     $              ,T_q_ ,delta_x,delta_y,T_q_)
               call rotate_q_to_q(n_r,n_w_,n_w_sum,T_q_,gamma_z,T_q_)
               call innerproduct_p_quad(n_r,grid_p_,weight_p_,n_w_
     $              ,n_w_sum,T_q_,M_q_,S_T_R_x_T_R_CTF_M_q__(nw))
               call cp1_c16(n_w_sum,S_q_,T_q_)
               call rotate_q_to_q(n_r,n_w_,n_w_sum,T_q_,gamma_z,T_q_)
               call transf_svd_q_to_q_polyval_0on_0(svd_r_max,n_svd_r
     $              ,svd_r_,svd_d_max,n_svd_d,svd_d_,n_svd_l,svd_l_
     ...
     $              ,T_q_ ,delta_x,delta_y,T_q_)
               call innerproduct_p_quad(n_r,grid_p_,weight_p_,n_w_
     $              ,n_w_sum,T_q_,M_q_,S_R_T_x_T_R_CTF_M_q__(nw))
               nC4 = nw + n_w_max*ndv
