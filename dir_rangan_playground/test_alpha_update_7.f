...
!> 4.  Update stack of image-parameters alpha_est__. ;\n
      subroutine test_alpha_update_7(
...
c$$$      svd_calculation_type == 2 --> account for displacements via brute-force. ;
     ...
     $      ,n_w_ ! n_w_(nk) is the number of points on the ring of radius grid_k_p_(nk) for a template or image in k-space polar coordinates. also called ngridc(nk). ;
     $      ,n_k_low !integer *4: lowest value of nk (k-index in k-space polar coordinates) to use when building models. Note that this runs from 1 to n_k_p_max. ;
     ...
     $      ,n_azimu_b__sum_ ! total number of points on sphere at radius grid_k_p_(nk). also called numonsphere(nk). ;
...
c$$$  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
      ...
      integer n_azimu_b__sum_(0:n_k_cur-1) ! total number of points on sphere at radius grid_k_p_(nk). also called numonsphere(nk). ;
...
c$$$  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
      ...
      integer n_azimu_b__sum_cur !temporary: n_azimu_b__sum_(n_k_cur-1). sometimes called numonsphere_cur. ;
      ...
      integer n_azimu_b__sum_tmp !temporary: total number of points on sphere at a particular radius grid_k_p_(nk) in k-space polar coordinates. sometimes called nspherebig_tmp. ;
...
c$$$  svd_calculation_type == 2 --> account for displacements via brute-force. ;
     ...
     $        lsq_interpolation_order ! least-squares-solver interpolation order. sometimes named kord. ;
         ...
         call print_sub_i4(n_k_cur,n_azimu_b__sum_,
     $        ' n_azimu_b__sum_: ') ! total number of points on sphere at radius grid_k_p_(nk). also called numonsphere(nk). ;
      ...
      n_azimu_b__sum_cur = n_azimu_b__sum_(n_k_cur-1)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $     ,CTF_k_p__ !complex *16 array (length at least ld_CTF*n_ctf): stack of CTF functions. ;
     $     ,alpha_est__ !real *8 array (length at least n_alpha*n_M_sample): estimated image parameters associated with sampled image set stored in 1-dimensional array. Note that we expect alpha_est__(n_alpha*nm) to apply to image M_k_p__(I_M_sample_(nm)*ld_M). ;
     $     ,n_w_csum_ !integer *4 array (length at least n_k_cur): cumulative sum of n_w_. also called icstart(nk). ;
     $     ,n_polar_a_ !integer *4 array (length at least n_k_cur): number of polar_a values on sphere of radius grid_k_p_(nk). also called nlats(nk). ;
     $     ,quadrature_type_azimu_b ! quadrature type in the azimuthal direction. sometimes named itypep. ;
     $     ,grid_k_p_ !real *8 array (length at least n_k_cur): values for k on successive shells for k-space polar coordinates. sometimes called grid_p_ or xnodesr. ;
     $     ,n_w_ !integer *4 array (length at least n_k_cur): n_w_(nk) is the number of points on the ring of radius grid_k_p_(nk) for a template or image in k-space polar coordinates. also called ngridc(nk). ;
     $     ,n_k_low !integer *4: lowest value of nk (k-index in k-space polar coordinates) to use when building models. Note that this runs from 1 to n_k_p_max. ;
     $     ,n_k_cur !integer *4: current index for current maximum value of n_k. Note that this runs from 1 to n_k_p_max. ;
     $     ,n_Y_lm_csum_ !integer *4 array (size at least n_k_cur): cumulative sum of n_Y_lm_. also called isph_start(nk). ;
     $     ,n_Y_l_ !integer *4 array (size at least n_k_cur): order of spherical harmonic expansion on sphere at radius grid_k_p_(nk). also called nterms_sph(nk). ;
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $     ,n_Y_l_ !integer *4 array (size at least n_k_cur): order of spherical harmonic expansion on sphere at radius grid_k_p_(nk). also called nterms_sph(nk). ;
     $     ,n_k_cur !integer *4: current index for current maximum value of n_k. Note that this runs from 1 to n_k_p_max. ;
     $     ,n_w_ !integer *4 array (size at least n_k_cur): n_w_(nk) is the number of points on the ring of radius grid_k_p_(nk) for a template or image in k-space polar coordinates. also called ngridc(nk). ;
     $     ,ld_S !integer *4: leading dimension of S_k_p__. Must be at least n_A. ;
     $     ,n_w_csum_ !integer *4 array (size at least n_k_cur): cumulative sum of n_w_. also called icstart(nk). ;
     $     ,n_polar_a_ !integer *4: number of polar_a values on sphere of radius grid_k_p_(nk). ;
     ...
     $     ,n_k_cur !integer *4: current index for current maximum value of n_k. Note that this runs from 1 to n_k_p_max. ;
     $     ,n_azimu_b__sum_cur !temporary integer *4: n_azimu_b__sum_(n_k_cur-1). sometimes called numonsphere_cur. ;
     $     ,S_k_p__ !complex *16 array (length at least ld_S*max_i4_f_(I_S_sample_)): stack of templates associated with reconstructed molecule. sometimes called Y_slice__ or cslices or templates. ;
     $     )
      timing_toc = omp_get_wtime()
     ...
     $        ,timing_toc-timing_tic
      ...
      call getspheregrid(
     $     n_polar_a_(n_k_cur-1) !integer *4: number of polar_a values on sphere of radius grid_k_p_(nk). ;
     $     ,quadrature_type_azimu_b !integer: quadrature type in the azimuthal direction. sometimes named itypep. 0=same number of points for all n_polar_a_ (oversampling poles). 1=adaptive (samples with roughly uniform density). ;
     $     ,grid_cos_polar_a_tmp_ !temporary: real *8 array (size at least n_k_p_max): values for dcos(polar_a) associated with n_polar_a_(nk) in k-space polar coordinates for a particular grid_k_p_(nk). sometimes called xnodesth(nk). ;
     $     ,grid_sin_polar_a_tmp_ !temporary: real *8 array (size at least n_k_p_max): values for dsin(polar_a) associated with n_polar_a_(nk) in k-space polar coordinates for a particular grid_k_p_(nk). sometimes called sthetas(nk). ;
     $     ,weight_cos_polar_a_tmp_ !temporary real *8 array (size at least n_k_p_max): weight associated with grid_cos_polar_a_(nk) in k-space polar coordinates for a particular grid_k_p_(nk). sometimes called wtsth_(np). ;
     $     ,n_azimu_b_tmp_ !temporary integer *4 array (size at least n_k_p_max): number of nodes in azimu_b for each polar_a in k-space polar coordinates for a particular grid_k_p_(nk). also called ngridps(np). ;
     $     ,azimu_b_step_tmp_ !temporary real *8 array (size at least n_k_p_max): grid-spacing for azimu_b. ;
     $     ,n_azimu_b__sum_tmp !temporary integer *4: total number of points on sphere at a particular radius grid_k_p_(nk) in k-space polar coordinates. sometimes called nspherebig_tmp. ;
     $     )
      if (n_azimu_b__sum_tmp.ne.n_azimu_b__sum_cur)
     $     then
         write(6,'(A,I0,A,I0)') 'Warning, n_azimu_b__sum '
     $        ,n_azimu_b__sum_tmp,'.ne.'
     $        ,n_azimu_b__sum_(n_k_cur-1)
      end if
      nA = 0
...
c$$$  calling checkset. ;
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      include 'test_alpha_update_7_checkset.f'
