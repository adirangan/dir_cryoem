...
!> Upgraded to use radial quadrature weight. ;\n
      subroutine innerproduct_q_k_svdr_quad_polyval_0on_0(flag_S_vs_M
     $     ,svd_r_max,n_svd_r,svd_r_,n_svd_l,svd_l_,svd_s_,svd_V_r_,n_r
     $     ,grid_p_,weight_p_,n_w_,n_A,T_q_,M_q_,C_q_)
...
c$$$      Upgraded to use radial quadrature weight. ;\n
      ...
      if (verbose.gt.0) then
         write(6,'(A,A,A,I0)') ' % [entering '
     $        ,'innerproduct_q_k_svdr_quad_polyval_0on_0','] n_r ',n_r
      end if
     ...
     $           ,svd_r(0),V_r_(nl+nr*n_svd_l))
      ...
      do nl=0,n_svd_l-1
         do nw=0,n_w_max-1
            C_q_(nl+nw*n_svd_l) = dcmplx(0.0d0,0.0d0)
         enddo
      enddo
...
c$$$         We assume that the fourier basis is orthonormal (not merely orthogonal)
     ...
     $                    ,'; nw_fix ',nw_fix,'; (full loop)'
                  ...
                  if (flag_S_vs_M.eqv..true.) C_q = dconjg(D_s*D_V_r
     $                 *T_q_(ict))*M_q_(ic)
                  if (flag_S_vs_M.eqv..false.) C_q = dconjg(D_s*D_V_r
     $                 *T_q_(ic))*M_q_(ict)
     ...
     $                 *dconjg(D_s*D_V_r*T_q_(ict))*M_q_(ic)
                  if (flag_S_vs_M.eqv..false.) C_q = 0.0d0*0.5d0
     $                 *dconjg(D_s*D_V_r*T_q_(ic))*M_q_(ict)
                  nw_C = nl + nw_fix*n_svd_l
                  C_q_(nw_C) = C_q_(nw_C) + C_q*dAn
                  nw_fix = nw - n_w_(nr) + n_w_max
                  if (verbose.gt.3 .and. nr.lt.5) then
     ...
     $                 *T_q_(ic))*M_q_(ict)
      ...
      if (verbose.gt.0) then
         write(6,'(A)')
     $        ' % [finished innerproduct_q_k_svdr_quad_polyval_0on_0]'
      end if
      end
