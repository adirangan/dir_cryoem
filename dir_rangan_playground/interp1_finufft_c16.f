      subroutine interp1_finufft_c16(n_x,min_x,max_x,F_,fftw_plan_frwd
     $     ,fftw_plan_back,fftw_0in_,fftw_out_,n_x_loc,x_loc_,output_)
...
c$$$      periodic boundary conditions
      ...
      if (verbose.gt.0) then
         write(6,'(A)') ' [entering interp1_finufft_c16] '
     ...
     $     *dcmplx(0.0d0),fftw_out_,G_) ! frequency range 2.0d0*pi*[0,n_x-1]
      ...
      iflag = +1
      eps = 1.0d-12
      call finufft1d2_f(n_x_loc,y_loc_,J_,iflag,eps,n_x,H_,ier)
      if (ier.ne.0) then
         write(6,'(A)') 'Warning! ier.ne.0 in interp1_finufft_c16.f'
     ...
     $     *dcmplx(0.0d0),J_,output_) 
      ...
      if (verbose.gt.0) then
         write(6,'(A)') ' [finished interp1_finufft_c16] '
      end if !if (verbose.gt.0) then
      end !subroutine
