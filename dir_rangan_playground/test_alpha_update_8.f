...
!> 4.  Update stack of image-parameters alpha_est__. ;\n
      subroutine test_alpha_update_8(
...
c$$$      svd_calculation_type == 2 --> account for displacements via brute-force. ;
     ...
     $      ,d_memory_limit !real *8: upper limit on the memory allowed by ti8 (in bytes). ;
...
c$$$      parameter(flag_memory_estimate=.false.)
      ...
      parameter(flag_time_Zstore=.false.)
      ...
      integer *4 n_k_p_r_max ! integer *4: maximum number of k-values (in k-space polar coordinates). sometimes named ngridr. ;
      real *8 k_p_r_max ! real *8, maximum value of k in k-space polar coordinates. sometimes called rmax. ;
      integer *4 n_k_p_r_cur !integer *4: current index for current maximum value of n_k. Note that this runs from 1 to n_k_p_r_max. ;
      integer *4 n_polar_a_(0:n_k_p_r_cur-1) !integer *4 array (length at least n_k_p_r_cur): number of polar_a values on sphere of radius grid_k_p_r_(nk). also called nlats(nk). ;
      real *8 grid_k_p_r_(0:n_k_p_r_cur-1) !real *8 array (length at least n_k_p_r_cur): values for k on successive shells for k-space polar coordinates. sometimes called grid_p_ or xnodesr. ;
      real *8 weight_k_p_r_(0:0) ! real *8 array (length at least n_k_p_r_max). Weight associated with grid_k_p_r_(nk) in k-space polar coordinates. sometimes called wtsr(nk). ;
      real *8 weight_k_p_(0:0) ! real *8 array (length at least n_w_sum). Contains weight for each point in unrolled polar grid. ;
...
c$$$      svd_calculation_type == 2 --> account for displacements via brute-force. ;
      ...
      real *8 d_memory_limit !real *8: upper limit on the memory allowed by ti8. ;
      ...
      complex *16, allocatable :: C_M_(:) !temporary: complex *16 array (length at least n_M_sample): actual l2-norm (out to frequency n_k_p_r_cur) for each sampled image. Note that we expect C_M_(nm) to apply to image M_k_p__(I_M_sample_(nm)*ld_M). Note also that C_M_(nm) is a raw l2-norm, with no consideration for the CTF. ;
c$$$  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
c$$$  Other variables used to generate templates and perform lsq-solve. ;
c$$$  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
      integer n_w_csum_(0:n_k_p_r_cur-1) ! cumulative sum of n_w_. also called icstart(nk). ;
      ...
      integer n_w_(0:n_k_p_r_cur-1) ! ngridc(nk) is the number of points on the ring of radius grid_k_p_r_(nk) for a template or image in k-space polar coordinates. also called ngridc(nk). ;
      ...
      integer n_Y_lm_csum_(0:n_k_p_r_cur-1) ! cumulative sum of n_Y_lm_. also called isph_start(nk). ;
      integer n_Y_l_(0:n_k_p_r_cur-1) ! order of spherical harmonic expansion on sphere at radius grid_k_p_r_(nk). also called nterms_sph(nk). ;
      ...
      integer n_azimu_b__sum_(0:n_k_p_r_cur-1) ! total number of points on sphere at radius grid_k_p_r_(nk). also called numonsphere(nk). ;
...
c$$$  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
      ...
      integer nk !temporary: index for grid_k_p_r_(nk). ;
      ...
      integer n_k_p_r_upperbound ! upper bound on the number of k-values (in k-space polar coordintes). sometimes named ntmax. ;      
      real *8, allocatable :: grid_cos_polar_a_tmp_(:) !temporary: values for dcos(polar_a) associated with n_polar_a_(nk) in k-space polar coordinates for a particular grid_k_p_r_(nk). sometimes called xnodesth(nk). ;
      real *8, allocatable :: grid_sin_polar_a_tmp_(:) !temporary: values for dsin(polar_a) associated with n_polar_a_(nk) in k-space polar coordinates for a particular grid_k_p_r_(nk). sometimes called sthetas(nk). ;
      real *8, allocatable :: weight_cos_polar_a_tmp_(:) !temporary: weight associated with grid_cos_polar_a_(nk) in k-space polar coordinates for a particular grid_k_p_r_(nk). sometimes called wtsth_(np). ;
      integer, allocatable :: n_azimu_b_tmp_(:) !temporary: number of nodes in azimu_b for each polar_a in k-space polar coordinates for a particular grid_k_p_r_(nk). also called ngridps(np). ;
      real *8, allocatable :: azimu_b_step_tmp_(:) !temporary: grid-spacing for azimu_b. ;
      integer n_azimu_b__sum_tmp !temporary: total number of points on sphere at a particular radius grid_k_p_r_(nk) in k-space polar coordinates. sometimes called nspherebig_tmp. ;
      ...
      n_w_sum = sum_i4_f(n_k_p_r_cur,n_w_)

      if (verbose.gt.0) write(6,'(A)')
     $     '[entering test_alpha_update_8]'
      if (verbose.gt.1) then
         ...
         write(6,'(A,I0)') ' n_k_p_r_cur ' , n_k_p_r_cur !integer *4: current index for current maximum value of n_k. Note that this runs from 1 to n_k_p_r_max. ;
         call print_sub_i4(n_k_p_r_cur,n_polar_a_,' n_polar_a_: ') !integer *4 array (length at least n_k_p_r_cur): number of polar_a values on sphere of radius grid_k_p_r_(nk). also called nlats(nk). ;
         call print_sub_i4(n_k_p_r_cur,grid_k_p_r_,' grid_k_p_r_: ') !real *8 array (length at least n_k_p_r_cur): values for k on successive shells for k-space polar coordinates. sometimes called grid_p_ or xnodesr. ;
...
c$$$  svd_calculation_type == 2 --> account for displacements via brute-force. ;
         ...
         call print_sub_i4(n_k_p_r_cur,n_w_csum_,' n_w_csum_: ') ! cumulative sum of n_w_. also called icstart(nk). ;
         write(6,'(A,I0)') ' quadrature_type_azimu_b ' ,
     $        quadrature_type_azimu_b ! quadrature type in the azimuthal direction. sometimes named itypep. ;
         call print_sub_i4(n_k_p_r_cur,n_w_,' n_w_: ') ! ngridc(nk) is the number of points on the ring of radius grid_k_p_r_(nk) for a template or image in k-space polar coordinates. also called ngridc(nk). ;
         write(6,'(A,I0)') ' n_k_p_r_low ' , n_k_p_r_low ! lowest value of nk (k-index in k-space polar coordinates) to use when building models. Note that this runs from 1 to n_k_p_r_max. ;
         write(6,'(A,I0)') ' n_k_p_r_cur ' , n_k_p_r_cur ! current value of nk (k-index in k-space polar coordinates) to use when reconstructing mole
         ...
         call print_sub_i4(n_k_p_r_cur,n_Y_lm_csum_,' n_Y_lm_csum_: ') ! cumulative sum of n_Y_lm_. also called isph_start(nk). ;
         call print_sub_i4(n_k_p_r_cur,n_Y_l_,' n_Y_l_: ') ! order of spherical harmonic expansion on sphere at radius grid_k_p_r_(nk). also called nterms_sph(nk). ;
     ...
     $        lsq_interpolation_order ! least-squares-solver interpolation order. sometimes named kord. ;
         ...
         call print_sub_i4(n_k_p_r_cur,n_azimu_b__sum_,
     $        ' n_azimu_b__sum_: ') ! total number of points on sphere at radius grid_k_p_r_(nk). also called numonsphere(nk). ;
      ...
      n_k_p_r_upperbound = max(4*n_k_p_r_max,2*nint(pi*k_p_r_max)) ! upper bound on the number of k-values (in k-space polar coordintes). sometimes named ntmax. ;      
      n_azimu_b__sum_cur = n_azimu_b__sum_(n_k_p_r_cur-1)
      allocate(C_M_(0:1+n_M_sample-1))
      call cs1_c16(n_M_sample,C_M_)
      allocate(S_alpha_S_index_all_(0:1+n_azimu_b__sum_cur-1))
      call cs1_r8(n_azimu_b__sum_cur,S_alpha_S_index_all_)
      allocate(S_alpha_polar_a_all_(0:1+n_azimu_b__sum_cur-1))
      call cs1_r8(n_azimu_b__sum_cur,S_alpha_polar_a_all_)
      allocate(S_alpha_azimu_b_all_(0:1+n_azimu_b__sum_cur-1))
      call cs1_r8(n_azimu_b__sum_cur,S_alpha_azimu_b_all_)
      allocate(grid_cos_polar_a_tmp_(0:1+n_k_p_r_upperbound-1))
      call cs1_r8(n_k_p_r_upperbound,grid_cos_polar_a_tmp_)
      allocate(grid_sin_polar_a_tmp_(0:1+n_k_p_r_upperbound-1))
      call cs1_r8(n_k_p_r_upperbound,grid_sin_polar_a_tmp_)
      allocate(weight_cos_polar_a_tmp_(0:1+n_k_p_r_upperbound-1))
      call cs1_r8(n_k_p_r_upperbound,weight_cos_polar_a_tmp_)
      allocate(azimu_b_step_tmp_(0:1+n_k_p_r_upperbound-1))
      call cs1_r8(n_k_p_r_upperbound,azimu_b_step_tmp_)
      allocate(n_azimu_b_tmp_(0:1+n_k_p_r_upperbound-1))
      call cs1_i4(n_k_p_r_upperbound,n_azimu_b_tmp_)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $     ,Y_est_ !complex *16 array (size at least n_Y_lm_sum): spherical harmonic coefficients obtained using estimated-angles to reconstruct molecule. sometimes called modsph_est. ;
     $     )
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $     'generating n_azimu_b__sum_cur = '
     $     ,n_azimu_b__sum_cur ,' templates'
      timing_tic = omp_get_wtime()
      call cl1_c16(ld_S*n_azimu_b__sum_cur,S_k_p__)
      call template_gen(
     $     Y_est_ !complex *16 array (size at least n_Y_lm_sum): spherical harmonic coefficients obtained using estimated-angles to reconstruct molecule. sometimes called modsph_est. ;
     $     ,n_Y_lm_sum !integer *4: total number of basis functions used in spherical harmonic expansion (in k-space polar coordinates). sometimes called nsphstore. ;
     $     ,n_Y_lm_csum_ !integer *4 array (size at least n_k_p_r_cur): cumulative sum of n_Y_lm_. also called isph_start(nk). ;
     $     ,n_Y_l_ !integer *4 array (size at least n_k_p_r_cur): order of spherical harmonic expansion on sphere at radius grid_k_p_r_(nk). also called nterms_sph(nk). ;
     $     ,n_k_p_r_cur !integer *4: current index for current maximum value of n_k. Note that this runs from 1 to n_k_p_r_max. ;
     $     ,n_w_ !integer *4 array (size at least n_k_p_r_cur): n_w_(nk) is the number of points on the ring of radius grid_k_p_r_(nk) for a template or image in k-space polar coordinates. also called ngridc(nk). ;
     $     ,ld_S !integer *4: leading dimension of S_k_p__. Must be at least n_A. ;
     $     ,n_w_csum_ !integer *4 array (size at least n_k_p_r_cur): cumulative sum of n_w_. also called icstart(nk). ;
     $     ,n_polar_a_ !integer *4: number of polar_a values on sphere of radius grid_k_p_r_(nk). ;
     ...
     $     ,n_k_p_r_cur !integer *4: current index for current maximum value of n_k. Note that this runs from 1 to n_k_p_r_max. ;
     $     ,n_azimu_b__sum_cur !temporary integer *4: n_azimu_b__sum_(n_k_p_r_cur-1). sometimes called numonsphere_cur. ;
     $     ,S_k_p__ !complex *16 array (length at least ld_S*max_i4_f_(I_S_sample_)): stack of templates associated with reconstructed molecule. sometimes called Y_slice__ or cslices or templates. ;
     $     )
      timing_toc = omp_get_wtime()
     ...
     $        ,timing_toc-timing_tic
      ...
      call getspheregrid(
     $     n_polar_a_(n_k_p_r_cur-1) !integer *4: number of polar_a values on sphere of radius grid_k_p_r_(nk). ;
     $     ,quadrature_type_azimu_b !integer: quadrature type in the azimuthal direction. sometimes named itypep. 0=same number of points for all n_polar_a_ (oversampling poles). 1=adaptive (samples with roughly uniform density). ;
     $     ,grid_cos_polar_a_tmp_ !temporary: real *8 array (size at least n_k_p_r_max): values for dcos(polar_a) associated with n_polar_a_(nk) in k-space polar coordinates for a particular grid_k_p_r_(nk). sometimes called xnodesth(nk). ;
     $     ,grid_sin_polar_a_tmp_ !temporary: real *8 array (size at least n_k_p_r_max): values for dsin(polar_a) associated with n_polar_a_(nk) in k-space polar coordinates for a particular grid_k_p_r_(nk). sometimes called sthetas(nk). ;
     $     ,weight_cos_polar_a_tmp_ !temporary real *8 array (size at least n_k_p_r_max): weight associated with grid_cos_polar_a_(nk) in k-space polar coordinates for a particular grid_k_p_r_(nk). sometimes called wtsth_(np). ;
     $     ,n_azimu_b_tmp_ !temporary integer *4 array (size at least n_k_p_r_max): number of nodes in azimu_b for each polar_a in k-space polar coordinates for a particular grid_k_p_r_(nk). also called ngridps(np). ;
     $     ,azimu_b_step_tmp_ !temporary real *8 array (size at least n_k_p_r_max): grid-spacing for azimu_b. ;
     ...
     $     )
      if (n_azimu_b__sum_tmp.ne.n_azimu_b__sum_cur)
     $     then
         write(6,'(A,I0,A,I0)') 'Warning, n_azimu_b__sum '
     $        ,n_azimu_b__sum_tmp,'.ne.'
     $        ,n_azimu_b__sum_(n_k_p_r_cur-1)
      end if
      nA = 0
      do npolar_a = 0,n_polar_a_(n_k_p_r_cur-1)-1
      ...
      n_azimu_b__sum_tmp = nA
      if (verbose.gt.1) write(6,*) '  n_azimu_b__sum is '
     $     ,n_azimu_b__sum_tmp
      if (n_azimu_b__sum_tmp.ne.n_azimu_b__sum_cur)
     $     then
         write(6,'(A,I0,A,I0)') 'Warning, n_azimu_b__sum '
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      n_S_sample = min(n_azimu_b__sum_cur,n_S_sample_max)
...
c$$$  calling checkset. ;
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      include 'test_alpha_update_8_excerpt_checkset.f'

c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
c$$$  Extracting sample templates
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      do nS_sample=0,n_S_sample-1
         if (n_S_sample.lt.n_azimu_b__sum_cur) then
            I_S_sample_(nS_sample) = min(n_azimu_b__sum_cur-1
     $           ,floor(n_azimu_b__sum_cur*1.0d0*nS_sample
     $           /(n_S_sample -1)))
         else
     ...
     $        S_alpha_azimu_b_all_(ns)
      ...
      call get_delta_1(n_pixels_in,n_k_p_r_cur,half_diameter_x_c
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        n_k_p_r_cur
         call Fig_gen_ver5(n_k_p_r_cur,n_polar_a_,grid_k_p_r_,n_S_sample
