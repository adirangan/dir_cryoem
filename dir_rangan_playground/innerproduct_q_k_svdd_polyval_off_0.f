...
!> Assumes that polynomial evaluation is precomputed. ;\n
      subroutine innerproduct_q_k_svdd_polyval_off_0(flag_S_vs_M
     $     ,svd_d_max,n_svd_d,svd_d_,n_svd_l,svd_l_,svd_U_d_
     $     ,svd_polyval_U_d_,n_delta_v,delta_x_,delta_y_ ,C_q_)
...
c$$$      real *8, allocatable :: U_d_(:)
     ...
     $        ' % [entering innerproduct_q_k_svdd_polyval_off_0] '
...
c$$$      allocate(U_d_(0:n_svd_l-1))
     ...
     $           ' in innerproduct_q_k_svdd_polyval_off_0'
         end if
...
c$$$            theta = nl_pre*(pi/2 - omega).
     ...
     $              ,'>l_max in innerproduct_q_k_svdd_polyval_off_0'
...
c$$$      deallocate(U_d_)
      ...
      if (verbose.gt.0) then
         write (6,'(A)')
     $        ' % [finished innerproduct_q_k_svdd_polyval_off_0]'
      end if
      end
