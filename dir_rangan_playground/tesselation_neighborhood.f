...
!> Finds neighborhood of a given tesselation element. ;\n
     ...
     $ LT_neighborhood_)
      ...
      integer *4 verbose
      data verbose / 0 /
      integer *4 nl_max,nl_0in,n_L,index_0input
     ...
     $        '[entering tesselation_neighborhood]: ',nl_0in ,
     $        T_nl_(index_0input)
     ...
     $        *index_0input) , T_vm_(1 + 3*index_0input) ,T_vm_(2 + 3
     $        *index_0input)
      end if !if (verbose.gt.0) then
      call distance_r8(3,T_vm_(3*index_0input),vp,distance_to_vm)
      distance_lowerbound = distance_to_vm - T_tr_(index_0input)
     ...
     $           ' <= distance_req ', distance_req
         ...
         if (T_lf_(index_0input)) then
            if (verbose.gt.0) then
               write(6,'(A,A)') trim(prefix) , 'leaf, checking self'
            end if ! if (verbose.gt.0) then
            allocate(vs(0:3-1))
            n_L_sub = T_ll_(index_0input)
            ls_start = T_ls_(index_0input)
     ...
     $                    'adding LT_tmp: ' , LT_tmp
         ...
         end if !if (T_lf_(index_0input)) then
         if (.not. T_lf_(index_0input)) then
            if (verbose.gt.0) then
               write(6,'(A,A)') trim(prefix) ,
     $              'branch, checking children'
             end if ! if (verbose.gt.0) then
            if (T_c0_(index_0input).ge.0) then
     ...
     $              n_LT_neighborhood ,
     $              LT_neighborhood_)
            end if !if (T_c0_(index_0input).ge.0) then
            if (T_c1_(index_0input).ge.0) then
               call tesselation_neighborhood(
     $              nl_max , nl_0in+1 ,
     $              vp ,
     $              distance_req ,
     ...
     $              n_LT_neighborhood ,
     $              LT_neighborhood_)
            end if !if (T_c1_(index_0input).ge.0) then
            if (T_c2_(index_0input).ge.0) then
               call tesselation_neighborhood(
     $              nl_max , nl_0in+1 ,
     $              vp ,
     $              distance_req ,
     ...
     $              n_LT_neighborhood ,
     $              LT_neighborhood_)
            end if !if (T_c2_(index_0input).ge.0) then
            if (T_c3_(index_0input).ge.0) then
               call tesselation_neighborhood(
     $              nl_max , nl_0in+1 ,
     $              vp ,
     $              distance_req ,
     ...
     $              n_LT_neighborhood ,
     $              LT_neighborhood_)
            end if !if (T_c3_(index_0input).ge.0) then
         end if !if (.not. T_lf_(index_0input)) then
