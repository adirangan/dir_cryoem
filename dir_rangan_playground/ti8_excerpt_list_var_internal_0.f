...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%      
     ...
     $ ,svd_polyval_V_r_ ! array storing polynomial evaluations of svd_V_r_ at the various grid_k_p_r_(nr) values. ;
     $ ,flag_warning
...
c$$$      pointer (p_CTF_p__,CTF_p__) !another name for CTF_k_p__. ;
     ...
     $ ,ZZ_pos_ !temporary storage for a single array. ;
     $ ,ZZ_pos_omp__ !temporary storage for a few arrays across multiple threads. ;
...
c$$$     $ ,nr,na,ns,nctf,nm,nw
     $ ,nr,na,ns,nctf,nm,nw
     $ ,n_r !integer: copy of n_k_p_r_cur
     $ ,n_w_ !array (length at least n_k_p_r_cur) to store number of angles (omega) for each nk. ;
     $ ,n_w_csum_ !array (length at least n_k_p_r_cur) to store cumulative sum of n_w_. ;
     $ ,n_w_max !integer: largest value in n_w_. ;
     $ ,n_w_sum !integer storing the sum of n_w_. ;
c$$$      temporary: image-parameters for each image
     ...
     $ ,alpha_0in_
     $ ,alpha_0in_omp__
c$$$      temporary: image-parameters for MS storage
     $ ,n_MS_omp__ !integer *4 array (length at least n_S*n_omp_sub_0in): actual number of images whose innerproduct-information is stored for a particular template. ;
     $ ,alpha_MS_omp__ !real *8 array (length at least n_alpha*n_MS_max*n_S*n_omp_sub_0in): actual innerproduct-information stored for various image-template pairs. ;
c$$$      fftw for omp
     $ ,fftw_plan_frwd__
     $ ,fftw_plan_back__
     $ ,fftw_0in__
...
c$$$      parameters for blocking S and M
     ...
     $ ,nm0,nm1,nm2,nm3,nm4
     $ ,ns0,ns1,ns2,ns3,ns4
     $ ,n_r_tmp,n_w_sum_tmp,n_X_tmp,n_Y_tmp,n_F_tmp,n_S_tmp
...
c$$$      Note that timing summation is not thread-safe
     ...
     $ ,timing_total_merge_alpha_MS,gnump_total_merge_alpha_MS
     $ ,timing_string
