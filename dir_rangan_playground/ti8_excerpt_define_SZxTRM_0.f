...
c$OMP DO 
     ...
     $        *dcmplx(1.0d0,0.0d0),Z_S_q__(nx1),n_r,O_T_R_CTF_M_q__(nx2)
     $        ,n_r ,0.0d0*dcmplx(1.0d0,0.0d0),S_Z_T_R_CTF_M_q__(nx3)
     $        ,n_transf*nS_1_per)
...
c$$$      %%%%%%%%%%%%%%%%
     ...
     $     ,S_T_T_R_CTF_M_q__)
      timing_tic = omp_get_wtime()
      if (n_omp_sub_0in.gt.1) then
c$OMP PARALLEL PRIVATE(n_Y_tmp,n_X_tmp)
c$OMP DO 
      do nw=0,n_w_max-1
         n_Y_tmp = n_delta_v*nS_1_per*nM_1_per*nw
         n_X_tmp = n_transf*nS_1_per*nM_1_per*nw
         call zgemm('T','N',n_delta_v,nS_1_per*nM_1_per*1
     $        ,n_svd_l ,0.5d0*dcmplx(1.0d0,0.0d0),Z_S_svdd_,n_svd_l
     $        ,S_Z_T_R_CTF_M_q__(n_X_tmp) ,n_svd_l,0.0d0*dcmplx(1.0d0
     $        ,0.0d0) ,S_T_T_R_CTF_M_q__(n_Y_tmp),n_delta_v)
      enddo ! do nw=0,n_w_max-1
c$OMP END DO
c$OMP END PARALLEL
      else !if (n_omp_sub_0in.gt.1) then
