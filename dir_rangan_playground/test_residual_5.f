...
!>    alpha2d__(3,*) real *8: 2d array of image parameters (see excerpt_define_nalpha.f). ; ;\n
...
!>    n_k_p_r_low    integer: index of lowest frequency sphere under consideration. Note that this runs from 1 to n_k_p_max. ; ;\n
!>    n_k_p_r_cur    integer: index of highest frequency sphere under consideration. Note that this runs from 1 to n_k_p_max. ; ;\n
...
c     alpha2d__(3,*) real *8: 2d array of image parameters (see excerpt_define_nalpha.f). ;
...
c     n_k_p_r_low    integer: index of lowest frequency sphere under consideration. Note that this runs from 1 to n_k_p_max. ;
c     n_k_p_r_cur    integer: index of highest frequency sphere under consideration. Note that this runs from 1 to n_k_p_max. ;
...
C***********************************************************************
      ...
      integer n_w_csum_(0:n_k_p_r_cur-1)
      integer n_polar_a_(0:n_k_p_r_cur-1)
      integer n_w_(0:n_k_p_r_cur-1)
      integer quadrature_type_azimu_b
      real *8 grid_k_p_r_(0:n_k_p_r_cur-1)
      integer n_k_p_r_low,n_k_p_r_cur
      integer n_Y_lm_csum_cur
      integer n_Y_lm_csum_(0:n_k_p_r_cur-1)
      integer n_Y_l_(0:n_k_p_r_cur-1)
...
c
      n_polar_a_oversample_max = nint(lsq_oversample
     $     *n_polar_a_(n_k_p_r_cur-1))
      if (verbose.gt.1) then
         write(6,*) ' lsq_oversample = ',lsq_oversample
...
c
      ...
      allocate(n_w_M_(0:1+n_k_p_r_cur-1))
      n = n_k_p_r_cur
...
c$$$      call cs1_c16(n,H_)
     ...
     $           ' ld_M: ' , ld_M
         ...
         na = 0
         do nk = n_k_p_r_low-1,n_k_p_r_cur-1
     ...
     $           ,fftw_out_(na)
     $           ,alpha2d__(0,nM)
     $           ,M_transform_(n_w_csum_(nk)-1)
     $           ,polar_a__(n_w_csum_(nk)-1 + ld_M*nM)
     $           ,azimu_b__(n_w_csum_(nk)-1 + ld_M*nM)
     ...
     $              ,n_w_csum_(nk)
            ...
            call test_residual_multa(polar_a__(n_w_csum_(nk)-1 + ld_M
     ...
     $           *n_polar_a_(nk))
            call test_residual_multah(
     $            polar_a__(n_w_csum_(nk)-1 + ld_M*nM)
     $           ,azimu_b__(n_w_csum_(nk)-1 + ld_M*nM)
     $           ,n_w_M_(nk)
     $           ,M_residual__(n_w_csum_(nk)-1+nM*ld_M)
     $           ,n_Y_l
     $           ,n_polar_a_oversample
     $           ,lsq_interpolation_order
     $           ,weight_CTF_k_c__(n_w_csum_(nk)-1 + ld_M*nM)
