...
!>    various images. ;\n
!> D. Then we call test_innerproduct_8 to calculate ;\n
...
!> D. The routine test_innerproduct_8 is used ;\n
...
!> ;\n
!> integer n_k_cur       Number of rings to pass into  ;\n
...
!>                       cartesian-grid).  ;\n
!>                     - Values of n_k_cur~10 --> low resolution.  ;\n
!>                     - Values of n_k_cur~50 --> moderate resolution. ;\n
!>                     - Values of n_k_cur~100 --> high resolution. ;\n
!>                     - We have not generated libraries for values ;\n
!>                       of n_k_cur.gt.192 yet. ;\n
!> ;\n
...
!>                       n_gamma_z to be on the same order as n_k_cur ;\n
!>                       (e.g., n_gamma_z = n_k_cur/2 or so). ;\n
!> ;\n
...
c$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
      program test_innerproduct_8_dr
c$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
c$$$      This program is a driver which tests the subroutine
c$$$      test_innerproduct_8.f ,
...
c$$$         various images.
c$$$      D. Then we call test_innerproduct_8 to calculate
...
c$$$      D. The routine test_innerproduct_8 is used
...
c$$$         ./dir_jpg/Fig_NMC_[n_k_cur].jpg:
...
c$$$         ./dir_jpg/Fig_MTS_[n_k_cur].jpg:
...
c$$$
c$$$      integer n_k_cur       Number of rings to pass into 
...
c$$$                            cartesian-grid). 
c$$$                          - Values of n_k_cur~10 --> low resolution. 
c$$$                          - Values of n_k_cur~50 --> moderate resolution.
c$$$                          - Values of n_k_cur~100 --> high resolution.
c$$$                          - We have not generated libraries for values
c$$$                            of n_k_cur.gt.192 yet.
c$$$
...
c$$$                            n_gamma_z to be on the same order as n_k_cur
c$$$                            (e.g., n_gamma_z = n_k_cur/2 or so).
c$$$
...
c$$$      
      ...
      logical flag_memory_checkset !temporary: logical flag used in test_innerproduct_8_dr_checkset.f ;
c$$$      
      rseed = 1
      pi = 4*datan(1.0d0)
c$$$      
...
c$$$      tesselation_distance_req = 2.0d0
      ...
      n_gamma_z = n_k_cur
      n_polar_a = max(6,nint(pi*n_k_cur))
      n_w_max = n_polar_a*2
      n_gamma_z = n_w_max
      fpm_howmany_max = max(1,fpm_howmany_max)
      if (verbose.gt.0) then
          write(6,'(A)')
     $        '[entering test_innerproduct_8_dr]: '
       end if
       if (verbose.gt.0) then
...
c$$$         n_polar_a calculation similar to testrebuild.f:
     ...
     $        ,' determine n_w_ and n_A'
      ...
      call get_template_size(n_polar_a_,n_r,n_A,n_w_
     $     ,n_w_csum_)
      n_w_max = n_w_(nr-1)
      if (verbose.gt.0) then
...
c$$$      indices
      ...
      ld_M = n_A
      ld_CTF = n_A
      allocate(T0_k_p_(0:1+n_A-1))
      call cs1_c16(n_A,T0_k_p_)
      allocate(T1_k_p_(0:1+n_A-1))
      call cs1_c16(n_A,T1_k_p_)
      allocate(T2_k_p_(0:1+n_A-1))
      call cs1_c16(n_A,T2_k_p_)
      allocate(T3_k_p_(0:1+n_A-1))
      call cs1_c16(n_A,T3_k_p_)
...
c$$$      generating grids for templates and images 
      ...
      if (verbose.gt.0) then
         ...
         call print_sub_r8(1*n_A,weight_k_p_,' weight_k_p_: ')
      end if !if (verbose.gt.0) then
...
c$$$      generate templates
     ...
     $        ,' ncur and get_template_size.'
      ...
      allocate(S_x_p__(0:1+n_A*n_S_max-1))
      call cs1_c16(n_A*n_S_max,S_x_p__)
      allocate(S_k_c__(0:1+2*n_r*2*n_r*n_S_max-1))
      call cs1_c16(2*n_r*2*n_r*n_S_max,S_k_c__)
      allocate(S_k_p__(0:1+n_A*n_S_max-1))
      call cs1_c16(n_A*n_S_max,S_k_p__)
      allocate(S_k_q__(0:1+n_A*n_S_max-1))
      call cs1_c16(n_A*n_S_max,S_k_q__)
      do ns=0,n_S-1
...
c$$$      Calculating k-space template on quasi-uniform polar-grid
     ...
     $        ,S_k_c__(nS_sample*2*n_r*2*n_r),n_r,grid_k_p_r_,n_w_,n_A
...
c$$$end;%if disp_flag;
     ...
     $        ,' 2.0d0*pi/3*(j%3).'
      ...
      allocate(N_k_p__(0:1+n_A*n_M_max-1))
      call cs1_c16(n_A*n_M_max,N_k_p__)
      allocate(N_k_q__(0:1+n_A*n_M_max-1))
      call cs1_c16(n_A*n_M_max,N_k_q__)
...
c$$$      Calculating k-space image on quasi-uniform polar-grid
         call interp_c_to_p(2*n_r,max_k_c,2*n_r,max_k_c
     $        ,N_k_c__(nM_sample*2*n_r*2*n_r),n_r,grid_k_p_r_,n_w_,n_A
...
c$$$         the N_k_p__ in k-space.
         ...
         call transf_p_to_p(n_r,grid_k_p_r_,n_w_,n_A,N_k_p__(nM_sample
...
c$$$      enddo
     ...
     $        ' These are deliberately chosen to be anisotropic.'
      ...
      allocate(CTF_k_p__(0:1+n_ctf*n_A-1))
      call cs1_c16(n_ctf*n_A,CTF_k_p__)
...
c$$$      of anisotropy to test our code.
      do nctf=0,n_ctf-1
         ...
         ctf_p_2 = 2
         if (flag_ctf) then
         call get_ctf_star_k_p_(n_r,grid_k_p_r_,n_w_,n_A,CTF_k_p__(nctf
     $        *n_A),ctf_p_0 ,ctf_p_1,ctf_p_2)
         else
         call get_ctf_ones_k_p_(n_r,grid_k_p_r_,n_w_,n_A,CTF_k_p__(nctf
     $        *n_A))
...
c$$$         /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      ...
      if (flag_memory_estimate.eqv..false.) then
      allocate(M_k_p__(0:1+n_A*n_M_max-1))
      call cs1_c16(n_A*n_M_max,M_k_p__)
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      do nm=0,n_M-1
         ...
         call af1_c16(n_A,1.0d0*dcmplx(l2_norm,0.0d0),1.0d0
     $        *dcmplx(0.0d0,0.0d0),M_k_p__(nM_sample*n_A)
     $        ,M_k_p__(nM_sample*n_A))
      enddo                     ! do nm=0,n_M-1
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      ...
      do ns=0,n_S-1
         ...
         do nm=0,n_M-1
            nM_sample = I_M_sample_(nm)
            nctf = I_ctf_(nm)
            call test_innerproduct_bruteforce_1(verbose ,n_r
     $           ,grid_k_p_r_,n_w_ ,half_diameter_x_c,S_k_p__(n_A
     $           *nS_sample),M_k_p__(n_A *nM_sample) ,CTF_k_p__(n_A
     $           *nctf) ,delta_x_est_(nm) ,delta_y_est_(nm)
     ...
     $           ,n_gamma_z,gamma_z_)
      ...
      verbose = 1
c$$$      call print_sub_c16(n_A,S_k_p__,' S_k_p__: ')
c$$$      call print_sub_c16(n_A,M_k_p__,' M_k_p__: ')
c$$$      call print_all_r8(n_alpha,alpha_est__,'alpha_est__: ')
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $     ,n_polar_a_ !integer *4 array (length at least n_k_cur): number of polar_a values on sphere of radius grid_k_p_r_(nk). also called nlats(nk). ;
     $     ,grid_k_p_r_ !real *8 array (length at least n_k_cur): values for k on successive shells for k-space polar coordinates. sometimes called grid_p_ or xnodesr. ;
     $     ,weight_k_p_r_ !real *8 array (length at least n_k_cur): weights for radial quadrature. ;
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
     ...
     $        ,n_A,S_k_p__,n_M,I_M_sample_,n_A,M_k_p__ ,n_A,CTF_k_p__
     ...
     $    ' S_index_err_:', nint(max_r8_f(n_M,S_index_err_))
      ...
      include 'test_innerproduct_8_dr_checkset.f'

 10    continue

      if (verbose.gt.1) then
         write(6,'(A)') '[finished test_innerproduct_8_dr]'
      end if
