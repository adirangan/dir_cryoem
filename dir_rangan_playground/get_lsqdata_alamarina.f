...
!> set up right-hand-side for least-squares problem ;\n
      subroutine get_lsqdata_alamarina(
     $     n_M
     $     ,I_M_sample_
     $     ,ld_M
     $     ,M_k_p__
     ...
     $     ,n_w_M
      ...
      real *8, allocatable :: kcosw_(:)
      real *8, allocatable :: ksinw_(:)
...
c$$$         %%%%%%%%
      if (verbose.gt.0) then 
         write(6,'(A)') '[entering get_lsqdata_alamarina]'
      ...
      allocate(kcosw_(0:1+n_w-1))
      call cs1_r8(n_w,kcosw_)
      allocate(ksinw_(0:1+n_w-1))
      call cs1_r8(n_w,ksinw_)
...
c$$$         %%%%%%%%
      do nw=0,n_w-1
         kcosw_(nw) = k_p_r*dcos((2.0d0*pi*nw)/(1.0d0*n_w))
         ksinw_(nw) = k_p_r*dsin((2.0d0*pi*nw)/(1.0d0*n_w))
      enddo !do nw=0,n_w-1
c$$$         %%%%%%%%
...
c$$$         %%%%%%%%
     ...
     $        ,' gamma_z: '
     $        ,gamma_z
         call print_sub_r8(n_w,k_c_0_,' k_c_0_: ')
         call print_sub_r8(n_w,k_c_1_,' k_c_1_: ')
         call print_sub_r8(n_w,k_c_2_,' k_c_2_: ')
...
c$$$         %%%%%%%%
     ...
     $           ,cos_polar_a)
            kd = kcosw_(nw)*delta_x + ksinw_(nw)*delta_y
            M_k_c_(n_w_M) = M_k_p_single_(nw) * 
     $           dcmplx( +dcos(kd) , +dsin(kd) )
...
c$$$         %%%%%%%%
      ...
      call cxs_r8(n_w,kcosw_,'kcosw_',flag_memory_checkset)
      call cxs_r8(n_w,ksinw_,'ksinw_',flag_memory_checkset)
...
c$$$         %%%%%%%%
      ...
      if (verbose.gt.0) then 
         write(6,'(A)') '[finished get_lsqdata_alamarina]'
      end if !if (verbose.gt.0) then 
      end !subroutine
