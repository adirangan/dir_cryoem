...
!>        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;\n
!>        try: ;\n
!>        make -f test_ver17.make ; ./test_ver17.out 20 0.4 0.3 0.2 0.1 100.0d0 1 ; 
!>        make -f test_ver17.make ; ./test_ver17.out 20 0.4 0.3 0.2 0.1 10.0d0 1 ; 
!>        make -f test_ver17.make ; ./test_ver17.out 20 0.4 0.3 0.2 0.1 1.0d0 1 ; 
!>        make -f test_ver17.make ; ./test_ver17.out 20 0.4 0.3 0.2 0.1 0.1d0 1 ; 
!>        make -f test_ver17.make ; ./test_ver17.out 20 0.4 0.3 0.2 0.1 0.01d0 1 ; 
...
c$$$      svd_calculation_type == 2 --> brute force displacements ;
      ...
      integer ctf_type ! flag determining which kind of ctf to use (e.g., an anisotropic ctf). 0=unity, 1=adi_ctf, 2=niko_ctf (from marina). ; 
...
c$$$      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
      ...
      integer n_k_p_upperbound ! upper bound on the number of k-values (in k-space polar coordinates). sometimes named ntmax. ;
...
c$$$      command line input variables
      ...
      k_p_max = 2.0d0*n_k_p_max
      n_k_p_upperbound = max(4*n_k_p_max,2*nint(pi*k_p_max))
...
c-----------------------------------------------------------------------
      ...
      allocate(ngridc(1+n_k_p_upperbound))
      call cs1_i4(n_k_p_upperbound,ngridc)
      allocate(n_w_(0:1+n_k_p_upperbound-1))
      call cs1_i4(n_k_p_upperbound,n_w_)
      allocate(ngridc_tmp(1+n_k_p_upperbound))
      call cs1_i4(n_k_p_upperbound,ngridc_tmp)
      ...
      allocate(ngridps(1+n_k_p_upperbound))
      call cs1_i4(n_k_p_upperbound,ngridps)
      allocate(n_azimu_b_(0:1+n_k_p_upperbound-1))
      call cs1_i4(n_k_p_upperbound,n_azimu_b_)
      ...
      allocate(xnodesth(1+n_k_p_upperbound))
      call cs1_r8(n_k_p_upperbound,xnodesth)
      allocate(grid_cos_polar_a_(0:1+n_k_p_upperbound-1))
      call cs1_r8(n_k_p_upperbound,grid_cos_polar_a_)
      allocate(sthetas(1+n_k_p_upperbound))
      call cs1_r8(n_k_p_upperbound,sthetas)
      allocate(grid_sin_polar_a_(0:1+n_k_p_upperbound-1))
      call cs1_r8(n_k_p_upperbound,grid_sin_polar_a_)
      allocate(weight_cos_polar_a_(0:1+n_k_p_upperbound-1))
      call cs1_r8(n_k_p_upperbound,weight_cos_polar_a_)
      allocate(azimu_b_step_(0:1+n_k_p_upperbound-1))
      call cs1_r8(n_k_p_upperbound,azimu_b_step_)
...
c-----------------------------------------------------------------------
     ...
     $     ,weight_k_p_)
      call print_sub_r8(n_k_p_max,grid_k_p_,' grid_k_p_: ');
      n_Y_lm_sum = 0
      n_image_size = 0
      ld_M = 0;
      do nk = 0,n_k_p_max-1
         ...
         if (n_polar_a_(nk).gt.n_k_p_upperbound) then
            write(6,'(A,I0,A,I0,A,I0)') ' Warning, n_k_p_upperbound ' ,
     $           n_k_p_upperbound ,
     $           ' lower than n_polar_a_(' , nk , '): ' , n_polar_a_(nk)
            stop !exit program due to error.
         end if !if (n_polar_a_(nk).gt.n_k_p_upperbound) then
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */         
     ...
     $           ,n_w_,n_k_1,n_k_cur,n_Y_lm_csum_cur ,n_Y_lm_csum_
     $           ,n_Y_l_ ,lsq_oversample,lsq_interpolation_order,Y_tru_
...
c$$$  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */         
     ...
     $           ,n_w_csum_,n_polar_a_,quadrature_type_azimu_b,grid_k_p_
     $           ,n_w_,n_k_1,n_k_cur,n_Y_lm_csum_cur ,n_Y_lm_csum_
     $           ,n_Y_l_ ,lsq_oversample,lsq_interpolation_order,Y_est_
     $           ,M_residual_sample__,n_residual_loading
     $           ,n_residual_iteration,M_residual_loading_)
            tmp_d_(0) = n_M_sample
            tmp_d_(1) = 1
