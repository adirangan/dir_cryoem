      subroutine innerproduct_q__k_only(n_r,grid_p_,n_w_,n_A,T_q_,M_q_
     $     ,C_q_)
...
c$$$      Stores C_q_ in fft ordering (mode 0 , mode 1 , ... , mode -1)
      ...
      if (verbose.gt.0) then
         write(6,'(A,I0)') ' % [entering innerproduct_q__k_only] n_r '
     $        ,n_r
      end if
      pi = 4.0*atan(1.0)
      n_w_max = n_w_(n_r-1)
...
c$$$         We assume that the fourier basis is orthonormal (not merely orthogonal)
     ...
     $                 ,nw_fix,'; (full orig)'
      ...
      enddo
      if (verbose.gt.0) then
         write(6,'(A)') ' % [finished innerproduct_q__k_only]'
      end if
      end
