      subroutine innerproduct_q_delta_gamma_svdread(type_flag
...
c$$$      variables to hold Jsvd library
      ...
      complex *16 C_Z,Z_q_(0:n_delta_x*n_delta_y*n_gamma_z-1),C_Z_use
      integer n_fftw
...
c$$$      parameters for timing
     ...
     $        ,' too large in innerproduct_q_delta_gamma_svdread'
      end if
     ...
     $        ,' too small in innerproduct_q_delta_gamma_svdread'
      end if
      if (verbose.gt.0) then
         write(6,*) 'selecting Jsvd library'
      end if
      if (.false.) then
      include './dir_gen_Jsvd_1/gen_Jsvd_svdpick.txt'
      end if
      if (verbose.gt.0) then
         write(6,*) 'allocating and loading chosen Jsvd library'
      end if
...
c$$$               Z_q_(nC5) = Z_q
     ...
     $        -timing_tic
      ...
      if (verbose.gt.0) then
         write(6,*) 'deallocating Jsvd library'
      end if
      include './dir_gen_Jsvd_1/gen_Jsvd_svdfree.txt'

      if (verbose.gt.0) then
         write(6,*) '[finished innerproduct_q_delta_gamma_svdread]'
      end if
      end
