function X_k_p_ = test_F_X_k_p_4(n_point,max_x_c,delta_d,delta_w,...
				 n_svd_r,svd_r_,svd_r_m,svd_r_c,svd_r_w_,svd_r_Lv_,n_svd_d,svd_d_,svd_d_m,svd_d_c,svd_d_w_,svd_d_Lv_,n_svd_l,svd_l_,svd_U_d_,svd_s_,svd_V_r_...
				 );
...
U_d = 0;
for nkB=0:b_K-1;
b_tmp = polyval(svd_d_Lv_(1+nkB,:),(delta_d - svd_d_m)/svd_d_c);
U_d = U_d + svd_U_d_(1+nkB,ns)*b_tmp;
end;% for nkB=0:b_K-1;
V_r_ = zeros(1,length(grid_k_r_));
for nkA=0:a_K-1;
a_tmp = polyval(svd_r_Lv_(1+nkA,:),(2*pi*grid_k_r_ - svd_r_m)/svd_r_c);
