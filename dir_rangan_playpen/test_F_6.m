% test 2d-fourier transform, generating qring representation in realspace and freqspace ;
...
grid_q = (0:ng-1) ; max_q = ng; qlim = [-2,+2];
...
figure;
% calculate function values on realspace qring meshgrid ;
F_x_q = fft(F_x_p);
% plot function in realspace ;
subplot(3,3,1);imagesc(recenter(real(F_x_c)),Flim_x);title('real(F_x_c)'); colorbar; set(gca,'Ydir','normal');
...
subplot(3,3,4);polarpatch(R_x_p,W_x_p,F_x_p,Flim_x);title('real(F_x_p)'); c = colorbar; set(c,'Ticks',0.025 + 0.95*linspace(0,1,4),'TickLabels',linspace(Flim_x(1),Flim_x(2),4));set(gca,'XTick',[],'Ytick',[]);
subplot(3,3,7);imagesc(abs(F_x_q([ng/2:ng,1:ng/2-1],:))); title('abs(F_x_q)'); colorbar; set(gca,'Ydir','normal'); set(gca,'XTick',[1,ng/2+1,ng],'XTickLabel',[0,grid_x_r(end/2+1),grid_x_r(end)]); set(gca,'YTick',[1,ng/2+1,ng],'YTickLabel',[grid_q(1),grid_q(end/2+1),grid_q(end)]-max_q/2);
...
% calculate function values on realspace qring meshgrid ;
F_k_q = fft(F_k_p);
% plot function in freqspace ;
subplot(3,3,2);imagesc(recenter(real(F_k_c)),Flim_k);title('real(F_k_c)'); colorbar; set(gca,'Ydir','normal');
...
subplot(3,3,3);imagesc(recenter(imag(F_k_c)),Flim_k);title('imag(F_k_c)'); colorbar; set(gca,'Ydir','normal');
set(gca,'XTick',[1,ng/2+1,ng],'XTickLabel',[grid_k_c(1),grid_k_c(end/2+1),grid_k_c(end)]-ng/2);set(gca,'YTick',[1,ng/2+1,ng],'YTickLabel',[grid_k_c(1),grid_k_c(end/2+1),grid_k_c(end)]-ng/2);
subplot(3,3,5);polarpatch(R_k_p,W_k_p,real(F_k_p),Flim_k);title('real(F_k_p)'); c = colorbar; set(c,'Ticks',0.025 + 0.95*linspace(0,1,4),'TickLabels',linspace(Flim_k(1),Flim_k(2),4));set(gca,'XTick',[],'Ytick',[]);
subplot(3,3,6);polarpatch(R_k_p,W_k_p,imag(F_k_p),Flim_k);title('imag(F_k_p)'); c = colorbar; set(c,'Ticks',0.025 + 0.95*linspace(0,1,4),'TickLabels',linspace(Flim_k(1),Flim_k(2),4));set(gca,'XTick',[],'Ytick',[]);
subplot(3,3,8);imagesc(real(F_k_q([ng/2:ng,1:ng/2-1],:)),qlim); title('real(F_k_q)'); colorbar; set(gca,'Ydir','normal'); set(gca,'XTick',[1,ng/2+1,ng],'XTickLabel',[0,grid_k_r(end/2+1),grid_k_r(end)]); set(gca,'YTick',[1,ng/2+1,ng],'YTickLabel',[grid_q(1),grid_q(end/2+1),grid_q(end)]-max_q/2);
subplot(3,3,9);imagesc(imag(F_k_q([ng/2:ng,1:ng/2-1],:)),qlim); title('imag(F_k_q)'); colorbar; set(gca,'Ydir','normal'); set(gca,'XTick',[1,ng/2+1,ng],'XTickLabel',[0,grid_k_r(end/2+1),grid_k_r(end)]); set(gca,'YTick',[1,ng/2+1,ng],'YTickLabel',[grid_q(1),grid_q(end/2+1),grid_q(end)]-max_q/2);
