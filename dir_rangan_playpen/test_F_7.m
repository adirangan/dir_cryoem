...
S_x_c = tmp_crop.*(tmp_a+tmp_b+tmp_c+tmp_d); Slim_x = max(0,mean(S_x_c(:)) + 1.0*std(S_x_c(:))*[-1,+1]);
...
% set disp_flag to 1 to show overlap integrals as they are being calculated ;
...
%subplot(3,1,1); plot(-C_q_(:)); subplot(3,1,2); plot(C_k_(:)); subplot(3,1,3); plot(C_x_(:));
