...
dt = 0.05; t_frame = 8.0; % draw every t_frame seconds. ;
n_iteration = 1024*4;
E_quad_ = zeros(n_iteration,1);
E_avg_ = zeros(n_iteration,1);
...
E_quad = abs(E_sample_)*quad_weight_sample_;
...
E_quad_(niteration) = E_quad; E_avg_(niteration) = E_quad / (pi*D_target.^2);
dEx_quad_ = dEx_sample_*quad_weight_sample_;
dEy_quad_ = dEy_sample_*quad_weight_sample_;
%%%%%%%%%%%%%%%%;
...
polarpatch_adaptive(r_,n_w_,E_sample_,colormap('hot'),[0,max(E_sample_)]);
plot(D_target*cos(linspace(0,2*pi,256)),D_target*sin(linspace(0,2*pi,256)),'k-');
...
polarpatch_adaptive(r_,n_w_,log10(E_sample_),colormap('hot'),log10(max(E_sample_))+[-3,0]);
plot(D_target*cos(linspace(0,2*pi,256)),D_target*sin(linspace(0,2*pi,256)),'k-');
plot(delta_node_(1,:),delta_node_(2,:),'p','MarkerSize',15,'MarkerFaceColor','g');
set(gca,'XTick',[],'YTick',[]);axis square;
...
plot(1:niteration,E_avg_(1:niteration),'k.-');ylim([0,max(E_avg_)]); xlim([1,n_iteration]); xlabel('iteration'); ylabel('Avg Error');
hold off;
subplot(2,2,4); cla; hold on;
plot(1:niteration,log10(abs(E_avg_(1:niteration))),'k.-');ylim([-9,0]); xlim([1,n_iteration]); xlabel('iteration'); ylabel('log10(Avg Error)');
hold off;
set(gcf,'Position',1+[0,0,1024*1.5,1024]);
drawnow(); t_mid = toc(t_start); t_draw = t_mid;
end;%if flag_disp;
%%%%%%%%%%%%%%%%;
delta_node_ = delta_node_ - dt*transpose([ dEx_quad_ , dEy_quad_ ]);
for nnode=1:n_node;
