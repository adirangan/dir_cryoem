...
colormap('hot');
subplot(1,1,1); imagesc(log10(svd_s__),[log(eps_target),1]); %colorbar;
...
set(gcf,'Position',1+[0,0,512*1,1024]);
fname_fig = sprintf('%s/Sigma_N%d_FigA',dir_fig,round(100*N_pixel));
...
l2_G = sum(abs(G_x_c_(:)).^2 .* d_x_c^2);
l2_F = sum(abs(F_x_c_(:)).^2 .* d_x_c^2);
%X_delta_ = real(ifft2(conj(F_k_c_).*F_k_c_))*n_x_c*n_x_c/sqrt(l2_F*l2_F);
X_delta_ = real(ifft2(conj(F_k_c_).*G_k_c_))*n_x_c*n_x_c/sqrt(l2_F*l2_G);
...
disp(sprintf(' %% Xlim_: %0.4f %0.4f',Xlim_));
%%%%%%%%;
flag_plot=1;
if flag_plot;
figure(n_figure); clf; n_figure = n_figure + 1;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
% Now calculate innerproduct for multiple translations and rotations. ;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
...
%%%%%%%%;
tic;
X_1__ = zeros(n_delta,n_k_w); %<-- svd-approximation to innerproduct for all rotations in grid_k_w_. ;
tmp_v__ = zeros(n_svd_l,n_k_w);
tmp_w__ = zeros(n_svd_l,n_k_w);
for ns=1:n_svd_l;
tmp_l = svd_l_(ns); nq1 = tmp_l;
for nq0=0:n_k_w-1;
nq3 = periodize(nq0-nq1,0,n_k_w);
tmp_w__(ns,1+nq0) = tmp_w__(ns,1+nq0) + svd_s_(ns) .* sum(conj(F_k_q_(1+nq3,:)).*G_k_q_(1+nq0,:).*V_k__(ns,:) .* k_p_quad_weight_(1,:));
end;%for nq0=0:n_k_w-1;
end;%for ns=1:n_svd_l;
tmp_v__ = ifft(tmp_w__,[],2)*n_k_w;
tmp_u__ = U_d__ .* exp(+i*(delta_w_ + pi/2)*svd_l_);
X_1__ = tmp_u__*tmp_v__;
disp(sprintf(' %% calculating X_1__: time %f',toc));
%%%%%%%%;

%%%%%%%%;
Xlim_ = [min(real(X_1__(:))),max(real(X_1__(:)))];
l2_G = sum(abs(G_k_p_(:)).^2 .* k_p_quad_weight_(:)) / sum(k_p_quad_weight_(:)) ;
l2_F = sum(abs(F_k_p_(:)).^2 .* k_p_quad_weight_(:)) / sum(k_p_quad_weight_(:)) ;
Xlim_C_ = real(Xlim_/sum(k_p_quad_weight_(:)) ./ sqrt(l2_G.*l2_F));
disp(sprintf(' %% Xlim_C_: %0.4f %0.4f',Xlim_C_));
Xlim2_ = mean(real(X_1__(:))) + std(real(X_1__(:))) * 1.5 * [-1,+1];
flag_plot=1;
if flag_plot;
figure(n_figure); clf; n_figure = n_figure + 1;
colormap(colormap_beach());
prows = 5; pcols = 9; n_p = prows*pcols;
k_w_sub_ = periodize( round(1 + linspace(-n_k_w/2,+n_k_w/2,n_p)) , 1 , n_k_w );
for np=1:n_p;
subplot(prows,pcols,np);
imagesc(reshape(real(X_1__(:,k_w_sub_(np))),n_delta_x,n_delta_y),Xlim2_); title(sprintf('%+d',periodize( round( 180/pi*grid_k_w_(k_w_sub_(np)) ) , -180 , +180 )));
set(gca,'XTick',[],'YTick',[]);
end;%for np=1:n_p;
...
n_c = size(c_,1);
nx = 0.325; disp(sprintf(' %% contour at level %0.4f',min(Xlim_C_) + nx*diff(Xlim_C_)));
tmp_X = min(Xlim_) + nx*diff(Xlim_);
hpatch = patch(isosurface(delta_x_,delta_y_,grid_k_w_,reshape(real(X_1__),n_delta_x,n_delta_y,n_k_w),tmp_X));
isonormals(delta_x_,delta_y_,grid_k_w_,reshape(real(X_1__),n_delta_x,n_delta_y,n_k_w),hpatch);
nc = max(1,min(n_c,floor(n_c*nx)));
%hpatch.FaceColor = c_(nc,:);
hpatch.FaceColor = [0.25,0.65,1.00];
hpatch.EdgeColor = 'none';
hpatch.FaceAlpha = 1.0;
xlim(1*[delta_x_(1),delta_x_(end)]);ylim(1*[delta_y_(1),delta_y_(end)]);zlim([0,2*pi]);
xlabel('\delta_x');ylabel('\delta_y');zlabel('\omega');
set(gca,'Xtick',[],'Ytick',[],'Ztick',[0,2*pi],'ZtickLabel',{'0','2\pi'});
view([-45,20]); 
axis vis3d; camlight right; lighting gouraud;
set(gcf,'Position',1+[0,0,512*1,512*1]);
fname_fig = sprintf('%s/blob_FIGE_0',dir_fig);
if flag_plot; xlabel('');ylabel('');zlabel(''); set(gca,'Ztick',[]); print('-djpeg',sprintf('%s.jpg',fname_fig)); end;
if flag_plot; xlabel('');ylabel('');zlabel(''); set(gca,'Ztick',[]); print('-depsc',sprintf('%s.eps',fname_fig)); end;
%disp('return');return;
end;%if flag_plot;
