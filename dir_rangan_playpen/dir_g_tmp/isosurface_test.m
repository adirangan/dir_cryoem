...
param_A_m = 8; param_B = 8;
%tmp_crop = 0.5*(1+erf(-16.0*(R_m - 0.25*(1 + 0.5*cos(param_A_m*A_m)) - 0.25*(1 + 0.5*cos(param_B*B)))));
tmp_crop = 0.5*(1+erf(-16.0*(R_m - 0.125*(1 + 0.5*cos(param_A_m*(A_m-1*pi/8))) - 0.125*(1 + 0.5*cos(param_B*B)))));

for nF=0:1;

if (nF==0);
F_x_c = tmp_gaus ; %F_x_c = tmp_crop ; %F_x_c = tmp_crop .* tmp_gaus ;
X = X_m; Y = Y_m; Z = Z_m;
end;% if;
if (nF==1);
F_x_c = MDA_read_c16('./S_3d_x_c_.mda'); F_x_c = permute(F_x_c,[2,1,3]);
X = MDA_read_r8('./x_3d_x_c_.mda'); X = permute(X,[2,1,3]);
Y = MDA_read_r8('./y_3d_x_c_.mda'); Y = permute(Y,[2,1,3]);
Z = MDA_read_r8('./z_3d_x_c_.mda'); Z = permute(Z,[2,1,3]);
end;% if;
