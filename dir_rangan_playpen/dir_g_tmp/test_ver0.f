...
ccc      complex *16 cslices(350,1232)
      ...
      real *8 ffhatnorm
      integer kk,ll,ii,jj,nslices,nnn,nspherebig,ntemplatesize
      real *8 err,denom,boxsize,ctheta,stheta,phstep
      real *8 phi
      external fgauss
      real *8 rmodelnorm2,rmodelnorm3
      real *8 oversamp
      integer kord,nlow,ncheck,nlowstart
...
c$$$            alphas_(2 + 3*n_A) = na*2*pi/n_a_3d_k_p_tmp_(nb)
c$$$            alphas_(2 + 3*n_A) = -rand()
            alphas_(2 + 3*n_A) = 0.0d0
            n_A = n_A+1
         enddo
      enddo
...
c$$$         write(6,*) '1,2,3: ',ctheta,stheta,phstep,ngridps(kk)
         do ll = 1,ngridps(kk)
            ...
            alphas(2,nnn) = phi
c$$$            alphas(3,nnn) = (ll-1)*2*pi/ngridps(kk)
c$$$            alphas(3,nnn) = -rand()
            alphas(3,nnn) = 0.0d0
         enddo
      enddo
