% Here we just test out svd-expansion in the case where n_k_w_ is not a constant (w.r.t. nr). ;
...
[max_k_c,max_k_p,grid_x_c_,d_x_c,X_x_c_,Y_x_c_,R_x_c_,W_x_c_,grid_k_c_,d_k_c,X_k_c_,Y_k_c_,R_k_c_,W_k_c_,grid_x_r_,grid_x_w_,R_x_p_,W_x_p_,grid_k_r_,d_k_r,grid_k_w_,d_k_w,R_k_p_,W_k_p_,X_k_p_,Y_k_p_] = test_F_grid_1(n_point,max_x_c);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
N_pixel = 2.0; % number of wavelengths allowed in displacement grid. ;
eps_target = 0.00001; % tolerance used for svd-expansion. ;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
n_delta_x = 1+4; n_delta_y = 1+4; n_delta = n_delta_x*n_delta_y; 
...
figure(n_figure); clf; n_figure = n_figure + 1;
hold on;
plot(gamma_,recenter1(C_p_),'ko-'); xlabel('gamma'); ylabel('X(gamma)');
ndelta = ceil(n_delta/2);
plot(gamma_,recenter1(real(X_1__(ndelta,:))),'rx-'); xlabel('gamma'); ylabel('X(gamma)');
hold off;
xlim([-pi,+pi]);
set(gca,'YTick',[]);
set(gcf,'Position',1+[0,0,512*1,512]);
