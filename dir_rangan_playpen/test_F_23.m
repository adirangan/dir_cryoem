...
% Try running: ;
% for N_pixel=[0.5:0.5:5]; test_F_23(N_pixel); end;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
delta_d = D_target; delta_w = 1*pi/6; 
[T_k_p_] = test_F_T_k_p_0(n_point,max_x_c,delta_d,delta_w);
Tlim_k = [-1,+1]; 
...
[X_k_p_] = test_F_X_k_p_5(n_point,max_x_c,delta_d,delta_w,n_svd_r,svd_r_,svd_r_m,svd_r_c,svd_r_w_,svd_r_Jv_,n_svd_d,svd_d_,svd_d_m,svd_d_c,svd_d_w_,svd_d_Jv_,length(tmp_ij_),svd_l_(tmp_ij_),svd_U_d_(:,tmp_ij_),svd_s_(tmp_ij_),svd_V_r_(:,tmp_ij_));
Xlim_k = [-1,+1]; tmp_ = abs(T_k_p_-X_k_p_); Dlim_k = prctile(tmp_(:),95)*[-1,+1];
subplot(2,length(eps_target_),0*length(eps_target_) + ne); 
clim = polarpatch(R_k_p_,W_k_p_,real(X_k_p_),Xlim_k,0,0,1,cra);  title(sprintf('real(T^{svd}_{%d})',length(tmp_ij_))); axis equal;
subplot(2,length(eps_target_),1*length(eps_target_) + ne); 
clim = polarpatch(R_k_p_,W_k_p_,real(T_k_p_-X_k_p_),Dlim_k,0,0,1,cra);  title(sprintf('real(T-T^{svd}_{%d}) ~ 10^{%0.2f}',length(tmp_ij_),log10(max(Dlim_k)))); axis equal;
...
n_w = max(3,ceil(2*pi*n_r_degree)); n_w_ = n_w*ones(n_svd_r,1);
r_quad_weight_ = ( (2*pi/n_w)*ones(n_w,1) * transpose(svd_r_w_) ) .* ((2*pi*R_target).^2/4);
r_w_ = linspace(0,2*pi,n_w+1); r_w_ = r_w_(1:end-1); e_w_ = exp(+i*(transpose(r_w_)-delta_w)); 
delta_w = 1*pi/6; 
...
verbose=1;
n_node_ = [8,16,24,32];
F2_abs__ = zeros(n_svd_d,length(n_node_));
F2_rel__ = zeros(n_svd_d,length(n_node_));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
F2_abs_ = transpose(F2_abs__)*svd_d_w_*2*pi .* ((D_target).^2/4) / (pi*(D_target).^2);
F2_rel_ = transpose(F2_rel__)*svd_d_w_*2*pi .* ((D_target).^2/4) / (pi*(D_target).^2);
%%%%%%%%;
n_figure = n_figure+1; figure(n_figure); clf;
...
save(fname_mat,'n_svd_l_','E2_rel_','n_node_','F2_rel_','N_pixel');
