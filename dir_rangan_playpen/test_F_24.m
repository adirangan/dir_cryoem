function [E2_avg_] = test_F_24(N_pixel,n_node,dt,n_iteration);
...
% try: ;
%{
  E2_avg_ = test_F_24(1.5,21,0.075,32);
  %}
...
eps_target = 1e-1; % tolerance used for svd-expansion. ;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
quad_weight_ = svd_d_w_ .* ((D_target).^2/4);
quad_node_scale_ = svd_d_;
d_r_ = quad_node_scale_;
n_A=0; n_w_ = zeros(n_d_degree,1);
for nq=1:n_d_degree;
tmp = max(3,ceil(2*pi*n_d_degree*quad_node_scale_(nq)/D_target));
n_w_(nq) = tmp;
n_A = n_A + tmp;
...
%n_iteration = 1024*1;
E2_quad_ = zeros(n_iteration,1);
E2_avg_ = zeros(n_iteration,1);
...
if (nnodeA~=nnodeB); H_(nnodeA,nnodeB) = hx_(norm(d)*max_k_p); end;
...
tmp_ = dhx_(hypot_dd_*max_k_p)*max_k_p./hypot_dd_; tmp_(find(hypot_dd_==0)) = 0;
dhx__ = - tmp_ .* ddx_;
dhy__ = - tmp_ .* ddy_;
%%%%%%%%%%%%%%%%;
dE2k_sample___ = zeros(n_A,2,n_node);
...
ft_ = hx_(hypot_dd_*max_k_p); ft_(ij_) = (2*pi)^2;
alpha_= G_*ft_;
E2_sample_ = ((2*pi).^2 - sum(alpha_.*ft_,1)) / (2*pi).^2; % Sampled relative error. ;
E2_quad = abs(E2_sample_)*quad_weight_sample_;
tmp_ = dhx_(hypot_dd_*max_k_p)*max_k_p./hypot_dd_; tmp_(ij_) = 0;
dftx_ = - tmp_.*ddx_;
dfty_ = - tmp_.*ddy_;
dE2x_sample_ = 2*alpha_.*(dhx__*alpha_) - 2*alpha_.*dftx_;
dE2y_sample_ = 2*alpha_.*(dhy__*alpha_) - 2*alpha_.*dfty_;
t_mid = toc(t_start); disp(sprintf(' %% ni %d/%d; time %f',niteration,n_iteration,t_mid));
E2_quad_(niteration) = E2_quad; E2_avg_(niteration) = E2_quad / (pi*D_target.^2);
dE2x_quad_ = dE2x_sample_*quad_weight_sample_;
dE2y_quad_ = dE2y_sample_*quad_weight_sample_;
%%%%%%%%%%%%%%%%;
...
polarpatch_adaptive(d_r_,n_w_,E2_sample_,colormap('hot'),[0,max(E2_sample_)]);
plot(D_target*cos(linspace(0,2*pi,256)),D_target*sin(linspace(0,2*pi,256)),'k-');
plot(delta_node_(1,:),delta_node_(2,:),'p','MarkerSize',15,'MarkerFaceColor','g');
set(gca,'XTick',[],'YTick',[]);axis square;
title(sprintf(' ni %d/%d: Error<=%f',niteration,n_iteration,max(E2_sample_)));
hold off;
subplot(2,2,3); cla; hold on;
polarpatch_adaptive(d_r_,n_w_,log10(E2_sample_),colormap('hot'),log10(max(E2_sample_))+[-3,0]);
plot(D_target*cos(linspace(0,2*pi,256)),D_target*sin(linspace(0,2*pi,256)),'k-');
plot(delta_node_(1,:),delta_node_(2,:),'p','MarkerSize',15,'MarkerFaceColor','g');
set(gca,'XTick',[],'YTick',[]);axis square;
title(sprintf(' ni %d/%d: log10(Error)<=%f',niteration,n_iteration,log10(max(E2_sample_))));
hold off;
subplot(2,2,2); cla; hold on;
plot(1:niteration,E2_avg_(1:niteration),'k.-');ylim([0,max(E2_avg_)]); xlim([1,n_iteration]); xlabel('iteration'); ylabel('Avg Error');
hold off;
subplot(2,2,4); cla; hold on;
plot(1:niteration,log10(abs(E2_avg_(1:niteration))),'k.-');ylim([-9,0]); xlim([1,n_iteration]); xlabel('iteration'); ylabel('log10(Avg Error)');
hold off;
set(gcf,'Position',1+[0,0,1024*1.5,1024]);
drawnow(); t_mid = toc(t_start); t_draw = t_mid;
end;%if flag_disp;
%%%%%%%%%%%%%%%%;
delta_node_ = delta_node_ - dt*transpose([ dE2x_quad_ , dE2y_quad_ ]);
for nnode=1:n_node;
...
save(fname_mat,'E2_avg_','N_pixel','n_node','dt','n_iteration');

fname_fig = sprintf('./dir_svd/E2_avg_N%.2d_n%d',round(10*N_pixel),n_node);
