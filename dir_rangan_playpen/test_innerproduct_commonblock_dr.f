c     gfortran -fcray-pointer -fno-align-commons -w -O2 -o test_innerproduct_commonblock_dr.out test_innerproduct_commonblock_dr.f -I ./dir_gen_Jsvd -lfftw3 ; ./test_innerproduct_commonblock_dr.out 36 10.0 3.0 13 13 12 ;
c     gfortran -fcray-pointer -fno-align-commons -w -O2 -o test_innerproduct_commonblock_dr.out test_innerproduct_commonblock_dr.f -I ./dir_gen_Jsvd -lfftw3 ;
c     ./test_innerproduct_commonblock_dr.out 36 10.0 3.0 13 13 12 ;
c     svn commit dir_rangan_playpen/ --username adirangan --password githubpass0 -m "testing innerproduct"
      program test_innerproduct_commonblock_dr
      implicit none
...
c$$$      list: ncur N_pixels n_delta_x n_delta_y n_gamma
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[entering test_innerproduct_commonblock_dr]'
      end if
...
c$$$      gamma_M = +3.0/n_gridpoints*2.0*pi
      gamma_M = 1.0*pi/4.0;
...
c$$$      Initializing figure file for image
      unitnumber = 7
      write(fname_pre,'(A)') 'test_innerproduct_commonblock'
...
c$$$      translations simultaneously.
     ...
     $     ,M_k_q_,M_k_q_,C_M)
      ...
      C_Z = C_S*C_M
      timing_tic = second()
      call innerproduct_q_delta_gamma_commonblock(0,eps_target,n_delta_x
     $     ,delta_x_,n_delta_y,delta_y_,n_gamma,gamma_,n_gridpoints
     ...
     $        ,timing_toc-timing_tic
      ...
      timing_tic = second()
      call innerproduct_q_delta_gamma_commonblock(1,eps_target,n_delta_x
     $     ,delta_x_,n_delta_y,delta_y_,n_gamma,gamma_,n_gridpoints
     $     ,grid_k_p_,ngridc_,Wlast_,ntemplatesize,S_k_q_,M_k_q_,C_Z
     $     ,Z_k_q_)
      timing_toc = second()
...
c$$$      printing innerproducts
      do ngamma=0,n_gamma-1
         ...
         x_side = 10.0d0
         y_side = 10.0d0
         x_offset = +7.5d0*cos(gamma) + Y_k_q_x*x_side/12.5
         y_offset = +7.5d0*sin(gamma) + Y_k_q_y*y_side/12.5
     ...
     $        ,F_max ,x_offset,y_offset ,x_side,y_side)
         x_offset = +7.5d0*cos(gamma) + Z_k_q_x*x_side/12.5
         y_offset = +7.5d0*sin(gamma) + Z_k_q_y*y_side/12.5
         call Fig_c16_cart2(unitnumber,n_delta_x,delta_x_ ,n_delta_y
...
c$$$      adding labels
     ...
     $        ,y_side)
      ...
      if (verbose.gt.1) then
         ...
         write(6,'(A)') 'try:
     $"gthumb test_innerproduct_commonblock.jpg"'
      end if
c$$$      Printing figure file for image
     ...
     $     ,trim(fname_fig),trim(fname_jpg)
      ...
      if (verbose.gt.0) then
         write(6,'(A)') '[finished test_innerproduct_commonblock_dr]'
      end if
      ...
      include 'innerproduct_q_delta_gamma_commonblock.f'
      include 'hsv2rgb.f'
