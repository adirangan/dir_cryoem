      subroutine transf_q_to_q(l_max,n_J,f_,b_,p_,n_r,grid_p_,n_w_
     $     ,n_A,S_q_,delta_x,delta_y,M_q_)
c$$$      Assumes that M_q_ is the same size and dimensions as S_q_
c$$$      Currently brute force convolution
      implicit none
     ...
     $        'Warning, n_J does not match l_max in transf_q_to_q'
      ...
      do nr=0,n_r-1
         ...
         enddo
      enddo
      call copy_c16(n_A,n_A,Z_q_,0,1,n_A,M_q_,0,1)
      deallocate(g_)
      deallocate(Z_q_)
      end
